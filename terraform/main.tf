...
terraform {
  # [OPS.21] ⛔ Обидві межі мусять дорівнювати ТОМУ, ЩО ПРОГНАНО — доти брехали
  # обидві: верх `>=` заявляв підтримку мажора 2, якого в апстріма не існує
  # (перевірено прогоном по релізах І по git-тегах, нуль `v2*`), а підлога
  # обіцяла десяток мінорів, яких жодна смуга не бачила. Джерело істини для
  # мінора — точний пін кроку `Setup Terraform` у `ci.yml`; сюди його НЕ
  # копіюємо, інакше цей коментар почне брехати з першого ж бампу.
  # Рухаючи пін — рухай і цю межу, і дзеркало в `terraform/akash/main.tf`.
