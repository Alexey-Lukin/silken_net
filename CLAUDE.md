...
**Носій ≠ список команд.** Смуга `Docs` = `ruby scripts/docs_band.rb` (кроки читаються з `docs.yml`, не з голови). **Частина цих команд уже стоїть на `.githooks/pre-push`** — станом на 2026-08-28 це DCO · i18n-трійка · path-gated інструменти · `bin/brakeman` · `bin/rubocop`, тобто вони спрацюють, навіть якщо про них не згадати. ⛔ **Ростер блоків читай у САМОМУ хуку, не тут** — перелік у прозі старіє на кожному його рості (ця клауза вже казала «лише i18n+DCO», коли блоків було п'ять). Поріг — ЦІНА, не кількість: кожен блок несе власне виміряне число, повна сюїта (~1.5 хв) свідомо не їде. Усі три дискримінують exit-кодом; «`check-normalized` завжди 0» — мертве твердження (гем 1.1.2).
