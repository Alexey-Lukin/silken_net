...
- **Мінтинг guard-clauses:** oracle-гілка `verified_by_iotex? && oracle_status_fulfilled?` = PATH 1 (latent — ARCH.53 §🗄️: Chainlink-dispatch = local marker без RPC, callback unwired; замикання PATH 1 відмовлено founder 2026-07-19 — superseded by Merkle-lineage ARCH.12/MRV.1); живий PATH 2 мінтить оптимістично (guard = KYC **бенефіціара** [KYC.1]: `Wallet#kyc_approved_for_minting?` — власна адреса → власний статус, custodial успадковує `organizations.hadron_kyc_status`; per-tx SKIP, не raise; auto-verify на біндингу адреси; чесна L0-custodial + ex-post clawback). `WEB3_STRICT_MODE=="true" || Rails.env.production?` (belt-and-suspenders — Hadron був ЄДИНИЙ flag-only, hardened INF.11 2026-07-10; забутий прапор ≠ fake-KYC mint). 🔑 **Але ця пара = «загартований рантайм», і з OPS.37 вона БІЛЬШЕ НЕ відповідає на «чи можна торкатись справжніх грошей»** — то друга вісь, `WEB3_CHAIN_ENV` ∈ `mainnet`/`testnet` (дзеркальні твердження, не bypass; відсутнє → `mainnet`) у `Web3NetworkGuard` поруч із КЛАСОМ ПРОЦЕСУ. ⚠️ Той клас несуть ДВА kwargʼи, не один (`signer_process:` + `web_process:`, 2026-09-01): ключі скоуплені підписантом, а presence **адрес** — ПОКОНТРАКТНО через три класи (job ⊥ web ⊥ coap), бо `CARBON_COIN_CONTRACT_ADDRESS` має контролер-досяжний тихий read-сайт (`ChainAuditService` ковтає збій під RPC-rescue й рапортує хибне «all clean»). Читати presence адрес як signer-boot явище — саме та напів-правда, яку фікс зняв. Токен `production` ніс обидва значення, а стейджинг потребує першого без другого → `04_02 §Web3NetworkGuard` / `deploy`-скіл → Hadron-стаб + callback-HMAC + W3bstream raise на missing-creds (lazy at-call; IoTeX-fallback + Solana-creds теж prod-regardless) → `05_02` / `web3-pipeline`.
