...
- **Організація запиту = `acting_organization!`, НЕ `current_user.organization`** (SEC.25 Ф2): super_admin працює в контексті ОДНІЄЇ організації за раз і перемикає її (`session[:acting_org_id]`); Pundit дістає `UserContext` (несе лише org — `delegate_missing_to` там fail-OPEN на `present?`); політику звужуй ПАРОЮ `Scope`+предикат. Броадкаст натомість іде в org **власника** ресурсу, не глядача → `04_03 §3.1`.
