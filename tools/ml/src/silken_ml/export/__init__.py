...
def emit_golden(p: dict, X: np.ndarray, idxs) -> str:
...
def build_from_run(run_dir, header_out="firmware/soldier/silken_net_audio_model.h",
    ...
    for c in range(cfg.n_classes):
        ...
        if len(ci):
            idxs += [int(v) for v in rng.choice(ci, min(3, len(ci)), replace=False)]
    idxs = sorted(set(idxs))[:n_golden]

    Path(header_out).write_text(emit_header(p, prov), encoding="utf-8")
    Path(golden_out).write_text(emit_golden(p, x_te, idxs), encoding="utf-8")
    fp = footprint(p)
