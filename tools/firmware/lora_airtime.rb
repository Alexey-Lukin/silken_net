...
# енергосценарій 02_03 §9.8 (Сценарій C) і код
...
when /\Asf-sweep(=\d+)?\z/
  # [ARCH.8] Розгортка ToA по SF при незмінному профілі — ЧИТАЛЬНИЙ режим.
  # ⛔ Нічого не обирає й нічого не пінить: `LORA_PHY_SF` лишається каноном, а
  # цей вихід лише дає моделі сказати те, що вона вміла з народження. Заведено
  # 2026-09-22, бо розвилка ARCH.8 перелічувала ТРИ важелі й не мала жодного
  # приладу під четвертий (SF), хоча формула Semtech параметризована ним.
  # ⛔ ОГОЛОШЕНА СТЕЛЯ, і вона тут вужча за стелю файла: про РЕАЛЬНИЙ запас
  # зв'язку цей режим не знає нічого. Чутливість приймача, шлях поширення й
  # польові умови лежать поза моделлю (02_01 §5.3 — десктопний розрахунок;
  # 02_03 §9.8 прямо каже, що справжній link budget потребує польового виміру).
  # Тож стовпчик «Δ до профілю» є ціною в ЕФІРІ, а не дозволом її заплатити.
  pl = Integer(ARGV.first.split("=", 2)[1] || 30)
  base = toa_ms(pl, spreading_factor: SF)
  puts format("ToA payload'у %d Б за SF (BW %d кГц · CR 4/%d · преамбула %d) — профіль НЕ змінюється",
              pl, BW_HZ / 1000, CR + 4, PREAMBLE)
  puts format("  %-6s %10s %12s %8s", "SF", "T_air, мс", "Δ до SF#{SF}", "LDRO")
  (7..12).each do |sf|
    t = toa_ms(pl, spreading_factor: sf)
    puts format("  SF%-4d %10.1f %+11.1f %8s%s",
                sf, t, t - base, ldro?(sf, BW_HZ) ? "1" : "0", sf == SF ? "   ← профіль" : "")
  end
  puts "  ⛔ Ціна в ефірі ≠ дозвіл її заплатити: запасу зв'язку модель не знає (00_07 ARCH.8)"
else
  warn "невідомий режим #{ARGV.first.inspect}; доступні: (порожньо) · --assert · --check-canon · pl=<байт> · sf-sweep[=<байт>]"
  exit 2
end
