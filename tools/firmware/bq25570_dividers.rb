...
def report
  ...
  # ⚠️ Ціль 5.40 В, НЕ 5.5 — §4.Б прямо каже, що 5.5 недосяжні з гарантованим
  # запасом (guard-craft #113: мутація-доказ є твердженням про ЦЕЙ виклик, не
  # про функцію взагалі — тут звірено окремо, щоб report() і --check-canon
  # рахували ОДНУ й ту саму пару, а не дві правдиві-кожна-по-собі).
  ov1, ov2 = solve_ov(5.40)
  ...
  rows = [
    [ "VBAT_OV  (Eq.2, ×3/2)", "ROV1=#{ov1} ROV2=#{ov2}", ov, "[transitional] 5.40", ov1 + ov2, RSUM_SPEC[:ov] ],
  ...
  puts "   розкидом деталі: стабілітрон 1 % має V_z ±1 %, тобто ≈±54 мВ на 5.4 В, а типовий"
  puts "   5 %-ряд — ±270 мВ; TVS standoff нормується ще грубіше. **Вікно, вужче за розкид"
  puts "   клампа, означає, що питання «TVS чи zener» відповіді не має ЗА ПОБУДОВОЮ** — і"
  puts "   тоді ⚖️ HW.12 є не вибором деталі, а вибором того, чи кламп узагалі можливий."
end
