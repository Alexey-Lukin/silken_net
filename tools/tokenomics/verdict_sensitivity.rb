...
#
# ЧОМУ ВОНА ІСНУЄ. `00_07` SLASH-1 ніс ⚖️: «чи мовчання ≥ порога є підставою вважати
# дерево вибулим для РОЗМІРУ вироку». ✅ **ПРИСУД УХВАЛЕНО 2026-08-27 — НІ, знаменник
# гілки смерті лишається «ліс ДО події»** (три підстави — в самому пункті; ⛔ не
# перевідкривати як «порахувати ще раз»). ⚠️ Шапка казала «несе ВІДКРИТИЙ ⚖️» до
# 2026-09-04, тобто HARD-гейт стверджував, що питання, заради якого він існує, досі
# відкрите — а присуд ухвалили того самого дня, коли інструмент написали.
# 🔑 **Модель від цього не втратила предмета, і це варте рядка:** вона лишається ЄДИНИМ
# носієм ЦІНИ присуду — саме вона дала «0.2% проти 99% реальної шкоди», тобто число,
# яким названо ціну ВІДМОВИ, і будь-який перегляд починається з її прогону.
# Присуд не був заблокований нічим зовнішнім —
# він був заблокований тим, що НІХТО НЕ ЗНАВ ЙОГО ЦІНИ: єдина модель слешингу в дереві
...
#     «вцілілими» й РОЗБАВЛЯЮТЬ вирок — це і є ОБРАНИЙ бік (`00_07` SLASH-1);
...
#     — ⚠️ і цей самий прогін ЗВУЗИВ те застереження: воно справджується лише на
#     ВИРОДЖЕНОМУ краї (при 80 мовчазних зі 101 виключення дає 1.9% проти правди 1.0%).
...
#   • Розподілу мовчання в полі в нас НЕМА, тож `silent_healthy` є ВХОДОМ СЦЕНАРІЮ,
#     а не виміряною величиною. ⚠️ [SILENCE-1, виправлено 2026-09-05] Тут стояло
#     «розкид `delta_t` ×35-190» — помилка КАТЕГОРІЇ: `35.7` і `190` у джерелі є
#     СЕКУНДАМИ L4-моделі, а не множником. Справжній розкид між режимами ≈5×; порога
#     не пускає ІНШЕ — невизначеність МАСШТАБУ (20–101 с проти годинного енергобюджету;
#     SUMMARY L4 після переякорення 2026-09-18, до нього 45–238 с).
#     ⛔ Не «уточнювати» модель під старе число: воно ніколи не існувало.
...
#   • 🔴 `gamma`/`penalty_factor` тут — РОБОЧА ТОЧКА, не джерело правди: обидва
#     DAO-керовані (`SystemParameter` ← `ProtocolParameters.sol`), тож живий голос
#     може їх зрушити, а цей дефолт за ним не піде. Пороги інваріантів підібрані
#     САМЕ під γ=1.3 — після DAO-зміни їх треба переміряти, а не «підкрутити».
#     Дзеркала bounds тут свідомо НЕМА (третій дім рухомих чисел — відмова [E.67]);
#     сусідній supply_stress тримає ту саму форму, тож прецедент, не виняток.
#
# Виклик:
