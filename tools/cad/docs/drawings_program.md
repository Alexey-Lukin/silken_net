...
2. **Simple anchor parts** (**Phase 2**) — Деталь 3 flange **✅ landed** (`Drawing.CathodeFlange` +
   ...
   **Zone-2 sleeve ✅ landed** (`Drawing.Zone2Sleeve` + `Zone2SleeveDxf`) — its manifest had carried a complete `tolerances`+`notes` block with no carrier at all, and it is the one part that goes to a PEEK CNC shop rather than an SLM one. ⛔ **Деталь 4 radome deliberately NOT drawn, and the ground is named — but it is now ONE ground, not two.** The flat crown R5 (⚖️ 2026-09-11) was APPLIED 2026-09-22 once its gate ⚖️ HW.30 settled the pad BESIDE the piezo, joining the flat rim and the internal rim boss (2026-09-14). What still holds the sheet back is the collar leg: it reshapes the socket (socket and collar are two halves of one band), so a sheet issued today would print a socket about to change — draw the radome with the LAST of those changes, never with the first ([`00_07`](../../../docs/00_07_Action_Plan_Tracker.md) HW.33). ⊕ The flange sheet (step 2) now carries the applied single O-ring groove as geometry in both readers, and its depth tolerance as loud absence (`o_ring_groove_depth`, both limits NOT SPECIFIED — the open shop question).
