...
- **Lattice spec callout** (not geometry): porosity — ⛔ **the sheet must NOT print one number**, because canon carries three that are not interchangeable (nominal 65 % · verification 65 ± 2 %, valid only for the CONSTANT branch · factory acceptance 60–70 %), and their relation — RATIFIED 2026-09-17 (`00_07` HW.33): 65 % is the nominal, the factory is judged by 60–70 %, a graded part as a whole — is not a single number; the shipped generator prints the manifest's TARGET and denies it as an acceptance band in the same breath, pinned by `DrawingTests.Anchor_Sheet_Never_Prints_The_Porosity_Target_As_An_Acceptance_Band` — pore period, topology (⚖️ 2026-09-10 → network
...
2. **Simple anchor parts** (**Phase 2**) — Деталь 3 flange **✅ landed** (`Drawing.CathodeFlange` +
   ...
   **Zone-2 sleeve ✅ landed** (`Drawing.Zone2Sleeve` + `Zone2SleeveDxf`) — its manifest had carried a complete `tolerances`+`notes` block with no carrier at all, and it is the one part that goes to a PEEK CNC shop rather than an SLM one. ⛔ **Деталь 4 radome deliberately NOT drawn, and the ground is named:** its cap is still the hemisphere the ratified flat crown R5 rejects (⚖️ 2026-09-11), and the crown waits on ⚖️ HW.30 — the piezo placement decides the internal height it fixes ([`00_07`](../../../docs/00_07_Action_Plan_Tracker.md) HW.33). The flat rim and the internal rim boss ARE applied (2026-09-14), so a sheet issued today would be wrong in exactly one respect, the cap, and print the rejected one. Draw it with the crown, not before. ⊕ The flange sheet (step 2) now carries the applied single O-ring groove as geometry in both readers, and its depth tolerance as loud absence (`o_ring_groove_depth`, both limits NOT SPECIFIED — the open shop question).
3. **CEM `tolerances`/`notes` block** (**Phase 0**) — fits (a µm band: an ISO 286 read today; the band is to be solved from the Lamé window canon asks for — ratified 2026-09-18, `00_07` HW.3, only its inputs open), GD&T datums, surface-finish,
