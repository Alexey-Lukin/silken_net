...
# 2026-09-21: 25526 → 25932 (+406) — БАМП ЗА КОРЕКТНІСТЬ, не за зміст: founder спіймав, що
# індекс досі приміює мертву координату («ти час від часу пишеш 07»), і саме always-read
# поверхня його й навчала. Заголовок `### §07 · IP · партнери` і present-tense роутер
# `§07-кластер` були ЖИВИМИ вказівниками на модуль, розчинений 2026-08-22 (DOC-T.83) —
# критерій узято не свій, а вже ратифікований у `00_04 §Cross-references`: живий вказівник
# правиться, історична проза про «§07-кампанію» НЕ чіпається. Тож +338 з бампу — це мітка
# розчинення з маршрутом (`00_04`·`00_02`·`02_06`, стан `00_07 §00b`), тобто НОСІЙ проти
# рецидиву координати, а не проза. Заплачено ДО бампу, чесно й мало: гачок `dialogue_not_monologue`
# стиснуто до унікального (він був ТРЕТЬОЮ копією преамбули «Narrate substantively» — той самий
# читач, та сама мить, той самий файл) і знято мертву мітку «§07» зі стаба legal-business.
# 🔴 Чому не більше: цього проходу вимірено, що індекс НАСИЧЕНИЙ, а не роздутий — 72 рядки,
# 133 адреси, 90.9% ваги у гачках при 324 B/рядок. Хаб-інлайн двох сплячих reference-рядків
# дав БІЛЬШЕ, ніж зекономив (рядки вже на рівні тригера), тож важіль «витіснення ЗАПИСУ» тут
# вичерпаний, а другий прохід golf'у заборонено виміром 2026-08-09 (шість циклів на 146 B).
# Ціна названа вголос: наступний новий дім мусить платити витісненням, бо запасу немає.
# ⊕ того ж дня, ДРУГИЙ founder-штовх («оці два рядки такі собі»): два ✅-рядки були стисненими
# ЛОГАМИ кампаній, не гачками — процесна дрібниця («9-агентна»), закритий СТАН («ВСІ 3 ланки»)
# і переказ гочі авто-інвокованого скіла `backend`. Переписано в тригери, знято дрейфливе
# «(три закриті хвилі)» → храповик опущено з 25932 до факту 25745. 🔑 Критерій, що вийшов із
# двох штовхів і тепер судить решту: товстий гачок ЛЕГІТИМНИЙ на правило-домі, який читаєш
# щосесії (теги = тригери ДО дії), і НЕЛЕГІТИМНИЙ на закритій кампанії, чиї уроки мають
# другий дім. За ним решта важких рядків (money-path, zsh, local_verify) свідомо ЛИШЕНА.
# 2026-09-21 (ІІ): 25917 →  — ріст за RECALL, і підстава змінилась разом із рамкою.
# Founder виправив мене прямим текстом: «память читаєш ти, я цю поверхню практично не читаю…
# я екземпляри класів показую — не все». Тобто (а) гачок судиться моїм write-time питанням
# «чи є вже дім для цього факту?», а не чиїмось скануванням, і (б) кожен його штовх — ЗРАЗОК
# класу, тож зупинятись на показаних рядках = підміняти периметр класу його вибіркою (я так
# і зробив у попередньому коміті й записав це як свідоме рішення — хибно).
# Свіп КЛАСУ «гачок-ЛОГ замість тригера» до кінця: шість рядків, що описували кампанію ID-ами,
# статусом чи процесом, тепер називають КЛАС, яким файл володіє (Merkle-lineage+archive-anchor ·
# Turbo-тракт = 5 ЛАНОК + вічний токен · друга тема = MUTATION-ТЕСТ · «потьомкінський peril» ·
# META-профіль vilize · провенанс живих гейтів). ⊕ Того ж свіпу знято ДВІ мертві координати в
# мітках: «мікро-кампанії 07» (читалось як модуль 07) і «Vilize-цикл §00–§08».
...
exempt = {
  ...
  # Записи нижче — ІСТОРІЯ кампаній: субʼєкт речення факт («інвест-лексику знято
  # з §1/§4/§5/§10», «модуль 08 осів у §1/§2», «§8 і BIZ.14 дружно описували»),
  # і роботу тоді робили САМЕ за тими адресами. Перенаведення зробило б запис
  # хибним — guard-craft #29. Живі вказівники тих самих файлів перенаведено окремо.
  # 🔴 ПЕРЕСУД 2026-08-23: `log_memory_reform` і частину `vilize_07_08` знято з винятків —
  # перечитування показало, що там субʼєкт НЕ факт, а вказівник («модуль розчинено → ось де
