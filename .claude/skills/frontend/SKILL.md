...
| `app/javascript/controllers/*_controller.js` | The Stimulus controllers (mobile_nav, clipboard, map, sidebar_scroll) — auto-registered via importmap. ⚠️ `sidebar_scroll` (added 2026-09-16) holds the sidebar's OWN `scrollTop` across a Drive visit: the platform cannot: `data-turbo-permanent` is banned AND measured not to help, and `turbo-refresh-scroll: preserve` scrolls `window` while `body` is `overflow-hidden` (`04_04 §7.5`). Under morph the node survives, so the scroll keeps itself — the controller's perimeter is NAVIGATION only. ⚠️ `theme_controller` is **gone** (UI.1): the theme is pure CSS now, so there is no JS in that chain to reach for. ⚠️ `reveal_controller` is **gone too** (UI.3) — it had zero consumers for seven months, and `eagerLoadControllersFrom` ships every file in this directory, so a scaffold here is bundle weight no gate can see. The list above is therefore EXHAUSTIVE, not a sample |
...
43. Скелетон lazy-фрейма займає коробку вмісту лише тоді, коли хром записано ОДИН раз — у ВЛАСНИКА вмісту, не в скелетоні; і `<turbo-frame>` за замовчуванням INLINE
