...
- **Change anchor geometry**: edit `cem/anchor_zone1.*.json` (Ø, rod, period, wallParam — an anchor declares NO bore: `bus_rod_diameter_mm` is the core, `AnchorCem` has no bore slot, and `AnchorTests.Every_Shipped_Anchor_Cem_Declares_Its_Bus_Rod_And_No_Bore` reds on a manifest that omits the rod or carries a bore key, which would otherwise evaporate on parse).
  ...
  sheet-era 1.0 lands network at 50.2 % porous / **≈23 GPa under the MEASURED `C` 0.983 · `n` 2.242
...
- **Measure apparent stiffness** — `dotnet run -- fea <cem> --sweep` (part) · `fea --ladder` (the same lattice as an n-cell cube, the material-scale control) · **`fea --fit` (the Gibson-Ashby `C` and `n` fitted over a `wall_param` sweep — TWO specimens on ONE flag: no cem argument ⇒ the lattice cube, a cem argument ⇒ the shipped annulus, `fea cem/anchor_zone1.pine.json --fit --step-div 10`)**. Reported as a fraction of `E_solid`, because the Zone-1 alloy is bake-off-gated (00_07 HW.24) and the candidates span 80–186 GPa. ⛔ A single row is an UPPER bound (staircase boundary + fully-integrated hexes, both stiff-biased), so never quote one without its sweep; and the shipped run EXCLUDES the monolithic bus rod (`--with-rod` exists, 00_07 HW.51). 🔴 **`C` and `n` are MEASURED since 2026-09-12, not assumed** — 0.983 / 2.242 on the lattice (cube, period/32) and 1.069 / 2.269 on the shipped annulus (period/10) — **so the textbook `n = 2` is retired AXIALLY and the member that was wrong there is the EXPONENT, not `C`.** 🔴 **And WHICH member is wrong is a property of the AXIS: the radial fit of the same part (`--with-radial`, period/10) gives `n` ≈ 1.95 at `C` ≈ 0.54 — on the load-bearing axis the exponent is textbook and the coefficient departs** (`01_01 §5.2`). ⛔ `--with-radial` is REFUSED on a step that does not divide the diameter (an odd divisor on Ø11): the grid gains an empty column and the radial load centre walks a quarter step off the axis (`VoxelFea.RadialLoadCentreOffsetMm`); the axial fit is unaffected and still runs there. ⛔ **Never compare a single-density `C` across geometries: the SAME cube reads 0.71 · 0.80 · 0.77 at 16 · 24 · 32 steps, i.e. the full width of the «cube↔part gap» canon used to call non-physical. Compare FITTED pairs or nothing.** ⛔ **And this verb does not produce what a physical test returns by default:** ISO 13314's comparable quantity is the **elastic gradient** — the secant of an UNLOAD/RELOAD hysteresis loop between 20 % and 70 % of plateau stress — never the quasi-elastic gradient, whose role in the standard is to locate the strain zero; so a printed-specimen order without an unloading cycle comes back with nothing to compare against, and any comparison must name a POINT on the curve. Numbers, the literature check and the declared ceiling → `01_01 §5.2`; the table is machine-held by `ruby scripts/fea_canon_sync.rb` (HARD in `docs.yml`, fourth layer).
