...
11. ✅ **FIXED 2026-08-28 (HW.1) — the four symptoms below are CLOSED in `Drawing.cs`; the CLASS and its
    ...
    s6 +23/+34 on Ø11; Lamé CONSUMES that band to compute a contact pressure, it does not produce one).
...
- **Generate an engineering drawing (`Drawing.cs` / `draw`, SHIPPED Phase 0+1)**: `draw <cem>` emits
  **SVG (human) + DXF via netDxf (factory, opens in AutoCAD/Fusion)** — no PDF (scoped in research, never
  ...
  ground, live in `tools/cad/docs/drawings_program.md §7`. Every kind carries the mirror xUnit set incl.
  the shipped-CEM round-trip and, if published, a gallery pin (⛔ test COUNTS quoted nowhere — the roster
  is `DrawingTests.cs`).
