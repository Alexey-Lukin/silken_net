...
| **Subgraph (The Graph)** | `subgraph/{package.json,package-lock.json}` — `@graphprotocol/graph-cli` + `graph-ts`, обидва пінені ТОЧНО (без `^`), бо саме CLI вирішує, який `specVersion`/`apiVersion` приймається; **плюс блок `overrides`** — його дім тут, бо JSON коментарів не має. ⊕ **Тест-шар мапінгу (`matchstick-as`) — ще НЕ доданий, і ціну переміряно 2026-08-28 [OPS.36]:** `npx graph test` САМ тягне прибудований rust-бінар (LimeChain/matchstick releases, без ручного тулчейну) і доходить до `🆘 No tests have been written yet` — тобто оточення не блокує, блокує відсутність тестів; на Linux-раннері лишається перевірити `libpq` (apt-рядок, не сервіс), є запасний `-d/--docker`. ⚠️ Що НЕ подешевшало: сам `matchstick-as` є новим devDep у ТОМУ дереві, яке присуд [OPS.35] стиснув із 44 алертів до 4 — тож рішення додати його є рішенням розширити дерево, і воно founder'ове | `npm view @graphprotocol/graph-cli dist-tags` (⚠️ `latest` буває = наша версія, а «свіжіше» = лише `alpha`/`rc` → карантин за невідомістю); `npm audit` ⊥ `gh api …/dependabot/alerts` — **дві РІЗНІ бази**, і числа розходяться в рази | `npm ci` → `npx graph codegen` → `npx graph build` (порядок несучий: `build` компілює проти `generated/`) — те саме, що робить `subgraph.yml`. ⚠️ `graph test`/matchstick у дереві **немає** → `00_07` OPS.36 |
