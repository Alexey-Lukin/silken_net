...
| **firmware C** | every git submodule under `firmware/extern/` — ⛔ take the count from `git submodule status`, never from prose: a hardcoded number here silently trims the newest entries off the sweep perimeter, and the newest are `subghz-phy` (radio, board-freeze gated) plus our OWN LoRaWAN fork, whose UB fix must be re-verified on every bump. ⚠️ `stm32wlxx-hal-driver` + `cmsis-device-wl` (+ `subghz-phy`) are ONE STM32CubeWL release — bump them together, never one alone | `gh api repos/<org>/<repo>/releases\|tags` vs `git submodule status`; for the CubeWL trio also `git -C firmware/extern/<sub> tag --contains HEAD` (empty = the pin sits between releases), then match the three tags against one CubeWL release's notes | host CMSIS-parity ctest (local) + `make -C firmware/test`; ARM build — local via `firmware/.toolchain/` (`firmware` gotcha 7.4) and in CI; QEMU parity **CI-only** (qemu not local) |
