...
53. 🔴 **A dismissal whose alert KEY carries a version is a treadmill — the next bump reopens it under a new number, so fix the SOURCE, not the instance** [OPS.22, 2026-09-26]. Trivy keys an image finding by its PATH, and the path contains the gem version: four doc-example JWTs in the SSO-OIDC client documentation that ships inside `aws-sdk-core`, dismissed `false positive` 2026-08-27 (#348–351), closed as «fixed» on the 09-22 SDK bump and reopened on the very same lines as #361–364. The dismissal was right about the finding and useless about the channel — the founder's ground for dismissing («they keep glowing and barking», `06_07 §1a`) was never met. **Reflex: before dismissing, ask whether the alert's key survives the next bump of its package; if it does not, the durable cure is scanner config at the source** (here `.github/trivy-secret.yaml`, an allow-rule on the registry-gem container whose ground holds for EVERY member — `ssot-maintenance` guard-craft #102), **proved by the alerts auto-closing on the next CI scan** — not by the absence of a red. ⚠️ Narrowing a security scanner is a founder verdict, not a delegated one: the auto-mode classifier stopped the first commit as «Security Weaken», and it was right to.

54. 🔴 **A scanner's declared filter can be INERT on one output path — read the wrapper action's entrypoint at the pinned SHA, not its input list** [OPS.22, 2026-09-26]. `image_cve_scan.yml` carried `severity: CRITICAL,HIGH` for three weeks and canon described the posture with it, but trivy-action's `entrypoint.sh` does `unset TRIVY_SEVERITY` for `format: sarif` unless `limit-severities-for-sarif: true` — so the Security tab received every severity. Two consequences, neither visible to any gate: a local `--severity` run measured a NARROWER set than CI (the «6 locally vs 40 in CI» that had been blamed on Trivy-DB snapshots), and the lane the scan exists for (the Go module inside `thruster`, Medium) was visible ONLY because the filter was inert — «fixing» the filter would have silenced it (mutual masking). **Reflex: when a count or a severity looks impossible under a flag, open the action's code at the pinned SHA before theorising about the data.**
