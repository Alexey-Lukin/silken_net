...
8. **Item трекера може брехати про ВЛАСНИЙ артефакт.** Двічі за один аудит §07-пункт описував як відкриту діру те, що вже написане в `protocols/` (тайминг-конфлікт ™-класу «якого brief не обговорює» — brief тримав його цілою секцією; «інженерного дому немає» — дім існував в іншій секції трекера, і той самий трекер це визнавав деінде). Тому **перед плануванням роботи за §07-item'ом ВІДКРИЙ названий артефакт** і перевір, чи діра ще існує: інакше напишеш удруге те, що є, або, гірше, понесеш людині неправду про власну готовність. Дзеркальне: під хибною премісою часто лежить **вужча справжня** діра — не стирай твердження, звузь його.
...
| Per-component BOM Soldier | [`02_01 §3`](../../../docs/02_01_Hardware_Architecture_and_BOM.md) — **не** `07_01 §11` (той агрегує rollup, не per-component spec) |
| Node-rollup $ · cluster CAPEX · ROI-крива | [`07_01 §11.2`](../../../docs/07_01_Nature_as_a_Service_Contracts.md) · `§15.3` · `§17.3` (решта — дзеркала, правити в домі) |
| Виробничі хаби, EU-backup, Frame Agreement | [`07_01 §18`](../../../docs/07_01_Nature_as_a_Service_Contracts.md) |
...
Реєстр — це [`07_03 §1`](../../../docs/07_03_Academic_Integration_and_IP.md), портфель — `§2`. Але **метод** «лишається чи ні» живе тут, бо він операційний.
...
1. **rate-guard — найімовірніший стріл.** Рядок із `10 000 … = 1 SCC` або `2000 SCC = 1 t…` (ловить і кирилицю) валить CI **поза** `05_03`/`07_01`/`00_07`/`manifest`. Лік — **не прибирати число, а поставити дзеркало-маркер у ТОМУ Ж рядку** (`дзеркало` / `mirror` / `05_03` / `07_01` / `SystemParameter` / `ProtocolParameters`). Клієнтський документ **мусить** називати курс — тому це єдиний коректний шлях. ⚠️ Інверсна форма (`1 SCC = 0.5 кг CO₂`) гейт **не ловить** — не спокушайся, One-Home від цього не зникає.
...
3. **Procurement — це ДВА workflow, не один.** R&D-RFQ (spot-quote, academ-co-pub, дім `protocols/procurement/`) ⊥ production-procurement (Frame Agreement, дім [`07_01 §18`](../../../docs/07_01_Nature_as_a_Service_Contracts.md)). І дисципліна реєстру: він індексує **всі** компонент-групи stub-рядками, а повні аркуші авторяться **інкрементально, як компонент реально procure'иться** — не boil-the-ocean.
...
Це **метод**. Сюди не накопичуються: *значення* (курси, ціни, payback) → канон `07_01`/`05_03` · *статуси й рішення* → `00_07 §07` · *профілі партнерів і план публікацій* → `07_03 §1`/`§2` · *історія кампаній і «чому»* → memory. Якщо кортить дописати сюди факт — він належить одному з тих домів, і саме ця дисципліна тут несуча: домен, у якому founder не перевірить мене сам, найменше пробачає впевнено написану неправду.
