...
1. **WEB3_STRICT_MODE=true** — disables Chainlink + Hadron stubs; missing credentials raise **at call-time (lazy, NOT at boot)**. Two nuances: IoTeX SHA256-fallback is banned in ANY production env (`Rails.env.production? || STRICT_MODE` — OR, so prod raises even with STRICT_MODE=false); **Solana creds ALWAYS raise** (`ENV.fetch { raise }`, no stub mode at all) regardless of STRICT_MODE. Dev/test stubs Chainlink/Hadron by default.
