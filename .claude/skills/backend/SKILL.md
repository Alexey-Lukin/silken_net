...
79. Дедуп-ключ, вужчий за множину ПРИЧИН, губить другий вердикт МОВЧКИ — і найдорожче тут не втрата запису, а те, що на цьому ж ключі висить чужий глушник — **знайшовши дедуп-ключ, спитай не «чи не буде дублів», а «чи всі, кого він зливає, стверджують ОДНЕ І ТЕ САМЕ» — і окремо: хто ЩЕ читає цю множину як однорідну**
80. Реєстр, що класифікує КОЛОНКИ, до ВМІСТУ JSONB сліпий за побудовою — і ціну задає не колонка, а КАНАЛ, у який той вміст їде — **побачив JSONB/text-колонку — спитай не «чи вона в реєстрі», а «хто читає її ВМІСТ і куди той вміст їде»; а якщо канал незворотний, ліків ДВА, і обирає між ними одне питання — чи є поле ЄДИНИМ ДЖЕРЕЛОМ свого факту. ТАК → ДЕКЛАРАЦІЯ (оголошений перелік ключів + відмова на межі), бо форма ключа не каже, чи значення персональне — це рішення людини, і гейт має судити його НАЯВНІСТЬ, як робить сусідній PII-реєстр. НІ (поле є переказом сусідніх структурних полів) → ЗНЯТИ поверхню: fail-closed за побудовою, без переліку, який треба вести, і без гарда, що може протухнути**
