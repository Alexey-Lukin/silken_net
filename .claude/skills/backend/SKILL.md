...
71. В OAuth-callback власника вирішує `uid`, НІКОЛИ збіг за поштою — і зворотний порядок не червонить нічого — **коли гем «нормалізує» поле, питай не «чи є значення», а «чим він позначає НЕВАЛІДНЕ» — бібліотека, що фільтрує за тебе, повертає `nil` там, де наївний код чекає рядок**
