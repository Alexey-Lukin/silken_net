...
| committed model header == committed run | `python tools/ml/scripts/check_model_header.py` | conda `silken_ml` (TF — `extract_params` reads the tflite through `tf.lite.Interpreter`) |
...
✅ **Freshness gate shipped 2026-09-10 [FW.4]** — `python tools/ml/scripts/check_model_header.py` (`ml_smoke.yml`, conda) compares the committed `silken_net_audio_model.h` BYTE-FOR-BYTE against a regen from the newest committed registry run: `extract_params` over the committed `model_int8.tflite` for the arrays, `reproducibility.json` + `export_report.json` for the provenance line, their `run_id`s cross-checked. Mutation-verified on the header arm. 🔒 **The GOLDEN stayed out of scope, and that is a measured boundary rather than an omission:** `emit_golden` needs the held-out vectors from `parity_test.npz`, and `tools/ml/.gitignore` excludes `*.npz` on purpose — the input does not exist in a CI checkout at all (learned by reddening CI with the first version, which had been scoped off `ls` instead of `git ls-files`). **So re-emitting is STILL a two-file act — the `.h` now has a machine double, its goldens do not, and only the host test that consumes them stands behind those.** Declared ceilings → `00_06 §3` row «model header freshness»; state → `00_07` FW.4.
...
- 🔴 **`environment.yml` installs from CONDA, so a floor verified against a PyPI release is safe only once conda-forge serves that version — and «it doesn't» must be asked of the CHANNEL, never of a local resolver's cached index** (re-measured 2026-09-13). The 2026-08-24 rollback of `librosa>=1.0` stood on a stale local index: conda-forge had served 1.0.0 since 2026-08-11. Nor was the lane ever green on an old env: `ml_smoke`'s `setup-micromamba` keys its cache on the spec file's sha256 with no restore-keys, so every floor edit resolves cold, and `ML passed` has run on `librosa 1.0.0` since that very rollback. The floor is `>=1.0` since 2026-09-13. Reflex whenever you touch a floor here: `mamba create -f tools/ml/environment.yml --dry-run` against the committed spec, and check any NEGATIVE against `api.anaconda.org/package/conda-forge/<pkg>` plus the first CI resolve log.
