...
- **Stimulus-мінімалізм (ADR-CDX-8):** рівно 1 контролер (`codex--comment`); решта — нативний `data-turbo-confirm`. ⚠️ **Живості немає ЗОВСІМ — і мова тут точна, бо була неточною.** Attunement-counter · live-comments · citation-pills не «шлють сирий ActionCable без consumer'а» (так було до 2026-07-27) — вони **не шлють нічого**: усі 8 сирих `ActionCable.server.broadcast` знято разом із `AttunementBroadcastWorker`, і заборону тримає `spec/security/no_raw_action_cable_spec.rb`. Компоненти лишились зібраними, DOM-id на місці, продюсера немає. ⚠️ **`Discoveries::Toast` із цього переліку ВИПАВ 2026-08-03 — його знесено цілком** (компонент, спека, i18n-ключі, Stimulus-пара `codex--reveal`), і підстава не «мертвий», а **«НЕ готовий, попри заяву сусіда»**: ані `id`, ані `turbo_stream_from` у ньому не було, він інтерполював англійський заголовок повз `Codex::Node#title(locale)`, а його auto-dismiss суперечив присуду платформи (WCAG 2.2.1). Не відбудовуй — спростування живе в [`00_07` UI.2](00_07_Action_Plan_Tracker). Робочий патерн живості = `turbo_stream_from` + `Turbo::StreamsChannel.broadcast_*_to`, і новий стрім одразу винен доказ скоупу (`spec/security/turbo_stream_scope_spec.rb` → скіл `frontend` #9). ✅ `ApplicationCable::Connection` **шипнуто 2026-07-28** (SEC.25 Ф1) — сокет тепер ідентифікований (`identified_by :current_user, :session_id`, salt-bound cookie), анонімний відкидається, `remote_connections…disconnect` живий per-device. ⚠️ Але це ІДЕНТИЧНІСТЬ, не авторизація підписки: власного каналу немає й свідомо не буде (клас каналу вибирає клієнт, а `reject` бʼє легітимного глядача — [`04_04 §8.1`](04_04_Phlex_UI_and_Tailwind)). Тож дротуючи живість Codex, не розраховуй, що сокет перевірить скоуп за тебе — його тримає підписане імʼя стріму. Wire-vs-descope = відкрите ⚖️ у [`00_07` UI.2](00_07_Action_Plan_Tracker).
...
| Stimulus | `app/javascript/controllers/codex/` (comment) |
