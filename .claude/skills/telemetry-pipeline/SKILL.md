...
6. **Partition-aware lookups** — `TelemetryLog` is RANGE-partitioned by `created_at` (monthly leaves; +1 every month, no retention/detach exists, and `telemetry_logs_default` is **never** pruned — measured by EXPLAIN, so it is a permanent extra probe). One-Home is **`TelemetryLog.partition_pruned(created_at_iso, metric_caller:)`** — a *chainable scope* carrying a 1-second window plus a degraded-path counter. It is **NOT** `find_with_partition_pruning`: that one lives on `BlockchainTransaction`, is a *finder*, and has no counter (asymmetry is a live residual → `00_07` PERF.1). `GatewayTelemetryLog` and `Codex::Match` are partitioned too and have **no helper at all**, so the [S6.16] invariant covers half the family. 🔴 **Never hand-roll the narrowing.** Exact equality on `created_at` misses by construction — ISO-8601 carries second precision, the column microsecond — and that form has now shipped three times (`MintingRollbackService` rollback silently became a no-op; three mint-path workers, 2026-08-07). On the mint path the miss is **silent**: `return unless log` makes the job succeed having done nothing — no retry, no DeadSet, no alert. Canon: `04_01` ⚡ PARTITION PRUNING INVARIANT [S6.16].
