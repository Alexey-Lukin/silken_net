...
1. 🔴 **Redis має два доми за слотом — production = Upstash, canopy = Kamal-accessory на app-хості — і Free-межа Upstash рахує КОМАНДИ, тож її зʼїдає навіть порожній Sidekiq.** Production = Upstash (ОДИН інстанс, регіон обирається ПРИ СТВОРЕННІ й ніколи потім; числа тарифу, вимір і `/ready` → 503 на Free — [`06_01`](../../../docs/06_01_Deployment_Kamal_Terraform.md) §Redis Isolation, `00_07` INF.28 §🗄️) ⊥ canopy = self-hosted Kamal-ACCESSORY на app-хості (⚖️ 09-03, `config/deploy.canopy.yml` `accessories.redis`: `requirepass` із того самого `REDIS_URL`, один секрет — один дім; бутиться кроком `Kamal Deploy to Canopy` ПЕРЕД ролями з `-d canopy`). 🔑 Accessory-імена НЕ несуть destination (механізм — #8), тож canopy-only accessory бутиться ЛИШЕ з `-d canopy`, а спільний Alloy — ЛИШЕ без нього (піни — `alloy_scrape_topology_spec`); підстава відмови `options.publish` (rolling app-роль) на accessory не переноситься — він не роллиться. Multi-zone Global DB — НЕ план, а ескалація з названим тригером: повторюваний день-у-день `sn-alert-m2m-nonce-fallback`/`-qatt-` на production (разовий blip = ні). Дім тригера й критерію — [`06_01`](../../../docs/06_01_Deployment_Kamal_Terraform.md) рядок Upstash у §«Хто може вимкнути НАС» + [`04_03 §5.15`](../../../docs/04_03_REST_API_v1_Reference.md); провенанс — `00_07` S6.1 §🗄️. Той, хто його закриє, читає ЦЕЙ скіл, а не §04 — тому вказівник стоїть тут.
2. 🧰 **`gcloud` — передумова ШЕСТИ кроків дня, а не першого: Фаза −1 перелічує АКАУНТИ, а рунбук говорить із CLI.**
  Шість залежних кроків — [`06_01 §DEPLOY-DAY`](../../../docs/06_01_Deployment_Kamal_Terraform.md) Фаза −1а. 🔑 **І логінів ДВА, вони НЕ дубль:**
  `gcloud auth login` кладе user-credentials (їх уживає сам gcloud), `gcloud auth
  application-default login` кладе **ADC** — і terraform читає ЛИШЕ другий (`bootstrap.sh` пінить обидва окремо). Перелік тулчейну (+ ланки з ВЛАСНИМ: `solana`/
...
3. ⛔ **`§Quickstart` у `06_01` БІЛЬШЕ НЕ ІСНУЄ — не шли туди нікого** (⚖️ 2026-08-31). Це була
  ТРЕТЯ копія процедури, і саме вона протухла, при цьому читалась першою (розбір — `06_01`, секція «Перший деплой»).
  **Єдиний носій — `§DEPLOY-DAY`**; `DEPLOY-1` несе лише СТАН фаз.
...
6. 🔴 **`deletion_protection` — ДВА РІЗНІ ЗАХИСТИ ПІД ОДНИМ СЛОВОМ, і на Cloud SQL стояв лише
  ...
  виклик. ⊥ **На `google_compute_instance` слово те саме, а механізм ІНШИЙ:** там
  ...
  а «скільки їх під цим іменем і на який із них я зараз дивлюсь».** → [`06_06 §2`](../../../docs/06_06_Disaster_Recovery_and_Backup.md) (рядок `deletion_protection`).
7. **Observability = Alloy → Grafana Cloud SaaS; self-hosted Prometheus НЕ потрібен (OBS.1).**
  Реєстр **in-process** → web:80 НЕ бачить job/coap-інкрементів напряму → Alloy МУСИТЬ скрейпити
  **по одному таргету на процес і слот** (лейбл `process`; canopy — зі `slot` на таргеті) — цей КОНТРАКТ
  незмінний. ⚖️ Адресація — per-role `network-alias` у спільній docker-мережі `kamal` [OPS.37]; canopy —
  під ВЛАСНИМИ `canopy-*` аліасами, дизʼюнктними з production. ⛔ Відхилено з виміром: `options.publish`
  (зламав роллінг) ⊥ docker-socket прямий чи через socket-proxy (inspect із `Config.Env` = env-read
  money-квінтета job-ролі). Топологія, аліаси й пускач accessory — [`06_03 §2.9`](../../../docs/06_03_Prometheus_Observability.md);
  пін — `spec/deploy/alloy_scrape_topology_spec.rb`; зміна `config.alloy` → свідомий reboot через dispatch-вхід
  `gh workflow run deploy.yml -f alloy=reboot` (локальний `kamal accessory reboot alloy` з ноутбука падає
  на posix-акаунті SA).
8. 🔴 **ALLOY-КОНТЕЙНЕР ОДИН НА ДВА СЛОТИ — тож `accessory boot alloy` НІКОЛИ не беруть із
  `-d <destination>` (canopy-only accessory — навпаки, ЛИШЕ з `-d canopy`, #1)** (⚖️ 2026-08-31; звірено з джерелом kamal 2.12, не виведено).
  ...
  ⛔ canopy не перевизначає `accessories.alloy`: мітка canopy живе на ТАРГЕТАХ `config.alloy`, тож будь-яке Alloy-значення слоту лише перемітило б продові серії. ⛔ Не «полагодити» це
...
10. 🔴 **Що метрика ОЗНАЧАЄ, вирішує її СПОЖИВАЧ, а не докстрінг** (INF.26, 2026-08-13).
  ...
  **Рефлекс: перш ніж судити метрику, знайди її `expr` у `deploy/grafana/` і прочитай ЗАГОЛОВОК
  ...
  (`[<ID>; diagnostic tier: <подія дротування>]` у докстрінгу; три гілки, межа «паритет, не законність»
  і виміряні інстанси — [`06_03 §2.8`](../../../docs/06_03_Prometheus_Observability.md) 🎚️, носій
  `spec/quality/metric_registry_doc_sync_spec.rb`). ⛔ На грошовому, слешинговому, MRV-доказовому чи
  безпековому шляху «діагностична» УЗАКОНЮЄ діру — там метрика дістає споживача.
  🔑 **Дротування метрики без споживача дає ТИШІ ГОЛОС** (`sn-alert-treasury-check-errors`):
...
11. 🔴 **Перш ніж лагодити підозрілу метрику, спитай не «чи форма підозріла», а «що ця величина
  ...
  ⊕ **Четвертий рід, найтихіший: величина ПРАВИЛЬНА, а лічильник
...
12. 🔔 **Розбираючи «алертів туча» — спершу РОЗДІЛИ стан і сповіщення, і ЗМІРЯЙ, а не вір відчуттю** (виміряно 2026-09-05).
  ...
  🔴 **⛔ АЛЕ ЛИШЕ ДЛЯ ДОБОВОГО ВОРКЕРА** (переміряно 2026-09-06): фільтр робить правило сліпим до «писач не біг
  ЖОДНОГО разу після рестарту». **Дискримінатор — каденс крону проти `for`, не копія сусіда**; механізм і
  ратчет `grafana_alerts_spec` (додати `> 0` = НАЗВАТИ каденс) — `deploy §Gotchas #27`.
...
15. 🔴 **«TLS термінує Cloudflare» — правда про КЛІЄНТСЬКЕ плече й пастка про друге [INF.4, виміряно 2026-08-30].**
  ...
  **валідного сертифіката НА ORIGIN**. Origin його не має: HAProxy анкера — `mode tcp` на 80 і
  ...
  не шляхи. Ланцюг пари (`.kamal/secrets-common` + `env:` обох воркфлоу) записано над `proxy.ssl` у
  `config/deploy.yml`; енфорсить `env_fetch_declaration_spec`.
...
17. 🎰 **Інтейк: анкер ОДИН, демон ОДИН, слот = три рядки `coap.env` + `systemctl restart coap-daemon`
  ...
  Приймальний рядок демона — `Listening on coap://0.0.0.0:5683 slot=<слот>`; доказ слоту — рядок у БД ТОГО слоту, не лог. Симулятор
...
20. **Секрети One-Home: канонічний дім — `config/deploy.yml env.secret`; повний
  ...
  ланцюг:** kamal-секретами є й `registry.password`, `proxy.ssl.{certificate_pem,private_key_pem}`,
  `servers.<role>.env.secret` і `accessories.*.env.secret` — кожен потребує `$VAR` у `secrets-common` +
  ключа в `env:` обох воркфлоу; повний фан-аут поверхонь — #23.
21. **SSH на ОБИДВІ машини = IAP-тунель + OS Login, keyless (INF.20 (в)).** Порт 22 в інтернет
  ...
  заради `allow_iap_ssh`. ⚠️ Тут висить Kamal-нога (б)-клею, і ходів у ній **ПʼЯТЬ**: (1)–(4) виписано над ключем `ssh:` у
  `config/deploy.yml`, (5) — крок «Register an ephemeral SSH key with OS Login» обох deploy-воркфлоу. ✅ **УСІ ПʼЯТЬ ХОДІВ ДОВЕДЕНІ ЖИВИМ `kamal deploy` 2026-09-01; 2026-09-02 доведено й сам БУТ, холодний `db:prepare` на трьох базах і шлях крізь Cloudflare (`00_07` DEPLOY-1)** (серія прогонів `Deploy · Canopy`; розбір — [`00_07`](../../../docs/00_07_Action_Plan_Tracker.md) §🗄️ `INF.20`). 🔑 **Пʼятий не називав ЖОДЕН дім: реєстрація ефемерного SSH-ключа в OS Login.** Той пускає за ключем в АКАУНТ-ПРОФІЛІ, не в metadata; `gcloud compute ssh` мінтить і реєструє його сам, а kamal іде звичайним ssh крізь ProxyCommand і не несе нічого. ⛔ `gcloud compute os-login ssh-keys add` під WIF **не працює** — CLI виводить адресата з креденшела, а федеративний його не має (`Request for user [None]`); лік = REST `oslogin.googleapis.com/v1/users/<SA_EMAIL>:importSshPublicKey`, і адресат саме **email**, бо числовий `unique_id` дає 403, ПОПРИ те що вірний. 🔴 **Дві різні ідентичності того самого SA у двох сусідніх викликах:** `ssh.user` = `sa_<unique_id>`, адресат API = email. ⊕ І ще два кроки, без яких ланцюг не доходить: мережа `kamal` не існує (її створює `kamal setup`, якого ми свідомо не ганяємо), а `Reboot Kamal Proxy` потребує ВЛАСНОГО `env:` із `GCP_ARTIFACT_REGISTRY_KEY` — він тягне образ проксі з приватного AR, і без пароля `docker login` іде з порожнім `-p` (exit 125). ⚠️ Кроки CI **не успадковують** `env:` один одного, а сусідній блок виглядає як відповідь. 🔑 **«Задротовано» ≠ «працює» — хід доводить лише прогін під самим SA.** (4) `roles/iap.tunnelResourceAccessor` **на deploy-SA**
  ...
  (1)+(2): `ssh.user` = `sa_<unique_id>` + `proxy_command` через `start-iap-tunnel`. ⚠️ **Імя ДЕРИВОВАНЕ,
  ...
  обидва боки деривують із `unique_id`.
...
23. 🔴 **Додав boot-гард на ENV — мусиш пройти фан-аут поверхонь нижче і ДВА процеси, інакше ти
  щойно зробив деплой неможливим** (2026-08-14; ⛔ НЕ цитуй сюди tracker-ID: цей наратив стояв під `ARCH.60`, а той пункт 2026-08-21 поглинув інші й тепер цілком про доставку сповіщень — ID резолвився, тож жоден гейт не почервонів. Механізм живе в каноні: `03_05 §3.1` (3.1а) boot-guard, `03_06 §5` fail-closed, `04_02 §Security::WeakKeyDetector` інвокери, `06_04 §5.7` coap-виняток). Поверхні: `config/deploy.yml` **env.clear**
  ...
  env-блок (`servers.{web,job,coap}` + `accessories.alloy` у `config/deploy.yml`, `accessories.redis` у `config/deploy.canopy.yml`) і його
  ...
  `config/environments/*.rb` або з дефолтом, для нього **не існує**.
  🔴 **Процеси — окрема вісь, і саме там ламається тихо:** `after_initialize` біжить у КОЖНОМУ,
  хто вантажить середовище (canopy-job із власними testnet-двійниками — #28), а `coap_listener` =
  UDP-клей, що вантажить усі ініціалізатори, — вимагав би можливостей, яких не має. Платформа має **ТРИ** зразки звільнення — видати змінні
  ...
  canopy вона була єдиною до 2026-09-03 (слот скрейпиться з 2026-09-03 — `06_03 §2.9`).
24. **Money/signing-п'ятірка = JOB-ONLY: ключі підпису живуть лише в job-ролі, ніколи в глобальному `env.secret`** (`ORACLE_MINTER/SLASHER/CELO` + `ETHEREUM_ANCHOR` +
  ...
  (`web:` у `SILENT_ADDRESS_ENVS`), тож web-контейнер вимагає `web: true`-членів `SILENT_ADDRESS_ENVS`/`SILENT_RPC_ENVS`.
...
25. **SEC.22 latch: at-rest ≠ runtime — провайдер читає `/proc/environ`, тож жоден секрет не сміє
  жити лише за `RAILS_MASTER_KEY`-vault у runtime.** credentials→ENV (сервіси + `storage.yml` — перелік `06_04 §5.7`);
...
26. **Secrets-at-rest = ISOLATED KMS-keyring'и з key-level IAM, НЕ generic keyring (merge-trap); які живі, а які лише спроєктовані — таблиця `06_04 §5.6`, не число тут.**
  Живі — `silken-disk-ew1` (boot-disk CMEK) і `silken-tfstate-ew1` (bootstrap-owned); спроєктовані —
  `silken-sign-ew1` (money-signing SEC.17; у `terraform/kms.tf` — `count` за `enable_oracle_signing_keys`,
  дефолт `false`) і `silken-mac-ew1` (`provisioning-mac` для crown-jewel `PROVISIONING`: SEC.22, тір 2 у `03_06 §5`). ⚠️ Money-квінтет лишається plaintext у deploy-ENV до АКТИВАЦІЇ `SEC.17` (`KmsSigner` ✅ код 09-03; provision KMS + `ORACLE_*_KMS_KEY_VERSION` — 👤 pre-mainnet, `06_04 §5.5` крок 3) — поточний масштаб цієї
...
27. **Deploy/release ланцюг: Canopy = continuous push у `main` після CI; Production = GitHub Release**
  ...
  (required-контексти — `:required` у `scripts/workflow_gate_perimeter.rb`; прямий push власника їх обходить). Діаграма/гейти → `06_07 §1`/`§2`.
