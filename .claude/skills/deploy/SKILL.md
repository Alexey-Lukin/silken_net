...
24. 🔴 **«Тривога, що звучить ЗАВЖДИ, не звучить ніколи» — і на демо-слоті це не метафора, а щоденний механізм** [S2.4/E.64, тричі за один день 2026-09-05]. `bin/forest_simulator` бере `bio_status = rand(0..3)`, а DCI питає, чи збігся заявлений стан із серверним Z — розбіжність гарантована, тож КОЖЕН демо-прогін запалює критичний `Telemetry fraud detected`, істинно-позитивний для механізму: «шахраєм» є наш власний симулятор. Так само вічно-червоними стають `Queen offline` і `Silent trees` після ре-сіду (у базі є вузли, передавати нікому). ⚠️ **Питання при кожному firing не «чи горить», а «ХТО саме це стверджує і чи є в нього вимірювач»** — і якщо канал не може стати зеленим на цьому слоті за побудовою, він уже не алерт, а фон. Той самий закон ратифіковано для production-silence (silence з ДАТОЮ, не з умовою) і для DeadSet.
