...
| **Resilience** — Queen failover (4 рівні) + Per-Chain Fallback Matrix (⚠️ **у КОДІ; у проді порожня** — жодне фолбек-імʼя не доїжджає в контейнер, тож `ResilientClient` не інстанціюється взагалі: `ARCH.114` 2026-09-01) + **топологія черг Sidekiq** (`§2.5` — anti-starvation через ізоляцію ПРОЦЕСІВ, не перестановку черг; flip = `sidekiq -q`-прапори в deploy-конфізі, тобто deploy-рішення, НЕ код) | `06_08` |
...
- 🔴 **«TLS термінує Cloudflare» — правда про КЛІЄНТСЬКЕ плече й пастка про друге [INF.4, виміряно 2026-08-30].**
  ...
  **валідного сертифіката НА ORIGIN**. Origin його не має: HAProxy анкера — `mode tcp` на 80 і
  ...
  маніфестах → перший же запит крізь CF дав би **521/525**. ✅ **Діру закрито 2026-08-31:**
