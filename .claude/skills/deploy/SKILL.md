...
9. 🔴 **Живі прогони `import.rb` знайшли вже ЧОТИРИ дефекти, яких `--dry-run`/`--verify` не бачать ЗА ПОБУДОВОЮ** — вони судять ФОРМУ артефактів, а всі чотири були про ФІТ із реальним API. Четвертий (2026-08-30) найпідступніший, бо його не бачить навіть УСПІШНИЙ імпорт: **Ruler API ЗБЕРІГАЄ правило, чий SSE-вираз не парситься** — валідація запису слабша за evaluation, тож 57 правил сіли зеленими, `--verify` чесно підтвердив наявність, а перший живий тік дав `[FIRING:57] DatasourceError` в щойно задротований contact point. Корінь: `type: threshold`-model без ключа `expression: "A"` (classic-форма `conditions.query.params` його не заміняє). **Рефлекс: після імпорту дивись не на успіх POST, а на стан правил через хвилину (`alerting/list` — хоч одне має бути Normal/Pending, не Error).** Решта три (2026-08-29):
