...
- **A missing closing ` ``` ` fence silently desyncs EVERY fence-aware guard** (the `in_fence` toggle runs in ~9 `DocsLinter` methods) + truncates the ToC — one unclosed fence disables them all at once. Scan for balanced fences after editing a doc with code blocks (a parity gate is not yet built — tracked as `00_07` DOC-T.45).
