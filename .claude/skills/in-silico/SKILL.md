...
- **Cache co-write hazard** — two scripts can write ONE cache file (`21b_dft_os_bpy_full` plain + `21f_dft_os_dimethyl` B5 → both write `os_complex.json`, run-order-dependent → content = whoever ran last). Re-running the "wrong" one silently reverts the cascade. Grep `scripts/*.py` for duplicate `DFT_CACHE / "X.json"` out-paths; a cache fed downstream (22/comparison) should have ONE owner. [flagged → 00_07 OPEN, 2026-06-19]
