...
| `docs/00_02_Academic_Integration_and_IP.md` §1 | Partner registry: Мінаєв = spin-forbidden O₂-activation kinetics, NOT DFT-redox (that is self-owned; Стаття 1 goes WITHOUT an external collaborator — ⚖️ founder 2026-09-23, explicit-water QM/MM is its named limit; a collaborator is a follow-up option only) · in-vitro EBFC enzymology = profile biochemist/electrochemist, TBD (ЧМА = toxicology only) · xylem-sap composition = ЧНУ bio hub |
...
- **An HTML primary hides the same trap one layer deeper: an inline FORMULA can be an IMAGE, and the text layer drops it without a trace.** PMC553296 (Gray & Winkler 2005) prints «self-exchange in [image] is ≈2.4 eV»; stripped of tags it read as a sentence about aqueous Cu(II/I), and was carried that way for a day — the image says Cu(phen)₂²⁺/⁺. When a species, unit or symbol sits exactly where the text layer goes blank, fetch the image (`<img … alt="Inline graphic">`) and read it before the number gets a role.
