...
system_params = [
  ...
  { key: "slash_evidence_validity_hours", value: "168", value_type: "integer", category: "alerts",
    min_value: 1, max_value: 8760,
    description: "[SLASH-1] Строк дії доказу Кат-A від його запису. Ворота не знають, ЯКИЙ інцидент довів доказ, тож без строку він відчиняв би незворотний слеш на будь-якому пізнішому, не повʼязаному тригері. Рецепт 06_08 §4.6 проганяє вирок інциденту одразу після ескалації." },
...
# Локально: рядок `SEED_EMAIL_BASE=…` у `.env`. На canopy — НЕ змінна деплою:
# сід там запускають руками й нечасто, тож база передається В МОМЕНТ ДІЇ
# (`docker exec -e SEED_EMAIL_BASE=… … bin/rails db:seed`). Постійна змінна
# означала б або адресу в публічному дереві, або секрет-слот під демо-дані;
# обидва дорожчі за один прапорець у команді, яку однаково набирає людина.
#
...
naas_contract = NaasContract.create!(
  ...
  status: :active,
  # ⛔ [BIZ.22] Сід не вмикає того, чого продукт не обіцяє — це стосується ОБОХ умов
  # дострокового виходу. `burn_accrued_points` лишається `false` (⚖️ 2026-09-10): прапор є
  # ОПЦІЄЮ контракту, і `ContractTerminationService` ставить burn у чергу лише за ним;
  # увімкнувши тут «реалістичніші» демо-дані, ти робиш форфейтуру видимою поведінкою
  # продукту рівно тоді, коли `msa_skeleton §B.6.4` її забороняє як звичайний
  # termination-remedy. `early_exit_fee_percent` НЕ ставиться взагалі (⚖️ делеговано
  # 2026-09-22): Опція 1 MSA (`msa_skeleton §B.6.3`, ⚖️ 2026-08-29) каже «early-exit-fee
  # немає взагалі», і `00_04 §3` стверджує це канонічно — тож вписаний тут відсоток
  # рендерився б Клієнтові (`contracts/show`) як чинна умова продукту, якої продукт не має.
  # ⚠️ Поверхню НЕ чіпати: рендер умовний і показує рівно те, що в даних, тож дефект був у
  # ПИСАЧІ, а не в екрані — а зняття рядка з поверхні дало б гірше (умова в даних, невидима
  # Клієнтові). Без ключа `show` друкує «—» = «умови не задано», що [ARCH.84] свідомо
  # відрізняє від «0%» = «розірвання безкоштовне».
  cancellation_terms: { "burn_accrued_points" => false, "min_days_before_exit" => 30 }
)
...
# 🌲 [E.64 · ⚖️ founder 2026-09-05] Порода береться НЕ рівноімовірно, а за ВИМІРЯНОЮ
# часткою з відкритого набору `data.gov.ua` (санітарні рубки Черкащини, 4 347 виділів
# — `tools/ground_truth/cherkasy_sanitary.csv`). Частки в наборі: сосна 55.4 % ·
# дуб 24.8 % · ясен 6.4 % · граб 3.9 % · акація 3.9 % · липа 2.5 % · клен 1.3 % ·
# береза 0.9 %. Ми маємо ДВІ родини, тож беремо їхню ВІДНОСНУ пропорцію:
# 55.4/(55.4+24.8) ≈ 0.69 сосни.
# ⚠️ Частка реалізується СТОХАСТИЧНО (`rand < PINE_SHARE_IN_MIX` на кожне дерево), тож
# фактичний зріз гуляє: на n=100 виміряно 77 % сосни — це ≈1.7σ, тобто НОРМА, а не
# розбіжність із константою. ⛔ Не «виправляти» 0.69 під побачений зріз: біноміальний
# розкид на сотні дерев ±~4.6 п.п. за σ, і підгін під один прогін зафіксував би шум.
# ⛔ Решту порід НЕ заводити «для реалізму»: кожна нова `TreeFamily` потребує
# per-species Z-порогів, а ті сьогодні є ЗАПОВНЮВАЧЕМ (⚖️ founder: «наврядчи можна
# подумати, що я правильно заніс те, чого не знаю») — вигадати їх означало б
# відтворити рівно ту мітку без вимірювача, яку E.64 знімав тричі.
# ⛔ І таксацію з набору сюди НЕ заводити — ані колонками, ані міграцією. Набір несе
# медіани виділу (площа · вік · висота · діаметр · запас · повнота — числа й популяція
# в `tools/ground_truth/README.md`), і в `trees` таких колонок немає ЖОДНОЇ. Додати
# схему заради красивішого демо — це процесний шар (У-ВЕЙ): нова поверхня, яку треба
# сіяти, мігрувати, рендерити й стерегти, заради даних, яких вузол не міряє. ⊕ Зерно
# набору — ВИДІЛ (десятки-сотні дерев), тож per-tree вік із нього все одно вигаданий.
# ⛔ І назв лісництв із набору тут теж НЕ вживати: кластер належить організації з
# ДОГОВОРОМ, тож реальні імена створили б договірні відносини, яких не існує.
# Беремо з набору ФОРМУ, не імена (⚖️ founder, той самий день).
# ⚫ [E.64 · ⚖️ founder 2026-09-05] МАСШТАБ ДЕМО — НЕ ДОСИПАТИ, і це проти інтуїції.
# Первісна думка була «до 30 найбільших лісництв»; вимір її зняв: демо коштує своїми
# СЮЖЕТАМИ (мовчунка 73 год · пилка · посуха · аудит), а не масштабом, тож 25 порожніх
# кластерів РОЗЧИНИЛИ Б їх у наповнювачі — база стала б більшою й МЕНШ змістовною.
# ⊕ Якщо масштаб таки знадобиться (напр. навантажувальний вигляд мапи) — ЛИШЕ під ТІЄЮ
# САМОЮ організацією: лісгосп справді має багато кластерів, а нові організації = нові
# договірні відносини, яких не існує.
# ⏳ Тригер перегляду — перша демонстрація, де забракне саме масштабу; оголошує founder.
# 🔴 Присуд стоїть ТУТ, бо спокуса досипати виникає саме в цьому файлі, а не в трекері.
PINE_SHARE_IN_MIX = 0.69
100.times do |i|
  gateway = gateways.sample
  family = rand < PINE_SHARE_IN_MIX ? pine : oak
  ...
  #   homeostasis — z ≥ `critical_z_min` І z ≤ ρ-ВІДНОСНОЇ стелі,
  #   anomaly     — z вище тієї стелі.
  # 🔴 [E.64] Стеля НЕ дорівнює `critical_z_max`: `SilkenNet::Attractor.anomaly_ceiling`
  # дає ρ(temp) + (max − BASE_RHO). ⚠️ І вона ПЕР-РОДИННА, а цикл сіє ДВІ родини —
  # називати одне число тут означало б повторити ту саму помилку, що й «медіани без
  # популяції»: при 41 °C сосна (5/45) дає 36.2 + 17 = **53.2**, дуб (8/40) — **48.2**.
  # Тут стоїть 55.0 — вище ОБОХ, саме щоб пара (`z_value`, `bio_status`) сходилась із тим,
  # що порахувало б відвантажене ядро для будь-якого з ~31 % дубів у міксі. 🔑 І саме
  # ця двородинність робила стару 48.5 не просто хибною, а РІЗНО хибною: для сосни
  # (53.2) вона гомеостаз, для дуба (48.2) — аномалія, тобто один літерал давав дві
  # класифікації на тому самому рядку залежно від жеребу породи.
  # ⛔ Не «повертати» z під абсолютний `critical_z_max`: ambient-тепло більше не тригерить
  # хибної аномалії, і саме заради цього стелю зробили ρ-відносною.
  # ✅ Досяжність зміряна, а не припущена: при 41 °C / 150 детекцій z доходить до ~60, і
  # соснову 53.2 перетинає ~36 % cold-start зерен, дубову 48.2 — ~54 % (8 000 прогонів
  # `calculate_z_from_state`; вибірка випадкова, тож ±пара пунктів між прогонами).
  #
