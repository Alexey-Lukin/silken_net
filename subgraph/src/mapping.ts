...
function mintKindOf(identifier: string): string {
  // ⚠️ ТУТ порядок НЕ несучий, і доти цей коментар стверджував протилежне — виміряно
  // мутацією 2026-08-28 [OPS.36]: реверс двох `if` лишив УСІ дев'ять тестів зелені, бо
  // `TAX_BATCH_INS_<did>` не починається з `INS_`, тож перша умова його не перехоплює.
  // Порядок став би несучим лише якби один префікс був ПОЧАТКОМ іншого — а саме це
  // забороняє `spec/quality/mint_prefix_parity_spec.rb` окремою віссю. Тобто справжній
  // захист стоїть ТАМ, і читати цей рядок як гарантію означало б довіряти не тому місцю.
  // 🔑 Місце, де порядок СПРАВДІ несучий, — `subjectDidOf` нижче (послідовне зняття):
  // там та сама мутація валить тест поіменно.
