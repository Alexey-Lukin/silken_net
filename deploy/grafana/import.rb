...
# систематично сліпе до ЗВОРОТНОГО дрейфу — правила, якого в репо немає (створене
# руками в UI або ЗНЯТЕ з IaC). Саме воно й переживе наступний імпорт (upsert
# per-uid чужого uid не чіпає); помітним його робить лише цей режим — сирота
# червонить звірку (гілка `unless extra.empty?` нижче, ⚖️ 2026-09-06), — але лише
# коли звірку ЗАПУСКАЮТЬ: у CI біжить `--dry-run`, не `--verify`, тож між ручними
# прогонами будь-який дрейф стеку мовчить так само, як до цього режиму.
#
...
#     ⚠️ І «збіг із репо» тут НЕ повний: вирази звіряються лише у вузлах-ЗАПИТАХ
#     (`model.expr`), а `__expr__`-вузли (reduce/threshold — сам ПОРІГ) не
#     порівнюються зовсім, як і `title`: змінений у стеку поріг проходить зеленим.
...
#  5. ДАШБОРДИ не звіряються зовсім — лише правила: правка JSON у `dashboards/`
#     доїжджає в стек тільки імпортом, і розходження цей режим не покаже (так
#     2026-09-19 живий опис панелі пережив свою правку в репо).
# ---------------------------------------------------------------------------
if ARGV.include?("--verify")
  ...
  # ⛔ Це НЕ рішення про `--prune` — його ухвалено окремо, 09-06, блоком нижче
  # (відповідь «ні»). Тут виправлено хибне ТВЕРДЖЕННЯ, не додано поведінку:
