...
правило, яке не спрацює НІКОЛИ), і чи немає в стеку правил **поза IaC** —
створених руками або ЗНЯТИХ із репо (імпорт односторонній, upsert іде per-uid,
тож такого правила він не чіпає ніколи; розрізняє їх git-історія
`deploy/grafana/alerts/`, не звіт). Такий сирота ЧЕРВОНИТЬ звірку (`EXIT=1`,
⚖️ 2026-09-06): лік — завести в IaC або зняти зі стеку руками, автоматичного
`--prune` немає свідомо (підстава — коментар над `unless extra.empty?` у
`import.rb`). Оголошені стелі режиму (він не судить ПРАВИЛЬНІСТЬ порогів, не читає
