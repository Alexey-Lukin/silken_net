...
# 🔒 СТЕЛЯ, названа 2026-08-19 (UI.3), бо зелений інакше читається ширше: пара
#    розвʼязується ПІДЙОМОМ по батьках (`backdropOf`), тож абсолютно позиціонована
#    накладка-СУСІД із власним `background-image` — віньєтка `absolute inset-0 z-0`
#    при контенті `relative z-10` — у ланцюг не входить, і число рахується проти
#    тла-предка, якого в тому місці на екрані немає. ⚠️ `elementsFromPoint` тут не
#    лік: він поважає `pointer-events`, а декоративні накладки майже завжди
#    `pointer-events-none`.
#    **Чому це НЕ виправлено, а оголошено — вимір, а не смак.** Наївне правило
#    «накриває вузол → відмовити» реалізовано й зміряно: воно забирає **64 вузли
#    з ~407** по всьому контуру. А єдина така накладка в дереві — сторінкова
#    carbon-fibre текстура при `opacity-5`; її гірший випадок (чорне тло на 5%)
#    зсуває найтоншу пару контуру з **4.83 до 4.60** при барі 4.5, тобто НЕ
#    перетинає його. Платити шістнадцятьма відсотками покриття за цей зсув —
#    гірший обмін. 🔴 Але калібрування лишається чинним і на майбутнє: при
#    `opacity-0.10` той самий зсув дає **4.37**, тобто бар уже перетнутий, —
#    отже накладка, густіша за 5%, ре-відкриває це питання НЕГАЙНО, і правильна
#    форма тоді не «відмовити всім», а смуга невизначеності на самій парі.
#
