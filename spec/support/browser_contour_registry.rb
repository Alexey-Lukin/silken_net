...
module BrowserContourRegistry
    ...
    # ⚠️ Історично їх було ЧОТИРИ, і те, як вони зʼявлялись і зникали, — сам по собі
    # урок про класифікації, тому записаний, а не стертий. (в) «гілка ДОСЯЖНА,
    # відкладена лише через мʼякший симптом — тиша замість блоба» і (г) «джерело Є,
    # сервіс СЮДИ відповідає, а гілку закриває скоуп у СУСІДНЬОМУ екшені, тож
    # залишок — ГОНКА, не неможливість» мали рівно по ОДНОМУ носію кожен, обидва
    # у шарі Codex, і пішли разом із ним 2026-08-15. Два висновки переживають
    # своїх носіїв: підвид (г) знайдено adversarial-проходом уже ПІСЛЯ того, як
    # таксономію оголосили повною на трьох — **класифікація без комірки для рядка
    # не червоніє, вона мовчки кладе його в найближчу**; а підвид, що тримається
