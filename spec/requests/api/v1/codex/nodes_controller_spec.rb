...
RSpec.describe "Api::V1::Codex::Nodes", type: :request do
  ...
  describe "GET /codex/nodes" do
    it "returns 401 without authentication" do
      get "/codex/nodes", as: :json
    ...
    it "renders the Atlas dashboard as HTML when format=html" do
      get "/codex/nodes", headers: headers.merge("Accept" => "text/html")
  ...
  describe "GET /codex/nodes/:slug" do
    it "returns the node payload by slug (not id)" do
      get "/codex/nodes/cherkasy-bir", headers: headers, as: :json
    ...
    it "returns 404 for unknown slug" do
      get "/codex/nodes/does-not-exist", headers: headers, as: :json
    ...
    it "renders the Show dashboard as HTML with comments + attunement state" do
      ...
      get "/codex/nodes/#{node1.slug}", headers: headers.merge("Accept" => "text/html")
    ...
    it "aims the un-attune button at the DELETE route once the viewer is attuned" do
      ...
      expect(response.body).to include(%(action="#{codex_node_my_attunement_path(node1.slug)}"))
      expect(response.body).not_to include(%(action="#{codex_node_attunements_path(node1.slug)}"))
    end
  end
end
