...
RSpec.describe "Api::V1::Codex::Comments", type: :request do
  ...
  describe "POST /codex/nodes/:slug/comments" do
    it "rejects unauthenticated requests" do
      post "/codex/nodes/#{node.slug}/comments", as: :json
      expect(response).to have_http_status(:unauthorized)
    end

    it "creates a comment and increments comments_count" do
      expect {
        post "/codex/nodes/#{node.slug}/comments",
    ...
    it "404s on unknown slug" do
      post "/codex/nodes/no-such-slug/comments",
           params: { comment: { body_md: "hi" } },
           headers: headers, as: :json
