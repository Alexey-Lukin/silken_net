...
RSpec.describe "Api::V1::Codex::Leaderboard", type: :request do
  ...
  describe "GET /codex/leaderboard" do
    it "is publicly accessible (no auth required)" do
      get "/codex/leaderboard", params: { realm: realm.slug }
      expect(response).to have_http_status(:ok)
    end

    it "returns JSON sorted by Elo desc" do
      get "/codex/leaderboard", params: { realm: realm.slug, format: :json }
      expect(response).to have_http_status(:ok)
    ...
    it "honours the limit param (clamp to MAX_LIMIT=100)" do
      get "/codex/leaderboard", params: { realm: realm.slug, limit: 1, format: :json }
