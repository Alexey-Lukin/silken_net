...
describe ".customer_facing_threshold_drift" do
  ...
  # ⛔ `08_02_Academic_Institutions_Registry` і `99_99_Missing_Doc` — НАВМИСНО МЕРТВІ
  # координати, не застарілі фікстури. Модуль 08 розчинено 2026-07-24, а `99_99`
  # синтетичний за побудовою. Саме мертвість тут і є предметом: приклади доводять
  # поведінку резолвера на таргеті, якого НЕМАЄ, — на живому doc-id вони стали б
  # тавтологічними. ⛔ Не «модернізувати» на чинний документ: це знищить доказ і
  # осиротить іменний виняток у `scripts/code_doc_section_refs.rb` (`EXEMPT_REFS`),
  # після чого його ліхтар `stale_exempts` порадить виняток видалити — і наступний
  # СПРАВЖНІЙ фантом за тією ж адресою пройде мовчки.
