...
RSpec.describe Tracker::Dashboard do
  ...
  describe ".open_items" do
    ...
    # 🔦 Ліхтар на живому корпусі — ПЕРЕПРИЦІЛЕНИЙ 2026-09-05, і саме тому вартий
    # коментаря. Доти він стояв на `open_count < total`, тобто його преміса була
    # ВЛАСТИВІСТЮ КОРПУСУ, якої стандарт не вимагає: рівність означає лише, що в
    # реєстрі не лишилось жодного `🌿`/`⚫`-пункту з нулем відкритих ніг — а такий
    # пункт стандарт ДОЗВОЛЯЄ, але не зобовʼязує тримати (00_07 §Форма пункту).
    # Червоне прийшло від архівації останнього такого (`E.27`), тобто від того,
    # що трекер СТАВ ЧИСТІШИМ, і жодного дефекту в коді за ним не було.
    # 🔑 Заміна СИЛЬНІША за зняте, а не слабша: страх ліхтаря — що `open_items`
    # виродиться в `parse` — тепер перевіряється МУТАЦІЄЮ ЖИВОГО КОРПУСУ, тож
    # доказ прямий і не залежить від того, скільки закритих пунктів реєстр має
    # сьогодні. Тавтологію ловить і синтетичний приклад вище (два пункти → один
    # відкритий), там же позитивний і негативний контроль.
    it "на живому трекері не вироджується в parse (доведено мутацією корпусу)" do
      ...
      # Зняти КОЖЕН відкритий чекбокс живого реєстру: множина пунктів не змінюється,
      # множина відкритих мусить впасти в нуль. Рівність тут і була б тавтологією.
      closed = md.gsub(/^-\s+\[ \]/, "- [x]")
      expect(described_class.parse(closed).size).to eq(total)
      expect(described_class.open_items(closed)).to be_zero
    end
  end
