...
it "no expression reads a raw datasource query — only a reduce may" do
  ...
  # в обох).
  # 🔴 ДРУГА підстава цього ⛔ у первісній формі ПОМЕРЛА 2026-08-31 і ОЖИЛА в іншій 2026-09-03.
  # Стояло «фільтр зробив би смерть canopy-Alloy невидимою» — canopy-Alloy НЕ ІСНУЄ: контейнер
  # один на два слоти (`Kamal::Configuration::Accessory#service_name` не несе destination),
  # і `-d canopy` не створював другого агента, а перемарковував єдиного (§«the ONE-Alloy
  # invariant» у `alloy_scrape_topology_spec.rb`). Але з 2026-09-03 той самий агент скрейпить
  # і canopy-таргети зі `slot="canopy"` НА ТАРГЕТІ, тож фільтр `{slot="production"}` зробив би
  # невидимою смерть уже canopy-ТАРГЕТІВ (`sn-alert-scrape-target-down` з `slot=canopy`).
  # Вердикт не рухався жодного разу — пару тримаємо розведеною ([`00_05 §5`]: спростований
  # мотив ≠ спростована заборона, і воскреслий мотив — теж не привід її переписувати).
  #
