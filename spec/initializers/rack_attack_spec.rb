...
RSpec.describe "Rack::Attack", type: :request do
  ...
  describe "login throttle (logins/ip)" do
    # 🔴 [SEC.32] Тут стояв приклад «throttles login attempts after 10 POSTs»:
    # 11 POST'ів на `/login` і чекання 429. Мутацією (ліміт `logins/ip` → 1000)
    # доведено, що він лишався ЗЕЛЕНИМ при фактично знятому throttle — 429 віддавав
    # контролерний `rate_limit to: 5` (`sessions_controller`). Тобто приклад був
    # названий іменем механізму, якого не торкався, і зникнення правила з конфіга
    # не червонило нічого.
    #
    # Заступників у критерію досяжності (§configuration нижче) ДВА, не один:
    # Fail2Ban і Rails-нативний ліміт У КОНТРОЛЕРІ. `logins/ip` живий рівно на
    # ОДНОМУ зі своїх трьох шляхів — `PATCH /reset_password` (`passwords#update`,
    # а контролерний ліміт там `only: :create`), і саме його пінить приклад нижче.
    # `POST /login` (5/хв) та `POST /forgot_password` (3/5хв) заступлені.
