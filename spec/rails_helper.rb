...
# [TEST.8] Ключ-власник прогону в Kredis DB 1 — тій самій, яку сюїта флашить
# перед кожним прикладом, тобто рівно там, де конкуренція й шкодить.
...
RSpec.configure do |config|
  ...
  # рівно два — Redis DB 1 (`flushdb` нижче біжить перед КОЖНИМ прикладом, тобто
  ...
  # ресурси. Хто ізолює лише Redis (`KREDIS_REDIS_URL=…/5`), той гард приглушить,
  ...
  config.before(:suite) do
    ...
    if owner.present? && owner != Process.pid.to_s && process_alive?(owner)
      raise <<~MSG
        ...
        Два прогони НЕ ізольовані: `Kredis…flushdb` перед кожним прикладом вимиває
