...
module ModelMessageLocalization
  ...
  DECLARED = {
    ...
    [ "app/models/ews_alert.rb",
      "доказ Категорії A закриває лише платформа (06_08 §4.6)" ] => {
      why: "бекстоп замка доказу Кат-A [SLASH-1]; людський шлях — `resolve!`, і він відмовляє " \
           "РАНІШЕ власним `EvidenceLocked` → локалізована 403 (`flash.alerts.evidence_locked`). " \
           "Сюди доходять лише двері без викликачів (`ignore!`, голий запис статусу), тож " \
           "спрацювання означає баг у коді, не дію людини",
      back: "поява людського шляху, що виводить тривогу з `:active` повз `resolve!`"
    },
