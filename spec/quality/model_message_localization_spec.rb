...
#
# ⚠️ **Ключ реєстру — `файл:РЯДОК`, тобто якір VOLATILE за побудовою** (виміряно
# 2026-08-15): будь-яка правка ВИЩЕ по файлу зсуває номер, і гейт віддає ДВА
# падіння одразу — «незадеклароване зашите повідомлення» + «декларація пережила
# свій предмет», — які читаються як два різні дефекти, хоч насправді це один
# зсунутий якір. Ціна прийнятна (оновити рядок у реєстрі — секунда), але знати
# треба: **червоне тут не завжди про повідомлення, спершу звір номер із файлом.**
# Ключувати на ВМІСТ (файл + фрагмент рядка) було б стійкіше й лишається
# відкритим — окрема робота, бо міняє форму всіх записів разом.
#
...
module ModelMessageLocalization
  ...
  DECLARED = {
    ...
    "app/models/blockchain_transaction.rb:293" => {
