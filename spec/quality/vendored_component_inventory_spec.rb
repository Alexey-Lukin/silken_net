...
# обох нотіс-файлах, а `leaflet.css` і його спрайти не несли й власного
...
#
# ⚠️ ПЕРША РЕДАКЦІЯ ЦЬОГО ГЕЙТА БУЛА ЗЕЛЕНОЮ З ХИБНОЇ ПРИЧИНИ, і спіймав це не
# прогін, а РУЧНИЙ перерахунок множини. Деривація імені брала передостанній
# сегмент шляху, тож на спрайтах давала «images» — рядок, що ВИПАДКОВО збігався
# підрядком із записом інвентарю. Тобто гейт звітував про покриття, якого не мав,
# і зелене означало «збіг підрядка», а не «компонент названий». Рефлекс: для
# будь-якої деривації ключа зі шляху перевір, що вона дає на НАЙГЛИБШОМУ і на
# найкоротшому члені множини — і порахуй множину рукою бодай раз.
