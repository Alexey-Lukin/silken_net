...
# Клас (B) лишається за ревʼю — як голий `expectRevert` у Solidity (`04_06 §B.2`).
#
# ⚠️ ДРУГА ФОРМА того самого множника — альфа на самому КОЛЬОРІ (`text-…/NN`) —
# свідомо поза гейтом, і це вимір, а не пропуск: 16 сайтів у дереві, з них 15 —
# watermark'и `text-emerald-900/5` (свідомі 1.09:1, декларовані в реєстрі
# `contrast_audit`), а єдиний не-watermark (`Telemetry::BatchSummary`
# `text-gaia-text-strong/80` — ex-`Telemetry::LogEntry`, компонент перейменовано
# [UI.16] 2026-09-06; множник і пара кольорів не змінились, тож числа чинні)
# проходить із запасом — 9.28:1 світла / 12.18 темна.
# Тобто живих дефектів у цій формі НУЛЬ, а гейт на неї народився б із
# пʼятнадцятьма винятками, тобто реєстром. Клас реальний — наступний `/NN` на
# змістовному тексті буде так само невидимий; ловити його доти ревʼю.
module OpacityContrastMultiplier
  ...
  # Машинні звільнення — рівно ДВА, і обидва про сам ШАР, не про доступність:
  ...
  #
  # 🔴 `aria_hidden` сюди НЕ входить, і перша редакція цього гейта помилково його
  # включала. 1.4.3 — критерій ВІЗУАЛЬНИЙ, а атрибут ховає від скрінрідера, не від
  # ока: `Shared::UI::EmptyState` тримав під ним гліф на **2.32:1** (світла тема),
  # тобто ледь видимий саме як візуальний акцент, заради якого стоїть. Декоративне
  # в цьому дереві виключається ЯВНОЮ декларацією в реєстрі `contrast_audit`
  # (15 watermark'ів `text-emerald-900/5`, свідомі 1.09:1), ніколи автоматично за
  # атрибутом — політика чинна й до цього гейта, знайдена закриваючим свіпом.
  EXEMPTION_PATTERN = /pointer-events-none|disabled/
...
RSpec.describe "opacity як прихований множник контрасту", type: :model do
  ...
  it "пропускає ЗАКОННУ форму — декоративний оверлей (GREEN-половина)" do
    ...
    # Взірець живий у дереві: фонова текстура `DashboardLayout`.
    overlay = %q(div(class: "absolute inset-0 opacity-5 pointer-events-none text-gaia-text-muted"))

    expect(gate.dimmed_text_node?(overlay)).to be(false)
  end

  it "НЕ звільняє вузол лише за aria_hidden — 1.4.3 судить око, не скрінрідер" do
    # 🔴 Перша редакція гейта звільняла `aria_hidden` автоматично, і це знайшов
    # закриваючий свіп, а не тест: чинна політика приладу-сусіда (`contrast_audit`)
    # виключає декоративне ЯВНОЮ реєстровою декларацією, ніколи за атрибутом.
    # Ціна помилки була б виміряна — гліф `EmptyState` на 2.32:1 проходив би тихо.
    hidden = %q(p(class: "text-gaia-text-muted text-lg opacity-50", aria_hidden: "true") { @icon })

    expect(gate.dimmed_text_node?(hidden)).to be(true)
  end
