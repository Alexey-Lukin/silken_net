...
# констант, ніколи логіку, що їх вживає. Порядок перевірок у `mintKindOf`
# (зовнішній `TAX_BATCH_` спершу, бо податковий запис несе identifier першої tx і
# може бути `TAX_BATCH_INS_<did>`) — окреме твердження, і його стереже коментар у
# самому мапінгу, не цей файл. Так само поза скоупом — чи ВСІ природи емісії мають
# префікс: `GROWTH` його не має за конструкцією дроту.
#
