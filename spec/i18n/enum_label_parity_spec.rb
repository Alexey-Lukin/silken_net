...
#     взагалі (`AuditLog#action`), сюди не входить — він відкритий у
#     `00_07` I18N.1. ⚠️ Breadcrumb-сегменти зі стелі ЗНЯТО: їхнє джерело —
#     `Rails.application.routes`, а не enum моделі, тож вони не могли б стати
#     рядком цього реєстру навіть після появи скоупа; вісь тримає власний
#     сиблінг `spec/i18n/breadcrumb_segment_parity_spec.rb`.
...
registry = [
  ...
  },
  {
    ...
    # [BIZ.22] `investor` — ЄДИНА легітимна мітка без enum-значення, і це
    # ДЕКЛАРАЦІЯ з підставою, не послаблення гейта. Роль перейменовано на
    # `subscriber` 2026-08-28 (integer 0 збережено), але історичні
    # `audit_logs.metadata` несуть старе значення РЯДКОМ — журнал переписувати
    # не можна, а `User#role_label` fail-open, тож без мітки клієнтська сторінка
    # аудиту надрукувала б сирий токен «investor» рівно на тому екрані, чию мову
    # й вичищали. ⛔ Знімати виняток можна лише разом із доказом, що жоден
    # audit_logs-рядок цього значення більше не несе.
    exclude: %w[investor]
  },
  {
...
RSpec.describe "enum ↔ locale label parity" do # rubocop:disable RSpec/DescribeClass
  ...
  registry.each do |entry|
    describe entry[:name] do
      ...
      #
      # 🔴 ОГОЛОШЕНА СТЕЛЯ (виміряна мутацією 2026-08-29): ця перевірка бачить
      # лише БАЗОВУ локаль — `declared_labels` читає піддерево `default_locale`.
      # Мітка-сирота, додана в `uk`/`lv`/`lt`, для неї НЕВИДИМА: мутація в
      # `uk.yml` лишила гейт зеленим, та сама мутація в `en.yml` — червоним.
      # Це прийнятно (сирота в небазовій локалі не має споживача, бо fallback
      # веде в базову), але знати про це треба: зелене тут означає «базова
      # локаль чиста», ніколи «жодна локаль не має сиріт».
