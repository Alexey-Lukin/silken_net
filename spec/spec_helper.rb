...
SimpleCov.start "rails" do
  ...
  # підлогою Є дозволом ерозії. Branch — тонший сигнал (line ~99.8 усюди).
  # Services branch → 98 (15 leave-гілок після DOC-T.89; на 99 дво-гілковий float
  # дав би хибне падіння).
  #
  # 🔴 **Запас тут ≤0.1pp ЗА ПОБУДОВОЮ, і це не деталь, а робоча умова.** Підлоги
  # стоять на сітці 0.1, а беруться як floor(факт) — тож «~0.5–1pp під seed-flake
  # margin» (так цей коментар стверджував до 2026-08-26) було недосяжним при самій
  # формі ратчета, і вимір це підтвердив: три групи з пʼяти сиділи в діапазоні
  # 0.03–0.05pp, тобто МЕНШЕ ОДНІЄЇ гілки. Практичний наслідок: одна нова
  # непокрита гілка — включно з мертвою `§B.4 leave` — кладе CI, і це очікувана
  # поведінка, а не поломка. **Перш ніж лишати `leave`-гілку в цих групах, зміряй
  # запас** (`coverage.json` → `covered_branches`/`total_branches` по префіксу
  # групи), а не вгадуй його з цього тексту.
  ...
  # як ПОФАЙЛОВИЙ поріг, тож групова підлога мовчки зникає при зеленому прогоні (негативний
  # контроль — тіло коміту c64ff0b1, прохід OPS.22).
  # 🔴 **ДРУГА форма того самого мовчання, куплена бампом SimpleCov 1.3 (OPS.22): група, що
  # НЕ МАТЧИТЬ ЖОДНОГО файлу, зникає зі звіту — і підлога на ній проходить МОВЧКИ**, замість
  # доти-падінгу 100%. Виміряно на цьому ж бампі: `Helpers` (нуль файлів) зникла з
  # `coverage.json`, тоді як КОЖНА гейтована нижче група лишилась байт-у-байт. Тобто сьогодні
  # гейт цілий рівно тому, що кожна гейтована група має файли. **Перейменувавши каталог або
  # звузивши `group "…", "app/…"` вище, спитай не «чи сюїта зелена», а «чи ця група ще має файли» —
  # порожня віднині не червоніє НІКОЛИ.** Мутаційний доказ, що підлога жива під 1.3:
  # підняти будь-яку `minimum` вище факту → `exit 2` з назвою групи в рядку.
