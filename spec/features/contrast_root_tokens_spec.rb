...
#   · 🔴 `sessions/new` поза контуром, і ПІДСТАВА ЗМІНИЛАСЬ: доти `AuthLayout`
#     хардкодив `dark`, тобто світлої теми та сторінка не мала взагалі. Тумблер
#     знято, хардкод разом із ним — auth-екрани тепер такі самі хамелеони, як
#     решта, і їхня світла половина НЕ ВИМІРЯНА жодного разу. Це не «поза
#     скоупом», а незакрита діра: сторінка живе ДО автентифікації, тож у цей
