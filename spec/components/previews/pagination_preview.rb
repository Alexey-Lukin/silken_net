...
class PaginationPreview < Lookbook::Preview
  ...
  def middle_page
    pagy = OpenStruct.new(page: 3, last: 10, previous: 2, next: 4)
  ...
  def first_page
    pagy = OpenStruct.new(page: 1, last: 5, previous: nil, next: 2)
    render Views::Shared::UI::Pagination.new(pagy: pagy, url_helper: ->(page:) { "#page=#{page}" })
  end
  ...
  def last_page
    pagy = OpenStruct.new(page: 5, last: 5, previous: 4, next: nil)
    render Views::Shared::UI::Pagination.new(pagy: pagy, url_helper: ->(page:) { "#page=#{page}" })
  end
end
