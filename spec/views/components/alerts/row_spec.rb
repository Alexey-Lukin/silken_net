...
RSpec.describe Alerts::Row do
  ...
  describe "alert content" do
    ...
    # [ARCH.31] Вхід на сторінку тривоги (а з нею — на SOP-панель). Обидві гілки
    # пиняться ПОЗИТИВНО: сам факт «є посилання» нічого не каже, доки дефолт не
    # доведено мовчазним, бо саме він стоїть усередині `Alerts::Show`, де лінк
    # вів би на себе.
    # ⚠️ Ціль посилання цей приклад НЕ доводить — компонент рендериться повз
    # маршрутизатор і повз викликача; це робить request-пін у
    # `spec/requests/api/v1/alerts_controller_spec.rb`.
    it "у реєстрі (адресу дав продюсер) тип веде на сторінку тривоги" do
      linked = render_component(alert: build_alert(id: 42), detail_href: "/alerts/42")
      expect(linked).to include('href="/alerts/42"')
    end

    it "усередині самої сторінки тривоги (дефолт) посилання НЕМА — воно вело б на себе" do
      expect(html).not_to include("href=\"/alerts/")
    end
  ...
  # ⊕ Пін лишається розрізнимим (на відміну від `04_06 §B.2` #20): `bg-status-neutral`
