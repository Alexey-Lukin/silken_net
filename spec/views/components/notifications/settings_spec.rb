...
RSpec.describe Notifications::Settings do
  ...
  # [I18N.1] про ІНШУ вісь — мітки `select`.
  # ✅ **Носій ВІДНОВЛЕНО 2026-09-21** у `maintenance/form_spec` §"error display"
  # (мутаційно звірений: зняття запису локалі робить його червоним). ⚠️ І тут
  # рецепт цієї нотатки був ХИБНИЙ: вона називала кандидатами `tree_families`
  # і `firmwares`, а `config/locales/attributes/*.yml` має запис для ОДНІЄЇ
  # моделі — `maintenance_record`. На формі без такого запису `humanize` і
  # резолвер віддають те саме слово, тож пін був би зеленим і на зламаному
  # резолвері. **Називаючи дім для піна, перевіряй не де рендериться компонент,
  # а де ДАНІ роблять дві гілки розрізнимими.**
