...
unless defined?(Kredis)
  ...
  describe "forest_coin transaction" do
    ...
    # активації governance (SEC.1). ⚠️ Підстава ПЕРЕМІРЯНА: quorum Governor'а рахується від
    # immutable `QUORUM_BASE` = SFC `MAX_SUPPLY` (той самий DOC-T.89), тож «перші 10k = 100%
    # голосів» спростовано; вціліла вужча — `proposalThreshold` = абсолютні `10_000e18`, тобто
    # перші 10k дають ОДНООСІБНЕ ПРАВО ПОДАВАТИ. Живих writer'ів `forest_coin` сьогодні НУЛЬ.
