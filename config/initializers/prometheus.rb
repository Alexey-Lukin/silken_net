...
module SilkenNet
  module Metrics
    ...
    # те (⚖️ У-ВЕЙ 09-05 — дефолт `noDataState: OK` НЕ чіпаємо). ⚠️ Наслідок,
    ...
    # 🔴 [S2.4, 2026-09-06] СВІДОК ЗАВЕРШЕНОГО money-path проходу — і він накриває
    # ВІСІМ алертованих ґейджів, які без нього замерзали нечутно.
    #
    # Класифікація приладів під алертами (`06_03 §2.9`) дала не 13 сторожів, а 4
    # писачі, і `Treasury::MonitorService` несе девʼять із тринадцяти. Смертей у
    # писача ДВІ, і жоден запит їх не розводить:
    #   (А) метод кинув, а `rescue` ковтнув — тут це не гіпотеза: три підпроходи
    #       (`update_money_path_metrics`, `update_payout_float_metrics`,
    #       `detect_mint_volume_anomaly!`) мають власний `rescue`, який СВІДОМО не
    #       валить monitor-цикл (баланси важливіші за спостережність). Наслідок —
    #       ґейджі застигають на останньому значенні, а їхні правила читають
    #       ЗАМЕРЗЛЕ число як здорове.
    #   (Б) воркер не біг узагалі — жоден `rescue` не спрацює, бо коду не було.
    #       `noDataState: NoData` тут не рятує: він приходить, лише коли ряд ЗНИКНЕ
    #       (рестарт процесу), а голодування черги `web3_low`(8) лишає ряд ЗАМЕРЗЛИМ.
    # ⛔ І з тієї ж причини правило НЕ сміє фільтрувати `> 0`: безлейблову серію
    # `prometheus-client` реєструє зі значенням 0.0 від старту процесу, тож фільтр
    # зробив би алерт сліпим рівно до (Б). Виміряно 2026-09-06 (`values={{}=>0.0}`).
    # 🔑 Штамп накриває ОБИДВІ однією величиною — саме тому він ставиться ЛИШЕ коли
    # всі три підпроходи доповіли завершення, і ЛИШЕ в кінці `perform`.
    # ⛔ Ставити його на старті або поза гардом завершення ЗАБОРОНЕНО: свіжий штамп
    # над застиглим числом бреше переконливіше за відсутність (та сама підстава, що
    # в `TREE_SWEEP_TIMESTAMP` / `PARTITION_SAMPLE_TIMESTAMP`).
    # ⚠️ Оголошена стеля, і межа проведена ОДНИМ питанням — «чи нога пише ґейдж, на
    # який дивиться правило». Тому поза гардом ДВІ: `update_payout_float_metrics`
    # (її `PAYOUT_FLOAT_BALANCE` алерту не має) і `update_metrics` (девʼятий ґейдж
    # `oracle_balance_ratio` теж може замерзнути, але його тишу вже озвучує
    # `TREASURY_CHECK_ERRORS_TOTAL` + `sn-alert-treasury-check-errors`, INF.26).
    # Втягнути будь-яку з них означало б платити другим свідченням за вже засвідчене.
    TREASURY_MONITOR_TIMESTAMP = REGISTRY.gauge(
      :silkennet_treasury_monitor_timestamp_seconds,
      docstring: "Unix time of the last COMPLETED treasury monitor pass (freshness witness for the eight alerted money-path gauges it writes; the ninth, oracle_balance_ratio, is voiced by treasury_check_errors_total instead)"
    )
    ...
    # ⚠️ Доставка (ARCH.60 — сама пошта, відколи Telegram знято 2026-09-06) реальна й
    # лічильника НЕ має, тож ім'я мусить
