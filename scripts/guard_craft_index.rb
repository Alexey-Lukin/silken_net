...
#
# ФОРМА РЯДКА — ТІЛЬКИ ЛІД, і це змінено 2026-09-08 після виміру. Довго тут
# стояло `лід — Reflex`, бо ліди РАННЬОЇ когорти писались ЯРЛИКАМИ («Decorative»,
# «A noisy advisory»), і самого ліда для них не вистачало. Обовʼязок (3) нижче
# велів розширювати ліди в ДЖЕРЕЛІ — його виконали, і форма пережила свою
# підставу: на день зміни 149 лідів зі 150 у `guard-craft` стріляли самі, а
# рефлекс дублював у ЩОСЕСІЙНОМУ файлі те, що вже лежить у companion. Зріз
# рефлексів зняв ~50 kB з авто-шару (ssot −22 kB · backend −16 kB · frontend
# −6 kB · web3 −5 kB) при нулі втрати: 236 рефлексів лишились у джерелах.
# ⚠️ Разом із формою переїхала й ПІДЛОГА: `WORD_FLOOR` рахує тепер ЛІД САМ, а
# не пару. Доти ярликовий лід проходив, бо його підпирав рефлекс — тобто гейт
# мовчки дозволяв індексу рендерити ярлик. Полагодити терсий рядок можна ЛИШЕ
# в companion-файлі, тобто тиск іде в джерело, не в дзеркало.
#
...
WORD_FLOOR   = 8
LEAD_CEILING = 400   # B; виміряно 2026-09-08: медіана ліда 184 · p90 295 · max 364
...
TARGETS.each do |t|
  ...
  list.each do |it|
    ...
    next if it[:lead].to_s.split.size >= WORD_FLOOR
...
# ⛔ СТЕЛЯ — відсутня симетрична половина, і її відсутність ВИМІРЯНО (2026-09-08):
# `WORD_FLOOR` штовхає рядок РОСТИ й не тримає його ніде, тож індекс дійшов до
# 53 kB при задекларованих у цій же шапці «5–13% від тіла» — 13.6%, за межею
# власної норми. Це `guard-craft` #150 у чистому вигляді: гейт, що ВИМАГАЄ
# запису, сам його роздуває, і стеля мусить жити в ТОМУ САМОМУ гейті.
# ⚠️ Судить РОЗМІР, ніколи ЗМІСТ — довгий лід не є гіршим лідом, він є лідом,
# що зʼїв бюджет сусідів у файлі, який вантажиться ЩОСЕСІЇ.
list.each do |it|
  next if it[:lead].to_s.bytesize <= LEAD_CEILING

  errs << "item #{it[:num]}: lead is #{it[:lead].to_s.bytesize} B, ceiling #{LEAD_CEILING} B — " \
          "the index loads EVERY session; move the detail into the body in " \
          "#{File.basename(t[:aux])} and keep the lead a carrier, not a paragraph"
end
