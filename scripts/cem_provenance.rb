...
#   не побачить, і мовчав про те, яке побачить) · зазор сокета · висота порожнини
...
#
# 🔬 ЩО САМЕ ЗВІРЕНО МУТАЦІЄЮ (мігровано з `00_07` HW.48 при цементації 2026-09-17 — `00_06 §3`
#    маршрутизує «мутаційний доклад» саме сюди, і доти тут його не було). Осі звірялись ПО ОДНІЙ,
...
#    підставу (`name` → `kind` → `*` — див. lookup нижче). Тобто зняття яруса не ламає гейт, воно
#    ПІДМІНЮЄ підставу, і саме проти цієї шкоди ярус і заведено. Банер нижче називає лише дзеркальну
#    половину (запис на `kind` там, де провенанс РІЗНИЙ), а ця, знімальна, дому не мала.
# ⛔ І ДРУГА діра, оголошена вголос: перевірки ЖИВОСТІ самих записів немає. Ключ у `fields`, що не
#    відповідає жодному маніфесту, не червоніє нічим — сьогодні таких нуль, і саме нуль є умовою, за
#    якої ця діра гниє невидимо: перший же перейменований чи знятий SKU лишить по собі запис-привид,
#    який виглядатиме як чинна підстава.
#
...
#    Взірець, що купив клас — `o_ring_groove_depth_mm` у фланці й радомі: ⚖️ 2026-09-10 поставив ОДИН паз
#    1.344 проти ПЛАСКОЇ кромки, а відвантажена пара зустрічних 0.9 давала стиск −1.1 %, тобто не
#    ущільнювала. Без цього класу таке число читалось як «design» — тобто як чинне рішення. (Застосовано
#    2026-09-14: обидва поля знято, паз виводиться з блоку `o_ring`; клас лишається для наступного присуду.)
