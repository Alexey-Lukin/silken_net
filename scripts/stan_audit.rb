...
#      • статуси ⚪/⚫/🌿/🔗 → mute (дім у мета-рядку = ПРИЗНАЧЕННЯ майбутньої
#        роботи, не клейм на наявний зміст; lifecycle: айтем стає 🟢/🟡 —
#        клейм активується і токени випливають самі; muted-список друкується
#        ID×count — цемент-око бачить, ЩО заглушено, і може пірнути).
#
...
items.each do |id, it|
  ...
  tokens.uniq.each do |tok|
    ...
    # ⚪/⚫/🌿/🔗: дім = призначення роботи, не клейм — mute (would-be хіт)
...
if muted.any?
  list = muted.map { |id, n| n > 1 ? "#{id}×#{n}" : id }.join(" · ")
  puts "  (заглушено статусом ⚪/⚫/🌿/🔗: #{list} — дім = призначення роботи; випливуть самі при переході в 🟢/🟡)"
end
