...
def takeable_report(markdown)
  ...
  puts "  · 🔴 НАЙБІЛЬША сліпота цього звіту не в прозі й не в STAGE, а в ОДИНИЦІ: він рахує"
  puts "    НОГУ, а гейтування є властивістю ПОЛОВИНИ ноги. Рядок, де машинна половина передує"
  puts "    людській, успадковує `👤` і зникає звідси цілком — і рецепт легенди `00_07`"
  puts "    («шукай там, де нога закінчується на передати · спитати · замовити») ловить лише"
  puts "    ОДИН підклас. Виміряно 2026-09-22 на HW.26: машинна половина ховалась усередині"
  puts "    👤-ноги ANSYS-FEA й НАЗИВАЛА СЕБЕ САМА клаузою «⊕ Спершу звести вхід» — ні тобі"
  puts "    «передати», ні «спитати», тобто рецепт її пропускав за побудовою. **Другий рецепт,"
  puts "    що його доповнює: грепни ТІЛА 👤-ніг на клаузи-передумови — `спершу` · `перш ніж` ·"
  puts "    `⊕ звести` · `до того, як` — і спитай, якого WHO та передумова.**"
  puts "  · форми, які фільтр знає: #{DEFERRAL_FORMS.keys.join(' · ')}"
  puts "  · ⚠️ І фільтр НЕ читає заперечення: «не <форма>» матчить так само, як «<форма>»."
  puts "    Виміряно 2026-09-22 на собі — самодекларація «нога НЕ гейтована», дописана"
  puts "    в 00_07 заради ЛЮДИНИ, додала нозі другу форму й викинула її зі списку"
  puts "    «схованих РІВНО ОДНІЄЮ формою», тобто з периметра для читання очима."
  puts "    **Оголошення свободи, написане словником заборони, ховає краще за саму"
  puts "    заборону** — пиши самодекларацію словами, яких у переліку вище НЕМАЄ."
  puts
