...
EXEMPT_REFS = {
  ...
  # розчинено [DOC-T.68 фаза 1]. ⚠️ ПІДСТАВУ ПЕРЕПИСАНО 2026-08-22: «мертвий назавжди»
  # більше НЕ так — номер `00_05` заселено новою сторінкою (DOC-T.83). Фікстура чинна,
  # бо нова сторінка секції `§2.7` не має; тримає це не обіцянка, а stale-exempt ліхтар
  # нижче: щойно адреса почне резолвитись, виняток перестане стерегти й гейт упаде.
  # Урок: заселення вільного номера перевіряє КОЖЕН виняток, обґрунтований «назавжди».
  ...
  # 🔴 `spec/lib/` БУЛО виключено ЦІЛОЮ ТЕКОЮ до 2026-08-23 (DOC-T.86). Доти це було
  # дешево: тамтешні рефи цитували мертві doc-id. Заселення слота `00_05` змінило
  # ціну — фікстура нижче стала рефом у РЕАЛЬНИЙ док із неіснуючою секцією, тобто
  # рівно тим дефектом, що гейт і ловить, схованим винятком по теці (guard-craft #88).
  # Ціну звуження зміряно ДО вмикання: рівно 19 рефів, усі — фікстури, жодного
  # справжнього дефекту. Перелічені ПОІМЕННО, а не патерном `08_02 §`, свідомо:
  # зіставлення тут підрядкове (`h.include?(r)`), тож короткий запис ЗАТІНЮЄ довші —
  # якби модуль 08 повернувся, «08_02 §» лишався б задоволеним іншим мертвим рефом
  # і мовчки прикрив би оживлений. Поіменний перелік полісить stale-ліхтар окремо
