...
# ── СТЕЛЯ НА КЛІТИНКУ ОПИСУ [DOC-T.102] ─────────────────────────────────────
# Дзеркало трьох підлог вище, у протилежний бік — і воно тут не з естетики.
# CHECK B вимагає РЯДОК на кожну нову мітку, тобто цей самий скрипт і є тиском,
# що змушує клітинки рости; отже спинити їх може теж лише він. Зростали вони
# не таблицею, а ЖИРОМ: клітинки ДОПИСУВАЛИ — датовану хроніку, оголошені стелі
# рушіїв, мутаційні доклади — при тому, що дім усього цього є шапка самого
# рушія, яка гниє РАЗОМ із кодом і тому не розходиться з ним.
#
# 🔒 ОГОЛОШЕНА СТЕЛЯ ЦІЄЇ ПЕРЕВІРКИ: вона судить РОЗМІР, ніколи ЗМІСТ. Клітинка
# на 800 B може бути чистою хронікою, і про це не почервоніє ніщо; вона ловить
# лише той клас, що вже коштував — коли одна клітинка дійшла до 14.1 kB, а весь
# §3 до 77% файлу, який фізично не відкривався цілком.
#
# ⚖️ ЧОМУ 3000, а не медіана: після зрізу 2026-09-07 найбільша клітинка = 2403 B
# при медіані ~1300, тож стеля має ~600 B запасу й НЕ поліціює звичайний рядок —
# вона відсікає рівно монстра. Судиться КОЛОНКА ОПИСУ, не команда: команда
# зобовʼязана бути VERBATIM (CHECK A) і стелі не підлягає.
#
# Підіймати можна — але ВИДИМОЮ правкою тут, і спершу спитавши, чи те, що не
# влазить, не належить шапці рушія. Це і є єдина дія, якої стеля вимагає.
CELL_CEILING = 3000
...
registry_rows.each do |row|
  ...
  desc = cols[2].gsub("\x00", '\|')
  next if desc.bytesize <= CELL_CEILING

  name = cols[1].to_s.strip[0, 60]
  errors << "§3 клітинка «#{name}» = #{desc.bytesize} B опису, стеля #{CELL_CEILING} B. " \
            "Реєстр називає ПРЕДМЕТ гейта в теперішньому часі плюс адресу рушія; стеля, " \
            "мутаційний доклад і датована хроніка живуть у шапці рушія (вона гниє разом із " \
            "кодом) і в `00_07`. Якщо зміст справді належить сюди — підійми `CELL_CEILING` " \
            "видимою правкою в цьому скрипті."
end
