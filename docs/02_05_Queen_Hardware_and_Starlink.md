...
> **⚠️ Транспортна надійність через D2C (2026-05-28):** Direct-to-Cell працює за LTE-протоколами (SMS + базовий 4G) через жорсткий **Carrier-NAT**, який може блокувати вхідний UDP (а CoAP — UDP) або змінювати порти. ⊕ **Висновок «тож чистий CoAP/UDP ненадійний, потрібен TCP» знято разом із засновком:** Королева сама опитує сервер після флашу ([`03_02 §4а`](03_02_Queen_Gateway_Firmware), [`00_07`](00_07_Action_Plan_Tracker) FW.60), тож вхідний напрямок їй не потрібен. Відкритим лишається інше — чи витримує CoAP/UDP втрати й jitter супутникового LTE; це судить перший вимір на D2C ([`00_07`](00_07_Action_Plan_Tracker) HW.41), а фолбек **CoAP-over-TCP** ([RFC 8323](https://www.rfc-editor.org/rfc/rfc8323)) чи **MQTT-SN** — варіант на випадок, якщо вимір покаже втрати, а не архітектурна вимога. Серверний носій толерантності до jitter у дереві не названо (Ingress Proxy [`06_01`](06_01_Deployment_Kamal_Terraform), на який тут доти слали, — компонент пропускної здатності, про jitter і NAT він не каже). Планований HIL-профіль навантаження (**ще не в коді** — [`00_03 §3.2`](00_03_TRL_Matrix_HIL_and_Beyond)) моделює саме ці умови.
...
**Висновок (зима, Phase 1/2.5):** 15.0 Вт·год генерації vs ~7.4 Вт·год споживання (чесний Victron-quiescent 20 мА, §4; ⚠️ струм BMS не враховано — з найощадливішим кандидатом шортлиста HW.15 профіцит ≈ +4.7) → **профіцит +7.6 Вт·год/добу ✅** без BMS. 10W-панель дала б −4.4 Вт·год/добу → відхилено (HW.39).

**Висновок (зима, Phase 3 Hard OFF):** 15.0 Вт·год vs ~75.6 Вт·год → дефіцит ~60.6 Вт·год → автономність **3.2 дні** на LiFePO4 12V/20Ah.

> Блокує лише **Phase 3** (Unit Economics — [`02_06`](02_06_Unit_Economics_and_BOM)). ⚠️ Мітигації **не взаємозамінні** (навіть 100W сама = 30 Вт·год < 75.6 спожив; лише duty-cycle+комбо закриває баланс) → [`00_07` — HW.14](00_07_Action_Plan_Tracker).
...
| MPPT quiescent (BMS не враховано — запас ≤ 17 мА, HW.15) | 20 мА × 12V (Victron 75/15, мануал Rev 10 с. 66) | 5.76 |
...
> **Висновок:** Phase 1/2.5 (SIM7070G / DTC) — стійка взимку під кронами на **50W-панелі + 20Ah** (рішення HW.39, 2026-07-13; закуп-BOM [`02_06 §4`](02_06_Unit_Economics_and_BOM) синхронізовано). Phase 3 (Starlink Mini) вимагає duty-cycle-скорочення **плюс** комбо ≥40Ah/100W ([`00_07` — HW.14](00_07_Action_Plan_Tracker)). **Наскільки скорочення (прогін 2026-09-25):** на комбо 40 Ah + 100 W зимовий баланс перетинає нуль біля **1 хв/год** Starlink, а deploy-запас 20 % дає лише **≈ 0.4 хв/год** — близько 10 хв на добу, у ~12 разів менше за 5 хв/год таблиці вище; відтворює `ruby tools/firmware/queen_energy_budget.rb battery_ah=40 panel_w=100 starlink_min_per_h=0.4`. ⚠️ Стеля висновку: BMS-рядка модель ще не має ([`00_07` HW.15](00_07_Action_Plan_Tracker) — бюджет ≤ 17 мА, тобто до ≈ 4.9 Вт·год/добу на 12 В), і на верхньому краю він зʼїдає весь цей запас.
...
> Енерго-Wh SSOT — модель HW.39 (§4); тут — теплова проєкція. Quiescent-тепло Victron (240 мВт; BMS не враховано — HW.15) сидить у MPPT/BMS-блоці — чи ділить він відсік із електронікою, вирішує enclosure-layout при BOM-freeze.
...
**Phase 1/2.5 (середнє розсіювання ~0.33 Вт, з них 0.24 Вт — MPPT quiescent, BMS не враховано (HW.15)):**
...
- Розряд LiFePO4 OK до −20°C — це межа КОМІРКИ. ⚠️ Розряд відсікає BMS, і заводський дефолт кандидатів JBD-класу — −20 ± 3…5 °C (у файлі SP04S028A — −10 °C), тож у цій зимовій точці Королева гасне вже при −15…−17 °C; поріг розряду перепрограмовується в межах даташиту комірки, і це чинне за будь-якого вибору SKU ([`queen_bms_shortlist`](protocols/hardware/queen_bms_shortlist.md) · [`00_07`](00_07_Action_Plan_Tracker) HW.16)
...
| Backend critical_fault? for T < −20°C | `GatewayTelemetryLog::LOW_TEMPERATURE_THRESHOLD` + ❄️ EwsAlert message — код ✅, але гілка **DATA-STARVED**: v2-пульс температури не несе (ARCH.54), тож вона не спрацює до HW.16-hardware (DS18B20 — цифровий 1-Wire: потрібні драйвер і GPIO, не АЦП; АЦП — лише для NTC-варіанту) + wire-розширення | **P0 (зима)** |
| Active cooling | Не потрібно при T_зовн ≤ +40°C | — |
| Heater для LiFePO4 | Опційно для T_зовн < −20°C deployments | P3 |

**Висновок:** теплова архітектура **проходить** для нормальних кліматичних зон Європи (Карпати, Полісся). Зимовий charge-protection обов'язковий: hardware-частина (DS18B20 + MOSFET) — 👤; backend-частина (`GatewayTelemetryLog#critical_fault?` детектує `temperature_c < −20°C` → ❄️ EwsAlert) — код написано, але вона **DATA-STARVED** і сама себе не ввімкне: `temperature_c` приходить NULL, доки немає датчика з драйвером (HW.16: DS18B20 по 1-Wire, не АЦП) + wire-розширення. ⛔ Не читати як «половина зимового захисту вже стоїть».
...
| 11 | **LTE-M / NB-IoT антена** | Wideband Cellular **700–2700 МГц** (покриває смуги Kyivstar ∩ SIM7070G: **B1/B3/B8**, згодом B28; ⚠️ «B7/B20», що тут стояли, хибні — B7 модем не підтримує, B20 в українських джерелах не знайдено, [`queen_antenna_shortlist`](protocols/hardware/queen_antenna_shortlist.md) §2.1). Зовнішня SMA, IP67. **БЕЗ суміщення з 868 МГц LoRa** — окремі чіпи (STM32WLE5JC vs SIM7070G), окремі RF-порти. Опційно: LTE+Active GNSS combo — додає GPS L1 1575 МГц для PPS time-sync SIM7070G; ⚠️ названі тут Taoglas FXUB63 (наліпна вбудовувана з U.FL, без IP-класу й без активного GNSS) і Pulse W3007 (у первинці не знайдено) цьому класу не відповідають — шортлист §1. «868/LTE-M dual-band» **виключено** (поганий VSWR на вузькому 868, низьке gain). ⚖️ **Модель — Taoglas `G30.B.108111` (прохідна screw-mount: гайка M12 з ущільненням, IP67, −40…+85 °C) з пігтейлом ≤ 0,5 м (ратифіковано founder 2026-09-26 за рекомендацією).** **Підстава:** єдиний кандидат, чиє кріплення саме є IP67-проходом крізь стінку, тож окремий гермоввід не потрібен; пігтейл ≤ 0,5 м вкладає тракт у межу втрат SIMCom (< 0,5 дБ на 700–960 МГц); у LTE пік антени нижчий за кожну стелю, під яку сертифіковано SIM7070G (FCC `2AJYU-8VC0001`; стель для наших смуг грант не має, тож порівняння — з найближчими американськими): запас 4,64 дБ на B1/B3, 2,69 дБ на B8, 2,07 дБ на B28. **Ціна:** 2G для Королеви закрито — GSM-стелі (2,03 / 3,85 dBi) нижчі за пік антени, тож «лише LTE» стало воротами передачі в прошивці ([`03_02 §4`](03_02_Queen_Gateway_Firmware)), а повернення 2G означало б іншу антену; вкорочений кабель — замовна опція (MOQ і строк не звірені); отвір M12 у стінці корпусу (поз. 13 не специфікована); середнє підсилення внизу −3,36 дБ, на 0,36 дБ нижче підлоги варіанта -HP. **Найслабша ланка:** стелі FCC виведено з RF-експозиції на 20 см за американськими лімітами, а ЄС міряє експозицію за ICNIRP — перерахунку й звіту CE(RED) немає; ворота прошивки не тримають сигналізацію реєстрації свіжого модема до першого OK, а їхня ознака не перечитується (ребут модема за незбереженого CNMP — [`00_07`](00_07_Action_Plan_Tracker) HW.31). Запасний — Siretta Delta 55 (диполь, від землі не залежить, але −30 °C). Розбір — [`queen_antenna_shortlist`](protocols/hardware/queen_antenna_shortlist.md) §2. | 1/2.5 | ✅ **Зафіксовано** (HW.31, 2026-09-26) — закупити |
| 12 | **LoRa антена** | 868 МГц **tuned** (вузькодіапазонна, 863–870 EU ISM), 5 dBi Fiberglass collinear omni. Зовнішня, IP67, **зʼєднувач N** (не SMA: трійки «5 dBi + SMA + IP67» у первинці немає). ⚖️ **Модель для заяви НКЕК — RAKwireless `RAKARG18` (5.0 dBi, IP67, N-male прямо на N-female гермоввід корпусу) + внутрішній LMR-240 ≤ 0.5 м → ЕВП ≤ 12.85 дБм на +10 дБм (ратифіковано founder 2026-09-26 за рекомендацією).** **Підстава:** заява вимагає моделі й кабелю ДО подання, а BOM-freeze чекав відповіді НКЕК — кільце розірвано розведенням «модель для заяви (пілот) ⊥ BOM-freeze (після відповіді)»; RAKARG18 — єдиний 5-dBi колінеар 868 МГц з первинкою та IP67, а прямий монтаж робить коаксіал у 6–10 разів коротшим за межу 3 м Положення № 87. **Ціна:** «SMA» тут і в заяві стало N; 480-мм склопластиковий виліт на стінці ABS/PC-корпусу потребує металевої монтажної пластини; постачання — RAK або ЄС-реселер, не TME. **Найслабша ланка:** відповідь НКЕК «п. 104» робить непридатними обидва 5-dBi кандидати, а єдиний IP67-кандидат ≤ 2 дБі (TI.84.A113) тримає 1.06 dBi лише без металевої землі (3.50 на 15 × 9 см) — виміру на нашому корпусі немає; паспорт RAKARG18 існує лише як вебсторінка без ревізії. Розбір — [`queen_antenna_shortlist`](protocols/hardware/queen_antenna_shortlist.md) §3.6. ⚠️ Названі тут доти Mobilemark OD8-868 і Taoglas ALL.4101 первинкою не підтверджені (сторінка виробника — 404 · пошук за P/N — нуль, 2026-09-25), а трійки «5 dBi + SMA + IP67» у первинці немає: обидва 5-dBi колінеари з паспортом мають N-type (кандидати, ЕВП і кабель — [`queen_antenna_shortlist`](protocols/hardware/queen_antenna_shortlist.md) §1, §3). Призначена для пробивання вологого лісу 150+ м до Soldiers. **НЕ використовувати dual-band/wideband** — VSWR > 2.5 на 868, втрата ~3-5 дБ EIRP → втрата покриття. ⚠️ З цією антеною провідну потужність TX Королеви знижено до +10 дБм — стеля ЕВП НКЕК 13.98 дБм (⚖️ founder 2026-09-24, [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security)); чи дозволені 5 dBi взагалі — п. 104 постанови ставить «до 2 дБі», рядок розсуджує НКЕК ([`00_07`](00_07_Action_Plan_Tracker) ARCH.24). | 1/2.5/3 | ✅ Архітектурно |
