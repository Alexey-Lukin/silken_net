...
| **Runtime memory-safety** | 🟡 PARTIAL (SEC.21) | ✅ `-fstack-protector-strong` (`arm-none-eabi.cmake`) інструментує canary на attacker-reachable парсери (LoRa-RX / AT-токенайзер жують untrusted байти в сирому C ДО MIC-чеку); CI flag-gate (`firmware_arm_build`) стереже прапор у ARM build-config — nm-на-.o false-fail'ить (`-strong` інструментує лише функції з addressable-locals); newlib резолвить `__stack_chk_fail` на лінку. Residuals → [`00_07`](00_07_Action_Plan_Tracker) SEC.21: власний reset-handler + HRNG-seed guard'а (fielded-quality) · MPU NX-stack/RO-code (bench-активація) |
