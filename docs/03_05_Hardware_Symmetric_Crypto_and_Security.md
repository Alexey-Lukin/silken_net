...
входить — унікальність (key, nonce) гарантує сам FC, а `Rails.cache` replay-guard (Solid Cache, SET NX 25 год, обмежений розміром)
...
- **Залишковий ризик / severity:** при живому якорі повтор (key, nonce) **неможливий за конструкцією** (інваріант I-HW). Імовірнісний ризик `N/2²⁴` на cold-boot лишається **тільки** на fallback-шляху: перше втілення до першого КЕНОЗИСУ або стабільно мертвий Flash-program — **Severity: MEDIUM на fallback, не baseline** (CTR-reuse дає лише витік `P1⊕P2` двох 14-байтних низькоентропійних сенсорних payload'ів, **не** компрометацію ключа; forge все одно потребує per-device key). Cold-boot'и рідкісні (drain ≈ сезонний). Epoch-край: 24-bit простір одноразовий — біля `0xFFFFFF` межа клемпиться, нову nonce-епоху відкриває лише ротація ключа (FW.17, §3.8); за life-budget край недосяжний (64× margin).
...
// nonce[12] ‖ Q=FW2_CCM_PLAINTEXT_LEN = 14), AAD — окремим Header; Size обох фаз — у БАЙТАХ.
...
HAL_CRYP_Init(&hcryp);
// Фаза 1 (payload): 14B sensor → 14B ciphertext (rev2.1)
HAL_CRYP_Encrypt(&hcryp, pt_w, FW2_CCM_PLAINTEXT_LEN, ct_w, 1000);
...
> **Примітка airtime:** 30-байтний пакет rev2.1 коштує ті самі **226.3 мс**, що й 28B rev2 — обидва в одному символьному блоці (27..30 Б), тож **+2B EMA rev2.1 їдуть задарма**, як і планувалось. Duty cycle — з двопорядковим запасом (розрахунок нижче). 🔴 Цей рядок доти казав «493.6 мс · SF10/DR2 · блок 28..31B» — числа LoRaWAN-детуру, не нашого тракту; виправлено 2026-09-11 ([`00_07`](00_07_Action_Plan_Tracker) FW.61). ⛔ **Межі блоку тут більше не переказуються** — вони в колонці «Блок» таблиці нижче, яку перераховує `ruby tools/firmware/lora_airtime.rb`: переказ уже раз пережив власне джерело. Детальний розрахунок + wire-budget ledger — нижче.
...
**Апаратна периферія:** AES-блок STM32WLE5 (`hcryp.Instance = AES`) — апаратне прискорення без залучення ядра Cortex-M4. Не потребує програмних крипто-бібліотек. **Підтримує обидва KeySize** через runtime re-init.
...
**Бюджетні факти (символьні блоки — колонка «Блок» airtime-таблиці нижче; окремого переліку меж тут немає, бо він уже раз пережив свою формулу — цитата межі поруч із власним висновком лишається):**
- 🔴 **Headroom після rev2.1 — НУЛЬ, а не «+1 байт безкоштовно».** 30B є останнім байтом свого блоку (27..30), тож 31-й коштує **+20.5 мс (+9 %)**. Доти тут стояло протилежне — прямий наслідок SF10-моделі, знятої 2026-09-11 ([`00_07`](00_07_Action_Plan_Tracker) FW.61): ledger є **меню, з якого обирають байти**, тож хибна межа тут дає не хибне речення, а хибний ХІД. Безкоштовний байт з'явиться лише разом зі зміною профілю — тобто ніколи в межах цієї ревізії.
- Top-байт старого FC32-поля був **завжди 0x00** (лічильник 24-бітний) — 1 безкоштовний cleartext-байт в AAD.
- **Ціну VPD-байта rev1→rev2 перераховано, не втрачено:** 24 Б і 28 Б лежать у РІЗНИХ блоках (22..26 ⊥ 27..30), тож той байт коштував **+20.5 мс airtime** і був оплачений свідомо. Доти тут стояло «+12 мДж/TX» — число з SF10-моделі й зі струму, що не належить жодному сценарію; у джоулях ціну рахує [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power).
- Біти, які ще можна реклемувати без нового байта: `mesh_ctrl.ttl:4` тримає max 15 hop при потребі 0..5 (1 біт вільний), `vpd_index` — НЕ резерв: до HW.32 його несе SEC.20 contract-звіт (`[reverted:1|id7]`, єдиний сигнал відкату в CCM-ері).
...
| HW.32 VPD-індекс (закрив double-booking байта 14 у 21B-плані: 03_01 §HW.32 і 03_02 §5а.2 обидва претендували на byte 14) | 1 B | ciphertext byte 19 (до BME280 — SEC.20 contract-звіт `[reverted:1\|id7]`) | ✅ (шкала — при калібруванні) |
...
- **FW.59 reset-cause / crash-forensics (кандидат наступної ревізії):** Soldier потребує 3-біт cause на дроті (Королевину половину ✅ відвантажено 2026-09-08 — era-invariant health-flags **bits5..7**, розкладка §2.2, механіка [`03_02 §7.1`](03_02_Queen_Gateway_Firmware); ⚠️ **не** bits4..7: bit4 несе бронь `QATT_HFLAG_CANARY` [SEC.21], і наїзд на неї тепер помилка компіляції). Consec-лічильник на дріт **не їде** — вісім кодів причини вичерпують поле; для Soldier це той самий розрахунок. Слот-кандидати з headroom вище: реклемований `mesh_ctrl.ttl`-біт / 31-й байт (⚠️ НЕ airtime-free: +20.5 мс, новий символьний блок — бюджетні факти вище); `vpd_index` кандидатом НЕ є — його до HW.32 несе SEC.20-звіт відкату. Рішення при CCM-wire-фіналі → [`00_07` — FW.59](00_07_Action_Plan_Tracker).
...
#### 🤖 Базлайн PHY і верифікація LoRa Airtime Budget (16B ECB · 28/30B CCM-ревізії · 21B Rails-розпаковка)

**Базлайн raw-LoRa P2P (SSOT профілю; компілятор бере ці ж номінали з `firmware/common/lora_phy.h`, парність тримає `ruby tools/firmware/lora_airtime.rb --check-canon`):**

| Параметр | Значення | Аргумент драйвера |
|---|---|---|
| Частота | 868.0 МГц | `Radio.SetChannel(868000000)` |
| SF | 9 | `datarate = 9` |
| BW | 125 кГц | `bandwidth = 0` |
| CR | 4/5 | `coderate = 1` |
...
⛔ **Не плутати з LoRaWAN-детуром Королеви:** 868.1/868.3/868.5 МГц і DR2 (SF10) — то канали й датарейт MAC-сесії Helium ([`02_05 §6.1`](02_05_Queen_Hardware_and_Starlink), ARCH.34), а не наш P2P-тракт; саме тому після кожного детуру `Radio_Reinit_RawLoRa_868MHz()` вертає PHY у рядки вище. 🔴 **Цей блок стояв тут із підписом «Silken Net default» і числами детуру** (868.1 · SF10) — тобто називав чужу модуляцію нашою, і з неї ж виводились «бюджетні факти» ledger'а вище. Спростовано обчисленням 2026-09-11 ([`00_07`](00_07_Action_Plan_Tracker) FW.61): SF і потужність задає ратифікований Сценарій C ([`02_03 §9.8`](02_03_BQ25570_MPPT_Nano_Power) — +14 дБм @ SF9), частоту тримає цей же профіль ([`02_05 §2.1`](02_05_Queen_Hardware_and_Starlink) її дзеркалить), і все збігається з тим, що програмує прошивка.

**Формула LoRa airtime (Semtech SX126x):**

```
T_symbol = 2^SF / BW = 2^9 / 125000 = 4.096 мс

T_preamble = (n_preamble + 4.25) × T_symbol = 12.25 × 4.096 = 50.18 мс

payload_symbols = 8 + max(ceil((8×PL − 4×SF + 28 + 16×CRC − 20×IH) / (4×(SF − 2×DE))) × (CR+4), 0)
  де PL = payload у байтах · CR = 1 (4/5) · CRC = 1 · IH = 0 (explicit header)
  DE = LowDatarateOptimize
```

⚠️ **`DE` не є нашим вибором і не є одиницею:** його виводить сам драйвер — `1` лише при (BW 125 кГц ∧ SF ∈ {11,12}) або (BW 250 кГц ∧ SF 12), інакше `0` (`radio.c`, `RadioSetRxConfig`/`RadioSetTxConfig`). При SF9/BW125 → **DE = 0**, тобто знаменник `4×SF`. Попередня редакція формули писала `4×(SF−2)`, тобто вшивала `DE = 1` у тракт, де кремній його не вмикає — і саме тому всі числа нижче доти були завищені.

**Розрахунок (перевідтворюється `ruby tools/firmware/lora_airtime.rb`; колонка «блок» = сусідні довжини, що коштують РІВНО стільки ж):**

| PL, Б | Символів | T_air, мс | Блок, Б | Що це |
|---:|---:|---:|:---:|---|
| 16 | 28 | 164.9 | 13..17 | ECB air-кадр — transitional, живий сьогодні |
| 17 | 28 | 164.9 | 13..17 | найменший не-control кадр ([`00_07`](00_07_Action_Plan_Tracker) SILENCE-1) |
| 21 | 33 | 185.3 | 18..21 | Rails-розпаковка (`N n c C n C C a4`) |
| 24 | 38 | 205.8 | 22..26 | CCM air wire-rev1 (історичний) |
| 28 | 43 | 226.3 | 27..30 | CCM air wire-rev2 (історичний) |
| 30 | 43 | 226.3 | 27..30 | CCM air **wire-rev2.1** — заморожений формат |
| 31 | 48 | 246.8 | 31..35 | rev2.1 + 1 байт |

> **Символьна квантизація (ключ wire-budget ledger вище):** межі блоків читаються з колонки «Блок» — окремої прозової таблиці меж тут свідомо немає, бо вона вже раз розійшлася з формулою. ⚠️ Два наслідки нижче МЕЖІ ЦИТУЮТЬ, і це свідомо: вони несуть рішення, а рішення без числа не читається. Правило вужче за «не згадувати»: **межу можна процитувати поруч із висновком, який на ній стоїть, і не можна винести в окремий перелік, що переживе таблицю.** Два наслідки: **(1)** 28B і 30B лежать в ОДНОМУ блоці (27..30), тож `+2 B` під EMA у rev2.1 коштували **нуль** airtime — висновок 2026-07-03 вистояв; **(2)** 🔴 **30B є ОСТАННІМ байтом свого блоку**, тож наступний байт коштує **+20.5 мс (+9 %)**, а не нуль. Ledger вище доти обіцяв «+1 байт безкоштовно» — то було наслідком SF10-моделі й спростовано разом із нею.

> 🔴 **Ефірний час — не лише ціна кадру, а й ЧАС, протягом якого радіо не можна чіпати** ([`00_07`](00_07_Action_Plan_Tracker) FW.61, 2026-09-27). `Radio.Send` асинхронний: він пише буфер і `SetTx`, а кадр летить ще 164.9 мс (16 Б) чи 226.3 мс (30 Б). Будь-яка наступна команда посеред ефіру його обриває — `SetRx` перемикає чіп на прийом, `Sleep` гасить, новий `Send` переписує буфер, з якого модем ще читає. Тому кожен кадр обох вузлів іде через ОДИН шов — `Lora_Phy_Send` (`firmware/common/lora_phy_apply.h`): він повертає ефір, який рахує сам драйвер (`Radio.TimeOnAir` над цим профілем — та сама формула, що в таблиці), плюс запас `LORA_PHY_TX_GUARD_MS` (`lora_phy.h`, підстава — там само: пробудження TCXO), і викликач чекає його перш ніж дати радіо будь-яку наступну команду. PANIC-кадр чекає власну преамбулу. Носії: `warn_unused_result` на шві і `make -C firmware/test lora_phy`, що червонить на голому `Radio.Send(` у будь-якому `main.c`. ⚠️ **Стеля:** запас модельний — точний кінець кадру дає TxDone-IRQ, який жодна прошивка ще не реєструє; його латентність міряє стенд (`[bench:radio-phy]`, [`00_07`](00_07_Action_Plan_Tracker) FW.61). Хост-сюїта ефіру не має (`Radio` — стаб), тож «зелене» тут судить форму шва й відсутність голого виклику, а не те, що кадр справді дожив до кінця.

**Duty cycle бюджет (EU868, смуга 868.0–868.6 МГц — «g1» старої редакції ERC 70-03: 1% duty cycle; чинні умови в Україні — постанова НКЕК № 361, [`certification_roadmap`](protocols/legal/certification_roadmap.md) §2):**

```
Duty cycle = T_airtime / T_period       (кадр 30B rev2.1, T_air = 226.3 мс)

  1 TX / годину:                 0.0063% ✅ (159× запас)
  1 TX / 10 хв (stress mode):     0.038% ✅  (27× запас)

  Стеля 1% на цьому кадрі = мінімальний період 22.6 с.

  Королева (16 Б по 164.9 мс, OTA-серія 8 КБ ≈ 123 с): ≤ 36 с у будь-яку
  годину тримає лімітер firmware/queen/tx_duty.h — FW.61, 03_02 §5.

  ⛔ Рядка «PANIC-бурст 5 хопів» тут БІЛЬШЕ НЕМА, і причин дві, кожна
     достатня: (1) mesh-естафета (вкл. panic-hop TTL=5) у CCM-ері МЕРТВА —
     star-only до ARCH.43 (flip-checklist (а) вище); (2) навіть у ECB-ері
     число було б не про цей кадр: PANIC їде з преамбулою 4 с (973 симв,
     02_03 §9.10), тож бурст вимірюється секундами, а не сотнями мс.
```

⚠️ **Duty cycle — НЕ єдина регуляторна вісь:** постанова НКЕК № 361 обмежує для SRD 868.0–868.6 МГц ще й **ЕВП — 25 мВт = 13.98 дБм** (ЕВП = провідна + підсилення антени − 2.15; первинка й рядки — [`certification_roadmap`](protocols/legal/certification_roadmap.md) §2), і два боки лінка рахуються ОКРЕМО, бо антени в них різні.

⚖️ **Потужність TX Королеви — +10 дБм провідних (РАТИФІКОВАНО founder 2026-09-24 за рекомендацією, [`00_07`](00_07_Action_Plan_Tracker) FW.61).** Проєктний профіль давав їй +14 дБм у 5-dBi антену ([`02_05 §7`](02_05_Queen_Hardware_and_Starlink)) = **16.85 дБм ЕВП ≈ 48 мВт**, майже вдвічі понад стелю. **Підстава:** Королева здебільшого ПРИЙМАЄ, а стеля ЕВП на прийом не діє — тож знижується провідна потужність, а не антена, і підсилення лишається на всьому uplink; +10 + 5 − 2.15 = **12.85 дБм ЕВП**, запас ≈ 1.1 дБ під незвірений допуск PA (+11 дБм лишали б 0.13 дБ). **Ціна:** −4 дБ на downlink (OTA · beacon · CMD); і профіль перестав бути спільним — потужність стала сталою РОЛІ (`LORA_PHY_TX_POWER_DBM_SOLDIER` ⊥ `_QUEEN` у `firmware/common/lora_phy.h`, шов бере її аргументом). Стелю тримає `_Static_assert` у тому ж заголовку — перевищення є помилкою компіляції в обох прошивках і host-тестах. **Найслабша ланка:** якщо наш рядок постанови — той, де антена «до 2 дБі» (п. 104), знижувати потужність недосить: потрібна інша антена, а assert цієї умови не бачить; рядок розсуджує НКЕК ([`00_07`](00_07_Action_Plan_Tracker) ARCH.24). Остаточне число — після виміру ЕВП на стенді ([`00_07`](00_07_Action_Plan_Tracker) FW.61). **Солдат** — власна бортова антена ([`02_01 §5`](02_01_Hardware_Architecture_and_BOM)), ще не обрана: на +14 дБм він вкладається в ЕВП лише при підсиленні ≤ 2.1 дБі ([`00_07`](00_07_Action_Plan_Tracker) HW.17). Доти «✅» вище стосується рівно duty cycle, нічого більше.

**Енергетичний вплив — дім числа [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power), не тут.** Ця сторінка віддає airtime; у струм і джоулі його переводить енергобюджет, і там же живуть `E_TX` та робоча точка `H`. ⚠️ Частки EDLC за один постріл там НЕМАЄ — доти вона стояла тут, порахована з хибних входів, і відновлювати її як ЧИСЛО не варто: рішення несе не вона, а добовий баланс Сценарію C.

🔴 **Тут стояла ВЛАСНА енерго-таблиця, і вона була другим домом грошового числа — з двома незалежними похибками.** Струм `87 мА` не належить ЖОДНОМУ канонному сценарію: [`02_03 §9.4`](02_03_BQ25570_MPPT_Nano_Power) дає +22 дБм = **118 мА**, [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power) дає +14 дБм = **23.5 мА** (`RFO_LP`, SMPS); 87 — число без провенансу, посередині між ними. Airtime при ньому був SF10-івський. Добуток двох похибок давав `142 мДж` і «2.0 % заряду» там, де сусідній канон-дім рахує той самий постріл у **рази** дешевшим. Знято без перерахунку: одне число — один дім ([`00_06 §2`](00_06_SSOT_Documentation_Standard)), а те, чого нема, не розходиться.

**Висновок:** перехід з ECB-кадру 16B на CCM 30B (wire-rev2.1) коштує **+61.4 мс airtime (+37 %)** — 164.9 → 226.3 мс; у джоулях це рахує [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power). Зміна key size 256→128 (ARCH.42) airtime **не змінює** взагалі (block size = 128 біт в обох; різниця лише в кількості раундів 14→10). Duty cycle лишається з двопорядковим запасом (вище). **LoRa airtime budget верифіковано: AES-128-CCM 28B (wire-rev2) схвалено founder'ом 2026-06-12**, і поправка моделі 2026-09-11 цього висновку не зачепила — `+2 B` під EMA у rev2.1 і далі коштують нуль, бо 28B і 30B лежать в одному символьному блоці (27..30, таблиця вище). ✅ ⚠️ Числа поза таблицею на цій сторінці стоять у ЧОТИРЬОХ місцях — цей абзац, duty-cycle-блок, примітка airtime вгорі й bullet ledger'а про headroom, — і ЖОДНЕ з них `--check-canon` не читає. Змінюючи профіль, перечитай усі чотири; гейт стереже лише таблицю.
...
Сторожовий пес панічного каналу до приходу повного FW.2 CCM. Counter інкрементується перед кожним `Trigger_Emergency_LoRa_TX`, упаковується BE у вільні PAD-байти (14..15), персистується у packed `RTC_BKP_DR0[31:16]` поряд з `acoustic_events[7:0]` — **без використання нових RTC слотів**. Cold-boot resync через HRNG (range 0x0001..0xFFFF) уникає колізії з ще-не-протухлими replay-ключами `Rails.cache` попереднього втілення (probability ≈ 1/65536). На брауноуті `HAL_PWR_PVDCallback` (ARCH.21) теж зберігає packed DR0.
...
```
+--------+--------+--------+--------+--------+--------+--------+-----------+--------+
|  0x99  |ChunkIdx|ChunkIdx|TotalCh |TotalCh |  Len   |  Len   | Bytecode  |  CRC   |
| (OTA)  |  [MSB] |  [LSB] |  [MSB] |  [LSB] | [MSB]  | [LSB]  |  payload  |  [2B]  |
+--------+--------+--------+--------+--------+--------+--------+-----------+--------+
...
|----------|---------------------|--------------------------------------|------------------------|
| **Носій wire** | телеметрія + panic (30B CCM uplink rev2.1) | увесь downlink-broadcast (`0x99/0x9A/0x9B/0x9C/0x9D/0x9E`) + uplink-запити `0x55/0x56/0x57` (16B ECB) | CoAP batch/downlink (AES-256-CBC) |
...
3. **Cold-start з 0 В — штатний дизайн-шлях, не аварія, і цей висновок ВИСТОЯВ, попри те що його підстава впала:** порівняння «EBFC ≥500 мВ > 330 мВ порогу» більше не діє — `VIN(CS)` = 600 мВ typ / 700 мВ max ([`02_03 §1.1`](02_03_BQ25570_MPPT_Nano_Power), [`00_07`](00_07_Action_Plan_Tracker) HW.46). Вистояв він тому, що спирається на ІНШЕ: cold-start із нуля лишається штатним шляхом незалежно від того, чим саме його забезпечено (одним EBFC чи EBFC+preboost), а shipping-mode не торкається ані порога, ані драбини мітигацій `R_int` (HW.13, [`02_03 §1.5`](02_03_BQ25570_MPPT_Nano_Power)).
4. **Вібраційний wake ІСНУЄ** — пʼєзо-EXTI на PA0 поруч із RTC WUT ([`03_01 §1.10`](03_01_Firmware_Lifecycle_and_DMA); вектор `EXTI0_IRQHandler` стоїть у `soldier/main.c` з 2026-09-09, [`00_07`](00_07_Action_Plan_Tracker) ARCH.102), тож «прокинеться від вібрації у коробці» — не мертва преміса. Висновку вона не валить: у коробці цей wake нічим живити (п. 2), а поки factory-заряд ще є, трусіння в дорозі лише пришвидшує той самий розряд.
...
- **Active** (1.5 мс): 14 мА × 3.3 В = 46 мВт → ~69 мкДж/пакет на VOUT (~79 мкДж з VSTOR, η_buck 0.88). Проти Сценарію C: **≈0.5% LoRa-TX** (+14 dBm SF9, 12.79 мДж на VOUT на обраному фронтенді — ⚠️ ECB-ерний кадр 16 Б; під CCM 30 Б знаменник 17.55 мДж, тож частка лише падає, [`00_07`](00_07_Action_Plan_Tracker) ARCH.8) і **≈0.2% повного active-циклу** (37.25 мДж з RX-вікном і TCXO, [`02_03 §9.4`](02_03_BQ25570_MPPT_Nano_Power)); у годинному вимірі (1 TX/1.95 год) ≈0.04 мДж/год — дрібно, але запасу, у який це вписувати, Сценарій C більше не має: його точка беззапасна ([`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power)). Проти 0.47 F EDLC (≈7 Дж) — не «вбивця іоністора». Per-packet active-енергія SE **мала** — і саме тому вона **не** головний критерій вибору ролі (див. підрозділ нижче). ⚠️ Не рахувати % проти «TX ~39 мДж» — то E_TX @ +22 dBm зі *списаного* сценарію ([`02_03 §9.4`](02_03_BQ25570_MPPT_Nano_Power)).
- **Sleep: 150 нА — НЕ нехтовно** (інверсія попередньої подачі, що спиралась на застарілий бюджет ~1.5 мкА): у канонічному Сценарії C (300 нА RTC-only) always-on SE = 150 нА × 3.3 В / η_buck 0.50 (нанострумовий режим) ≈ **3.6 мДж/год з VSTOR — а запасу Сценарій C не має зовсім (беззапасна точка, [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power))** → баланс ≈ −3.6 мДж/год на каденції C, supercap повільно розряджається. **Вимога-висновок:** SE сидить за load-switch гейтом (патерн TPS22860, як BME280 — [`02_01 §3.4`](02_01_Hardware_Architecture_and_BOM)); живлення лише в active-вікні. Сумісно з обома ролями: per-packet вмикає SE на кожен wake, provisioning-only — лише при ротації/фабриці. З гейтом sleep-внесок ~0.
- Числа — ATECC608B (порядок величини); **SE050 — paper-verified 2026-07-02** (таблиця datasheet-verify нижче): active той самий клас (AES-coproc 6.5 мА; PK/Ed25519 14.4 мА), але **сон СУТТЄВО гірший за ATECC-проксі — Deep-Power-Down 3–5 µA (НЕ 150 нА, ~20–30×)**, PD-I2C 450 µA → інверсія-висновок вище ПІДСИЛЮЄТЬСЯ: навіть DPD (≈10–16 µВт) більший за ВЕСЬ надлишок заряду Сценарію C (`E_gen − E_sleep` ≈ 6 µВт, [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power)), тобто баланс мінусовий і без жодного TX, load-switch = **повне зняття живлення**, не DPD-надія; головне eval-питання стає **cold-boot заряд** (датащит його не специфікує). Silicon-confirm при eval-kit ([`00_07` — SE050-MIGRATION](00_07_Action_Plan_Tracker)).
...
>
> ⛔ **Не відтворювати тут крокову послідовність провіжну — виміряно й відкинуто.** Дві копії розійшлись на найдорожчій осі, яку тільки має цей тракт: **чи AES-ключ їде мережею**. Дім відповідає ні («Жоден ключ не летить мережею»; провіжн — **host-side one-pass [FW.54]**, без network round-trip; `SLOT 0 (AES LoRa)` пост-SEC.14 писатись не мусить — код-лаг емісії ще живий, SE050-MIGRATION), і §3.2 цього ж файла каже те саме `(Zero-Trust: aes_key НЕ повертається у відповіді)`. **Копія крокового опису тут не має чим лишатись правдивою, тож її місце — у [`03_06 §1`](03_06_Factory_Flashing_and_Key_Provisioning), і лише там.**
**Дорожня карта:**
...
- [x] 🤖 (SEC.14, 2026-06-12) Cross-check проти Сценарію C ([`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power)) — точні %: active ≈0.5% TX / ≈0.2% циклу / ≈3% годинного запасу (підтверджує «малий»; ⚠️ з обраним фронтендом запасу в точці C немає — [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power), тож висновок про load-switch нижче лише міцнішає); **знахідка-інверсія**: always-on sleep 150 нА ≈ 3.6 мДж/год > запас Сценарію C → **load-switch гейт SE обов'язковий** (Power impact вище; стосується обох ролей)
...
**ACK без окремого каналу:** backend ротує `HardwareKey` при dispatch'і команди (новий ключ; старий → `previous_aes_key_hex`, Dual-Key Grace 03_06 §2) → перший uplink, що декриптнувся новим ключем, = неявний per-device ACK (`clear_grace_period!`). ⚠️ Для ДЕРЕВА ця машинерія не зашита: `clear_grace_period!` кличе лише CoAP-шлях Королеви (`UnpackTelemetryWorker#attempt_decryption`), а CCM-шлях дерева (`process_ccm_chunk`) бере тільки поточний ключ — без відкату на `previous_aes_key_hex` і без закриття grace; передумова фліпу — [`00_07`](00_07_Action_Plan_Tracker) FW.17. Cluster-wide ротація = батч per-device команд; «cluster ACK» = усі grace-вікна кластера закриті.
...
for (uint8_t i = 0U; i < 4U; i++) {
    if (HAL_RNG_GenerateRandomNumber(&hrng, &batch_iv[i]) != HAL_OK) { hrng_ok = 0U; break; }
}
if (!hrng_ok) {
    // [SEC.12] увесь IV — ключова PRF (дім: firmware/queen/coap_iv.h):
    // HMAC-SHA256(coap_key, label ‖ uid_hash ‖ unix_ts ‖ flush_seq ‖ tick)[0:16] —
    // унікальний І непередбачуваний без ключа; host-тести test_encryption.c
    coap_fallback_iv((uint8_t *)batch_iv, (const uint8_t *)coap_key, sizeof(coap_key),
...
| Fallback | `coap_fallback_iv(…)` — HMAC-SHA256 під `coap_key` (`coap_iv.h`, SEC.12): увесь IV однією PRF, унікальний І непередбачуваний без ключа (§HRNG Fallback) |
...
Flush_Cache_To_Rails():
  ...
  7. AT+CCOAPSEND → SIM7070G → CoAP PUT /telemetry/batch/<queen_uid>
         ...
         │ [header:17][IV:16][ct][sig:64] — §2.2) via SIM7070G → Rails
         ▼
RAILS BACKEND
...
| Emergency TX format | `firmware/test/test_soldier_logic.c` (`test_panic_*`) | ✅ |
| HRNG fallback behavior | `firmware/test/test_encryption.c` (`test_coap_fallback_iv_*` — parity з OpenSSL) + `test_queen_logic.c` (`test_hrng_*`) | ✅ |
...
**Загальний статус:** host-based тести проходять (`make -C firmware/test`). HRNG fallback, panic-кадр і CCM-флоу покриті host-тестами (рядки вище); відкритою лишається лише кремнієва звірка CCM (`CRYP_AES_CCM` HAL на STM32WLE5 — bench).
...
| **HRNG Fallback** | ✅ Harden (SEC.12) | `coap_fallback_iv` (HMAC-SHA256 під `coap_key`, `coap_iv.h`) — унікальність І непередбачуваність; §4.2 |
