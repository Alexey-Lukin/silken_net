...
| **Runtime memory-safety** | 🟡 PARTIAL (SEC.21) | ✅ `-fstack-protector-strong` (`arm-none-eabi.cmake`) інструментує canary на attacker-reachable парсери (LoRa-RX / AT-токенайзер жують untrusted байти в сирому C ДО MIC-чеку); CI nm-gate (`firmware_arm_build`) стереже присутність `__stack_chk_fail` (newlib резолвить). Residuals → [`00_07`](00_07_Action_Plan_Tracker) SEC.21: власний reset-handler + HRNG-seed guard'а (fielded-quality) · MPU NX-stack/RO-code (bench-активація) |
