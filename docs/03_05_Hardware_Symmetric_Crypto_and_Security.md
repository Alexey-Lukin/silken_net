...
- **E.63 EMA-delta_t (точний stateless метаболічний DCI):** `m(wire_dT)==GP` потребує, щоб wire ніс САМЕ EMA-згладжений delta_t, з якого рахувались GP (wire=raw, GP=f(EMA) — розрив [`03_01 §13.6`](03_01_Firmware_Lifecycle_and_DMA)). **Рішення V3+ (founder 2026-07-03): вилка вирішується НА ФЛІП-ДНІ, не зараз** — фізика E.63 не підтверджена (пороги 600/7200 placeholder; реальний recharge може сплющити шкалу — FW.49-блокер), а точний DCI = parity-integrity (після CCM+MIC консистентну брехню обома полями він не ловить, лише баги/десинк — той самий клас, що z-DCI FW.31). Зашивати парність поперед фізики = передчасна конкретика. Кандидати на фліп-день: «+2B EMA-поле» (28..31B — той самий символьний блок, airtime-задарма; transient як device_z — рекомендований) vs «замінити семантику dT» (0 байтів, але жертвує raw-діагностикою 13.6) vs «лишити observational». Гейт (г) фліп-чекліста вище не дасть проґавити ([`00_07` — E.63](00_07_Action_Plan_Tracker)).
...
| **Унікальність ключа** | ✅ Per-device HKDF (FW.1 + ARCH.42) | LoRa: `HKDF-SHA256(MASTER, uid, "silken-aes-128-lora-key")`; CoAP: `HKDF-SHA256(MASTER, uid, "silken-aes-256-device-key")` — domain separation 03_06 §2 |
