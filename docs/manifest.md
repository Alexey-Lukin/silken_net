...
Any divergence between device-computed Z and server-recomputed Z raises a fraud flag. We call this **Dual Computation Integrity**. Today the categorical check is live; the numeric tolerance band (an absolute ε, not a percentage) ships feature-flagged off until the wire carries raw device-Z, and minting is deliberately optimistic — the enforcement arm is ex-post reconciliation and clawback, not a pre-mint wallet freeze. We say this plainly because the distinction between *built* and *designed* is the whole point of §5.

#### 2.4. Acoustic Biodiversity Diagnostics (Edge AI)

A piezoelectric pickup mounted on the trunk and sampled at 16 kHz via DMA feeds a convolutional classifier through a CMSIS-DSP log-mel front-end. The landed inference runtime is a self-contained integer forward pass — no TFLite-Micro, no CMSIS-NN — sized to roughly a kilobyte rather than the ~16 KB tensor arena a textbook ESC-CNN would demand, because at this power and RAM budget that textbook arena does not physically deploy. The model discriminates between **anthropogenic threat signatures** (chainsaw, vehicle ingress), the **faunal soundscape** (a proxy for biodiversity), a low-frequency **water-stress proxy**, and silence. (True ultrasonic acoustic-emission detection of xylem cavitation — physically 25–150 kHz, Tyree & Dixon 1983 — is a roadmap item requiring a dedicated high-rate channel, not the current 16 kHz audible chain; the water-stress class today is a low-frequency structural proxy, not the ultrasonic AE signal itself.)
...
- **Oracle consensus:** the verified result is then dispatched to a **Chainlink DON** for cross-validation against external climate oracles (dClimate, NASA FIRMS) before being relayed to the minting contract. The callback carries an HMAC-SHA256 signature verified by timing-safe comparison, and replay protection is DB-atomic rather than nonce-based: a callback is only honoured against a row still marked `dispatched`, so a replay updates zero rows and is refused outright.
