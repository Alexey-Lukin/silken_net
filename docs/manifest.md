...
- **Category B (Force majeure / acts of nature)** — lightning, wildfire of natural origin, severe drought, earthquake. Slashing is **disabled** and funds are frozen. A payout is *not* automatic: it requires a second, independent confirmation of the peril, and **today no peril has one wired** — the satellite fire feed was dropped in September 2026 and not replaced, drought has no satellite oracle at all, and the NOAA quake feed is specified but not connected. Every Category-B case therefore routes through a human Field Audit; the Etherisc parametric rail exists in design, not in operation. The insurance pool is replenished by a 2% Dynamic Tax on minting whenever its balance falls below 100,000 SCC.
