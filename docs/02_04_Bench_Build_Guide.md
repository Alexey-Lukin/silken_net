...
> або майбутній реальний EBFC — гейт `V_OC` §6 крок 4), а решта вузла — production-компоненти
...
- **Гібрид живлення:** front-end — legacy-харвестер (44 мВ AA-симулятор → LTC3108,
  §3.1) або майбутній EBFC напряму в BQ25570 (гейт `V_OC` — §6 крок 4); решта вузла — production-компоненти
...
| Джерело рампи нижче 0.8 В | лабораторне джерело з кроком ≤ 10 мВ АБО 10-обертовий потенціометр від елемента 1.5 В + резистор 4.7 кΩ | 🛒 | вимір `VIN(CS)` партії BQ25570 (HW.46, RUNBOOK §6 `[bench:bq-coldstart]`): PPK2 як джерело нижче 0.8 В не йде, а рампа стартує з 0.40 В |
| Генератор сигналів | синус 2–8 кГц із кроком ≤ 1 Гц + режим пачки; для кривої спрацювання DNP ще й 50–200 Гц | позичити | `[bench:acoustic]` (HW.30 · HW.11): прилади, rig і три осі — аркуш [`acoustic_bench_sheet`](protocols/hardware/acoustic_bench_sheet.md); осцилограф там потрібен 2-канальний (драйв + приймач), збудник — ЗП-3 з Блоку 6 |
...
| Компонент | Модель | Статус | ⚠️ Кусає |
|---|---|---|---|
| LoRa PHY | радіо SX126x на кристалі (у LoRa-E5) | 🛒 (їде з модулем) | — |
| Антена | 868 МГц (SMA/u.FL до модуля) | 🛒 | 🔴 **ОБОВ'ЯЗКОВА ПЕРЕД ЖИВЛЕННЯМ** — PA згорить без неї |

**Bench-carrier комплект (🛒 разом):** 2× LoRa-E5 mini (+1 недоторканий резерв — нижче) · ST-LINK-V3MINIE · BME280.
...
> LTC3108/трансформатор **не стоять у production за замовчуванням** (при `V_OC` ≥ 700 мВ EBFC живить
> BQ25570 напряму; 600–700 мВ — populate, ⚖️ 2026-09-10), і лишаються **DNP cold-start fallback** ([`02_03 §1.5`](02_03_BQ25570_MPPT_Nano_Power)).
...
2. `VIN_DC` BQ25570 ← напряму вихід EBFC Gen 2.0 ([`01_03`](01_03_EBFC_Enzymatic_Bio_Fuel_Cell)) — лише якщо гейт `V_OC` кроку 4 пройдено; ⛔ «>500 мВ» — ера порога 330 мВ, не діє.
...
| Компонент | Модель | ⚠️ Кусає |
|---|---|---|
| 868 антена | **tuned** 5 dBi fiberglass omni — RAKARG18 (зʼєднувач N) + LMR-240 ≤ 0.5 м, модель для заяви НКЕК ⚖️ 2026-09-26 ([`02_05 §7`](02_05_Queen_Hardware_and_Starlink) поз. 12); на стенді LoRa-E5 mini має SMA-K — перехід N ↔ SMA | 🔴 НЕ dual-band (VSWR>2.5 @868 → −3-5 дБ); 🔴 антена ПЕРЕД живленням |
...
Пʼять конденсаторів на шину VBAT модема обовʼязкові — без них піковий струм SIM7070G просаджує шину й модем ресетиться. **Номінали, part-номери, ESR і відстані — [`02_05 §2.2.1`](02_05_Queen_Hardware_and_Starlink)** (component-дім за реєстром [`00_06 §2`](00_06_SSOT_Documentation_Standard)); ⛔ тут вони НЕ дублюються — власна Мета цього документа прямо каже, що production-специфікації компонентів (part-номери, ціни, DC-bias, DNP-стратегія) сюди не переносяться.

⚠️ **Bench-специфічне, чого в домі немає і що лишається ТУТ:** на макетці паразитна індуктивність провідників гірша за PCB-доріжки, тож конденсатори садять максимально близько до VBAT-піна breakout-плати — інакше tank не встигає віддати заряд у пік, і симптом читається як «модем помирає на `AT+CFUN=1`», а не як проблема живлення.
