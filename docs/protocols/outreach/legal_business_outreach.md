...
> **Правило чесності:** імена — ТІЛЬКИ ті, що називає канон; де виконавця немає — «TBD, не контактовано». Аблязов/Гедз/Ус названі каноном, але контакту ще НЕ БУЛО (UNI.14 — КОНТАКТУ не було (⚠️ STAGE пункту `🟡`, бо машинна половина зроблена; «Не почато» тут про зустріч, не про пункт)) — це адресати, не домовленості.
...
|---|---|---|---|---|---|---|
| `legal/securities_review.md` | Блоки 2–4: **Аблязов Д.Е.** (к.ю.н., госп. право — [`00_02 §1.5`](../../00_02_Academic_Integration_and_IP.md)); Блоки 1+5: крипто/securities-юрист **TBD** ([`00_02 §4.2`](../../00_02_Academic_Integration_and_IP.md)) | платну консультацію за готовим питальником (fact-pattern F1–F15 верифіковано проти коду) | **2 меморандуми** (RWA vs Лісовий Кодекс · SCC-класифікація UA/MiCA) + open-license/AF3-review + **securities-ВИСНОВОК** (не присуд: напрямок ратифіковано 2026-08-28, у юриста купуємо кваліфікацію as-built) → рішення про залишок product-redesign | BIZ.22 (гейт Web3 mainnet) · BIZ.11 (RWA-нога) · STK.3 · BIZ.20 Phase-2 · UNI.20 (IP-пункт MoU) | UNI.14-контакт (Аблязов-половина); вакансія юриста (Блок 1) | н/о |
| `legal/msa_skeleton.md` | Аблязов + той самий крипто-юрист TBD | юр-ревю каркаса + заповнення `[⚖️ TBD]`-полів + фіналізація | підписний B2B MSA (counterparty = ActiveBridge, за BIZ.20-присудом) | перший B2B-підпис | BIZ.20 ✅ · securities-ВИСНОВОК юриста (BIZ.22, не присуд — напрямок ухвалено 2026-08-28) + меморандуми UNI.16 ПЕРЕД підписом · BIZ.21-CoI · BIZ.18-числа · product-redesign (Consistency Gate — §2 п.7) | н/о |
...
| `legal/tax_posture_ua.md` | **Облікова половина: Гедз М.Й.** (фін. облік криптоактивів — [`00_02 §1.5`](../../00_02_Academic_Integration_and_IP.md)); **податкова половина: фахівець TBD** ([`00_02 §4.2`](../../00_02_Academic_Integration_and_IP.md) — ЧЕТВЕРТА вакансія) | облік: класифікація активу + момент визнання; податки: пріоритет-питання a/c + **90%-тест Дія.City** — ⚠️ **у СЄУ цю половину закрити нема кому** | tax-позиція: фізособа/ФОП vs ТОВ · токен-емісія · фіат-ПДВ → вхід у BIZ.20 Phase-2 | 90%-тест-чекбокс BIZ.20 · закриття UNI.14 | UNI.14-контакт (облік); **вакансія податкового фахівця** (податки) — ⊕ ПДВ-питання має обхід: власна ІПК ДПС, фахівця не потребує | н/о |
| `legal/trademark_brief.md` | повірений УкрНОІВІ — **особа TBD** + TISC (опційно; консультативний, САМ не подає — [`00_02 §4.1`](../../00_02_Academic_Integration_and_IP.md)) | clearance-search + подача 3 марок: SilkenNet 9+42 · GaiaNexus 42 · SCC 9 (+42 за порадою повіреного; кл. 36 — ⚫ знято для SilkenNet, ⏳ для SCC після юр-висновку, ⚖️ 2026-09-22 — [`trademark_brief`](../legal/trademark_brief.md) §1.2) ([`00_02 §5`](../../00_02_Academic_Integration_and_IP.md)) | **пріоритет-дата** (одразу при подачі) → свідоцтва ~18 міс | UNI.3 TDCommons-splash (™ ПЕРЕД розкриттям — [`00_02 §4.3`](../../00_02_Academic_Integration_and_IP.md)) | немає: заявник = фізособа (знято BIZ.20-присудом) | ~15–24 тис. грн зборів (перераховано після ⚖️ 2026-09-22, [`trademark_brief`](../legal/trademark_brief.md) бюджет Phase 0) + ~5–10 тис. грн повірений 🟡 |
...
**Гроші одним поглядом:** єдина витрата, доступна і доцільна ЗАРАЗ, — ™-подача (~20–34 тис. грн all-in: збори ~15–24 після ⚖️ 2026-09-22 + повірений ~5–10 🟡). Решта — тригер-gated: поліси на B2B-signing, методолог на реальний ліс, SOC 2 на запит покупця. Вартість юр-консультацій (Аблязов · крипто-юрист · carbon-юрист · GDPR-юрист) ніде в пакеті не оцінена — чесне «н/о».
...
2. **[`00_07`-прямий]** UNI.14 = **юр-хаб**: один воркшоп Аблязова фанається в BIZ.20-carve-out · BIZ.2 · BIZ.15 · UNI.16 · STK.3 · **BIZ.17** (юр-review CDA/NDA-каркаса — під лаб-MoU ЧНУ/ЧДТУ: UNI.18 ⚫ passive · UNI.10) · **UNI.20** (IP-пункт у ЧНУ-MoU) · **BIZ.22** транзитивно через UNI.16. ⛔ **Перелік не скорочувати:** доти тут стояли перші пʼять, а рядок «Воркшоп Аблязова» в `00_07 ⛓️` — дім цього набору — прямо каже, що занижений перелік означає пакети, лишені вдома.
...
7. **[артефакт msa_skeleton §0]** Products-Contract Consistency Gate: каркас пише securities-safe мову, але КОД частково наздогнав: роль `investor` перейменовано на `subscriber` 2026-08-28, `current_yield_performance` знято 08-14; колонку `total_funding` перейменовано на `total_service_fee` 2026-09-25; лишається сама МЕХАНІка (F1–F10) → реальний підпис MSA лише ПІСЛЯ product-redesign, обсяг якого дасть securities-ВИСНОВОК юриста (BIZ.22; присуд 2026-08-28 обрав лише напрямок). **[наш висновок]** Інакше contract-vs-product розбіжність, яку юрист покупця побачить на DD.
8. **[`00_07`-прямий]** STK.3 заблоковано на UNI.16 + UNI.14; кандидат-канал = Спрягайло (ННІ права ЧНУ) + Аблязов.
9. **[`00_07`-прямий]** BIZ.11: гейт — правова природа запису (UNI.16-RWA-меморандум) + securities-ознака «реєстрація реального активу» (BIZ.22); «після BIZ.2» — не той корінь (переформульовано 2026-07-26), BIZ.20 із ланцюга випав; land-access — STK.1.
...
- **P2 — UNI.14-контакт СЄУ** (лист Аблязову — текст: [`owner_meeting_briefs`](owner_meeting_briefs.md) §«Лист першого контакту», founder не бачив): відкриває хаб п.2 — без нього стоять усі пакети, перелічені там.
...
- **P5 — internal-хвости**: ✅ обидва є — DCO у `CONTRIBUTING` (гейт `dco_check`) і чернетка carve-out угоди ([`ip_carve_out_draft`](../legal/ip_carve_out_draft.md)) до воркшопу.

**Серіалізовано (НЕ починати зараз — тригер названо):**

- **S1** брокер/поліси → тригер = перший B2B-signing (п.5 §2).
- **S2** MSA-фіналізація → після securities-ВИСНОВКУ юриста (BIZ.22) і меморандумів UNI.16 + product-redesign (п.7 §2).
