...
> **Статус: робоча чернетка, не канон.** Дім механіки i18n — [`04_04 §12`](../../04_04_Phlex_UI_and_Tailwind); стан роботи — [`00_07`](../../00_07_Action_Plan_Tracker) I18N.1, **§🗄️ Архів із 2026-08-21**: residual «глосарій domain-термінів поруч із локалями» закрито разом із пунктом, тож цей файл більше не має відкритого власника — він **живий довідник**, який читають ПЕРЕД авторингом, а не незакінчена задача (звірено 2026-08-27, DOC-T.93). Сестринський документ — [`translation_audit_2026_07.md`](translation_audit_2026_07.md) (перший лінгвістичний аудит, 136 файлів ключ за ключем); цей файл **не дублює** його зміст — лише посилається там, де факти перетинаються.
>
...
`contracts/lt.yml:47` (`burn_points: Sukauptų SCC **deginimas**` — ⚠️ цитата оновлена 2026-09-04: доти тут стояло `Sukauptų taškų`, тобто ОДИНИЦЯ, спростована [SLASH-1] ще 08-29; саме спостереження про префікс живе, бо `deginimas` без «su-» лишилось) — без префікса «su-», тоді як решта корпусу консистентно тримає «su**deg**inimas»/«su**deg**inta»/«su**deg**inti» (`trees/lt.yml:9,22-23`, `alerts/lt.yml:19-20,34`, `dashboard/lt.yml:5`, `reports/lt.yml:40`). Низька гострота (той самий корінь, лише відсутній перфективний префікс) — не в топ-5, але вартий рядка для повноти.
