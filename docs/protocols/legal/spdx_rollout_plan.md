...
| Scope-питання | ✅ **ЗАКРИТО founder-присудом 2026-07-26** — усі три «так»: `config/` (крім `*.enc`) · `terraform/`+`subgraph/` · `spec/` (§6) |
| Рекомендація | **DEFERRED** (Phase 7) — лишається розмір diff'у; scope більше не блокує, технічного блокера немає (§6) |
...
- `tools/cad/{src,tests}` (.cs+.sh) ≈ 25 — ⚠️ **більше НЕ сіра зона:** ID ратифіковано [DOC-T.47] = AGPL-3.0-or-later (§2.3, §6); лишається сам прохід, founder-рішення не потрібне
- `terraform/` (.tf+.sh) ≈ 14
...
2. ~~3 нерозвʼязані scope-питання~~ ✅ **ЗАКРИТО founder-присудом 2026-07-26 — усі три «так»:** (a) `config/` — **включати** (крім `*.enc`, які не чіпаємо ніколи: initializers/routes = наш авторський код, локалі несуть наш контент, YAML тримає `#`-коментар); (b) `terraform/`+`subgraph/` — **включати обидва** (subgraph-mappings = справжні програми, HCL = авторська інфра-логіка; 16 файлів — найдешевше дерево); (c) `spec/` — **включати** (однорідність дешевша за виняток: інакше 522 файли, найбільша купа в репо, читаються SCA-тулами як unlicensed). *(Колишнє питання про `tools/cad/` закрито ратифікацією DOC-T.47 — AGPL для нашого коду; звідси 4→3.)* Дім присуду → [`00_07`](../../00_07_Action_Plan_Tracker.md) UNI.3.
