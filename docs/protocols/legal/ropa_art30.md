...
> **⏳ Станом на 2026-08-29.** Спирається на реальну схему БД і на зовнішнє право — обидва рухаються незалежно від цього документа; перед використанням звіряй колонковий рівень проти [`04_01 §11`](../../04_01_Data_Models_and_Entities.md) (One-Home) і правовий рівень з юристом. ⊕ **Дата — ПІДЛОГА, не остання звірка:** ніщо тут не молодше за неї, але окремі твердження можуть нести ПІЗНІШУ звірку з первинкою — тоді дата стоїть при самому твердженні ([`legal_registry §2`](legal_registry.md)).
...
| **(d) Отримувачі** | Внутрішньо (сам застосунок, console-читання глобального ланцюга). Для **money/MRV-записів** (`archive: true`, MRV.1) — додатково **Pinata** (`api.pinata.cloud`, IPFS/Filecoin pinning-шлюз, `Filecoin::ArchiveService`) — 🔴 **публічна децентралізована мережа, не контрактний процесор** (див. §6). ⚫ **ПЕРЕДАЧА ЗУПИНЕНА 2026-09-07 — activation-gate, ⚖️ founder:** `FILECOIN_API_KEY` знято, `Filecoin::ArchiveService.configured?`=false, тож жоден із трьох enqueue-сайтів не спрацьовує і **нових даних до Pinata не йде**. Рядок лишається в реєстрі, а не знімається — за прецедентом Telegram ([`b2c_tos_privacy §D.4`](b2c_tos_privacy.md), 09-06): канал знято, отже рядок ПЕРЕКВАЛІФІКОВАНО, не видалено. Дві підстави тримати його: вже запінене **незнищенне за природою IPFS**, і діяльність відновиться за названим тригером ([`00_07`](../../00_07_Action_Plan_Tracker) INF.22 — перші реальні money-tx). 🔴 **Доти тут стояло «і теж відсутня в `b2c_tos_privacy §D.3`» — це було НЕПРАВДОЮ 18 днів:** §6 цього ж файлу рядком нижче каже «✅ усі три ВНЕСЕНО», і джерело підтверджує — рядок Pinata стоїть у §D.3 із позначкою «Додано RoPA-свіпом 2026-08-20». Тобто документ суперечив сам собі, а зовнішній чинний стан бачив лише той, хто пішов за вказівником. Пінується `content_attrs` (id/org_id/action/`chain_hash`/auditable-посилання/`created_at`) + **`metadata` (jsonb) цілком** + добове зведення телеметрії кластера. `ip_address`/`user_agent` напряму НЕ пінюються (лишаються лише всередині `chain_hash` — SHA-256-дайджест, необоротний), а зміст `metadata` з 2026-08-25 має **оголошену стелю** (`AuditLog::ARCHIVED_METADATA_KEYS` + відмова `Filecoin::ArchiveService` на межі піна, DPIA захід M6): у пін їдуть лише декларовані ключі money-тракту, тож `[TBD verify]` звузився з «чи будь-коли несе PII» до вмісту одного вільнотекстового `error` (текст чужого RPC/винятку) — судяться КЛЮЧІ, ніколи ЗНАЧЕННЯ. Привілейовані записи з `archive: false` (ключі/ролі/актуатори) свідомо не пінюються публічно (ARCH.57 — «security-метадані не на публічний IPFS»). |
...
2. **Pinata / IPFS-Filecoin** (`app/services/filecoin/archive_service.rb`, `api.pinata.cloud`) — публічно пінує `AuditLog.metadata` для money/MRV-записів (§2.5) — з 2026-08-25 лише **декларовані** ключі (DPIA захід M6; зміст значень і далі не класифікується). Це не «процесор» у звичному сенсі — публічна permissionless-мережа без юрисдикційного контролю, що ще гостріше, ніж типовий DPA-gap. ⚫ **Із 2026-09-07 потік ЗУПИНЕНО** (activation-gate, ⚖️ founder — `FILECOIN_API_KEY` знято): нових передач немає, тож цей DPA-gap перестав ЗРОСТАТИ. ⚠️ Але не зник: запінене раніше лишається публічним і невідкличним, а тригер реактивації названо ([`00_07`](../../00_07_Action_Plan_Tracker) INF.22). 🔑 Форма збігається з п.1 вище (Hadron, клас `ARCH.118`) і це не збіг: **«отримувач призначений, передачі не відбувається» — окремий стан, і Art.30 має його розрізняти**, інакше реєстр перебільшує власні потоки.
