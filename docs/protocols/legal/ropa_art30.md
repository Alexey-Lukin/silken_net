...
> **⚠️ Не юридична порада.** Робочий вхід у платну консультацію з DPO/юристом (профільний GDPR/крипто-юрист — TBD, [`07_03 §4.2`](../../07_03_Academic_Integration_and_IP.md)), не її заміна. **Це чернетка — показувати наглядовому органу на запит МОЖНА лише після юридичного review**, не в поточному вигляді.
...
**Представник у ЄС (Art.27):** `[TBD — ще не призначений]` ([`b2c_tos_privacy §B.12`](b2c_tos_privacy.md); чекбокс — `00_07` SEC.18). Обовʼязковість активується систематичним EU-таргетингом — мовні локалі `lt`/`lv` вже несуть цей сигнал за EDPB Guidelines 3/2018 (той самий факт паралельно важить у securities-аналізі, BIZ.22).
...
| Art.30(1) | Зміст |
|---|---|
| **(b) Цілі** | Екологічний/росто-вий моніторинг (D-MRV); Proof-of-Growth для нарахування growth points/SCC-токена ([`07_01 §3`](../../07_01_Nature_as_a_Service_Contracts.md)); тривоги раннього попередження (EWS). *(Art.6(1)(b) — надання самої послуги моніторингу)* |
| **(c) Субʼєкти + дані** | **Дерево не є субʼєктом даних** — фізіологічна телеметрія (напруга EBFC, Z-показник Атрактора Лоренца, температура/вологість/тиск/VPD/акустичні події) сама по собі персональних даних не несе ([`03_04 §6.3`](../../03_04_mruby_Lorenz_Attractor.md) — canon-підстава «у дерев немає GDPR-даних»). **Крайовий випадок — межа, не вирішення:** гранулярна геолокація вузла (`Tree.latitude/longitude`) + межі кластера (`Cluster.geo_boundary`, PostGIS) у поєднанні з кадастровими/земельно-реєстровими записами теоретично може пере-ідентифікувати малого/сімейного власника ділянки (той самий принцип, що smart-meter-дані). ⚖️ **DPIA Art.35 ще не проведено** — відкритий чекбокс `00_07` SEC.18 (anchor-geo re-identification): 3 з 9 EDPB WP248-критеріїв уже зійшлись (location + systematic monitoring + innovative tech). |
...
| **(d) Отримувачі** | Внутрішньо (сам застосунок, console-читання глобального ланцюга). Для **money/MRV-записів** (`archive: true`, MRV.1) — додатково **Pinata** (`api.pinata.cloud`, IPFS/Filecoin pinning-шлюз, `Filecoin::ArchiveService`) — 🔴 **публічна децентралізована мережа, не контрактний процесор**, і теж відсутня в [`b2c_tos_privacy §D.3`](b2c_tos_privacy.md), див. §6. Пінується `content_attrs` (id/org_id/action/`chain_hash`/auditable-посилання/`created_at`) + **`metadata` (jsonb) цілком** + добове зведення телеметрії кластера. `ip_address`/`user_agent` напряму НЕ пінюються (лишаються лише всередині `chain_hash` — SHA-256-дайджест, необоротний), але зміст `metadata` не класифіковано (та сама стеля) → `[TBD verify]`, чи будь-коли несе PII. Привілейовані записи з `archive: false` (ключі/ролі/актуатори) свідомо не пінюються публічно (ARCH.57 — «security-метадані не на публічний IPFS»). |
...
| **(c) Субʼєкти + дані** | Користувачі організації (переважно ролі admin/forester). → [`04_01 §11`](../../04_01_Data_Models_and_Entities.md) рядок `users`: `phone_number`, `telegram_chat_id`, `push_token`, `email_address`, `locale`. |
| **(d) Отримувачі** | **Email** — вихідний SMTP-релей (ENV-керований: `SMTP_ADDRESS`/`PORT`/`USER_NAME`/`PASSWORD`; ARCH.60 machine-half shipped 2026-08-04, `mail_transport_check.rb` boot-guard). Конкретний ESP-вендор (Postmark/SES/Mailgun/SendGrid/Resend — будь-який, дизайн навмисно vendor-agnostic) **ще не обрано** → `[TBD]`. **SMS/Push** — 🔴 **не задеплоєно.** `TwilioClient`/`FcmClient` — класи не існують у коді взагалі (нуль gem-залежностей); `SingleNotificationWorker` пише лише `warn "Канал не сконфігуровано"` (ARCH.60/ARCH.78 — брехливий-лог машинну половину закрито 2026-08-04, транспорт — ні). **Telegram** — 🔴 **поле є, клієнта немає взагалі**: `telegram_chat_id` зберігається, але диспетчер `SingleNotificationWorker#perform` знає лише `:sms`/`:push` — на відміну від SMS/Push навіть не входить у ARCH.60-скоуп. |
...
| **(g) TOM** | `filter_parameters` скраб `phone_number`/`telegram_chat_id`/`push_token` з логів (той самий allow-list, що §2.1, розширений Sentry); `spec/quality/no_self_attesting_logs_spec.rb` — гейт проти логу, що стверджує недоставлену дію (ARCH.78; точність журналу — Art.5(1)(d) accuracy, не PII-безпека per se). |

### 2.7 Web3-гаманці та KYC/AML (Hadron custodial)

| Art.30(1) | Зміст |
|---|---|
| **(b) Цілі** | Облік гаманця для нарахування growth points/SCC/USDC/cUSD ([`07_01 §3`](../../07_01_Nature_as_a_Service_Contracts.md)); KYC/AML-верифікація бенефіціара перед мінтом. *(Art.6(1)(b) виплата нагород + Art.6(1)(c) юридичний обовʼязок — KYC де застосовно, [`b2c_tos_privacy §B.3`](b2c_tos_privacy.md))* |
...
- **`filter_parameters`** — PII-скраб логів: `email`/`phone_number`/`telegram_chat_id`/`push_token`/`first_name`/`last_name`/`recovery_codes` (+ секрети/ключі), той самий список успадковує Sentry.
...
- **JSONB-поля непрозорі за побудовою.** `audit_logs.metadata`, `ai_insights.reasoning`/`summary` та інші jsonb/text-стовпці можуть містити довільний вміст, який жодна автоматика не класифікує — якщо туди колись потрапить PII, цей реєстр цього не побачить, доки хтось не прочитає вміст руками.
- **Вільні тексти приладу не видно.** `maintenance_records.notes`, повідомлення тривог (`EwsAlert`/`AiInsight`) — та сама стеля.
- **EXIF-метадані фотододатків не проскановані** (§2.4) — нова знахідка цього документа, без власного носія-гейту.
...
2. **Pinata / IPFS-Filecoin** (`app/services/filecoin/archive_service.rb`, `api.pinata.cloud`) — публічно пінує `AuditLog.metadata` (jsonb, зміст не класифіковано) для money/MRV-записів (§2.5). Це не «процесор» у звичному сенсі — публічна permissionless-мережа без юрисдикційного контролю, що ще гостріше, ніж типовий DPA-gap.
...
- [`07_01 §3`](../../07_01_Nature_as_a_Service_Contracts.md) — фінансові константи (growth points/SCC), не переказані тут.
- [`07_03 §4.2`](../../07_03_Academic_Integration_and_IP.md) — UA-юр-review контакти.
