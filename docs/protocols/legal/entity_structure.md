...
> - Founder особисто = **ФОП** (персональний-дохід-канал **+ IP/trademark-holder**, НЕ токен — п. **291.6** ПКУ; це ДЕРИВАЦІЯ з вимоги грошової форми, не літера заборони — межа в §6).
>
...
- **Консистентно з наявною mint-guard-архітектурою.** Модель «організація-of-record як KYC-якір» уже вшита в код: за custodial-minting-периметром [KYC.1] (`Wallet#kyc_approved_for_minting?`; гейт-канон [`05_02`](../../05_02_Proof_of_Growth_Pipeline.md)) custodial-гаманці **успадковують `organizations.hadron_kyc_status`**, і seed-фікстура демо-організації в `db/seeds.rb` ([`04_01 §8`](../../04_01_Data_Models_and_Entities.md)) уже несе `hadron_kyc_status: "approved"` + `crypto_public_address`. Рішення «operational-під-vehicle» **не потребує code-змін**.
  > ⚠️ Точність: seed — **демо-фікстура для розробки**, а не юридичний факт про будь-яку реальну компанію; збігу імені seed-org із реальною компанією більше немає: демо-Клієнта перейменовано на «Сосновий бір» (⚖️ founder 2026-09-24, [`msa_skeleton`](msa_skeleton.md) §E.1).
...
- **не 🤖-питання** — це рішення + документ (carve-out clause, corporate consent) → **юр-review** — 🏠 дім питань про carve-out: [`ip_carve_out_draft.md`](ip_carve_out_draft.md) §8 (⚠️ доти цей реф вів у `securities_review` Блок 4.5, який 2026-09-07 став вказівником сюди — на воркшоп треба саме цей файл, бо норми ЦК/2811-IX стоять у ньому);
...
| **ФОП founder'а** | персональний-дохід + IP-holder-«фізособа» | Персональний дохід (гранти-фізособі, консалтинг). Носій IP/™ (як фізособа). **ФОП ВИКЛЮЧЕНИЙ для крипто** (п. **291.6** — вимога розрахунків виключно в грошовій формі; деривація, §6) → ФОП НЕ тримає й не емітує токен. |
...
| **Наявний перелік кваліфікованих видів діяльності** | IT/hardware/data-дохід SilkenNet — **ймовірно** кваліфікований (VASP — п. 9 ч. 4 ст. 5 Закону 1667-IX; «…інструментів і обладнання для вимірювання, дослідження та навігації» — перелік КМУ №467/2022 через п. 12; первинку звірено 2026-09-25 — [`tax_posture_ua`](tax_posture_ua.md) Q3.2) |
...
| **2** | **Фізособа founder'а (+ ФОП)** | IP/trademark-holder + персональний дохід | 0 | ФОП-режим (не крипто) | ✅ **solo** — SilkenNet-IP + ™ тут | ❌ крипто виключено (п. **291.6**) | N/A | **NOW** (IP/™-шар) |
...
- **Висока:** ТОВ-мінімум для web3; operational-vehicle — наявна UA-компанія й Дія.City-резидент (founder-факт); базова Дія.City-механіка; модель «організація-of-record як KYC-якір» уже в коді (KYC.1); operational-vehicle multi-founder (founder-факт); securities-fact-pattern grounded у коді ([`securities_review`](securities_review.md) F1–F13).
- **Висока щодо ВИСНОВКУ, але не щодо ЛІТЕРИ — «ФОП виключений для крипто».** 🔴 Переміряно первинкою 2026-09-07 (`zakon.rada.gov.ua`, ПКУ 2755-VI, **чинна редакція від 31.05.2026**, підстава 4894-IX), і вимір змінив три речі. **(1) Точка кріплення інша:** норма живе в **п. 291.6**, а не 291.5 — «*Платники єдиного податку першої - третьої груп повинні здійснювати розрахунки за відвантажені товари (виконані роботи, надані послуги) **виключно в грошовій формі — готівковій або безготівковій (у тому числі з використанням електронних грошей)***». П. 291.5 натомість відкривається словами «*Не можуть бути платниками єдиного податку першої - третьої груп:*» і перелічує **суб'єктів за видами діяльності** (азартні ігри, лотереї тощо) — форми розрахунків там немає взагалі. **(2) Це ДЕРИВАЦІЯ, не заборона:** стаття 291 **жодного разу** не згадує ані «крипт»/«віртуальні активи», ані «бартер» — норма формулює позитивну вимогу грошової форми, а «отже, крипто-оплата неможлива» є нашим висновком із неї. **(3) Тому рубрика тут розділена:** висновок лишається високої впевненості, **формулювання «пряма заборона» — ні**, і в контрагентському чи консультаційному тексті писати треба саме «вимога розрахунків виключно в грошовій формі (п. 291.6)», а не «заборона крипти (п. 291.5)». ⚠️ Хибний номер прийшов із датованого web-знімка [`R1_ua_legal.md`](../research/R1_ua_legal.md), чия шапка прямо каже «НЕ верифіковано пофактно» — тобто дефект був не в знімку, а в цитуванні його як факту; ⛔ **знімок навмисно НЕ переписано** (він є свідченням того, що research видав).
