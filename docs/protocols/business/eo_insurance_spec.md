...
| **Тригер виплати** | Trigger-1 (AI-оракул, `ParametricInsurance#evaluate_daily_health!`) озброює кандидата; Trigger-2 (незалежне підтвердження — ⛔ **сьогодні ЛИШЕ Field-Audit, і це стосується ОБОХ перилів**: супутникового постачальника для пожежі зрізано ⚖️ 2026-09-05 ([`00_07` S3.2](../../00_07_Action_Plan_Tracker.md)) — пряме джерело назване, але не дротоване, тож `fire_detected` не спрацьовує жодного разу; drought-оракула немає й поготів; ⛔ «шкідники» зняті з тригерів 2026-09-10 — детектора немає, [`00_07` ARCH.102](../../00_07_Action_Plan_Tracker.md)) підтверджує payout | Claim-процес через страхового carrier'а (adjuster, underwriting-review) — стандартний insurance-claims workflow, не on-chain механіка |
...
- **Наслідок:** будь-який cyber-поліс підписаний ПІСЛЯ 2022 майже гарантовано містить один із цих clauses. Ukraine = активна ціль Russian state-sponsored cyber-операцій; страхувальник домицільований в Україні (незалежно від того, що серверна інфра GCP поза Україною) → «атака атрибутована РФ» — правдоподібний сценарій, НЕ хвостовий ризик.
