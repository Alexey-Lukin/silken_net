...
**Best-practice-висновок для BIZ.22 (integration, не нове рішення):** 2025-26 стандарт індустрії — **redesign до першого live-mint** — саме те вікно, в якому ми зараз (контракти code-complete, на mainnet не задеплоєні — звірено 2026-09-24: живі лише testnet-двійники, [`05_03`](../../05_03_Tokenomics_SCC_and_SFC.md); UNI.16). Мінімальний securities-safe пакет за best-practices, який має оцінити юрист ([`securities_review`](../legal/securities_review.md) Блок 1.5, питання 9–15):
...
- CRA чинний з 10 груд 2024; **vulnerability/incident-reporting з 11 вер 2026 (24-год вікно!)**; повний compliance (включно з formal SBOM у technical documentation) — **11 груд 2027**. Практично SBOM потрібен ДО вересня 2026, бо component-visibility — передумова 24h-reporting.
  🔴 **Ці чотири джерела — БЛОГИ, і дві речі вони пропускають; дім первинки — [`eu_market_entry.md`](../legal/eu_market_entry.md), звіряти там.** (1) **CRA не настає першим:** до нього діє кібер-делегований акт до RED (DR (EU) 2022/30, чинний з 2025-08-01), скасований DR (EU) 2026/339 з 2027-12-11 — тобто CRA поглинає його de jure. (2) **Дата 11 вер 2026 гостріша, ніж тут написано:** derogation CRA Art. 69(3) поширює обов'язки Art. 14 на **ВСІ** продукти на ринку, а не лише на розміщені після повного застосування. ⚠️ Вторинні джерела масово переказують обмеження EN 18031-1 **інвертовано** — саме тому цей блок не є підставою ні для чого, крім орієнтації.
- **Скоуп:** «products with digital elements» placed on the EU market. **Некомерційний FOSS — exempt** (нуль комерційної активності); комерційна активність (paid support, premium, business-entity-involvement) знімає виняток; «**open-source steward**» (юрособа, що sustained-підтримує OSS-розробку — типово foundation) = полегшений режим, але не нульовий. 🔴 **Це загальна норма — У НАШІЙ МОДЕЛІ вона не спрацює, і це primary-verified присуд ([`00_07`](../../00_07_Action_Plan_Tracker.md) BIZ.24 (в)):** recital 15 прямо називає ознакою комерційності «providing a software platform through which the manufacturer monetises other services» — це буквально NaaS, а продаж пристрою є самостійною підставою; а Art. 3(14) вимагає, щоб steward був юрособою, ІНШОЮ за manufacturer, тож фізособа-власник IP стюардом бути не може взагалі.
- **Наша специфіка (несуча):** CRA б'є нас не як SaaS (чистий SaaS — це NIS2-територія), а як **hardware-manufacturer: Soldier/Queen + firmware = класичний «product with digital elements»** (→ [`00_07`](../../00_07_Action_Plan_Tracker.md) **BIZ.24** — стан і присуди; норми — [`eu_market_entry.md`](../legal/eu_market_entry.md). ⚠️ **Тут стояв реф на `02_06 §8` як на «канон-дім», і він був ХИБНИЙ, хоч і резолвився:** ту секцію [`00_06 §2`](../../00_06_SSOT_Documentation_Standard.md) віддав vendor-landscape'у, а CRA/RED/RoHS у ній немає й ніколи не було — виміряно 2026-09-10). Продаж/поставка анкерів+сенсорів на EU-ринок = full manufacturer-obligations (SBOM, secure-by-default, vuln-handling, CE-marking процес).
- **Фаза:** зараз поза скоупом **не через FOSS-виняток, а тому, що продукт ще не розміщено на жодному ринку** (та сама підстава, що в `SECURITY.md`). Перший EU-B2B-контракт з поставкою заліза (або комерційний SaaS-supply з firmware-компонентом) → CRA-годинник вмикається. Дія 🟢 зараз (бо дешево і enterprise-очікувано незалежно від CRA): **SBOM у CI**. Частина вже стоїть — image-SBOM генерується в CI (mirror-ghcr; вхід Trivy-скану, `00_07` OPS.10); ✅ **повний multi-lock SBOM відвантажено 2026-07-25** — агрегований CycloneDX-1.6 усієї polyglot-поверхні (`.github/workflows/sbom.yml` + власні `sbom_merge.rb`/`sbom_conda_lock.rb`/`sbom_submodules.rb`: сабмодулів `firmware/extern` не бачить ані Syft, ані Trivy, ані dependency-graph GitHub, а `conda-lock.yml` не читає жоден інструмент). ⚠️ **Оголошені стелі стоять у шапці самого workflow і зеленим прогоном не знімаються:** `tools/ml/environment.yml` без lock-файла (версії — діапазони), importmap без lockfile (CDN-пін leaflet не перелічений), NuGet лише прямі пакети, плоский merge без `dependencies[]`. Полиця «SBOM є» — це те, що enterprise-procurement 2025-26 питає ДО того, як спитає CRA.
- 🟡 На Phase-2/EU-market: SBOM-у-tech-doc + 24h-vuln-процес + визначити, ХТО саме є **manufacturer** (залежить від entity-розкладки BIZ.20: operational-vehicle? token-co? — питання в юр-workshop). ⛔ **Не ставити це як розвилку «manufacturer vs steward»:** stewardʼа за Art. 3(14) ми обрати не можемо (BIZ.24 (в) ↑), тож питання лише про особу manufacturerʼа.
...
1. **ВНЗ-flag (ядро UNI.20):** DCO студента-фізособи покриває **вільні PR**; НЕ обов'язково покриває контрибуції в контексті **формального ЧНУ-гранту/thesis/supervised-роботи** — там UA-режим службових творів може дати інституційну claim ЧНУ (R5 §5.3; UA-норми не досліджені — proxy з US-policies). Тому: у будь-який майбутній ЧНУ-MoU — **явний IP-пункт про студентські контрибуції окремо від DCO** (уже питання до юр-workshop — [`securities_review`](../legal/securities_review.md) Блок 4.4). Зараз тиску немає (ЧНУ-канал пасивний, MoU unsigned, UNI.18 archived) — але пункт має лежати готовим у MoU-шаблоні ДО підписання. ✅ **Каркас написано 2026-08-30** — [`vendor_templates.md §C.8`](../procurement/vendor_templates.md): три розвилки (чий титул · форма узгодження · dual-license-стеля) з названими наслідками, плюс текст-заготовка, що наповнюється ПІСЛЯ юр-відповіді. ⛔ Саме формулювання свідомо не написане — див. там-таки: UA-режим службових творів для СТУДЕНТА цей огляд не досліджував, і видати проєкцію з US-політик за норму означало б перетворити власний дисклеймер на твердження.
...
| 3 | MiCA Art. 6 whitepaper (machine-readable) / Art. 4 exemptions | Немає (і не треба ще) | Лише при EU-офері. ⛔ **«DEX-vs-admission» юристові НЕ нести** — предмета немає: SCC на DEX не торгується, ціни ми не читаємо й не публікуємо, `PriceOracleService` знятий (вимір коду, [`R2 §2.4`](../research/R2_offshore_token_securities.md)) | 🟡 | UNI.16 Блок 3 |
...
| 9 | SBOM у CI (CycloneDX/SPDX); CRA: reporting 2026-09-11, full 2027-12-11 | Image-SBOM у CI вже є (mirror-ghcr, OPS.10); повного multi-lock SBOM нема; зараз поза скоупом бо не розміщено на ринку (⛔ НЕ через FOSS-виняток — [`eu_market_entry`](../legal/eu_market_entry.md) §3); Soldier/Queen = майбутній «product with digital elements» | Multi-lock SBOM-CI-job зараз; manufacturer-процеси — при EU-market-вході | 🟢 старт / 🟡 hard | **BIZ.24** |
...
1. **BIZ.22-redesign** — лишається найвищим пріоритетом, але ПРЕДМЕТ звузився (звірено 2026-09-07): мову інверсовано, refund/exit-fee й yield знято з коду серпнем 2026, тож відкритою є **одна вісь — конструкція передоплати** (імʼя колонки вирівняно 2026-09-25), поки контракти не задеплоєні (мапа → [`securities_review`](../legal/securities_review.md)); «no-price-talk» у публічних доках — настанова вже зараз.
...
4. ✅ **Multi-lock SBOM-CI-job — зроблено 2026-07-25** (`sbom.yml`, BIZ.24 🤖-half закрита; оголошені стелі — §3.2). Scorecard-action ✅ уже live — дій не потребує.
