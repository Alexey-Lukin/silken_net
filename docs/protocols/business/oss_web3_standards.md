...
**Вхідні артефакти:** [`entity_structure`](../legal/entity_structure.md) (token-контур/фази) · [`securities_review`](../legal/securities_review.md) (securities fact-pattern F1–F15) · [`trademark_brief`](../legal/trademark_brief.md) · [`spdx_rollout_plan`](../legal/spdx_rollout_plan.md) · [`R2_offshore_token_securities`](../research/R2_offshore_token_securities.md) · [`R5_trademark_ip`](../research/R5_trademark_ip.md)
...
2. **Dual-licensing-tension (несуча, майже ніде не проговорюється явно):** комерційна dual-license поверх AGPL (§5) вимагає, щоб licensor мав **повні права на весь релліцензований код**. DCO лишає copyright у контриб'юторів → зовнішні DCO-контрибуції **не можна** включити в комерційну ліцензію без згоди авторів. Саме тому dual-license-компанії (MongoDB історично, Grafana Labs) тримають **CLA**, а Nextcloud (DCO) — монетизує підпискою/послугами, НЕ dual-license. Для нас це не блокер: ядро = 100% founder-copyright (solo, [`07_03 §3`](../../07_03_Academic_Integration_and_IP.md); [`securities_review`](../legal/securities_review.md) Блок 4.3 standing), зовнішні контрибуції будуть рідкі й периферійні → DCO зараз, а якщо Phase-2 обере dual-license-монетизацію — вона реалізовна на founder-owned ядрі; масовий зовнішній contribution-потік (не наш кейс) вимагав би перегляду в бік легкого license-grant-CLA. Свідома стеля, познач у CONTRIBUTING.

✅ **Зроблено (2026-07-25):** `CONTRIBUTING.md` §DCO + enforcement (🤖-чекбокс [`00_07`](../../00_07_Action_Plan_Tracker.md) UNI.20). Замість `dcoapp/app` — власний `scripts/dco_check.rb` у `dco.yml`: сторонній App на публічному репо вимагає org-install із write-скоупом, а його дефолтний author-match ламається саме на нашому кейсі (Dependabot підписується як `support@github.com`, авторить як бот → строгий match валить кожен dep-PR п'яти щотижневих екосистем; presence-only нічого не доводить). Лишився 👤-флип branch-protection на `DCO passed`. Це водночас підсилює OpenSSF contribution-policy-вісь (§3.3 синергія; Badge silver уже earned — DCO зміцнює posture, не гейтить бейдж). ЧНУ-MoU-IP-пункт — шаблоном у папку partnership-паперів (= 👤-чекбокс UNI.20).
...
| 7 | REUSE/SPDX per-file headers | Станом на 07-24: 21/1914 tagged; план готовий ([`spdx_rollout_plan`](../legal/spdx_rollout_plan.md)); scope-осі частково ратифіковані (DOC-T.47), решта відкрита | Founder закриває решту scope-питань → per-дерево кампанія | 🟢 (рішення) / Phase-7 (робота) | UNI.3 |
| 8 | Ліцензійна карта доків = коду | ✅ **ЗАКРИТО** (ратифіковано 2026-07-24): `/NOTICE` + [`07_03 §3`](../../07_03_Academic_Integration_and_IP.md) вирівняні — contracts = MIT (industry-правильно: composability/tooling/OZ) | — (зроблено) | ✅ | DOC-T.47 (closed) |
...
| 11 | DCO + enforcement-bot (CNCF/OpenInfra 2025-консенсус; Nextcloud-AGPL-прецедент) | ✅ `CONTRIBUTING.md` §DCO + enforcement **власним** гейтом (`scripts/dco_check.rb` у `dco.yml`, не сторонній App — на публічному репо він вимагав би org-install із write-скоупом, а його author-match-дефолт саме тут і ламається: Dependabot підписується `support@github.com` при бот-авторі → строгий match завалив би кожен dep-PR) | лишився 👤-флип branch-protection на `DCO passed` + ВНЗ-MoU IP-пункт | 🟢 | **UNI.20** |
