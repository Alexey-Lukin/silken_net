...
🟢 **Зараз (годинна робота):** `CONTRIBUTING.md` з DCO-текстом + `dcoapp/app` на repo + рядок у README (= 🤖-чекбокс [`00_07`](../../00_07_Action_Plan_Tracker.md) UNI.20). Це водночас підсилює OpenSSF contribution-policy-вісь (§3.3 синергія; Badge silver уже earned — DCO зміцнює posture, не гейтить бейдж). ЧНУ-MoU-IP-пункт — шаблоном у папку partnership-паперів (= 👤-чекбокс UNI.20).
...
| 11 | DCO + enforcement-bot (CNCF/OpenInfra 2025-консенсус; Nextcloud-AGPL-прецедент) | Немає CONTRIBUTING/DCO | CONTRIBUTING.md + dcoapp; ВНЗ-MoU IP-пункт шаблоном; dual-license-tension позначити | 🟢 | **UNI.20** |
