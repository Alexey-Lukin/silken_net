...
✅ **Зроблено (2026-07-25):** `CONTRIBUTING.md` §DCO + enforcement (🤖-чекбокс [`00_07`](../../00_07_Action_Plan_Tracker.md) UNI.20). Реалізовано **власним** гейтом, а не `dcoapp/app`, який радив цей огляд — обидві причини (org-install із write-скоупом на публічному репо · author-match-дефолт стороннього App ламається на Dependabot) живуть у шапці `scripts/dco_check.rb`, вона ж і дім. Лишився 👤-флип branch-protection на `DCO passed`.
...
| 11 | DCO + enforcement-bot (CNCF/OpenInfra 2025-консенсус; Nextcloud-AGPL-прецедент) | ✅ `CONTRIBUTING.md` §DCO + enforcement власним гейтом (`scripts/dco_check.rb` у `dco.yml`) | лишився 👤-флип branch-protection на `DCO passed` + ВНЗ-MoU IP-пункт | 🟢 | **UNI.20** |
