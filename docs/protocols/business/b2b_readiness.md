...
>
> **Контекст:** solo-founder, phase-1, **у production нічого не задеплоєно, нуль users, нуль revenue** (звірено 2026-09-24: живі лише testnet-слот `canopy` і testnet-контракти — [`06_01`](../../06_01_Deployment_Kamal_Terraform.md), [`05_03`](../../05_03_Tokenomics_SCC_and_SFC.md)). Operational-vehicle = наявна UA-компанія, Дія.City-резидент, співзасновником якої є founder (тришар-присуд → [`00_01 §8`](../../00_01_Vision_Mission_and_Roadmap.md) + [`entity_structure`](../legal/entity_structure.md)); IP/™ — на фізособі founder'а.
>
...
|---|---|---|
| **SEC.23** — RPC/vendor-DPA ланцюг неверифікований (🔴 «Alchemy логує IP+wallet» виправлено 2026-09-06: IP там НАШ, серверний; IP субʼєкта отримують Cloudflare · CARTO · Google · експлорери) | **Art.28 (processor-контракт)** + Art.30 (RoPA мусить назвати processors) + Art.44+ (transfers) | Compute-плейсмент вирішено конструкцією (⚖️ 2026-08-29): децентралізований compute не будується, PII-шар живе на GCP (🟢 **Art.28-крок для GCP НЕ ІСНУЄ** — Cloud DPA інкорпорована через URL Terms без окремої дії, прочитано 2026-09-06; ⚠️ доти цей хвіст звав крок ПРОСТРОЧЕНИМ, тоді як голова того ж рядка правилась тим самим виміром — виправлено 2026-09-07). Живими лишаються дві ноги: Alchemy — гаманець-адреса як псевдонімний ідентифікатор (⚠️ дужку про *Breyer* знято 2026-09-06: IP у цьому потоці серверний, наш) → DPA прочитано того ж дня, лишається звірити чинність для нашого акаунта; і сам vendor-DPA ланцюг (GCP/Upstash/Grafana/Sentry/Pinata/AWS ⊕ Cloudflare · CARTO · Google Maps · експлорери (дописані 2026-09-06); ⛔ Hadron із ланцюга ЗНЯТО 2026-09-04 — вендора виміряно неіснуючим; ⚫ Telegram ЗНЯТО 2026-09-06 — канал зрізано ⚖️ founder, процесорних відносин більше не виникає) — verify напряму, поіменний перелік і стан → [`b2c_tos_privacy §D.4`](../legal/b2c_tos_privacy.md). **Гейтить live-деплой PII; для enterprise-DD — це рядок субпроцесор-списку, який privacy-office покупця не пропустить** |
...
|---|---|---|---|---|---|
| 1 | **Security-questionnaire readiness** (SIG Lite/CAIQ) | Заповнені відповіді на ~150+ питань про контролі | 🟡 | Заповненого ще нема; сирі відповіді сильні: [`06_07 §1`](../../06_07_CICD_and_Runbook_Index.md) (9 required CI-gates, CodeQL, secret-scanning, SLSA provenance+SBOM, SHA-pinning, Scorecard weekly), Sentry `pii=false`, Argon2id, AR-encryption, KENOSIS-межі | — (пакет §5 п.9; живить BIZ.2-DD) |
...
| 9 | **BC/DR-plan** («exist AND tested») | Документ + докази drill | 🟡 (перший drill + 1-pager для клієнта перед signing) | Runbooks існують ([`06_06 §5`](../../06_06_Disaster_Recovery_and_Backup.md): PITR restore, TF-state rollback, region rebuild) + RTO/RPO-цілі ([`06_06 §3`](../../06_06_Disaster_Recovery_and_Backup.md); `sla_exhibit` Додаток A — свідомо інформативні); **DR-drillʼів ДВА** — 2026-09-02 (§5.1 PITR-клон: RTO 11:00, RPO 0; §5.2 половина) і 2026-09-03 (**§5.7 логічний бекап слоту, ПОВНИЙ цикл, RPO 0 / RTO ≈ 16 хв**); §5.3–5.6 не проганялися, тож «tested» для §5.1 **і §5.7**. ⚠️ Свіп 09-07 оновив §5 п.11 і `sla_exhibit`, а цей рядок — той самий, яким відповідають покупцеві на «tested, not just documented», — лишався під старою лічбою; `sla_exhibit` і далі забороняє контрактні RTO/RPO до ПОВНОГО циклу | DR.1 (drill-чекбокс) |
...
| 17 | **Vendor-DPA ланцюг (Art.28 вгору)** | Підписані DPA ВІД наших processors | 🟢 (до live; ⚠️ **GCP БІЛЬШЕ НЕ «дія» — вимір 2026-09-06:** Cloud DPA інкорпорована через URL Terms без окремого підпису, як і AWS/Upstash; реальні кроки лишились у Sentry · Cloudflare · Grafana) | Чеклист готовий — [`b2c_tos_privacy`](../legal/b2c_tos_privacy.md) §D.4: GCP = 🟢 **дії НЕМАЄ — DPA інкорпорована автоматично** (вимір 2026-09-06; ⚠️ доти хвіст продавав GCP як «найлегшу дію», тоді як голова цього ж рядка вже казала, що реальні кроки лишились у Sentry · Cloudflare · Grafana); решта = **verify напряму, не припускати**. ⚠️ Перелік цілей рахуй у домі: тут стояли чотири вендори, тоді як §D.4 уже називав удвічі більше — множина verify-цілей росте разом із реєстром §D.3, тож копія старіє в бік НЕДООЦІНКИ обсягу | SEC.23 (verify-чекбокс) |
| 18 | **SEC.23 — GCP × Art.28** | Named законтрактований processor для PII-workload | 🟢 (⚠️ **«click-accept» СПРОСТОВАНО 2026-09-06:** механізм GCP її не потребує — DPA інкорпорована автоматично, тож крок не «невиконаний», а неіснуючий; питання розміщення вже закрите) | 🟡 **Плейсмент вирішено конструкцією** (⚖️ 2026-08-29): децентралізований compute не будується взагалі, PII-workload один і живе на GCP. ⚠️ **Art.28-контракт для GCP не «невиконаний», а неіснуючий** (див. голову рядка) — невиконаним лишається ланцюг ІНШИХ вендорів — і 🔴 з 2026-08-31 уже НЕ через «акаунта немає»: акаунт є, інфра розгорнута. Це й далі не архітектурне рішення, а простий названий крок (click-accept DPA+SCC), лише тепер він відкладається при живому проєкті. Рядок 🟡, не 🟢 | SEC.23 (DPA-verify) |
...
### 🟢 Phase-1 / до першого live-деплою (усе — нуль грошей, 🤖-робота + 1 рішення)

Порядок = пріоритет. Це НЕ «зараз сідай і роби» — це «мусить бути зроблено до того, як перший EU-user/власник ділянки з'явиться в проді»; але 1–3 можна зробити 🤖 будь-коли, вони дешеві. ⚠️ **Лічба перерахована 2026-08-29:** з двох рішень, які тут раніше стояли (п.1 — Art.36-залишок, п.4 — SEC.23-плейсмент), друге вже ухвалене (п.4, ✅ нижче) — лишається одне, 👤-половина п.1.
...
2. ~~**RoPA-скелет** (#12; SEC.18-кластер)~~ ✅ **написано 2026-08-20** ([`ropa_art30.md`](../legal/ropa_art30.md)) — залишається 👤-половина (retention-строки, гейтовані юр-каналом). ⛔ **І рецепт, що тут стояв («переформатувати `b2c_tos_privacy` §B.3+§D.3 в Art.30-структуру, година-дві»), був хибний ЗА ПОБУДОВОЮ, а не лише простроченим** — джерелом істини для Art.30 є **код**, а не сусідній документ; переформатування дало б реєстр, що дзеркалить чужий переказ. Це прямо сказано вище в цьому ж файлі.
3. ~~**Breach-72h runbook** (#14; дім SEC.18)~~ ✅ **написано 2026-08-23** ([`gdpr_runbook.md`](../legal/gdpr_runbook.md)) — залишається 👤-половина: контактна адреса прийому (`[TBD]`), без якої строки Art.12(3)/33 нема кому запустити.
   ⚠️ **Обидва пункти простояли у формі відкритої роботи після власного закриття, тоді як п.1 і п.4 поруч закреслені акуратно** — і той самий файл спростовує їх двічі (рядок «усі чотири готові» + перелік артефактів). Виправлено 2026-09-07 (`DOC-T.101`); преамбула цієї секції перераховувала лише **рішення**, тому артефактів не побачила.
4. ~~**SEC.23-рішення** (#18) — ⚖️ founder+архітектура: PII-шар НЕ їде на децентралізований compute без Art.28-шляху~~ ✅ **ухвалено ⚖️ 2026-08-29** — сильніше за обидва розглянуті дефолти: децентралізований compute не використовується взагалі, PII-шар живе на GCP.
5. **Vendor-DPA ланцюг** (#17; SEC.23 👤-чекбокс, той самий захід) — ⚠️ GCP click-accept **знято** (DPA чинна без дії, вимір 2026-09-06); решта verify напряму; **поіменну множину бери в** [`b2c_tos_privacy`](../legal/b2c_tos_privacy.md) §D.4, вона росте разом із реєстром §D.3.
...
11. **DR-drill — ПОВНИЙ цикл** (#9; дім DR.1) — drill'ів **ДВА**, не один: 2026-09-02 (§5.1 повністю реальним PITR-клоном, §5.2 наполовину) і 2026-09-03 (**§5.7 — логічний бекап слоту, ПОВНИМ циклом, RPO 0 / RTO ≈ 16 хв**); результат — [`06_06 §6`](../../06_06_Disaster_Recovery_and_Backup.md), лічба — [`06_06 §Gaps`](../../06_06_Disaster_Recovery_and_Backup.md). ⚠️ Виправлено 2026-09-07: цей рядок і §5-таблиця вище називали лише перший, тобто **занижували перевірену поверхню на повний цикл** саме в документі, який відповідає покупцеві на «tested, not just documented». Лишається §5.2 у повній формі (`terraform apply` з витягнутої версії на throwaway-стеку) + §5.3–5.6 за drill-процедурою + 1-pager «BC/DR summary» (master-key-backup чекбокс — окремий, deploy-time).
