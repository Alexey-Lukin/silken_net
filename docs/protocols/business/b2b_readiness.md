...
| 18 | **SEC.23 — GCP × Art.28** | Named законтрактований processor для PII-workload | 🟢 (Art.28-контракт — click-accept стандартного DPA+SCC — ДО live-PII; питання розміщення вже закрите) | 🟡 **Плейсмент вирішено конструкцією** (⚖️ 2026-08-29): децентралізований compute не будується взагалі, PII-workload один і живе на GCP. **Art.28-КОНТРАКТ лишається невиконаним** — акаунта GCP ще немає, тож це не архітектурне рішення, а простий названий крок (click-accept DPA+SCC). Рядок 🟡, не 🟢 | SEC.23 (DPA-verify) |
