...
- **Наш стан:** окремого RoPA нема, АЛЕ ~70% контенту вже написано: `b2c_tos_privacy` §B.3 (таблиця категорій даних × правова підстава × джерело в коді) + §D.3 (субпроцесори × регіони × transfer-механізм) — лишається переформатувати в Art.30-структуру. Це 🤖-робота на годину-дві, нуль грошей.
...
- **Наш стан:** DPIA нема; проблема вже чесно зафіксована (`b2c_tos_privacy` §B.7 + §🔴3 — «не вирішувати заднім числом»), і в трекері вже заведено прямий чекбокс ([`00_07`](../../00_07_Action_Plan_Tracker.md) SEC.18: anchor-geo re-identification → DPIA Art.35 ДО обробки). DPIA — це **self-executed структурований документ** (опис processing → necessity/proportionality → ризики → мітигації), не зовнішній аудит — вартість = наш час. Мітигації, які DPIA ймовірно призначить (агрегація/огрублення координат у B2B-виданні, окрема lawful basis), — це і є відповідь на red-flag 3.
...
|---|---|---|---|---|---|
| 1 | **Security-questionnaire readiness** (SIG Lite/CAIQ) | Заповнені відповіді на ~150+ питань про контролі | 🟡 | Заповненого ще нема; сирі відповіді сильні: [`06_07 §1`](../../06_07_CICD_and_Runbook_Index.md) (7 required CI-gates, CodeQL, secret-scanning, SLSA provenance+SBOM, SHA-pinning, Scorecard weekly), Sentry `pii=false`, Argon2id, AR-encryption, KENOSIS-межі | — (пакет §5 п.9; живить BIZ.2-DD) |
...
| 11 | **Sub-processor list** (публічний) | Реєстр + вікно заперечення | 🟡 (публікація разом із trust-page) | ✅ **Реєстр готовий** (`b2c_tos_privacy` §D.3: GCP/Akash/Alchemy/Upstash/Grafana/Sentry з регіонами+DPA-статусами) + модель general authorization (§D.2) — не опублікований (нема куди: сайту нема) | SEC.23 (реєстр-чекбокс; живить BIZ.3) |
| 12 | **RoPA (Art.30)** | Внутрішній реєстр processing-активностей; <250-виняток НЕ рятує (processing not occasional) | 🟢 (ДО першого live-деплою; 🤖-переформатування, нуль $) | Окремим документом ще нема; ~70% контенту існує (`b2c_tos_privacy` §B.3 data-inventory + §D.3) | SEC.18-кластер (data-subject compliance) |
| 13 | **DPIA (Art.35)** — **anchor-geo re-identification** | DPIA ДО high-risk processing; EDPB: 2+ з 9 критеріїв → у нас 3 (location data + systematic monitoring + innovative tech) | 🟢 (ДО першого live-processing EU-власників; self-executed, нуль $) | Ще не написаний; ризик чесно зафіксований (`b2c_tos_privacy` §🔴3, red-flag «не вирішувати заднім числом»); мітигація (огрублення geo у B2B-виданні) не впроваджена; його мітигації = закриття red-flag 3 | SEC.18 (прямий DPIA-чекбокс) |
| 14 | **Breach-notification 72h (Art.33)** | Внутрішній процес: детекція → кваліфікація → нотифікація ≤72h | 🟢 (1-pager runbook до live) | Контрактні хуки є (`sla_exhibit` §6.3, `b2c_tos_privacy` §D.1); внутрішнього runbook'а ще нема; детекція-інфраструктура (Sentry/Grafana alerts) є | SEC.18 (breach-notification чекбокс) |
...
| 17 | **Vendor-DPA ланцюг (Art.28 вгору)** | Підписані DPA ВІД наших processors | 🟢 (до live; GCP — найлегша дія) | Чеклист готовий (`b2c_tos_privacy` §D.4): GCP = click-accept (5 хв); Alchemy/Upstash/Grafana/Sentry = verify напряму | SEC.23 (verify-чекбокс) |
...
1. **DPIA anchor-geo** (#13; дім SEC.18) — структурований self-assessment: processing-опис → necessity → ризики re-identification → мітигації (огрублення координат у B2B-виданні / lawful basis). Закриває red-flag 3 + Art.35. Найбільша юридична діра з нульовою ціною закриття.
...
5. **Vendor-DPA ланцюг** (#17; SEC.23 👤-чекбокс, той самий захід) — GCP click-accept; Alchemy/Upstash/Grafana/Sentry verify.
