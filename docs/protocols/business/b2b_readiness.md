...
| 9 | **BC/DR-plan** («exist AND tested») | Документ + докази drill | 🟡 (перший drill + 1-pager для клієнта перед signing) | Runbooks існують ([`06_06 §5`](../../06_06_Disaster_Recovery_and_Backup.md): PITR restore, TF-state rollback, region rebuild) + RTO/RPO-цілі ([`06_06 §3`](../../06_06_Disaster_Recovery_and_Backup.md); `sla_exhibit` Додаток A — свідомо інформативні); **DR-drill №1 проведено 2026-09-02** (§5.1 PITR-клон: RTO 11:00, RPO 0; §5.2 — лише половина «витягти версію»; §5.3–5.6 не проганялися) — «tested» лише для §5.1; `sla_exhibit` і далі забороняє контрактні RTO/RPO до ПОВНОГО циклу | DR.1 (drill-чекбокс) |
