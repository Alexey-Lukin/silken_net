...
|---|---|---|
| **SEC.23** — Alchemy RPC логує IP+wallet + неверифікований vendor-DPA ланцюг | **Art.28 (processor-контракт)** + Art.30 (RoPA мусить назвати processors) + Art.44+ (transfers) | Compute-плейсмент вирішено конструкцією (⚖️ 2026-08-29): децентралізований compute не будується, PII-шар живе на GCP (DPA click-accept механізм публічний; 🔴 акаунт створено й інфра розгорнута 2026-08-31 — крок із «ще рано» перейшов у ПРОСТРОЧЕНИЙ-до-live-PII). Живими лишаються дві ноги: Alchemy — IP+wallet = персональні дані (CJEU *Breyer*) → верифікувати DPA або задокументовано прийняти ризик; і сам vendor-DPA ланцюг (GCP/Upstash/Grafana/Sentry/Hadron/Pinata/AWS/Telegram) — verify напряму, поіменний перелік і стан → [`b2c_tos_privacy §D.4`](../legal/b2c_tos_privacy.md). **Гейтить live-деплой PII; для enterprise-DD — це рядок субпроцесор-списку, який privacy-office покупця не пропустить** |
