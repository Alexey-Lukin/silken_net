...
| **3.5 Cascade & the PCM limit** | raw cascade uphill in *every* method; the ~1 eV gap **decomposes** (computed, ②) into **speciation** (chloro→aqua +0.51, bis-Im +0.30 eV; **functional-robust at ωB97X**, 34b) + **explicit solvation** (+0.20 eV / 3 Cl⁻-H₂O → full shell); [Os(H₂O)₆] benchmark recovers the ~1 V group-8 PCM error (n6→n18 +0.98 eV); QM/MM (Минаєв) = rigorous capstone | scripts 21/21d/33 + **34/34b ②** | Fig 5, Table 2 | **[② DONE]** + Minaev |
