...
| **3.4 Cathode DET (ZIF)** | bimetallic Cu-Co-Ce hopping; geom-fixed t_ij + **computed λ** (Nelsen, not assumed) → k_DET **borderline at ΔG = 0** (Cu-Co bottleneck ~turnover, ×1–30 not ×10⁵) and **below turnover on the adverse driving-force reading** (lit-λ bracket ×0.032…×40); honest λ- AND ΔG-sensitivity + Ru/cMOF/enzyme-free mitigation — a *finding*, not a failure; **FO-DFT (24b) confirms borderline robust to coupling method** (t_ij ~4× crude + 0.18 eV site-gap) | scripts 24/24b/25/35 | Fig 4, Table 3 | **[③ DONE]** |
