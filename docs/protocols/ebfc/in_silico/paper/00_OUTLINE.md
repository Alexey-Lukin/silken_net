...
| **3.3 Mediator structure–activity** | Hammett LFER: E°(Os III/II) linear in σ_para (slope ≈ −0.93 eV/σ); **cascade-Δ design rule** — electron-withdrawing 4,4'-bpy improves FADH₂→Os alignment; triangulated DFT↔Lever↔Hammett. **Caveat (frame carefully):** max cascade-Δ (NO₂) ≠ *optimal* EBFC mediator — a higher E°(Os) raises the anode potential → **lowers OCV**; the rule is for cascade *thermodynamics*, the exp Os optimum (~+309 mV, Mano/Heller) balances driving-force vs overpotential | ① `os_mediator_series.json` | Fig 3b, Table 4 | **[① DONE]** |
...
| **3.5 Cascade & the PCM limit** | raw cascade uphill in *every* method; the ~1 eV gap **decomposes** (computed, ②) into **speciation** (chloro→aqua +0.51 eV) + **explicit solvation** (+0.20 eV / 3 Cl⁻-H₂O → full shell); [Os(H₂O)₆] benchmark recovers the ~1 V group-8 PCM error (n6→n18 +0.98 eV); QM/MM (Минаєв) = rigorous capstone | scripts 21/21d/33 + **34 ②** | Fig 5, Table 2 | **[② DONE]** + Minaev |
