...
| **3.5 Cascade & the PCM limit** | raw cascade uphill in *every* method; the gap **decomposes** (computed, ②) into a **chloro↔bis-Im differential-solvation bracket** (chloro +1/+2 +0.21 ↔ bis-Im +2/+3 +0.55 eV, dimethyl) + **4,4'-dimethyl substituent** (+0.146); [Os(H₂O)₆] benchmark recovers the ~1 V group-8 PCM error (+0.98 eV); QM/MM of the chloro species (Минаєв) = rigorous capstone | scripts 21f/21g/33 + **34/34b ②** | Fig 5, Table 2 | **[② dimethyl recompute]** + Minaev |
| **3.6 Protein environment on E°** | bound FAD-GDH E° = **−265 mV vs SHE** (verified, Schachinger, Ma, Ludwig 2022) — protein tunes free −208 → bound −265 (−58 mV, *better* donor → larger cascade driving force); active-site model from AF3 PDB (isoalloxazine + catalytic HIS537 / charged GLU61 / backbone amides). Rigorous QM-cluster E° = Мінаєв capstone (bundled w/ the cascade QM/MM) | ④ (canon fixed; QM-cluster deferred) | — | **[④ canon ✅ · QM-cluster → capstone]** |
