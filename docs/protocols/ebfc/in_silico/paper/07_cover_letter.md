...
reorganisation energy, with an explicit low-λ mitigation path; and (4) the apparently
...
**reproducibility**: every reported number is regenerated by deterministic open-source scripts run in a
recorded, version-exact environment (committed with them), and the full pipeline is provided for the reviewers.
