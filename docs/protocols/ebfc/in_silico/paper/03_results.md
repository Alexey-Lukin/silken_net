...
nearest exit), inside the 18–20 Å window over which biological electron tunnelling remains efficient [CITATION NEEDED].
...
pH (it reduces to the amine on cycling,<sup>54</sup> collapsing the cascade to its worst case). The realistic
...
the literature value [CITATION NEEDED], a Cu(I) d¹⁰ hexa-aqua optimisation being unphysical, so that λ_hop(Cu–Co) is half
...
exponentially on λ: at the literature first-row values [CITATION NEEDED] the Cu–Co hop is borderline (≈ ×1.4 over
...
nanozyme site coverage, a surface-confined (Laviron<sup>55</sup>) estimate spans ~0.002–230 Ω — the cathode arc could
...
N5 sits **16.0 Å** below the surface — within the 18–20 Å tunnelling window [CITATION NEEDED] — and the through-bond path
