...
| 54 | `anchor_thermal_bridge` | **HW.34** bus thermal bridge: Cu bus dominates the PEEK break (G_anchor ~10× Ti-bus → anode pocket ~15°C below core, freeze-risk); Ti monolithic invisible; ALL bake-off alloys ≪ Cu. ⚠️ той самий `D_BUS=1.3` (канал), що й у 55, але напрям похибки ПРОТИЛЕЖНИЙ: A∝d² → міст завищено ×1.69, тобто консервативно; на канон-роді Ø1.0 обидва несучі вироки вистоюють, флипає РІВНО один кеш-булеан `cell_freeze_risk[316SS]` — caveat і re-run `00_07` HW.34 | `mechanical/anchor_thermal_bridge.json` |
