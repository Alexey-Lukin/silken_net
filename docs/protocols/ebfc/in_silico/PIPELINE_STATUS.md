...
| 58 | `thermal_install_field` | **HW.6** thermal install, 2D axisymmetric (solver verified on 4 axes incl. radial annulus vs analytic log law): as written the cambium reaches 117.9 °C, crossing at 111 s, killing living tissue to Ø47.5 mm ⇒ DBH ≥ 111 cm under the same 4 % CODIT rule; heating the only accessible metal never reaches the anode (20.5 °C) because the PEEK break works. ✅ Constructive half — the error is DURATION: wall coagulates at 5 s, cambium dies at 111 s, a 10 s pulse gives 8 s of coagulation at cambium 49.3 °C. Also the GENERATOR for the 1D orphan cache | `mechanical/thermal_install_field.json` |
