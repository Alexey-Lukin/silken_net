...
| 54 | `anchor_thermal_bridge` | **HW.34** bus thermal bridge на канон-роді **Ø1.0** (`lib.D_BUS_ROD_MM`): Cu bus dominates the PEEK break (G_anchor 9.3× Ti-bus, ×18.2 vs no-bus → anode pocket −12.8 °C, freeze-risk); Ti monolithic invisible (−0.70 °C); ALL bake-off alloys ≪ Cu, −2 °C gate still splits α+β Ti (safe) від CP-Ti/Ta (freeze) | `mechanical/anchor_thermal_bridge.json` |
