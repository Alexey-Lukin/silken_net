...
| 55 | `bus_mechanical` | **HW.34** bus rod на канон-роді **Ø1.0**, ДВІ гілки фабрикації (`printed` superseded ⊥ `welded` SHIPPED, ⚖️ 2026-09-10): buckling SF 25× (pogo) — non-issue; sway-fatigue WITH liner — нескінченний ресурс для КОЖНОГО сплаву в обох (SF 4.2–11.7× / 8.5–23.3×). 🔴 Unsupported вирок задає ФАБРИКАЦІЯ: друкований — нескінченний ресурс у **4 з 6** (SF 1.10–3.04), відмов немає, маргінальні Ta 1.10 і CP-Ti 1.54; зварений — межу SF 2 беруть **усі шість** (SF 2.21–6.08 — ⚠️ **на СЕРЕДИНІ смуги `σ_e/σ_y`; на її кінці 0.40 Ta читає 1.96 і межі не бере**, §`endurance_ratio_band`). ⚠️ Числа ери `L_FREE_UNSUP` = 36 мм (0.71–1.94 / 1.41–3.89 / «4 з 6») зняті 2026-09-12: протрузію виведено з CEM-стека = **23 мм**. 🔴 **«Звар опору звузив, а не скасував» стояло тут до 2026-09-11 і СПРОСТОВАНО:** фатигову підставу лайнера **ЗНЯТО ЦІЛКОМ**, бо обидві колонки `L_FREE_*` є ВІЛЬНИМИ консолями, а реальний стрижень іде крізь отвір Ø1.35 і, лишаючи зазор, лягає на стінку — лайнер на всьому свіпі µ, конформні плівки лише на його частині (§`clearance_regime.partly_gap_limited_branches`) — тобто на ВСЬОМУ свіпі unsupported-SF не описує жодної гілки (деривовано, §`clearance_regime.free_cantilever_sf_describes_these`). Лайнер несе **ЗНОС**: для нього контакт вимушений геометрією на кожному µ, протирання = коротке анод↔катод ~0.5 В. ⛔ Подвоєння лишається про ДРІТ, не про ЗʼЄДНАННЯ — але сам ШОВ із 2026-09-12 уже не безцінний: §`weld_seam` **обмежує** його зверху замість припускати. Знокдаун `k` не виміряний ніде (`knockdown_k_measured: null` — жодного канон-рядка, жодної відповіді вендора), тож модель інвертує питання: шов сидить у корені модельованої консолі, і `SF_seam = k·SF_wire` тримається на тотожності перерізу (⛔ не на максимумі: пік моменту реального вильоту — гирло отвору, відкликано 2026-09-12), отже break-even `k` виводиться. Пораховано на підпертому прольоті, роздутому поправкою, що МІРЯЄТЬСЯ ВЗДОВЖ ПРОТРУЗІЇ (⛔ «лише на підпертому» було оголошенням, якого код не виконував), із двома поправками — найгірше тертя свіпу й виміряні 40.8 % оптимізму прольоту. Звʼязна ланка — `Ta`, і 🔴 **результат не цитується одним числом: він ПЕРЕВЕРТАЄТЬСЯ на `L_FREE_UNSUP`**, яку той самий пункт записує хибною (поправка на оптимізм прольоту міряється вздовж протрузії). ⚠️ **Доти модель друкувала ПАРУ прольотів і деривувала переворот (`protrusion_sensitivity`, `verdict_flips_on_it`) — обидва знято 2026-09-12 разом зі спірністю:** проліт виводиться з CEM-стека, тож рядок ОДИН — `k` = **0.554**, і наш маркер 0.50 його **НЕ покриває** (−0.054). Звідки береться проліт, каже `weld_seam.span_provenance`. ⊕ Заведено §`interference_window` (2026-09-12): натяг пари «трубка ↔ дріт», якого доти в дереві не було жодного, **обмежено** — вікно 0.38…11.03 мкм радіально = **21.3 мкм діаметрально на обидві деталі разом** (стеля — фон Мізес у стінці на межі плинності PEEK при −30 °C, підлога — термічна втрата при +40 °C); обидва вендорські допуски лишаються `null`. 🔴 Головне не ширина, а що **номіналу, якого потребує присуд, не задано ніде** (`nominal_finding`: «ID 1.00» — припущення з відкритої RFQ-ноги, не креслення): Ø1.0 проти припущеного ID 1.00 = нульовий натяг, тож «щільна на дроті» вирішують допуски — потрібен зсув номіналу на 11.4 мкм діаметрально (⚖️, не допуск). ⊕ Два неочікувані наслідки: люфт 25 мкм є значенням при НУЛЬОВОМУ натягу (на стелі вікна 15.3), і **тертя на дроті є другим захопленням** (3.33 Н на 40 К проти хвату вже при µ·δ ≥ 0.032 мкм), тож підстава одно-кінцевого захоплення розрізняє лише на підлозі вікна. ⊕ Заведено §`axial_thermal`: осьовий член 27.6/55.3 мкм на ратифікованій довжині трубки 18 мм — ⚠️ та сама ДЕФОРМАЦІЯ, що радіальний, тож 14× є відношенням довжин; захоплення з обох кінців коштує ≈6.1 МПа, тобто не неможливість, а двадцятирічне повзання. ✅ **Зазор застосовано 2026-09-11** (гілка (в), канал 1.30→1.35): модель тепер розрізняє БІК люфту (`play_side` — плівка на стрижні ⊥ трубка на каналі) і рахує композитну жорсткість пари; звідси три деривовані числа, яких доти не було — `play_reduction.factor` 6.8× (не «стократне»), `supported_span_check` (опора на 2.45 мм глибше за припущені 6 мм ⇒ напруження занижене на 40.8 % на CEM-прольоті 23 мм; «0.51 мм / 8.5 %» були ерою 36 мм) і `channel.aspect_ratio_l_over_d` 12.59, що доти жило лише в прозі. ⊕ Заведено §`wear_budget` (2026-09-12) — **ЗНОС, підстава лайнера з 09-11, доти не рахувався ніде**: питомої швидкості PEEK↔Ti у дереві немає, тож рахується БЮДЖЕТ (наряд = лічба гойдань, ЧИТАНА з кешу `62`, × проковзування з кінематики балки; навантаження = реакція підпертої консолі; межа = обʼєм до протирання). 🔑 На тісному кінці **контактна сила скорочується** (`k = стінка/(p_течії·s)`), тож найхиткіша ланка у звʼязну половину відповіді не входить; розводить кінці бюджету **геометрія контакту**, тобто наш відкритий ⚖️. ⊕ Відхилення конформних гілок перезароблено на осі зносу | `mechanical/bus_mechanical.json` |
...
| 68 | `bus_contact_equilibrium` | **HW.34 / HW.23** bus rod in the cathode channel as a CONTACT problem (FE + active set, rigid wall), not a free-cantilever reading: under pogo drag the wall is met only at the pad plane and the root stress is capped whatever µ; off-axis the mouth becomes a contact station and static root bending grows past the play. Swept against the Zone-1 insertion PLACEHOLDER (30 mm) and the lock window ends, the play (zero interference + 55's interference window), tilt about three pivots and an off-axis pogo contact — the placeholder moves the offset price by an order of magnitude. No verdict | `mechanical/bus_contact_equilibrium.json` |
