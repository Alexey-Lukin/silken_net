...
| 62 | `wind_duty_cycle` | **HW.43** Cherkasy wind duty-cycle (NASA POWER, 10y daily WS10M, real open data): confirms 6.3e8 is an overcount — even the most generous Beaufort-3 bracket bounds N_eff ≤ 4.4e8; NOT closed to a single number (sway-onset threshold "poorly constrained" per lit + >4x-variable canopy-attenuation factor, both named) | `mechanical/wind_duty_cycle.json` |
