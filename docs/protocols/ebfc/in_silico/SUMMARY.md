...
🔬 **Where the wall STARTS — derived 2026-09-12, and it is the input the open axial ⚖️ needed.** Every first-contact number here bisects from the ROOT over a wall the model assumes along the whole span; the first 6 mm have none (PEEK gap inside the Ø11 sleeve). So a contact computed SHORTER than the mouth means the rod has already taken up its play by the time it arrives — and meets the bore **EDGE**, not the wall. Free deflection at the mouth is 13.0–32.6 µm against 25 µm of play, so the **shipped** branch (PEEK liner) bears on the edge at µ 0.4 and 0.5, while both REJECTED conformal branches do not (their 165–170 µm play is never taken up there). ⚖️ The verdict this feeds is therefore not «how long» but **«does the tube protrude into the PEEK gap»**: flush with the mouth puts a line contact on titanium, protruding puts it on polymer — and wear is the liner's ratified ground. ⛔ Declared ceiling: a GEOMETRIC condition only; no notch factor and no contact model exists in this tree. (`mechanical/bus_mechanical.json` §`clearance_regime.edge_bearing`)
