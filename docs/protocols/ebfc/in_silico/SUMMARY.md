...
> 🔴 **Clearance regime added 2026-09-11, and it RETIRES the fatigue ground rather than narrowing it further** (`00_07` HW.34, ⚖️ founder). Both `L_FREE_*` columns are FREE cantilevers — no wall anywhere — while the rod threads the cathode bore. At the ~165 µm radial play a 10 µm conformal film leaves, the rod takes up that play and **bears on the wall 4.5–11.1 mm inside the bore on every µ the script sweeps** (first contact 10.47–17.15 mm from the root). So the unsupported SF is a number for a configuration that does not exist, and the liner’s structural ground moved from FATIGUE to WEAR: a 10 µm film asked to be a bearing under a beam reaction in a THROUGH bore of `L/D` ≈ 12.6, whose wear-through is a ~0.5 V anode↔cathode short. (`mechanical/bus_mechanical.json` §`clearance_regime`; the verdict line is DERIVED from `gap_limited_branches`, never typed). 🔴 **The bracketed figures above are HISTORICAL as of 2026-09-12 and the qualifier «on every µ» no longer holds for a conformal film** — the protrusion correction below shortened the span from 36 to 23 mm, which makes the rod STIFFER, so a 10 µm film now first touches the wall at 13.9–23.0 mm from the root, i.e. past the bore end at the lowest friction point (µ 0.2) only — it still bears inside the bore on µ 0.3–0.5. `gap_limited_branches` went from three to **one — the PEEK liner**; the two conformal branches are `partly_gap_limited_branches`, and `free_cantilever_sf_describes_these` stays **empty** (it listed them from 2026-09-12 until the regime label stopped reading «not on every µ» as «never», fixed in `55` 2026-09-14). ⚖️ The ratified verdict is untouched, because the shipped branch is precisely the one that still bears (bonded member: first contact 8.45 / 6.81 mm from the root on µ 0.2 / 0.3, i.e. past the mouth at 6.0, and 5.85 / 5.21 mm on µ 0.4 / 0.5, i.e. ON the mouth edge — `clearance_regime.branches`, `edge_bearing`); what weakened is the ARGUMENT AGAINST the rejected branches, and it is recorded here rather than quietly re-derived.
> 🔴 **And the clearance verdict of the same day moved this section's OWN numbers — branch (в), channel Ø1.30 → Ø1.35, applied 2026-09-11.** Two things changed and only the second is a digit. **(1) The model can now say WHICH SIDE the leftover play sits on**, which it could not before: a conformal film is bonded to the rod, so its play is on the ROD side and the bending member is the bare wire; the ratified liner is tight on the wire and enters the bore as one body, so its play is on the CHANNEL side and the member is the rod-plus-tube composite (both stiffness bounds computed, `ei_Nm2_bare_rod` ⊥ `ei_Nm2_bonded`). **(2) The "hundredfold reduction of free travel" this page fed into canon was a ROD-SIDE number and is gone: the measured factor is `play_reduction.factor` ≈ 6.8×** (170 → 25 µm). ⚠️ The conclusion it supported survives with a named error bar rather than intact: the liner no longer bears exactly AT the mouth but past it (deepest first contact **8.45 mm** vs the assumed 6.0 span, i.e. **2.45 mm** over), so the supported-column root stress is **understated by 40.8 %** — `supported_span_check` in the cache, derived. ⚠️ This sentence read «0.51 mm / 6.51 / 8.5 %» until 2026-09-12: those belong to the retired 36 mm span, and the paragraphs on either side had already been dated while this one was missed. SF stays an order above the SF-2 line, so nothing is overturned; the point is that `L_FREE_SUP = 6 mm` is now MEASURED against the contact it assumes instead of asserting it.
> 🔴 **Weld seam BOUNDED 2026-09-12 (`00_07` HW.34) — and what makes it a measurement rather than a guess is that the input stays missing.** Canon ([`01_01 §1.4`](../../../01_01_Coaxial_Gyroid_Topology_and_PEEK.md) and the factory protocol `§3` step 1) had been sending the reader here for this state while nothing here held it. The seam's fatigue-strength knockdown `k` is in **no** canon row, **no** vendor answer and **no** experiment, so `weld_seam.knockdown_k_measured` stays `null` and the model bounds `k` instead: the seam and the supported-span stress refer to the SAME section, therefore `SF_seam = k · SF_wire` and the break-even `k` follows by inversion (⛔ the ground «the root is where the moment peaks» is RETRACTED 2026-09-12 — the drag acts at the pad beyond the bore, so the real peak is the bore MOUTH; the relation survives on identity, not on a maximum). **Priced on the supported span, inflated by an optimism term measured along the PROTRUSION** — ⛔ and that coupling is REAL, not a defect to declare away: the earlier wording «priced on the supported span ONLY» described an exclusion the code never performed. Conservatism comes from two corrections that belong to the real span: the worst friction the load sweep carries (µ 0.5) and the span optimism, **40.8 %** at the corrected 23 mm protrusion (`weld_seam.span`; the neighbouring `supported_span_check` reports the same 40.8 % from the same member — ⚠️ until 2026-09-14 the seam block re-read the regime row's ROUNDED EI and printed 41.3 %, which this sentence used to explain the other way round). **Result — the binding candidate is `Ta`**, and ⚠️ **this note is the record of a day when the result was NOT quotable as a single number.** It rode `L_FREE_UNSUP = 36`, a protrusion whose third component («flange/pad standoff ~16 mm») cited nothing, while the CEM stack gives **23 mm** and the pad is the rod's own end face — so the model emitted BOTH spans and derived the flip. **That correction landed the SAME day** (`00_07` HW.34): the span is CEM-derived now, `protrusion_sensitivity` is gone with the dispute it priced, and `weld_seam.span_provenance` carries where the span comes from. The settled single row is below:

🔴 **SETTLED 2026-09-12 — the protrusion was corrected, and the seam verdict went with it.** The span is
derived from the CEM stack now (sleeve 50 − Zone-1 insertion 30 − shank 14, + shank + flange 3 = **23 mm**),
so the pair of rows this table used to carry has collapsed into the one that was always the real one:

| protrusion | span optimism | σ worst corner | `Ta` `k` → SF-2 | vs our own marker `AS_PRINTED_DERATE` = 0.50 |
|---|---|---|---|---|
| **23 mm** (CEM-derived, shipped) | 40.8 % | 43.0 MPa | **0.554** | **DOES NOT CLEAR**, −0.054 |

⚠️ **The bound did not get worse — it was never that good.** The 36 mm row that stood here read the span
optimism along a protrusion the part does not have; at the true span the stress is higher and our own
as-printed marker no longer covers the joint. **Consequence, and it is a REQUIREMENT rather than a
caveat: `k` must now come from the vendor** (weld class, WPS, post-weld treatment of the toe — `00_07`
HW.34 👤), because there is no longer a house number that brackets it.
...
🔬 **Where the wall STARTS — derived 2026-09-12, and it is the input the open axial ⚖️ needed.** Every first-contact number here bisects from the ROOT over a wall the model assumes along the whole span; the first 6 mm have none (PEEK gap inside the Ø11 sleeve). So a contact computed SHORTER than the mouth means the rod has already taken up its play by the time it arrives — and meets the bore **EDGE**, not the wall. Free deflection at the mouth is 13.1–32.8 µm against 25 µm of play, so the **shipped** branch (PEEK liner) bears on the edge at µ 0.4 and 0.5, while both REJECTED conformal branches do not (their 165–170 µm play is never taken up there). ⛔ **The materials do NOT change with it** — the play is channel-side, so what meets the bore is the liner's outer surface, polymer against titanium either way. What changes is the FEATURE: flush with the mouth the bore edge meets the tube's END FACE (ring on ring), protruding it meets the cylindrical flank. Derived: the member arrives **7.8 µm** inside the wall at **0.60°**, and the earliest computed contact is 5.21 mm from the root against a mouth at 6.0, so the tube must start **0.79 mm** before it (⚠️ read on the BONDED column — the composite rod-plus-tube is the member this branch declares; it stood at 5.03 / 0.97 until 2026-09-12, taken from the BARE rod, i.e. from the configuration the direction verdict excludes) (`edge_bearing.liner_start`). ⚖️ Both verdicts of 2026-09-12 follow from this: the liner's lower end protrudes ≥ 1.0 mm, and the bore entry carries a RADIUS rather than a chamfer — a 30–45° lead-in is two orders steeper than the approach angle, so the rod never lands on the chamfer face, only where it meets the cylinder; a chamfer MOVES the edge, a radius removes it. ⛔ Declared ceiling: a GEOMETRIC condition only; no notch factor and no contact model exists in this tree. (`mechanical/bus_mechanical.json` §`clearance_regime.edge_bearing`)

🔬 **The liner↔wire FIT — derived 2026-09-12, and it is the number every other line of this section was standing on.** The 2026-09-11 direction verdict says the tube is TIGHT on the wire and the pair enters the bore as one body; the composite stiffness bound, every first-contact figure and the whole wear axis inherit that sentence, and until this block no interference for that pair existed anywhere — canon said so outright. Bounded the same way the seam is, because both vendor bands are missing: the window the geometry allows is **0.38 … 11.03 µm radial**, i.e. a **21.3 µm DIAMETRAL** budget for the tube bore and the wire OD **together**. Ceiling = von Mises at the tube bore reaching PEEK yield 100 MPa at −30 °C, where the thermal term ADDS (PEEK is the outer member on this interface — the opposite sign to the liner-OD/bore interface, which is the confusion canon warns about); floor = the thermal loss at +40 °C, i.e. the fit merely stays a fit. ⛔ **Both tolerance bands stay `null` in the cache** — a quoted band wider than 21.3 µm means the ratified fit cannot be BOUGHT: it has to be selected, machined, or heat-assembled (and a heated fit is available only before enzyme functionalisation).

🔴 **And the headline is not the width — it is that the nominal the verdict needs is SPECIFIED NOWHERE** (`interference_window.nominal_finding`; this line said «the specified nominals contradict the verdict» until 2026-09-14, treating an assumption as a spec — the same error 55 corrected in its own key on 2026-09-12). Canon freezes the tube WALL; «ID 1.00» lives only in an open RFQ leg. At that assumed bore, rod Ø1.0 against «ID 1.00» is a LINE-TO-LINE fit: whether the tube is tight is decided by which way the two tolerances fall, so half the population comes out with clearance. Same shape as the F3 gate's `≤` before 2026-09-11 — true about the arithmetic, false about the assembly. Landing inside the window needs the nominal to move **11.4 µm diametrally** at mid-window, which is a ⚖️ and not a tolerance (`00_07` HW.34). ⊕ Two consequences the same block derives, neither of them forecast. **(1) The 25 µm radial play §`clearance_regime` treats as a constant is the ZERO-interference value** — the one fit the direction verdict excludes: the fit swells the tube's OD by Lamé, and at the window ceiling the play falls 25 → **15.3 µm**, which makes EDGE bearing *more* likely, not less. The free-outer premise of the Lamé model is checked rather than assumed and holds across the whole band. **(2) Friction on the wire is the SECOND capture.** The differential axial growth needs 3.33 N to restrain at 40 K (6.66 N at 80 K) and friction supplies it once µ·δ ≥ 0.032 µm — at the friendliest swept µ of 0.1 that is δ ≥ 0.32 µm to hold at all and 0.63 µm to lock the mid-section (⚠️ two thresholds, two verbs: this sentence attached the LOCK value to the HOLD verb until 2026-09-12, a 2× row-mix), a small fraction of the window. So the axial compression is incurred **whichever end is mechanically fixed**, and the ratified ground for one-end capture («20 yr of creep under sustained compression») discriminates only at the very FLOOR of the window, where the tube still slips and relieves. ⚖️ The verdict stands — it also carries retention and assembly — but its stated ground weakened, and which of the two regimes ships is set by a number no drawing carries. ⛔ Declared ceiling on all of it: creep/relaxation is modelled NOWHERE, so the true window is narrower on BOTH sides (the floor rises as grip decays, the ceiling falls because a sustained-stress limit is below yield); form error (tube ovality, wire out-of-round, bore straightness) is assumed zero, and a real pair spends part of the window on form before it spends any on size. (`mechanical/bus_mechanical.json` §`interference_window`)

🔬 **THE ENDURANCE BAND ENTERED THE MODEL 2026-09-12 — and one standing conclusion did not survive it.** `ENDURANCE_OVER_YIELD` was written as a BAND (0.40–0.50) in the constant's own comment from the file's first commit, while only its MIDPOINT ever entered the model; every SF scales linearly with it, so «the welded bare rod clears SF 2 for all six alloys» was a statement about one point of an unmeasured band wearing the clothes of a statement about the band. Swept (`§endurance_ratio_band`), the two standing conclusions split:

| `σ_e/σ_y` | bare: ∞-life | binding `Ta` SF | seam `k` → SF-2 (§5 corner) | our marker 0.50 |
|---|---|---|---|---|
| 0.40 | 5 / 6 | **1.96** | 0.624 | does not clear |
| 0.45 (model) | 6 / 6 | 2.21 | 0.554 | does not clear |
| 0.50 | 6 / 6 | 2.45 | **0.499** | clears, by 0.001 |

🔴 **Both standing conclusions FLIP inside the band — «all six clear SF 2» at 0.40, and «our as-printed marker covers the seam» at 0.50, where the marker clears the break-even by 0.001.** The second flip is the one worth reading twice, because until 2026-09-14 this line said the opposite — «fails at both ends, short by 0.001 at the friendliest», hence «the seam conclusion is robust to this coefficient». That sign came from the seam block reading the regime row's ROUNDED EI (+0.6 %); with the exact member the friendliest end crosses the marker. ⛔ So neither reading is a finding: a margin of a thousandth on a bound whose inputs are unmeasured is a TIE, and «robust» was a statement about the third decimal of a rounding. At the model's midpoint the marker still does not clear (−0.054). ⛔ The seam column is priced at §5's OWN corner (worst swept µ × span optimism) and pinned to equal its headline at the model's ratio, because the same quantity at a different corner is the substitution this file keeps catching elsewhere. ⛔ Which end to stand on is a ⚖️ (`00_07` HW.34) — the block measures, it does not choose, and the band ends are the constant's own comment, not a measurement of our alloys.

🔬 **WEAR BOUNDED 2026-09-12 — the ground the liner actually stands on stopped being an assertion.** ⚖️ 2026-09-11 replaced the liner's fatigue ground with WEAR, and for a day nothing computed it: every `wear`/`fretting` mention in script `55` was prose, one of them literally «The discriminating costs are NOT computed here», so «rated for 20 years» had no instrument while [`fmea_fmeca_register.md`](../../hardware/fmea_fmeca_register.md) `#21` — RPN 288, second only to `#4` — asserted wear-through with none either. Same inversion as the seam and the fit, third time: the specific wear rate of PEEK on Ti is in **no** canon row, **no** vendor answer and **no** experiment, so `wear_budget.specific_wear_rate_measured` stays `null` and the model prices the BUDGET. Chain, each link attackable on its own: **duty** = the sway-cycle count LOADED from script `62`'s real-wind cache (never retyped) × the slip per cycle from beam kinematics (`r·θ` at the contact, an upper bound by construction); **load** = the propped-cantilever reaction, bracketed over the active run; **budget** = wear-through volume ÷ (load × duty). Shipped branch at its worst corner:

| what the contact is | allowable `k` |
|---|---|
| flow-limited patch (PEEK at the tightest swept flow pressure) | **4.35 × 10⁻⁸** mm³/(N·m) |
| worn in over the whole run (projected bearing) | **2.21 × 10⁻⁴** mm³/(N·m) |

🔑 **Two findings the leg did not order, and both are worth more than the numbers.** **(1) At the tight end the reaction CANCELS.** The area is flow-limited (`A = R/p_flow`) and the duty is `R·s`, so `k = h/(p_flow·s)` exactly — the flow-limited budget contains no contact force at all, which takes the chain's weakest link (a single prop standing in for a distributed contact) out of the binding half of the answer. Pinned in-run against the closed form, not described. **(2) The two REJECTED conformal branches demand a rate more than an ORDER OF MAGNITUDE stricter than the shipped liner at every swept friction** (⚠️ the factor differs between the two branches, so a single figure for both would be false — read them per row) — i.e. the wear axis re-earns their rejection on its own terms, which matters because the protrusion correction above had weakened the ground that carried it. ⛔ Declared ceilings, all of them real: the budget's span is **5073×** and nothing in the tribology decides it — the CONTACT GEOMETRY does, and that is an open ⚖️ of ours (the bore entry carries a radius whose value is unnamed; the ratified protrusion decides which FEATURE of the tube meets the edge — end face or cylindrical flank — while the materials are polymer on titanium either way). Archard assumes GROSS slip, and at these amplitudes a real pair may be in partial slip, which removes less material and fails by fretting FATIGUE instead — a mode nothing here models. Third-body debris in a 25 µm clearance, the titanium side of the pair, and the whole −30…+40 °C dependence are outside the bound. The second driver — the liner's differential thermal travel — is priced and comes out **~4 orders** below the sway sliding, so «the sway drag is the wear axis» is DERIVED. (`mechanical/bus_mechanical.json` §`wear_budget`)

🔬 **CONTACT EQUILIBRIUM, script `68` (2026-09-14) — script 55's DRAG contact picture is not an equilibrium, and the stress the seam bound stands on moves with it.** `55` reads the wall station off the free tip-loaded cantilever at FULL drag; under a tip load the deflection grows monotonically from the root, so the first station to reach the wall is the pad plane, where the tube ends flush, and from then on the drag is reacted there. `68` solves the unilateral contact instead (Euler-Bernoulli FE, active set on prescribed displacements, rigid wall), with controls that can fail: the closed form at the pad plane, the closed form at the mouth, the wall at element midpoints, a grazing row at two element sizes; an adversarial review reproduced every returned number with a continuum-analytic solution and a KKT certificate. ⚠️ **The unsupported gap comes from `Z1_INSERTION_MM = 30`, an HW.8 placeholder that lies outside the Zone-1 lock's own insertion window, so each row is given at the placeholder AND at both window ends** (zero-interference play, bonded member, worst swept µ):

| Zone-1 insertion (geometry) | drag cap at the root, MPa | offset secant, MPa/µm | reversing-drag amplitude, max, MPa | pogo on the rod edge, MPa |
|---|---|---|---|---|
| 30 mm — script 55 placeholder, gap 6 mm | **8.04** | **5.74** | **8.04** | **10.52** |
| 15 mm — lock window near end, gap 21 mm | **2.88** | **0.46** | **3.85** | **5.37** |
| 14 mm — lock window far end, gap 22 mm | **2.73** | **0.42** | **3.76** | **5.22** |

🔑 **What moves.** (1) The coaxial drag cap sits well below both stresses `55` prices at that section above; ⛔ do NOT re-derive the seam `k` from the cap — a static offset puts a MEAN stress on the same section, and no mean-stress correction exists anywhere in the tree. (2) Under drag the contact is the tube's flush END at the pad-plane exit — ring on ring, the form the ≥ 1.0 mm protrusion avoids at the mouth — and no exit radius is specified anywhere. (3) Off-axis the mouth IS a contact station, so the protrusion and the entry radius keep a physical ground under misalignment. (4) In every configuration computed the root is the peak-moment section. (5) The insertion placeholder moves the offset price by an order of magnitude, so the coaxiality a drawing must demand cannot be priced before the insertion is (HW.26 G1); on the lock-window geometry an off-axis pogo contact and a reversing drag over an offset both exceed the coaxial cap. ⛔ Ceilings: rigid frictionless wall (no liner compliance or contact pressure), perfect clamp, offsets · tilts · eccentricity swept and never measured, no liner creep, no P-δ. (`mechanical/bus_contact_equilibrium.json`)
...
⚠️ And a second one since script 68: «sound» under DRAG stands on a coaxial channel; the static bending a channel off the root axis adds is priced by 68 and its magnitude waits on the Zone-1 insertion (HW.26 G1).
