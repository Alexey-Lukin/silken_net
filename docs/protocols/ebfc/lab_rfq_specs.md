...
> (`01_03` / `00_07` HW.5 / L1); **правити у домі, не тут** (One-Home, `00_06 §2`). Цей аркуш реферить,
...
>
> 🛑 **IP/embargo:** патент **не поданий**, repo публічний ([`patent_filing_readiness.md`](../anchor/patent_filing_readiness.md) §1).
> Розсилка детальних специфікацій CRO **під NDA** = розкриття **не-публічне** → новизну зберігає (на
> відміну від публічного repo). **Кожен RFQ — лише під підписаний NDA/CDA.**
...
2. **Перед відправкою:** підписати NDA (IP-причина вище). Cover-note має містити: коротку мету
   («academic R&D, tree-integrated EBFC»), **не** розкривати синергію/застосування понад необхідне для
   квоти, посилання на confidentiality.
...
| **IP** | NDA; ген — наша IP, CRO лише експресує |

> ⚠️ **Gate перед заморожуванням гена (sequence freeze ≠ now):** два in-silico кроки на dgr-мутанті ще
> відкриті ([`L1 §2`](in_silico/L1_protein_architecture.md), → `00_07` HW.5.IS):
...
| **IP** | NDA; за академ-партнерства — узгодити співавторство (`08_01 §2`) |
...
| **IP** | NDA; вибрати fluoropolymer-спеціалізований CRO |
...
- [ ] 👤 **NDA-шаблон** готовий до підпису (IP-причина — §IP вище + [`patent_filing_readiness.md`](../anchor/patent_filing_readiness.md)).
...
- [ ] 👤 **Spec B (ZIF)** — RFQ або ЧНУ-партнерство (`08_02`) паралельно.
- [ ] 👤 **Spec D (мембрана)** — окремий fluoropolymer-вендор (3–6 тиж).
- [ ] 👤 **Spec E (CNC)** — закупка ENERON.
- [ ] 👤 Усі deliverables → **Stage 2 Ti-coins (HW.24)**: in-vitro CV/EIS у синтетичному ксилемному соку (рецептура — біо-хаб ЧНУ, `08_02`), 30-day stability, chloride-tolerance (0.25 М NaCl), UCST winter-lock. Електрод із «вушком» під потенціостат-кліпсу, A=2 см².

> **Залежність:** Spec A–E живлять **Stage 2** гейту HW.24 (Ti-coins) → Stage 3 (full anchor) → Stage 4
> (100 шт). Передчасне замовлення 100-DMLS-партії без Stage 2/3 — методологічна помилка (`00_07` HW.24).
...
| [`patent_filing_readiness.md`](../anchor/patent_filing_readiness.md) | IP/embargo — чому RFQ лише під NDA |
