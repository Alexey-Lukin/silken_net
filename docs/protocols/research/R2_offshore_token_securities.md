...
**Метод:** public web-research (WebSearch/WebFetch, липень 2026) + verification-read репо-канону (`docs/05_03`, `docs/07_01`, `app/models/user.rb`, `app/models/wallet.rb`) — щоб securities-аналіз ґрунтувався на РЕАЛЬНІЙ токеноміці SilkenNet, не на абстрактній «типовій ICO-схемі».
...
Я перевірив `docs/05_03_Tokenomics_SCC_and_SFC.md`, `docs/07_01_Nature_as_a_Service_Contracts.md`, `app/models/user.rb`, `app/models/wallet.rb` (read-only). Факти:

1. **`NaasContract` — це модель "підписки", де клієнт ("Organization" АБО individual) платить `total_funding` і отримує SCC + SFC.** Документ `07_01` буквально каже: «клієнти... платять за моніторинг лісів і отримують натомість... SCC та... SFC».
2. **RBAC явно містить роль `investor` (`enum :role, { investor: 0, forester: 1, ... }`, `app/models/user.rb`).** Не метафора — literal enum value.
3. **`total_funding`** документовано в `04_01`/`07_01` як **«Загальна сума інвестиції (USDC/USD)»** — слово "інвестиції" в самому каноні, не моя інтерпретація.
...
5. **B2C track (`07_01 §1.2`)** прямо описує individuals, що хочуть **"монетизувати"** свій ліс: SCC + мікро-USDC-нагороди (0.01-0.0162 USDC/LoRa-пакет) + Celo ReFi (5 cUSD/добу) — DePIN-style "заробіток за участь" fact-pattern (та сама категорія, що привертала SEC-увагу до Helium-подібних мереж).
6. **SCC має floating DEX-ціну** (Uniswap V3 Quoter, fallback $25.50, `PriceOracleService`) — активний secondary-market price-discovery, класичний Howey-фактор («чи є ринок, де токен може зростати в ціні завдяки зусиллям promoter'а»).
