...
|---|---|---|---|
| **1a** | креслення пружини: d · D · n · вільна довжина · k · попереднє підтискання | **HW.9** нога 👤 «спитати креслення пружини» → **HW.43** нога 🔗 «прийняти у `59`» (`missing_datum` пружини — повне креслення: wire diameter · mean coil diameter · active coils · rate) | закриває ПЕРШИЙ із двох відсутніх входів рамки B. ⛔ Не закриває: амплітуду (HW.43 нога (1)) — отже й саму endurance-звірку |
...
| **1c** | сплав пружини для ОБОХ серій | **HW.43** ⚖️ 2026-09-18 (присуд — у Стані пункту) — Validation Gate [`02_02 §2.2`](../../02_02_Blind_Mate_Pogo_Pin_Interface.md) | «BeCu для обох» — присуд стоїть; інша пружина в 0908 → присуд повертається до «обрати серію й сплав» |
| **2a–d** | коди покриття · 30 µin · найтовше, що є · тверде | **HW.8.1** (P/N при BOM HW.9) · ⊕ у [`02_02 §2.2`](../../02_02_Blind_Mate_Pogo_Pin_Interface.md) «чи є варіант 30 µ″, не перевірено» | «є / код» — P/N рядка BOM; «немає» — ратифіковане 0.76 мкм зустрічається з каталогом, і це вже рішення founder-а, не листа |
