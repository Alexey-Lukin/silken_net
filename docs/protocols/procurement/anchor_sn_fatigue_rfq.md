...
| частота гойдання в полі | лист її не називає — «the service frequency itself is still being fixed»: канонна смуга 1–5 Гц не має джерела, а первинка описує частоту як функцію розміру дерева й породи (план §5.1) | HW.43 |
...
| хто виготовляє зразки й яким шляхом вони їдуть | «would reach you from the part manufacturer we select, or via us — confirmed before the order»: через нас, якщо серії кодуватимуться, бо сертифікат вендора в посилці розкрив би маршрут серії | HW.23 (п.15) · HW.24 (п.12) |
| сліпий аналіз (переглянуто) · чи кодувати до звіту статус bake | делеговано й переглянуто (§0 п.3): процедури й критерії — до випробувань, факти процесу — при замовленні; «The process information each series needs … is agreed at the order stage» лишає відкритим, чи статус bake серед них | HW.24 (п.12) |

---

## 3. Dispatch checklist (👤)

- [ ] 👤 **До відправки — лишити або перевернути делегований сліпий аналіз** (§0 п.3; запис і ціна — [`00_07`](../../00_07_Action_Plan_Tracker.md) HW.24). Повернення до повної сліпоти (кодування серій) змінює: останній абзац «Scope of request» · «Stress levels» п.5 · фрактографію (критерії класифікації) · шлях посилки зразків (через нас, без сертифіката вендора). Повна відмова від сліпоти — ще й «Scope of request» (смуга σ_e) і «Stress levels» п.1–2.
...
- [ ] 👤 **Адресати — не контактовано.** Кандидати з desk-пошуку 2026-09-14 (публічні сторінки, реєстр НААУ й анотації; спроможність під наш зразок НЕ підтверджена ні в кого — лист її питає):
  - **ФМІ ім. Г.В. Карпенка НАНУ, Львів** — корозійно-втомні випробування заявлено послугою; акредитація НААУ 202167 називає втомну міцність, але чинна до **20.10.2026** (питати про поновлення), методи обсягу — ГОСТ/ДСТУ; ультразвуку не знайдено.
  - **ІЕЗ ім. Є.О. Патона НАНУ, Київ** — акредитація НААУ 20362 до 02.04.2030, в обсязі ISO 1099:2017; корозійної комірки й ультразвуку не знайдено; поруч із київським друком.
...
- [ ] 👤 **Зразки йдуть DMLS-листом, і його пп. 2–4 та 14 (HIP · орієнтація · bake · активація — «mandatory») не поширюються на серії, що від них відходять.** Питання вже стоїть у pre-qual листі ([`vendor_templates`](vendor_templates.md) §Processing п.16, разом із ціною свідків на розтяг кожної серії) — перевірити, що воно поїхало тим самим листом; сам рядок замовлення зразків пишеться після ⚖️ «хто виготовляє зразки» (план §9 п.15). ⚠️ Якщо серії кодуватимуться — посилка йде через нас, без сертифіката вендора всередині.
...
What we buy from you is the test: planning, machine time, fractography and traceable raw data. The specimens would reach you from the part manufacturer we select, or via us — confirmed before the order.
...
- Specimens would reach you from the manufacturer we select, which may be in Ukraine, or via us; tell us your receiving requirements.
