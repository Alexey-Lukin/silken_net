...
16. **Fatigue-test specimens — a possible later line, and one question now.** We may also order smooth fatigue-test specimens printed and post-processed along the route of the lattice part, so that a fatigue curve describes that route rather than a generic one; the specimen drawing would come from the test laboratory. Some of those specimen series would depart from items 2–4 **on purpose** — built without HIP, without the bake of item 4, or at a build orientation other than item 3's — because the difference between such series is what the test measures. **Items 2–4 stand for the parts; for those specimen series they would not apply, and a specimen built to them would be the wrong specimen.** Tell us whether you would accept such an order, whether you can print a specimen's gauge section to size without machining it, and whether you can machine the gauge of one series before surface activation (item 14); quote per specimen as an option.
