...
- **QC deliverables per batch:** SEM of the active face with Sa/Sv measurements per alloy · hydrogen content report (< 100 ppm) · ICP-MS of the final rinse water, with **Al < 1 ppb** for the aluminium-bearing alloys · material CoC per powder lot (lot/heat number, O/N/H chemistry, virgin-to-reused powder ratio declared) · dimensional report against the supplied geometry.
  - ⊕ **Oxygen acceptance limit — ask the supplier to state it against the standard, added 2026-09-12.** We already require the O/N/H numbers; what we never stated is what counts as a pass. The AM specs carry it: **ASTM F2924 (Grade 5) caps O at 0.20 wt%**, **ASTM F3001 (Grade 23 ELI) at 0.13 wt%**. This matters because powder REUSE picks oxygen up cycle by cycle, and an over-oxygenated lot loses ductility and fracture toughness while still looking like titanium on a CoC — which is precisely the failure a 20-year wind-cycled anchor cannot carry. ⚠️ Our sources for the two numbers are secondary (the primary ASTM texts are paywalled and unread), so the RFQ line asks the supplier to **declare the limit they certify to and the measured value**, rather than quoting the figure at them.
