...
- 8 економічних параметрів (tokenomics/minting/insurance + slashing/alerts) з bounds-clamp (§7); 8 Lorenz-ключів контракту — **DCI-locked**, свідомо НЕ синхронізуються (tripwire-WARN на голос — §7)
...
✅ **Вікно дешевого рішення (до деплою [`SEC.1`](00_07_Action_Plan_Tracker)) використано:** фікс — рядок конструктора плюс тіло `quorum()`, без міграції токена; після mainnet це коштувало б редеплою Governor'а. Носій — `test_quorum_is4PercentOfMaxSupply` (`contracts/test/SilkenGovernor.t.sol`), і він пінить ДВІ речі: абсолютне число **і незалежність від обігу** — друга половина є саме тим, чого тихе повернення до `getPastTotalSupply` підробити не може. ⊕ Що присуд про базу quorum НЕ закрив і що лишається в [`00_07`](00_07_Action_Plan_Tracker) DOC-T.89: емісія SFC **не входить** до восьми економічних governance-ключів `ProtocolParameters` (перелічені §7), тоді як [`00_01`](00_01_Vision_Mission_and_Roadmap) обіцяє «DAO для управління параметрами емісії» — розбіжність між обіцянкою й реєстром ключів належить окремій нозі — **вона заведена 2026-08-27** (`DOC-T.89`, чекбокс «чи емісія SFC узагалі під DAO»), і разом із нею туди ж поїхала **стеля голосів на актора**: `SilkenGovernor` не перевизначає `_getVotes`, проксі в `contracts/` нуль, тож ГАК незворотний після деплою, а саме ЧИСЛО однорідної стелі лягає в `ProtocolParameters` живою ручкою — дві половини розводити.
...
>
> **Кандидати в партнери** (канон — [`00_02`](00_02_Academic_Integration_and_IP); контакту ще не було з жодним): Аблязов Д. (СЄУ, правова рамка — запит не надіслано), Карапетян (ЧДТУ, статистика «на виріст» — зустріч ще не відбулась).
...
⚫ **КОНЦЕПТ ЗНЯТО ⚖️ 2026-09-05 — не «відкладено», а void за конструкцією ([`00_07`](00_07_Action_Plan_Tracker) ARCH.17).** Він читався так: вартість мінтингу SCC алгоритмічно залежить від глобального показника здоров'я лісу — **середнього Z-value атрактора Лоренца** по всіх кластерах (здоровіший ліс → цінніший актив → більше growth_points за 1 SCC). Секція сама несла застереження «передбачає доведений Z↔health», і саме воно й спрацювало проти неї.

🔴 **Підстава — [`E.64`](00_07_Action_Plan_Tracker), 2026-09-05: Z є печаткою ЦІЛІСНОСТІ (DCI), а не оракулом здоров'я.** Хаотична динаміка підсилює розбіжність, тож Z доводить, що обидва боки рахували ОДНЕ І ТЕ САМЕ — і рівно нічого не каже про стан дерева (data-processing inequality). Серверний Z-присуд про здоров'я знято з коду того ж дня. Крива, чий єдиний вхід не несе потрібної величини, не стає кращою від відкладення.

⊕ **Три підстави, що стояли й до того, і жодна не залежить від Z:** ціни канон не має взагалі й відмова це моделювати ратифікована ([`00_07`](00_07_Action_Plan_Tracker) E.67 — «модель міряє КІЛЬКІСТЬ, не вартість»); `PriceOracleService` уже одного разу знято за нуль прод-споживачів ([`00_07`](00_07_Action_Plan_Tracker) DOC-T.89); і алгоритмічна крива ціни рухає токен у Howey-бік, від якого §07 свідомо відходить ([`00_07`](00_07_Action_Plan_Tracker) BIZ.22). Технічні заперечення, що стояли тут доти (складність аудиту, MEV-вектори, потреба в ліквідності) правдиві, але вони — про ЦІНУ механізму, тоді як вирок винесено його ПРЕДМЕТУ.

⛔ **Не відбудовувати як «те саме, але на TRL 9+».** Механізм цінового відкриття, якщо він колись знадобиться, є НОВИМ предметом: вхід має бути величиною, що справді вимірюється, а допустимість алгоритмічної ціни — питання securities-периметра, тобто присуд юриста раніше за дизайн.
...
- **Синхронізуються + читаються назад (повний DAO-ефект):** `emission_threshold` → `TokenomicsEvaluatorWorker.emission_threshold` (One-Home: селектор, `EvaluateTreeBatchWorker`-конверсія, `OracleVisionsController`; `MintingRollbackService` legacy-fallback свідомо на дефолт-константі — історичний refund не переоцінюється) · `slash_threshold` → `ContractHealthCheckService` (Rational-точність) · `stress_threshold` → `AiInsight.slash_stress_threshold` (спільний для тригера й damage-сайзингу — ARCH.46) · `slash_gamma`/`slash_penalty_factor_max` → `BlockchainBurningService` · `dynamic_tax_rate`/`insurance_pool_threshold` → `BlockchainMintingService` · `scc_per_tonne_co2` (синхронізується; прод-споживач прийде з carbon-registry інтеграцією — BIZ.1). Поза on-chain: kill-switches (`parametric_insurance_oracle_enabled`, `slash_cause_uplift_enabled`) та oracle-balance пороги (`INF.22` — ⚠️ тут стояв `E.51`, ID, перенумерований 2026-07-20 і в коді відсутній; фантом виправлено 2026-09-08) — admin-only за дизайном.
- **Bounds-clamp:** sync-воркер пише `min_value`/`max_value` (One-Home меж = `PARAMETER_MAP` ↔ `db/seeds.rb`) — out-of-bounds DAO-значення (клас «18-decimals мис-скейл», напр. tax=2e18 «200%» → `beneficiary_amount<0` → mint-halt) **відхиляється**: чинним лишається попереднє, `silkennet_governance_param_rejected_total` + ERROR-лог → потрібен коригувальний голос ([`06_03 §2.7`](06_03_Prometheus_Observability)). ⊕ Дротовано `sn-alert-governance-param-rejected` (2026-08-29, severity `info`): захист спрацював як задумано — ламати нічого, але **голос відбувся, а стан не змінився**, і без цього сигналу DAO вважає рішення застосованим. 🔴 **І ось асиметрія, без якої «bounds» читається сильніше, ніж є:** `ProtocolParameters.setParameter` значення НЕ ВАЛІДУЄ взагалі (перевіряє лише `key != 0`), тож on-chain простір параметра **безмежний**, а обмежений він лише на читанні назад. Наслідки два, і обидва несучі: (1) параметричний простір будь-якого стрес-свіпу задає таблиця `PARAMETER_MAP`, а не контракт; (2) стан «ланцюг ПРИЙНЯВ, бекенд ВІДКИНУВ» є реальним і спостережуваним — голос вважається ухваленим, а поведінка не міняється. ⚠️ Це не діра, а свідомий поділ (контракт лишається generic-реєстром), але модель економічної стабільності мусить покривати САМЕ РОЗРИВ — інваріант `slash_gamma_voted ⊥ slash_gamma` у `tools/tokenomics/supply_stress.rb` [E.67] пінить і ІСНУВАННЯ розриву, і його ЗНАК.
