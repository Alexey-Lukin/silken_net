...
- **Поточний TRL:** TRL 8 — **інструментарій** методології зібраний і частково CI-живий (labels-as-code, поле `R&D Cluster`, `trl_sync.yml`, cool-down-контракт §5.3). 🔴 **Це зрілість інструменту, не практики:** жодного Shape Up циклу ще не проведено, Betting Table жодного разу не збиралась (див. [`00_07`](00_07_Action_Plan_Tracker) OPS.3/OPS.6) — документ описує **метод, готовий до вмикання**, а не чинний ритм.
- **Контекст:** Синхронізація двох потоків: **Атоми** (Hardware) та **Байти** (Software: Rails, прошивка, смарт-контракти). ⚠️ Атоми-потік поки **цільовий**: лаб-доступ ЧНУ гейтований юр-review CDA/NDA ([`00_07`](00_07_Action_Plan_Tracker) BIZ.17), фабричний друк анкера не розпочато ([`00_07`](00_07_Action_Plan_Tracker) HW.1).
...
**Цільова** модель розподілу паралельних задач — **чотири кластери**; кожна Big Bet прив'язується до одного кластера у момент betting table. 🔴 **Сьогодні всі чотири кластери = Архітектор + AI-агенти:** ростер [`07_03 §1`](07_03_Academic_Integration_and_IP) — партнери **ідентифіковані, не законтрактовані** (режим passive, §07b трекера). Кластери тримаються в каноні не як штат, а як **партиція робочих доменів** — на ній стоять labeler-автороутинг, `CODEOWNERS` і поле `R&D Cluster` у Projects V2 ([`00_05 §2`](00_05_GitHub_Projects_and_IaC_Automation)).

> Ростер ВНЗ (хто / кафедра → що валідує) — канон [`07_03`](07_03_Academic_Integration_and_IP); колонка «Базова команда» нижче — **композиційний зріз** (кластер ↔ потенційна база), а не другий ростер: імена в ній = **дзеркало 07_03, правити там**, і жодне з них не законтрактоване (passive-режим, §07 трекера). Єдиний рядок, що описує реальний сьогоднішній склад, — кластер **C** («Architect + AI coding-agents»).
...
> **⚠️ Хто такий «Лід кластера»:** **роль-слот, який сьогодні виконує сам Архітектор** (проєкт solo; майбутній інженер вставляється зміною одного рядка в `CODEOWNERS` — той самий патерн уже описаний там). Це той, хто оперує GitHub (PR, approve, CI). 🔴 Слот **не заповнюється людьми сторонньої компанії-роботодавця**: SilkenNet — solo-mission, і залучення співробітників спільної компанії у власний процес суперечить IP-carve-out'у, який саме й розводить контури ([`00_07` BIZ.20](00_07_Action_Plan_Tracker)) — а канон у публічному repo є письмовим доказом такого залучення. Це **НЕ** професор ЧНУ/ЧМА/ЧІПБ — науковці не мають GitHub-акаунтів і не роблять Git-approve. Академічний «апрув» — це **підписаний офіційний лабораторний звіт / протокол (PDF/Markdown)**. Лід кластера агрегує ці звіти від університетів і конвертує їх у затверджений PR з посиланням на звіт. Тобто Git-процес — для інженерів Silken Net; підпис на лабораторному звіті — для науковців.
...
| **TRL 3-4** (PoC / Breadboard) | Лід кластера + Required CI checks — тепер **enforced** на захищеному `main` через агрегат `CI passed` (ci-ok: rubocop/rspec/brakeman/bundler-audit/host-firmware/ARM-build/scan-js/i18n/terraform-validate/docker-smoke). ⚠️ **Solidity і CodeQL — НЕ в складі `ci-ok`**: `Solidity passed` = власний required-агрегат `solidity_audit.yml`, а CodeQL — GitHub default-setup узагалі без workflow-файлу; повний перелік required-чеків = [`06_07 §2`](06_07_CICD_and_Runbook_Index). Для Atoms **підписаний фізичний лаб-звіт (in-vitro) = hard-тригер гейту 3→4** (in-silico сам по собі дає лише TRL 3, [`00_02 §5`](00_02_AI_Native_Engineering_and_TRL)). | Тільки на TRL Gate (перехід з 4 → 5). |
...
| **TRL 9** (Operational) | **SFC-голдери через DAO-vote**, на основі незалежного аудиторського звіту (ISO / D-MRV-аудит — профільний carbon-методолог, [`00_07` BIZ.9](00_07_Action_Plan_Tracker); не Гедз — фінанси). *Інструмент* переходу (НЕ суб'єкт рев'ю): Multi-sig + DAO Timelock 48h; Slither/Aderyn/Halmos/Medusa/Foundry — CI-гейти етапу коду, а не TRL-9-рев'ю. | Always + production sign-off. |
...
> **⚠️ Корекція:** 4-годинний синхронний колл (4 кластери × 15 хв презентації + 10 хв питань + огляди) для 5 ВНЗ і розподіленої команди = некерований хаос і марнування часу професури. Тому фази «Огляд» та «Презентація shape-документів» — **асинхронні** (Wiki за 3 дні); синхронно обговорюємо лише `Rabbit holes` і рішення `Drop/Park/Bet`.

> 🔴 **Усе нижче до кінця §5.2 — target-режим, не чинна практика.** Betting Table жодного разу не збиралась, учасників-представників кластерів не існує (§2), bandwidth-check рахувати нікому. Свідомо **не зрізано**: метод готовий і коштує нуль, доки лежить — активується з першим законтрактованим партнером. Для соло сьогодні працює вироджена форма: **Bet / Park / Drop у Week 0 — рішення Архітектора**, плюс anti-patterns нижче (вони й соло валідні). Саме на цю **подію** (а не на регламент) посилаються `github_bootstrap.rb` і `labels.yml`.
...
| :--- | :--- | :--- | :--- |
| TRL 4-5 (lab validation) | Fall (вересень — січень) | Лабораторні протоколи, перший draft публікації | UNI.2 (зустрічі ФОТІУС) + UNI.4 (школа Мінаєва) + UNI.5 (школа Гусака) |
| TRL 5-6 (relevant environment) | Spring (лютий — травень) | Магістерські та дипломні **захисти у червні** | UNI.13 (ЧМА) / UNI.9–UNI.11 (ЧДТУ) — наукова верифікація. ⚠️ НЕ UNI.14: то юр/обліковий воркшоп СЄУ, не наукова нога |
