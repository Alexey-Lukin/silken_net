...
**`Idempotency-Key`:** ✅ обов'язковий для JSON (відсутній → `400`); повтор із тим самим ключем → `200 OK` із тим самим тілом і БЕЗ другого запису. Скоуп ключа — **автор**, не запис: у момент перевірки запису ще не існує. Мотив [E.20]: offline-черга guild-клієнта повторює запит, чия відповідь загубилась, а на записах обслуговування рахується `critical_unmaintained?` у слешинг-тракті — тобто дубль тут не косметичний. Механізм — [`§7`](#7-заголовки-запитів) + concern `IdempotentRequest`.
