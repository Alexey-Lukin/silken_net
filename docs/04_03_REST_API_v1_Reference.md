...
- 🧱 **Форма — курована мапа, що ЛИШЕ СКОРОЧУЄТЬСЯ**, і вона обрана проти двох гірших: skip-list, який дозволяє дописувати винятки, гниє тихо й за півроку стає фактичною політикою (та сама підстава, за якою відхилено `verify_authorized` — §3); а «полагодити всі» купувало б вакуумні піни й з'їдений coverage-слек проти промаху, якого ніхто не побачить, бо гілки недосяжні з UI. Кожен запис реєстру несе **підставу і УМОВУ ПОВЕРНЕННЯ** — подію, після якої рядок зникає, а гілка стає must-fix («з'явиться лінк», «у формі з'явиться це поле»). Голий запис без них відхиляє власний ліхтар гейта.
...
- 🔒 Стелі названі в самому файлі; головна: гейт бачить СИНТАКСИС, не намір — чи маршрут браузерний, вирішує `routes.rb` (ARCH.77), тому реєстр є декларацією людини, а не висновком екстрактора.
