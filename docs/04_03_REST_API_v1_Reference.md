...
> 📐 **І ширша межа, ніж стеля гейта: ця таблиця документує МАРШРУТ, роль і призначення — але НЕ ФОРМУ ВІДПОВІДІ.** Які поля віддає кожна JSON-гілка, канон-дому не має **за побудовою**: рішення про payload живе в самому блупринті (`app/blueprints/*`) і в піні над ним. Тому питання «чи віддає ця гілка ПОНАД те, що показує сторінка» тут не резолвиться — його дім [`00_07`](00_07_Action_Plan_Tracker) SEC.36, а носіями рішень лишаються коментар блупринта + спека. Це не діра, а названа межа: знати її треба, щоб не шукати тут відповіді, якої сторінка не дає.
