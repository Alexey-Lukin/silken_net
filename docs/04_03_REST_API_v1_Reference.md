...
🔴 **Тут доти стояло «Rails сам віддає 303 для non-GET» — це НЕПРАВДА, виміряно рантаймом 2026-08-01** (`PATCH /settings` без явного статусу → **302**). Заява була несуча в найгіршому сенсі: на неї спиралось ціле сімейство редиректів, які через це лишались 302 після PATCH/DELETE. Механізм шкоди — той самий, що вже записаний для `destroy` ([UI.7], скіл `backend` #22): Turbo читає `_method` і шле **справжній** PATCH/DELETE (`turbo.js`, `determineFetchMethod`), а Fetch-спека конвертує 3xx у GET **лише для POST**, тож браузер перевидає той самий метод на адресу редиректу — де майже завжди зареєстровано лише `get`. Симптом найпідступніший: **дія вже відбулась, а користувач бачить 404**. ⚠️ Тобто правило ширше за `destroy`: `see_other` потрібен будь-якому редиректу з **не-POST** мутації, і `account_security` (11 сайтів) та `maintenance_records` жили без нього саме тому, що канон обіцяв автоматику. ✅ Виправлено 2026-08-01 [SEC.25 Ф4].
