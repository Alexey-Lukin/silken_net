# 04\_03 REST API v1 Reference

> **Статус:** TRL 4 — Повна синхронізація контрактів API з кодовою базою  
...
| 33 | GET | `/api/v1/gateways/:id/telemetry` | `telemetry#gateway_history` | 🔑 Auth | Телеметрія Gateway |
...
| 66 | POST | `/api/v1/oracle_callbacks` | `oracle_callbacks#create` | 🌐 Public | Chainlink Oracle callback |
| **🔔 Сповіщення** | | | | | |
...
| 77 | POST | `/api/v1/provisioning/register` | `provisioning#register` | 🌿 Forester | **Реєстрація нового вузла (Tree/Gateway)** |
...
**Призначення:** Ініціація нового Soldier (дерева) або Queen (шлюзу). Повертає AES-ключ та DID для прошивки.
...
> **Важливо:** `aes_key` повертається **лише один раз** при реєстрації. Зберігайте його в захищеній пам'яті мікроконтролера (Flash Option Bytes або OTP-region). Деталі реалізації зберігання та використання AES-ключа у прошивці: `docs/FIRMWARE.md`. Повторний запит для того ж `hardware_uid` поверне HTTP 409.
...
### 5.4 GET `/api/v1/gateways/:id/telemetry` — Телеметрія Gateway (Queen)
...
2. [Одноразово] POST /api/v1/provisioning/register
   → отримати DID та AES-ключ для кожного нового Soldier

3. [Регулярно] GET /api/v1/gateways/:id/telemetry
   → передати накопичену телеметрію (альтернативно: CoAP PUT на порт 5683)
...
5. [Автоматично] POST /api/v1/oracle_callbacks
   → Chainlink автоматично викликає цей endpoint після верифікації даних
```

> **Примітка:** Основний канал передачі телеметрії від Queen до Backend — **CoAP/UDP на порт 5683** (через CoAP listener daemon). HTTP API використовується для управління, реєстрації та звітності.
