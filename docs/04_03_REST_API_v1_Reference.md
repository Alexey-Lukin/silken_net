...
- `spec/requests/sidekiq_web_spec.rb` — **[ARCH.61]** `/sidekiq` route-constraint (анонім / org-`admin` / stale-cookie → 404; **`super_admin` → 200**). ⚠️ **Звужено ⚖️ 2026-09-05 ([`00_07`](00_07_Action_Plan_Tracker) S2.4): `admin` сюди БІЛЬШЕ НЕ ХОДИТЬ** — роль організаційна, а `Sidekiq::Web` платформенний і показує аргументи джоб УСІХ орендарів повз `acting_organization!`. ⛔ Не повертати `admin_or_above?`: збіг формул був би випадковим
