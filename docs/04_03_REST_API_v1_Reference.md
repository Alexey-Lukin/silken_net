...
- `spec/requests/sidekiq_web_spec.rb` — **[ARCH.61]** `/sidekiq` route-constraint (анонім / org-`admin` / stale-cookie → 404; **`super_admin` → 200**). ⚠️ **Звужено ⚖️ founder 2026-09-05 (носій підстави — коментар над `mount` у `config/routes.rb`): `admin` сюди БІЛЬШЕ НЕ ХОДИТЬ** — роль організаційна, а `Sidekiq::Web` платформенний і показує аргументи джоб УСІХ орендарів повз `acting_organization!`. ⛔ Не повертати `admin_or_above?`: збіг формул був би випадковим
...
>
> ⚠️ **Оголошена стеля — гейт судить рівно пару (метод, шлях).** Колонки `Доступ`, `Controller#Action` і `Опис` він НЕ звіряє: право доступу живе в `before_action`-ланцюгу й політиках, а зіставляти його з прозою означало б доводити узгодженість двох наших описів, ніколи коректність. **Рядок із хибним `🔑 Auth` пройде зеленим** — ця вісь лишається ручним семантичним аудитом ([`00_06 §3.0`](00_06_SSOT_Documentation_Standard)). ⊕ Гейт свідомо виключає `PUT`, що ділить шлях **І** `controller#action` із власним `PATCH` — це авто-дзеркало `resources`, не авторський ендпоінт (без цього правила точність падає з 2/2 до 2/5). ⛔ І він свідомо **не має рядка в реєстрі** [`00_06 §3`](00_06_SSOT_Documentation_Standard): джерелом істини мусить бути живий роутер, тобто потрібен Rails-boot, якого джоба `docs_check` не робить — а CHECK E реверс-перевіряє дослівну команду у workflow, тож рядок став би вічно червоним. Дім форми — сама ця секція плюс шапка спеки.

> 📐 **Межа ширша за стелю гейта, і вона про ЦЮ ТАБЛИЦЮ, не про документ:** §4 документує МАРШРУТ, роль і призначення, а форма відповіді живе в §2 (конверт) і §5 (per-endpoint тіла) — **для тих ендпоінтів, які §5 накриває**. Для решти гілок канон-дому немає, і носієм рішення про payload лишається коментар блупринта плюс пін над ним; питання «чи віддає гілка ПОНАД те, що показує сторінка» судить [`00_07`](00_07_Action_Plan_Tracker) SEC.36. 🔴 **І §5 проти блупринтів НЕ гейтований — розходження можливе й не червоніє нічим** (`route_doc_parity_spec` судить пару «метод, шлях», інші сторожі модуля 04 читають моделі й компоненти, не payload). Живий інстанс — §5.9b нижче.
...
| 57 | PATCH | `/alerts/:id/resolve` | `alerts#resolve` | 🌿 Forester | Закрити тривогу. **403** на доказі Категорії A (`vandalism_breach`): його відкликає лише платформа (`super_admin`, [`06_08 §4.6`](06_08_Resilience_and_Failover_Policy)) — лісник організації, яку доказ звинувачує, гасити його не може; браузер дістає редирект на сторінку тривоги з `error`-флешем [SLASH-1, 2026-09-22]. **409** на вже закритій (гонка подвійного кліку — броадкаст тротлений 5 с, тож оператор не бачить оновлення й тисне вдруге). Доти цей шлях кидав `AASM::InvalidTransition` у catch-all і віддавав **500**; `if @alert.resolve!` обіцяв булеву гілку, якої метод не має — він завершується `true` або кидає [SEC.25, 2026-07-30] |
...
> 🔴 **ТІЛО НИЖЧЕ РОЗІЙШЛОСЬ ІЗ КОДОМ — не правити його «за пам'яттю», спершу прочитати блупринт.** Від 2026-09-07 екшен віддає `AiInsightBlueprint` ([`00_07`](00_07_Action_Plan_Tracker) SEC.36), тож `cluster_id` і `recommendation` він БІЛЬШЕ НЕ несе, а `insight_type`, `probability_score` і `yield_impact` не документовані тут. Гейта на цю вісь немає (§4 ↑), тому розходження прожило непоміченим; зняття боргу — нога в [`00_07`](00_07_Action_Plan_Tracker) SEC.36.
...
| Поле | Тип | Значення |
|---|---|---|
| `total_contracted` | decimal | Σ `total_service_fee` активних контрактів організації — **USD**, плата за послугу ([`00_04 §5`](00_04_Nature_as_a_Service_Contracts)), не токени |
