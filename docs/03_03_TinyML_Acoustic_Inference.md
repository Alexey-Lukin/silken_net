# 03_03: TinyML Акустичний Інференс (Аналіз звуку пилки та low-freq water-stress proxy)

---

## 🎯 Мета

Задокументувати повний аудіо-пайплайн Edge AI вузла **Soldier**: від апаратного переривання (п'єзодиск → вібрація) через збір сигналу ADC/DMA у буфер RAM, нормалізацію у float, до запуску нейромережевого інференсу та прийняття рішення (low-freq water-stress proxy / бензопила / тиша — ⚠️ клас 2 НЕ є самою кавітацією: її акустична емісія 25–150 кГц лежить поза Nyquist цього тракту, докладно §4.2). Цей документ є SSOT для всіх команд, що будують на результатах TinyML: EwsAlert pipeline (03_05), Payload Packing (03_01), та backend TelemetryUnpackerService (04_02).
...
> **⚠️ Wakeup-чутливість — interrupt-storm: hardware-поріг ратифіковано ⚖️ founder 2026-09-10, номінали — bench.** П'єзо-EXTI спрацьовує на *будь-яку* вібрацію (вітер, дощ, гойдання гілок), не лише chainsaw/кавітацію; hardware-захист GPIO доти — лише `BAT54S` voltage-clamp ([`02_01`](02_01_Hardware_Architecture_and_BOM)), **без амплітудного порогу**. Ціна виміряна ([`00_07` HW.30](00_07_Action_Plan_Tracker)): кожне хибне пробудження = повний цикл ≈ 42 мДж з VSTOR (`E_active` [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power), ECB-ера), а Сценарій C стоїть у беззапасній точці — тож баланс перекидає вже **перше** хибне пробудження, а вітер дає десятки. NVIC race-guard (FW.11 вище) захищає від *race*, але **не** від *частоти* пробуджень. Mitigation, впорядкована за тим, що вона лікує: (1) **hardware R-дільник + RC-поріг перед EXTI** — перша лінія, DNP-контур у BOM ([`02_01 §3`](02_01_Hardware_Architecture_and_BOM) поз. 6): поріг ставиться понад вітер/дощ і нижче chainsaw-рівня — МЕТА, щоб будила пилка, а не погода; ⚠️ чи вона досяжна, не виміряно: амплітуд подій на анкері в дереві немає, а злива дає високий пік при малій енергії ([`acoustic_bench_sheet`](protocols/hardware/acoustic_bench_sheet.md) §5), і будь-який поріг понад погоду глушить заразом слабкий сигнал класу 2 (low-freq water-stress proxy), бо весь інференс іде лише під `if (vib)`; (2) software rate-limiter на кількість пробуджень/год — друга лінія (fast-amplitude gate одразу після wake економить лише ~21 % циклу (TinyML 9.0 з 42.33 мДж), бо стоїть ПІСЛЯ пробудження й ADC-семплу — не заміна порогу).

### 1.3 Периферія TinyML

| HAL Handle | Периферія | Роль у TinyML пайплайні |
|------------|-----------|-------------------------|
| `hadc` | ADC (12-bit SAR) | Оцифровка звуку з п'єзодиска — ⛔ **канал аудіо-входу на кремнії НЕ сконфігурований** (`MX_ADC_Init` порожній до board-freeze; див. ⚠️ під §2.1) |
| `htim2` | TIM2 (32-bit timer) | Метроном DMA: 16 kHz — ⛔ **те саме: хендл оголошений, `MX_TIM2_Init` порожній** |
| `hdma_adc` | DMA1 Channel 1 (DMAMUX `DMA_REQUEST_ADC`) | Перенесення ADC→`raw_audio_buffer` — ✅ **шар збудований** (`MX_DMA_Init` + `__HAL_LINKDMA` + вектори `DMA1_Channel1_IRQHandler`/`EXTI0_IRQHandler`, ARCH.102 2026-09-09) |
...
> ⛔ **[ARCH.102] ЦІ ПАРАМЕТРИ ОПИСУЮТЬ ПРОЄКТ, А НЕ ВІДВАНТАЖЕНЕ — але межа зсунулась 2026-09-09, тож читай її точно.** DMA-**шар** тепер існує (`MX_DMA_Init` + `__HAL_LINKDMA` + обидва вектори — раніше тут стояло «нуль хітів `MX_DMA_Init`», і це вже НЕ так). Досі бракує саме того, чого не можна вигадати без board-freeze ([`FW.46`](00_07_Action_Plan_Tracker)): **тіл `MX_ADC_Init`/`MX_TIM2_Init`** — тобто самого каналу аудіо-входу (канальна карта [`03_01`](03_01_Firmware_Lifecycle_and_DMA) оголошує лише температуру + VREFINT, аудіо-входу немає й на папері) та прескалера метронома від замороженого клок-дерева. Тому `hadc`/`htim2` тут лишаються цільовою конфігурацією, а `acoustic_events` із поля сьогодні джерела НЕ МАЄ. ⚠️ Змінилась при цьому ПОВЕДІНКА відмови, і це половина ARCH.102: доки конфігурації немає, Фаза 1.5 чесно НЕ міряє (гейт `Audio_Dma_Peripherals_Ready`) замість вішати вузол до сторожового пса. 🔴 Наслідок, який коштував окремого присуду, стоїть незмінним: `max_acoustic` не можна вживати як ПРЯМИЙ сигнал, доки тракт не поїде — саме через це бекендний ML-шар знято У-ВЕЙ ([`00_07`](00_07_Action_Plan_Tracker) E.52), а не «докалібровано». Стан і тригер розблокування — [`ARCH.102`](00_07_Action_Plan_Tracker).
...
// 2. Гейт ПЕРЕД HAL'ом [ARCH.102]: HAL_ADC_Start_DMA розіменовує
//    hadc.DMA_Handle без перевірки, тож незалінкований DMA дав би HardFault,
//    а не HAL_ERROR. Доки MX_ADC_Init/MX_TIM2_Init порожні (board-freeze,
//    FW.46), ця умова хибна і вікно чесно НЕ знімається.
if (Audio_Dma_Peripherals_Ready(hadc.Instance, htim2.Instance, hdma_adc.Instance)) {
    ...
    if (HAL_ADC_Start_DMA(&hadc, (uint32_t*)raw_audio_buffer, 512) == HAL_OK) {
        ...
        //    SysTick НЕ зупиняємо: із замороженим HAL_GetTick дедлайн ні з
        //    чого зробити, а ~32 зайвих пробудження дешевші за IWDG-ребут.
    ...
    // 5. Конвеєр зупиняємо НЕЗАЛЕЖНО від результату — озброєний DMA
    //    дописував би у буфер посеред наступних фаз.
    HAL_ADC_Stop_DMA(&hadc);
    HAL_TIM_Base_Stop(&htim2);
}
// Інференс біжить ЛИШЕ при audio_ready == AUDIO_DMA_DONE: усі три способи
// не виміряти (периферій немає · старт відмовив · дедлайн збіг) лишають
// прапорець в IDLE, а помилка ADC/DMA ставить AUDIO_DMA_ERROR.
```

> **[ARCH.102] Межі вікна — дім предикатів `firmware/common/audio_dma.h`** (One-Home: прошивка й host-сюїта компілюють той самий заголовок; тести — `make -C firmware/test tinyml`). Бюджет очікування `AUDIO_DMA_TIMEOUT_MS = 250` мс = 8× номіналу 32 мс і ~1% IWDG-вікна, віднімання тіків беззнакове (переживає 49-денний переворот `HAL_GetTick`). ⚠️ Це host/compile-рівень: кремнієм НЕ перевірено — стенд лишається гейтом ([`02_04`](02_04_Bench_Build_Guide)).
...
- **[SEC.10]** `panic_frame_counter` (saturating @ 0xFFFF) у байтах 14..15 (BE) — anti-replay nonce (сервер рубає повтори `Rails.cache` SET-NX — Solid Cache, не Redis), персиститься у DR0 негайно;
...
Для довідки: 1× LoRa-TX @ +14 dBm ≈ **12.8 мДж** на VOUT (SF9, `RFO_LP` 23.5 мА — [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power); «39 мДж» = TX списаного +22 dBm сценарію; ⚠️ 12.8 — ECB-ерний кадр 16 Б, під CCM 30 Б знаменник росте до ≈17.6 мДж, тобто кратність нижче лише ЗМЕНШУЄТЬСЯ — [`00_07`](00_07_Action_Plan_Tracker) ARCH.8); fauna-сесія ≈ **6× TX**. Бюджет EDLC 0.47F/5.5V (повний) = 7.1 Дж → fauna-доба = **~2.2 %** буфера. ⚠️ 5.5 В — абсолютний максимум деталі, а не стеля заряду: з ратифікованим `VBAT_OV` 4.822 В ([`02_03 §4`](02_03_BQ25570_MPPT_Nano_Power)) повний буфер ≈ 5.46 Дж, а робоче вікно 3.4 → 4.822 В — 2.75 Дж, тож та сама fauna-доба важить ≈ 2.9 % повного й ≈ 5.7 % робочого вікна (ті самі ≈ 0.16 Дж). Енергетично pivot сумісний з EBFC+EDLC.
...
⊕ **Спостерігач дротовано 2026-08-29** — `sn-alert-fauna-skip` (`rate(silkennet_fauna_skip_reports_total[15m]) > 0`, for 30m). Поодинокі пропуски взимку штатні; СТІЙКИЙ ненульовий rate = енергодефіцит кластера (деградація EDLC · затінення · обрив pogo-контакту), тобто питання до ЗАЛІЗА, не до софту. Дзеркало сусіда по тому ж diag-байту — `sn-alert-tinyml-threshold-invalid` (§5.4).
...
**Критичне обмеження:** STM32WLE5 (вузол `CC` · стенд `JC`; ARM Cortex-M4, C/C++) **не може** виконувати Ruby/Python артефакти (`.marshal`, `.pkl`, `.h5`). Допустимі формати для OTA-оновлення моделі:
...
Модель **обов'язково** повинна використовувати **INT8 post-training quantization** для розгортання на STM32WLE5 (вузол `CC`):
...
> - **Edge Reinforcement Learning:** tabular Q-learning з 12-state × 4-action lookup для прийняття рішень (sleep_extend / normal / sample_extra / emergency_tx); reward = days-to-next-VBAT_OK. State buffer — Flash-KV ([`03_01 §2.3`](03_01_Firmware_Lifecycle_and_DMA)) / SRAM (RTC-розкладка практично повна; DR20+ не існують на WLE5 — [`03_01 §2`](03_01_Firmware_Lifecycle_and_DMA)).
...
> - **Tabular Q-learning:** сам lookup дешевий, але reward «days-to-next-VBAT_OK» вимагає **тижневих** епізодів, а ε-greedy exploration з 0.1 ймовірністю «sample_extra» не має з чого платити: Сценарій C стоїть у беззапасній точці (≈0 мДж/год, [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power)).
...
> |--------|----------------|----------|
> | **L1 Soldier** | Inference-only: виконання **попередньо скомпільованого** mruby bytecode + емісія 1-bit stigmergic сигналу | STM32WLE5CC + 0.47F, headroom ≈0 (беззапасна точка Сценарію C, [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power)) |
