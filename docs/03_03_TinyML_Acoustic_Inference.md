...
|------------|-----------|-------------------------|
| `hadc` | ADC (12-bit SAR) | Оцифровка звуку з п'єзодиска — ⛔ **на кремнії НЕ ІНІЦІАЛІЗОВАНА** (див. ⚠️ під §2.1) |
| `htim2` | TIM2 (32-bit timer) | Метроном DMA: 16 kHz — ⛔ **те саме: хендл оголошений, тракт не збудований** |
...
> ⛔ **[ARCH.102] ЦІ ПАРАМЕТРИ ОПИСУЮТЬ ПРОЄКТ, А НЕ ВІДВАНТАЖЕНЕ.** DMA-аудіотракт на кремнії **не збудований**: `MX_DMA_Init` має НУЛЬ хітів у `firmware/` (те саме сказано в код-блоці нижче, але таблиця читається ПЕРШОЮ й доти казала протилежне — виправлено 2026-09-05). Отже `hadc`/`htim2`/`HAL_ADC_Start_DMA` тут — цільова конфігурація, а `acoustic_events` із поля сьогодні джерела НЕ МАЄ. 🔴 Наслідок, який коштував окремого присуду: `max_acoustic` не можна вживати як ПРЯМИЙ сигнал, доки тракт не поїде — саме через це бекендний ML-шар знято У-ВЕЙ ([`00_07`](00_07_Action_Plan_Tracker) E.52), а не «докалібровано». Стан і тригер розблокування — [`ARCH.102`](00_07_Action_Plan_Tracker).
...
// ⚠️ [ARCH.102] ЦЕ ЦІЛЬОВА СХЕМА, НЕ ВІДВАНТАЖЕНА: DMA-інфраструктури в прошивці
//    НЕМАЄ (`MX_DMA_Init` — нуль хітів у `firmware/`), тож на кремнії цей виклик
//    підвісить вузол до спрацювання сторожового пса. Тригер розблокування —
//    перший реальний записаний фрейм на стенді (02_04).
