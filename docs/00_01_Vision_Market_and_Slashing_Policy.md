...
- **Живлення з метаболізму:** Тризонний коаксіальний анкер ([`01_01`](01_01_Coaxial_Gyroid_Topology_and_PEEK)) — Ti-6Al-4V анод-гіроїд у заболоні з ферментами Combi-CLEA `GOx+Catalase` (Gen 1.0) або GDH (Gen 2.0) + PEEK-терморозрив + Ti-катодний фланець на межі кори/повітря з Laccase-AuNP DET та PTFE-GDL мембраною для атмосферного O₂. Розщеплює глюкозу ксилемного соку, генеруючи >500 mV.
...
- **Лабораторний етап:** Партнерство з ЧНУ (TRL 1-4). Staged validation gate (`01_01` §6.1): Stage 1 (5 SLA-макетів для form & fit) → Stage 2 (10 Ti-«монет» 10×10 мм для in vitro біохімії Combi-CLEA + Laccase-AuNP DET у синтетичному ксилемному соку) → Stage 3 (3–5 повноцінних SLM+HIP тризонних анкерів).
