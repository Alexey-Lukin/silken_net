...
- 🟡 **GAP (sap closed 2026-05-29; acoustic open):** heuristic-fallback (`calculate_stress_index_heuristic`, активний доки ML-модель не натренована) спирався на `max_status` (device-Z) + `avg_z` + temp, **ігноруючи `sap_flow`/acoustic**. ✅ **sap тепер вмонтовано** (`sap_stress_contribution`: signed below-baseline, **inert доки калібрування не задасть** `STRESS_SAP_LOW_THRESHOLD`+`STRESS_SAP_WEIGHT`; обмежено так, що sap **корелює, але не слешить сам** — status-0+sap ≈0.2 ≪ 0.83). Лишилось: **acoustic** (TinyML, `Run_Inference` закоментована) + ground-truth калібрування sap-порогу (`08_06 Завдання В` / `08_02 §4`). Принцип незмінний: high-stress вимагає прямого, некорельованого підтвердження (HW.19-аналог).
