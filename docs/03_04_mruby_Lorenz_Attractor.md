...
## 📊 8. Зведена Схема Потоку Даних

```
┌─────────────────────────────────────────────────────────────┐
│  ФАЗА 1: SENSE (firmware/soldier/main.c)                    │
│                                                             │
│  HAL_RNG → chaos_seed (uint32)  ─────────────────────────┐ │
│  ADC ch1  → internal_temp (int8, °C)  ──────────────────┐ │ │
│  RTC DR0  → acoustic_events (uint8) ───────────────────┐ │ │ │
│                                                        │ │ │ │
└────────────────────────────────────────────────────────┼─┼─┼─┘
                                                         │ │ │
┌────────────────────────────────────────────────────────▼─▼─▼─┐
│  ФАЗА 3: mruby Bio-Contract (bio_contract.rb)                 │
│                                                               │
│  calculate_state(seed, temp, acoustic)                        │
│       │                                                       │
│       ▼ Attractor.calculate_z_axis                            │
│  x₀, y₀, z₀ ← f(seed)          (початковий стан)            │
│  σ_eff ← clamp(10 + acoustic/10, 5, 30)                      │
│  ρ_eff ← clamp(28 + temp/5,     10, 50)                      │
│  β     = 8.0/3.0                (константа)                  │
│       │                                                       │
│       ▼ 250 × Euler step (DT=0.01)                           │
│  z_val = final Z-coordinate                                   │
│       │                                                       │
│       ▼ BioContract.evaluate_and_pack                         │
│  if z < 2.0:   status=1, gp=1                                │
│  if z > 45.0:  status=2, gp=0                                │
│  else:  deviation=|29.0 - z|                                  │
│         reward = 50 - deviation.to_i                          │
│         gp = clamp(reward > 0 ? reward : 10,  0, 63)         │
│       │                                                       │
│  payload_byte = (status << 6) | gp  ← 8-bit result           │
└────────────────────────────────────────────────────────────┬──┘
                                                             │
┌────────────────────────────────────────────────────────────▼──┐
│  ФАЗА 2: PACK → lora_payload[10] = payload_byte              │
│  AES-256-ECB → Radio.Send(16 bytes) → Queen → CoAP → Rails   │
└────────────────────────────────────────────────────────────┬──┘
                                                             │
┌────────────────────────────────────────────────────────────▼──┐
...
│  growth_points = payload[10] & 0x3F                          │
│  bio_status    = payload[10] >> 6                            │
│  z_server = SilkenNet::Attractor.calculate_z(seed, t, a)    │
│         (BigDecimal, незалежна верифікація)                   │
│                                                               │
│  → TelemetryLog.create!(z_value: z_server, ...)              │
│  → wallet.credit!(growth_points)                              │
│  → IotexVerificationWorker.perform_async                      │
└───────────────────────────────────────────────────────────────┘
```

---

## 🔢 9. Математичний Аудит: Зведена Таблиця Ризиків

| # | Ризик | Тип | Severity | BLOCKER | Статус |
|---|---|---|---|---|---|
| 1 | `delta_t`/`vcap` не передаються у атрактор (spec vs impl) | Архітектурний | 🔴 Critical | [BLOCKER-1](#-blocker-1-розбіжність-специфікації-та-реалізації-delta_t--vcap) | ⚠️ Потребує рішення |
| 2 | Коментар `ідеал (20.0)` суперечить `OPTIMAL_Z_TARGET=29.0` | Документація | 🔴 Critical | [BLOCKER-2](#-blocker-2-коментар-vs-константа-optimal_z_target) | ⚠️ Потребує виправлення |
| 3 | Стан `(x,y,z)` не зберігається між циклами STOP2 | Архітектурний | 🔴 Critical | [BLOCKER-3](#-blocker-3-відсутність-збереження-стану-x-y-z-між-циклами-stop2) | ⚠️ Потребує рішення |
| 4 | Float32 vs Float64 у mruby — невизначена точність | Числовий | 🔴 Critical | [BLOCKER-4](#-blocker-4-float32-vs-float64--невизначена-точність-mruby) | ⚠️ Потребує верифікації |
| 5 | Метод Ейлера, DT=0.01 — нестабільність при збурених параметрах | Числовий | 🟡 Medium | [BLOCKER-5](#-blocker-5-чисельна-нестабільність-методу-ейлера-при-dt001) | ✅ Прийнято (by design) |
| 6 | `.to_i` усічення у `deviation.to_i` (зона ±1.0 "безкоштовна") | Алгоритмічний | 🟡 Medium | [BLOCKER-6](#-blocker-6-усічення-deviationtoi-truncation-vs-round) | ✅ Документовано |
| 7 | `reward > 0 ? reward : 10` — недосяжна гілка (мертвий код) | Якість коду | 🟡 Medium | [BLOCKER-7](#-blocker-7-нижня-межа-growth_points-як-мертвий-код) | ✅ Документовано |
| N/A | Ризик `NaN`: відсутній — всі входи є цілими числами (Fixnum) | — | ✅ Відсутній | — | ✅ |
| N/A | Ризик ділення на нуль: відсутній — ділення тільки на константи ≠ 0 | — | ✅ Відсутній | — | ✅ |
| N/A | Ризик переповнення Float32: mруby Float + Clamp запобігає | — | ✅ Мінімальний | — | ✅ Clamp |
