...
| **Дострокове розірвання** (Early Exit клієнта) | `ContractTerminationService.call(contract)` | Sync (API call) | `SilkenCarbonCoin.sol` | `slashUpTo(wallet, min(net_minted_SCC_кластера, balanceOf))` — 🔴 [SLASH-1] **МОНЕТИ, не бали**, і база КЛАСТЕРНА, не контрактна (якщо `burn_accrued_points = true`; `contractual: true` — пропускає positive-A gate, бо це погоджена форфейтура, не slash-за-провину) | `NaasContract.status = :cancelled`, ⛔ **повернення НЕМАЄ** — передоплачена послуга не повертається, early-exit-fee не існує (⚖️ 2026-08-29, §4 нижче + `msa_skeleton §B.6.3`); клітинка обіцяла «повернення з вирахуванням штрафу» до 2026-09-04, тобто суперечила власному документові |
