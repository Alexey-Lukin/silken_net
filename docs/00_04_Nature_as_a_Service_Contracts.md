...
- Real-time dashboard зі станом лісового кластера (платформа + Prometheus/Grafana).
...
- SFC мінтується за ті ж самі кластери, що генерують SCC, але через окремий виклик `SilkenForestCoin.mint(to, amount, clusterId, archiveRoot)` з `MINTER_ROLE`. 🔴 **ОБІЦЯНКА, А НЕ ОПИС — код цього не робить** (виміряно 2026-08-26): `Wallet#lock_and_mint!` дефолтить `token_type: :carbon_coin`, і його ЄДИНИЙ живий викликач третього аргумента не передає, тож емісії SFC за ріст не існує. 🔴 **І живих writer'ів `forest_coin` у дереві НУЛЬ** (переміряно 2026-08-29): страхова виплата була останнім, і її знято ТИМ САМИМ заходом — енум `ParametricInsurance` звужено до `{ carbon_coin: 0 }`. ⚠️ Сигнатуру теж виправлено: аргументів ЧОТИРИ, не три. Правила емісії SFC (курс, бенефіціар, стеля голосів) не визначені ніде — осі присуду в [`00_07`](00_07_Action_Plan_Tracker) DOC-T.89.
...
| **Посуха** (`extreme_drought`) | `EwsAlert` `severe_drought` — ⛔ **живого писача НЕМАЄ** (⚖️ 2026-09-05, E.64 (A)): Z-смугу знято як вердикт про здоровʼя, виведений із Z, а єдина решта — ПРИСТРІЙНИЙ wire-статус `stress` — недосяжна за конструкцією (ρ-clamp тримає `z_eq ≥ 9`; нуль випадків на 5 000 прогонів, [`03_04 §5.3`](03_04_mruby_Lorenz_Attractor)). Перил юридично живий, машинного кандидата не породжує ніщо — рядок рухає лише людський Field-Audit, доки не з'явиться ПРЯМИЙ вимірювач вологи ([`00_07`](00_07_Action_Plan_Tracker) E.64) | `InsurancePayoutWorker` | `SilkenCarbonCoin.sol` або Etherisc DIP | `mint(to, payout)` або `triggerClaim()` | Параметрична виплата |
| **Дострокове розірвання** (Early Exit клієнта) | `ContractTerminationService.call(contract)` | Sync (API call) | `SilkenCarbonCoin.sol` | `slashUpTo(wallet, min(net_minted_SCC_кластера_за_ТЕРМІН_ДОГОВОРУ, balanceOf))` — 🔴 [SLASH-1] **МОНЕТИ, не бали**; координата бази кластерна, але вікно — **термін цього договору** (`start_date … cancelled_at`, ⚖️ 2026-09-04 — §4 нижче несе підставу). Доти вікна не було взагалі, тож вихід із продовження палив емісію попереднього, виконаного договору (якщо `burn_accrued_points = true`; `contractual: true` — пропускає positive-A gate, бо це погоджена форфейтура, не slash-за-провину) | `NaasContract.status = :cancelled`, ⛔ **повернення НЕМАЄ** — передоплачена послуга не повертається, early-exit-fee не існує (⚖️ 2026-08-29, §4 нижче + `msa_skeleton §B.6.3`); клітинка обіцяла «повернення з вирахуванням штрафу» до 2026-09-04, тобто суперечила власному документові |
...
| **Денне накопичення** | **Calibration-pending** (`delta_t` recharge-каденція + GP-магнітуда = placeholder, чекають bench-кривої, [E.63](00_07_Action_Plan_Tracker)). Self-consistent realistic (Variant C, `delta_t` ≈ 1.95 год — енергомодель з обраним радіо-фронтендом [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power), ⚖️ founder 2026-09-26, дім присуду [`02_06 §7.1`](02_06_Unit_Economics_and_BOM); CCM-кадр 30 Б зсуває каденс до ≈ 2.22 год, тобто пакетів на добу МЕНШЕ, [`00_07`](00_07_Action_Plan_Tracker) ARCH.8): ~12.3 пакети/добу × ~12 stored GP = **~148 growth_points/добу → ~5.4 SCC/дерево/рік**. ⚠️ Це точка моделі за 46 с від сходинки wire-GP 12 → 10 (там 4.46, −17 %, [`02_06 §7.1`](02_06_Unit_Economics_and_BOM)), тож у MSA/SLA її НЕ можна подавати як обіцянку продуктивності. Магнітуда = f(EBFC recharge): швидший `delta_t` → вище (фіз. стеля Δt=600s ≈ 326 SCC/рік; 1 TX/год = energy-negative без мітигацій). wire 5–31 × [FW.29] ×2 | [`05_03`](05_03_Tokenomics_SCC_and_SFC), [`02_06 §7.1`](02_06_Unit_Economics_and_BOM), [`02_03 §9`](02_03_BQ25570_MPPT_Nano_Power) |
...
>
> 🔴 **Що саме означає «нараховане» — ⚖️ присуд 2026-09-04 (делеговано founder'ом): емісія кластера, намінтована ПОКИ ЦЕЙ ДОГОВІР ДІЯВ** (`start_date … cancelled_at`), а не за весь час життя кластера. Доти вікна не було зовсім, тож достроковий вихід із місячного **продовження** палив емісію за роки під ПОПЕРЕДНІМ договором — повністю виконаним і оплаченим; це конфіскація, а не форфейтура, і напрямок помилки завжди був проти утримувача. 🔑 Присуд рухає код **У БІК** підписуваного документа, а не від нього: [`msa_skeleton §B.6.4`](protocols/legal/msa_skeleton.md) взагалі заперечує burn при termination-for-convenience («токен, раз виданий, є власністю Клієнта»), а кожна клієнт-видима поверхня — MSA, §5 нижче й усі чотири локалі (`contracts.show.legal.burn_points`) — каже «**нараховані**» без жодної шкали, тобто читається як «за цим договором». **Після присуду цей підпис став ПРАВДИВИМ** для кожного кластера з одним договором, включно з усім B2C (де кластер однодеревний за побудовою, §1.2). ⚖️ **Перетин термінів — присуд 2026-09-04: НЕ забороняємо, РОЗКРИВАЄМО.** Питання звучало «заборонити перетин чи попередити підписанта», і вимір зняв першу опцію: [`msa_skeleton §B.3`](protocols/legal/msa_skeleton.md) робить **Order Form** носієм ділянки, Service Term І обраного Tier'а, а сам Order Form «замінює `NaasContract`-запис» — отже зміна Tier'а всередині терміну природно дає ДРУГИЙ запис із перехлестом, і схемна заборона суперечила б структурі договору, який ми ж пишемо. ⊕ Другий вимір знімає й гостроту: `clusters.organization_id` один, а `activate!` не має ЖОДНОГО викликача в коді — договори переводить у дію людина з консолі, тож найчастіша форма співіснування є **достроковим продовженням**, чиї терміни НЕ перетинаються, і вікно розводить його точно. 🔴 **Тож лишається рівно обовʼязок РОЗКРИТТЯ, і він тепер формулюється точно:** база = емісія кластера за Service Term ЦЬОГО Order Form, а якщо термін іншого Order Form накриває той самий період, його емісія лежить усередині тієї ж бази — тобто достроковий вихід з одного залучення може спалити монети, за які платив і другий. Жоден підписуваний документ цього не каже; клауза належить [`msa_skeleton §B.6.4`](protocols/legal/msa_skeleton.md), а стан її написання — [`00_07`](00_07_Action_Plan_Tracker) `BIZ.2` (дім MSA). ⚠️ Нового пункту під це свідомо НЕ заводимо: відкритої роботи тут рівно одна — КЛАУЗА, а BIZ.2 уже закривав дефект того самого роду (подвійне визначення Tier, ⚖️ 2026-08-28). Обчислювальна половина лишається провенансом `SLASH-1`. ⛔ Носія «яка монета під яким договором» не заводимо: величина не має ВИЗНАЧЕННЯ, доки кластер несе кілька договорів ([ARCH.103] зняв `emitted_tokens` саме тому).
...
| Securities fact-pattern: інвест-МОВУ знято з вітрини й локалей, refund/exit-fee — з коду; **механіка знята НЕ вся** (живі pooled 5%/95%, форфейтура-опція, custodial-гаманець, DePIN-винагороди), і відкритий — висновок крипто-юриста на as-built перед першим live-mint | [`00_07`](00_07_Action_Plan_Tracker) BIZ.22 |
