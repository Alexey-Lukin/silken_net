...
>
> 🔴 **Що саме означає «нараховане» — ⚖️ присуд 2026-09-04 (делеговано founder'ом): емісія кластера, намінтована ПОКИ ЦЕЙ ДОГОВІР ДІЯВ** (`start_date … cancelled_at`), а не за весь час життя кластера. Доти вікна не було зовсім, тож достроковий вихід із місячного **продовження** палив емісію за роки під ПОПЕРЕДНІМ договором — повністю виконаним і оплаченим; це конфіскація, а не форфейтура, і напрямок помилки завжди був проти утримувача. 🔑 Присуд рухає код **У БІК** підписуваного документа, а не від нього: [`msa_skeleton §B.6.4`](protocols/legal/msa_skeleton.md) взагалі заперечує burn при termination-for-convenience («токен, раз виданий, є власністю Клієнта»), а кожна клієнт-видима поверхня — MSA, §5 нижче й усі чотири локалі (`contracts.show.legal.burn_points`) — каже «**нараховані**» без жодної шкали, тобто читається як «за цим договором». **Після присуду цей підпис став ПРАВДИВИМ** для кожного кластера з одним договором, включно з усім B2C (де кластер однодеревний за побудовою, §1.2). ⚠️ Лишається межа: при ДВОХ одночасних договорах на кластері монета з періоду перетину лежить в обох вікнах — і саме цієї можливості жоден підписуваний документ не називає. Стан → [`00_07`](00_07_Action_Plan_Tracker) SLASH-1. ⛔ Носія «яка монета під яким договором» не заводимо: величина не має ВИЗНАЧЕННЯ, доки кластер несе кілька договорів ([ARCH.103] зняв `emitted_tokens` саме тому).
