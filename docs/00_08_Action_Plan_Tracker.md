...
- [x] 🤖 **L2 OpenMM MD:** ✅ Виконано (2026-05-24). GcGDH(deglyco)+FAD+10×genipin у TIP3P/NaCl боксі (477k атомів), 100 ps production (min + NVT 50ps + NPT 100ps). Backbone RMSD 0.95 ± 0.20 Å (max 1.142 Å) **≪ 3 Å threshold** → ензим стабільний у геніпін-матриці при pH 4.5. 10.37 ns/day (Apple GPU, OpenCL). Скрипти: `tools/in_silico/scripts/10_genipin_stability_md.py` (+ `02_parameterize_fad.py`, `03_parameterize_genipin.py`). L2 → L3 gate відкрито.
...
- [x] 🤖 **Repo structure:** ✅ `docs/protocols/ebfc/in_silico/` + `tools/in_silico/` повністю укомплектовано (updated 2026-05-25). Реалізовані: `deglycosylate.rb`, `L1_protein_architecture.md`, `dgrGcGDH_AF3.pdb` (AF3 top model), `02_parameterize_fad.py`, `03_parameterize_genipin.py`, `10_genipin_stability_md.py` (L2), `20_dft_lumiflavin.py` + `21_dft_os_bipy_complex.py` (NH₃, superseded) + **`21b_dft_os_bpy_full.py`** (full bpy, current) + `22_compare_homo_lumo.py` (L3), `L3_quantum_chemistry.md`. CI gate: `.github/workflows/in_silico_smoke.yml`. Залишається: `cantera_psbma_diffusion.py` (L4).
