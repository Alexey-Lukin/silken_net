...
- **P2** · 🤖 · → `00_00`
- Реструктуризація 00_08 (#4) виявила факти, дубльовані у багатьох канон-доках — порушення «одна річ — одне місце». Призначити ОДИН дім + замінити решту на 1-рядковий ref (значення лишається ТІЛЬКИ в home):

| Факт | Канон-дім | Дублюється у (→ має рефати дім) |
|---|---|---|
| AES-режими per-channel (ECB→CCM / CBC 256) | `03_05 §3.7` | ~20 доків (00_01/00_02/03_01/03_02/04_02/04_03/05_01/05_02 + 08_xx) — **найбільший дубль** |
...
- [ ] 🤖 У кожному дублі замінити re-statement значення на 1-рядковий ref на home-док (значення — лише в home). **Прогрес:** ✅ Lorenz (05_02).
- [ ] 🤖 Розширити `tracker:check` на канон↔канон: детект hardcoded-значень поза home-доком (stretch — складно без AST доків).
- 📌 08_xx-академічні згадки часто легітимний контекст (не чистий дубль) — рефати, але не видаляти контекст.
- ✅ **Lorenz-pilot finding (2026-05-29):** аудит «14 доків» показав — справжню ПОВНУ ре-декларацію канон-блоку мала ЛИШЕ `05_02 «Фаза 2»`; решта — контекст / self-labeled mirror / академ. верифікація (легітимні, не стрипати). 05_02 → SSOT-ref `03_04 §4.1`; 11 значень верифіковано present у home ПЕРЕД стрипом (0 loss), метод-логіка (`calculate_z_axis`/`evaluate_and_pack`/`payload_byte`) лишена. **Безпечний патерн для решти фактів:** грепнути ПОВНІ ре-декларації (`grep -cE` блок ≥3-4 значень), а не згадки; верифікувати home містить КОЖНЕ значення; замінити лише декларацію, лишити контекст/логіку.
