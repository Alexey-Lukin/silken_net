...
- [x] 🤖 **L3 PySCF DFT:** ⚠️ Partial pass (2026-05-24). Pipeline PySCF 2.11.0 + B3LYP/6-31G(d)+LANL2DZ+ECP (Os)+C-PCM ✅. FAD: HOMO(FADH₂) = −5.14 eV, gap = 3.55 eV. Os (NH₃ surrogate): LUMO(Os(III)) = −3.42 eV. Raw Koopmans → UPHILL (артефакт: B3LYP −0.64 eV + відсутній π-backbonding bpy ≈ +1.22 eV). Bias-corrected → Δε ≈ **+0.14 eV downhill** ≡ exp. E°(Os)−E°(FAD) = +140 мВ (Cosnier 1999). Патентний клайм Gen 2.0 підтверджений. Definitive rerun: full [Os(bpy)₂(Im)Cl]⁺ + ωB97X-D/def2-TZVP + ΔSCF → Q1-стаття зі школою Мінаєва. Скрипти: `20_dft_lumiflavin.py`, `21_dft_os_bipy_complex.py`, `22_compare_homo_lumo.py`. Деталі → [`L3_quantum_chemistry.md`](../protocols/ebfc/in_silico/L3_quantum_chemistry.md).
...
- [x] 🤖 **Repo structure:** ✅ `docs/protocols/ebfc/in_silico/` + `tools/in_silico/` повністю укомплектовано (2026-05-24). Реалізовані: `deglycosylate.rb`, `L1_protein_architecture.md`, `dgrGcGDH_AF3.pdb` (AF3 top model), `02_parameterize_fad.py`, `03_parameterize_genipin.py`, `10_genipin_stability_md.py` (L2), `20_dft_lumiflavin.py` + `21_dft_os_bipy_complex.py` + `22_compare_homo_lumo.py` (L3), `L3_quantum_chemistry.md`. CI gate: `.github/workflows/in_silico_smoke.yml`. Залишається: `cantera_psbma_diffusion.py` (L4).
