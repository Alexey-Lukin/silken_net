# 09_05: SSOT Documentation Standard & Drift Prevention
...
- **Поточний TRL:** TRL 8 — стандарт впроваджено та **CI-enforced** (`docs:check_refs` + `tracker:check` як HARD-гейти у `docs.yml`/`ci.yml`). Відкриті — TRL range-consistency guard (roadmap) → [09_06](09_06_Action_Plan_Tracker).
...
| Ресурс | Опис |
|--------|------|
| [09_04_GitHub_Projects_and_IaC_Automation](09_04_GitHub_Projects_and_IaC_Automation) | Workflows, що ганяють ці гейти (`docs.yml`, `ssot_guard.yml`); Projects/Labels SSOT |
| [09_01_AI_Native_Engineering_and_TRL](09_01_AI_Native_Engineering_and_TRL) | AI-Native філософія + Wiki-First (no code until spec approved) |
| [09_02_TRL_Matrix_HIL_and_Beyond](09_02_TRL_Matrix_HIL_and_Beyond) | Per-module TRL matrix — канон-дім (§2 registry посилається сюди) |
...
| [09_06_Action_Plan_Tracker](09_06_Action_Plan_Tracker) | Канонічний дім блокерів (§1) + open backlog |
...
| `ssot_guard.yml` | protected code змінено → docs мусять оновитись | CI PR gate (09_04 §2.3) |
...
| TRL single-value | `09_02 §1` matrix-клітинки — одинарне 1-9, без діапазонів (09_04 §1.1) | `bin/rails docs:check_refs` (hard; `lib/docs_linter.rb`) |
| blocker-hygiene | канон-док не тримає `🛑 Блокери`/`✅ Архів` секцій — блокери лише в `09_06` (§1) | `bin/rails docs:check_refs` (**HARD** — sweep завершено 2026-05-30) |
...
Коли SSOT-факт переростає свій док (тема ≈ пів-сторінки + розпорошена дублями по сусідах), її виносять у **власну канон-сторінку**. Метод (verified 05/07-реструктуризацією 2026-05-30: slashing `07_01 §6` → `05_05`, governance `05_03 §749` → `05_06`; та дворівневою реструктуризацією 2026-05-30: junk-drawer Module 00 → Tier I System Architecture + Tier II 07/09):
...
4. **Реєстр + індекс:** оновити `§2` home-registry, `00_00` reading-order, README; archival-pointer у `09_06` (DOC-row).
