...
Поточна архітектура записує state root напряму в Ethereum L1 — це найбезпечніший, але і найдорожчий варіант (gas costs ~$5-15 за транзакцію при нормальному ринку).

**Альтернатива:** [EigenLayer](https://www.eigenlayer.xyz/) Actively Validated Services (AVS) — децентралізована верифікація стану з безпекою Ethereum (restaked ETH), але без прямого L1 запису.

| Параметр | Пряме L1 якоріння (поточне) | EigenLayer AVS |
|----------|---------------------------|----------------|
| **Безпека** | Ethereum consensus (найвища) | Restaked ETH consensus (висока, залежить від TVL) |
| **Вартість** | ~$5-15 / тиждень | ~$0.01-0.10 / тиждень |
| **Латентність** | 12-60 сек (L1 finality) | Хвилини (AVS consensus) |
| **Аудиторська прийнятність** | ✅ Золотий стандарт | ⚠️ Нова технологія, не всі аудитори приймуть |
| **Складність** | Низька (1 TX) | Висока (AVS operator setup, slashing) |
