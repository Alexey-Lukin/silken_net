...
def generate_state_root
  ...
  ActiveRecord::Base.transaction(isolation: :repeatable_read) do
    ...
    #     (Σmints − Σburns, дискримінатор — колонка `direction` [ARCH.95]), спільний
    #     із ChainAudit. ⛔ НЕ деривувати напрямок із `sourceable_type`: та форма
    #     рахувала ESG-погашення емісією (погашення `sourceable` не має), завищуючи
    #     саме `leaf0` якоря і базу слешингу — заборона стоїть у самій моделі.
...
**Приклад ВІДТВОРЮВАНИЙ** — це його призначення, а не оздоба: скопіюй рядок `Payload` і візьми від нього SHA-256, отримаєш рівно `Result`. Значення взяті з колонки «Приклад» таблиці полів вище.

```
Payload:  "0.12500005e7|0.5e3|4250|a3f8c2d1e4b7f9a0c2e5d8b1f4a7e0d3c6b9e2a5f8c1d4e7b0a3f6c9d2e5b8f4|2026-03-23T03:00:01Z|0.48e2"
Result:   "977da85c20383e4660e72e0047515966d6934b7fc0d614e4130c4614a3aafcc9"  (64 hex chars = 32 bytes)
```

> 🔴 **Decimal-поля входять у хеш у формі `BigDecimal#to_s`, тобто в E-НОТАЦІЇ — `1 250 000.5` стає `0.12500005e7`, а не `1250000.5`.** Це не стиль запису прикладу, а те, що реально хешується: `aggregate_payload` інтерполює BigDecimal (`Wallet.sum(:balance).to_d` при generate, decimal-колонка при verify), і Ruby серіалізує його саме так. Три decimal-поля з шести — `total_growth_points`, `total_sfc`, `total_scc_supply`. **Аудитор, що підставить десяткову форму, отримає корінь, який не збіжиться з жодним якорем в Ethereum, і прочитає це як tamper** — тому приклад показує байти, а не людяне число. ⊕ Round-trip перевірено окремо й тримає: `to_s` нормалізує хвостові нулі, тож `numeric(30,6)` повертає з БД той самий рядок, що дав `sum().to_d` при генерації (`1250000.5` ⟷ `1250000.500000` → обидва `0.12500005e7`) — цілі й нуль так само. Тобто наша власна верифікація сходиться; розходився лише цей приклад. ⚠️ **Історія дефекту, бо вона називає його справжній рід [ARCH.117]:** тут стояв payload на **пʼять** полів (до-[ARCH.97]-ера, без хвостового `total_scc_supply`), із `chain_hash` на 60 hex і `Result` на 63 — тобто ні те, ні те не було валідним SHA-256, і жоден гейт цього не бачив: крок 18 смуги пінить дзеркала ФОРМУЛИ, ніколи приклад. **Правити цей блок — означає перерахувати `Result`, а не лише дописати поле.**
...
# (RpcConnectionPool#build_client) і перенакладає перед підписом
# (KeySigner#transact) — обидва рази ТЕ САМЕ 100/2 Gwei з ENV, бо
# запінена мережа виміру не питає взагалі  [ARCH.62]
       │
...
- **Ops-застереження ре-кластеризації:** переїзд дерева між кластерами робить tier1-пруфи всього старого кластера `subroot_diverged` для ВСІХ минулих якорів (збережений субкорінь без leaf-list), а листя самого переїхалого дерева дістає `unprovable_regrouped` — але лише якщо нового кластера в якорі не було [ARCH.70 ⚖️ 2026-08-29] — **генеруй/архівуй lineage-bundle ПЕРЕД ре-кластеризацією**.
