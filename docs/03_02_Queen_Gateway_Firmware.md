...
| `AT+CNMP=38` | `AT_INIT_BUDGET_MS` | Режим «лише LTE» (вимикає GSM, не NB-IoT) — SIMCom AT Command Manual V1.03 §5.2.16. ⚠️ Несучий і для антени поз. 11: GSM-стелі підсилення модуля нижчі за пік рекомендованої антени ([`queen_antenna_shortlist`](protocols/hardware/queen_antenna_shortlist.md) §2.1); провал команди досі не перевіряється ([`00_07`](00_07_Action_Plan_Tracker) HW.31) |
