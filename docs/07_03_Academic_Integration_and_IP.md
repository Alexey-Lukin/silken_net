...
Звести в один дім **зовнішній науковий шар** Silken Net: реєстр академічних партнерів (5 ВНЗ — хто/кафедра → що валідує → канон-дім), план спільних публікацій (Scopus/WoS), поставу інтелектуальної власності (defensive-publication + open-license) і бренд-архітектуру. Інженерна субстанція deliverable'ів **живе в каноні Tier I (01–06)** і реферується звідси, а не дублюється.
...
- [3. Розподіл Інтелектуальної Власності (IP Framework)](#-3-розподіл-інтелектуальної-власності-ip-framework)
...
**SSOT/IP:** числа — дім [`SUMMARY.md`](protocols/ebfc/in_silico/SUMMARY.md) (стаття реферить); сабміт **вільний** — publish-to-protect (§3), без патентного гейту.
...
## ⚖️ 3. Розподіл Інтелектуальної Власності (IP Framework)
...
1. **Defensive publication** — інвентивне ядро (Synergy A: EBFC = одночасно живлення + zero-noise `delta_t`-сенсор; Synergy B: gyroid triple-function) опубліковане як prior art → [`defensive_disclosure.md`](protocols/anchor/defensive_disclosure.md) (TDCommons + публічний repo + Стаття 1). Ландшафт новизни/анти-захоплення — [`prior_art_landscape.md`](protocols/anchor/prior_art_landscape.md).
...
3. **Trademark** — SilkenNet™ / GaiaNexus™ / SCC™ зарезервовані (захист довіри/бренду — справжній рів MRV-мережі); відкритими ліцензіями НЕ покриваються. Заявка через прямого повіреного УкрНОІВІ (§4).
...
🔴 **Межа розкриття названа явно (ратифіковано 2026-08-08), бо доти вона була наслідком конфігурації, а не рішенням.** `wiki.yml` авто-публікує **всі** `docs/NN_NN_*.md` у публічну GitHub-вікі на кожен push у `main`, і глоб включає [`00_07`](00_07_Action_Plan_Tracker) — тобто **реєстр відкритих блокерів**, із іменами файлів, методів і констант, а також ~409 явно названих стель приладів по канону й коду. Перевірено наскрізь: сторінка віддається без логіну, і несе пункти, дописані хвилинами раніше. Founder-присуд: **«нам нема чого приховувати»** — це узгоджено з AGPL-копілефтом і з нoгою «невідбирано» критерію місії ([`00_01 §1.1`](00_01_Vision_Mission_and_Roadmap)), тож лишається як є, **свідомо**.
...
- **Partner-внески** йдуть у repo під тими ж ліцензіями зі спільним авторством: R-аналітика (Карапетян), RF-дані (Гончаров / РТРС), акустичні калібрування (Базіло / Бондаренко / ПМКТ), юр-шаблони NaaS (Аблязов Д.), крипто-облік SCC (Гедз).

---

## 🏛️ 4. IP-інструменти (TISC + trademark + UA-юр-review)

> Під defensive-publication (§3) prior-art landscape уже готовий ([`prior_art_landscape.md`](protocols/anchor/prior_art_landscape.md)); лишаються TISC-консультація, trademark і точковий UA-юр-review.
...
**Точковий UA-юр-review:** RWA як `hadron_asset_id` vs Лісовий Кодекс/ПЗФ ([`00_07` BIZ.11](00_07_Action_Plan_Tracker)) · SCC utility-vs-security за ЗУ «Про віртуальні активи» + MiCA ([`05_03`](05_03_Tokenomics_SCC_and_SFC)) · NaaS у Civil Code + `parametric_insurance` ([`07_01`](07_01_Nature_as_a_Service_Contracts)) · AGPL-enforcement + open-license/AF3 (§3). Виконавець: **Аблязов Д.Е.** (СЄУ, персонально) + профільний крипто/IP-юрист TBD.
...
> **Рішення (2026-06-16, [`00_07` — BIZ.16](00_07_Action_Plan_Tracker)):** імена проєкту розведені **за висотою** — продукт vs планетарна федерація. Це модель найменування (naming), не IP-права (§3).
