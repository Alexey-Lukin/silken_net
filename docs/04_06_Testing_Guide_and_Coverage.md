...
**Seed-флак покриття (точний hunt).** Коли line/branch «плавають» між прогонами (маржа гейту їх ховає), винуватець — код, що вправляється лише за певного ПОРЯДКУ тестів. Інструмент: `scripts/coverage_seed_diff.rb` — порівнює два `.resultset.json` від двох fixed-seed прогонів і друкує рядки/гілки, чия покритість різниться (рецепт зняття пари — у шапці скрипта; SimpleCov мержить у 10-хв вікні, тож `.resultset.json` обовʼязково чистити МІЖ прогонами). Тріаж знахідок: класичні корені — **незворотні `Module#prepend` у спеках** (анонімний модуль тіньовить продакшн-метод до кінця процесу — решта сюїти тестує підміну; worked example — §B.5), memoization first-test-wins, class-level стан. Прецедент (2026-06-12, TEST.1): єдиними флоатерами всієї сюїти були 2 рядки + 2 гілки `sessions_controller#current_session` — `around`-хук препендив підміну за хибною преміссою «методу нема»; видалення препенду зробило покриття детермінованим. View-стаби з `prepend` (asset/route-хелпери, ідемпотентні patched-once гарди) — легітимні: тіньовлять фреймворкові методи поза скоупом покриття.
