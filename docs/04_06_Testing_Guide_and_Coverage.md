...
6. **Solidity** — naming: `test_` (happy), `testRevert_` (error), `testFuzz_` (fuzz), `check_` (Halmos symbolic proof, `test/symbolic/`), `property_` (Medusa fuzz, `test/medusa/`). Пара `test_`⊥`testRevert_` **HARD-gated** з 2026-08-09 (`scripts/solidity_test_naming_check.rb`, TEST.14): тіло з `expectRevert` під іменем `test_*` → RED. 🔴 **Наслідок, що робить переіменування тесту НЕ косметикою:** імʼя ключує `contracts/.gas-snapshot` **і** визначає 4-байтовий селектор, тобто зсуває порядок бінарного диспетчера — переіменування 22 тестів дало 22 зниклих + 22 нових ключі І 70 записів зі зміненим ГАЗОМ у сусідів. Будь-яка правка імені їде разом із `forge snapshot --no-match-test "invariant_|testFuzz_"`. CI-аудит-шари: static (Slither + Aderyn) · symbolic (Halmos — доводить інваріант для ВСІХ входів) · property-fuzz (Foundry invariant + Medusa). Інструменти/roadmap → [`05_03`](05_03_Tokenomics_SCC_and_SFC).
