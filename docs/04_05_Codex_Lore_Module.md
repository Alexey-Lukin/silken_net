...
Обґрунтування: українська + англійська — це SSOT-мови, і ми економимо один JOIN
+ одну залежність від гему. Якщо з'явиться третя мова — мігрувати додаванням
колонок; **не** ретрофітити `globalize`.

> 🔴 **ПЕРЕДУМОВА ЦЬОГО ADR ПОМЕРЛА, і рішення від того не змінилось — але речення, яким
> воно обґрунтовувалось, було ХИБНИМ** (виміряно 2026-08-14). Тут стояло «додаткові локалі
> **не заплановані**», тоді як `config.i18n.available_locales` = `%i[uk en lv lt]` уже давно.
> Тобто ADR спирався на факт, який спростовується одним рядком `config/application.rb`, і
> **жоден гейт цього не бачить за побудовою**: обидва твердження коректні кожне окремо, а
> суперечність між шаром лору й шаром конфігурації не порівнює ніщо.
>
> **Що чинне, переформульоване через ЦІНУ замість неіснуючого плану.** Резолвери
> (`Codex::Node#title`/`#subtitle`, `Codex::Realm#name`) роблять `locale.start_with?("uk")
> ? _uk : _en`, тож латвієць і литовець дістають англійський лор. Це **оголошена стеля**,
> не тихий дефект, і вона тримається на вимірі: прози lv/lt не існує взагалі (98 записів ×
> 2 мови), балтійських партнерів у реєстрі [`07_03`](07_03_Academic_Integration_and_IP) немає,
> а ратифікований закон масштабування каже, що вартість локалізації йде за **попитом**, не
> за каталогом ([`04_04 §8.1а`](04_04_Phlex_UI_and_Tailwind)). Лор — read-only наративний
> шар; він не несе жодного операційного рішення, тож англійська для lv/lt нікого не блокує.
>
> ⚠️ **І знай, чого коштуватиме зміна рішення, бо резолвер тут — НЕ важіль.** Його обходять
> дев'ятнадцять сайтів, що читають колонки напряму (включно з двома `aria-label`), плюс
> пʼять блупринтів, admin-`permit`, `search_title`, два trgm-індекси й пʼять YAML-сідів.
> Полагодити самі резолвери означало б лише змінити, ЯКИЙ із двох рядків показується.
> Відкрите питання (форма схеми: jsonb-per-locale ⊥ таблиця перекладів ⊥ locale-інваріантний
> slug + i18n-мітка) живе в [`00_07`](00_07_Action_Plan_Tracker) I18N.3, і воно
> **не є передумовою** для жодної іншої роботи.
...
Codex-UI тримає рівно **один** Stimulus-контролер, що дає UX, якого сервер дати
не може:
...
- ~~`codex--reveal`~~ → 🔴 **знято 2026-08-03 разом зі своїм єдиним споживачем**
  (`Codex::Discoveries::Toast`, 0 продових інстанціювань за всю історію репо —
  `git log -S` порожній; компонент до того ж інтерполював `@node.title_en`
  напряму, хоча `Codex::Node#title(locale)` сам обирає мову, тож оживлення
  «як є» дало б англійський заголовок усім локалям). Підстава ADR була «без JS
  тост висить вічно» (auto-dismiss через 8 с + пауза на hover) — і саме її
  скасував пізніший присуд платформи: повідомлення, що живе до наступної
  навігації, — норма ([`04_04 §6.1`](04_04_Phlex_UI_and_Tailwind)), а часовий
  ліміт, навпаки, сам створює WCAG 2.2.1 («Timing Adjustable») зобов'язання
  дати подовжити/вимкнути — і на тачскріні (`hover: none`) паузи для нього
  фізично нема. Codex тут не виняток — вирівняно під загальний присуд, а не
  просто прибрано мертвий файл ([`00_07`](00_07_Action_Plan_Tracker) SEC.25).
...
⚠️ **Друга стеля ЗНЯТА присудом ⚖️ 2026-07-30** — тут стояло, що policy-скоупи дерев і
тривог кажуть протилежне (`OR cluster_id IS NULL` = «сирота видимий КОЖНІЙ
організації») і що дві семантики співіснують. Більше не співіснують: осиротілий вузол
не має екстенсіоналу (заведення вимагає кластер криптографічно — без нього не
деривується `K_ota`), `Cluster has_many :trees` став `restrict_with_error`, а
`OR ... IS NULL` знято звідусіль. Семантика одна: **безкластерний запис не належить
нікому**. ⚠️ Самі ті політики знято ЦІЛКОМ як мертвий код (⚖️ 2026-07-31,
[`04_03 §3`](04_03_REST_API_v1_Reference)), тож шукати підтвердження в `app/policies/`
більше немає сенсу: носієм семантики тепер є асоціації `Organization has_many
:trees/:ews_alerts, **through: :clusters**`, а виконуваним сторожем — два піни в
`spec/models/organization_spec.rb` (перенесені з policy-спек саме тому, що інші
`cluster: nil`-приклади в репо пінять СТВОРЕННЯ сироти, а не її невидимість). Розбір і підстава —
