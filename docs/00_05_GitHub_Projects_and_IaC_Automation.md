...
- 🔴 **`all:`-обгортка обов'язкова скрізь, де є негативні глоби.** Матчери під одним `changed-files` дефолтяться в `any:` (**OR**) → негативна клауза сама матчить кожен PR поза виключеними піддеревами, і лейбл липне на все. AND дає лише `- all:` з окремими `changed-files`-блоками. Цей баг був живий (C липнув на всі PR) і пофікшений 2026-07-16 — деталь у [`00_07`](00_07_Action_Plan_Tracker) OPS.3. **Машинно стережено з 2026-07-29** — `scripts/labeler_config_check.rb` (HARD `docs.yml`, [`00_06 §3`](00_06_SSOT_Documentation_Standard)); і точна умова строгіша за «є `all:` над негативом»: правила розкладаються в диз'юнкцію кон'юнкцій, і кожна гілка з негативом мусить нести позитивний глоб — бо негатив, що ділить один `changed-files` із позитивом, або `all:` лише з негативним блоком, обидва липнуть попри наявний `all:`.
...
- PR, що чіпає різні top-level дерева (`app/` + `contracts/`) — справді cross-cluster: auto-labeler чесно ставить **обидва** primary-лейбли (схема labeler'а правила «matched ≥2» не виражає — стабільно з v5, версія живе лише в SHA-піні workflow), і саме dual-primary Є машинним сигналом; `cluster:cross-cluster` як **транзитний** triage-маркер вішається вручну при розборі (§4.1) і резолвиться у рівно один драйвер + `cluster-ref:*`.
- **Ключ правила мусить існувати в [`.github/labels.yml`](../.github/labels.yml)** (Labels-as-Code, §4): phantom-ключ не помилка, а **тиха інертність** — labeler лейбла не створює, правило просто не стріляє, і жоден червоний крок про це не скаже. Саме так канон-дзеркало колись іменувало неіснуючий `module:06-infra` (§2.6-врізка вище). Стережено тим самим `labeler_config_check.rb`; перевірка одностороння НАВМИСНО — `labels.yml` ширший (`type:*`, `priority:*`, ручні triage-маркери на кшталт `cluster:cross-cluster`, які labeler не ставить).
...
| `cluster:cross-cluster` | `#F1E05A` (yellow) | **Транзитний triage-маркер** (НЕ resting-стан, НЕ власник): вішається **вручну** при triage multi-tree PR — авто-сигналом слугує dual-primary від labeler'а, бо схема labeler'а правила «matched ≥2» не виражає (§2.6). **Зобов'язаний** до Betting Table резолвитись у **рівно один** `cluster:*` (драйвер) + будь-яку к-сть `cluster-ref:*` (консультанти) — завдання не може лишатися «нічиїм» (100% accountability, [`00_04 §2`](00_04_Shape_Up_Operations_and_RnD_Clusters)). |
