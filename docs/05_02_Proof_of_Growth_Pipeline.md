...
- **Контракти:** `mint`/`mintForTree`/`batchMint` +`bytes32 archiveRoot` **симетрично SCC + SFC** (founder-рішення 2026-07-19 — один Ruby ABI, нуль per-token гілок); події `CarbonMinted`/`ForestMinted` +**indexed** archiveRoot (topic-lookup за root) + subgraph-поле.
