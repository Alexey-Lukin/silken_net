...
| **Субграф TheGraph** (`subgraph/subgraph.yaml`) | **ми** | `eventHandlers` — індексуються ЛИШЕ грошові події (`CarbonMinted`/`TokenSlashed`/`ForestMinted`/`GovernanceSlashed`). Поза вибіркою: `ParameterUpdated` (кожен голос про ставку/γ/поріг), `StateRootStored`, успадковані `RoleGranted`/`RoleRevoked`, `Paused`/`Unpaused`, увесь Timelock і Governor. 🔴 **Тобто платформена «правда» аудитора містить ГРОШІ й не містить ЗАКОНУ, за яким їх видали, ані того, хто закон міняв** — і саме тут вісь вибірки змикається з [`ARCH.112`](00_07_Action_Plan_Tracker) | ✅ тут (розширення набору — [`00_07`](00_07_Action_Plan_Tracker) ARCH.111) |
...
🔑 **Як читати колонку «стеля»:** ✅ означає, що вибір **оголошений** — у константі з підставою, в `VERIFICATION_INSTRUCTIONS` або в цьому рядку, — а не що він правильний. Оголошення — це все, що можна зробити машинно; чи виносити котрийсь із цих виборів з-під нашого контролю зовсім, вирішує ⚖️ у [`00_07`](00_07_Action_Plan_Tracker) ARCH.111.
