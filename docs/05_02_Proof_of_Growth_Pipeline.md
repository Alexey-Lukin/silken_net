...
>
> 🔴 **Таблиця вище — ПРОПОЗИЦІЯ, і вона РОЗІЙШЛАСЬ із тим, що реально засіяно** (виміряно 2026-09-05): `db/seeds.rb` дає сосні `critical_z_min` **5.0** (не 2.0) і дубу **8.0 / 45.0**. Живі значення — у сідах, ця таблиця їх не наздогнала. ⚠️ Різниця не косметична: саме засіяні числа годують `Tree#effective_lorenz_thresholds`, тож будь-яке міркування «на TRL-6 усі види на однакових дефолтах» вже неправдиве. Яке з двох правильне — ⚖️ некалібрований присуд, не факт; обидва набори чекають того самого ботанічного baseline. Стан → [`00_07`](00_07_Action_Plan_Tracker) FW.8.
...
50 слотів `EdgeCache`; слот, формати й уся логіка дедуп/вставки/витіснення — pure-заголовок `firmware/queen/cifo_cache.h` (канон — [`03_02 §2`](03_02_Queen_Gateway_Firmware)).

**Дедуплікація:** Останній пакет від кожного DID перезаписує попередній
(тільки найсвіжіші дані).

**DID = 0 зарезервовано:** бекенд відкидає нульовий DID в обох ерах (ARCH.54); здоровʼя Королеви їде не фальшивим деревом у батчі, а health-блоком у заголовку QATT-v2 ([`03_05 §2.2`](03_05_Hardware_Symmetric_Crypto_and_Security)).
...
| **L2** per-tree device-voice | **дерево саме** (SE050 non-extractable Ed25519, щотижневий Merkle-корінь) | повне device-origin, **операторо-непідробне** («голос дерева») | — | анкер-TRL + енергія (1 TX ≈ 12.8 мДж SF9 +14 dBm `RFO_LP` — ⚠️ **ECB-ера, payload 16 Б**; CCM-кадр 30 Б ≈ 17.6 мДж; Scenario C з обраним фронтендом — **беззапасна точка**, тож weekly Merkle-корінь (підпис + ≈ 5 LoRa-кадрів на тиждень — [`00_07`](00_07_Action_Plan_Tracker) SE050-MIGRATION) у запас уже не «влазить»: його ціну — довший Δt або дефіцит на тій самій каденції — міряє перерахунок [`ARCH.8`](00_07_Action_Plan_Tracker), а не переписування тут; daily-cadence = відкритий energy-⚖️ — не «39 мДж→Scenario D» [deprecated +22dBm], бо cold-boot/headroom поза цим числом; повний бюджет [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power)) + SE050 populate | **North-Star** (механізм = §E.60 + [`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security)) |

> **Крипта доповнює, не замінює:** L0–L2 доводять, що голос **дерева** і **цілий**; але **ЗВТ-метрологія** (STK.5) доводить, що голос **точний** (legal carbon) — 🏠 **внутрішній бюджет цієї точності рахує модель `tools/firmware/uncertainty_budget.rb`** (pure Ruby, `--assert` = HARD-гейт `docs.yml`; канон посилається сюди, не restate'ить числа): у робочій точці Variant C (Δt ≈ 1.95 год — енергомодель з обраним радіо-фронтендом, ⚖️ founder 2026-09-26, [`02_06 §7.1`](02_06_Unit_Economics_and_BOM)) комбінована `u_c ≈ 14%`, розширена `U(k=2) ≈ 28%`, і **86% дисперсії дає одна складова — `delta_t`, підсилений відображенням `m(Δt)` у ×4.9**. ⚠️ **Точність тут є функцією РОБОЧОЇ ТОЧКИ, а не лише приладу:** біля швидкого краю `U ≈ 5%`, біля повільного — `≈ 30%`, тобто енергетичний оптимум (Variant C обрано за [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power) з енергії — нині це беззапасна точка) і метрологічний оптимум **не збігаються**, і це ⊥ треба назвати метрологу першим. ⛔ Ланцюг бюджету зупинено на `SCC`: крок `SCC → tCO₂` (2000:1) — облікова конвенція, не вимір, тож інструментальної невизначеності не має за побудовою. ⛔ **І з цього прямо випливає межа ЛЕГАЛІЗАЦІЇ, а не лише бюджету** (⚖️ founder 2026-09-05, [`00_07`](00_07_Action_Plan_Tracker) STK.5): **повірочного шляху для КОМПОЗИТНОГО measurand'а «метаболічна жвавість → SCC» не існує** — і не через брак національного еталона чи експертизи, а за **КОНСТРУКЦІЄЮ**: останній крок ланцюга є домовленістю, а над домовленістю ЗВТ не будується. Тому ЗВТ, якого потребує ladder, є **ПОКАНАЛЬНИМ** (`internal_temp` → термометр-еталон · `vcap`-VDDA → вольтметр · `delta_t`/RTC → частотно-часовий еталон), і саме це є всім реальним змістом контакту з метрологом. ⚠️ **Межа присуду вузька:** він знімає ЛЕГАЛІЗАЦІЮ композита, не ТОЧНІСТЬ — `U(k=2) ≈ 28%` лишається чинним внутрішнім числом, а SCC як був, так і лишається utility-токеном, не compliance-інструментом. ⚠️ Дрейф-складові є **апріорними межами при нулі виміряних значень** ([ARCH.84] — писачів калібрувальних колонок у дереві немає), а **slashing** ([`05_05 §3`](05_05_Slashing_and_Risk_Policy)) робить брехню **дорогою**: у чинній Моделі A слеш падає на ту саму організацію, що фінансує кластер, тож skin-in-game дає вертикальна інтеграція, а не bond (⚫ `operator-bond`/BIZ.13 відкликано ⚖️ 2026-08-24 разом із Моделлю B — [`05_05 §3.1`](05_05_Slashing_and_Risk_Policy)). 🔴 Третя нога при цьому **неповна**: у Моделі A бенефіціар є ще й свідком фізичної роботи, тож ціна брехні тримає лише в парі з правилом «атестатор ≠ бенефіціар», чия форма ще не обрана ([`00_07`](00_07_Action_Plan_Tracker) E.20). Трійця (origin + accuracy + skin-in-game) = довірений RWA. Жоден рунг ladder не знімає потреби у ЗВТ та економічному шарі — ⚠️ **але ЗВТ тут читається ПОКАНАЛЬНИМ**, не сертифікацією композита: та знята присудом вище.
...
- **Побудова** (у `BlockchainMintingService`, per token-group, ПІСЛЯ KYC/SEC.13/circuit-фільтрів, ПОЗА oracle-локом): union вікон через `Mrv::LineageWindow` у **глобальному порядку `(created_at, id)`** → `archive_root = MerkleTree.root(leaf_cids)`. Batch-row + **set-once** `blockchain_transactions.archive_batch_id` для ВСІХ tx — в одній транзакції зі звіреним member-set (root-set ≡ bind-set СТРОГО): **partial-bind** (конкурент забрав частину tx між читанням членства і bind'ом) → rollback усього + до двох проходів перечитування, друга гонка поспіль → `build_failed` (fail-open); `create_or_find_by(archive_root, token_type)` → конкурентні build'и конвергують; re-dispatch групує по членству і реюзає stored root; transact-цикл — **per-підгрупою** («один on-chain виклик = один root» фізично, bisect у межах підгрупи), rescue живе пер-підгрупою (збій пізньої групи не чіпає вже-sent ранню), **claim під `FOR UPDATE`** бере лише вільні рядки — `:pending` і `:failed` без балів ([ARCH.62](00_07_Action_Plan_Tracker): retry після часткової multi-групової відмови й джоб зі застарілим статусом у памʼяті не сміють re-flip'ити чуже — double-mint guard).
...
⛔ **Преміса цього блоку СПРОСТОВАНА [ARCH.118 · ARCH.119, 2026-09-04], і механіка від того не змінилась — змінилось, що вона ЗНАЧИТЬ.** Модель була «Hadron-простій», вимір дав **неіснування вендора**: `Polygon::HadronComplianceService` є єдиним рантайм-писачем `hadron_kyc_status = "approved"`, а адресата в нього немає. Отже «auto-heal» щогодини переозброює драбину, якій нема куди дійти, і «тихий mint-skip» є не рідкісним крайовим випадком, а **постійним станом кожного custodial-бенефіціара**. ✅ **Лік ухвалено й відвантажено [ARCH.119, ⚖️ 2026-09-04]: enqueue гейтований `verification_reachable?`** (`configured? || simulation_allowed?` — дім предикатів у сервісі, [`04_02`](04_02_Business_Logic_and_Services)). Три наслідки, які варто читати разом. (1) Обидва `after_commit`-хуки й крон-ре-арм більше не ставлять джоб у проді без ключа — драбина не будується взагалі, замість того щоб будуватись і падати. (2) **dev/test не змінився**: `simulation_allowed?` там істинний, тож заглушка апрувить і мінт-демо живе — саме тому гейт стоїть на `verification_reachable?`, а не на `configured?`. (3) 🔴 **Статус бенефіціара при цьому НЕ покращився ані на крок** — він і далі вічно `pending`, і mint-skip лишається постійним станом; гейт зняв марну роботу, а не причину. Причина одна — вибір провайдера, [`00_07`](00_07_Action_Plan_Tracker) `BIZ.20`.
...
> - ⚖️ **Свіжість сигналу дерева НЕ є гейтом ЖОДНОГО шляху, і це ПРИСУД, а не пропуск** (SILENCE-1, 2026-09-04). `TokenomicsEvaluatorWorker.eligible_wallets` фільтрує лише `trees.status: :active`; `Wallet#lock_and_mint!` про `last_seen_at` не питає жодною гілкою; `Tree.silent`/`Tree#fresh_signal?` не читає ЖОДЕН money-path сайт. Підстава — гейт був би **порожній за побудовою**: `lock_and_mint!` блокує `(available/threshold).floor × threshold`, тож після кожного мінту залишок < порогу, а мовчазне дерево нового порогу не набере (воно перестало заробляти). ⛔ Утримувати мінт «про всяк випадок» відкинуто окремо: бали зароблені минулими кадрами, а тиша не є доказом смерті — машина знімає твердження про СИГНАЛ і не про світ за ним. 🔴 **Умова перегляду названа: поява будь-якого механізму, що накопичує хвіст ≥ порогу при мовчазному дереві** — і сьогодні такий існує (KYC-скіп: хвіст росте без стелі, при розблокуванні мінтиться ВЕСЬ, включно з періодом тиші). Тобто ризик несе не тиша, а ПАРА «тиша × заблокований мінт».
> - ⚖️ **ДЗЕРКАЛЬНА половина того самого присуду: тиша не ЗАРОБЛЯЄ — окремого time-weighted нарахування GP за pulse не буде** (SILENCE-1, 2026-09-04; кредит-половину ратифіковано раніше — [`00_07`](00_07_Action_Plan_Tracker) ARCH.8 нога 3). Це **підстава під ⛔ вище** («бали зароблені минулими кадрами»), яка доти жила лише в трекері: `Wallet#credit!` має рівно ОДИН продовий виклик — `TelemetryUnpackerService#commit_telemetry` на ОТРИМАНИЙ кадр, тож нуль кадрів = нуль балів **за побудовою**, а не за політикою. ⊕ Відмова позитивна, не скупа: pulse засвідчує ІСНУВАННЯ, `m(delta_t)` міряє МЕТАБОЛІЗМ — довга тиша приходить великим `delta_t` і сама себе здешевлює, тож окремий шар рахував би той самий інтервал **удруге, ще й екстраполяцією замість виміру**. ⛔ Не відбудовувати: це заповнення прогалин ВЛАСНОЇ вибірки (контролер вибірки, не значень), тож повернути питання може лише новий ВИМІР, ніколи нова формула. ⚠️ **Межа прайсингу названа, щоб «інтервал уже все оплачує» не читалось сильніше за правду:** шкала `m` вироджується на `DELTA_T_SLOW_S` = 7200 с — далі бали стоять на підлозі `GP_HOMEO_MIN`, тобто дві години тиші й два тижні коштують ОДНАКОВО; вище того wire-`dT` ще й сатурується @0xFFFF (≈18.2 год). Формула, пороги й підлога — [`03_04 §4.3`](03_04_mruby_Lorenz_Attractor) (дім, тут не дублюємо).
...
**Після SEC.11** — обидві сторони мають однакову початкову точку, тому Float vs Float drift між ARM та x86 IEEE-754 за 250 ітерацій < 1e-12 (емпірично після FW.7 closure). `check_z_divergence!` зберігає категоричну невідповідність як safety net — ⚠️ **із названою межею, [FW.8] 2026-09-05: сітка натягнута на смугу ПРИСТРОЮ (глобальні 2.0/45.0), а не на per-species.** Доти вона судила за бажаними порогами, тобто ловила не підробку, а розбіжність КОНФІГУРАЦІЙ, і на чесному дереві давала фрод (виміряно: MAX-бік 1.4 % пакетів, MIN-бік 0.14 %). Ціна звуження названа вголос: підроблений `status_byte = homeostasis` із Z у смузі `(стеля_родини, стеля_пристрою]` цим чеком більше не ловиться — обмін свідомий, бо старе покриття ловило чесних РАЗОМ із підробленими й розрізнити їх не могло. Дім розведення — [`03_04 §5.3`](03_04_mruby_Lorenz_Attractor) і отримує hook для числового tolerance band (`(server_z - device_z).abs > 0.001`) — flip під feature-flag після інструментального вимірювання реального drift у польових умовах. Атакер без знання `K_seed` не може передбачити очікуваний Z → fake-телеметрія falls within `< 0.001` band з ймовірністю ~`6/45000` → детекція ≈ 99.99%.
...
🔭 **Друга вісь, дорожча за першу й окрема від неї: хто може КАНАЛ ВІДІБРАТИ.** Контроль вибірки питає «що свідок побачить»; ця вісь питає «чи свідок узагалі лишиться нашим». Виміряний інстанс: пряме супутникове джерело, яким заміщають зрізаного посередника, є службою **одного іноземного уряду** — тобто каналом, який можна відібрати рішенням, що від нас не залежить. ⚠️ Посередник цю ставку **номінально** хеджував багатоджерельністю — номінально, бо ми того хеджу не тримали жодного дня, отже НЕ втрачаємо його, а лише перестаємо вдавати, що він є. 🔑 **Наслідок для форми майбутньої задачі, і він не «свап постачальника»:** за цим же ARCH.111 незалежність не є КРАЩИМ оракулом — вона є множиною, якої ніхто не добирає, тож пряме джерело є **ПЕРШИМ свідком набору**, а не заміною одного посередника іншим. Один птах, порахований двічі, лишається одним птахом. Стан роботи → [`00_07`](00_07_Action_Plan_Tracker) `S3.2`.

| Оракул / агрегат | Хто обирає вибірку | Що саме обирається | Стеля оголошена? |
|---|---|---|---|
| **dClimate / FIRMS** (`Dclimate::VerificationService`) — ⚫ **канал НЕ підключений з 2026-09-05** (вендора зрізано ⚖️; рядок лишається, бо питання про вибірку переживе зміну джерела) | **ми** | точка (`EwsAlert#coordinates`) + вікно `FIRMS_WINDOW_DAYS` довкола `alert.created_at.to_date`. Отже **хто керує моментом алерту, керує сценою**: дата деривується з того, коли МИ помітили, не коли ГОРІЛО | ✅ константа з підставою + пін на обидві межі вікна |
