...
| 2026-04-22 | **ARCH.4/BIZ.4/E.35/E.1** — Governance DAO реалізовано. **SilkenGovernor.sol**: OZ Governor + GovernorVotes (flash loan defense via getPastVotes) + GovernorTimelockControl (48h) + GovernorCountingSimple + GovernorVotesQuorumFraction (4% quorum). votingDelay=7200 blocks (~1 day), votingPeriod=50400 blocks (~7 days), proposalThreshold=100 SFC. **SilkenTimelock.sol**: TimelockController з 48h мін. затримкою. **ProtocolParameters.sol**: on-chain registry з GOVERNANCE_ROLE, 13 well-known keys (Lorenz σ/ρ/β/dt/iterations/z_min/z_max/z_target, tokenomics emission_threshold/dynamic_tax_rate/insurance_pool_threshold, slashing slash_threshold/stress_threshold), generic setParameter/setParameters + named getters, admin protection. **Governance::ParameterSyncWorker**: queue web3_low, unique_for 12h, Eth::Contract + Web3::RpcConnectionPool, Timeout 10s per call, BigDecimal conversion, SystemParameter sync. **E.1** досліджено: SFC voting power коректно зменшується після slashing через ERC20Votes._update → _transferVotingUnits → checkpoint update (OZ v5). Docs оновлено: `05_03`, `10_02`, `foundry.toml`. |
