...
| # | Перевірка | Деталі |
|---|-----------|--------|
| **1** | **DNS propagation до `kamal setup`** | Після `terraform apply` скопіюй IP та створи A-запис (`api.silkennet.com → <IP>`). Дочекайся поширення: `dig api.silkennet.com` повертає правильний IP. **Тільки тоді** запускай `kamal setup`. Traefik використовує Let's Encrypt HTTP-01 challenge — якщо DNS не поширився, сертифікат не видасться, Traefik не підніметься, сервер не стартує. |
| **2** | **`.kamal/secrets` файл існує з реальними значеннями** | Kamal читає секрети з `.kamal/secrets` (не з environment). Створи файл локально з реальними значеннями для `RAILS_MASTER_KEY`, `DATABASE_URL`, `REDIS_URL`, `KREDIS_REDIS_URL`, `GCP_ARTIFACT_REGISTRY_KEY`. Без нього контейнери стартують, але одразу падають (немає доступу до БД або ключа дешифрування). |
| **3** | **Крипто-гаманці поповнені (Gas)** | Web3-воркери (`MintCarbonCoinWorker`, `EthereumAnchorWorker`, `SolanaMicroRewardWorker`, `CeloRewardWorker`) потребують нативної криптовалюти для оплати мережевих комісій. Поповни oracle-гаманець перед першим запуском Sidekiq: **MATIC** (Polygon), **ETH** (Ethereum L1), **SOL** (Solana), **CELO** (Celo). Навіть ідеальний код поверне "Insufficient Funds" — Sidekiq потоне у ретраях. |
| **4** | **LoRa-антена фізично підключена** | **КРИТИЧНО (апаратне).** Ніколи не подавай живлення на плату Queen або Soldier без антени на SMA/U.FL порту. Радіомодуль SX1262 відображає RF-енергію назад у чип (high VSWR) — RF front-end згоряє за мілісекунди. Незворотно. **Порядок:** підключи антену → подай живлення. |
| **5** | **Симетрія AES-ключів (Soldier ↔ Queen)** | Масив `aes_key[8]` у `firmware/soldier/main.c` та `firmware/queen/main.c` має бути **побітово ідентичним**. Один відмінний біт — Королева генерує сміття з кожного пакету Солдата. Ліс здається мовчазним без жодної помилки. Перевіряй перед кожним циклом прошивки. Виробничий ключ провізіонується через `/api/v1/provisioning` + `HardwareKeyService`. |
