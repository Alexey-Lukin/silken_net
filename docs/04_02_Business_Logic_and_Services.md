...
| `Web3::Explorer` | **[INF.27]** One-Home tx-лінків block-explorer'а: `tx_url(family, hash)` для `:evm` · `:solana` · `:celo` · `:ethereum`, шаблон обирається за ОГОЛОШЕНОЮ родиною чейну слоту (`Security::Web3NetworkGuard.chain_env` — той самий читач, що й обидва мігровані money-сайти; нерозпізнане → `mainnet`, як у гарда). 🔴 **Це не косметика:** `EthereumAnchor#etherscan_url` їде АУДИТОРОВІ як референс якоря в `Mrv::LineageReportService`, тож лінк не на той чейн читається як «якоря не існує», а не як «не те середовище». 🔑 Два доми стояли на ПРОТИЛЕЖНИХ боках осі — `BlockchainTransaction#explorer_url` був зашитий у testnet (хибний уже на mainnet), `etherscan_url` у mainnet (хибний із появою testnet-слоту); та сама геометрія, що в `Web3NetworkGuard#hardcoded_fallback_violations`, лише на READ-шляху. ⚠️ Celo-рядок веде на Blockscout, бо `alfajores-forno.celo-testnet.org` відповідає **NXDOMAIN** (виміряно 2026-08-30 із позитивним контролем; cLabs депрекейтнули Alfajores разом із сансетом Holesky) — RPC-двійники того ж мертвого хоста лишаються відкритим ⚖️ у [`00_07`](00_07_Action_Plan_Tracker) `ARCH.118` (літерал `polygon-rpc.com` у `MintingRollbackService`: перецілити чи зняти за Celo-прецедентом), бо вони грошові. 🔒 Оголошені стелі: вісь каже «testnet», не КОТРИЙ; і декларація процес-широка, тоді як чейн рядка — факт порядковий (безпечно, доки слоти тримають окремі бази). Носій — `spec/services/web3/explorer_spec.rb`, чий дискримінатор ПОЗИЧЕНО в boot-гарда (`TESTNET_MARKER`), тож «як виглядає testnet-адреса» лишається одним домом на write- і read-шлях. |
