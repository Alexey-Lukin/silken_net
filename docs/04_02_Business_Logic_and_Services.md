...
🔴 **Стеля діагностики `unprovable_regrouped` — вимір 2026-08-28 [ARCH.70].** Сайтів цього статусу ДВА, і вони різні за причиною. Перший (порівняння `cluster_id` проти застиглого jsonb-снапшоту `subtree_roots`) ретеншном НЕдосяжний за побудовою. Другий злипає **три** стани, не два: перегрупування ⊥ втрата джерельних рядків ⊥ **підміна payload'а (tamper)** — і саме третій не має власного імені, тоді як сусідній `Mrv::TelemetryArchiveBatchService` зве його `mismatch`.

⛔ **Форму сиблінга сюди перенести НЕ можна:** його дискримінатор спирається на лічильник відновлених листів, якого цей тракт не має. Тож поки джерельні рядки не дропаються, діагноз чесний; **у день першого дропу ретеншну він стане НЕВІРНИМ для зовнішнього аудитора** — застарілі рядки він назве перегрупуванням.

⚖️ **Відкрите й невирішене — ФОРМАТ бандла, не код.** Новий статус ламає fail-closed офлайн-верифікатор (`scripts/verify_lineage_bundle.rb` має `else → failures`), тож додати ім'я = змінити контракт, який хтось уже міг узяти. Стан рішення → [`00_07`](00_07_Action_Plan_Tracker) ARCH.70.
