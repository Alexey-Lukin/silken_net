...
- **Обґрунтування:** Заглушки (dClimate, Puro.earth) замінено на бойові Web3/HTTP-клієнти. ⚠️ **«Клієнт бойовий» ≠ «канал живий», і різницю тут довелось купити виміром [ARCH.118]:** dClimate-база більше не одна на вендора (§13-рядок), Puro перецілено, Hadron не має адресата взагалі. TRL-8 тут стверджує зрілість НАШОГО коду; досяжність чужих ендпоінтів він не стверджує й стверджувати не може. Бізнес-логіка пройшла параноїдальний AI-аудит: повністю усунуто пастки `Network-in-Transaction`, витоки пам'яті (OOM) та ризики подвійної витрати (Double-Spend). Воркери fault-tolerant; money-path idempotency **шарова** — status guards / pessimistic lock (concurrent) + [ARCH.45] durable intent-marker + in-flight guard на on-chain↔DB crash-window (§4 / §10 / §11). **Примітка:** Chainlink dispatch демоутнуто до local correlation-marker **[ARCH.53]** — on-chain `sendRequest` вилучено (DON-callback unwired; LINK-cost без відповіді); мінт іде PATH 2 tokenomics, callback-endpoint live для майбутнього PATH 1.
- **Відкрите:** Planned-сервіси (Forester Guild → E.20, Cross-Registry → ARCH.5, ~~Federated Learning → E.52~~ — ⚠️ **бекендний ML-шар E.52 зрізано У-ВЕЙ 2026-09-05; предмет federated-навчання живе в `E.31` (residual — федеративний мікросервіс) і `ARCH.6`, не в E.52**) → [`00_07`](00_07_Action_Plan_Tracker); §13b doc↔code синхронність — enforced `model_doc_sync`-гейтом ([`00_06 §3`](00_06_SSOT_Documentation_Standard)), не ручний моніторинг.
...
2. **Multi-chain rails** (§10): сервіси для не-Polygon мереж (Solana, Celo, Ethereum L1, Filecoin, The Graph, dClimate, Klima, Hadron) — окремою секцією, бо вони побудовані по тому ж API-патерну (`Web3::RpcConnectionPool` + `Eth::Contract` / `Web3::HttpClient`).
...
> 🔴 **Мітка `[MAINNET READY]` судить НАШ КОД, і нічого не каже про ЧУЖИЙ КАНАЛ.** Вона означає: сервіс написаний під продову мережу, а не під заглушку. Вона **НЕ** означає, що зовнішній канал досяжний, що акаунт заведено, що креденшли видані або що інтеграція коли-небудь проходила наскрізь. Приклад стоїть поруч у цьому ж доці: `PuroEarth::PassportService` несе мітку, а `ORACLE_PURO_PRIVATE_KEY` є activation-gated і не заданий ніде. ⚠️ **Підміна вже стріляла назовні:** [`carbon_registry_matrix.md`](protocols/business/carbon_registry_matrix.md) двічі читав мітку як «канал уже працює» — у документі, який читає партнер. Клас — один токен, два домени; сестра-форма для колонки «Статус» — скіл `ssot-maintenance` §Guard-craft #131.
...
- [10. Домен: Мультичейн — Паралельні Рейки (Multi-chain)](#-10-домен-мультичейн--паралельні-рейки-multi-chain) — Solana, Celo, Klima, Hadron, Ethereum L1, Filecoin, The Graph, dClimate, Treasury
...
| `Web3::ResilientClient` | Обгортка навколо `Eth::Client` з автоматичним fallback cascade (Primary→Secondary→Public) та Circuit Breaker: `MAX_FAILURES=3` послідовних збоїв → провайдер вимикається на `CIRCUIT_OPEN_DURATION=60s`. Перелік `RETRIABLE_ERRORS` — `Net::ReadTimeout`, `Errno::ECONNREFUSED`, … і будь-який `IOError`, плюс текст HTTP 429. ⚠️ **Транспортний обрив на EVM-шляху до цього переліку НЕ доходить (виміряно 2026-09-23):** eth 0.5.17 кличе `.body` на `HTTPX::ErrorResponse` → `NoMethodError`, тож каскаду на фолбек немає й breaker його не рахує — відкрита нога [`00_07`](00_07_Action_Plan_Tracker) INF.22. 🔴 **[ARCH.62] `Eth::Client::RpcError < IOError`, тож доти каскад рахував збоєм провайдера КОЖНУ JSON-RPC-відповідь-помилку вузла** — реверт `eth_estimate_gas`, «nonce too low», «insufficient funds», «gas limit is too low»: три поспіль — breaker відкритий на здоровій інфраструктурі, і `sn-alert-circuit-breaker` будить оператора. З 2026-09-23 відповідь вузла про НАШ запит (`Web3::NodeAnswer.answered?`, рядок нижче) лічильника й `silkennet_rpc_errors_total` не бачить; ЧИТАННЯ з такою відповіддю raise-иться одразу (стеля: відповідь залежить від стану вузла — відсталий реверт оцінки газу свіжіший фолбек не переперевіряє; ціна лягає ДО мемпулу). ⛔ ЗАПИС каскадується далі (маршрут ратифікований, повтор безпечний закріпленим `nonce` — картка `Web3::KeySigner`), але з двома несучими межами: REJECTED зупиняє каскад (шопінг вузла, що прийме після чесної відмови, дає лише лімб), а раніша НЕОДНОЗНАЧНА помилка домінує над пізнішою REJECTED — інакше мінт прочитав би «не полетіла» й перемінтив новим `nonce`, а Celo перевиплатив би. ⚠️ «Та сама підписана tx» на фолбеку правдива лише для детермінованого підпису (`LocalEnvSigner`): кожен хоп перепідписує, і HSM (`KmsSigner`) дасть двійника з іншим хешем. ⚠️ Невідома JSON-RPC-помилка лишається провайдерською — множини є allowlist-ом, бо шлюз, що вже переслав tx, теж відповідає JSON-RPC-помилкою. ⊕ Реверт dry-run під бісекцією сюди НЕ доходив ніколи: `Eth::Client#call` сам перекидає `RpcError` як `ContractExecutionError < StandardError`. Thread-safe (Mutex). Метод `provider_health` для Prometheus-моніторингу. 🔴 **[ARCH.84] Звіт читає ЧИСТИЙ предикат `provider_reachable?`, і це інваріант, а не стиль:** доти він кликав мутуючий `provider_available?`, який при вичерпаному cooldown обнуляє лічильник, знімає `@circuit_opened_at` і переписує gauge — тобто **відкриття панелі здоровʼя напів-відкривало справжні circuit breaker`и**, а два оператори, що дивляться одночасно, міняли маршрутизацію RPC самим фактом перегляду. Розділено на пару: чистий `provider_reachable?` (звіт) ⊥ мутуючий `provider_available?` (лише диспетчер `available_urls`, бо саме він має право випробувати провайдера). ⚠️ **Дзеркало того самого дефекту жило в `Web3::HttpClient#circuit_status`** — методі, чий власний коментар каже «для моніторингу / Prometheus»; вилікувано тією ж парою. Носії — по одному приклад-піну на клас, обидва mutation-verified (звіт, що кличе мутуючий предикат, червонить поіменно). |
| `Web3::NodeAnswer` | **[ARCH.62, 2026-09-23]** One-Home питання «вузол ВІДПОВІВ про наш запит ⊥ провайдер зламався». Дві множини, межа між якими — безпека: `REJECTED` (кадр відкинуто на валідації чи виконанні, у мемпул він не потрапив → безпечний `fail!`/re-send; сюди ж газ-лімітна родина, що 2026-09-05 дала 44 мінти в `manual_review` з порожнім хешем) ⊥ `ALREADY_SUBMITTED` (слот `nonce` уже використано → ambiguous, ⛔ ніколи сліпий re-send). Читачі: `BlockchainMintingService#transact_error_pre_broadcast?` · `Celo::CommunityRewardService` · `Ethereum::StateAnchorService` · обидва breaker-и (`answered?`). Доти ті самі множини жили в трьох регулярках, що розійшлись: газ-лімітну родину знав лише мінт. ⚠️ `rejected?` судить ТЕКСТ, не клас — власний `InsufficientGasReserve` мусить читатись як вирок ноди — і ніколи не збігається з `ALREADY_SUBMITTED` (текст обох множин — неоднозначний, межу тримає рішення, не дані); `answered?` вужчий — найближча мережева причина в ланцюгу мусить бути `Eth::Client::RpcError` (транспорт, піднятий усередині rescue відповіді, лишається збоєм). Allowlist: невідома JSON-RPC-помилка лишається провайдерською. |
| `Web3::OracleSigner` | **[SEC.17]** One-Home «яка РОЛЬ якою парою ключів підписує». `for(role)` → підписант; ролі `:minter :slasher :celo :puro :klima :etherisc :anchor`, невідома → `ArgumentError`. Розвилка бекенду ТУТ і на жодному call-site: `ORACLE_<ROLE>_KMS_KEY_VERSION` присутній (`KMS_KEY_ENVS` — minter/slasher, ролі, що їх провіжить keyring) → `KmsSigner`, інакше `LocalEnvSigner` над plaintext deploy-ENV ([`06_04 §5.5`](06_04_Secrets_Checklist), ✅ код 2026-09-03). `address_for`/`resolvable?` — форма для read-only споживача (`Treasury::MonitorService`): `nil` = «бекенду нема» (гаманець не активовано), не `KeyError`. KMS-імена читаються `ENV[]`, не `ENV.fetch`, СВІДОМО — відсутність означає «роль на ENV-ключі», а їхню гучність несе гард A2 (формат · зомбі · колізія). ⛔ **ENV-імена мусять лишатись ЛІТЕРАЛЬНИМИ `ENV.fetch("…")`, по одному на роль:** `spec/deploy/env_fetch_declaration_spec.rb` [INF.12] сканує `app/**` саме на цю форму й рахує SET-DIFF — табличний lookup випав би зі скану, і гейт пройшов би ЗЕЛЕНИМ, ВТРАТИВШИ змінну. |
| `Web3::KeySigner` | **[SEC.17]** Спільна поверхня обох бекендів — `#address` · `#transact(client, …)` · `#static_call(client, …)` — над ОДНИМ ключ-обʼєктом, duck-типізованим на `#address` + `#sign(blob, chain_id)`: рівно те, чого торкається `Eth::Client#transact` → `Eth::Tx::Eip1559#sign` (виміряно по eth 0.5.17; `is_a?(Eth::Key)` на цьому шляху немає). `LocalEnvSigner` і `KmsSigner` — нащадки, що володіють лише тим, ЗВІДКИ ключ. ⊕ **Шов несе ДВА виміри газу, і другий є money-гардом [⚖️ 2026-09-07]:** `#gas_limit_for` (оцінка ліміту, `eth_estimate_gas × HEADROOM`) і `#assert_gas_reserve!` — предикат «чи проїде ЦЯ транзакція» за формулою самого EVM (`gasLimit × maxFeePerGas + value` проти балансу), що стоїть **ПОРУЧ** із `oracle_min_balance_*`, ніколи замість: поріг лишається ОПЕРАТОРСЬКОЮ шкалою (`ratio`, дашборд, два правила), бо константа не відповідає на питання ціни ні за яких обставин. ⛔ Дім тут єдиний можливий — `gas_limit` народжується всередині `#transact`, тож гард у сервісі другого множника не має; Solana поза периметром ЗА ПОБУДОВОЮ (не `Eth::Client`). Механіка, стеля fail-OPEN і контракт тексту винятку (`insufficient funds` читають ДВА доми — `Web3::NodeAnswer` і `Web3::TransactionErrorClassifier`) — у шапці `Web3::KeySigner::InsufficientGasReserve`. ⊕ **Третій вимір — `nonce`, закріплений ДО каскаду [ARCH.62, 2026-09-22]:** для `Web3::ResilientClient` шов бере `get_nonce` сам і передає його `nonce:`-kwargʼом, бо каскад повторює `transact` на фолбеку, а гем без `nonce:` бере там свіжий `pending`-слот — і фолбек, що вже бачить першу tx, дав би ДРУГУ з тими самими рядками. З одним слотом ланцюг сам гарантує не більше одного включення. Голий `Eth::Client` не чіпається (повтору нема), явний `nonce:` викликача (L1-якір, ARCH.66) бʼє закріплення; підстава — шапка `#pin_nonce`. |
| `Web3::KmsKey` · `Web3::KmsSigner` | **[SEC.17 ✅ код 2026-09-03; активація 👤 pre-mainnet]** Cloud-KMS бекенд: приватник живе в HSM (`EC_SIGN_SECP256K1_SHA256`, адресується key-VERSION), у дріт іде лише 32-байтний keccak-дайджест — поле зветься `digest.sha256` за ІМЕНЕМ алгоритму, значення наше (⛔ не хешувати вдруге). `KmsKey#sign` = DER `SEQUENCE{r,s}` через `OpenSSL::ASN1` → low-s (EIP-2; HSM не нормалізує) → recovery-id перебором обох парностей ПРОТИ власного публічного ключа (після нормалізації — вона парність фліпає) → `r‖s‖v` у кодуванні, байт-у-байт рівному `Eth::Key#sign`; `#address` з публічного ключа, мемоїзований (Kredis-lock ARCH.47). Транспорт — REST `cloudkms.googleapis.com/v1/{name}/publicKey` + `{name}:asymmetricSign` через `Web3::HttpClient` (пул + circuit breaker: відкрите коло = raise → ретрай джоби, нічого не підписується наосліп) під ADC-токеном `googleauth` — без `google-cloud-kms`/`grpc`. ⚠️ На VM потребує OAuth-скоупу `cloud-platform` (00_07 SEC.17 👤). Носії — `kms_key_spec` (фейк-HSM = локальний OpenSSL secp256k1: справжній DER і high-s; 96 підписів на трьох формах chain-id з доказом, що обидві парності й high-s траплялись; наскрізний `Eth::Tx::Eip1559#sign` → `Eth::Tx.decode` віддає адресу HSM як `sender`; негативи), `kms_signer_spec`, гард A2. |
| `Web3::LocalEnvSigner` | **[SEC.17]** Дефолтний бекенд підписання: `KeySigner` над `Eth::Key` з deploy-ENV — рівно те, що money-сервіси робили інлайном; kwargs проходять НАСКРІЗЬ недоторканими. 🔴 Клієнт — ПАРАМЕТР кожного виклику, ніколи не стан: `RpcConnectionPool` є per-thread кешем, тож підписант, що тримав би клієнта, дублював би той кеш. 🔴 `#address` віддає `Eth::Address` ВЕРБАТИМ — значення інтерполюється в Kredis-ключ `lock:web3:oracle:<addr>`, точку серіалізації nonce'ів [ARCH.47]; будь-яка нормалізація ПЕРЕСУВАЄ ключ і два процеси беруть різні локи на одну адресу. Blank-ключ → `ArgumentError`, бо `Eth::Key.new(priv: nil)` НЕ падає, а тихо генерує ВИПАДКОВУ пару. |
| `Web3::FeePolicy` | **[ARCH.62]** One-Home EIP-1559 fee. Накладається у ДВОХ моментах, і обидва несучі: на МІСЦІ НАРОДЖЕННЯ клієнта (`RpcConnectionPool#build_client`) — тож кожен клієнт дерева народжується вже з політикою, а не за памʼяттю автора нового money-сайту, — і ПЕРЕД КОЖНИМ ПІДПИСОМ (`Web3::KeySigner#transact` → `RpcConnectionPool#refresh_fee!`), бо виміряне число старіє разом із per-thread клієнтом. Народження лишається сіткою під порушенням SEC.17-шва; підпис — тим моментом, де ціна справді споживається. Мережа деривується СТАТИЧНО з `rpc_url_env_key` (`network_for`) — ні RPC, ні `chain_id` не потрібні. 🔴 **Проблема була не в тому, що стелі бракувало — вона СТОЯЛА, і обрав її гем:** `eth 0.5.17` присвоює fee у власному конструкторі (42.69 / 1.01 Gwei, позначені в ньому ж `# Do not use.`) і ціни з ноди не питає ніколи, а присвоєння в усьому `app/` існувало рівно одне — на L1. ⚠️ **Напрямок ризику інверсний до інтуїції:** `maxFee` — не стеля витрат, а підлога готовності майнитись (платиться `baseFee+priority`, `maxFee` лише обмежує), тож занизьке число не економить, а лишає tx у вічному `:sent` → `MintingRollbackService` → `manual_review`. 🔑 **ДВІ ПОЛІТИКИ, і порядок між ними — присуд (2026-09-05): ENV-число бʼє ВИМІР, мережа без ENV ПИТАЄТЬСЯ.** Ethereum несе успадковані 100/2 Gwei дефолтами `ENV.fetch`, тож поведінка якоря незмінна; Polygon і Celo ENV не мають ніде — саме вони й перейшли на вимір. Обидва числа мережі задаються РАЗОМ: cap без priority лишив би tip 1.01 Gwei, тобто «полагоджено» на вигляд і невключабельно насправді. 📐 **Вимір — не «краще», а ЄДИНЕ чесне, і це показав один прогін проти живих нод:** Polygon Amoy — tip ≈99.57 Gwei при базі 63 wei (чайова домінує), Celo — база 200 Gwei при чайовій 2.5 (домінує база). Форми ІНВЕРСНІ, тож жодна одностороння евристика не покриває обидві мережі, а спільного ЧИСЛА не існує в принципі; той самий прогін виніс вирок статус-кво — gem-cap 42.69 Gwei нижчий за потрібне на ОБОХ. Формула — `base × 2 + tip` (`BASE_FEE_HEADROOM`: за EIP-1559 база росте ≤12.5%/блок, тож 2× переживає ≈6 блоків). ⛔ **`eth_fee_history` НЕПРИДАТНИЙ через цей гем — виміряно, не припущено:** гем маршалить аргументи в hex-quantity рядки, а нода чекає `float64` у percentiles (`invalid argument 2: json: cannot unmarshal string into Go value of type float64`, падає і на `[50]`, і на `[50.0]`), тож cap рахується з `baseFeePerGas` останнього блоку. 🕰 **Освіження перед підписом — і ⛔ НЕ TTL, бо пускач відомий точно.** Доти вимір жив від народження клієнта до деплою, тобто число далі лише старіло; розвилку записували як «TTL на кеш пулу», і саме її зняв присуд «TTL є ПРОКСІ для „щось змінилось“ — при відомому пускачі гаси кеш на ПОДІЇ». Подія тут — мить перед підписом, тож освіження стоїть на шві `KeySigner#transact`, ПЕРЕД гейтом резерву (той множить `gas_limit` саме на `max_fee_per_gas`, тож освіження після нього судило б не ту транзакцію, яка поїхала). ⊕ Це заразом знімає потребу в ЧИСЛІ порога, якого не існує в принципі — форми Polygon і Celo інверсні. ⚠️ Ціна названа: два JSON-RPC читання на кожен transact мережі БЕЗ ENV-піна; запінена мережа (L1-якір) не дістає жодного — `apply!` віддає статику й виходить. 🔒 Решта оголошених стель: освіження стосується САМЕ fee, решта кешованого клієнта далі старіє; клієнт ПОЗА пулом не впізнається й не допитується (другий гард поруч із `MEASURABLE_CLIENTS`); transact-сайт повз `KeySigner` поїхав би з протухлим cap, і гейта на цю вісь немає; мертва нода веде на статику, статика — на гучне мовчання, і жодна сходинка не вигадує числа. ⚠️ `rescue StandardError` тестових дублів НЕ рятує — `MockExpectationError` є `Exception`, тож клієнт-мок мусить оголошувати й вимір. |
| `Web3::WeiConverter` | `BigDecimal`-based конвертація `amount → wei` (ERC-20). Запобігає Float-похибкам у фінансових операціях. |
| `Web3::Explorer` | **[INF.27]** One-Home tx-лінків block-explorer'а: `tx_url(family, hash)` для `:evm` · `:solana` · `:celo` · `:ethereum`, шаблон обирається за ОГОЛОШЕНОЮ родиною чейну слоту (`Security::Web3NetworkGuard.chain_env` — той самий читач, що й обидва мігровані money-сайти; нерозпізнане → `mainnet`, як у гарда). 🔴 **Це не косметика:** `EthereumAnchor#etherscan_url` їде АУДИТОРОВІ як референс якоря в `Mrv::LineageReportService`, тож лінк не на той чейн читається як «якоря не існує», а не як «не те середовище». 🔑 Два доми стояли на ПРОТИЛЕЖНИХ боках осі — `BlockchainTransaction#explorer_url` був зашитий у testnet (хибний уже на mainnet), `etherscan_url` у mainnet (хибний із появою testnet-слоту); та сама геометрія, що мав `Web3NetworkGuard` для зашитих фолбеків (тепер `armed_path_violations` — фолбеків у дереві нуль), лише на READ-шляху. ⚠️ Celo-рядок веде на Blockscout, бо `alfajores-forno.celo-testnet.org` відповідає **NXDOMAIN** (виміряно 2026-08-30 із позитивним контролем; cLabs депрекейтнули Alfajores разом із сансетом Holesky) — RPC-двійник того ж мертвого хоста — літерал `polygon-rpc.com` у `MintingRollbackService` — ЗНЯТО ⚖️ 2026-09-03 (`ARCH.118`): він був недосяжний за побудовою, бо presence `ALCHEMY_POLYGON_RPC_URL` з 09-02 вимагає гард на job і web. 🔒 Оголошені стелі: вісь каже «testnet», не КОТРИЙ; і декларація процес-широка, тоді як чейн рядка — факт порядковий (безпечно, доки слоти тримають окремі бази). Носій — `spec/services/web3/explorer_spec.rb`, чий дискримінатор ПОЗИЧЕНО в boot-гарда (`TESTNET_MARKER`), тож «як виглядає testnet-адреса» лишається одним домом на write- і read-шлях. |
...
| **Що робить** | Добова агрегація телеметрії → `AiInsight`. Включає: AI Fraud Guard (відхилення sap_flow/temp від кластерного базлайну > 30%), евристика `stress_index` — ⛔ **ЄДИНИЙ шлях із 2026-09-05**: бекендний ML-шар (Rumale RandomForest + `silken_forest.marshal` + SHA256-перевірка + `rake ai:train`) ЗНЯТО присудом У-ВЕЙ, бо легітимних входів у вектора виявилось НУЛЬ ([`00_07`](00_07_Action_Plan_Tracker) E.52). Евристика має ОГОЛОШЕНУ стелю `0.6 < 0.83`; ML її не мав. Денормалізує `tree.latest_stress_index`. ⛔ **Телеметрії НЕ чистить** — рядковий `cleanup_old_logs!` (`delete_all` старших 7 днів із гардом на `oracle_status`) зрізано присудом ⚖️ [`ARCH.59`](00_07_Action_Plan_Tracker) 2026-08-21: ретеншн сирої телеметрії робить ВИКЛЮЧНО дроп партицій, бо другий механізм зникнення рядків зробив би `TelemetryArchiveBatch.retention_expired` неоднозначним. Дім дозволу — [`04_01 §3`](04_01_Data_Models_and_Entities) `DOC.8`; фактичний ретеншн сьогодні **нульовий** (дропу ще не збудовано, [`ARCH.70`](00_07_Action_Plan_Tracker)). |
...
>
> 🔴 **[ARCH.84] Навчальний набір моделі стресу — ЛИШЕ деревні рядки, і дім множини — `AiInsight.stress_training_set`.** Тренер (`lib/tasks/ai_train.rake` — ⛔ **знято з дерева 2026-09-05** разом із бекендним ML-шаром; урок про ДАНІ його переживає, бо стереже НАСТУПНОГО тренера, а `AiInsight.stress_training_set` лишається живим скоупом зі спекою) будував вектор фіч із `average_temperature` + `reasoning[max_acoustic]` — ⚠️ **ДВІ фічі з 2026-09-05**, не чотири: `avg_z` вилучено як DCI-печатку ([`00_07`](00_07_Action_Plan_Tracker) E.64/E.52), `avg_vcap` як вироджений VDDA-проксі (FW.50/ARCH.99); механізм ARCH.84 від цього не змінився, змінилась лише АРНІСТЬ вектора — а КЛАСТЕРНИЙ рядок не має жодної з них і досі, він агрегат. Доти вибірка йшла `daily_health_summary.where.not(stress_index: nil)` без фільтра типу, і кожен кластерний рядок заходив у набір як **`[0.0, 0.0]`** (0 °C, нуль акустики) з міткою від кластерного середнього — виміряно рантаймом; на трьох деревах це чверть набору. ⚠️ Напрямок несучий: на здоровому лісі мітка `0`, тобто модель училась би, що нульові покази — норма, і мовчазний сенсор класифікувався б здоровим. ⊕ Фільтр живе СКОУПОМ, а не рядком у rake, саме тому, що rake-таска спеки не має — носій інакше не існував би ([`04_01 §7`](04_01_Data_Models_and_Entities) `AiInsight`).
>
...
| **Вихід** | `:slashed` / `:frozen` (positive-A gate · доказ витрачено слешем сусіднього договору [SLASH-1, 2026-09-23] · no-data magnitude [ARCH.46] — no burn; ключ Field-Audit `slash_frozen_{no_evidence,evidence_spent,indeterminate}_{cluster,tree}`) / `nil` (no-op або healthy-data 0-damage). Створює `BlockchainTransaction` intent (audit, `:pending`→`:sent`); **[ARCH.48]** rescue розрізняє: `LockTimeout`→intent `:failed`, контракт `:active`, retry re-slash; помилка `transact` (broadcast невідомий)→`escalate_to_review!` (`:manual_review`, in-flight guard блокує blind re-slash проти double-burn); крах після broadcast (`:sent`)→`:breached`. `:breached` ≡ slash-broadcast. Можливі outcome: `:slashed`/`:frozen`/`:manual_review`/`nil`. |

### `Slashing::CauseEvidence`

| | |
|---|---|
| **Файл** | `app/services/slashing/cause_evidence.rb` |
| **Вхід** | `Cluster` + `NaasContract` (обовʼязковий `contract:`) + опційне дерево-джерело; сигнали — `EwsAlert`-и кластера |
| **Що робить** | Класифікує причину деградації за Кат-A/B/C і відповідає на ОДНЕ питання, від якого залежить необоротність: **`positive_a?` — чи є ПРЯМИЙ доказ категорії A (tamper) для ЦЬОГО договору**. Доказ — незакритий critical-алерт типу з `EwsAlert::CATEGORY_A_EVIDENCE_TYPES`, записаний у термін ЧИННОГО договору (`active`, `start_date … cancelled_at || end_date`) і не старший за `slash_evidence_validity_hours` (дефолт 168, DAO-live). Закриває доказ лише платформа, тож відкритий він жив би скільки завгодно, а межі не дають йому спалити договір, підписаний після інциденту, прострочений-але-`active`, або договір на пізнішому, не повʼязаному тригері [SLASH-1, ⚖️ 2026-09-22]. 🔴 **Одна шкода — один слеш на кластер [SLASH-1, ⚖️ 2026-09-23]:** доказ «витрачено», щойно в реєстрі є не-`:failed` слеш-інтент СУСІДНЬОГО договору кластера, створений після нього (`spent?` → freeze з власною причиною) — похідне, без запису, тож revert сусіда повертає доказ сам; свої інтенти договору не рахуються. ⚠️ Правильне лише в одній транзакції зі створенням інтенту під рядковим локом кластера (`BlockchainBurningService#claim_verdict!`) — інакше два договори проходять ворота раніше, ніж хтось створить інтент. Будь-яка відсутня дата → «доказу немає» (freeze). `contract:` обовʼязковий — пропущений мовчки обирав би гілку без межі. Політика й повне визначення категорій — дім [`05_05 §3.2`](05_05_Slashing_and_Risk_Policy) |
| **Хто читає** | `BlockchainBurningService` — чокпоінт перед `slash()`: `positive_a?` хибне → `:frozen` + Field-Audit, **не** спалення [SLASH-1] |
| **Вихід** | булеві `positive_a?` · `spent?` (доказ є, але після нього вже записано спалення за сусіднім договором — слеш чи форфейтура; freeze не бреше «доказу немає») + символ-причина для аудит-сліду (`nil`, якщо живого доказу A немає) |
| **Спека** | `spec/services/slashing/cause_evidence_spec.rb` |

> ⚠️ **Автописача `vandalism_breach` НЕМАЄ**, тож доти, доки A-сет не наповнено, авто-slash фактично **freeze-only**: wire `status=3` є `vm_error` (софт-збій mruby-VM → `firmware_fault`), а справжня пилка їде panic-каналом → `chainsaw_detected`. Дім присуду — [`05_05 §3.2`](05_05_Slashing_and_Risk_Policy).
...
| **Що робить** | **[DOUBLE-SPEND GUARD]** Rollback при вичерпанні всіх Sidekiq-ретраїв у `MintCarbonCoinWorker`. Логіка вирішення: (1) `tx_hash` відсутній → безпечний rollback (транзакція не покинула бекенд): розблоковує `locked_balance`, маркує `status = :failed`; (2) `tx_hash` існує → перевіряє стан on-chain через RPC: а) receipt підтверджено → `tx.confirm!` (НЕ rollback); б) receipt null (pending) → `escalate_to_review!` (кошти залишаються заблокованими); в) RPC timeout → `escalate_to_review!`. Multichain: EVM-мережі використовують `eth_getTransactionReceipt`; Solana — `getTransaction` через прямий HTTP-запит. **[BUGFIX 2026-05-18 envelope-aware]** `fetch_evm_transaction_receipt` делегує до спільного **[ARCH.50]** `Web3::EvmReceiptClassifier.classify(envelope)` (One-Home tri-state classifier — той самий дім, що й `CeloConfirmationWorker`; витягнутий із колишнього приватного `classify_evm_receipt` заради DRY), який приймає або wrapped JSON-RPC відповідь (`{"id":…, "result": {"status":"0x1"}}`, реальний eth gem 0.5.x) або flat shape (`{"status":"0x1"}`, легасі-фікстура). Раніше код читав `receipt["status"]` напряму на envelope-формі, де статус живе під `result.status`, тому ВСЯ продакшн-телеметрія класифікувалась як `:reverted` → safe rollback розблоковував кошти навіть для confirmed on-chain mint'ів (точно та double-spend дірка, яку сервіс мав закривати). Тепер: `nil`/`{}` envelope → `:pending`; `result` null/empty → `:pending`; `status` ∈ {"0x1", "0x01", 1} → `:confirmed`; `status == nil` → `:pending`; інше → `:reverted`. **[E.49]** Per-chain cascade БЕЗ жодного зашитого фолбека: Polygon — `fallback_env_keys: ["POLYGON_RPC_URL_FALLBACK_1", "POLYGON_RPC_URL_FALLBACK_2"]` (keyless PublicNode/dRPC з 2026-09-02, [ARCH.114]); Celo — `Celo::CommunityRewardService::RPC_FALLBACK_ENV_KEYS`. Порожня primary-змінна = `KeyError` на money-шляху обабіч: Celo — ⚖️ 2026-08-31 (`DEFAULT_RPC_URL` знято, не перецілено); Polygon — ⚖️ 2026-09-03 (`polygon-rpc.com` знято: він був НЕДОСЯЖНИЙ за побудовою, бо з 09-02 гард вимагає presence `ALCHEMY_POLYGON_RPC_URL` на кожному класі процесу, що її читає, — тож і дзеркальне testnet-правило гарда стерегло подію, яка не могла статись; обидва пішли разом, ARCH.118). |
| **Нотатка rollback'у** | 💰 **`notes` називає РОЗБЛОКОВАНЕ, ніколи ЗАБОРГОВАНЕ, і при розбіжності друкує недобір** (2026-09-22). Гілок три, і збігаються вони лише в першій: холду вистачає → звільняється борг; холду менше → звільняється **весь наявний**; холду немає → **нічого**. Доти всі три друкували борг, тобто рядок money-path audit-trail стверджував величину, якої система не рухала ([`00_01 §1.1`](00_01_Vision_Mission_and_Roadmap) — «а звідки ти це знаєш?»). ⚠️ Бали цілі за конструкцією, а `locked_balance` приходить `BigDecimal` — `.to_i` стоїть у ЗВІТІ, не у звільненні, тож нотатка ніколи не завищує. Носій — `spec/services/minting_rollback_service_spec.rb` (дві гілки, що розходяться, пінуються окремо: сусідній приклад про частковий холд судив лише баланс, і саме тому фабрикація числа проходила зеленою). |
...
| **Що робить** | Чистий content-judge небезпечної Web3-конфігурації (дзеркалить `WeakKeyDetector`). **A1 (chain identity)** — сканує `*_RPC_URL` (Ethereum/Polygon/Celo/Solana) на testnet-маркери (`amoy`/`mumbai`/`sepolia`/`goerli`/`holesky`/`devnet`…, alnum-boundary-anchored проти хибних збігів у API-ключі). Chain-id константи в коді немає, а live `eth_chainId` на boot свідомо НЕ робиться (boot почав би залежати від доступності RPC = гірший failure-mode). 🔑 **Який бік «неправильний» — оголошує САМ СЛОТ через `WEB3_CHAIN_ENV` ∈ `mainnet`/`testnet` (відсутнє → `mainnet`), і це ДРУГА вісь поруч із `signer_process:`, а не виняток** [OPS.37 — розщеплення токена `production`]. ⛔ **Не bypass:** кожне значення є ТВЕРДЖЕННЯМ, якому мусить відповідати проводка, і напрямки дзеркальні — `mainnet` відмовляє testnet-ендпоінту (реальна вартість на throwaway-чейні), `testnet` відмовляє mainnet-ендпоінту (стейджинг, здатний підписати справжню транзакцію). Отже хибно ОГОЛОШЕНИЙ слот падає так само гучно, як хибно ПРОВЕДЕНИЙ, а забутий прапор лягає на суворий бік. Нерозпізнане значення = власне порушення (мовчазний відкат до дефолту зробив би слот суворим із причини, якої оператор не бачить). Blank-RPC скіпається — порожня змінна є `KeyError` на use, бо зашитих code-side фолбеків у дереві **нуль** з 2026-09-03 (Celo знято ⚖️ 08-31; Polygon `polygon-rpc.com` знято ⚖️ 09-03 як НЕДОСЯЖНИЙ за побудовою — presence `ALCHEMY_POLYGON_RPC_URL` з 09-02 вимагає A5 на job і web, тож дзеркальне testnet-правило пішло разом із літералом). Виняток — `armed_path_violations`: ОЗБРОЄНИЙ Celo-шлях (`ORACLE_CELO_PRIVATE_KEY` присутній) мусить нести і `CELO_RPC_URL` (на `mainnet`), і `CELO_CUSD_CONTRACT_ADDRESS` — правило E.49 на новій підставі переносить падіння з першої продової події на деплой; ключ живе лише на job-поверхні → web/coap чисті. **A2 (oracle keys)** — кожен підписант резолвить dedicated-ключ (`ENV.fetch` без fallback; легасі спільний `ORACLE_PRIVATE_KEY` retired — INF.22): відсутність MINTER/SLASHER → `KeyError` глибоко в Sidekiq-воркері → тихий DeadSet, тому presence-чек на boot; формат-чек (64 hex, опц. `0x`) покриває всю dedicated-сімку (MINTER/SLASHER/CELO/ETHERISC/PURO/KLIMA/ANCHOR); значення під retired-ім'ям = violation (zombie-config tripwire). ⊕ **[SEC.17 2026-09-03] KMS-вісь A2:** `ORACLE_MINTER_KMS_KEY_VERSION`/`ORACLE_SLASHER_KMS_KEY_VERSION` (`SIGNER_KMS_KEYS`; key-VERSION resource path, судиться ЛИШЕ в signer-процесі — клас, що не підписує, імені не читає) задовольняє presence ролі замість сирого ключа; сирий ключ ПОРУЧ із KMS-імʼям = зомбі (violation — один підписант, один дім); minter і slasher на ОДНІЙ версії = та сама колізія lock-ключа, що й однакові сирі ключі. **A3 (silent-address set)** — `SILENT_ADDRESS_ENVS` (`DAO_TREASURY_ADDRESS` + `CARBON/FOREST_COIN_CONTRACT_ADDRESS`): read-sites під rescue-парасольками для RPC-збоїв ковтають config-баг — mint-tax (E.46) тихо off і лог хибно «RPC degraded», `ChainAuditService` повертає хибне «delta 0, all clean» (db↔chain fraud-detector маскується), а `Insurance::ReserveGate` fail-closed, але маркує баг як transient `:eval_error` — тому presence + формат `0x`+40hex чекаються на boot; значення у violation-текст не echo-иться (могло бути mispasted-секретом). 🔴 **Presence тут скоуплена ПЕР-ЗМІННОЮ, а не одним `signer_process:` — виправлено 2026-09-01, і асиметрія була небезпечною в тихий бік.** Presence-гілка була скоуплена одним `signer_process:`, а формат не був скоуплений зовсім — тож на web-only слоті (canopy, без Sidekiq) ВІДСУТНЯ адреса бутила чисто, а плейсхолдер бут відмовляв: протилежні вердикти на тому самому споживачі, і тихий із них лишав `ChainAuditService` на вічному `delta: 0, critical: false`. ⊕ **2026-09-02 [INF.27 Q3] формат скоуплено ТИМ САМИМ по-змінним предикатом:** клас, що не читає змінної, не судить ані присутності, ані значення — виміряно на Kamal-ролі `coap`, яка успадковує глобальні плейсхолдери й падала на трьох `[address]`, з якими оператор не мав що робити; web відтоді судить лише SCC-адресу, job — усі три, coap — жодної. Критерій `web:` — «читає БУДЬ-ЯКИЙ процес класу» (контролер або rake-задача в web-контейнері), не «контролер-досяжність» [INF.27 Q2]. Вимір споживачів (`grep` по `app`+`lib`): `CARBON_COIN_CONTRACT_ADDRESS` — 6 сайтів, з них ОДИН контролер-досяжний (`chain_audit_service` ← `Api::V1::SystemAuditsController#index`) ⊥ `DAO_TREASURY_ADDRESS` 3 і `FOREST_COIN_CONTRACT_ADDRESS` 1 — усі job-only. Тож мапа несе `web:` на кожному члені, і вимога присутності = `signer_process ∨ (web_process ∧ web:)`. ⛔ Не зводити назад до беззастережної: `coap_listener` вантажить усі ініціалізатори, а його env-файл контракт-адрес не несе — ⛔ **спостереження про сід-heredoc `terraform/compute.tf`, НЕ канонізований інваріант; «за побудовою (§5.7)» тут стояло хибно** (та секція — secrets-at-rest latch), і гейта за ним немає — процес-класів ТРИ. **A5 (silent-RPC set)** [INF.27 Q1, 2026-09-02] — `SILENT_RPC_ENVS` (`ALCHEMY_POLYGON_RPC_URL`, `web: true`): сестра A3 без format-гілки. `ChainAuditService#fetch_chain_total_supply` кличе `RpcConnectionPool.client_for` (голий `ENV.fetch`) під `rescue StandardError`, тож відсутня змінна (`KeyError`), присутня-порожня (`Eth::Client.create("")` → `ArgumentError`) і мертвий хост дають ОДНЕ й те саме `delta: 0, critical: false` — доведено виконанням, не читанням; `chain_violations` при цьому свідомо скіпає порожній URL («raise at use» — підстава, яку цей сайт спростовує). Presence судиться там, де змінну читають (job: кожен mint/rollback-сайт; web: audit read-site), coap звільнений; мертвий хост поза скоупом за дизайном (availability, не config). **A4 (Solana signer set)** — `SOLANA_SIGNER_ENVS` (keypair · fee-payer pubkey · token-account · USDC-mint; E.61): stub-режиму нема, а batch-payout цикл ковтає per-wallet помилки БЕЗ escalation-шляху (акумульовані виплати форестерам мовчки не йдуть) → presence-чек у signer-процесі. |
| **Зовнішні виклики** | — (in-memory, без мережі за дизайном). |
| **Публічні методи** | `.violations(env = ENV, signer_process: true, web_process: true) → Array<String>` (порожній = безпечно) = сума чотирьох per-вісь методів (викликаються й окремо — цитати в 05_01/05_02/06_04 реферять їх за іменем): `chain_violations` `[chain]` (A1) · `oracle_violations` `[oracle-key]` (A2) · `address_violations` `[address]` (A3) · `rpc_violations` `[rpc]` (A5) · `solana_violations` `[solana]` (A4); + `.chain_env(env)` → нормалізоване оголошення слоту. 🔑 **Осей ДВІ, класів ТРИ — і плутати ці числа не можна, бо перше формулювання казало «осей ТРИ» й суперечило власному наступному реченню.** Дві осі: **КЛАС ПРОЦЕСУ** (job ⊥ web ⊥ coap) і **КОНФІГ** (`WEB3_CHAIN_ENV`). Клас процесу виражений ДВОМА kwargʼами, але вони не незалежні: ініціалізатор деривує обидва з однієї величини (`signer = Sidekiq.server?`, `coap = $PROGRAM_NAME…`, `web = !signer && !coap`), тож із чотирьох комбінацій живі рівно три, а дефолтна `(true, true)` не відповідає ЖОДНОМУ реальному процесу — вона є навмисною фікцією «найсуворіше», не станом. Отже kwargʼи: `signer_process:` скоупить presence-вимогу **ключів** до процесу-підписанта (Sidekiq), `web_process:` — presence-вимогу тих **адрес**, чий read-сайт контролер-досяжний (`web:` у `SILENT_ADDRESS_ENVS`; сьогодні це рівно `CARBON_COIN_CONTRACT_ADDRESS`). Обидві дефолтяться на СУВОРИЙ бік — тією самою підставою, що й `chain_env`→`mainnet`: забутий kwarg може лише зайво відмовити бут, ніколи не послабити його. Процес-класів через це ТРИ (job ⊥ web ⊥ coap), і третій — не дубль другого: `coap_listener` вантажить усі ініціалізатори, але не читає ані ключів, ані адрес. Web/coap-контейнери свідомо бутяться без money-ключів (least-privilege: web/coap — ширша, інтернет-виставлена поверхня, тож signing-пʼятірка живе лише в Kamal `job`-ролі; ⚠️ і глобальний `env.secret` успадковують УСІ ролі, тому `deploy_secret_scan` інваріант B судить ще й його); формат/колізія/address перевіряються скрізь, де значення Є. `WEB3_CHAIN_ENV` = властивість КОНФІГУ, тож читається з `env` — сигнатура незмінна, і вісь інжектиться тим самим хешем, що решта. Скоупить вона лише A1. |
| **Тест coverage** | `spec/services/security/web3_network_guard_spec.rb` — testnet-RPC (Amoy/devnet), alnum-boundary false-positive, умовний Celo-unset (armed/unarmed/mainnet), missing/malformed/`0x` oracle-key, silent-address set (missing DAO_TREASURY/SCC · placeholder · no-echo), Solana-presence, **process-scoping трьома класами** (coap: keyless-clean, нуль адрес і НУЛЬ формат-вироків на успадковані плейсхолдери, без `[rpc]` · web: вимагає САМЕ `CARBON_COIN_CONTRACT_ADDRESS` і судить формат ЛИШЕ її, вимагає Polygon-RPC (`[rpc]` на відсутній і на присутній-порожній), чистий щойно обидва є · job: усі три адреси presence+формат і `[rpc]` · обидва: колізія/testnet і без ключів; + пін «`web_process` дефолтиться на суворий бік»; стеля блоку записана з мутаційного прогону ×4, включно з мутантом, що перший раз сів не на той рядок); **приклади над ЖИВИМИ маніфестами** — `spec/deploy/web3_env_loudness_spec.rb` резолвить `role.env(host).clear` через `Kamal::Configuration` для ролей `coap`/`web` обох призначень (coap бутиться повз успадковані плейсхолдери · canopy оголошений `testnet` і не несе адрес-плейсхолдерів у web · production web відмовляє рівно на web-скоуплених плейсхолдерах), **`WEB3_CHAIN_ENV`-контекст** (testnet-слот на testnet-ендпоінтах чистий · mainnet-ендпоінт на testnet-слоті = violation · регресійний пін старого правила при невизначеній осі · нормалізація регістру/пробілів · нерозпізнане значення · інвертований Celo · зашитий polygon-mainnet-fallback armed/unarmed · address-вісь НЕ послаблена). Класифікаційний пін guard-sets — `spec/deploy/web3_env_loudness_spec.rb` (INF.12 behavior-half: кожен web3-ENV deploy-поверхні ∈ guard-set ∪ documented-LOUD ∪ documented-SOFT; нове ім'я = RED до свідомої класифікації) |
...
| **Що робить** | У `Rails.env.production?`/canopy АБО `WEB3_STRICT_MODE=true` (той самий gate, що IoTeX/Hadron) після `after_initialize` викликає `Security::Web3NetworkGuard.violations(ENV, signer_process: Sidekiq.server?, web_process: !Sidekiq.server? && !$PROGRAM_NAME.include?("coap_listener"))`; будь-яке порушення → raise `SecurityError` fail-closed ДО прийому трафіку. ⚠️ Другий kwarg тут — це рівно те, що ІНІЦІАЛІЗАТОР зобовʼязаний обчислити, бо гард процесу не бачить; форма `$PROGRAM_NAME` не вигадана, а позичена в `master_key_strength_check.rb`, який тим самим виразом звільняє coap від власної перевірки. 🔑 **Цей тригер — половина «загартований рантайм», і він свідомо НЕ отримав вісь середовища: стейджинг хоче його УВІМКНЕНИМ.** Питання «до якого чейну слот має право торкатись» — інша половина, оголошена в `WEB3_CHAIN_ENV` і присуджена всередині гарда (OPS.37). Читати `Rails.env` як відповідь на друге питання — та сама злитість, яку розщеплення й знімає. Повідомлення raise echo-їть **ПʼЯТЬ** координат (`slot` · `process` · `RAILS_ENV` · `WEB3_STRICT_MODE` · `WEB3_CHAIN_ENV`), бо `[chain]`-порушення без чейн-осі нечитабельне (вона є половиною твердження), а `[address] … is not set` нечитабельне без `process`: presence тепер скоуплена пер-змінною по трьох класах, тож наступний хід оператора залежить від того, ЯКИЙ контейнер відмовив. ⚠️ Число тут уже одного разу протухло тихо — рядок казав «ТРИ», коли код друкував чотири (`slot` додано [INF.27] і в цьому переліку не відбилось); тож звіряй його з самим `raise`, а не з памʼяттю. Presence-вимога **ключів** діє лише в signer-процесі (job) — відсутній ключ усе одно падає гучно на job-boot, ДО DeadSet; web/coap бутяться keyless by design. Presence **адрес** — інша вісь: див. картку гарда вище. У dev/test без strict-mode — вимкнений. Asset-build skip через `SECRET_KEY_BASE_DUMMY`. |
...
| **Вхід** | `wallet` (для `verify_wallet!`) або `naas_contract` (для `register_asset!`) |
| **Що робить** | **KYC**: пише `hadron_kyc_status` за відповіддю зовнішнього identity-провайдера. **RWA**: реєструє лісову ділянку як Real World Asset (ERC-3643). **[BLOCKER-04]** raise `ComplianceError` при відсутності ключа, якщо `WEB3_STRICT_MODE=true` **АБО** `Rails.env.production?` — belt-and-suspenders [INF.11]: у проді simulation вимкнена **незалежно від прапорця**, тож «зняти прапорець, щоб розблокувати» не працює за побудовою. Поза цим — simulation fallback для dev/test. ⚫ **[ARCH.118] Адресата не існує** (`api.hadron.polygon.technology` без DNS, перевимір 2026-09-04): цей сервіс — ЄДИНИЙ рантайм-писач `approved`, тож у проді статус лишається `pending` назавжди і mint-гейт скіпає бенефіціара мовчки → [`05_01 §6`](05_01_Multichain_Architecture) · [`00_07`](00_07_Action_Plan_Tracker) `BIZ.20` |
| **Активація** | ⚖️ **[ARCH.119, 2026-09-04] ДВІ ОСІ, один дім — класові предикати сервісу.** `configured?` = чи є РЕАЛЬНИЙ провайдер (ключ); `simulation_allowed?` = чи легальна заглушка (**НЕ** `production` і **НЕ** `WEB3_STRICT_MODE`); `verification_reachable?` = `configured? \|\| simulation_allowed?`, тобто чи виклик здатен ЗАВЕРШИТИСЬ хоч якось. **Гейт enqueue читає ТРЕТІЙ** — і це не переказ прецеденту IoTeX, а його поправка: там несконфігурована нога рейзить, тут поза `production` вона **фейково апрувить**, тож гейт на `configured?` перевернув би dev/test із «мінт працює» на «мінт тихо скіпає». Дві осі розводять це за побудовою: dev-заглушка лишається живою, а в проді без ключа джоба просто не ставиться в чергу замість того, щоб пройти `retry: 5` у Dead Set |
| **Зовнішні виклики** | `Web3::HttpClient.post` → `HADRON_API_URL/identity/kyc/verify` або `HADRON_API_URL/assets/rwa/register` |
| **Вихід** | `verify_wallet! → "approved"/"rejected"`. `register_asset! → asset_id (String)`. |
...
> ✅ **DOC-R.11 — Cron / partition guardian:** `PartitionMaintenanceWorker` запускається `30 0 * * *` UTC (cron у `config/sidekiq.yml`), створює партиції на попередній + поточний + наступний місяць для **3 RANGE-таблиць**: `telemetry_logs`, `gateway_telemetry_logs`, `blockchain_transactions`. Phase 7 додав `Sentry.capture_exception` у `rescue` блок. ⚠️ **Рід ціни тут доти був СПРОСТОВАНИЙ** (переміряно 2026-08-28, [`00_07`](00_07_Action_Plan_Tracker) ARCH.70): `no partition of relation` 1-го числа НЕ стається — усі три таблиці мають `_default`-лист, тож рядок тихо осідає туди, а незворотна ціна приходить наступним проходом (`PG::CheckViolation` назавжди). Якщо додаєте нову RANGE-таблицю — внесіть її в `PartitionMaintenanceWorker::PARTITIONED_TABLES` І оновіть `spec/workers/partition_maintenance_worker_spec.rb` (очікуване число OK-ліній = `tables × 3 months`). ⊕ **[ARCH.70] Той самий прохід семплить РІСТ** (`silkennet_partitions` · `silkennet_partitioned_table_bytes` · штамп свіжості) **і ПОЛОМКУ** (`silkennet_partition_default_occupied` — непорожній DEFAULT-лист незворотно блокує створення партиції свого місяця; рунбук [`06_06 §5.5`](06_06_Disaster_Recovery_and_Backup)) — дім приладу тут, бо `PARTITIONED_TABLES` уже живе в цьому воркері, а факт глобальний (одна БД), тож семпл іде в job-процесі, а не на скрейпі. 🔴 Вимірювання свідомо стоїть у ВЛАСНОМУ `rescue` і не піднімається у критичний: зовнішній інкрементить P0-лічильник збоїв і re-raise'ить, тож виняток ПРИЛАДУ інакше давав би пейдж «партиція може бути відсутня» + Sidekiq-ретрай уже виконаного DDL. Пороги й підстави — [`06_03`](06_03_Prometheus_Observability) + `deploy/grafana/alerts/silkennet-alerts.yaml`.
...
| **Side Effects** | Створює `GatewayTelemetryLog` (voltage — nullable до ADC-тракту, temp — до датчика DS18B20 по 1-Wire, HW.16). `mark_seen!` без voltage. `critical_fault?` → `health_message_key` дає пару «ключ + параметри», а **тип виводиться з ключа** (`ALERT_TYPE_BY_MESSAGE_KEY`, [SLASH-1]): канальні ключі → `comms_fault` (weak-CSQ · `coap_fail ≥ 10`), температурні → `hardware_fault` (❄️ T<−20°C · 🔥 T>65°C — **data-starved** до HW.16: v2-пульс температури не несе, «Королева без ADC» ARCH.54), **[FW.59]** `gateway_reset_fault` → `firmware_fault`. ⚠️ Ключ поза мапою лишається `system_fault` СВІДОМО — розкол кошика йде поштучно за атрибуцією. 🔴 **Порядок гілок `health_message_key` є присудом:** reset-гілка стоїть ПЕРШОЮ, бо `coap_fail_count` насичується за аптайм і не спадає, тож нижче за нього вона не спрацювала б жодного разу (пін — `gateway_telemetry_worker_spec`). Анти-спам guard по cluster-level алертах (`tree_id: nil`) — **дедуп ПО ТИПУ**: стоячий tree-scoped `system_fault` (fraud / power-loss / hardware-decay, авто-резолвера нема) НЕ глушить новий gateway-fault; залишковий конфлат з cluster-level писарями (`Actuator`, slashing-failure) → типова декомпозиція кошика [`00_07`](00_07_Action_Plan_Tracker) SLASH-1. → (via `after_create_commit` Transactional Outbox) → `AlertNotificationWorker.perform_async`. |
...
> 🔴 **У методі ДВІ гілки з РІЗНИМИ адресатами, і зливати їх дорого** [ARCH.60]. **(А) Формальна звітність:** `severity_critical?` + непорожній `organizations.billing_email` → `AlertMailer`, і роль тут **не фільтрується взагалі**. **(Б) Оперативні канали:** `organization.users.where(role: [:admin, :forester])` — а дефолтна роль `User` це `subscriber` ([`04_01 §5`](04_01_Data_Models_and_Entities)), тож у гілку Б не потрапляє ані B2C-власник, ані `super_admin`, і це ЗА ПОБУДОВОЮ. Розрізнення несуче, бо саме воно є підставою двох зрізів: канал гілки Б збирали під аудиторію, якої її запит не вибирає — звідси ⚫ Telegram 2026-09-06 і припинений збір `push_token`. ⚠️ Юр-бік: публічна політика не сміє обіцяти сповіщення тому, кого гілка Б не обирає (звуження в `b2c_tos_privacy`). **Рефлекс при додаванні каналу: питай не «чи він доставить», а «ЯКА з двох гілок і кого САМЕ вона вибирає» — транспорт, гілка й аудиторія тут ТРИ різні осі, і мовчить третя.**
>
...
> - **Батчинг:** `AlertNotificationWorker` має групувати recipients по каналу та відправляти батчами по 500 (FCM), а не делегувати кожне повідомлення окремому воркеру. ⚫ Другого каналу в цій вимозі більше немає — Telegram зрізано ⚖️ 2026-09-06 ([`ARCH.60`](00_07_Action_Plan_Tracker)), і разом із ним зникла єдина ланка БЕЗ bulk-ендпоінта, чию межу ~30 msg/s мусила брати на себе черга (E.33). Тобто вимога спростилась: лишився один канал, і він batch-aware за побудовою.
>
> 🔴 **[E.33, 2026-08-21] Перший крок зроблено, і він виявився НЕ лімітером: найдешевше обмеження — не ставити в чергу канал, якого немає** (механіка — картка воркера вище). 🔴 **Арифметика цієї половини перевернулась ⚖️ 2026-09-06, і це не косметика.** Доти `OPERATIONAL_CHANNELS` мав два імені — `:push` мертвий, `:telegram` живий, — тож фільтр справді ділив чергу НАДВОЄ. Після зняття Telegram [`ARCH.60`](00_07_Action_Plan_Tracker) лишився сам `:push`, ратифіковано недоступний [`ARCH.108`](00_07_Action_Plan_Tracker), тож набір оголошено порожній і фільтр не ділить чергу, а зводить її оперативну половину ДО НУЛЯ. Ні тоді, ні тепер він нічого не вгадує про масштаб — але число, яким його описували, встигло стати хибним раніше за сам механізм.
>
> ⚠️ **Решта гейтована МАСШТАБОМ, а не «відсутністю адаптерів» — і саме тому не будується наперед.** ⚫ Опис адресної вибірки через `users.telegram_chat_id` тут стояв доти, доки канал жив; зрізано ⚖️ 2026-09-06 ([`ARCH.60`](00_07_Action_Plan_Tracker)) разом із колонкою, тож при поточному флоті межі не має жоден живий канал. Три виміри, які знадобляться в день, коли вона стане досяжною: (1) `Sidekiq::Limiter` **шим уже має все, крім тіла** — API `window(name, limit, period)`, живий споживач (`WEB3_RPC_LIMITER`, 50/s) і `rescue OverLimit` у `ApplicationWeb3Worker`, — тож форма лімітера тут не проєктується з нуля, вона **чекає присуду про Sidekiq Ent** ([`00_07`](00_07_Action_Plan_Tracker) ARCH.59, крок 2 «шим → raise замість тихого no-op»); ⚠️ `wait:` шим приймає й мовчки відкидає. (2) `expires_in` у `sidekiq_options` обох notification-воркерів — **теж Pro-опція без гема**, тобто оголошений TTL не діє: застаріла тривога не протухає, вона доїжджає. (3) `Rails.cache` тут НЕ підходить під лічильник вікна — прод це Solid Cache, чий `increment` **не атомарний** (доказ уже стоїть у шапці `rack_attack.rb`, і саме тому Rack::Attack ходить у Redis, а не в Rails.cache).
>
> 🔬 **⚫ Вимір про 429 переживає власний предмет — і саме тому лишається тут, а не зникає разом із каналом.** Telegram їхав через `Web3::HttpClient` із `service_name: "Telegram"` і тим ділив circuit breaker із Web3-трактом: 3 поспіль не-2xx → ключ відкривається на 60 с. Донор форми — RPC із **каскадом провайдерів**; у Bot API endpoint був один, тож 429 перетворювався на повну відмову каналу на хвилину. 🔑 **Клас, а не інстанс: спільний breaker припускає каскад, і будь-який майбутній одноендпоінтний споживач (FCM у першу чергу) успадкує цю ж пастку, не питаючи.** Тому запис лишається як застереження до наступного дротування, а не як опис живого шляху. Канал зрізано ⚖️ 2026-09-06 ([`ARCH.60`](00_07_Action_Plan_Tracker)); борг «`parameters.retry_after` не читається ніде» пішов разом із ним.
...
| **Вхід** | `user_id`, `ews_alert_id`, `channel` (`:push` — єдиний, відколи Telegram зрізано ⚖️ 2026-09-06) |
| **Сервіси** | ⚫ **Жодного: не-поштових транспортів у дереві не лишилось.** FCM не задротований ([`ARCH.108`](00_07_Action_Plan_Tracker)); SMS відкинуто ⚖️ [`ARCH.78`](00_07_Action_Plan_Tracker) 2026-08-20 разом із `users.phone_number`; Telegram зрізано ⚖️ [`ARCH.60`](00_07_Action_Plan_Tracker) 2026-09-06 разом із `users.telegram_chat_id`. Обидва застарілі джоби (`"sms"`, `"telegram"`) гучно падають в unknown-гілку — саме її наявність робить це станом, а не дірою |
| **Side Effects** | ⚠️ **[E.33, 2026-08-21] Канал без транспорту сюди більше не доїжджає** — його відсіює `AlertNotificationWorker` на вході в чергу (картка вище). Гілки нижче лишаються **backstop-ом, а не основним шляхом**: джоба могла лягти в чергу за живого каналу й виконатись уже після зняття токена, тож їхня чесність і далі несуча — просто зустріти їх тепер можна лише у вузькому вікні. **[ARCH.78]** Доки транспорту немає, Push-гілка пише `Rails.logger.warn` про **СТАН КАНАЛУ** («Канал не сконфігуровано — … НЕ доставлено»), а не про результат. Попередня редакція логувала «Надіслано»/«Доставлено» біля закоментованого клієнта — журнал, що стверджує дію, якої не сталося, на тракті пожежної та вандалізм-сирени; під час розбору інциденту він читався б як доказ доставки. `case channel` має гучний `else` (невідомий канал → `logger.error`): тиша тут невідрізненна від доставки. Носій класу — `spec/quality/no_self_attesting_logs_spec.rb` (пара «закоментований транспорт + лог із дієсловом-результатом» у `app/**`; форма-агрегатор і статус-у-БД поза його стелею — вона задекларована в шапці). ⚫ **Telegram-гілку знято 2026-09-06 разом із `send_telegram`** [`ARCH.60`](00_07_Action_Plan_Tracker). ⊕ Що з неї ПЕРЕЖИЛО зняття (урок форми, не каналу): текст рендерився в **локалі отримувача** через `Notifications::RecipientLocale`, бо в Sidekiq `I18n.locale` завжди базова — і цей дім лишається живим, просто його єдиними читачами стали мейлери; а помилка HTTP свідомо НЕ ковталась (`RequestError` = Sidekiq-retry тієї самої атомарної доставки), і саме цю форму успадкує наступний транспорт. 🔴 **Гард на вході СЕМАНТИЧНИЙ, не часовий** [ARCH.59, 2026-08-21]: воркер виходить на `alert.status_active?` — `EwsAlert` при резолві не видаляється, тож затримана джоба інакше розсилала б **ВИРІШЕНУ** тривогу. Гард стоїть в ОБОХ воркерах свідомо, і це не дубль: батько судить раз на фан-аут, а дитина лежить у черзі вдвічі довше (`expires_in` 5 хв ⊥ 10 хв). ⚠️ **`expires_in` цієї осі НЕ закриває** — на Sidekiq OSS опція інертна, тож доти вісь була покрита НУЛЕМ, а не слабко. |
...
| **API** | `available?(channel)` → Boolean · `available` → Array\<Symbol\> · `configured_sender(env)` → String · `ALL` = `%i[email push]` |
...
| **Push** | **Оголошений** мертвим: адаптера в дереві немає, і конфіг-поверхні теж — імена ENV назве `ARCH.60`. ⚖️ **Присуд ухвалено 2026-08-21: push НЕ потрібен до появи мобільного клієнта** — тоді сценарій покривали email (BOOT-CRITICAL) і Telegram. 🔴 **Половина тієї підстави відпала 2026-09-06** ([`ARCH.60`](00_07_Action_Plan_Tracker)): Telegram зрізано, і покриття тримає сама пошта. Присуд від цього не перевертається — він і був про ВІДСУТНІСТЬ КЛІЄНТА, а не про надлишок каналів, — але ціна його стала вищою, і це записано тут, щоб наступний читач не виводив «два канали покривають» із рядка, де лишився один; канал і далі оголошено-мертвий за рішенням, не через недоробку; тригер адаптера тепер сама ПОДІЯ (поява клієнта), а не чийсь присуд, і форму клієнта вирішує [`00_07`](00_07_Action_Plan_Tracker) **ARCH.108** (виділено з `E.20` 2026-08-24; `E.20` тепер про фізичного виконавця, не про клієнта — реф резолвився й вів у ЧУЖИЙ предмет). SMS у `ALL` більше не існує — канал відкинуто ⚖️ [`ARCH.78`](00_07_Action_Plan_Tracker) 2026-08-20 |
...
#### `Notifications::TelegramTransport` — ⛔ ВИДАЛЕНО 2026-09-06 [ARCH.60]

⛔ Модуль, його спека, гілка `:telegram` у `DeliveryChannels`/`SingleNotificationWorker`, `OPERATIONAL_CHANNELS`, колонка `users.telegram_chat_id` і `TELEGRAM_BOT_TOKEN` з усього деплой-ланцюга знято ОДНИМ проходом (⚖️ founder — «зрізати, якщо з ним більше проблем, ніж він помагає»). Підстава — не дефект каналу, а його ЦІНА проти нуля споживачів: колонка персональних даних, супровід у трьох юр-документах, вендор без processor-DPA і відкрите питання «процесор чи контролер щодо чат-користувача». 🔴 **Стан цього зняття ТРЕТІЙ, і плутати його з сусідами дорого:** SMS ([`ARCH.78`](00_07_Action_Plan_Tracker)) відкинули НЕ збудувавши; Hadron ([`ARCH.118`](00_07_Action_Plan_Tracker)) не існував ніколи; Telegram збудували, увімкнули й зняли — живий зонд на canopy в день зрізу дав `configured?` = `true` при нулі непорожніх `chat_id`. Тобто канал працював і не мав до кого говорити.

⊕ **Що з цього ПЕРЕЖИЛО зняття** (уроки форми, не тракту):
- **Предикат жив У ТРАНСПОРТІ**, поруч з ENV-імʼям і форматом токена — «чи працює канал» і «чим він вмикається» не були двома домами. Саме тому зняття транспорту зняло канал ЦІЛКОМ, не лишивши осиротілого предиката в `DeliveryChannels`. Форму варто повторити для FCM.
- **Форматна перевірка токена** (`\d+:[\w-]{30,}`) читала деплой-плейсхолдер `REQUIRED_SECRET_NOT_SET` як «вимкнено», а не як живий канал — та сама нога, що `sender_configured?` і `Security::Web3NetworkGuard`. Клас живий, носіїв два.
- **Спільний `Web3::HttpClient` circuit breaker припускає КАСКАД провайдерів.** У Bot API endpoint був один, тож 429 перетворювався на повну відмову каналу на хвилину. Наступний одноендпоінтний споживач (FCM першим) успадкує цю пастку, не питаючи — і це записано тут саме тому, що предмет зник, а клас лишився.
...
| **Черга** | `critical` |
| **Retry** | 10 |
| **Тригер** | **[INS.1 dual-trigger]** Daily-оракул (`InsuranceOracleWorker`) лише ОЗБРОЮЄ кандидата (`:triggered`), payout НЕ enqueue. Settlement (Trigger-2) — `Dclimate::VerificationService` (fire_confirmed → знаходить `:triggered`) + recovery sweep `InsurancePayoutRecoveryWorker` (cron `15,45 * * * *`). 🔴 **⛔ Автоматичного settlement сьогодні НЕ БУВАЄ:** постачальника зрізано ⚖️ 2026-09-05, служба стоїть за `configured?`-гейтом і тихо виходить, тож `fire_confirmed` не приходить ЖОДНОГО разу — рухає виплату лише людський Field-Audit (імʼя класу — названий борг, [`00_07`](00_07_Action_Plan_Tracker) S3.2) |
...
| **Що робить** | Викликає `InsightGeneratorService#process_cluster_batch(cluster_ids)` — обробляє чанк кластерів: per-cluster delete+insert `AiInsight`, fraud detection, евристичний `stress_index` (⛔ ML-шар знято 2026-09-05 — картка `InsightGeneratorService` вище), денормалізація `latest_stress_index`. |
...
| **Side Effects** | `CREATE TABLE IF NOT EXISTS ... PARTITION OF ...` для таблиць `telemetry_logs`, `gateway_telemetry_logs`, `blockchain_transactions`. Перевіряє та створює партиції для попереднього, поточного та наступного місяця (формат: `{table}_y{YYYY}m{MM}`). DDL-операція ідемпотентна — повторний запуск безпечний. Phase 7 додав `Sentry.capture_exception` у `rescue` блок. ⚠️ Не заради `no partition of relation` — той не стається (`_default`-лист ловить рядок); справжня ціна — `PG::CheckViolation` на наступному проході, незворотно ([`00_07`](00_07_Action_Plan_Tracker) ARCH.70). |
...
| **Вхід** | `tx_hash` (String) |
| **Сервіси** | — |
| **Side Effects** | `eth_get_transaction_receipt` (Polygon RPC). 🔑 **Спершу `settled, actionable = txs.partition(&:status_confirmed?)` [2026-09-07]** — скоуп виключає лише `:manual_review` [ARCH.115], тож повторний прогін бачить і термінальні рядки, а `confirm` (на відміну від `fail`) з `:confirmed` не переходить; дім розбору — [`04_01`](04_01_Data_Models_and_Entities) картка `BlockchainTransaction`. При `0x1`: `actionable.each(&:confirm!)` у транзакції (вже-`confirmed` тихо пропускаються — це повторний прогін). При revert: `actionable.each { fail! }`, а `settled.any?` → **гучний лог розбіжності з переліком id і НУЛЬ автоматичних дій** (рядок `confirmed` при revert-квитанції = ланцюг ⊥ наш стан, не повтор). **[M2]** `fail!` для mint-tx звільняє `locked_balance` через AASM `fail`-hook — дискримінатор `locked_points`). Retry при pending (ще в мемпулі). **[MEMPOOL LIMBO GUARD]** `sidekiq_retries_exhausted` handler: при вичерпанні всіх 10 ретраїв (~15-20 хвилин поллінгу) делегує до `MintingRollbackService.call(transactions: BlockchainTransaction.where(tx_hash:, status: :sent))`. Запобігає зависанню транзакцій у статусі `:sent` з замороженим `locked_balance` після потрапляння job у Sidekiq Dead queue. |
...
| **Вхід** | — |
| **Side Effects** | **[ARCH.55]** Re-arm `BlockchainConfirmationWorker` для tx, що застрягли в `:sent` довше 15 хв (`sent_at < 15.minutes.ago`) — клас, який ConfirmationWorker-`retries_exhausted` не ловить: OOM/евікшн ПІД ЧАС поллінгу (pending-discovery дивиться лише pending/processing; `MintingRollbackService` — тільки з `retries_exhausted`). **Скоуп = `blockchain_network: "evm"` only** — ConfirmationWorker Polygon-специфічний; Solana/Celo `:sent` без фільтра летіли б у чужий поллер (15-20 хв змарнованого RPC + хибний `manual_review`; Solana/Celo мають власні reconcile-шляхи). Ключ на `sent_at` (broadcast-момент), НЕ `created_at` (момент НАМІРУ: рядок буває довго `:pending` до відправки — KYC-skip, circuit-HOLD, накопичення колектора). Дедуп по tx_hash з earliest created_at (partition-prune); safety-cap `BATCH_LIMIT` re-arm'ів за прохід. Покриває mint/burn/insurance (спільний ConfirmationWorker). 🔴 **Ідемпотентність дубля з живим поллером НЕСЕ СПОЖИВАЧ, а не подія — виправлено 2026-09-07 після інциденту canopy, і стара редакція тут була хибною в НЕСУЧІЙ половині.** Стояло: «= AASM `confirm` (одноразовий; дубль → `InvalidTransition` → retry → **no-op**)», тобто канон НАЗВАВ саме той виняток, який ми потім побачили, і класифікував його як нешкідливий — через що інцидент був невидимий як дефект для кожного, хто читав цю картку. Ціна обох половин: «одноразовий/no-op» справдився лише в БД і лише на ОДНОРІДНОМУ батчі (441 подія Sentry + 47 джоб у DeadSet на ОДНОМУ `tx_hash` — тобто в черзі це драбина, не no-op); а «безпечно» було **хибним**, бо `confirm!` викликається всередині `ActiveRecord::Base.transaction` по БАТЧУ, і на батчі зі ЗМІШАНИМИ станами виняток відкочував підтвердження СУСІДІВ — такий батч не сходився ніколи. Тепер `BlockchainConfirmationWorker` відсіює `status_confirmed?` рядки `partition`-ом ДО транзакції, тож винятку не виникає взагалі. ⛔ Не повертати міркування через exception-шлях: подія лишається одноразовою **навмисно** — її `before`-хук штампує `confirmed_at`, а по цій колонці `TreeChronicleService` сортує й датує хроніку дерева, тож самолуп переставляв би дату on-chain події на час retry ([`00_01 §1.1`](00_01_Vision_Mission_and_Roadmap) — відтворюваність). **[ARCH.45 :processing-orphan, 2026-07-05]** Другий прохід `escalate_stuck_processing!`: stale `:processing` (updated_at > 15 хв — `created_at` є моментом НАМІРУ, а рядок буває довго `:pending`, ARCH.52) → `escalate_to_review!` (`:manual_review`): крах між `transact("mint")` і `mark_as_sent` = ambiguous (мінт міг landed, tx_hash невідомий) → політика ARCH.48/M6, НІКОЛИ blind re-mint; double-mint і так неможливий (mint-шляхи фільтрують `:pending`), закрито observability/locked-balance хвіст. |
...
| **Сервіси** | `Polygon::HadronComplianceService` → `verify_wallet!` (wallet із власною адресою) / `verify_organization!` (custodial-бенефіціар) |
| **Side Effects** | Пише `hadron_kyc_status` (approved/rejected) на суб'єкті; custodial-wallet без власної адреси — skip (успадковує org-статус через `Wallet#kyc_approved_for_minting?`). Dev/no-key = simulate-approve; prod = raise (адресата не існує — [ARCH.118], [`05_01 §6`](05_01_Multichain_Architecture)), тобто «реальний API» тут описує НАМІР, не поверхню. Канон-гейт: [`05_02` — Крок E](05_02_Proof_of_Growth_Pipeline). |
...
| **Вхід** | — |
| **Side Effects** | **[ARCH.65]** Re-enqueue `HadronKycVerificationWorker` для Wallet/Organization з `hadron_kyc_status="pending"` (з власною адресою, `updated_at > 1год`). Клас: `HadronKycVerificationWorker` (retry:5) НЕ має `sidekiq_retries_exhausted` + enqueue лише разово `after_commit` → Hadron API down усі 5 retry → Dead Set → KYC `"pending"` НАЗАВЖДИ → mint-gate `Wallet#kyc_approved_for_minting?` щоцикл тихо скіпає pending-tx бенефіціара. Idempotent auto-heal (повторний verify безпечний; скоуп лише `"pending"`, approved/rejected не чіпаємо); `BATCH_LIMIT=500` oldest-first проти thundering-herd на mass-recovery. Backlog-видимість — `silkennet_hadron_kyc_pending_depth` gauge. ⛔ **Преміса «Hadron API **down**» СПРОСТОВАНА [ARCH.118 · ARCH.119, 2026-09-04]: вимір дав НЕІСНУВАННЯ вендора, а не простій.** Механіка вище описана точно, але її СЕНС інший: єдиний рантайм-писач `approved` адресата не має, тож «auto-heal» переозброює драбину, якій нема куди дійти, а «тихо скіпає» описує не крайовий випадок, а ПОСТІЙНИЙ стан кожного custodial-бенефіціара. Лік розчеплено 2026-09-04 (⚖️ founder): ✅ **правду закрито відразу** — сім сайтів перестали стверджувати «вендор оживе», а `sn-alert-hadron-kyc-backlog` піднято `info → warning` (бенефіціар випадає щоцикл і сам не повернеться); ⚖️ **ПРИСУД 2026-09-04 — гейтувати ЗАРАЗ, і форма важливіша за строк** [ARCH.119]: гейт стоїть на `verification_reachable?` (не на `configured?` — дві осі, картка сервісу вище) і накриває РЕ-АРМ, **не прохід**. 🔴 `sample_pending_depth!` лишається безумовним, і це не деталь: він ЄДИНИЙ писач `silkennet_hadron_kyc_pending_depth`, а алерт `sn-alert-hadron-kyc-backlog` має `noDataState: OK` — гейт на всьому `perform` замовкнув би рівно тоді, коли беклог росте. Обидві половини мутаційно запінені окремими прикладами (наївна форма валить другий). Перейменування `hadron_*` лишається гейтованим вибором провайдера ([`BIZ.20`](00_07_Action_Plan_Tracker)) і з цією правкою не перетинається — ідентифікатори ⊥ потік керування. |
...
| **Polygon Hadron** | HTTPS REST | `hadron_api_key` / `HADRON_API_URL` (default `https://api.hadron.polygon.technology` — ⚫ **[ARCH.118, перевимір 2026-09-04] хоста НЕ ІСНУЄ**: ні `A`, ні `CNAME` в авторитетній зоні при живому `polygon.technology`, Wayback без жодного знімка за всю історію, а compliance-доки Polygon Labs слова «Hadron» не знають. Тобто впав не хост, а **вендор**; сервіс лишається fail-closed заявкою без адресата, KYC-провайдера не обрано — [`00_07`](00_07_Action_Plan_Tracker) `BIZ.20`. ⚠️ Цей рядок пережив свій свіп, стоячи МІЖ двома вже поміченими — сусідство з виправленим читається як виправленість) | Polygon::HadronComplianceService |
...
**Механічна частина — ✅ enforced by `scripts/model_doc_sync.rb`** (CI `docs.yml`): кожен `app/services/**` / `app/workers/**` клас згаданий у цьому реєстрі — зловив би сервіс, повністю відсутній у доку (як `FactoryFlashing::*`-пропуск, що цей гейт і закрив). Семантику (сигнатури/ENV/guard-clauses/queue-розміщення) скрипт НЕ перевіряє — ручний семантичний аудит ([`00_06 §3.0`](00_06_SSOT_Documentation_Standard)).
...
> **«Planned» — сьогодні в цьому кошику лишився ОДИН: Cross-Registry** ([`ARCH.5`](00_07_Action_Plan_Tracker)) — design-RFC, у коді **відсутній** (Post-TRL 6/7, нормально — не code-drift, тому не у `model_doc_sync`). 🔴 **Двох інших тут більше немає, і це не переліком, а присудами:** Forester Guild — ⚫ won't-do ⚖️ 2026-08-24 ([`E.20`](00_07_Action_Plan_Tracker)), Federated Learning — предмет зрізано У-ВЕЙ 2026-09-05 ([`E.52`](00_07_Action_Plan_Tracker); федеративна нога живе в `E.31`/`ARCH.6`). ⛔ Не читати їхню відсутність у коді як «нормальну для планового» — для знятого предмета цей рядок гасив би червоне назавжди.
...
> ✂️ **Механіку зрізано 2026-09-04 (⚖️ founder, DOC-T.98) — і це СКАСУВАННЯ попереднього рішення «лишається як design-RFC», не його виконання.** Знято ≈18 kB: постановка проблеми, архітектура гільдії, Task Assignment Algorithm (шість етапів, race-resolution, crash-recovery, anti-Sybil, dClimate-фолбек) і економічний шар Фази 2 (operator-bond · guild-sponsor). **Підстава зрізу — та сама, що й підстава won't-do:** Мета цього документа є «карта ПОТОЧНИХ сервісів для запобігання дублювання логіки», а специфікація неіснуючого є backlog'ом, не каноном ([`00_06 §2`](00_06_SSOT_Documentation_Standard)). Числа в ній були `calibration-pending` placeholder'ами й нічим іншим. **Що НЕ зрізано і чому: підстава відмови вище** — вона стереже майбутню роботу, а не описує минулу, і [`06_08`](06_08_Resilience_and_Failover_Policy) цитує саме її. Розібрана механіка лишається в git.

> 🏠 **Куди пішли предмети, що жили тут:** живий positive-A-guard (Фаза 1, ✅ landed) має дім [`05_05 §3.2`](05_05_Slashing_and_Risk_Policy) — там і політика, і чокпоінт `BlockchainBurningService`; відкрита робота фізичного виконавця — [`00_07`](00_07_Action_Plan_Tracker) `E.20`; cross-registry export — [`00_07`](00_07_Action_Plan_Tracker) `ARCH.5` (⚠️ і він НЕ greenfield); кластерне edge-перенавчання — [`00_07`](00_07_Action_Plan_Tracker) `ARCH.6` (⛔ канон-назва саме така; дизайн уже в каноні — delegation-таблиця L1/L2/L3 [`03_03 §11.4`](03_03_TinyML_Acoustic_Inference) + [`03_04 §6.3`](03_04_mruby_Lorenz_Attractor), і «Federated Learning» їй не синонім).
