...
| **Зв'язок з HKDF tree** | Captured-критично: master-ключ є коренем для **п'яти** info-strings (post-FW.2 (в)): `HardwareKeyService.derive_lora_key` (Tree session AES-128, info `"silken-aes-128-lora-key"`), `derive_broadcast_key` (cluster KEYB AES-128, info `"silken-aes-128-broadcast-key"`), `derive_device_key` (Gateway AES-256 CoAP, info `"silken-aes-256-device-key"`), `OtaHmacKeyService` (K_ota, info `silken-ota-hmac-v1`), `SilkenNet::SeedDerivation` (Lorenz `K_seed`, info `silken-lorenz-seed`). Компрометація master = каскадна компрометація всіх п'яти — тому guard працює fail-closed до запуску HTTP-сервера. |
...
| **Вхід** | — (no args) |
| **Що робить** | **[NEW]** Централізований моніторинг балансів Oracle-гаманців на всіх 4 мережах. **[E.51]** Пороги читаються через `SystemParameter.current` (param_key `oracle_min_balance_{matic,sol,celo,eth}`, 24h-кеш — без DB-хіту на цикл) і governance-aware (оновлювані через `ProtocolParameters.sol` → `Governance::ParameterSyncWorker`); значення в коді = DEFAULT-fallback, не зафіксовані константи: Polygon (MATIC, default 0.05), Solana (SOL, 0.05 = 50M lamports), Celo (CELO, 0.05), Ethereum (ETH, 0.01 — weekly anchoring). Для EVM-мереж: `Eth::Key` → `client.get_balance`. Для Solana: `Web3::HttpClient.post` → `getBalance` RPC. `RPC_TIMEOUT = 10с` на кожну мережу. Результат: масив Hash з `{ network, currency, balance_raw, balance_human, ratio, status (:healthy/:critical/:error) }`. |
...
| **Side Effects** | Створює `GatewayTelemetryLog` (voltage/temp — nullable до ADC-тракту). `mark_seen!` без voltage. `critical_fault?` (weak-CSQ / `coap_fail ≥ 10`) → `EwsAlert(system_fault)` з анти-спам guard'ом по кластеру → (via `after_create_commit` Transactional Outbox) → `AlertNotificationWorker.perform_async`. |
