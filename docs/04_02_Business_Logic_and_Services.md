...
> 🔴 **«Stakeholders» тут — це РІВНО `role: [:admin, :forester]`, а дефолтна роль `User` — `subscriber` ([`04_01 §5`](04_01_Data_Models_and_Entities)), тож B2C-власник дерева не дістає сповіщення ЗА ПОБУДОВОЮ** [ARCH.60]. Це не недогляд вибірки, а несуча підстава двох зрізів: канал, зібраний під аудиторію, якої запит адресатів не вибирає, різали саме тому — ⚫ Telegram 2026-09-06 і припинений збір `push_token`. ⚠️ Юр-бік того самого: публічна політика не сміє обіцяти сповіщення тому, кого цей запит не обирає (звуження в `b2c_tos_privacy`). **Рефлекс при додаванні каналу: питай не «чи він доставить», а «кого взагалі вибирає запит адресатів» — транспорт і АУДИТОРІЯ тут різні осі, і друга мовчить.**
>
