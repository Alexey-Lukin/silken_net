...
| **Вхід** | `Tree` + супутні `EwsAlert`-и (сигнали, з яких деривується категорія причини) |
| **Що робить** | Класифікує причину деградації за Кат-A/B/C і відповідає на ОДНЕ питання, від якого залежить необоротність: **`positive_a?` — чи є ПРЯМИЙ доказ категорії A (tamper)**. Політика й повне визначення категорій — дім [`05_05 §3.2`](05_05_Slashing_and_Risk_Policy) |
