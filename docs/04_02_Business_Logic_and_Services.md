...
> 6. **[OSS — ліцензії НЕ потребує]** Збільшити Redis pool: `pool_size = concurrency + 5` (зараз буфер = 0). 🔴 **Єдиний крок переліку, що не чекає НІЧОГО:** `SIDEKIQ_REDIS_POOL_SIZE` (`config/initializers/sidekiq.rb`) дорівнює `:concurrency` з `config/sidekiq.yml`, а наш явний `size:` перекриває той розмір, який Sidekiq вивів би з капсули сам — тобто стеля тут наша власна й знімається однією змінною.
>
> 🔴 **Перелік вище змішує ДВІ осі, і мовчання про це вже коштувало помилки на сусідній поверхні:** ліцензію потребують кроки **1 · 2 · 4 · 5** (без гема вони no-op або деструктивні — крок 2 сьогодні валив би boot, бо шим є єдиним джерелом `Sidekiq::Batch`/`Limiter`, які вантажаться на класовому рівні), тоді як **3 і 6** — чиста OSS-робота. Отже «активація Pro» не є передумовою для двох кроків із шести, і читати перелік як однорідний блокер-пакет — хибно.
