...
| **Що робить** | Визначає протокол фізичної відповіді за типом загрози: `severe_drought` → відкрити water_valve (2г), `fire_detected` → water_valve (4г) + fire_siren (1г), `insect_epidemic` → water_valve (1г), `seismic_anomaly` → seismic_beacon (30хв). Пріоритизує актуатори за відстанню шлюза до дерева — інлайн Arel-сорт `POWER(lat−?,2)+POWER(lon−?,2)` у самому сервісі, **НЕ** `SilkenNet::GeoUtils` (той тут не викликається; дрейф виправлено 2026-07-27). Масове `insert_all` для ActuatorCommand. |
...
> ⚠️ **Відомий геп диспетчеризації [ARCH.75] — happy-path вище описує НАМІР, не поведінку.** Три вади, усі від зустрічі push-ерівського дизайну з poll-семантикою FW.60: (1) команди несуть TTL 15 хв, а Королева питає `poll/<uid>` лише після флашу (кеш 45/50 АБО година+джиттер, причому таймерна нога сама гейтована `cache_count > 0 \|\| ed25519_ready` — мовчазна legacy-Королева не флашить ніколи) — і народжуються вони ПІСЛЯ poll'а того флашу, що приніс докази, тож у рідкому кластері вся аварійна відповідь гине протермінованою, не виконавшись; (2) сирена стає в чергу ЗА чанками поливу (обидва `high`, сорт за `created_at`) — інверсія власного інваріанта моделі «сирена має витіснити полив»; (3) `duration_chunks` ріже 4 год на чотири накази заради валідації ≤3600 с, але вони не серіалізуються — видаються підряд і накладаються; (4) **найважча** — ріже за ВЛАСНОЮ константою `MAX_COMMAND_DURATION`, ніколи не питаючи `actuator.max_active_duration_s`, а `insert_all` валідації обходить, тож на актуаторі зі стелею < 3600 с (сіди: клапан 300, сирена 120) кожна пожежна команда лягає **невалідною** й не вміє навіть померти по TTL. Транспорт від цього вже захищено (force-fail у [`Downlink::PendingQueueService`](04_02_Business_Logic_and_Services) §8) — але фізична відповідь на такій конфігурації не спрацьовує взагалі. Стан і розвилки → [`00_07`](00_07_Action_Plan_Tracker) ARCH.75.
...
| **Вхід** | — (class method) |
| **Що робить** | Отримує поточну ціну SCC/USDC з Uniswap V3 Quoter (Polygon). Кеш 5 хвилин. Timeout 15 сек. Fallback: governance-aware `SystemParameter.current(:scc_fallback_price_usd, default: 25.5)` [S6.9] — $25.50 = DB-дефолт (Series A base), НЕ хардкод (оновлюється через `ProtocolParameters.sol` → `Governance::ParameterSyncWorker`). Mock в dev/test. |
...
Сервіси Lore-шару SilkenNet. Повна специфікація: **[`04_05`](04_05_Codex_Lore_Module)**.
...
| **Side Effects** | offline у робочих станах → `report_fault!` + критичний `EwsAlert(queen_offline)` (анти-спам по кластеру; skip: maintenance, `last_seen_at` nil); повернення в ефір → `recover!` + машинний auto-resolve ОБОХ comms-типів (`resolve_comms_alerts`: queen_offline + queen_uplink_lost — Helium-SOS без нього латчився вічно); attest-lapse (>24h без QATT-підпису при online) → warn+gauge; **[ARCH.59] залипла OTA** (`state=:updating` довше за `OTA_STUCK_MARGIN` = 24 год, АБО `:updating` без якоря `ota_started_at`) → `finish_update!` + зняття `pending_firmware_id`/якоря + `EwsAlert(system_fault, medium, message_key: queen_ota_stuck)`, дедуп по `uid`. Метрики: `gateways_offline_total`, `gateways_faulty`, `gateway_attest_lapsed`. |
...
| **Сервіси** | — (Twilio/FCM не задротовані — [`ARCH.60`](00_07_Action_Plan_Tracker)) |
| **Side Effects** | **[ARCH.78]** Доки транспорту немає, обидві гілки пишуть `Rails.logger.warn` про **СТАН КАНАЛУ** («Канал не сконфігуровано — … НЕ надіслано/НЕ доставлено»), а не про результат. Попередня редакція логувала «Надіслано»/«Доставлено» біля закоментованого клієнта — журнал, що стверджує дію, якої не сталося, на тракті пожежної та вандалізм-сирени; під час розбору інциденту він читався б як доказ доставки. `case channel` має гучний `else` (невідомий канал → `logger.error`): тиша тут невідрізненна від доставки. Носій класу — `spec/quality/no_self_attesting_logs_spec.rb` (пара «закоментований транспорт + лог із дієсловом-результатом» у `app/**`; форма-агрегатор і статус-у-БД поза його стелею — вона задекларована в шапці). |

#### `Notifications::DeliveryChannels` [UI.10]

Дім ОДНОГО питання: чи має платформа **транспорт** для каналу. Читають двоє — екран налаштувань (`Notifications::Settings`, через контролер) і, у міру дротування, сам notification-шар.

| Параметр | Значення |
|----------|----------|
| **Тип** | Модуль-оголошення (`module_function`), без стану й без І/О |
| **API** | `available?(channel)` → Boolean · `available` → Array\<Symbol\> · `ALL` = `%i[email sms telegram push]` |
| **Пошта** | ДЕРИВУЄТЬСЯ з двох спостережуваних: відправник ≠ `from@example.com` (незмінений скаффолд) **і** `smtp_settings[:address]` ≠ `localhost` (дефолт `ActionMailer`, коли налаштувань не задавали). Конʼюнкція несуча — перша нога [`ARCH.60`](00_07_Action_Plan_Tracker) везе обидві разом, тож щойно вона приїде, екран скаже правду без правки цього модуля |
| **SMS / Telegram / Push** | **Оголошені** мертвими: адаптерів у дереві немає, і конфіг-поверхні теж — імена ENV назве `ARCH.60`, тож вигадувати їх наперед означало б завести другий дім чужого рішення |
...
> **⚠️ Критична проблема орбітального вікна (Post-TRL 8):** При `severity: :critical` (наприклад, `fire_detected`) чекати ~1.5 доби на прояснення хмар неприпустимо. Якщо dClimate повертає `:obscured_by_clouds` для критичних тривог, воркер не повинен просто йти в retry-sleep.
>
