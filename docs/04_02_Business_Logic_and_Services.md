...
| **Вихід / Side Effects** | Створює `TelemetryLog` записи (з `lorenz_state_x/y/z` + `cold_start_flag` [SEC.11] + `time_unsynced_fallback` [ARCH.41]). Оновлює `tree.latest_voltage_mv`, `tree.health_streak`. Нараховує `wallet.balance` (growth_points). ⚠️ **Mismatch прошивки лише СПОСТЕРІГАЄТЬСЯ — стану НЕ міняє** (виправлено 2026-08-16; доти тут стояло «Позначає `tree.firmware_update_status = :fw_pending`», знято присудом `ARCH.85` ще 2026-08-14): `check_firmware_mismatch!` пише лог і виходить, бо тракт не біг жодного разу й перший прогін стався б одразу в полі — а ретрансміту в цього стану немає ([`FW.63`](00_07_Action_Plan_Tracker)). |
