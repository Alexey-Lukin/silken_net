...
| **Що робить** | Аналізує телеметрію по 6 напрямках: вандалізм (tamper), пожежа (thermal: `temperature_c ≥ fire_limit`), пилка (**[SLASH-1]** acoustic: `bio_status_anomaly?` БЕЗ термального порога → `chainsaw_detected`; до спліту конфлатилось у `fire_detected` → FIRMS бачив «ясне небо» → тавро rejected_fraud на жертві вирубки), сейсміка, посуха/атрактор, шкідники. Термальний сигнал має пріоритет (горіння+пиляння упереміш = спершу пожежна відповідь). Адаптивні пороги (з кластера/породи дерева). Redis-фільтр тиші (5 хвилин per `tree_id:alert_type`). **[SEC.10]** Per-DID rate limiting для critical alerts: `MAX_ALERTS_PER_DID_PER_WINDOW=5` critical alerts за `DID_RATE_LIMIT_WINDOW=1.minute` — захист від replay/injection атак (forged panic packets). Time-bucketed cache key `"ews_did_rate:#{tree.did}:#{time_bucket}"`, TTL = 2 хвилини. Перевищення → warn log + silent drop. |
