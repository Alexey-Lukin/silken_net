...
>
> 3. **[OSS — ліцензії НЕ потребує]** Розщепити Sidekiq на 4 процеси з queue-pinning (uplink окремо, web3_* окремо, critical/alerts/downlink окремо, default/low окремо) — single-process × 15 threads не витягне peak ~ N_trees / 3600 jobs/sec. Механізм рядовий (`sidekiq -q`, капсули), тож крок виконуваний сьогодні; чекає він не гема, а власного тригера — [`06_08 §2.5`](06_08_Resilience_and_Failover_Policy).
