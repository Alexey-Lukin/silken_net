...
| **Тест coverage** | `spec/services/security/web3_network_guard_spec.rb` — testnet-RPC (Amoy/devnet), alnum-boundary false-positive, missing/malformed/`0x` oracle-key, silent-address set (missing DAO_TREASURY/SCC · placeholder · no-echo), Solana-presence, `signer_process: false` scoping (keyless-clean · формат/колізія/testnet/address-формат і без ключів) |
