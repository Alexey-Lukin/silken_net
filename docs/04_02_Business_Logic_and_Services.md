...
| **Виклик** | ✅ **Self-service з 2026-08-21** (⚖️ founder): `DELETE /account_security/erase` → `AccountSecurityController#erase_account`, запобіжник — **step-up на пароль** (той самий зразок, що `toggle_mfa` disable). Плюс оператор через консоль. 🔴 **Гард fail-CLOSED, і розходження з `toggle_mfa` НАВМИСНЕ:** там акаунт без пароля step-up МИНАЄ (дія оборотна, спільного секрета немає), тут — **відмова**, бо акт незворотний, і пропуск відкрив би його будь-кому з вкраденою cookie тієї миті, коли зʼявиться перший passwordless-вхід ([`04_03 §1.2`](04_03_REST_API_v1_Reference) називає цю форму такою, що «чекає свого тригера»). Секція UI дзеркалить гард — для акаунта без пароля не рендериться взагалі, інакше кнопка вела б у гарантовану відмову. Людський шлях лишається для саме таких акаунтів — [`gdpr_runbook.md`](protocols/legal/gdpr_runbook.md) §2 (строк Art.12(3), ідентифікація, реєстр запитів) |
...
> - **`InsightGeneratorService.cleanup_old_logs!`** — КЕНОЗИС-очищення сирої телеметрії старше 7 днів. Продовий шлях агрегації йде `DailyAggregationWorker` → `InsightGeneratorOrchestratorWorker`, а оркестратор кличе сервіс лише по `cluster_baselines`/`reset_stress_outside`, ніколи `#call` — тобто єдиний продовий виклик живе в мертвому колбеку. Складається з незалежно задокументованим «механізму дропу партицій НЕ існує, листи +1/міс» ([`04_01 §11`](04_01_Data_Models_and_Entities)): **raw-retention не працює ОБОМА своїми шляхами одночасно**, і жоден із них не сигналить.
