...
Кожен "Солдат" (дерево) отримує тут цифровий паспорт (наприклад, `did:peaq:0x...`). **Чесно (TRL):** паспорт ідентифікує пристрій, але його автентичності не доводить — DID підписує один backend-ключ (`PEAQ_SIGNING_KEY`), тобто це L0 custodial; доказ фізичного походження = true-DePIN North-Star по ladder ([`05_02` — Trust-origin ladder](05_02_Proof_of_Growth_Pipeline)).
...
🔴 **Виплата пишеться `token_type: :usdc` — і це ВАЛЮТА рядка, а не його мережа** [ARCH.120]. Розрізнення несуче, бо агрегат «скільки SCC існує» (`BlockchainTransaction.net_minted_supply`, дім [`05_05 §3`](05_05_Slashing_and_Risk_Policy)) фільтрує по `token_type` і `blockchain_network` **не читає за побудовою**. Доти обидва писачі (`record_event_intent!`, `record_batch_intent!`) клали суму в USDC під `:carbon_coin`, тож підтверджена мікровинагорода підсумовувалась намінтованим SCC і роздувала базу слешингу, L1-якір, ESG-запас і кожну дашборд-поверхню. ⚠️ Фантом при цьому **переважав справжній заробіток того самого пакета на порядок**: винагорода має ненульову БАЗУ (10 000 lamports), тоді як SCC за той самий аплінк = `growth_points / 10 000`. Дефект був латентний (єдиний enqueue — гілка `fulfilled` оракул-колбека, PATH 1 unwired; `totalSupply()` = 0), тобто емісії не спотворив жодного разу.
