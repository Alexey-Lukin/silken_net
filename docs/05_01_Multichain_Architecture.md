...
| **Верифікація** | **IoTeX (W3bstream)** | Детектор брехні — ZK-proof | Hardware-to-Cloud довіра через ZK-proof. **Чесно (TRL):** сьогодні доводить цілісність pipeline + прив'язку до on-chain peaq DID (master-backed атестація), а НЕ криптодоказ «саме цей STM32». Доказ фізичного походження = true-DePIN North-Star по ladder ([`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security)); RWA-легітимність доповнює ЗВТ-метрологія (STK.4). «Дані з заліза» — мета, до якої будуємось |
