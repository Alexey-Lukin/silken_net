...
| **СЄУ** | Business | Регіональна економіка (Чудаєва — ректор/якір), UA-правова рамка RWA + господарське право (Аблязов Д. — глибока MiCA/крипто → профільний TBD), фінансовий облік криптоактивів (Гедз), цифрова економіка + моделювання (Ус); D-MRV/carbon-аудит → профільний TBD; дизайн — self | [`08_02 §5`](08_02_Academic_Institutions_Registry) | [§1F](#-1f-публікації-сєу-токеноміка-право) |
...
| Автор | Внесок |
|-------|--------|
| **Чудаєва І.Б.** (СЄУ) | Регіонально-економічна рамка + інституційна валідація макромоделі SCC-ринку (стрес-тест dual-token = наша Monte-Carlo-симуляція власного контракту, self-owned); профіль = регіонально-інноваційна економіка |
| **Ус Г.О.** (СЄУ) | Економіко-математичне моделювання цифрової економіки (ст. 2021) + управління знаннями (2024) |
...
**Тип зв'язку:** Послідовний — СЄУ (Чудаєва) будує економічну модель (stress-testing 3 сценаріїв) → СЄУ (Ус) додає економіко-математичне моделювання цифрової економіки
...
| Автор | Внесок |
|-------|--------|
| **Аблязов Д.Е.** (Денис Едуардович, СЄУ) | Господарське/комерційне право, протидія рейдерству; UA-юрисдикційна правова рамка RWA (Лісовий Кодекс / ПЗФ / ЗУ «Про віртуальні активи»); NaaS Term Sheet + MSA шаблони. Глибока MiCA/ERC-3643/ONCHAINID → профільний крипто-юрист (TBD) |
| **Гедз М.Й.** (СЄУ) | Фінансовий облік криптоактивів в Україні (2025); регіональна фінансова експертиза + якість/акредитація (ISO 9001). D-MRV/carbon-аудит (ISO 14064/ICROA) — окрема потреба → профільний carbon-методолог ([`00_07` BIZ.9](00_07_Action_Plan_Tracker)) |
...
| **Гедз М.Й.** | СЄУ | Фінансовий облік криптоактивів (2025); регіональна фінансова експертиза + ISO 9001. Carbon-аудит (ISO 14064/ICROA) → профільний методолог ([`00_07` BIZ.9](00_07_Action_Plan_Tracker)) |
...
| **Магістерська** | "Економіко-математичне моделювання цифрової економіки токенізованих ринків" | Ус |
| **Магістерська** | "UA-правові аспекти токенізації Real World Assets (Лісовий Кодекс / ПЗФ)" | Аблязов Д. (Денис Едуардович) |
| **Магістерська** | "Фінансовий облік криптоактивів в Україні" | Гедз |
