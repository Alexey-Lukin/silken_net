...
- [1F. Публікації СЄУ (Токеноміка, Право)](#-1f-публікації-сєу-токеноміка-право)
...
```
                    ┌──────────────────────────────────────────┐
                    │  GOVERNMENT / REGULATOR / END-USER LAYER │
                    │  ДСНС · Мінприроди · Черкаська ОДА · ПЗФ │
                    │  Європейські реєстри (ISO 14064 / ICROA) │
                    └────────────────────┬─────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       │                                 │                                 │
...
│  HARD-SCIENCE│  ◄──data──►   │   CONTROLLER       │   ◄──data──►  │   BUSINESS   │
│  LAYER       │               │   (Silken Net /    │               │   LAYER      │
│              │               │    ActiveBridge,   │               │              │
│ ЧНУ + ЧДТУ + │               │   TRL 8 system     │               │  СЄУ + ЧІПБ  │
│ ЧМА          │               │   integrator)      │               │  (legal arm) │
└──────────────┘               └────────────────────┘               └──────────────┘
```
...
|-----|-------|--------------------|---------------------|---------------|
| **ЧНУ (ректорат + hard-science школи)** | Hard Science | Фізика твердого тіла (Гусак), квантова хімія (Мінаєв), біоценологія/дендрологія (Спрягайло, Гаврилюк), кібернетика ФОТІУС (Порубльов, Ярмілко, Косенюк, Бушин, Любченко) + інституційна парасоля (Кирилюк) | [`08_02 §1`](08_02_Academic_Institutions_Registry) | [§1, §1C, §1G](#-1g-публікації-ректорату-чну--нні-економіки-і-права-біоекономіка-та-синергетика) |
| **ЧДТУ (3 кафедри)** | Hard Science | Data Science та статистика (Карапетян, кафедра статистики), радіофізика та EMC (Гончаров, ФЕТР), акустична мехатроніка та п'єзохарактеризація (Базіло, Бондаренко, ПМКТ) | [`08_02 §2`](08_02_Academic_Institutions_Registry) | [§1B](#-1b-публікації-чдту-data-science) |
| **ЧМА** | Hard Science | Біохімія EBFC та ферментна стабільність (Бушуєва, Глущенко), токсикологія Ti-6Al-4V (Суховой), ксилемоінтеграція як аналогія остеоінтеграції (Губенко, Боєчко) | [`08_02 §4`](08_02_Academic_Institutions_Registry) | [§1E](#-1e-публікації-чма-біохімія-ebfc-токсикологія-ксилемоінтеграція) |
| **ЧІПБ** | Business (regulated safety) | QRA для тригерів параметричного страхування (Зобенко), предиктивна fire-модель delta_t→FWI (Куліца), SOP конверсії EwsAlert→ДСНС (Биченко, Ротар, Несен) | [`08_02 §3`](08_02_Academic_Institutions_Registry) | [§1D](#-1d-публікації-чіпб-пожежна-безпека-параметричне-страхування-та-sop) |
| **СЄУ** | Business | Макроекономіка SCC-ринку (Чудаєва), правова архітектура RWA / MiCA / ERC-3643 (Аблязов Д.), ISO 14064 / ICROA D-MRV аудит (Гедз), unit-економіка та ESG-Accounting (Ус); інфраструктурні ролі поза публікаційним портфелем: CAD PEEK-радому з **anti-overgrowth shield** (Денисенко — [`02_01`](02_01_Hardware_Architecture_and_BOM)/[`02_02`](02_02_Blind_Mate_Pogo_Pin_Interface)), етнодизайн/UI-патерн (Теліженко — [`04_04`](04_04_Phlex_UI_and_Tailwind)) | [`08_02 §5`](08_02_Academic_Institutions_Registry) | [§1F](#-1f-публікації-сєу-токеноміка-право) |
...
> **Принцип ортогональності:** жодна комірка `(ВНЗ, ніша)` не дублюється. Якщо два університети потенційно претендують на одну тему то межа проходить за **об'єктом аналізу** (напр. ЧДТУ веде статистичне виявлення аномалій та шахрайства — `FRAUD_DEVIATION_THRESHOLD`, Ст.13); перетин фіксується як **міжуніверситетська синергія** (зони §1C, §1G).

### 0.4. Цінність Кластера — Absolute Credibility

Формування MOIC дає Silken Net **Absolute Credibility (Абсолютну Довіру)** — стратегічну перевагу при презентації Web3-фондам, регуляторам та грантовим комітетам:
...
Сукупно це робить консорціум **ідеальним кандидатом** на мега-гранти **Horizon Europe Cluster 6** (Biodiversity Monitoring + Bioeconomy, бюджет 4–6 М€ на грант, [`07_03`](07_03_Grant_Applications_Tracker)). Окремо — **повністю незалежний європейський D-MRV-стек**, що знімає геополітичну залежність від ринкових дій США/Китаю та робить Україну **точкою відліку** глобальної мережі планетарного моніторингу лісів.
...
| [`08_02 §1`](08_02_Academic_Institutions_Registry) | ЧНУ — Hard Science (§1A: фізика/хімія/біологія) + ФОТІУС (§1B: mruby-Lorenz-аудит + GA/NSGA-II ML) | Лабораторні протоколи + матем. основа Статей 1–3, 23–25, 28–30, 34–35 |
...
**Назва (EN):** _"Decentralized MRV at Planetary Scale: Lorenz Threshold Gating and Blockchain-Verified Carbon Accounting in Autonomous Forest IoT Networks"_

> ⚠️ **Reframe (E.63/E.64, 2026-07-17):** «Homeostasis **Detection**» знято з назви — Z НЕ вимірює здоров'я дерева (гіпотеза емпірично degenerate + temp-confounded, [`03_04 §4`](03_04_mruby_Lorenz_Attractor)). Лоренц = **пороговий гейт** статусу + DCI-anti-fraud; механізм гейта = дім Ст.34. ⚠️ **Mesh-ланки нижче — відкладені:** TTL-flood relay живий лише в ECB-ері (`#if !FW2_CCM_ENABLED`), CCM-фліп робить мережу **star-only** (ухвала FW.2 (а)); повернення = [`00_07` ARCH.43](00_07_Action_Plan_Tracker) post-TRL 6.
**Журнали:** Nature Machine Intelligence (Q1) · IEEE Internet of Things Journal (Q1) · Computers & Electronics in Agriculture (Q1)

| Автор | Внесок |
|-------|--------|
| **Порубльов І.М.** | Percolation/Markov `q_c`-теорія надійності mesh — ⚠️ **відкладено** (mesh star-only post-CCM, ARCH.43 post-TRL 6), **двері відкриті**; Monte-Carlo `P_delivery` G(V,E,P) / TTL-калібрування (E.51) **self-authored 07-20** ([`00_07` — E.51](00_07_Action_Plan_Tracker), `tools/mesh/`) |
...
> **Принцип:** Де ЧНУ ФОТІУС створює алгоритм або модель — ЧДТУ статистично валідує та розширює. Де ЧНУ ФОТІУС виконує аналітичний розрахунок (RF, фільтри) — ЧДТУ (ФЕТР, ПМКТ) верифікує лабораторно. Зони перетину описані у цій секції та §1G.

### Стаття 23: Прихована SMD-Антена LoRa у Лісовому Середовищі — Розрахунок, Експериментальна Верифікація та Енерго-Свідомий Вибір Coding-Rate

**Назва (EN):** _"Concealed LoRa SMD Antenna Under PEEK Radome for EBFC-Powered Forest IoT: Impedance Matching, 3D Radiation Pattern, VNA/EMC Verification and Energy-Aware Coding-Rate Selection"_
...
| Автор | Внесок |
|-------|--------|
| **Косенюк Г.В.** (ЧНУ ФОТІУС) | Аналітичний розрахунок імпедансу, FEKO/CST моделювання діаграми спрямованості, LC-узгодження; Link Budget LoRa у лісі (SF=7–9, [`02_01 §5.3`](02_01_Hardware_Architecture_and_BOM)); 3D-діаграма з Ti-анкером (Zone 1 + Zone 3 фланець) як Ground Plane; **енерго-свідомий вибір coding-rate** (CR 4/5 Hamming у кремнії SX126x vs cross-packet erasure-coding — airtime/EBFC-бюджет, [`00_07` FW.61/E.15](00_07_Action_Plan_Tracker)); CE/FCC compliance roadmap |
| **Гончаров А.** (ЧДТУ ФЕТР) | VNA-виміри S11 реальної зборки, натурні вимірювання path loss у лісі, EMC pre-compliance тестування |
| **Ярмілко А.В.** (ЧНУ ФОТІУС) | Крипто-навантаження того самого LoRa-хопа: **AES-128-ECB → AES-128-CCM** Soldier→Queen ([`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security), FW.2 — тракт зашитий обабіч, лишилась верифікація на кремнії); вплив MIC/довжини кадру на airtime-бюджет. Постквантова roadmap: [`03_05 §10`](03_05_Hardware_Symmetric_Crypto_and_Security) |
...
**Тип зв'язку:** Послідовний — ЧНУ (Косенюк) розраховує аналітично + обирає coding-rate → ЧДТУ (ФЕТР, Гончаров) верифікує на VNA/у лісі → коригування LC-ланцюга → серійна специфікація. ЧНУ (Ярмілко) паралельно тримає крипто-навантаження того ж кадру (CCM-MIC ↔ airtime).
...
_«Радіотехнічна оптимізація мікропотужних IoT-пристроїв у лісовому середовищі: узгодження антенних систем та енерго-свідомий вибір параметрів кодування»_
...
| **Ярмілко А.В.** (ЧНУ ФОТІУС) | SPI/DMA pipeline для високошвидкісного п'єзо-ADC (ultrasonic 25–150 кГц v3-тракт — **не** поточний 16 кГц audible-firmware, узгоджено з reframe вище), firmware інтеграція п'єзосенсора |
...
- [`08_02 §1B`](08_02_Academic_Institutions_Registry) — Бушин CNN-NDVI + fauna feature
...
| Автор | Внесок |
|-------|--------|
| **Зобенко Н.** (ЧІПБ) | QRA для 3 `trigger_event` (`critical_fire`/`extreme_drought`/`insect_epidemic`), актуарне обґрунтування `threshold_value` (поріг озброєння виплати). ⚠️ **Каральна крива поза scope статті** (`GAMMA`/`PENALTY_FACTOR_MAX` = DAO-governed — [`05_05 §3`](05_05_Slashing_and_Risk_Policy); їх вирішує голосування, не наука) |
...
| **Порубльов І.М.** (ЧНУ ФОТІУС) | **Обґрунтування Z-порогів гомеостазного гейта** (`CRITICAL_Z_MIN/MAX`, `OPTIMAL_Z_TARGET` — [`03_04 §4`](03_04_mruby_Lorenz_Attractor) сам вимагає «валідації з академічним партнером»; вихід = пороги ризику/slashing, [`05_05 §8`](05_05_Slashing_and_Risk_Policy) — «найбільший науковий ризик проєкту»). ⚠️ Перколяція `q_c` / ланцюги Маркова — **відкладено**: mesh-relay живий лише в ECB-ері (`#if !FW2_CCM_ENABLED`), CCM-фліп робить мережу star-only; повернення = [`00_07` ARCH.43](00_07_Action_Plan_Tracker) post-TRL 6 |
| Архітектор (Silken Net) | ParametricInsurance модель, Etherisc DIP Oracle, ContractHealthCheckService |

**Тип зв'язку:** Послідовний (4 ланки) — ФОТІУС (Z-пороги гомеостазного гейта) → ЧДТУ (актуарні моделі) → ЧІПБ (QRA + fire model) → Архітектор (on-chain integration)
...
## 💼 1F. Публікації СЄУ (Токеноміка, Право)

> **Контекст:** СЄУ (Східноєвропейський університет ім. Рауфа Аблязова) забезпечує академічну експертизу в макроекономічній валідації токеноміки NaaS, правовій архітектурі RWA-токенізації (Polygon Hadron / MiCA) та аудиті методології D-MRV (ISO 14064 / ICROA). Повний реєстр задач — у [`08_02` — Academic Institutions Registry](08_02_Academic_Institutions_Registry).
...
## 🏛️ 1G. Публікації Ректорату ЧНУ × ННІ Економіки і Права (Біоекономіка та Синергетика)

> **Контекст:** ЧНУ у консорціумі п'яти університетів (ЧНУ + ЧДТУ + ЧІПБ + ЧМА + СЄУ) виступає **подвійним вузлом**: hard-science школи (Мінаєв / Гусак / ФОТІУС / біо-хаб) розташовані у §1–§1C, а ректорат і ННІ економіки і права — окремий теоретично-економічний шар, що дзеркально доповнює СЄУ (§1F). Цей шар представлений двома постатями: **в.о. ректора Є.М. Кирилюк** (директор ННІ економіки і права, біоекономіст-синергетик) і **проректор з науки О.В. Спрягайло** (дендролог, ПЗФ-практик, біо-хаб координатор). Деталі їхніх профілів, точок дзеркальної синергії та операційної послідовності зустрічей — у [`08_02 §1A`](08_02_Academic_Institutions_Registry).

### Інституційна архітектура ЧНУ у консорціумі п'яти університетів

| Шар ЧНУ | Постаті | Роль у консорціумі | WP / статті |
|---------|---------|---------------------|-------------|
| **Парасоль ректорату** | Є.М. Кирилюк (в.о. ректора) | Підпис парасольового MoU ЧНУ↔Silken Net; інституційна координація з ректорами ЧДТУ / ЧІПБ / ЧМА / СЄУ; Triple Helix governance ([`08_02 §5`](08_02_Academic_Institutions_Registry), [`07_03`](07_03_Grant_Applications_Tracker)) | Стаття 34 (синергетика порогового гейта), Стаття 35 (біоекономіка) |
| **Біо-хаб** | О.В. Спрягайло (проректор з науки), М.В. Гаврилюк (директор ННІ природничих наук) | Координація дендрологічного, фітоценологічного та зоологічного блоку; ПЗФ-сумісність pilot'у; ground-truth для Mongabay-pivot | Стаття 24a (Mongabay), Стаття 29 (токсикологія), Стаття 30 (CODIT), Стаття 35 (біоекономіка) |
| **Хімія наноматеріалів** | Б.Ф. Мінаєв, В.О. Мінаєва | DFT/TDDFT моделювання EBFC та TiO₂ | Стаття 1 (EBFC), Стаття 28 (біохімія EBFC) |
| **Фізика твердого тіла** | А.М. Гусак | Кіркендалл, Arrhenius, FEM | Стаття 2 (тонкокорозія), Стаття 29 (токсикологія), Стаття 30 (FEM термо-механіка Ti↔деревина) |
| **ФОТІУС (cybernetics)** | Порубльов, Ярмілко, Косенюк, Бушин, Любченко | Firmware, math, RF, GA | Статті 3, 23, 24, 24a, 25, 34 |

> **Принцип розподілу:** Парасоль ректорату (Кирилюк) і біо-хаб (Спрягайло) **не дублюють** функції шкіл Мінаєва / Гусака / ФОТІУС, а перпендикулярно з'єднують їх з економічно-правовим шаром (СЄУ, §1F) та з зовнішнім регіональним полем (Черкаська ОДА, ПЗФ, Стратегія розвитку області — див. [`08_02 §1A`](08_02_Academic_Institutions_Registry)). Це робить ЧНУ **єдиним вузлом** консорціуму, що замикається сам на собі через два паралельні треки (hard science ↔ bio-economic theory) під одним ректорським підписом.

---

### Стаття 34: Синергетика Біо-Токенізованих Ринків — Пороговий Гейт Лоренца як Біфуркаційний Механізм Емісії Карбонових Монет

**Назва (EN):** _"Synergetics of Bio-Tokenized Markets: A Lorenz Threshold Gate as a Bifurcation Mechanism for Carbon Coin Emission in Forest Nature-as-a-Service Networks"_
**Назва (UA):** _«Синергетика біо-токенізованих ринків: пороговий гейт Лоренца як біфуркаційний механізм емісії карбонових монет у Nature-as-a-Service лісових мережах»_
**Журнали-цілі:** *Ecological Economics* (Q1) · *Chaos, Solitons & Fractals* (Q1) · *Journal of Evolutionary Economics* (Q2) · *Technological Forecasting and Social Change* (Q1)

> ⚠️ **Reframe (E.63/E.64 — премісу виправлено 2026-07-17):** стаття **НЕ** стверджує, що Z-координата вимірює здоров'я дерева — цю гіпотезу власний канон оголошує **емпірично degenerate + temp-confounded** ([`03_04 §4`](03_04_mruby_Lorenz_Attractor)), і магнітуда `growth_points` у гомеостазі задається **метаболізмом** `m(delta_t)`, не Z ([`03_04 §4.3`](03_04_mruby_Lorenz_Attractor)). Предмет статті вужчий і **чесний**: Лоренц працює як **пороговий гейт емісії** — `z > anomaly_ceiling` → `growth_points = 0` (емісія зупиняється), `z < CRITICAL_Z_MIN` → `growth_points = 1`; тобто **якісна зміна режиму при переході порогу = біфуркація в буквальному сенсі**. Саме цей дискретний перехід (а не неперервна кореляція «хаос ↔ здоров'я») з'єднує мікро-рівень із макро-емісією.

**Контекст / мотивація:** Стаття Кирилюка 2012 р. _«Методологія синергетики в дослідженні процесів трансформації економічних систем»_ застосовує нелінійну динаміку, самоорганізацію та біфуркації до економічних систем. Silken Net реалізує той самий апарат на mruby VM ([`03_04`](03_04_mruby_Lorenz_Attractor)): 250-ітераційний Ейлерів атрактор Лоренца рахується на кожному вузлі, а його Z-координата проходить **пороговий гейт**, який вирішує, чи вузол взагалі емітує `growth_points` (→ SCC). Стаття будує міст між двома рівнями синергетики: **режимний перехід одного STM32-вузла** (біфуркація за порогом) і **самоорганізація токенізованого ринку** (supply/demand SCC, Dynamic Tax як біфуркаційний параметр).

**Унікальність публікації:**
1. Перше застосування **синергетики Хакена-Пригожина** до емісії токенів реального біологічного активу (жива деревна біомаса), а не фінансових деривативів.
2. **Двошарова біфуркаційна рамка:** мікро-шар — пороговий гейт Лоренца (σ=10, ρ=28, β=8/3) як дискретний режимний перехід, що вмикає/вимикає емісію вузла; макро-шар — диференціальне рівняння supply/demand SCC з atractor-shaped рівновагою. **Спільний механізм — біфуркація, не кореляція.**
3. Перша математична формалізація **Dynamic Tax 2 %** ([`05_03`](05_03_Tokenomics_SCC_and_SFC)) як біфуркаційного параметра, що утримує SCC-ринок у стійкому атракторі при `insurance_pool < 100 000 SCC` (обидва числа — чинні: `ProtocolParameters.sol`).
4. **ρ-відносний поріг як приклад калібрування біфуркації емпірикою** (E.64): абсолютний `z > 45` хибно тригерився теплим днем (здорове дерево → обнулення емісії); перехід на `ρ + 17` знизив хибні аномалії **22% → 3%** — рідкісний випадок, коли біфуркаційний поріг кіберфізичної системи виміряно на реальному коді.

| Автор | Афіліація | Внесок |
|-------|-----------|--------|
| **Кирилюк Є.М.** | ЧНУ, в.о. ректора, директор ННІ економіки і права | Методологічна рамка синергетики економічних систем (стаття 2012); біфуркаційний аналіз SCC supply/demand; інтерпретація Dynamic Tax як параметра самоорганізації |
| **Порубльов І.М.** | ЧНУ ФОТІУС | Обґрунтування порогів гейта (`CRITICAL_Z_MIN/MAX`, `OPTIMAL_Z_TARGET=29.0` — [`03_04 §4`](03_04_mruby_Lorenz_Attractor) іменно його називає партнером валідації); числовий DCI-крок `\|server_z − device_z\| < ε` як anti-fraud-шар (відкритий після SEC.11 — [`03_04 §7`](03_04_mruby_Lorenz_Attractor)) |
| **Чудаєва І.Б.** | СЄУ | Макроекономічна модель SCC-ринку, stress-testing 3 сценаріїв (cross-ref зі Статтею 31) |
| **Карапетян А.Р.** | ЧДТУ | Ляпуновські показники, фрактальна розмірність, RQA-аналіз траєкторій — характеризація **режимної структури** атрактора навколо порогів гейта (дім ланки після вилучення Ст.15) |
| Архітектор (Silken Net) | — | mruby VM атрактор + `pack_status_byte` пороговий гейт ([`03_04 §4.2`](03_04_mruby_Lorenz_Attractor) Таблиця Рішень), BlockchainMintingService Dynamic Tax, ProtocolParameters.sol |

**Тип зв'язку:** Послідовний (4 ланки) — Кирилюк (рамка синергетики) → Порубльов (пороги гейта + DCI) → Чудаєва (макро-модель SCC) → Карапетян (режимна/нелінійна характеризація обох шарів). Архітектор інтегрує firmware ↔ smart contracts.

**Cross-references:**
- [`03_04 §4`](03_04_mruby_Lorenz_Attractor) — «Логіка Гомеостазу: Z → growth_points» + §4.2 Таблиця Рішень (дім гейта)
- [`05_03`](05_03_Tokenomics_SCC_and_SFC) — Dynamic Tax 2 % + insurance-pool поріг
- [`08_02 §1A`](08_02_Academic_Institutions_Registry) — обґрунтування участі Кирилюка
- [`00_08 §1`](00_08_Beyond_TRL9_Planetary_Roadmap) — Gap #1 (network-coupled chimera states) = far-horizon продовження цієї ж лінії
- Стаття 31 (Чудаєва, макроекономіка) — допоміжна

**Грантовий вектор:** наукове обґрунтування **Horizon Europe Cluster 6 WP7** (Bioeconomy & Tokenized Markets — теоретична рамка) + **NRFU «Наука для відновлення України»** (українська школа синергетичної біоекономіки токенізованих лісів).

**Магістерська робота (Науковий керівник — Кирилюк):**
_«Синергетична методологія аналізу трансформації економічних систем у контексті токенізації природних активів: пороговий гейт як біфуркаційний механізм емісії»_
_(ННІ економіки і права; запропонована тема; запуск після підписання парасольового MoU)_

---

### Стаття 35: Від Культивованої Дендрофлори до Токенізованого Лісового Капіталу — Біоекономічна Рамка Nature-as-a-Service для Середнього Подніпров'я

**Назва (EN):** _"From Cultivated Dendroflora to Tokenized Forest Capital: A Bioeconomic Framework for Nature-as-a-Service in the Middle Dnieper Region"_
**Назва (UA):** _«Від культивованої дендрофлори до токенізованого лісового капіталу: біоекономічна рамка Nature-as-a-Service для Середнього Подніпров'я»_
**Журнали-цілі:** *Land Use Policy* (Q1) · *Forest Policy and Economics* (Q1) · *Ecological Economics* (Q1) · *Journal of Rural Studies* (Q1)

**Контекст / мотивація:** Монографія Кирилюка 2020 р. _«Формування складників національної біоекономіки України в умовах прискорення науково-технічного прогресу»_ та англомовні Q1-статті 2024/2025 р. (*Science Horizons*) дають теоретичну рамку, чому жива біомаса є капіталом і як формуються нові ринки на природних ресурсах. Кандидатська дисертація Спрягайла 2013 р. _«Культивована дендрофлора Середнього Подніпров'я та перспективи її оптимізації»_ (захищена в НБС ім. М.М. Гришка НАНУ, [`08_02 §1A`](08_02_Academic_Institutions_Registry)) дає **верифікований інвентар реального капіталу** того самого регіону: видовий склад, віталітет, заповідний статус, інвазійні види (*Acer negundo*), 10-річні бази стресових подій Черкаського бору. Стаття 35 з'єднує ці два шари в єдиний прикладний фреймворк: **як саме культивована дендрофлора Середнього Подніпров'я перетворюється на on-chain капітал Nature-as-a-Service** ([`07_01`](07_01_Nature_as_a_Service_Contracts), [`05_03`](05_03_Tokenomics_SCC_and_SFC)) з урахуванням індексу різноманіття Шеннона, екосистемних послуг та правового режиму ПЗФ.

**Унікальність публікації:**
1. Перший в Україні **empirically grounded** біоекономічний фреймворк токенізації лісового капіталу, де теорія (Кирилюк) і польовий інвентар (Спрягайло) походять з одного регіону.
2. Інтеграція **індексу Шеннона** ($H' = -\sum p_i \ln p_i$) як on-chain метрики біорізноманіття у `ForestNFT` metadata — диференціація від чисто карбонових D-MRV систем.
3. Правовий аналіз сумісності SCC/SFC-токенізації з режимом ПЗФ України — критично для першого pilot'у в Черкаському борі (cross-ref зі Статтею 32 Аблязова Д. щодо MiCA / ERC-3643).
4. Економічна оцінка інвазійних видів (*Acer negundo*) як **negative externality** у tokenomics: чи мінтиться SCC за біомасу інвазійного клену, чи навпаки слешиться (cross-ref [`05_03`](05_03_Tokenomics_SCC_and_SFC)).

| Автор | Афіліація | Внесок |
|-------|-----------|--------|
| **Кирилюк Є.М.** | ЧНУ, в.о. ректора, ННІ економіки і права | Біоекономічна рамка (монографія 2020, Q1-статті 2024/2025); економіка нових ринків; ESG-позиціонування Nature-as-a-Service |
| **Спрягайло О.В.** | ЧНУ, проректор з науки, біо-хаб | Дендрофлора Середнього Подніпров'я (дисертація 2013, НБС Гришка); інвазійні види (*Acer negundo*); 10-річні бази Черкаського бору; правовий режим ПЗФ (досвід проєкту ОДА 2018–2020) |
| **Гаврилюк М.В.** | ЧНУ біо-хаб | Зоологічний компонент екосистемних послуг; remote sensing / GIS прив'язка |
| **Чудаєва І.Б.** | СЄУ | Макроекономічна валідація (cross-ref зі Статтею 31); supply/demand SCC у регіональному масштабі |
| **Аблязов Д.Е.** | СЄУ | Правова рамка ERC-3643 / MiCA для лісових RWA (cross-ref зі Статтею 32); сумісність із режимом ПЗФ України |
| **Гедз М.Й.** | СЄУ | ISO 14064 / ICROA mapping для біорізноманіттєвих кредитів |
| Архітектор (Silken Net) | — | NaaS-архітектура, ForestNFT metadata schema, BlockchainMintingService guard clauses |

**Тип зв'язку:** Багатошарова паралель з фінальним синтезом — Кирилюк (теорія) ↔ Спрягайло+Гаврилюк (емпірія регіону) працюють паралельно з Чудаєвою (макроекономіка) ↔ Аблязовим+Гедзом (право та аудит). Архітектор інтегрує всі шари в технічну специфікацію.

**Cross-references:**
- [`08_02 §1A`](08_02_Academic_Institutions_Registry) — обґрунтування пари Кирилюк ↔ Спрягайло
...
**Магістерська робота (Науковий керівник — Спрягайло; ко-керівник — Кирилюк):**
_«Біоекономічна оцінка культивованої дендрофлори Середнього Подніпров'я як основи Nature-as-a-Service токенізованого ринку: випадок Черкаського бору»_
_(Спільна тема ННІ природничих та аграрних наук + ННІ економіки і права; запуск після підписання парасольового MoU; кандидат на спільну заявку NRFU)_
...
авторством: R-аналітика (Карапетян), RF-дані (Гончаров / ФЕТР), акустичні калібрування (Базіло /
Бондаренко / ПМКТ), юр-шаблони NaaS (Аблязов Д.), ESG-framework (Ус), CAD-радом (Денисенко).
...
|---|---|---|---|
| 1 | auxiliary MoU ЧНУ (через ректорат, Спрягайло — §1G) | 👤 Архітектор + Кирилюк | open-license + co-authorship MoU (**без** embargo/NDA на технологію) |
...
SilkenNet стає офіційним **полігоном для магістерських робіт** студентів ФОТІУС та природничих факультетів ЧНУ:

| Напрям | Тема магістерської | Науковий керівник |
|---|---|---|
| Комп'ютерні науки | "Енергетична ціна автентифікованого шифрування: AES-128-CCM проти ECB у LoRa-тракті мікропотужного вузла" | Ярмілко |
...
| Рівень | Тема |
|--------|------|
| **Магістерська** | "Методи виявлення аномалій у масивах біотелеметрії" |
| **Магістерська** | "Статистична кореляція супутникових та сенсорних даних" |
| **Бакалаврська** | "Аналіз розподілів growth_points на мові R" |
| **Бакалаврська** | "Візуалізація атрактора Лоренца та рекурентні графіки" |
| **Бакалаврська** | "Порівняльний аналіз класифікаторів стресу дерев" |
| **Курсова** | "Описова статистика телеметрії Silken Net (R)" |
| **Курсова** | "Автокореляційний аналіз Z-значень (R)" |
...
| **Бакалаврська** | "Характеризація п'єзоелектричних перетворювачів для детекції кавітаційних (ультразвукових 25–150 кГц) емісій у рослинній тканині" |
| **Бакалаврська** | "Створення калібрувального акустичного датасету (включно зі звуковим ландшафтом фауни) для навчання TinyML моделей біомоніторингу лісу" |
...
| **Бакалаврська** | "Орнаментально-етнодизайнерська візуальна мова B2B-інтерфейсу ESG-дашборду" | Теліженко (етнодизайн/UI-патерн → [`04_04`](04_04_Phlex_UI_and_Tailwind)) — ⚖️ **ВИРІШЕНО 2026-07-19:** product-UX-рамка = той самий mis-cast, що й Ст.33; ре-scope у її реальний фах (орнамент/патерн-мова, не product-UX) |
...
- Подав заявки до Web3-фондів (peaq, IoTeX, Chainlink, Filecoin, Giveth, Solana, Polygon — [`07_03`](07_03_Grant_Applications_Tracker))
- Приносить робочий стек + перспективу Scopus-публікацій
...
> ⚠️ **Чесність pitch'а (2026-07-17):** рядки вище раніше казали «**залучає** Web3-фінансування» і «**готовий** R&D-полігон». Перше: **жодна** заявка не має відповіді — усі в статусі «Подано» ([`07_03`](07_03_Grant_Applications_Tracker); Giveth «Активний» = профіль живий, донатів нема — теж не гроші). Друге: anchor/EBFC = TRL 3, польового стенду ще нема ([`00_03`](00_03_TRL_Matrix_HIL_and_Beyond)) — «робочий стек» = backend TRL 8 + firmware TRL 6 + in-silico Zero-Lab, і цього досить. Обидва overclaim'и валяться від **одного** уточнювального питання в кабінеті — не казати того, що не витримає перепитування.
...
| "Прошу допомоги з технічним питанням" | "Пропоную полігон для досліджень + Scopus-публікації" |
