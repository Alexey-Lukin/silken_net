...
| Архітектор (Silken Net) | ProtocolParameters.sol, BlockchainMintingService (Dynamic Tax), 05_03 Tokenomics; Monte Carlo supply/demand — self-owned (симуляція власного контракту) |

**Тип зв'язку:** Послідовний — СЄУ (Чудаєва) будує економічну модель (stress-testing 3 сценаріїв) → СЄУ (Ус) інтегрує в корпоративний Accounting Framework
...
**Тип зв'язку:** Послідовний (2 ланки) — Кирилюк (рамка синергетики) → Чудаєва (макро-модель SCC). Архітектор інтегрує firmware ↔ smart contracts. Пороги гейта входять параметром (money-path; валідація — ground-truth протокол [`05_05 §8`](05_05_Slashing_and_Risk_Policy)).
