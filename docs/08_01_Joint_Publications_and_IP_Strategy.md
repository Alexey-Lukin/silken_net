...
- [1D. Публікації ЧІПБ (Пожежна Безпека та SOP)](#-1d-публікації-чіпб-пожежна-безпека-та-sop)
...
| **ЧІПБ** | Business (regulated safety) | SOP реагування ДСНС на кіберфізичні тривоги: диспетчеризація + drone-розвідка (Биченко — тактика гасіння), SOP per alert_type (Ротар — правова регламентація) | [`08_02 §3`](08_02_Academic_Institutions_Registry) | [§1D](#-1d-публікації-чіпб-пожежна-безпека-та-sop) |
...
## 📊 1D. Публікації ЧІПБ (Пожежна Безпека та SOP)

> **Контекст:** ЧІПБ забезпечує академічну експертизу у розробці SOP для фізичного реагування ДСНС на кіберфізичні тривоги раннього попередження — двоє профільних експертів: **Биченко** (тактика гасіння / керівництво діями підрозділів → диспетчеризація + drone-розвідка) та **Ротар** (правова компетентність ЦЗ, тактичні алгоритми → SOP-регламентація). Повний реєстр задач — у [`08_02` — Academic Institutions Registry](08_02_Academic_Institutions_Registry).
...
| **Ротар В.** (ЧІПБ) | SOP-документи per alert_type (drought/epidemic/vandalism/fire/seismic/fault/entropy/field_audit — [`00_07` UNI.12/ARCH.31](00_07_Action_Plan_Tracker)), протоколи для форестерів та патрулів |
