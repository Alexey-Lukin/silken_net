...
**Тип зв'язку:** Послідовний — ЧНУ (Косенюк) розраховує аналітично → ЧДТУ (ФЕТР, Гончаров) верифікує на VNA/у лісі → коригування LC-ланцюга → серійна специфікація. ЧНУ (Ярмілко) паралельно тримає крипто-навантаження того ж кадру (CCM-MIC ↔ airtime).
...
**Тип зв'язку:** Послідовний — ЧДТУ (ПМКТ) валідує фізику фононної лінзи → ЧДТУ (Карапетян) статистично аналізує результати (firmware ADC-DMA для п'єзо-тракту — self-owned, [`03_01`](03_01_Firmware_Lifecycle_and_DMA))
...
**Тип зв'язку:** Послідовний (3 ланки) — ЧДТУ (актуарні моделі) → ЧІПБ (QRA + fire model) → Архітектор (on-chain integration). Z-пороги гомеостазного гейта входять параметром; їх валідація — ground-truth протокол [`05_05 §8`](05_05_Slashing_and_Risk_Policy)
...
| **ФОТІУС (cybernetics)** | Ярмілко, Косенюк, Любченко | Firmware, RF, GA | Статті 23, 24, 24a |
...
**Тип зв'язку:** Послідовний (3 ланки) — Кирилюк (рамка синергетики) → Чудаєва (макро-модель SCC) → Карапетян (режимна/нелінійна характеризація обох шарів). Архітектор інтегрує firmware ↔ smart contracts. Пороги гейта входять параметром (money-path; валідація — ground-truth протокол [`05_05 §8`](05_05_Slashing_and_Risk_Policy)).
...
**Грантовий вектор:** Стаття 34 → Horizon Europe Cluster 6 WP7 (Bioeconomy & Tokenized Markets) + NRFU «Наука для відновлення України».
...
**Грантовий вектор:** Стаття 35 → Horizon Europe Cluster 6 (Bioeconomy / Forest-based Value Chains); у парі зі Ст.24a — twin-pack під co-PI Кирилюк (WP-Bioeconomy) + Спрягайло (WP-Biodiversity).
