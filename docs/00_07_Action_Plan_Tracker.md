...
- [x] ✅ (2026-06-15d, A+) `recordPremiumPaid()` + `PremiumPaid`-event ЗНЯТО — never-fed pipe (0 backend-викликів) + архітектурно чужорідний (SCC-event для off-chain USDC-факту); видалено з контракту/тестів/subgraph (schema/mapping/yaml/abi), Real-Yield звіт тепер бере премію з БД (`NaasContract.total_insurance_premiums`). Канон [`05_03`](05_03_Tokenomics_SCC_and_SFC)

#### SEC.3 — Factory Flashing pipeline
- **P0** · 👤 · 🟡 · → [`03_06 §5`](03_06_Factory_Flashing_and_Key_Provisioning)
- **Стан:** Гілка A+B Rake-tool канонізовано — `provisioning_sessions` AASM + 2-Person Rule (operator≠supervisor + **supervisor-password-auth на `approve`** via `approve_with_credentials!`/Argon2id, SEC.3 — оператор сам не схвалить) + `factory_flashing/*` + rake `factory:flash|approve|execute` (dry-run); execute-шлях інтеграційно доведено шимом (fake `STM32_Programmer_CLI` → реальні subprocess'и: stdout/stderr/exit capture, stop-on-fail, AASM failed+transcript). Конвеєр [`03_06 §1`](03_06_Factory_Flashing_and_Key_Provisioning); bench-residual = фізичний SWD-флеш. **2026-06-15: console-bypass закрито кодом** — guard `credentials_verified?` на переході `approve` (true лише в `approve_with_credentials!` після Argon2-автентифікації) → сирий `approve!` з Rails-console відмовляється (`AASM::InvalidTransition`); RSpec-покрито.
