...
- [ ] 🤖 **cache `iotex_seed`/`K_ota`** — PROVISIONING-деривовані ключі з telemetry-hot-path (кожен uplink re-derive'ить через HKDF → зайвий crown-jewel-touch; кеш у-процесі як `HardwareKey#cached_binary_key`)
