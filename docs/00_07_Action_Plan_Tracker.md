...
- [x] 🔗 B ✅ **ВИРІШЕНО+SHIPPED wire-rev2.1 (founder 2026-07-03, друге читання — переглянуто V3+ «на фліп-дні»):** +2B EMA-поле (30B кадр, airtime-задарма — той самий символьний блок) за контрактом «wire = вхід GP» → `Attractor.expected_homeostasis_gp(ema)` recompute stateless байт-точно; гілка `check_metabolic_divergence!` **observational до bench-калібрування** (V3+-принцип «не зашивати парність поперед фізики» збережено як gate-перемикач). Ухвала + чому переглянуто = ledger [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security); механіка — [`03_01 §13.6`](03_01_Firmware_Lifecycle_and_DMA)
- [ ] 🤖 після bench-калібрування порогів (§вище): увімкнути строгість точної metabolic-гілки (observational → gate) — один перемикач, разом з калібруванням `DELTA_T_FAST_S`/`DELTA_T_SLOW_S`
