...
- **✅ CI size-gate канонізовано ([`03_03 §4.3`](03_03_TinyML_Acoustic_Inference) + build [`03_01 §12.4`](03_01_Firmware_Lifecycle_and_DMA)):** `make size-check` (host `.bss+.data` < 51200 B; `check_ram_budget.sh` + ci.yml); FW.46 real `arm-none-eabi-size` owned-code (logmel ~6.3KB, mruby ~117KB Flash / 0 static RAM). Tensor arena оцінка ~16KB (Path B §3.2 ~15-30KB); >46KB → stack overflow при Lorenz. · [ ] 🤖 повний `.elf` `.bss+.data` після FW.4 модель + `-DSILKEN_WITH_HAL=ON` · [ ] 🤖 якщо >46KB → INT8 quantization/prune
...
- **✅ Done (канон [`03_01 §2.3`](03_01_Firmware_Lifecycle_and_DMA) / §2.3.1 / §2.3.2):** 300nA-режим вимикає SRAM2 retention → RAM-only стан гине (EMA/mesh-кеш виживають у RTC; delta_t wall-маркер реюзає DR1). Flash-KV host-first (`flash_kv.{h,c}`, power-cut тести) + RAM-state інвентар (§2.3.1) + RTC-headroom 3-осьова реклемація (§2.3.2: live-набір вміщується в RTC, Flash для нього не потрібен). Лишається:
  - [ ] 👤 рішення: RTC-реклемація (§2.3.2) vs Flash-KV persist vs SRAM2-retain.
  - [ ] 👤 bench: HAL_FLASH glue + ECCD-політика + вимір 300nA + persist-roundtrip.
  - **🔴 DID-інверсія (founder-finding, канон [`03_01 §2.3.2`](03_01_Firmware_Lifecycle_and_DMA) Вісь 2):** `tree_did` write-once у дефіцитному `DR7`, self-gen UID⊕random → не VBAT-durable (розряд EDLC сиротить identity/гаманець) + не відтворюваний (хоч K_seed дериується з DID). Фікс (DID→Flash / DID=f(UID)) звільняє DR7 + робить DID VBAT-durable. **👤 рішення: source-of-truth DID.** Зчеплення: SEC.11/FW.30, SEC.3, HW.14.
...
- **P1** · 👤+🤖 · → `05_02`
- **Присуд:** FW.5 β-пертурбація НЕробоча — `delta_t` економічно нульовий + `vcap` інвертований; корінь — β не рухає z-нерухому точку Лоренца (z_eq=ρ−1 від ρ/temp). Повний присуд + paired-ensemble докази + L4-honesty (лаб-стеля ~140µW ↔ польова ~15µW) канонізовані: [`03_04 §4.3`](03_04_mruby_Lorenz_Attractor) (verdict/variance) · [`01_03`](01_03_EBFC_Enzymatic_Bio_Fuel_Cell) (L4 recharge-model) · [`02_03 §9.8`](02_03_BQ25570_MPPT_Nano_Power) (енергобюджет).
...
- **Присуд (E.63-лінза):** окрім delta_t (E.63 ✅), решта bio→economy слабка — **#2** Z→bio_status degenerate+temp-confounded (`stress` z<2 недосяжний за ρ-clamp; `anomaly` тригерилась ambient-temp → хибно обнуляла growth_points), **#3** `stress_index` confound-domination (temp-anomaly текла у фінансовий 1.0). **Throughline:** Лоренц як health-оракул **декоративний** — реальна цінність = DCI anti-fraud (device-Z≡server-Z); здоров'я мають вести ПРЯМІ сигнали (metabolism ✅ E.63 · sap · VPD · acoustic). Нічого не задеплоєно → correctness перед деплоєм, не пожежа. Повний присуд + фікси канонізовані: [`03_04 §4`](03_04_mruby_Lorenz_Attractor) (anomaly ρ-relative) · [`05_05 §7`](05_05_Slashing_and_Risk_Policy) (Z-alone-never-slash) · [`05_05 §8`](05_05_Slashing_and_Risk_Policy) (de-risk/ground-truth).
- [x] ✅ 🤖 **status-fix DONE (offset-обвідна, 2026-06-08):** anomaly = `z > ρ + (CRITICAL_Z_MAX−BASE_RHO)` (offset 17 → 45 при ρ=28). firmware `bio_contract.rb` + backend `Attractor.anomaly_ceiling`/`homeostatic?(…,temp)` + `check_z_divergence!`/`try_time_sync_recovery` + alert_dispatch + тести (firmware 39/39, backend 189/0). Warm-day false-anomaly 22%→3% (probe). stress (z<2 absolute) лишено — справжній колапс.
- [x] ✅ 🤖 **stress_index conformance DONE (2026-06-08):** ([`05_05 §7`](05_05_Slashing_and_Risk_Policy)) `max_status≥2→1.0` звужено до `≥3` (лише tamper/VM-error=1.0; Z-anomaly(2)→0.6 base, **<0.83 → сама не слешить**, потребує sap/acoustic corroboration); прибрано degenerate `avg_z>2→+0.2` (завжди-on) + погодний `temp→+0.1`; sap/acoustic max()-corroboration лишено (inert). `InsightGeneratorService` + specs; повна сюїта **6872/0**.
...
| DOC-T.15 | **Volatile `main.c:NNN` line-refs** (~50) — той volatile-line-ref drift, який кампанія уникає: `main.c` виріс до 2522 рядків → кілька вказують на ХИБНИЙ зміст (FW.4-трійка цілила в `DEFAULT_TTL`/CoAP-коментар/`CMD_THRESHOLDS` замість Run_Inference → виправлено 2026-06-09 на symbol/Phase-рефи). Mixed: `03_02` const-table «defined-at» колонка (#define-блок ~зверху, ~ок), historical-labeled (легіт), частина current | `docs/**` (03_02/02_05/03_05/05_02/03_03) + `.github/copilot-instructions.md` | de-line-ref пас: per-ref stale→`#define`/symbol-name (стабільно), historical лишити; опційно guard `main.c:\d+` у docs/.github | 🟡 Deferred (founder-flagged 2026-06-09) |
...
| DOC-T.1 | AES master-key doc contradiction («навмисно не публікується» vs «перші 4 слова = FIPS-197 Appendix B») — РЕЗОЛВНУТО (re-audit 2026-06-09): §3.1 тепер когерентний (FW.1 hardcoded-key removed → per-device HKDF + Protected Flash) + §3.1а WeakKeyDetector boot-guard. Оригінальна дія «видалити test-vector згадку» **суперседнута**: історична FIPS-197-нота лишена СВІДОМО як stop-and-escalate сигнал для аудиторів pre-FW.1 прошивки. Doc-side key-agnostic → НЕ залежав від SEC.9 key-replace (хибний «blocked»); stale line-refs (531-538=panic-map, main.c:66-67=removed) знято | `03_05 §3.1`, §3.1а |
