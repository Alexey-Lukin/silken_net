...
#### FW.60 — Queen inbound CoAP RX (Rails→Queen downlink) — wire-up ✅, bench-residual
- **P1** · 👤 · 🟢 · → [`03_02 §4а`](03_02_Queen_Gateway_Firmware), [`04_02`](04_02_Business_Logic_and_Services)
- **Стан:** ⚖️✅ **poll-після-флашу (founder 2026-07-12) → wire-up SHIPPED того ж дня обабіч.** Механіка-дім [`03_02 §4а`](03_02_Queen_Gateway_Firmware) (poll-цикл + OTA-fetch) і [`04_02`](04_02_Business_Logic_and_Services) `Downlink::PendingQueueService` (derivation-черга CMD > 0x9E(gated) > OTA-hint > time-only; спостережене підтвердження `?fw=`; MID-кеш CoapGate). **Дельти від першого читання (обидві — знахідки нори):** (1) «CMD ≤60 Б через CCOAP-ногу» фальсифіковано математикою (CMD-конверт ≥80 Б > NMI-стеля ~68 Б при `AT_LINE_MAX=160`, обрізаний hex = втрачена сирена) → **увесь poll їде однією CA\*-ногою** (`sim7070_udp.h`; PDU-примітиви ноги-1 збудовані — вони транспорт-агностичні); (2) canary SEC.20 несумісний із чистою hiwater-derivation → **`gateways.pending_firmware_id`** (таргет пише dispatcher атомарно з burn). Кампанію анонсує hint `[0x9F][fw_id:4][total:2]` (opcode-map [`03_01 §4.5а`](03_01_Firmware_Lifecycle_and_DMA)); push-воркери (Ota/ActuatorCommand) **superseded** — deprecated-банери, `dispatch_to_edge!` без enqueue (його швидкі ретраї в CGNAT-діру `fail!`'или команду ДО першого poll'а — гонка вбита); 16×512=8КБ-гейт у dispatcher ДО burn; `sidekiq_retries_exhausted` TimeSync/KeyRotation. Канон-лікування за картою + git-grep'ом ✅ (03_01/03_02/03_04/03_03/03_05/04_01/04_02/04_03/04_04/06_03). Крипто незмінне: конверт `[IV:16][AES-256-CBC KEYC]`, no-MAC стеля = вісь FW.17-гейт (i).
- [x] ⚖️ транспорт-рішення inbound RX — ✅ poll-після-флашу
- [x] 🤖 Rails-half: `poll/<uid>` + `ota/<uid>` chunk-server у CoapGate/CoapServerPdu + `Downlink::PendingQueueService` + 8КБ-гейт + retries_exhausted + спеки (e2e deploy→hint→chunks→confirmation; MID-ретрансміт; мутація-verify)
- [x] 🤖 firmware: `Coap_Build_Get`/`Coap_Reply_Extract_Payload` + `At_Read_N` + `Sim7070_Udp_Fetch` (CA\*-оркестратор) + `Queen_Poll_Downlink` = перший call-site `Handle_CoAP_Command` (void→int; 0x9F-hint гілка) + 7 host-тестів (golden GET = freeze-contract із `coap_server_pdu_spec`)
- [x] 🤖 канон-лікування (карта + семантичний grep push-мови — 4 позакартні місця, вкл. осиротілу метрику `ota_chunks_sent_total` → оживлено в chunk-server'і)
