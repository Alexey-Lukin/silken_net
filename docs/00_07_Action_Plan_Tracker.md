...
- **P2** · 🤖 · → `00_06 §3`
- 360 round 2 ✅ (commits 977de1f+e21e49f, wiki 50p): thread A whole-doc refs→links + `bare_doc_ref` HARD guard; thread B 03_02 fence-bug + `DocsToc.FRONT` engine-bug (6 dropped ToC sections) + wrong doc-ids + stale §s; thread C `tracker:check` §-validation + 12 stale §BLOCKER-N refs.
- **Round 3 ✅ (2026-05-31):** · [x] 🤖 `docs:graph` ref-граф — `lib/docs_graph.rb` + `rake docs:graph` + 10-ex spec: 50 nodes/535 links, **0 orphans · 1 dead-end (02_06 appendix) · §-refs ✓**; #anchor-валідація (нова, `gh_anchor` зберігає subscripts ₀) знайшла **8 stale → 5 fixed** (`scripts/fix_stale_anchors.rb`: 00_08/03_02/03_03/05_02/08_01) + 1 FP усунено тул-фіксом, **2 deferred (#7)**; on-demand audit, НЕ CI-gate → [`00_06 §3`](00_06_SSOT_Documentation_Standard) · [x] 🤖 value-consistency: 20% slash-trigger / 0.83 stress / 300nA / 15µW / member-TRL — **усі consistent, 0 drift** (slash home [`05_05 §3`](05_05_Slashing_and_Risk_Policy) + 05_06 params; 300nA/15µW home [`02_03 §9`](02_03_BQ25570_MPPT_Nano_Power)) · [x] 🤖 3 мапінги: FW.18b→`03_03 §5` ✓, HW.12→`02_03 §4` ✓; **HW.11 дім = `02_02 §3.4`** (conformal-coating канон), НЕ `02_01` (live-запис уже містить) · також fix `scripts/doc_structure_map.rb` (fence-aware section count).
- **Round-4 backlog:** **#7 dangling (×2)** `08_01:129/407 → 08_02#8-міжуніверситетська-синергія-чдту--чну-фотіус` — §8 розчинено в Taxonomy-P4 (08_02 = §0-6); repoint label+href (реком. → 08_02 §2 ЧДТУ; founder-confirm де канон cross-ВНЗ синергії) · 02_06 dead-end → backlink на 02_03? · 03_05 H2 «10. PQC» але підрозділи «11.x» — renumber heading (owner) · anchor-check як HARD-gate в `docs:check_refs`? (зараз on-demand) · 00_03 §1 «Апаратна частина (HW)=4» vs Module-02 capsule member-TRL 5-6 — anchor-gated system aggregate (CLAUDE.md: capsule 6 / anchor-EBFC 4); clarify row? · ⚠️ ID-колізія `DOC.12` (table-row §DOC = Taxonomy P4 ✅) — dedupe DOC-ID (next free `DOC.13`) наступним pass
