...
- **Staged-план:**
  ...
  - [ ] 🤖+👤 **S1-wiring:** `Wall_Seconds_Now()` + міграція delta_t/FW.20-S2/FW.27-B/current-UTC tick→wall + **три окремі часові опори** (boot/beacon/request markers — `Silken_Wall_Elapsed_Seconds`, бо `Wall_Seconds_Now` АБСОЛЮТНИЙ, не «від boot») + дзеркала. ⚠️ **bench-gated:** LSE/RTC clock-tree у repo ВІДСУТНІЙ (`SystemClock_Config` без LSE; `MX_RTC_Init` без тіла).
  - [ ] 🔗 **S2:** RTC-WUT-tick + Vcap-енергогейт → delta_t = справжній час перезаряду (чекає на резолюцію шкали ↓).
  - [x] ✅ 🤖 **S3 docs (2026-06-08):** One-Home wake-source [`03_01 §1.10`](03_01_Firmware_Lifecycle_and_DMA) = дім (📐 КАНОН-блок + ADR + розрізнення порогів: VBAT_OK 3.4 В HW-buck ≠ Vcap-енергогейт 4.5 В FW); `02_03 §7` (retitle → «живлення-гейт», прибрано стале «GPIO interrupt → Wakeup from STOP2») / `02_04 §2` / `00_00` index → референси; «built-in 3%»→програмовані ROK (`02_03 §VBAT_OK`). 3-стороннє протиріччя «VBAT_OK будить MCU» знято (wake = RTC-WUT).
- **👤 bench bring-up:** (1) LSE (кварц 32.768 кГц + load-caps + drive) у `SystemClock_Config` + `RCC_RTCCLKSOURCE_LSE`; (2) `MX_RTC_Init`: календар (async 127 / sync 255 → 1 Гц) + WUT + WUT-IRQ як STOP2-wake; (3) верифікувати `Wall_Seconds_Now` + реальний інтервал перезаряду EDLC. Скриптований день: `firmware/scripts/bench/RUNBOOK.md` (drift-скрипт `04_lse_drift.py`, recharge-крива `03_power_profile.py`).
...
- [x] 🤖 (2026-06-07) **Flash-KV store імплементовано host-first** (`03_01 §2.3` шлях A: `firmware/common/flash_kv.{h,c}` — ECC-атомарний dw-елемент, ping-pong FINI-останнім, power-cut тести `test_flash_kv.c`; сторінки 122-123, wear-бюджет зчеплено з E.63) — софт-половина FW.54 знята · [x] 🤖 (2026-06-07) **інвентар RAM-стану + key→поле map** (`03_01 §2.3.1`: групи A RTC-resident / B ефемерне / C must-survive→ключі; live-набір {warn_esc, sync_wall, ota_silence}, K≈1 conservative, OTA-буфер = per-mode SRAM2 НЕ Flash) · [x] 🤖 (2026-06-07) **RTC-headroom аналіз** (`03_01 §2.3.2`: Flash для live-набору **не потрібен** — DR2 (1-біт flag)+`DR0[15:8]` вміщують його; DR14/DR19 пакуються; **DR7/DID** = найбільший виграш) · [ ] 👤 рішення: RTC-реклемація (§2.3.2) vs Flash-KV persist vs SRAM2-retain · [ ] 👤 bench: HAL_FLASH glue + ECCD-політика + вимір 300nA + persist-roundtrip
- **🔴 Окремий finding (DID-інверсія, surface to founder):** `tree_did` write-once сидить у дефіцитному RTC `DR7`, самогенерується з UID **⊕ true_random** (`main.c` §7) → (1) НЕ VBAT-durable: повний розряд EDLC (HW.14) сиротить identity/гаманець новим random-DID; (2) НЕ відтворюваний, хоча K_seed провіженіниться **з** DID (`SilkenNet::SeedDerivation` `info="silken-lorenz-seed|<DID>"`). Фікс (DID→Protected Flash write-once, АБО детермінований DID=f(UID) без random) заразом **звільняє DR7** + робить DID VBAT-durable. 👤 рішення: source-of-truth DID. Канон — `03_01 §2.3.2` Вісь 2. Зчеплення: SEC.11/FW.30 (K_seed), SEC.3 (provisioning), HW.14 (розряд).
