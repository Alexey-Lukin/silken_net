...
- **P0** · 👤 · 🟢 · → `03_05 §2.1`
- **Стан:** AES-128-CCM 28B wire-rev2 — дизайн + backend-парсер (`process_ccm_chunk` / `Cryptography::LoraCcm`) + firmware freeze-contract + TRL-7 FC Flash high-water host-готові, KAT-parity з OpenSSL. **Pre-authoring інтеграції ✅ (2026-07-03):** (1) **WL-API знахідка** — `HAL_CRYPEx_AESCCM_Encrypt/Decrypt` у WL-HAL НЕ існують (клас `__HAL_RCC_CRYP_*`); контракт переписано на істинний двофазний флоу (📐 механіка/shape — [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security) + `lora_ccm.h`), мок валідує B0, CI-варіант `hal_check_ccm` компілює гейтований тракт проти залізного HAL щопушу (локальний ARM-proof пройдено). (2) **Обидва call-sites зашито** за гейтами: Soldier Фаза-4 TX + panic-CCM (SEC.10 DR0-лічильник гейтовано геть) + ungated RX-guard проти сміттєвого ECB-декрипту чужих 28B; Queen = **сліпий кур'єр** (`rx_route.h`; `Queen_Parse` = bench-артефакт, НЕ hot path). RAM-ціна фліпа виміряна ARM — леджер [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security). **INERT** за `FW2_CCM_ENABLED` / `TELEMETRY_CCM_ENABLED` (ECB живий у проді, бойовий .bss незмінний). FC/nonce/cold-boot (📐 ЄДИНЕ ДЖЕРЕЛО) + 28B wire + budget-ledger + **flip-checklist (вкл. три свідомі фліп-гейти: atomic-cutover · health-blackout · key-model)** — канон [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security). Закриває ECB→CCM/MIC/replay + SEC.10 panic + FW.29; LoRa-ключ у SE050 (SEC.6).
...
- [x] 👤 ✅ **обидва фліп-гейти рішень УХВАЛЕНО (founder, 2026-07-03; формулювання — flip-checklist [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security)):** (а) atomic-cutover як зашито (одиниця = кластер; фліп до першого field-deploy; wire-видимість = QATT-флаги; ціна = star-only, Сценарій Б гейтовано) · (б) знято ARCH.54 · (в) **ДВОКЛЮЧОВА модель** (session KEYL per-device + cluster control-plane KEYB — 📐 [`03_05 §3.1`](03_05_Hardware_Symmetric_Crypto_and_Security)) — імплементовано за FW2-гейтом обабіч + фабрика (`derive_broadcast_key`, KEYB-блок, Gateway KEYL-фікс цегли) + host-тести; замінило план «спільне значення до ARCH.43»
- [ ] 🔗 при спільному фліпі з ARCH.35: ring-запис 21B → 29B fmt-aware (`FLASH_RING_RECORD_SIZE`; позначено в коді Королеви)
...
- [ ] 👤 Bitwarden Secrets API live (`BitwardenAdapter` зараз `NotImplementedError`; DI-pre-req ✅ знято 2026-07-02 — див. Стан)
...
- [ ] 🔗 B (на FW.2, **рішення V3+ founder 2026-07-03**) — точний stateless GP↔delta_t recompute вирішується НА ФЛІП-ДНІ разом з bench recharge-кривою (фізика-поперед-математики: пороги placeholder, сигнал може бути плоский; точний DCI = parity-integrity, не anti-fraud). Вилка + кандидати + обґрунтування = wire-budget ledger [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security) (гейт (г) фліп-чекліста); механіка розриву — [`03_01 §13.6`](03_01_Firmware_Lifecycle_and_DMA)
