...
| **FW.46 board-freeze** `.ioc` (👤) | повний `.elf` → увесь bench-день: `FW.2`-фліп · `FW.3/8/17/20/23/31/49/50/52/54/55` · `ARCH.26/41` · `SEC.2`-OTA-verify · `SEC.15`-WUT · `SEC.3`-SWD · `ARCH.35`-розводка · `radio_conf.h` → компіляція `radio.c` (SubGHz submodule + `RadioEvents_t` уже в репо — останній radio-гейт); суміжно `ARCH.34`-ефір: TX-half ✅ (glue+adapter+host-smoke+форк v2.6.2-silken.1); вільний 🤖 = mock-LNS JoinAccept + main.c KV-bind, OTAA-ефір далі на bench (дім ARCH.34) |
| **Перший live-деплой** = акаунти/значення Фази −1 + секрети (`S1.1`+`S4.3`) + `OPS.11`-FinOps-значення (tf/workflow ✅ 07-05) → deploy фазами [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) (👤; SSH-нога wired — INF.20 (в)) | верифікації `INF.11/12/13/14/15/16` · `INF.17`-coap.env · `S1.5`-IP · `INF.20`-IAP-вхід · `PUMA-IPV6-1` · `INF.10`-фліп · `S5.2` · `S3.2`-staging · `S6.1`-Upstash · `INF.21`-pin · Grafana-сесія (`S2.1→S2.2` + `S2.4`-SLO + `FW.18b`) · активує escrow-watch (repo Variable `AKASH_OWNER_ADDRESS` — workflow ✅) |
