...
- **Стан:** Рішення (founder) = **true-DePIN** («голос дерева» — non-extractable Ed25519, що не підробить ні backend, ні оператор) → SE: ATECC608B (P-256, не тримає голос жодного ланцюга) → **SE050** (суперсет: Ed25519/EdDSA + AES-128/256 + monotonic counters + secure storage). AES-128 LoRa = свідомий вибір (не SE-constraint); ladder L0→L1→L2 (energy-gated); soft-freeze (footprint DNP, populate на mass post-FW.2). **Host/doc-side migration done** (ADR + Slot-1→Ed25519 + AES-128 One-Home-collapse + legacy-banner + true-DePIN honesty-pass: pipeline-integrity ⟂ physical-origin + trust-origin ladder дім); код / eval-kit / L1-L2 — residuals нижче. Ціна (DigiKey-зріз: $4.50/1 → $2.50/1k) — founder: не проблема. **Paper-half datasheet-verify ✅ + HIL L1 e2e ✅ (2026-07-02)** — деталі [`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security) datasheet-verify таблиця. Канон [`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security) + ladder [`05_02`](05_02_Proof_of_Growth_Pipeline).
...
- [ ] 👤 SEC.6 hardware (datasheet-half ✅ 2026-07-02 — paper-verify канонізовано [`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security): Ed25519 = конфіг C/E, F-FIPS відпадає · CCM on-chip НЕМАЄ · DPD 3–5 µA, «150 нА» був ATECC-проксі): замовити **OM-SE050ARD-E** (+опц. OM-SE050ARD; НЕ -F) + silicon-confirm paper-чисел (**cold-boot заряд = головне питання** — датащит не специфікує) + SEC.14 role + mass-BOM populate **разом з load-switch гейтом SE** (TPS22860; DPD > Scenario-C margin → повне зняття живлення)
