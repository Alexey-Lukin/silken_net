...
| **FW.60 Queen-inbound транспорт** (⚖️ рішення → 🤖 wire-up) | фізична доставка ВСЬОГО downlink Rails→Queen: `SEC.20`-OTA-кампанії · `FW.20` 0x9C-лег · `FW.17` 0x9E-ratchet · ActuatorCommand `CMD:*` · `ARCH.71`-TinyML · `ARCH.59` fw_pending-consumer · 8КБ↔Rails-ліміт узгодження + канон-лікування 10 дрейф-місць (карта в item) |
...
- **P1** · 🤖+⚖️ · ⚪ · → [`03_02 §5`](03_02_Queen_Gateway_Firmware), [`03_01 §4.6`](03_01_Firmware_Lifecycle_and_DMA)
- **Стан:** Queen не має ЖОДНОГО коду, здатного прийняти вхідний CoAP PUT: `Handle_CoAP_Command` визначена, але 0 call-sites (verified — розвідка SEC.20 Rails-half 2026-07-12); CoAP-сервера нема (SIM7070 AT-розмова суто клієнт-сесійна `CCOAPNEW→CCOAPSEND→CCOAPNMI`, server-mode коду не існує); `gateway.ip_address` = спостережений source-IP останнього uplink (CGNAT/Starlink egress, nil до першої телеметрії) — вхідну сесію на нього НЕ відкрити. Наслідок: **весь downlink Rails→Queen** (OTA-чанки 0x99/0x9B, TimeSync 0x9C, ratchet 0x9E, CMD-актуатори) фізично не доставляється — воркери шлють у порожнечу, fail-шляхи чесно валять gateway у `:faulty`. Діра **архітектурна, НЕ апаратна** — board-freeze сам по собі не розблоковує (FW.46-рядок мовчазно переобіцяв). Кандидати транспорту: SIM7070 UDP-server mode, MQTT-SN, CoAP-over-TCP ([`02_05`](02_05_Queen_Hardware_and_Starlink) D2C-нотатка), **uplink-piggyback poll** («Queen питає pending-downlink при флаші» — найдешевший: реюзить живий клієнт-тракт, NAT-friendly). **Той самий RX-контракт:** Queen OTA-збірка = 16×512 = 8КБ стеля (`OTA_MAX_CHUNKS`, `pending_ota_bytecode`) vs Rails-ліміти 256КБ (`MAX_BYTECODE_SIZE`) / 20МБ (controller upload) — чанки ch≥16 мовчки дропаються; узгодити тим самим рішенням. **Карта канон-дрейфу** (описують unwired-тракт як живий; лікувати РАЗОМ із wire-up — банер сьогодні лише в 03_02 §5): `03_01 §4.6` · `03_02 §5`-таблиця + `§5а` TimeSync-hub (✅ на Rails→Queen лезі) + `§4.5а` opcode-map · `03_05 §4.3`/`§5`-діаграма/`§6`-таблиця/`§7` · `04_02` ARCH.41-рядок + ActuatorCommandWorker/OtaTransmissionWorker Side-Effects · `04_03 §5.6/§5.7` (202 = enqueue-правда, не доставка). Rails-half продюсера полагоджено (SEC.20 ✅ 2026-07-12) — тракт готовий, «чекає вухо».
- [ ] ⚖️ транспорт-рішення inbound RX (server-mode vs piggyback-poll vs MQTT-SN; енерго/airtime-бюджет)
- [ ] 🤖 wire-up обраного транспорту + host-тести + канон-лікування дрейф-місць за картою ↑ + 8КБ↔Rails-ліміт узгодження
