...
- [ ] 👤 завендорити MAC-стек (1 команда): `git submodule add https://github.com/STMicroelectronics/stm32-mw-lorawan firmware/extern/stm32-mw-lorawan && cd firmware/extern/stm32-mw-lorawan && git checkout v2.6.2` (BSD-3, ST-форк LoRaMac-node, той самий Radio_s API `radio.h`, що вже реєструють обидва main.c; pin-конвенція FW.47 → [`03_01 §12.5`](03_01_Firmware_Lifecycle_and_DMA))
- [ ] 🤖 adapter-TU `Helium_Mac_SendSos` (OTAA join + MCPS unconfirmed + канальний hop + FCntUp через LoRaMac NVM → Queen flash_kv, ключі 0x30+ зарезервовано) — після submodule; повний link/ефір — bench разом з FW.46 board-freeze
