...
- **Стан:** Закрито (2026-06-28) — увесь Polygon base-EOA флот серіалізовано. `Chainlink::OracleDispatchService` (гарячий per-uplink), `PuroEarth::PassportService`, `Etherisc::ClaimService` тепер обгортають `transact` у спільний `Kredis.lock("lock:web3:oracle:#{addr}", expires_in: 30.seconds, after_timeout: :raise)` — **той самий** лок, що mint/burn (спільний nonce-простір base-EOA; eth-gem бере nonce per-call → без локу конкурентні підписи колізять nonce → orphan «sent-but-never-mined» tx). `LockTimeout` re-raise: Etherisc пробросить сам (нема StandardError-перехопу), Chainlink/Puro — explicit перед `StandardError` (інакше lock-не-взято хибно став би `Dispatch`/`AnchoringError` замість чистого retry). **Chain-prefix (#2) свідомо НЕ зроблено** (challenge, verify-by-data): Celo вже зняв contention двічі (prefix `celo:` + dedicated `ORACLE_CELO_PRIVATE_KEY`, ARCH.50), base-EOA = de-facto чисто Polygon (Celo пішов; Toucan/Klima dead) → `polygon:`-prefix вимагав би чіпати й mint/burn lock-key (інакше Polygon-флот розколовся б на дві lock-групи, що не серіалізуються) — money-path-ризик без виграшу. **Toucan/Klima** той самий патерн, але DEAD (0 enqueue, верифіковано) → lock при активації (Toucan → E.66; Klima dead). Specs: 3 lock-serialization + `LockTimeout` re-raise (дзеркало Celo). Канон [`05_01`](05_01_Multichain_Architecture) (Chainlink card) · [`04_02`](04_02_Business_Logic_and_Services) (3 service cards) · [`06_02`](06_02_Akash_Network_Integration) (key-topology mitigation).
