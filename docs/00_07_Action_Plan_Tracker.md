...
> 🧭 **Анкерний кущ — synergy map (dependency-DAG, 2026-06-20; v2 graded + ARCH.25 connectivity 2026-06-21).** Анкер-айтеми зшиті в один граф — виконувати **пакетами, не поштучно**. **[A] Дим-фриз** (HW.33 orient✓/Ø11✓ · HW.8 Ø фланця+пад · HW.26 barb/ring/hex · +PEEK/hole-chain) **→ [B] PicoGK CAD-родина** coin✅/anode-v2✅/**ARCH.25 connectivity-валідація✅**/radome+barbs-pending (`tools/cad` — **ЛІНЧПІН**, живить кожну стадію валідації; coin+v1+v2-graded + ARCH.25 двофазний аудит shipped — in-silico прокси Архімед/µCT/AM-island/EAAE-гейтів; radome+barbs наступні) **→ [D] Staged validation** (HW.24 SLA→coin→anchor→100 · HW.3 aging; ARCH.25 connectivity = машинний передвісник фіз-гейтів). **[C] Завод-пакет** (HW.23 HIP · HW.27 bake · HW.2 EAAE · coating-map `01_02 §3.6`) ∥ B. **[E] Збірка** (HW.17 radome · HW.25 PTFE-GDL · HW.28 shield · HW.8 pogo · HW.6 install · HW.22 steril) — після фіз-деталей. Хімія HW.5 → coin/anchor. Аудит-знахідки + founder-рішення → **HW.33**.

#### HW.1 — nTop model → SLM+HIP factory (Anode Zone 1)
- **P0** · 👤 · ⚪ · → `01_01`, `01_02 §1.7`
- **Стан:** Фабрична генерація тризонного анкера НЕ розпочата (Zone 1 SLM+HIP, Zone 3 SLM/EBM, Zone 2 CNC+annealing 200–250°C). Канон `01_01 §1/§5.5`, `01_02 §1.6/§1.7/§3.6`. **Code-as-CAD трек PicoGK** (`tools/cad/`, .NET 9, PicoGK 2.2 — primary; nTop-ліцензія є, але в nTop ~нуль): scaffold + Ti-coin + **анкер v1/v2-graded** + **ARCH.25 двофазна connectivity-валідація** SHIPPED 2026-06-20/21 (пайплайн end-to-end; код `60723a8`+`a5ed3d9`, усе в main); barbs/radome/cathode-flange pending (нижче). Геометрія-аудит + sheet→network-схиляння → **HW.33**.
...
- [ ] 🤖 HW.1.PicoGK: **barbs PicoGK (v2b, окрема сесія)** — annular h=0.25–0.4мм (`01_01 §4.3` A), реле HW.26; катод-фланець/PEEK-Radome — після HW.33 dim-freeze
