...
- [ ] 👤 IaC-scan baseline-triage (post-1st-CI-run) — `.trivyignore` by-design findings з точними AVD-ID (0.0.0.0/0 web+CoAP firewall `vpc.tf` · Cloud SQL `ipv4_enabled` `database.tf` — усі by-design, mitigated: empty authorized_networks/ENCRYPTED_ONLY/iptables-hashlimit); провіжн GCS state-bucket (S5.6) РАЗОМ з `GCP_SA_KEY` (інакше drift `init` red)
