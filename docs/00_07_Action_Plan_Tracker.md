...
- [ ] 🤖 (follow-on) Grafana-alert на `silkennet_hadron_kyc_pending_depth` (gauge живий, поріг калібрується з перших live-вікон — зараз нуль KYC-трафіку)
...
- [ ] 🤖 **[buildable]** anchor gas-gate `MAX_ANCHOR_GWEI` — guard у `EthereumAnchorWorker` (ENV unset → no-gate skip-clean; anchor толерує multi-week gap by design → при gas-spike відкласти тиждень, а не палити газ). Дім `06_08 §2.2` крок 12
- [ ] 🤖 **[buildable]** `FilecoinReconcileWorker` — daily-cron re-pin з `AuditLog` (джерело-правди у Postgres; `FilecoinArchiveWorker`+`FilecoinVerificationSweepWorker` вже є — це repair-половина поверх detect). Дім `06_08 §2.2` крок 11
