...
- **Стан:** 🔬 deep-dig завершено, recompute не розпочато. **Знахідка:** каскадне `E°(Os) +200 mV vs NHE` (→ driving force +465) — **недовизначений ранній якір** із `1a3460a` (Gen-2.0 baseline, у парі з тодішнім хибним FAD +60; FAD виправили→−265 Schachinger, Os ні). Реальний медіатор = `[Os(4,4'-dimethyl-2,2'-bipyridine)₂(PVI)Cl]` = **+309 mV vs NHE** — атестовано двічі: Zafar 2012 (`ac202647z`, "best-suited" для нашого GcGDH) + власний проєктний «+309 opt, Mano/Heller» (`SUMMARY` §Mediator). **Виправлений DF = +309 − (−265) = +574 mV** (більш downhill — було занижено, overclaim'у нема). ⚠️ value-spread: інші форми +191 (`[OsCl(Him)(dmbpy)₂]` His-мономер) / +307 (plain-bpy мономер) = інші комплекси/умови; +309 = наша PVI-система. Фінальний fact-check = Zafar 2012 PDF-таблиця 6 полімерів (paywalled, full-text не дістав).
- [ ] 🔗 **RH3 (найглибша, Мінаєв-grade) — ② speciation-новизна тріщить:** ядро статті (`L3` §E / `SUMMARY` §Cluster-Continuum) декомпозує ~1.35 eV gap як speciation (**chloro→aqua +0.51 «over half»** + bis-Im +0.30) + solvation (+0.20), з тезою «exp вимірює **аква/біс-ім**, не chloro». АЛЕ Zafar'ів +309 — `[Os(dmbpy)₂(PVI)Cl]` = **chloro-PVI форма** → теза підважена; gap росте (exp −0.47→−0.574); додається **substituent-компонент (plain→dimethyl ~0.14 eV)**, який ② не виділяє. Питання новизни — потребує наукового присуду (cross-ref CHEM.20/26).
- [ ] 🤖 **RH2 — DFT рахував PLAIN bpy, медіатор DIMETHYL:** headline `ΔSCF adiabatic +0.884 eV` (script `21d`) + Koopmans −0.909 (`21b`) = `[Os(bpy)₂(1-MeIm)Cl]` plain (X=H). Hammett-серія (`21e`) має X=Me лише на **Koopmans** (cascade Δ −1.051) — **dimethyl ΔSCF adiabatic НЕ існує**. → перерахувати ΔSCF adiabatic для X=Me.
- [ ] 🤖 **Recompute ТЗ (послідовність, ДО будь-якої правки доків):** (1) ΔSCF adiabatic dimethyl-bpy (розширити `21d`/`21e`); (2) speciation `34` (aqua/bis-Im/chloro) на **dimethyl** комплексі (зараз plain); (3) re-decompose gap vs exp +309/−0.574, **виділивши** substituent-внесок окремо від speciation+solvation; (4) перерахувати OCV (`01_03 §2.1`, +309 знижує OCV ~109 mV — консистентно з «+309=optimum» caveat); (5) **тоді** doc-sweep.
- [ ] 👤 **RH5/RH6 — sweep triage + naming (ПІСЛЯ recompute):** `+465`/`+200` у ~20 місцях (01_03 ×4, L3 ×6, SUMMARY ×3, paper Table 2/Fig 3/§3.5/conclusion, 08_01) — частина **історичні** (Cosnier +140 supersede, «−0.07 withdrawn») → **ручна тріажа, НЕ find-replace**. Назва комплексу `2,2'-bipyridine → 4,4'-dimethyl-2,2'-bipyridine` (01_03 §2.1 / L1 / L3 / paper). Заявити явно: обраний медіатор = OCV-оптимум (+309). Table 2 / `SUMMARY` — One-Home (правити SUMMARY + re-gen `61_paper_tables.py`, не hand-edit).
