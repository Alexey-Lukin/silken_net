...
| DOC-T.18 | **Tracker parser не моделює STAGE окремо** (post WHO+STAGE meta-sweep `9625297`): `lib/tracker/dashboard.rb` `EXECUTORS` досі мапить `🟡`→`:blocked` (а 🟡 тепер = in-progress STAGE) і не виділяє STAGE власною віссю. Gate GREEN (conformance вимагає лише WHO-наявність), але `render`/spec/guard STAGE не бачать → нема STAGE-conformance. | `lib/tracker/dashboard.rb`, `spec/lib/tracker/dashboard_spec.rb`, `00_06 §3` | прибрати `🟡` з `EXECUTORS` + `Item.stage` + parse STAGE з meta-line + conformance вимагає `WHO · STAGE` + новий STAGE-token guard | 🟡 Approved (2026-06-14) |
