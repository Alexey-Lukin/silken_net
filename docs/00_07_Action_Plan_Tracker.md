...
- [x] 🤖 (2026-06-07) **honesty-pass DONE** (true-DePIN наратив): `05_01` (DePIN-роль) / `04_02` (W3bstreamVerify card) / `05_03` (IoTeX row) / `06_08` — «доводить фізичний STM32» → розділено **pipeline-integrity + DID-binding (є, master-backed)** vs **physical-origin** (custodial-now / device-bound-roadmap ladder §3.7 / ЗВТ-metrology). `04_02` stale механізм виправлено (`binary_key`→`derive_iotex_seed` HKDF).
