...
- [ ] 👤 активувати kill-switch `:parametric_insurance_oracle_enabled` ЛИШЕ після закриття drought-latent (multi-peril Trigger-2 ↑) — інакше посуха піде хибним slash-шляхом
...
#### ARCH.49 — Nonce-race: Chainlink/PuroEarth/Etherisc підписують без Kredis-lock
- **P2** · 🤖 · ⚪ · → [`05_01`](05_01_Multichain_Architecture)
- **Стан:** Не усунуто (знахідка deep-trace ARCH.47) — `Chainlink::OracleDispatchService` (гарячий per-uplink шлях), `PuroEarth::PassportService`, `Etherisc::ClaimService` підписують EVM-tx на спільній `ORACLE_PRIVATE_KEY`-адресі БЕЗ `Kredis.lock("lock:web3:oracle:#{addr}")`, тоді як `BlockchainMintingService`/`BlockchainBurningService`/`Celo::CommunityRewardService` цей лок мають. eth-gem бере nonce per-call (`eth_getTransactionCount(pending)`) → конкурентні підписи на одній адресі колізять nonce → orphan «sent-but-never-mined» tx. Toucan/Klima мають той патерн, але DEAD (нема enqueue-сайту; Toucan → E.66). Окремий механізм від ARCH.47 — живе навіть із розділеними mint/slash ключами. **Уточнення (orchestration-аудит):** ARCH.47 boot-guard дає ХИБНУ впевненість — перевіряє лише `minter==slasher`, не моделює unlocked-флот → чистий boot із розділеними ключами все одно лишає Chainlink+Etherisc+Puro у гонці за base-EOA; + lock-key **chain-agnostic** (`lock:web3:oracle:#{addr}` без chain) → Celo (Celo-chain) хибно контендить з Polygon-base, а реальна Polygon-колізія негейтнута. **[ARCH.50] Celo-плече закрито:** `Celo::CommunityRewardService` отримав chain-prefixed lock `lock:web3:celo:oracle:#{addr}` + dedicated `ORACLE_CELO_PRIVATE_KEY` (fallback на base) → Celo↔Polygon false-contention знято для Celo-плеча; залишок ARCH.49 = unlocked Polygon-флот (Chainlink/PuroEarth/Etherisc) на спільній base-EOA. Канон [`05_01`](05_01_Multichain_Architecture) / [`04_02`](04_02_Business_Logic_and_Services) / [`06_02`](06_02_Akash_Network_Integration).
- [ ] 🤖 обгорнути transact-блок Chainlink/PuroEarth/Etherisc у `Kredis.lock("lock:web3:oracle:#{addr}", expires_in: 30.seconds, after_timeout: :raise)` (дзеркало `Celo::CommunityRewardService`)
- [ ] 🤖 lock-key з chain-префіксом (`lock:web3:#{chain}:oracle:#{addr}`) — зняти Celo↔Polygon false-contention
- [ ] 🤖 spec: lock тримається під час transact; конкурентні виклики серіалізуються (no nonce collision)
- [ ] 🤖 canon-sync: service cards `05_01`/`04_02` + key-topology mitigation `06_02`

#### ARCH.51 — Money-path idempotency consistency sweep (3 ідіоми, 2 латентні double-money)
- **P2** · 🤖 · 🟡 · → [`05_02`](05_02_Proof_of_Growth_Pipeline)
- **Стан:** Не усунуто (orchestration-аудит) — money-path використовує ТРИ розбіжні idempotency-ідіоми без єдиного правила (slash `unsettled_within(2h)` · insurance/Solana-batch `unsettled_within(7d)` · Celo date-window · mint `:processing`-flip · anchor власний `in_flight(1wk)` · Toucan plain status), escalate-політика нерівна (slash/Solana-batch/Etherisc → `:manual_review`; mint/Celo → ні). Латентні: (1) **insurance internal-mint гілка** (`InsurancePayoutWorker` ~117-124) без `status_sent?`-guard, що є у Etherisc-сиблінга → recovered `:sent` tx переподається у `BlockchainMintingService` (виключає лише `:confirmed`) → **латентний double-mint ✅ ВИПРАВЛЕНО (ARCH.51)** — спот-верифіковано реальним (mint фліпає `:pending`→`:processing` у lock ДО broadcast → recovered `:sent` re-mint-ився); guard «re-submit лише `:pending`, інакше escalate» (дзеркало Etherisc, деталь [`04_02`](04_02_Business_Logic_and_Services)); (2) **Solana-per-event** дзеркалить слабкість Celo (dormant за batch-threshold>0). Cleanup: **`BlockchainTransaction.in_flight` — мертвий код** (визначено+задокументовано як «той» guard, ніде не викликається — усі живі guard'и `unsettled_within`; лише `EthereumAnchor.in_flight` живий) → Ruthless-Prune + поправити misleading model-коментар. **[ARCH.50] Celo-ідіому вирівняно:** Celo date-window → `reward_date` logical-key dedup + intent-before-broadcast + `CeloConfirmationWorker` reconcile → Celo переїхав у ARCH.45-родину (intent+reconcile) і з «не-escalate» у «escalate»-колонку (retries-exhausted → `escalate_to_review!`); sweep-залишок = insurance/Solana-per-event idiom-spread + dead `in_flight` cleanup. Канон [`05_02`](05_02_Proof_of_Growth_Pipeline) / [`04_02`](04_02_Business_Logic_and_Services).
- [ ] 🤖 прибрати dead `BlockchainTransaction.in_flight` scope + поправити model-коментар
- [ ] 🤖 (опц.) Solana-per-event: `:pending`-intent перед broadcast (як batch-шлях), або задокументувати dormant-by-design

#### ARCH.52 — Money-path scale-death @ 100B дерев: queue-starvation + partition-scan hot-paths
- **P2** · 🤖+👤 · ⚪ · → [`05_02`](05_02_Proof_of_Growth_Pipeline)
- **Стан:** Не усунуто (orchestration-аудит, лінза 100 млрд; НЕ горить зараз, б'є на planetary-обсязі) — два scale-ризики: (1) **Strict-queue starvation** (`config/sidekiq.yml` `:strict:true`, `uplink`(1) > `critical`-slash(3) > `web3_critical`-mint(6)): на 100 млрд телеметрія = вічний firehose у `uplink` → slash/insurance/mint/ConfirmationWorker **голодують**; clawback-race на slash. Іронія: YAML-коментар каже strict *запобігає* інверсії, а `uplink` сам і є той мільйон над `critical`. (2) **Partition-scan hot-paths** (RANGE-partition по `created_at`, запит без нього → full-scan): `BlockchainConfirmationWorker` по `tx_hash` через 30с після КОЖНОЇ tx (найгарячіший); `MintBatchCollectorService` `:pending` кожні 5хв; `BlockchainMintingService#initialize` `where(id:)`; slash `total_minted` sum; anchor sum; dashboard recent-events. **[ARCH.50 note]** новий `CeloConfirmationWorker` (per-tx reconcile, 30с-delayed, черга `web3`) вже дотримується partition-prune-дисципліни (передає `created_at_iso`) — НЕ додає full-scan, але це +1 per-tx reconcile-воркер у family під queue-starvation-ризиком (1). **[ARCH.12 note]** майбутній weekly Merkle над per-record телеметрією = ще один partition-scan hot-path (запит за вікно МУСИТЬ нести `created_at` bound) + один плоский Merkle над усією телеметрією на planetary-scale не feasible → ієрархічний (cluster-subtree→root) обов'язковий. Канон [`06_08`](06_08_Resilience_and_Failover_Policy) / [`05_02`](05_02_Proof_of_Growth_Pipeline).
- [ ] 🤖+👤 queue-starvation: архітектурний вибір — weighted-черги vs виділений money-path процес (поза uplink-strict-ланцюгом)
- [ ] 🤖 partition-prune: прокинути `created_at`/`created_at_iso` у ConfirmationWorker / MintBatchCollectorService / BlockchainMintingService#initialize / slash+anchor sum / dashboard (як `find_with_partition_pruning`)
- [ ] 🤖 spec/bench: EXPLAIN partition-elimination на hot-path запитах
...
- [ ] 🤖 Фаза 1 (з першим споживачем): `MerkleTree` primitive (sha256, RFC-6962 domain-sep, ієрархічний, golden-vector) + застосування до L1 state-anchor та E.60 `archive_root`
- [ ] 🤖 озброїти dead E.60 `VerificationService` (cron / on-read verify; зараз 0 prod-callerів — guard не перевіряє намінтоване)
