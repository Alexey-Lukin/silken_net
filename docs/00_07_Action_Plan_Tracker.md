...
- **Стан:** Аудит 2026-07-04 (O3-MUST): web/job/coap у static SDL + `docker_image` tf-var default + `coap-daemon` systemd-юніт на анкорі (`compute.tf`) — усі на `ghcr.io/…:latest`, який `mirror-ghcr` перезаписує кожним push у main → будь-який container-restart / provider-міграція / VM-reboot підтягне ІНШИЙ образ, ніж тестований, і немає rollback-цілі (мутабельний тег не відкотиш). Контраст: alloy запінено `v1.16.3` (INF.14), base-образ digest-pinned. Mirror вже пушить іммутабельні `sha-<commit>`-теги + semver на release — лишилося споживати їх; machine-half ✅ 2026-07-04 (анкор-образ параметризовано tf-var'ом `coap_daemon_image`, обидва tfvars.example несуть sha-pin приклад; ⚠️-нотатка біля `:latest`-прикладу — [`06_02 §1.1`](06_02_Akash_Network_Integration)). Суміжне: Cloud SQL proxy `ADD` у Dockerfile — version-pinned URL без `--checksum` (та сама пін-непослідовність, дрібно).
