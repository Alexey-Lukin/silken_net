...
- [ ] 🤖 **[buildable]** Rack::Attack throttle → MemoryStore — поки web `count:1` (Redis-store = крос-регіон RTT на кожен запит; O4-нотатка). Value з першим публічним трафіком
...
- [ ] 🔗 **[PATH-1 latent]** `IotexBackfillWorker`-cron — PATH 2-мінт IoTeX НЕ гейтиться (ARCH.53), recovery поки = ручний re-enqueue; будувати при замиканні PATH 1. Дім `06_08 §2.2` крок 5
