...
- **P2** · 🤖 · → `04_06 §B.1`
- Baseline (full `bin/rspec`, 0 fail): line ~97.1% / branch ~91.4%, рухається до 99%. App/lib залишок — переважно 1-line edge defaults у Phlex-індексах та сервісах (`&.`-nil + fragment-cache блоки), плюс dev-tools `lib/tracker/dashboard.rb`, `lib/hil/lorenz_generator.rb`, `lib/github_bootstrap.rb`. Методологія `04_06 §B.2`. Пріоритет — реальні logic-гілки (status / empty-state / guard / error-path), НЕ defensive `&.`-nil. · [x] 🤖 batch (2026-06-02): `dashboard_layout` (rescue fail-open + `defined?(Codex::Fraction)`-guard) + `governance/parameter_sync_worker` (`system_bot` RecordNotFound fail-safe) + `dclimate/verification_service` (`extract_entries` []-fallback + nil-metadata ref) → усі три файли 100% L+B · [ ] 🤖 next top-gap (свіжий скан 2026-06-02 — фрагментовано, великих цілей нема: `web3/resilient_client`, `blockchain_minting_service`, `emergency_response_service`, `security/weak_key_detector`, `insurance_payout_worker`, codex/* views — по кілька branch-gaps кожен) · [ ] 🤖 gap-parser ПІСЛЯ повного `bin/rspec` (subset тригерить SimpleCov-артефакт, `04_06 §B.2`)
