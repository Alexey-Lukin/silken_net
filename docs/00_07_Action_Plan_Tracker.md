...
- **Стан:** Піднято з 📌 Backlog (2026-07-02). Рандеву-драбина канонізована [`03_01 §1.9`](03_01_Firmware_Lifecycle_and_DMA): L1 Queen always-on ✅ · L2 TDMA-вікна (beacon → RTC sync → координоване RX-вікно ~кожні 15 хв) ❌ · L3 CAD ❌. Передумови landed: роль Soldier/Provisioner ✅ ARCH.27 (`g_node_role` — [`03_01 §1.11`](03_01_Firmware_Lifecycle_and_DMA)); FW.20 beacon host-done і вже возить TDMA-резерв (байти 5..8 — [`03_02 §5а`](03_02_Queen_Gateway_Firmware)); gossip-точність ±128 с << потрібних ±10 мс → точний sync лише з beacon + LSE bench (FW.49). ⚠️ Беклог-енергоцифра «CAD щосекунди ≈9 µA» спростована канон-корекцією 2026-05-28 (повний wake-цикл = одиниці Дж/добу >> EBFC-харвест): CAD лише у TDMA-вікнах L2 / грубим інтервалом, миттєвий PANIC = extended-preamble відправника ([`03_01 §1.9`](03_01_Firmware_Lifecycle_and_DMA)). Розблоковує: FW.27 Design A ([`03_02 §5.1.4`](03_02_Queen_Gateway_Firmware)) · ARCH.43 (рандеву-передумова) · mesh-TTL rev2-рішення ([`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security)) · stigmergy-зворотний шлях ([`00_08 §1`](00_08_Beyond_TRL9_Planetary_Roadmap)). Активація post-TRL 6.
- [ ] 🤖 L2 host-half: Queen слот-розкладка у beacon TDMA-резерві (байти 5..8) + Soldier parse розкладу → RTC-заплановане RX-вікно + host-тести
- [ ] 🤖 L3 host-half: CAD-глю за role-прапорцем (gated, лише `ROLE_PROVISIONER`) + PANIC extended-preamble TX + переглянутий CAD-енергобюджет
- [ ] 👤 bench: LSE drift проти ±10 мс sync-бюджету + RTC sub-second/LPTIM wake + CAD енергопрофіль (PPK2) — спільний bench з FW.49/FW.20
