...
- **Перед польовим деплоєм** (life-safety + security): `SEC.9` · `SEC.3` · `SEC.1` · `FW.2`-фліп + `FW.17`/downlink-wire-rev (обидва ⚖️ **ДО першого field-deploy** [pin 07-11]: CCM-cutover + key-ratchet-активація — rollback до поля = хвилини, після = SWD-візити на статичний KEYL) · `FW.60`-bench (poll-тракт wire-up ✅ 07-12 — верифікувати живу розмову на модемі) · `ARCH.60` (notification-delivery — wildfire/chainsaw-алерт мусить дійти до людини: email/SMS/push сьогодні мертві)
...
| **Перший inclusion-proof-споживач** (🤖+👤; ⚖️-присуд ЩО ним стає [чекбокс `ARCH.12`]: ISO-звіт / API / UI) | `ARCH.12` Фаза-1 `MerkleTree` primitive · `E.60` follow-on (`archive_root` + per-tree witness) · `MRV.1`-residual PATH-2 lineage (credit→measurements) — три пункти, ОДИН відсутній блок |
...
- Cross-ref: SEC.20 (продюсер + OtaProgressBar-підписник — прогрес тепер лише hint/confirmation, Queen-driven), FW.56 (дзеркальна uplink-знахідка), FW.46 (board-freeze незалежний), ARCH.59 (OTA-watchdog той самий тракт), FW.20/FW.17 (їхні downlink-леги тепер фізично доставні)
...
- [ ] 🤖 `ota_channel_<uid>`/OtaProgressBar — broadcast без підписника (SEC.20-розвідка 2026-07-12): жодна вьюха не робить `turbo_stream_from`, і компонент ніде не рендериться initial-page — прогрес-бар мовить у порожнечу (клас ARCH.67 «broadcast-у-порожнечу») → підписка + initial-render на Firmwares::Index/gateway-картці
...
- [ ] ⚖️ (DAO, post-TRL-3) чи замикати PATH 1 — DON Functions wiring (source/consumer/relayer); до присуду dispatch = local marker, callback-endpoint live ([`05_01 §8.3`](05_01_Multichain_Architecture))
...
- [ ] ⚖️ вибір першого inclusion-proof-споживача (ISO-звіт / API / UI / subgraph) — присуд відкриває Фазу 1 (гейт-кластер 🚦: ARCH.12 + E.60 follow-on + MRV.1 PATH-2 lineage)
...
- [ ] 🤖+⚖️ оцінити AVS feasibility + security → присуд anchor-транспорту (direct-L1 baseline vs AVS) перед mainnet anchor-arch lock-in (low urgency)
...
- [ ] ⚖️ (після domain-purchase-рішення) обрати A/B (рекомендація: A — найменше рухомих частин; бренд у листах лишається через відправника `ops@silkennet.com`)
