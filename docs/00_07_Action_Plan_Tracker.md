...
| ID | Пункт | Канон |
|----|-------|-------|
| DOC-T.31 | 🤖 `verification_service_spec` it `logs the error` (query_dclimate network-failure) падає ізольовано **й на HEAD** — `credentials.dig(:dclimate,:api_key)` stub без default перехоплює lazy ActiveStorage-init `dig(:aws,:access_key_id)`. Фікс: default `allow(creds).to receive(:dig).and_call_original` перед вузьким stub. Pre-existing test-hygiene, знайдено при E.41. | `04_06 §A.2` |
