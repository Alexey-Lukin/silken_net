...
- [x] 👤 **Ø стрижня Zone 1 — ✅ Ø11 мм** (founder 2026-06-20) + центр-**стрижень** шини ~Ø1.3 мм (монолітний, `01_01 §1.4`/HW.34) + hex — pending
- [x] 👤 **Радіальний дим-ланцюг — ✅ frozen 2026-06-20 (data-grounded Lamé+CODIT):** вал Ø11 → PEEK стінка **2.0 мм** → **OD = рана Ø15 мм**. 2 мм = **робастний дефолт** (ізоляція + демпфер), **НЕ stress-мінімум** — ні CTE-mismatch, ні press-fit hoop не несучі (unified thick-wall Lamé: combined −30°C+s6-max **SF 5.6× / vM 4.7×**, script 56; старий «3.4× CTE-limited» = завищений (k²−1)-thermal-знаменник, fixed 2026-06-22 HW.3.IS); рана Ø15 < 25мм (не уповільнює ріст — CODIT) → дерева **DBH ≥ 38 см**. Рівномірний профіль (телескоп «жива-рана Ø11» відхилено як складніший вал). Канон [`01_01 §4.2`](01_01_Coaxial_Gyroid_Topology_and_PEEK) + §Zone 2. **Осьова вісь ✅ frozen 2026-06-20:** Zone 2 = **50 мм**, фланець **Ø25**, Z-stack (HW.8.7: spacer+bayonet) — деталі HW.8
- [ ] 🔭 **Future-revision lever — стінка термічно надлишкова (HW.3.IS 2026-06-22):** unified Lamé → combined SF **5.6× ≫ 3** → 2 мм НЕ stress-обмежені; тонша PEEK-стінка = менший OD = **менша рана** (нижчий DBH-поріг, менший CODIT) — латентний важіль для майбутньої ревізії. Геометрія лишається frozen (HW.33); перегляд FEA/структурно/bench-gated, не зараз. Канон [`01_01 §4.2`](01_01_Coaxial_Gyroid_Topology_and_PEEK)
