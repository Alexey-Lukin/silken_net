...
- **Перед польовим деплоєм** (life-safety + security): `SEC.9` · `SEC.3` · `SEC.1` · `FW.2`-фліп + `FW.17`/downlink-wire-rev (обидва ⚖️ **ДО першого field-deploy** [pin 07-11]: CCM-cutover + key-ratchet-активація — rollback до поля = хвилини, після = SWD-візити на статичний KEYL) · `FW.60` (Queen-inbound транспорт — без нього OTA/ratchet/актуатор-команди фізично не доставляються в поле) · `ARCH.60` (notification-delivery — wildfire/chainsaw-алерт мусить дійти до людини: email/SMS/push сьогодні мертві)
...
| **FW.60 Queen-inbound транспорт** (рішення ✅ 07-12 poll-після-флашу → 🤖 wire-up двох ніг + Rails-half) | фізична доставка ВСЬОГО downlink Rails→Queen: `SEC.20`-OTA-кампанії · `FW.20` 0x9C-лег · `FW.17` 0x9E-ratchet · ActuatorCommand `CMD:*` · `ARCH.71`-TinyML · `ARCH.59` fw_pending-consumer · 8КБ↔Rails-ліміт узгодження + канон-лікування 10 дрейф-місць (карта в item) |
...
#### FW.60 — Queen inbound CoAP RX unwired (весь Rails→Queen downlink аспіраційний)
- **P1** · 🤖+👤 · 🟡 · → [`03_02 §5`](03_02_Queen_Gateway_Firmware), [`03_01 §4.6`](03_01_Firmware_Lifecycle_and_DMA)
- **Стан:** ⚖️✅ **Транспорт вирішено (founder 2026-07-12): poll-після-флашу** — LwM2M Queue-Mode ідіома: Queen сама питає pending-downlink `poll/<uid>` одразу після `send_success` (патерн device-event хвоста — модем теплий, IP резольвлений, свіжий NAT-pinhole); server-mode (CASERVER існує, мережево мертвий за CGNAT), модем-MQTT (брокер-інфра + довіра модем-стеку проти FW.56-уроку) і модем-DTLS (ключ у модем) відхилені. Реалізація **двонога**, Rails-half спільний і тракт-агностичний; конверт `[IV:16][AES-256-CBC KEYC]` → незмінний `Handle_CoAP_Command` (нуль крипто-змін; no-MAC стеля = вісь FW.17-гейт (i), не FW.60). Нога-1 = наявний CCOAP-тракт + payload-екстрактор відповіді (стеля ~60 Б `AT_LINE_MAX` → CMD:\*/0x9C/0x9E/0x9A — 4 з 5 класів); нога-2 (OTA 512 Б-чанки, bitmap-16 незмінний) = `CAOPEN`-UDP + `+CADATAIND` + `CARECV` ≤1459 Б сирих — рішення+модем-факти канонізовано [`03_02 §4`](03_02_Queen_Gateway_Firmware) (inbound-тракти) + [`03_02 §5`](03_02_Queen_Gateway_Firmware)-банер. **Карта канон-дрейфу** (описують unwired-тракт як живий; лікувати РАЗОМ із wire-up): `03_01 §4.6` · `03_02 §5`-таблиця + `§5а` TimeSync-hub + `§4.5а` opcode-map · `03_05 §4.3`/`§5`-діаграма/`§6`-таблиця/`§7` · `04_02` ARCH.41-рядок + ActuatorCommandWorker/OtaTransmissionWorker Side-Effects · `04_03 §5.6/§5.7` (202 = enqueue-правда, не доставка). Rails-продюсер готовий (SEC.20 ✅); супутні дірки розвідки (лікуються Rails-half чекбоксом): TimeSync/KeyRotation-воркери без `sidekiq_retries_exhausted` → тихий DeadSet без сліду в БД; Rails не enforce'ить Queen-стелю 16×512 = 8 КБ (`OTA_MAX_CHUNKS` — пакує до ~512 чанків, ch≥16 мовчки дропаються).
- [x] ⚖️ транспорт-рішення inbound RX — ✅ poll-після-флашу (Стан ↑)
- [ ] 🤖 Rails-half: CoAP-ресурс `poll/<uid>` у `CoapGate` + `Downlink::PendingQueueService` (пріоритет CMD > 0x9C > 0x9E > OTA; джерела: `ActuatorCommand.pending` + `ota_version_hiwater`-derived + stateless time-sync) + 8 КБ/16-чанк enforcement у dispatcher + `sidekiq_retries_exhausted` для TimeSync/KeyRotation + спеки
- [ ] 🤖 нога-1 firmware: payload-екстрактор CoAP-відповіді (skip token/options до 0xFF) + poll-виклик після `send_success` + перший call-site `Handle_CoAP_Command` + host-тести (малі класи e2e)
- [ ] 🤖 нога-2 firmware: binary-read примітив `at_engine` (лічені сирі байти після `+CARECV: n,` — FW.3-клас акуратності, окрема host-смуга) + `Sim7070_Udp_Poll` оркестратор (`CAOPEN→CASEND→CADATAIND→CARECV→CACLOSE`) + OTA-чанк e2e host-тести
- [ ] 🤖 канон-лікування дрейф-місць за картою ↑ (тим самим wire-up заходом)
- [ ] 👤 bench: жива poll-розмова обома ногами + NMI/CARECV-поведінка verbatim (довгий NMI = модемна невідома; їде з FW.56-bench) [bench:coap]
- Cross-ref: SEC.20 (Rails-half продюсер ✅), FW.56 (дзеркальна uplink-знахідка «модем = UDP-труба»), FW.46 (board-freeze НЕ розблоковує цю діру), ARCH.59 (OTA-watchdog ловить stuck-`:updating` цього ж тракту)
...
- [x] 🤖 мікро-дрейфи — ✅ 2026-07-12: warm drift-watchdog `Soldier_Should_Request_Time_Sync` вшито 10-рядк hook'ом у ФАЗУ 4 (0x56 поверх телеметрії; вся інфраструктура вже чекала — лічильники/скидання/canary-патерн цитували «сплячий watchdog»); стейл-коментар mesh-relay виправлено (вшито за гейтом, не «окрема ітерація»); «маячить після кожного конверта» → reflex-перемотка на 0x56 (обидва доми: `beacon_dedup.h` + soldier). ⚠️ mesh-relay FW.20 = downlink-**маяк** (KEYB 16B ECB), НЕ ARCH.43 uplink → гейт = чистий bench-flip, не wire-rev3. E.51 cross-ref у Стані
...
- [x] 🤖 `ARCH26_CAD_ENABLED=1` compile-lane у `hal_check_ccm` — ✅ 2026-07-12 (патерн ARCH.34/ARCH.35: `#ifndef`-wrap гейта в soldier + `-D` у CMake; CAD-гілка бачить справжній Semtech radio.h); стеля лишається: link-бомбу НЕ ловить — compile-lanes не лінкують ([`03_01 §12.4`](03_01_Firmware_Lifecycle_and_DMA)), справжній link-guard = FW.46 full-`.elf`
...
- [ ] 🤖 `fw_pending`-consumer відсутній (SEC.20-розвідка 2026-07-12): `check_firmware_mismatch!` пише `Tree.firmware_update_status = :fw_pending`, але жоден worker/cron/scope його НЕ читає — глухий прапорець назавжди; природний дім споживача = OTA-sweep цього ж айтема (re-deploy-політика = 👤-рішення, не автоматизувати мовчки; доставка gated FW.60)
...
- **Стан:** deep-audit 2026-07-04 (O3), machine-half SHIPPED тієї ж доби: (1) **money-переходи → SHA-256 `AuditLog`-ланцюг** (`BlockchainTransaction` AASM `after_all_transitions` → async `record_money_audit_trail` — модель-канон `04_01` BlockchainTransaction/Wallet-картки: actor=oracle_executioner, org-ланцюг, metadata from/to/tx_hash; без org/actor — WARN-skip, tx не валимо); (3) **destroy-guard MRV-доказів** (`Wallet#guard_mrv_evidence!` — settled/in-flight tx → abort; чисто-pending видаляється); (4) **web3 incident runbooks** ([`06_08 §4`](06_08_Resilience_and_Failover_Policy): reorg / double-mint+rogue-MINTER / money-key compromise / `manual_review` **console-рецепт** — founder-рішення 2026-07-04: без admin-UI до першої реальної ops-потреби / contract-ops one-shot). Slash-атрибуція on-chain = `contextHash` (CONTRACT.1 §🗄️). Telemetry-ретеншн = політика партицій (drop старих партицій — легітимний шлях, НЕ хардделіт-діра). Дім `05_02` (модель довіри) + `06_08 §4`.
