...
| ARCH.32 | **Shape Up 6-week cycle Petri Net formalization** — PN-верифікація фази Shape Up (betting → build → cool-down): довести, що фіча завершується в межах cycle constraints. **Робота, що виробляє `08_01` Стаття 9** (Супруненко PN-half; сиблінг ARCH.20 → Стаття 7). Журнал Ст.9 = IJPM/TFSC/IEEE TII (беклог хибно казав «IEEE TSE» — то Ст.7/ARCH.20) | `08_02` §1B (Супруненко), `00_04` | Post-TRL 7 (R&D — Супруненко ЧНУ) |
