...
- [ ] 🤖 (2026-06-15 аналіз) `recordPremiumPaid()` (SCC) — `onlyRole(DEFAULT_ADMIN_ROLE)` (=Timelock post-SEC.1) і **0 backend-викликів** (grep app/lib) → vestigial: ніхто не робитиме 48h governance-цикл заради emit події премії. Винести під операційну роль (RECORDER) АБО прибрати як dead-code; рішення про долю — founder ([`05_03`](05_03_Tokenomics_SCC_and_SFC))
