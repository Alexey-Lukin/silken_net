...
| **Перший live-деплой** = акаунти/значення Фази −1 + секрети (`S1.1`+`S4.3`) + `OPS.11`-FinOps-значення (tf/workflow ✅ 07-05) → deploy фазами [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) (👤; SSH-нога wired — INF.20 (в)) | верифікації `INF.11/12/13/14/15/16` · `INF.17`-coap.env · `S1.5`-IP · `INF.20`-IAP-вхід · `PUMA-IPV6-1` · `INF.10`-фліп · `S5.2` · `S3.2`-staging · `S6.1`-Upstash · `INF.21`-pin · `SEC.19`-region-placement (EU-PII residency) · Grafana-сесія (`S2.1→S2.2` + `S2.4`-SLO + `FW.18b`) · активує escrow-watch (repo Variable `AKASH_OWNER_ADDRESS` — workflow ✅) |
...
- **P2** · 🤖 · ⚪ · → `04_05`
- **Стан:** Audit §04 (2026-07-04, A5-Codex). Чотири Codex «live»-фічі (attunement-counter, live-comments, citation-pills, discovery-toast) шлють `ActionCable.server.broadcast` у порожнечу — нема `@rails/actioncable` importmap-pin, нема `app/channels/`, нуль `subscriptions.create` у репо → браузер НІКОЛИ не отримує payload (робочий патерн — pair з `Turbo::StreamsChannel.broadcast_*_to`, як telemetry/burn; Codex робить лише raw-half). `Discoveries::Toast` взагалі ніде не інстанціюється; docstring'и стверджують протилежне shipped-поведінці. **Суміжне (coverage-sweep 2026-07-08): `Codex::Comments::Thread` вішає `data-controller="codex--comment"` на ВНУТРІШНІЙ `div#list`, а `Comments::Form` (form/body Stimulus-таргети) рендериться SIBLING поза ним → Stimulus-scope (лише нащадки controller-елемента) НІКОЛИ не бачить таргетів → Cmd/Ctrl+Enter submit + textarea-reset/focus архітектурно мертві (JS-behavior, не ловиться Phlex-string-специ).** Канон `04_05` (ADR-CDX-8).
...
- **P2** · 🤖+👤 · 🟢 · → `04_02`, `04_01`
- **Стан:** «Діставання нутрощів» для INF.23-гарнеса (§06) підсвітило борги, невидимі поза per-packet cost-обліком (drain-cost + red-team, дві незалежні лінзи, verify-vs-code). Гарнес їх квантифікує (S1-S6); фікс — окремо, не блокер.
- [ ] 🤖 `previous_lorenz_state_for` (`telemetry_unpacker_service.rb`) — un-prunable cross-partition MergeAppend на КОЖЕН пакет (`order(created_at:).limit(1)` без нижньої межі); fan-out росте щомісяця. Fix: межа `created_at >= 2.months.ago` (warm-lookup потребує лише свіжого; давно-мовчазне → cold-start ARCH.41). ⚠️ money/DCI critical-path — тести перед зміною.
- [ ] 🤖 listener (`lib/daemons/coap_listener`) ковтає Redis-enqueue-fail generic-rescue'ом без метрики → невидимий drop під навантаженням. Fix: `COAP_PACKETS_RECEIVED_TOTAL{status:"enqueue_error"}`.
- [ ] 🔗 3-Postgres-DB pool-tripling (primary+solid_cache+solid_cable на пакет) — sizing-знання, не баг (гарнес скрейпить DB_POOL_WAITING×3) · `StreamrBroadcastWorker` на `low` голодує під uplink (design-review) · WEB3_RPC_LIMITER slot без RPC (ChainlinkDispatch ARCH.53) · Codex SMEMBERS/пакет при 0 observers · `db:schema:load` не ідемпотентний (`sync_cluster_geo_boundary` без OR REPLACE; `db:test:prepare` OK)
...
- **Стан:** Виявлено 4-агентним ARCH.66-review (2026-07-07, поза scope того таску). Два timing/race-flaky тести падають у full-suite непередбачувано: (1) `spec/services/solana/batch_payout_service_spec.rb` — впав 2/4 full-прогонів, і **той самий seed 42 дав 5 failures, потім 0** → НЕ order-dependency (яку контролює seed), а справжня non-deterministic race (ізольовано завжди 0/0); (2) `coap_smoke_spec` тест «тиша (порт без Брами) = чесний фейл» (INF.6, `timeout: 0.2s` UDP) — seed-залежний timing (seed 18624 → 0, інші → fail). Той самий клас, що TEST.2 (ENV-leak flake, §🗄️). Ризик: seed НЕ пінується (`config.order = :random`) → CI `bundle exec rspec` вразливий до випадкового червоного. Корінь не копано (поза ARCH.66). Дім [`04_06`](04_06_Testing_Guide_and_Coverage).
- [ ] 🤖 root-cause `batch_payout_service_spec` race (shared mutable state / real-time-залежність / thread) + coap `0.2s`-timeout — стабілізувати (або пінити CI-seed як тимчасовий backstop)
...
- [ ] 🔗 (PATH-1 latent, low) B1 IoTeX-verify retry-exhaustion: `IotexVerificationWorker` (retry:5) без `sidekiq_retries_exhausted` → job тихо осідає в Dead Set (per-job reliability-лінза; smart-guard «B1» у `iotex_verification_worker.rb` = ІНШЕ, вже shipped). НЕ money-блокер (живий PATH 2 обходить IoTeX-gate). **Консолідовано з INF.22 §06 (`IotexBackfillWorker`)** — один cron покриє й per-job exhaustion, і outage-backfill; дім `06_08 §2.2` крок 5
...
- [ ] 🤖 (known-ceiling, pre-existing) Celo reward-tx (cluster-sourced, `wallet=nil`) не пише MRV `AuditLog` (`record_money_audit_trail` keys off `wallet.organization_id`) — escalation-видимість тримає `manual_review_depth` gauge; MRV-lineage для cluster-sourced money = окремий хвіст

#### ARCH.65 — Hadron KYC verify retry-exhaustion recovery
- **P2** · 🤖 · 🟢 · → [`05_02`](05_02_Proof_of_Growth_Pipeline), [`06_08 §2.2`](06_08_Resilience_and_Failover_Policy)
- **Стан:** Machine-half ✅ **SHIPPED (2026-07-07).** Money-recovery red-team знахідка: `HadronKycVerificationWorker` (retry:5) без `sidekiq_retries_exhausted`, enqueue лише разово `after_commit` на зміні crypto_public_address → Hadron API down усі 5 retry → Dead Set → `hadron_kyc_status="pending"` назавжди → mint-gate `Wallet#kyc_approved_for_minting?` щоцикл (`MintBatchCollectorWorker`, 5хв) мовчки скіпає pending-tx бенефіціара (prod `WEB3_STRICT_MODE` → гейт живий, реально блокує mint коштів). **Шиплено:** `HadronKycReverifyWorker` cron (:50) — pending KYC (з власною адресою, `updated_at` >1год) → re-enqueue verify (idempotent auto-heal коли API оживе; скоуп лише «pending»). **Review-hardened (3-агентний, 2026-07-07):** BATCH_LIMIT + oldest-first дренаж (проти thundering-herd на mass-recovery проти щойно-оживаючого API) + backlog-видимість gauge `silkennet_hadron_kyc_pending_depth` (симетрія з Celo `manual_review_depth`) + Org approved/fresh/rejected/BATCH-cap покриття. Дім [`05_02`](05_02_Proof_of_Growth_Pipeline) (KYC-gate) + [`06_08 §2.2`](06_08_Resilience_and_Failover_Policy) (Hadron-крок). Відкрите ↓.
- [x] 🤖 **[SHIPPED 2026-07-07]** Grafana-alert `sn-alert-hadron-kyc-backlog` (P2/info, `max(min_over_time(depth[6h])) > N`, for:30m — sustained-backlog vs нормальний in-flight KYC; поріг placeholder, калібрується live). + net-новий `grafana_alerts_spec` (кожна alert-метрика ∈ Prometheus-реєстру — ловить мертвий alert). info-severity (кошти НЕ заблоковані, auto-heal ARCH.65)
...
- **Стан:** Machine-half ✅ **SHIPPED (2026-07-07).** 5-агентний red-team INF.22-A знайшов замість відхиленого gas-gate (крок 12) реальну діру: `StateAnchorService#anchor_to_l1!` ставив `:sent` і зупинявся — жоден воркер не polls receipt → anchor НІКОЛИ не `:confirmed`, scopes `latest_confirmed`/`successful` вічно nil, а double-anchor guard деградував до 1-тижневого таймера (завислий `:sent` після 7д випадав із guard → ризик подвійного state_root). **Шиплено** (дзеркало `CeloConfirmationWorker`/ARCH.55, L1-специфіка): `EthereumAnchorConfirmationWorker` (poll receipt → `confirm!`/`mark_failed!`/`:pending`-raise; **reorg-depth gate** `ETHEREUM_ANCHOR_MIN_CONFIRMATIONS`=64 перед seal — «фінальна печатка» не confirm'иться на першому receipt, як money-path; **retries_exhausted робить ФІНАЛЬНИЙ receipt re-check** перед escalate — tx міг замайнитись в останній ретрай, дзеркало `MintingRollbackService`; enqueue `perform_in(30s)` після `:sent` + re-arm на weekly-resume) + `StuckSentAnchorSweeperWorker` (cron :40, re-arm застряглого `:sent` через scope `EthereumAnchor.stuck_sent` — **read-only re-poll, НІКОЛИ re-broadcast**: poll достатньо для `:sent`, розштовхування застряглого = операторський same-nonce gas-bump) + enum `manual_review`(4) (exhausted-і-все-ще-pending→escalate, виходить з `in_flight` → розблоковує наступний тижневий seal) + звужено `detect_missed_anchor_weeks!` на `[:confirmed]`-only (з реальним поллером `:sent` transient → stuck `:sent` більше не маскує gap) + метрики `ethereum_anchor_stuck_sent_depth` / `_manual_review_depth` (gauges, семплить `Treasury::MonitorService` 15-хв) / `_reverted_total` (counter) + 3 Grafana-alert'и (stuck-sent P1 · manual-review P1 · reverted P2). Модель = plain enum → `with_lock`+`status_sent?`-гард на кожному переході (idempotency без AASM). **Активація gated на деплой контракту** (SEC.1; `ETHEREUM_ANCHOR_CONTRACT`/`_PRIVATE_KEY` не задані → anchor-path структурно неактивний, worker+cron natural-inert) — код pre-deploy готовий. Канон [`05_04 §5.1`](05_04_Ethereum_L1_State_Anchor) + [`06_08 §2.2`](06_08_Resilience_and_Failover_Policy) + [`04_02`](04_02_Business_Logic_and_Services). Companion nonce-persist ✅ ↓ (machine-half тепер повний; лишок = deploy-активація SEC.1).
- [x] 🤖 **[SHIPPED 2026-07-08]** nonce-persist проти F2a double-send — колонка `ethereum_anchors.nonce` персиститься ПЕРЕД `transact()` (новий anchor → `client.get_nonce` pending-tag + persist; resume `:pending` → колонка вже заповнена, get_nonce НЕ кличеться) + явний `nonce:` у `transact` → resume ре-броадкастить на ТОМУ Ж слоті (replace / already-known / nonce-too-low = **≤1 on-chain tx на anchor, не N+1**) + **classify-and-escalate** (self-review Opus+Sonnet: persist сам = напівфікс — resume-rejection приходить як `RpcError`<`IOError` → осідав би у `:pending`-orphan поза поллером): `rescue Eth::Client::RpcError` перед generic `IOError` → `escalate_pending_ambiguous!`(`:manual_review`, tx_hash lost, звірка address+nonce) на resume-ambiguous, `raise` на fresh insufficient-funds/revert. Поллер уже конвертував limbo у benign transient; це прибирає й спалений-gas-хвіст. Авто-gas-bump НЕ роблю (розштовхування застряглого = операторський ручний same-nonce). Дім [`05_04 §5.1`](05_04_Ethereum_L1_State_Anchor) §nonce-persist
...
- **P0** · 👤 · 🟢 · → [`06_04 §1`](06_04_Secrets_Checklist), [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform)
- **Стан:** Machine-half ✅ повністю (INF.19/B1 — корінь 4-місячного deploy-блоку: workflows мапили 5 із 24 секретів → порожній інжект → boot-crash за зеленим verify; тепер обидва workflow маплять весь `env.secret`-набір, `verify-secrets` гейтить повний boot-critical сет fail-loud prod / skip-clean canopy + warn на lazy). **2026-07-04 аудит-дожим:** `KAMAL_MASTER_KEY` вирізано (фантом — Kamal 2.x не має такого механізму, а гейт блокував prod на неіснуючу залежність); гейт розширено інфра-передумовами `GCP_PROJECT_ID`+`SSH_PRIVATE_KEY`/`SSH_KNOWN_HOSTS`; заведено наскрізно `ORACLE_CELO_PRIVATE_KEY` (ARCH.50 — був відсутній у ВСІХ 7 deploy-поверхнях) + `HELIUM_WEBHOOK_SECRET` (був ніде → кожен деплой 500-ив Queen-SOS endpoint). Заповнення = дві партії Batch A/B (порядок: [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) Фази 0-1; секрет-специфіка `06_04 §5.1`).
- [ ] 👤 завести повний набір GitHub Secrets (Batch A → apply → Batch B) → верифікувати fail-loud `verify-secrets` + перший boot
...
- **P0** · 👤 · 🟢 · → `06_03`
- **Стан:** повний IaC готовий (2026-07-04: Runtime/Fleet-секції, flapping-rule, money-path панелі, README-реген, `--dry-run` CI-гейт — spec-at-source = `deploy/grafana/` + README): дашборд + alert rules (вкл. up-alert 3 таргети, mint-SLO, `sn-alert-gateway-faulty` ARCH.54) — один захід `deploy/grafana/import.rb` (auto-UID, ідемпотентний upsert). ⚠️ Без contact point УСІ P0-алерти летять у нікуди (O3-MUST).
...
- **Стан:** Аудит 2026-07-04 (клас B1 «конфіг повний, шлях мертвий»): секрети замаплені бездоганно, а SSH-нога, якою `kamal deploy` має зайти на анкор, не існує з ТРЬОХ незалежних причин: (1) `compute.tf` metadata має `enable-oslogin=TRUE` — OS Login **ігнорує** metadata ssh-keys і деривує POSIX-юзернейм з IAM (НЕ `deploy`); (2) `var.ssh_public_key`/`var.ssh_user` **не споживаються жодним tf-ресурсом** (workflow сумлінно передає `TF_VAR_ssh_public_key` у нікуди); (3) CI не передає `TF_VAR_ssh_source_ranges` → default `[]` + count-guard → firewall `allow_ssh` взагалі не створюється (а GitHub-раннер IP динамічний — у CIDR-allowlist не вкладається; tfvars.example CIDR = TEST-NET-3 doc-placeholder). Наслідок: CI-Kamal-шлях структурно недосяжний; локальний — теж (нема ні юзера, ні ключа на VM). Akash-шлях (перший деплой) від SSH НЕ залежить; verify-secrets тепер гейтить SSH-набір (S1.1). **Поглиблений аналіз 2026-07-04 (перевернув першу рекомендацію):** факти, що змінили рамку — (1) Kamal офіційно підтримує `ssh.proxy_command` (docs verified) → IAP-тунель для kamal реальний: `proxy_command: "gcloud compute start-iap-tunnel silken-net-ingress %p --listen-on-stdin --zone …"`; (2) опція (а) однаково НЕ вирішує CI (GitHub-раннери = динамічні IP з величезних діапазонів — у CIDR не вкладаються) і жертвує OS Login (IAM-ревокація, аудит-лог, незалежність від зміни домашнього IP у Черкасах) заради простоти, якої IAP-шлях досягає без жертв; (3) навіть IAP сьогодні мертвий — deny-all блокує і 35.235.240.0/20 (IAP-frontend CIDR): +1 firewall-правило потрібне ЗА БУДЬ-ЯКОГО вибору; (4) `gcloud compute ssh --tunnel-through-iap` при живому OS Login дає founder'у нуль-конфіг вхід (gcloud сам реєструє ключ у профілі) — але для `coap.env`/systemctl потрібен sudo = роль `compute.osAdminLogin` (НЕ osLogin; iam.tf дає SA лише osLogin); (5) при (в)/(б) секрети `SSH_PRIVATE_KEY`/`SSH_KNOWN_HOSTS` стають непотрібні → зняти з verify-secrets BOOT_CRITICAL (гейт-розширення 2026-07-04 писалось під (а)-припущення). Варіанти: **(а)** oslogin=FALSE + metadata-keys — класика, kamal as-is, але 22 відкритий на CIDR + ручна ключ-ротація + CI не вирішено; **(б)** OS Login + IAP повний (kamal proxy_command; ssh.user = OS Login-нейм — для founder і SA РІЗНІ → destination-нюанс); **(в) = (б)-лайт, РЕКОМЕНДОВАНО:** зараз лише firewall `allow-iap-ssh` (35.235.240.0/20) + `osAdminLogin` founder'у → Фаза 1 (coap.env) розблокована `gcloud compute ssh --tunnel-through-iap` без відкриття 22 в інтернет і без вимикання OS Login; kamal-клей (proxy_command + SA-роль) — коли Kamal-fallback реально знадобиться (він вторинний: перший деплой = Akash). **Рішення (в) ухвалено (founder 2026-07-04) і wired того ж дня:** `allow_iap_ssh` firewall (35.235.240.0/20) + `iap_admin_members` tf-var (osAdminLogin+tunnelResourceAccessor), мертві SSH-vars вичищені, SSH-трійка знята з verify-secrets/06_04 §1.1, `ssh_source_ranges` = break-glass-only; канон = [`06_01`](06_01_Deployment_Kamal_Terraform) (Фаза 0 + §Firewall).
...
- **P1** · 👤 · 🟢 · → `06_01`, `06_02 §1.4`, `03_02 §4` **[P2→P1 2026-07-05: передумова першого деплою — INF.6-gate чекає слухача]**
...
- [x] 👤→S2.2: `sn-alert-gateway-faulty` імпортується разом з усім пакетом правил (чекбокс переїхав у злиту Grafana-сесію S2.2, 2026-07-04)
...
- **Стан:** Custody-поріг ✅ **вирішено — GCP Cloud KMS remote-signer** (founder 2026-07-06). Наявний захист money-ключів ✅: `ORACLE_MINTER_PRIVATE_KEY`⊥`ORACLE_SLASHER_PRIVATE_KEY` фізично розділені (E.2, mint≠burn), Gnosis Safe тримає admin/PAUSER (`SilkenForestCoin.sol`), ARCH.47 boot-guard проти key-collision (`Security::Web3NetworkGuard`), WEB3_STRICT_MODE fail-closed. Лишкова діра — приватники **plaintext у deploy-ENV**: у момент mint'у за реальну вартість ENV-ключ мінтера/слешера = найбільша одинична точка катастрофи. KMS обрано над Fireblocks (enterprise-cost; забирає broadcast/nonce → перетин з `BlockchainConfirmationWorker`/ARCH.47-lock) і Safe-module-mint (on-chain роль + relayer = знову ключ; admin-вектор SEC.1, не hot-mint): HSM-grade + дешево + автоматизований hot-path + GCP у стеку. **Impl gated на pre-mainnet** (захищає ключ у момент mint реальної вартості; до prod-mint ENV-ключ теж нічого не мінтить → НЕ TRL-3-блокер + уникає bit-rot `google-cloud-kms`). Дизайн + 3-крокова impl-мапа → [`06_04 §5.5`](06_04_Secrets_Checklist). Дзеркалить S6.14 (peaq custody) на money-рівні.
- [x] 👤 обрати custody-поріг → ✅ **GCP-KMS** (founder 2026-07-06)
- [ ] 🤖 (pre-mainnet) `Web3::OracleSigner` seam (`address`+`transact`+`static_call` навколо 12 `sender_key`-call-sites; `LocalEnvSigner` default = behavior-preserving) → `Web3::KmsSigner` (`google-cloud-kms`; digest→asymmetric_sign→DER→low-s→recovery-id, offline crypto-тести) — заміна raw-ENV-приватника у `BlockchainMintingService`/`BlockchainBurningService`
...
- **Стан:** Аудит 2026-07-04: 06_08 §2.2/§2.3 ✅-ив ~10 механізмів, яких у коді немає — канон переписано чесно (🟡-target інлайн у §2.2 таблиці = джерело-правди станів). **Розкладено 2026-07-06** (bird's-eye §2.2/§2.3 + code-scan). НЕ чеклист «зробити все» — кожен класифіковано. 🔑 Знахідка розкладки: RPC-fallback-каскад (`Web3::ResilientClient` + `RPC_FALLBACK_ENV_KEYS`) і circuit_breaker (`Web3CircuitBreaker`, 5 воркерів) ВЖЕ живуть → половина «target'ів» = config/enhancement поверх наявного, не з нуля. O4 proxy-supervisor + DB_POOL ✅ SHIPPED 07-05 (дім [`06_01`](06_01_Deployment_Kamal_Terraform)/[`06_05`](06_05_Puma_Configuration)). Розклад ↓: 🤖 = machine-half buildable зараз (skip-clean, активація = ENV/Variable) · 🔗 = gated (PATH-1 latent / live-trigger) · 🌿 = YAGNI до scale. Підіймати дозріле в окремий item — коли беремося РОБИТИ. **Прогрес:** крок 7 (Solana RPC-fallback) ✅ SHIPPED 2026-07-06 — перший з 🤖-buildable-п'ятірки.
- [x] 🤖 **[buildable]** Solana RPC-fallback-каскад ✅ **SHIPPED 2026-07-06** — `SOLANA_RPC_URL_FALLBACK_1/2` каскад у `Solana::MintingService#execute_rpc_call` (Solana не-EVM → НЕ `Web3::ResilientClient`; per-service circuit-breaker "Solana" у `Web3::HttpClient` кооперує — успіх fallback скидає лічильник, open лише на вичерпанні всіх endpoint'ів). Заразом: транспортний `rpc_url`-проброс через 6 методів прибрано, E.47 prod-guard централізовано в резолвер `solana_rpc_urls`. skip-clean (unset → single-RPC). Дім `06_08 §2.2` крок 7
- [x] ✂️ **[ВІДХИЛЕНО — net-negative, 5-агентний red-team 2026-07-07]** anchor gas-gate `MAX_ANCHOR_GWEI` — pre-broadcast skip-гейт виявився шкідливим: (1) наявний `ETHEREUM_MAX_FEE_GWEI` cap уже дає opportunistic-landing (EIP-1559 не включить tx поки base_fee>cap — переплата неможлива); (2) gate на weekly-cron перетворює transient-спайк на ГАРАНТОВАНИЙ пропуск тижня (гірша liveness); (3) single-RPC base-fee-read = новий censorship-вектор (фейк-high → skip назавжди). Реальна anchor-діра поруч — anchor НІКОЛИ не `:confirmed` (fire-and-forget) → **ARCH.66** (§05, ✅ SHIPPED). Дім `06_08 §2.2` крок 12
- [x] 🤖 **[SHIPPED 2026-07-07]** `FilecoinReconcileWorker` — daily-cron (:48) re-pin outbox-marked `AuditLog.pending_archive`. **Outbox identity-fix** (не голий `not_archived`): `archive_requested_at`-маркер виставляє лише money/MRV-шлях (`AuditLogWorker`); codex/factory прямий `create!` навмисно поза периметром (інакше Pinata quota-waste + security over-exposure factory-provenance). + **detect-половина** (жива ARCH.65-діра: `FilecoinArchiveWorker` `sidekiq_retries_exhausted`-hook + `filecoin_unarchived_depth` gauge + P2-alert). Partial index `index_audit_logs_pending_archive` (eligibility-scoped). Дім `06_08 §2.2` крок 11 + [`04_02`](04_02_Business_Logic_and_Services)-card. Відкрите ↓
...
- [ ] 🤖 **[buildable, post-deploy value]** CI/infra-hardening пакет (дім CI-патернів = [`06_07`](06_07_CICD_and_Runbook_Index) + `ci_maturity`-gotchas): GH Environments для prod (секрет-скоупінг money-ключів + wait-timer замість plan-gate — solo-substitute людського approval, bus_factor=1) · WIF CI→GCP (Akash static SA лишається винятком — [`06_02`](06_02_Akash_Network_Integration)) · tfsec/checkov + scheduled drift-plan (⚠️ новий scan-tool SHA-pin: pinned-binary+sha256 / official SHA-pinned action, НЕ moving-tag — інакше self-inflict PinnedDeps Scorecard-alert; scan репортує власні чеки як Security-tab alerts = hygiene) · Loki-pipeline в Alloy (Akash lease-логи ефемерні → зараз виживають лише Sentry-exceptions). Усі config-half buildable зараз, цінність post-deploy. O3
...
- **Стан:** **Machine-half ✅ SHIPPED** — гарнес `lib/silken_net/load_test/` (factory · coap_flood · provisioning · drain_bench · lorenz_microbench · load_report) + `bin/coap_load` + методологія `README.md`. НЕ k6/locust (вони HTTP — не говорять наш CCM/CBC CoAP-wire; власний Ruby reuse `CoapClient.build_put`). **5-агентний research** (methodology/drain-cost/load-model/red-team/prior-art) переформатував задачу: **dev-число ≠ capacity** — bottleneck-class inversion (dev compute/GVL-bound; prod network-IO-bound через Cloud SQL+Upstash+solid_cable/cache → завищення 10-50×), тож гарнес = **regression+structural detector**, абсолютна стеля лише на staging з prod-adapters. Виміряно (dev, structural): **GVL-стеля ПЛОСКА** (~42k iter/s на 1/5/15 тредах — Lorenz = 1 ядро незалежно від concurrency, підтверджує ARCH.52 process-ізоляцію). drain-bench: backlog→μ + arrival→sustainable-λ, **композитний done-signal** (усі 9 черг+busy+retry, не uplink==0). Модель навантаження звірено з firmware+canon (Queens=N/100, records/s=N/3600; UDP-intake НЕ горло). Виявлені борги → PERF.1. Канон `06_08 §2.4/§2.5`, README.
- [x] 🤖 load-harness (власний Ruby): factory+flood+provisioning+drain(backlog+arrival)+microbench+report+CLI, 6 specs green
- [ ] 👤 прогнати проти staging з prod-adapters → фактична стеля vs E.5 (CoV≤5%, decompose-by-stage; дотично E.5/ARCH.2/E.27)
...
- **Стан:** Gap-pass §06/07 (2026-07-05) — verified: placement (`profiles.placement.silken-dcloud`) фільтрував провайдерів лише `attributes: host: akash` + `signedBy` — нуль country/region. Data-at-**rest** region-pinned (Cloud SQL `europe-west1` + [`04_01`](04_01_Data_Models_and_Entities) `data_region`-шардинг + SEC.18/ARCH.57 PII), але data-**in-use** (Rails-моноліт із User/Org-PII у пам'яті) сідав на будь-якого Akash-провайдера світу → напруга з EU-residency/DPA (BIZ.3 Privacy-Policy; той самий тригер, що ARCH.57 EU-PII-онбординг). **🤖-half ✅ SHIPPED 2026-07-06:** off-by-default `region`-атрибут — tf-var `akash_region` (порожній → омітиться) у `deploy.yaml.tpl` (`%{ if }`-умовний, дзеркало release_version) + закоментований приклад у static SDL + `tfvars.example` + чесна tradeoff-нотатка [`06_02 §1.3`](06_02_Akash_Network_Integration). 🔑 Research-факт, що переоформив задачу: Akash криптопідписує (аудує) **лише `host`/`tier`/`organization`** — `region` **self-reported без governance** (звірено байт-у-байт з akash-network/docs), тож `signedBy` географію НЕ підкріплює → фільтр = м'яка преференція (знижує випадкове не-EU-розміщення), НЕ residency-гарантія; справжній EU-PII-важіль = EU-пінований GCP Ingress Anchor (data-in-use) + Cloud SQL at-rest. ⚠️ Побічна знахідка research'у → INF.24 (auditor `signedBy`-адреса розходиться з поточними Akash-docs).
