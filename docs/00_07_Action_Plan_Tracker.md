...
- [ ] 🤖+👤 **SubGHz Шлях A — RadioEvents_t половина SHIPPED (2026-07-04):** ✅ реєстрація в обох `main.c` (static events-таблиці: Queen RxDone; Soldier RxDone+CadDone ARCH.26; прототипи; латентний `Radio.Init(NULL)` Queen-RX баг закрито; host 38/38 + cppcheck clean; компілюється проти owned-стаба — інваріант до заміни). 👤 Лишилось (auto-mode заблокував зовнішній submodule — виконати вручну): `git submodule add https://github.com/STMicroelectronics/stm32-mw-subghz-phy firmware/extern/subghz-phy` + `git -C firmware/extern/subghz-phy checkout v1.5.0` (звірити SHA `7dc059f3`, BSD-3-Clause = трійка з HAL v1.6.0+CMSIS v1.4.0) → потім 🤖 замінити owned-stub `hal_glue/radio.h` реальним + include-path у hal_check-lane → compile-lane бачить radio-шлях. Розблоковує money-path bench-зрізи (§12.5 pin-таблиця; sx126x.c для on-chip не існує — лише `radio_driver/`)
