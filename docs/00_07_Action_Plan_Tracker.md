...
| **FW.46 board-freeze** `.ioc` (👤) | повний `.elf` → увесь bench-день: `FW.2`-фліп · `FW.3/8/17/20/23/31/49/50/52/54/55` · `ARCH.26/41` · `SEC.2`-OTA-verify · `SEC.20`-OTA-replay-REJECT+fallback-erase+reverted-звіт (host ✅ вкл. wire-звіт, кремній) · `SEC.15`-WUT · `SEC.21`-MPU-activate (canary fielded ✅ · MPU-draft ✅ — лишився trap на кремнії) · `SEC.3`-SWD · `ARCH.35`-розводка · `radio_conf.h` → компіляція `radio.c` (SubGHz submodule + `RadioEvents_t` уже в репо — останній radio-гейт); суміжно `ARCH.34`-ефір: host-половина ПОВНА ✅ (glue+adapter+мок-LNS повний join+uplink+KV-bind; форк v2.6.2-silken.1); лишився OTAA-ефір на bench (дім ARCH.34) |
