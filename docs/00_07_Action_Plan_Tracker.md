...
- [x] 🤖 coment-fix SHIPPED (vilize 07-11): stale код-коментар `Ring_Serialize_Slot` «21B→29B»→«air+1 (=`QUEEN_CCM_RECORD_LEN`, rev2.1=31B)» + re-layout-пояснення в коді. ⚠️ `_Static_assert(ring ≥ CCM-record)` **ВІДКЛИКАНО 07-12**: `hal_check_ccm` фліпає FW2_CCM+ARCH35 РАЗОМ для compile-coverage БЕЗ ring-31-layout → assert падав на CI (`firmware_arm_build`). Guard передчасний — ring-31 недоспроектований (це 🔗-робота ↓, не compile-guard; derived-31 теж ламає SLOTS-layout).
