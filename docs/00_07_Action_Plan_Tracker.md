...
#### ARCH.66 — Anchor lifecycle: `:sent`→`:confirmed` ніколи не досягається (fire-and-forget)
- **P2** · 🤖+👤 · 🔗 · → [`06_08 §2.2`](06_08_Resilience_and_Failover_Policy), [`05_04`](05_04_Ethereum_L1_State_Anchor)
- **Стан:** 5-агентний red-team INF.22-A (2026-07-07, verify-by-code) знайшов замість відхиленого gas-gate (INF.22 крок 12) реальну anchor-діру: `StateAnchorService#anchor_to_l1!` ставить `:sent` і зупиняється — **жоден воркер не polls receipt → anchor НІКОЛИ не `:confirmed`**. Наслідки (grep-верифіковано): `EthereumAnchor.latest_confirmed`/`successful` = мертві scopes (вічно nil); `block_number`/`gas_used`/`verify_state_root` порожні; double-anchor guard (`in_flight` = pending/sent AND created_at>1тиж) деградує до 1-тижневого таймера — завислий `:sent` після 7 днів випадає з guard → ризик подвійного state_root, якщо старий tx колись приземлиться. `StuckSentTransactionSweeperWorker` явно виключає anchor («Solana/anchor reconcile via their own crons — out of scope») — а тих кронів НЕ ІСНУЄ (ARCH.55 закрив клас для mint, anchor лишив). **Gated на деплой контракту:** `StateRootAnchor` не задеплоєно (SEC.1), `ETHEREUM_ANCHOR_CONTRACT`/`PRIVATE_KEY` не задані → anchor-path структурно неактивний → confirmation нема що confirm'ити (YAGNI до pre-deploy). Наявний `detect_missed_anchor_weeks!` → `silkennet_anchor_missed_weeks_total` озброєно P1-alert'ом (`sn-alert-anchor-stalled`, 2026-07-07). Канон [`05_04`](05_04_Ethereum_L1_State_Anchor).
- [ ] 🤖 (pre-deploy) `EthereumAnchorConfirmationWorker`: poll receipt для `:sent` anchor → `:confirmed` (заповнити block_number/gas_used) + stuck-`:sent` reconcile (>N хв → re-broadcast/escalate) — дзеркало `BlockchainConfirmationWorker`/ARCH.55, EVM-anchor-специфічно
