...
- [ ] 🔴 **bench-OTAA блокер:** vendored UB `RegionCommonComputeSymbolTimeLoRa` (RegionCommon.c) — `(1<<phyDr)*1000000` signed-overflow при SF11/SF12 → RX-window symbol-timeout = сміття НА КРЕМНІЇ (наш SOS-DR_0=SF12!); зафіксовано `firmware/test/ubsan_vendored.supp` (ASan-смуга гейтить решту MAC). Шлях: звірити Semtech upstream fix / cherry (👤-рішення: fork-патч submodule проти FW.47-недоторканості)
