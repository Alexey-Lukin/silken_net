...
- [ ] 🤖 cluster-sourced money-tx (`wallet=nil`: celo reward + last-tree slash) пише нуль MRV `AuditLog` — `record_money_audit_trail` бере org лише через `wallet&.organization_id`; fix `|| cluster&.organization_id` + regression-spec обох родин [поглинув ARCH.64-хвіст]
...
- [ ] 🔗 cluster-sourced (`wallet=nil`) money-tx audit-gap → **MRV.1** (систематичний: celo reward + last-tree slash, не celo-ексклюзив; `manual_review_depth` gauge тримає видимість доти)
