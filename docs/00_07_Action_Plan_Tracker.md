...
- [x] 🤖 **[deep-audit 2026-07-04] penalty_factor-uplift self-referential ✅ ВИПРАВЛЕНО** (`fd447bb6`-наступний): `comms_no_ack?` тепер whitelist саме node-offline типів (`queen_offline`/`queen_uplink_lost`/`system_fault`, §6 root-cause), а `critical_unmaintained?` виключає `vandalism_breach` — раніше обидва рахували ТУ САМУ `vandalism_breach`, що дала `positive_a?` → множник СИСТЕМАТИЧНО на стелі (2.0=`PENALTY_FACTOR_MAX`), не градуйований. Механізм лишається INERT (`slash_cause_uplift_enabled` default false) → 👤 активація uplift = чекбокс ↑ (перший). Дім `05_05 §6`.
...
- [x] 🤖 NatSpec 200→100 **✅** + guardian-CANCELLER **✅** (Safe = CANCELLER_ROLE → 48h veto actionable в 1 tx; `Deploy.s.sol` + `Deploy.t.sol` veto-тест; forge 221/0) — 2026-07-04
- [ ] 🤖 (залишок дешевих) gas-snapshot + coverage-floor CI-гейти (`solidity_audit.yml`)
