...
#### ARCH.50 — Celo reward persist-after-broadcast → double-pay (4-й ARCH.45 сиблінг, ЖИВИЙ)
- **P1** · 🤖 · ⚪ · → [`05_01`](05_01_Multichain_Architecture)
- **Стан:** Не усунуто (orchestration-аудит) — **ЖИВИЙ money-loss.** `Celo::CommunityRewardService` робить `client.transact` (5 cUSD on-chain, ~рядок 116), а durable-рядок `create_reward_transaction` пише **ПІСЛЯ** (~123, лише `if tx_hash.present?`) — немає `:pending`-intent ПЕРЕД broadcast. Крах/DB-блип після transact → кошти пішли, рядка нема → `CeloRewardWorker` retry:3 АБО задокументований daily double-fire (`InsightBatchCallbacks` + 02:00 `cluster_health_arbitration` cron) → `reward_already_sent?` false (виключає крах-вікно) → **друга виплата 5 cUSD**. Kredis-lock не рятує (серіалізує лише конкурентні, не послідовні retry; не keyed by cluster/date). Той самий патерн P1, що ARCH.45 закрив у slash/Solana-batch/Etherisc/anchor — Celo єдиний EVM-money-path, що broadcast-ить ПЕРЕД persist. Канон [`05_01`](05_01_Multichain_Architecture) / [`04_02`](04_02_Business_Logic_and_Services).
- [ ] 🤖 дзеркало slash: `:pending`-intent ПЕРЕД `transact` + `mark_as_sent!` після + ConfirmationWorker + escalate→`:manual_review` на ambiguous (як `BlockchainBurningService`)
- [ ] 🤖 spec: transact succeeds → persist fails → retry НЕ double-pay-ить (failure-path, якого зараз нема)
