...
| **FW.46 board-freeze** `.ioc` (👤) | повний `.elf` → увесь bench-день: `FW.2`-фліп · `FW.3/8/17/20/23/31/49/50/52/54/55` · `ARCH.26/41` · `SEC.2`-OTA-verify · `SEC.15`-WUT · `SEC.3`-SWD · `ARCH.35`-розводка · `radio_conf.h` → компіляція `radio.c` (SubGHz submodule + `RadioEvents_t` уже в репо — останній radio-гейт); суміжно `ARCH.34`-ефір: machine-half ПОВНІСТЮ ✅ 2026-07-05 (glue+adapter+host-smoke+форк v2.6.2-silken.1) — лишився OTAA в ефірі на bench (дім ARCH.34) |
...
- [x] 🤖 **[P0] VM_ERROR↔tamper ✅ ВИРІШЕНО «чесним decode» (founder-вибір A, 2026-07-05):** byte-level verify ПОГЛИБИВ знахідку — wire status=3 пише ВИКЛЮЧНО `BIO_STATUS_VM_ERROR` (mruby `pack_status_byte` повертає лише 0..2), а фізичний tamper їде PANIC_FLAG-каналом → старий мапінг був **інверсією** (positive-A канал = 100% софт-збої; справжня пилка недосяжна; + евристика `max_status≥3→1.0` робила кластерний OTA-баг тригером І damage-sizing'ом одразу = детермінований 100%-slash невинного кластера). Шиплено backend-only: enum `tamper_detected`→`vm_error` · `AlertDispatchService` → новий `firmware_fault(11)` (аналіз продовжується; НЕ в A-сеті/`comms_no_ack?`/`critical_unmaintained?`) · евристика `vm_error`→0.0 · `vandalized`-scope прибрано (мертвий) · `vandalism_breach` = чесно-порожній (лише ручна Field-Audit C→A ескалація, `06_08 §4`). **Наслідок-політика: авто-slash не має живого тригера до наповнення A-сету** (field-validated chainsaw 👤-чекбокс ↑ / майбутній HW tamper-канал: tamper-switch/SE05x — ⚠️ dead-колонка `telemetry_logs.tamper_detected` = потенційний landing pad). Дім `05_05 §3.2/§6/§7` + `04_01`/`04_02`.
- [x] 🤖 **chainsaw-гейт ✅ ВИПРАВЛЕНО (2026-07-05):** гейт тепер `panic? || bio_status_anomaly?` — реальна пилка (panic-TX, status=homeostasis + PANIC_FLAG) їде `chainsaw_detected` (non-fire Field-Audit маршрут), НЕ `seismic_anomaly`; PANIC-TX-маркер досяжний; бонус-фікс: `voltage_mv<100` скипається на panic-кадрах (panic несе vcap=0 by design → фантомний `system_fault` забруднював `comms_no_ack?` і з'їдав SEC.10-ліміт). Діаграма `03_03 §7.2` переписана (`EwsAlertCreatorService`→`AlertDispatchService`, чесний panic-флоу).
...
- [x] 👤 **Celo community-reward на vm_error-день ✅ ВИРІШЕНО founder'ом (2026-07-05): ПЛАТИТИ** — «не карати жертву» нашого бага; сенсорна половина кадру жива (зламаний лише Лоренц-статус), MAX-агрегат непропорційний (1 crash-кадр отруїв би добу), fraud-поверхня не росте (vm_error-маскування домінується фейк-homeostasis → ловить DCI/attest), емісія захищена окремо (vm_error → 0 GP per-frame). Хронічний vm_error = ops-тріаж (`firmware_fault`), НЕ reward-стеля (стеля «N днів поспіль» = YAGNI до реальної потреби). Пін = spec «stays eligible on a vm_error day» + NatSpec-коментар на `eligible_for_reward?`.
- [ ] 👤+🤖 (secondary, design-gated) tree-side `streamr_undelivered` сигнал-джерело (guarded→0) + repeat-offence вага. **Оцінка 2026-07-05: НЕ вільний 🤖** — Kredis `streamr_undelivered` = backend-side збій (StreamrBroadcastWorker не зміг ре-броадкаст) і за нот.12 НЕ сміє штрафувати; чесний tree-side сигнал = Streamr-gap × LoRa/CoAP-мовчання, але node-offline root-cause ВЖЕ покривають alert-типи `comms_no_ack?` (max()-комбінатор той самий) → нове джерело без ground-truth = спекулятивний дизайн при INERT-uplift'і. Будувати РАЗОМ з DAO-калібруванням uplift-активації (перший 👤-чекбокс). repeat-offence вага = DAO-дизайн там само.
- [x] 🤖 **`field_audit` dedup-ключ ✅ (2026-07-05):** `EwsAlert.escalate_field_audit!` (One-Home; exists?-skip + `RecordNotUnique`-rescue) + частковий unique-index `(cluster_id) WHERE alert_type=7 AND status=0 AND tree_id IS NULL` (squash-baseline `20260705090000`) — одна АКТИВНА cluster-level ескалація на кластер, resolve відкриває наступну. Всі 5 creation-сайтів через хелпер (freeze slash-гейта · evasion · blackout health-check · insurance arm_candidate!/no-data). tree-level дзеркало (`..._unique_active_per_tree`) вже було.
...
- [x] 🤖 **`trigger_event` creation-гарантія ✅ (2026-07-05):** трасування показало — прод-creation-шляху `ParametricInsurance` НЕ ІСНУЄ (лише seeds — перил ставлять — і фабрики; поліс-створення = майбутнє E.20/NaaS-onboarding). Замість трасування неіснуючого — структурна гарантія: `validates :trigger_event, presence: true` (полісів мало, KENOSIS hot-path не стосується) — будь-який майбутній шлях перил проставить, peril-honest маршрутизація не осліпне. «Оживлення на payout» відкинуто verify-by-data (фабрика-дефолт → vacuous-тест)
...
- [ ] 🤖 (nice-to-have, **API-gated** — оцінка 2026-07-05: НЕ вільний) точніша Etherisc DIP claim-status звірка (`getClaim` ABI) замість conservative recovery-`manual_review` — реальна DIP-інтеграція unwired, будувати ABI-звірку без живого API нема проти чого верифікувати (клас drought-Trigger-2); conservative `manual_review` безпечний. (ToucanBridge-хвіст знято — E.66 prune → §🗄️.)
- [x] 🤖 **EVM-mint `:processing`-orphan reconcile ✅ (2026-07-05):** `StuckSentTransactionSweeperWorker#escalate_stuck_processing!` — stale `:processing` (updated_at > 15min; created_at труїть reset-to-pending, ARCH.52 trap) → `escalate_to_review!` (`:manual_review`; ambiguous — мінт міг landed, tx_hash невідомий → політика ARCH.48/M6, НІКОЛИ blind re-mint). Double-mint і до того був неможливий (mint-шляхи фільтрують `:pending`) — це закрило observability/locked-balance хвіст. Дім [`04_02`](04_02_Business_Logic_and_Services) картка sweeper'а.
...
- [x] 🤖 **TOCTOU double-slash ✅ ЗАКРИТО (2026-07-05):** ОБИДВА прописані шляхи виявились мертвими verify-by-data (`blockchain_transactions` = `PARTITION BY RANGE(created_at)` → partial-unique мусить включати partition-key = вбиває dedup, ARCH.52-клас; `unique_for` = Enterprise-шим), а перша реалізація на `Rails.cache` впала на code-review — прод = **SolidCache, `unless_exist` НЕ атомарний для неіснуючого рядка** → шиплено **non-blocking `Kredis.lock`-claim** (`slash:claim:{id}`, TTL 2min; Redis SET NX + UUID-токен CAS-release): атомарно пускає одного у вікно guard→transact→mark_as_sent, конкурент → `Kredis::LockTimeout` → Sidekiq-retry бачить інтент переможця (`:sent` → re-arm / stale-`:pending` → supersede). `unique_for`-залежність позначена коментарем на `BurnCarbonTokensWorker` (Enterprise = докупка перед mainnet, defense-in-depth поверх claim). Дім [`04_02 §4`](04_02_Business_Logic_and_Services) картка `BlockchainBurningService`.
- [ ] 🤖 (residual, low) B1 retry-exhaustion (>5 retry → Dead Set, recovery-крони нема; рідко + PATH 1 latent). **[дедуп: раніше дублювався у ARCH.45 — тепер ЄДИНИЙ дім тут як Chainlink-сиблінг]**
