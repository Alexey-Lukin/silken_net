...
- [ ] 👤 bench: верифікувати CCM-двигун на STM32WLE5JC REVB (RM0461 §27.4; атестація скриптована — RUNBOOK §2.1 + 2.1b e2e uplink-day) + Flash-KV HAL-глю (спільний bench з FW.8/FW.17) → flip трьох прапорів = **ЄДИНИЙ HW-залежний пункт**
- [ ] 👤 два фліп-гейти рішень ДО/РАЗОМ з фліпом (деталі — flip-checklist [`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security)): (а) atomic-cutover кластера · (в) key-model (спільний ключ до ARCH.43). Гейт (б) health-blackout ✅ ЗНЯТО by design 2026-07-03 — пульс живе у QATT-v2 конверті (ARCH.54), від CCM-stride не залежить
