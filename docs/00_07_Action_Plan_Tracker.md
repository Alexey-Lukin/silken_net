...
- [ ] 👤 (micro, поведінковий флiп — code-review знахідка 2026-07-05) Celo community-reward eligibility на vm_error-день: після P0-reframe `stress_index` за день з crash-кадром = 0.0 (< 0.2 `MAX_STRESS_INDEX`) → день знову reward-eligible, хоча біостатус того дня НЕ спостерігався. Emission і так per-frame gated (vm_error → 0 GP), тож грошова емісія захищена; питання лише про community-reward: платити за день з транзієнтним софт-збоєм (наш баг — «не карати жертву») чи виключати день як неспостережений. Вирішити разом з uplift-активацією (A-сет гейт-кластер).
