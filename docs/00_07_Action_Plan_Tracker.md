...
> - **Meta-line** (рівно один, перший рядок): `**PN** · WHO · STAGE · → канон-реф`. `PN` ∈ `P0`–`P3`; `WHO` = `🤖`/`👤` (комбо `🤖+👤`, провідний перший); `STAGE` = `⚪`/`🟡`/`🟢`/`🔗`/`🌿` (рівно ОДИН, окремо від WHO); реф = канон код-спан або лінк із реальним `§X`. (Module = §-секція, вже enforced.)
> - **Тіло — тонкий вказівник, не копія канону.** Стан = суть + канон-pointer (повний опис у каноні) АБО 1-рядок ✅-резюме зробленого; відкрите → `[ ]` (`[x]` done · `[~]` частково) з WHO-тегом. Bench/validation-чек-лист (runbook) лишається.
> - **Чекбокси — вертикальним списком.** Кожен відкритий residual окремим рядком `- [ ] WHO — …` (читабельність + чисті diff'и, без glue-ризику). Один-єдиний residual можна інлайн (`· [ ] …`) одразу після ✅-резюме; ≥2 residual'и — завжди список. НЕ паковати кілька `· [ ]` в один рядок.
