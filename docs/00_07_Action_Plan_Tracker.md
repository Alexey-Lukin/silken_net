...
| **Web3 mainnet-деплой** = Gnosis Safe + `Deploy.s.sol` + transfer admin→Timelock (👤; `SEC.1`, поглинув BIZ.4) | `GOV.1`-read-path (ParameterSyncWorker no-op доти — активується цим деплоєм; контракт-half ключів ✅) · `E.32`-Hacken post-deploy · `S3.5`-subgraph cutover · `ARCH.4`-governance (архів) |
...
- **Стан:** deep-audit 2026-07-04 (S4, verify byte-level). `Governance::ParameterSyncWorker` синхронізує 13 параметрів у `SystemParameter`, АЛЕ споживачі читають **хардкод-константи**: `SilkenNet::Attractor` (`BASE_SIGMA/RHO/BETA`, `DT`, `ITERATIONS`), `ContractHealthCheckService` (`Rational(1,5)` slash-threshold), `AiInsight::SLASH_STRESS_THRESHOLD` (0.83), `TokenomicsEvaluatorWorker::EMISSION_THRESHOLD` (10_000) — жоден не читає назад. Тобто DAO-голосування за ці параметри сьогодні **не має ефекту**. **Нюанс:** 8 Lorenz-параметрів = свідомо DCI-locked (bit-identical Float з прошитим firmware — міняти через DAO зламало б device↔server parity), АЛЕ тоді синхронізувати їх = майбутня пастка; `emission/slash/stress` = просто недороблений read-path. **[2026-07-04 контракт-half ЗРОБЛЕНО]** у складі SLASH.2-сесії: `ProtocolParameters.sol` тепер має ключі `slash_gamma`/`slash_penalty_factor_max` (+named getters); fallback-price key перейменовано `scc_fallback_price_usd_cents`→`scc_fallback_price_usd` (усунено 100× unit-mismatch на джерелі — контракт тепер збігається з `PriceOracleService`/`SystemParameter(:scc_fallback_price_usd)`+`04_02`); Lorenz-ключі позначено DCI-locked NatSpec (бекенд свідомо їх не синхронізує); stress-threshold NatSpec `0.30`→`0.83` (був хибний). PP-тести оновлено, forge green. **Лишається BACKEND-half:** `PARAMETER_MAP` ще не тягне `slash_gamma`/`pf_max`/`scc_per_tonne`/`fallback_price`; `emission/slash/stress` read-path недороблений; clamp economic-параметрів у sync-воркері (bounds; `setParameter` без on-chain меж → мис-скейл tax=2e18 «200%» → backend `forester_amount<0` → mint-halt); доля Lorenz-sync (видалити з `PARAMETER_MAP` як DCI-пастку АБО read-only-mirror). Канон-чесність: `05_06 §7` «✅ Реалізовано» ще виправити при backend-half. Дім `05_06 §7`.
- [x] 🤖 контракт-half: `slash_gamma`/`pf_max` on-chain ключі + fallback-price rename (unit-fix) + Lorenz DCI-locked NatSpec + stress-NatSpec fix ✅ 2026-07-04
- [ ] 🤖 backend-half: узгодити `PARAMETER_MAP` ↔ `ProtocolParameters` ключі + read-path для `emission/slash/stress` (не-DCI) + clamp economic-параметрів у sync-воркері (bounds) + доля Lorenz-sync + `05_06 §7` канон-чесність
