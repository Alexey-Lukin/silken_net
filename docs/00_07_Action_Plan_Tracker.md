...
- **P1** · 🤖+👤 · 🟡 · → `01_01 §4.1`, `02_02 §1.2/§2.3`, `07_02 §1.2`
- **Стан:** Аудит-у-роботі — machine-аналіз ✅, матеріал-рішення pending (founder deep-dig 2026-06-21). Центральна провідна шина (анод EBFC V−/GND знизу → центральна pogo-площадка вгору, `01_01 §2/§4.1`, `02_02 §1.2`) специфікована як **вимога** (§4.1: «~1–2 мм², мінімальний переріз») і змодельована як **отвір** у CAD (`Zone1Anode`/`CathodeFlange` Ø1.3–1.6), але **фізична реалізація недовизначена й суперечлива** між `01_01`/`02_02`/CAD (founder deep-dig 2026-06-21). Чотири діри: **(1) матеріал** — `01_01 §4.1` пише «**анодний вал / Cu-провідник**» через слеш (Ti-вал Ø11≈95 мм² vs окремий Cu-дріт ~1.3 мм² — фізично різне; «1–2 мм²» має сенс лише для тонкого дроту, але збірка §3 + втулка bore Ø11 описують Ti-вал); **(2) ізоляція каналу** — на торці є PEEK-кільце ≥1.5 мм (`cathode_flange.json`), а ~50 мм стінки Ø1.3-каналу крізь **голий катодний Ti** — без специфікації (дріт у повітряному каналі при гойданні торкнеться стінки → коротке анод↔катод); **(3) нижній спай** — якщо Cu, то спай Ti↔Cu у вологому каналі (`02_02 §2.3` сам флагує Ti↔Cu ΔE>0.5 В → корозія міді); **(4) у BOM немає рядка** — `07_02` ховає в «Анкер Zone 1+3 $20–25» (мідь + спай + selective-gold торця не виокремлені). Раніше **не трекалося** жодним HW.
- **Термо-обґрунтування (in-silico, machine-half ✅ 2026-06-21):** `tools/in_silico/scripts/54_anchor_thermal_bridge.py` → `cache/mechanical/anchor_thermal_bridge.json`. 1D резистивна **драбина** (PEEK-стінка Ø11→Ø15 безперервна ∥ центр: Z3-shank → зазор → Z1-shank, усе ∥ шина) + 2-вузловий steady-state анодної кишені (холодний фланець ↔ тепле ядро через wood-spreading). Знахідки: **(а)** Cu-шина = **домінантний осьовий холодовий шлях** — G_anchor(Cu) ≈ **10× Ti-шини**, несе ~98% тепла крізь розрив; **(б)** при −30°C повітря / +2°C ядро Cu тягне анодну кишеню до **−15°C (17° нижче ядра, freeze-risk)**, а Ti-шина **термічно невидима** (−1.4°C ≈ як без шини, в межах 2°C); робастно на 48-точковій сітці (Cu завжди на 5–15°C холодніший); **(в)** електрично swap **безкоштовний** (µA → sub-mV IR-drop навіть для 100× гіршого ρ титану). **Рекомендація:** Ti-6Al-4V шина **монолітна з анодом** (той самий SLM-друк) — знімає і термоміст, **і** нижній гал-спай Ti↔Cu одним рухом. Матеріал = **founder-рішення → НЕ baseline-канон** (no-premature-canon). Caveat: 1D lumped/steady-state; абсолютна T залежить від (swept) wood-резервуара — ранжування Cu≫Ti робастне; conjugate FEA = bench. Трек повного анкера TRL 4→6 — **НЕ блокує coin** (обходить PEEK/шину).
- [ ] 👤 **Матеріал-рішення:** Cu (спец) vs **Ti-monolithic** (in-silico рекомендує) vs 316SS — обрати; якщо Ti-monolithic → §4.1 «анодний вал / Cu» розв'язується, (1)+(3) зникають
- [ ] 🤖+👤 **Ізоляція каналу:** спец стінки Ø1.3-каналу крізь катодний Ti (PEEK-лайнер / емальований дріт / монолітний Ti що НЕ торкається) — наразі лише торцеве кільце
- [ ] 👤 **Нижній спай (якщо Cu):** метод Ti↔Cu з'єднання + ущільнення проти ксилемної вологи (або усунуто Ti-monolithic)
- [ ] 🤖 **BOM-рядок:** виокремити шину (провідник + спай + selective-gold торець) у `07_02 §1.2` / `01_02` — наразі лампується в анкер
- [ ] 🤖 **Doc↔CAD дрейф Ø:** `01_01 §1` «~Ø1.3» vs анодні CEM Ø1.4–1.8 — звести до одного числа
- [ ] 👤 **Shank-insertion ↔ розрив (ties HW.8.9):** Ti-вали містять 44/50 мм PEEK — справжній розрив = зазор `L_g`; інсерція = термо↔press-fit важіль (placeholder F2)
