...
> - **Meta-line** (рівно один, перший рядок): `**PN** · executor · → канон-реф`. `PN` ∈ `P0`–`P3`; `executor` = `🤖`/`👤`/`🔗` (+ `🌿` = far-horizon/post-TRL), комбо через `+`, провідний перший (`👤+🤖`) — не голим склеюванням (`👤🤖`); реф = канон код-спан або лінк із реальним `§X`.
