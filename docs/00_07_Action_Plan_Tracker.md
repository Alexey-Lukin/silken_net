...
- [x] 🤖 IaC-scan baseline-triage ✅ (2026-07-09, post-1st-CI-run) — 8 Trivy-findings review'нуто індивідуально (НЕ blanket-ignore): **3 REAL fix у terraform** (GCP-0030 `block-project-ssh-keys` · GCP-0020/0014 db `log_lock_waits`/`log_temp_files`) + **5 by-design** → `.trivyignore` з обґрунтуванням кожного (GCP-0074 deny-all-misflag · GCP-0017/0031 Akash/Anchor-public-IP mitigated · GCP-0033 CSEK-YAGNI TRL-3 · DS-0026 Kamal-healthcheck). ⚠️ drift `init` передумова = GCS state-bucket [S5.6] провіжн РАЗОМ з `GCP_SA_KEY`
