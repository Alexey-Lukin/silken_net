...
- **P1** · 🤖+👤 · 🟢 · → `06_04`
- **Стан:** Machine-half ✅ — set-diff `ENV.fetch`/`credentials.dig` vs deploy закрито: 10 контракт-адрес + `POLYGON_RPC_URL`/`CELO_RPC_URL` заведено в Kamal + Akash web/job (контракт-адреси = `REQUIRED_SECRET_NOT_SET` placeholder, deploy-order → не terraform-vars); `06_04 §2.2` +`aws`/`gcs`. ⚠️ `CELO_RPC_URL` порожній → fallback Alfajores TESTNET (обходить `web3_network_guard`; E.49) → mainnet обов'язковий. **Drift-guard ✅ 2026-07-10 (одноразовий set-diff → durable, deep-dig surface-топології):** `spec/deploy/env_fetch_declaration_spec.rb` — 15 `ENV.fetch`-без-default (crash-on-absence) звірено проти **ПРАВИЛЬНОГО набору поверхонь** (не 2, а до 4): clear-var (контракт-адреса) = deploy.yml env.clear + Akash SDL; secret-var (signing-ключ) = deploy.yml env.secret → `.kamal/secrets-common` (`$VAR`) → **workflow env: блок** (CI-інжект) → SDL. Example #3 генералізує на **ВСІ** 24 env.secret (B1/INF.19 — 4-місячний deploy-block root: env.secret без workflow-мапінгу = порожній інжект → boot-crash за зеленим verify; був лише comment-guarded «MUST mirror», тепер авто-гейт) з WIF-винятком (`GCP_ARTIFACT_REGISTRY_KEY` = auth-step output, не GitHub Secret). + activation-gated aux off усіх поверхонь (SEC.22). Ловить майбутній `ENV.fetch("NEW")` забутий на будь-якій поверхні. Інвентар — [`06_04 §2.1`](06_04_Secrets_Checklist).
