...
- [ ] 🤖 (investigate, named P2-sibling) Chainlink dispatch «double-request» seam — `Chainlink::OracleDispatchService`/`ChainlinkDispatchWorker`: guard×rescue чи crash між Functions-request і `oracle_status`-set → повторний request на той самий лог (LINK-cost; потенц. double-callback→mint). Unverified, dispatch hot per-uplink. Окреме від ARCH.49 (nonce-race, P3, закрито) / SEC.5 (callback-HMAC). Дзеркало ARCH.48 guard×rescue + P1 persist-before-request (`project_money_path_orchestration` P2 taxonomy).
