...
#### ARCH.46 — Damage-ratio no-data → повне вилучення (full-burn fallback)
- **P2** · 🤖+👤 · ⚪ · → [`05_05 §3`](05_05_Slashing_and_Risk_Policy)
- **Стан:** Знахідка (verified F2 + прямий read) — `BlockchainBurningService#calculate_damage_ratio` у термінальній гілці «нема AiInsight-даних І нема `@source_tree`» повертає `1.0` = повне вилучення (100% кластера). Гілка досяжна ЛИШЕ за пройденого positive-A gate (`positive_a_evidence?` — отже tamper підтверджено), але якщо в день tamper'у `AiInsight.daily_health_summary` порожній (AI-pipeline outage / stale insights / TZ-edge) → `critical_count = 0` + `@source_tree = nil` → 100% burn замість пропорційного `critical_count/total_trees`. Data-availability амплифікує severity: легітимний tamper + збіг AI-data-gap = максимальне покарання. Коментар гілки натякає на intent («повне вилучення»), але канон [`05_05 §3`](05_05_Slashing_and_Risk_Policy) цього явно не обґрунтовує. НЕ горить (gated на tamper), рішення потрібне ДО mainnet-слешингу.
- [ ] 👤 рішення: (а) лишити worst-case-by-design → канонізувати rationale в [`05_05 §3`](05_05_Slashing_and_Risk_Policy); або (б) додати floor (мін. пропорційна частка / `:frozen` Field-Audit при zero-data замість 100%). Без коду до рішення.
- [ ] 🤖 після рішення (b): zero-data-гілка → `freeze_for_field_audit!` (дзеркало positive-A-fail) або floored ratio + тест на гілку.
