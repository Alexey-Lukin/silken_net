...
- **Стан:** Піднято з 📌 Backlog (2026-07-02). Рандеву-драбина канонізована [`03_01 §1.9`](03_01_Firmware_Lifecycle_and_DMA): L1 Queen always-on ✅ · **L2 TDMA-вікна 🟡 host-half (2026-07-02)** · L3 CAD ❌. L2 host-half shipped: One-Home математика `firmware/common/tdma_schedule.h` (pack/parse fail-closed, next-window сітка — готовий вхід WUT-армінгу, in-window, DID-слоти для FW.27-A) + Queen пише байти 5..8 маяка + Soldier кеш-parse у Сценарії 0 — обидва за дзеркальними гейтами `ARCH26_TDMA_ENABLED 0` (wire `period=0`⇔off → нинішній нульовий ефір сумісний) + `test_tdma_schedule.c` (`make tdma`); кеш RAM-only (нуль нових RTC DR/Flash-KV). Wire-дім [`03_02 §5а.2а`](03_02_Queen_Gateway_Firmware) (incl. стеля фазової точності ±1 с + `ts_frac`-апгрейд byte 11). ⚠️ Енерго-корекція тепер покриває і L2 ([`03_01 §1.9`](03_01_Firmware_Lifecycle_and_DMA) другий ⚠️-блок): суцільний RX у вікні ≈2.9 Дж/добу >> EBFC-харвест → **політика ролей** (full-RX лише Провідник; Солдат = TX-слоти + майбутній L3 CAD-нюх); миттєвий PANIC = extended-preamble відправника. Передумови landed: роль ✅ ARCH.27 (`g_node_role` — [`03_01 §1.11`](03_01_Firmware_Lifecycle_and_DMA)); gossip-точність ±128 с << потрібних ±10 мс → точний sync лише з beacon + LSE bench (FW.49). Розблоковує: FW.27 Design A ([`03_02 §5.1.4`](03_02_Queen_Gateway_Firmware)) · ARCH.43 (рандеву-передумова) · mesh-TTL rev2-рішення ([`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security)) · stigmergy-зворотний шлях ([`00_08 §1`](00_08_Beyond_TRL9_Planetary_Roadmap)). Активація post-TRL 6.
- [x] ✅ 🤖 L2 host-half (2026-07-02): Queen слот-розкладка у beacon TDMA-резерві (байти 5..8) + Soldier parse розкладу → next-window математика під RTC-WUT + host-тести — див. Стан + [`03_02 §5а.2а`](03_02_Queen_Gateway_Firmware)
- [ ] 🤖 L3 host-half: CAD-глю за role-прапорцем (gated, лише `ROLE_PROVISIONER`) + PANIC extended-preamble TX + CAD-енергобюджет у канон (політика-рамка вже стоїть — [`03_01 §1.9`](03_01_Firmware_Lifecycle_and_DMA))
- [ ] 👤 bench: LSE drift проти ±10 мс sync-бюджету + RTC sub-second/LPTIM wake + CAD енергопрофіль (PPK2) + фліп `ARCH26_TDMA_ENABLED` (WUT-армінг — спільний bench з FW.49/FW.20/SEC.15)
...
- [ ] 🤖 DI-знахідка (2026-07-02) — **pre-req до будь-якого non-ENV adapter'а (Bitwarden/HSM)**: `MasterKeySource#fetch_master_key` = лише preflight-гейт (`Session#preflight!` відкидає результат), а HKDF-деривація читає `ENV["PROVISIONING_MASTER_KEY"]` напряму (`HardwareKeyService`/`OtaHmacKeyService`) → non-ENV adapter був би мертвим кодом (ключ «з vault» на словах, з ENV по факту); спершу DI-рефакторинг (ключ параметром від Session до обох сервісів)
...
- **Стан:** Рішення (founder) = **true-DePIN** («голос дерева» — non-extractable Ed25519, що не підробить ні backend, ні оператор) → SE: ATECC608B (P-256, не тримає голос жодного ланцюга) → **SE050** (суперсет: Ed25519/EdDSA + AES-128/256 + monotonic counters + secure storage). AES-128 LoRa = свідомий вибір (не SE-constraint); ladder L0→L1→L2 (energy-gated); soft-freeze (footprint DNP, populate на mass post-FW.2). **Host/doc-side migration done** (ADR + Slot-1→Ed25519 + AES-128 One-Home-collapse + legacy-banner + true-DePIN honesty-pass: pipeline-integrity ⟂ physical-origin + trust-origin ladder дім); код / eval-kit / L1-L2 — residuals нижче. Ціна (DigiKey-зріз: $4.50/1 → $2.50/1k) — founder: не проблема. **Paper-half datasheet-verify ✅ + HIL L1 e2e ✅ + baseline-uplift SE051C2 (founder, 2026-07-02)** — SE051 = та сама платформа (струми/корпус/I²C ≡) + CCM on-chip + SEMS Lite оновлюваність; SE050C2 = fallback, спільний footprint. Деталі [`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security) datasheet-verify таблиця. Канон [`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security) + ladder [`05_02`](05_02_Proof_of_Growth_Pipeline).
...
- [ ] 👤 SEC.6 hardware (datasheet-half ✅ + baseline = **SE051C2** 2026-07-02 — paper-verify канонізовано [`03_05 §3.7`](03_05_Hardware_Symmetric_Crypto_and_Security): SE051 = SE050-платформа + CCM on-chip + SEMS Lite; Ed25519 = конфіг C/E, F-FIPS відпадає · DPD 3–5 µA, «150 нА» був ATECC-проксі): звірити ціну/сток SE051C2 + замовити eval-пару **SE051-кіт** (OM-SE051ARD? / Mikroe Click) + **OM-SE050ARD-E** companion (НЕ -F) + silicon-confirm paper-чисел (**cold-boot заряд = головне питання** — датащит не специфікує) + SEC.14 role + mass-BOM populate **разом з load-switch гейтом SE** (TPS22860; DPD > Scenario-C margin → повне зняття живлення)
- [~] 🤖+👤 L1 Queen-attestation — 🤖 **shipped**: firmware Queen software-Ed25519 (Monocypher, pinned) підписує CoAP-батч (EDSK Protected Flash; конверт `firmware/common/queen_attest.h`) + backend verify-до-decrypt у `UnpackTelemetryWorker` (nonce anti-replay; маркери `gateways.last_attested_at`/`telemetry_logs.gateway_attested`) + factory EDSK + закрито `Load_CoAP_Key` TODO; parity host C↔RSpec golden-KAT. Wire [`03_05 §2.2`](03_05_Hardware_Symmetric_Crypto_and_Security), ladder-статус [`05_02`](05_02_Proof_of_Growth_Pipeline). Лишилось:
  ...
  - [x] ✅ HIL `queen_simulator` signed-режим (2026-07-02) — QATT-конверт як 4-та незалежна реалізація (Monocypher ↔ OpenSSL ↔ worker-spec ↔ HIL) + e2e `spec/integration/qatt_hil_e2e_spec.rb`: повний ланцюг simulator→`UnpackTelemetryWorker`→БД-маркери без заліза (attested · tamper · replay · unverified · legacy)
  - [ ] 🤖 crash-window смел (знахідка design-review 2026-07-02, ARCH.45-сімейний): QATT-nonce реєструється ДО `TelemetryUnpackerService.call` → падіння сервісу + Sidekiq-retry = `attest_replay`-reject = батч втрачено назавжди; лік = nonce-персист після успішного unpack (intent-marker патерн ARCH.45)
...
- [ ] 👤 доля PATH 1 oracle-callback: **замкнути** (DON Functions JS-source + consumer `fulfillRequest` + external-adapter, що POST-ить requestId↔log) АБО **чесно демоутити** (прибрати on-chain `sendRequest` LINK-cost, лишити tokenomics+clawback як модель) — впливає на DePIN-наратив (Chainlink Core/audit-critical у [`05_01`](05_01_Multichain_Architecture))
- [ ] 🤖 A9 Chainlink double-request intent-marker — КОЛИ PATH 1 замкнено (зараз latent: LINK-only, replay-capped)
