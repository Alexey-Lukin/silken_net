...
- **P2** · 🤖+👤 · 🟡 · → [`05_02`](05_02_Proof_of_Growth_Pipeline)
- **Стан:** Verify-by-data (dig 2026-06-28, з ARCH.45-Chainlink нори) розкрив: SCC мінтить **оптимістично** на `growth_points` — IoTeX-ZK + Chainlink-DON НЕ enforced на живому шляху (PATH 2 tokenomics guard-free за §DOC.7; єдиний gated PATH 1 oracle-callback **unwired** — DON Functions JS-source / consumer-контракт `fulfillRequest` / relayer відсутні, зберігаємо EVM tx_hash а DON ключить requestId). Це **by-design** (дзеркало E.63 Lorenz=anti-fraud-не-gate; SE050-MIGRATION L2 «ex-post reconcile+clawback, pre-mint=North-Star»), але канон **переобіцяв**: внутрішня суперечність 05_02 «Ключовий інваріант» (mint blocked без oracle) vs §DOC.7 PATH 2 «✗ does NOT need oracle» + хибний інваріант «growth_points вже верифіковані» (credit передує verify). Напрям = чесність канону, не код під переобіцянку. Enforcement, що виправдовує оптимізм (ex-post clawback device-Merkle↔намінтоване → `slash()`) — **не збудований**, трекано ARCH.12 (MerkleTree) + SE050-MIGRATION (L2). Trust-origin per-packet: L0-беззастережно / L1-Queen-attest soft-marker (не gate) / L2-unbuilt — SE050-MIGRATION. Канон [`05_02`](05_02_Proof_of_Growth_Pipeline) · [`05_01`](05_01_Multichain_Architecture) · [`05_05 §4`](05_05_Slashing_and_Risk_Policy).
- [x] 🤖 канон-reconcile — чесна trust-model у [`05_02`](05_02_Proof_of_Growth_Pipeline) (оптимізм + ladder + clawback=North-Star, «Ключовий інваріант»→«Модель довіри», зведено з DOC.7) + [`05_01`](05_01_Multichain_Architecture) Chainlink-Core-but-unwired + хибний коментар `blockchain_minting_service`
- [x] 🤖 B2 (live leak) — Dclimate fraud→slash enqueue-stranding закрито **reorder'ом** (slash enqueue ПЕРЕД `update!(:rejected_fraud)` → worker-retry переграє ідемпотентно; вичерпані 15 retry → :inconclusive→DAO) + crash-window spec. Канон [`05_05 §4`](05_05_Slashing_and_Risk_Policy)
- [x] 🤖 PuroEarth — double-anchor + ConfirmationWorker-gap закрито idempotency-guard'ами (skip re-anchor якщо tx_hash є; confirmation поза anchor-guard → retry переплановує; Phase-2 skip якщо corc_ref є) + spec. Канон [`04_02`](04_02_Business_Logic_and_Services)
- [x] 🤖 B1 — Iotex→Chainlink enqueue-stranding закрито **smart-guard'ом** (на retry: verified але chainlink_request_id nil + oracle pending → переenqueue) + spec. Канон [`05_02`](05_02_Proof_of_Growth_Pipeline)
