...
- [ ] 🔗 активація після CCM-flip: фліп трьох гейтів (Soldier + Queen + backend ENV) + глибина черги під per-device CCM-батч cluster-wide ротації + bench (re-key CRYP, Flash-KV erase/program на кремнії)
