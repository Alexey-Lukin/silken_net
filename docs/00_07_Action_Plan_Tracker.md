...
| **FW.46 board-freeze** `.ioc` (👤) | повний `.elf` → увесь bench-день: `FW.2`-фліп · `FW.3/8/17/20/23/31/49/50/52/54/55` · `ARCH.26/41` · `SEC.2`-OTA-verify · `SEC.15`-WUT · `SEC.3`-SWD · `ARCH.35`-розводка · `radio_conf.h` → компіляція `radio.c` (SubGHz submodule + `RadioEvents_t` уже в репо — останній radio-гейт); суміжно `ARCH.34`-firmware (glue-обв'язка ✅ 2026-07-04; ⚠️ vendored `extern/subghz-phy/lorawan` = LBM radio-шар (SWL2001), НЕ MAC-стек — LoRaMac-node = окремий submodule `stm32-mw-lorawan`, дім ARCH.34) |
...
- [x] 🤖 dashboard-gap'и закрито (2026-07-04, Sonnet-агент): дашборд 36 панелей — нові секції «⚙️ Process/Runtime Health» (RSS/GC/heap/threads/Puma-pool — усі runtime-метрики реєстру) + «👑 Gateway/Queen Fleet Health» (gateways_faulty · attest_lapsed · helium_sos by outcome) · insurance-SLO = 4-й ряд SLO-панелі · 3 money-path stat-панелі (manual_review_depth/limbo_locked/chain_audit_delta, `max()` — дзеркало alert-виразів) у «💰 Money-Path Reliability» · нове правило `sn-alert-gateway-flapping` (p2/info, `increase(gateways_offline_total[30m]) > 2` — 1 стабільний збій інкрементить РІВНО раз, faulty поза скоупом сапера, тож >2 = справжній flap) → правил 22 · README регенеровано з yaml (3 дрейфи severity/for + 3 відсутні правила — більше, ніж думав item) · CI-гейт `import.rb --dry-run` = крок у `alloy_config_validate` (path-гейт `deploy/**` уже покриває grafana; dry-run без креденшелів, звірено живим запуском)
...
- **Стан:** **Backend-half ✅ (2026-07-03, пакет ARCH.54):** `POST /api/v1/telemetry/helium` (`HeliumSosController`, HMAC `X-Helium-Signature` — SEC.5-патерн oracle_callbacks + Rack::Attack 30/хв) → `HeliumSosWorker` (черга alerts: 12B-wire parse, подвійна ідентичність `helium_dev_eui`↔`queen_did`-у-кадрі, ідемпотентний `EwsAlert(queen_uplink_lost)` + негайний `report_fault!`) — специ 15/15. **Wire 12B заморожено** (📐 One-Home [`06_08 §1.2`](06_08_Resilience_and_Failover_Policy)); канон-конфлікт lambda-summary vs SOS-only розрубано на SOS-only (рішення 2026-07-03; `02_05 §6.1` вичищено). **Firmware owned-half ✅ (2026-07-04):** `queen/helium_sos.h` (pure: 12B-pack байт-парність із фікстурою `HeliumSosWorker`-спеки + тригер-предикат канону + бюджет сліпоти wrap-safe + DID-парсер uid; host-тести `make -C firmware/test helium`, ASan ✅) + main.c обв'язка `queen_helium_lorawan_uplink()` за повним hard-rule `02_05 §6.1` (pre/post IWDG · безумовний `Radio_Reinit_RawLoRa_868MHz()` + `Restore_ECB_Mode()` · перевищення бюджету свідомо годує IWDG-reset) + тригер у main loop (компілюється КОЖНОЮ збіркою; гейт `ARCH34_HELIUM_ENABLED 0` вирізає лише MAC-шов `Helium_Mac_SendSos`). ⚠️ **Міф розвінчано:** vendored `extern/subghz-phy/lorawan` = LBM radio-шар (SWL2001 v4.8.0), НЕ LoRaMac-node — MAC-стек у репо ВІДСУТНІЙ; permission-гейт не дав завендорити автономно. [transitional] fill-джерело тригера = дедуплікований CIFO — кластер <25 Солдатів не набере 50% (чесний append-fill дасть ARCH.35-ринг; founder-Q: чи послабити buffer-умову до ринга?). vcap_mv у кадрі = 0 («не виміряно») до власного ADC-каналу батареї (board-freeze).
- [x] 👤 MAC-стек завендорено (founder 2026-07-05): `extern/stm32-mw-lorawan` @v2.6.2 (SHA `3b2c8b73`, BSD-3, ST-форк LoRaMac-node; Mac+Region / LmHandler / Crypto soft-se / Conf `_template.h` = owned-glue за FW.47; pin — [`03_01 §12.5`](03_01_Firmware_Lifecycle_and_DMA))
- [x] 🤖 adapter-TU ✅ (2026-07-05): `queen/lorawan_glue/` — owned-конфіги (lorawan_conf EU868-only/ClassA/CTX_MGMT=0 · se-identity нулі-плейсхолдери · Commissioning · platform/utilities_conf · mw_log no-op) + pure `soft_timer`/`systime` (host-тести) + `helium_mac.c` (LmHandler-епізод: Init→fresh-OTAA-join→Send 12B; **DevNonce** — єдиний персист між join'ами (FCntUp не потрібен: fresh-сесія legal) — `MIB_NVM_CTXS`→flash_kv 0x30, `Helium_Mac_Bind_Nvm(kv)` до bench-mount чесно ephemeral). **Host-smoke СПРАВЖНЬОГО MAC** (`make -C firmware/test helium_mac_smoke`): стаб-радіо ловить криптографічно валідний 23B JoinRequest (MHDR=0x00) + DevNonce-монотонність між епізодами; ARM compile-lane `lorawan_mac_check` (CMake hal-lane); cppcheck/ASan/UBSan зелені
- [x] ✅ **bench-OTAA блокер ЗНЯТО (2026-07-05, founder-go на fork):** vendored UB `RegionCommonComputeSymbolTimeLoRa` — `(1<<phyDr)*1000000` signed-overflow при SF11/SF12 (наш SOS-DR_0=SF12; Semtech upstream master МАЄ ту саму діру — звірено). Submodule перепінено на форк `Alexey-Lukin/stm32-mw-lorawan` @ **v2.6.2-silken.1** (= v2.6.2 + 1 fix-коміт, uint32-каст); suppression-милиці знято, ASan/UBSan-смуга знову сувора (`-fno-sanitize-recover=all`)
- [ ] 👤 upstream-PR: подати фікс у Semtech `Lora-net/LoRaMac-node` (+ дзеркально ST issue) — відкритий внесок; при прийнятті майбутній ST-реліз зніме потребу форка
