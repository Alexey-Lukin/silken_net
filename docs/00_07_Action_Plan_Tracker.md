...
- [ ] 👤 **Стаття 1 pre-submission punch-list** (self-review 2026-06-16 через [`self_review_checklist`](protocols/paper/self_review_checklist.md); нуль фабрикацій / нуль overclaim). **Потребує наукового рішення (свіжа сесія):** (1) **citation-manager прохід** (Zotero/Paperpile) — in-text↔scaffold розбіжності: `Degani & Heller 1989` (in-text) vs `1987` (scaffold `j100290a001`) — обидві реальні різні роботи, вибрати; `«Mano et al. 2003»` → ймовірно **Mao** (scaffold `ja029510e` = Mao/Mano/Heller); `«QM/MM flavin 2015»` не мапиться (scaffold = Bhattacharyya **2007**); Bhattacharyya `PMC4480342` → DOI `10.1021/jp071526+`; journal-shorthand (`Chem.Rev.2019`/`JPCB 2024`/`JPCC`) → author-year; **потім зовнішня DOI-existence перевірка** (gray=FAIL). (2) **Os +200 mV** — запінити primary-cite (як FAD-GDH −265 Schachinger 2022). (3) дрібниця: intro `~1.5 nm` vs §3.1 computed `16.0 Å` (=1.6 nm) — вирівняти. **✅ Вже закрито цією сесією:** AI-disclosure (`paper/08_declarations.md`, Gate 4) · devil's-advocate error-fitting hardening (§3.5). AF3-geometry — адекватно покрито (pLDDT≫80 + MD-ансамбль), не блокер.
