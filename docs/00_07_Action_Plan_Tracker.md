...
> 🧭 **Анкерний кущ — synergy map (dependency-DAG; оновлено 2026-06-20: дим-фриз + CAD-родина ЗАКРИТІ).** Анкер-айтеми зшиті в граф — пакетами, не поштучно. **[A] Дим-фриз ✅ DONE** (радіальний: вал Ø11 · PEEK стінка 2 · рана Ø15; осьовий: Zone 2 = 50 · фланець Ø25 · Z-stack spacer+bayonet · O-ring 15–30 %; HW.33 + HW.8 + HW.26 — Lamé/CODIT/3-spring data-grounded; анти-дрейф guard `00_06 §3`). **[B] PicoGK CAD-родина ✅ COMPLETE** (`tools/cad` — **ЛІНЧПІН**: coin✅/anode-v2✅/ARCH.25✅/barbs✅/Деталь3✅/Деталь4-radome✅ — in-silico прокси Архімед/µCT/AM-island/EAAE; **machine-half анкера закрита**). **→ фокус зміщується на ФІЗИЧНЕ:** **[C] Завод-пакет** (HW.23 HIP · HW.27 bake · HW.2 EAAE · coating-map `01_02 §3.6`) · **[D] Staged validation** (HW.24 SLA→coin→anchor→100 · HW.3 aging — bench/потенціостат) · **[E] Збірка** (HW.17 radome HFSS/друк · HW.25 PTFE-GDL · HW.28 shield · HW.8 pogo bench · HW.6 install · HW.22 steril). Хімія HW.5 → coin/anchor. Залишок machine: MATE-Ø (HW.17) · script 50-sync (HW.3.IS). Аудит-знахідки + founder-рішення → **HW.33**.

#### HW.1 — nTop model → SLM+HIP factory (Anode Zone 1)
- **P0** · 👤 · ⚪ · → `01_01`, `01_02 §1.7`
- **Стан:** Фабрична генерація тризонного анкера НЕ розпочата (Zone 1 SLM+HIP, Zone 3 SLM/EBM, Zone 2 CNC+annealing 200–250°C). Канон `01_01 §1/§5.5`, `01_02 §1.6/§1.7/§3.6`. **Code-as-CAD трек PicoGK** (`tools/cad/`, .NET 9, PicoGK 2.2 — primary; nTop-ліцензія є, але в nTop ~нуль): scaffold + Ti-coin + **анкер v1/v2-graded** + **ARCH.25 двофазна connectivity-валідація** SHIPPED 2026-06-20/21 (пайплайн end-to-end; код `60723a8`+`a5ed3d9`, усе в main); **barbs v2b + Деталь 3 катод-фланець + Деталь 4 radome v2c SHIPPED** (`207d104`/`404edfe` — §4.3 lock + `CathodeFlange.cs`/`Radome.cs`); **🏁 PicoGK анкер-CAD-родина ПОВНА — machine-half анкера закрита** (coin→anode-v2→ARCH.25→barbs→Деталь3→Деталь4). 👤-residual: фіз-друк/HIP. Геометрія-аудит + sheet→network → **HW.33**.
...
- [x] 🤖 HW.1.PicoGK: **Деталь 3 (Zone 3 катод-фланець) ✅ 2026-06-20** (`CathodeFlange.cs`, main) — solid Ti фланець **Ø25** (frozen) + reuse §4.3 barbs+DIN-471 shank (Ø9 placeholder, ShankCem-mapping) + 3 **bayonet-lugs** (radial) + bus-bore Ø1.3 + O-ring groove; verify OK (solid 2266мм³ · barbs 3 · lugs 3 · cathode-side 2.36см²) + 3 xUnit (28 total)
- [x] 🤖 HW.1.PicoGK: **Деталь 4 radome v2c ✅ 2026-06-20** (`Radome.cs`, main) — hollow PEEK купол Ø25 + shield-дзвін ≥3/R≥5 + bayonet-socket (L-slot, mate Деталь-3 lugs) + PCB cavity (antenna↔Ti≥12) + rim O-ring groove; verify OK (hollow 71% · bell-rise 12.5 · cavity 13 · mate-fit) + 3 xUnit (**31 total**). **🏁 PicoGK анкер-CAD-родина ПОВНА** (coin→anode-v2→ARCH.25→barbs→Деталь3→Деталь4); machine-half анкера закрита. ⚠ MATE-Ø (lugs Ø29 ↔ radome Ø25) → HW.17 👤 reconcile
...
- **P2** · 🤖 · 🟢 · → `01_02 §6`, `08_02 §1B`
- **Стан:** Машинну половину закрито (2026-06-21, commit `a5ed3d9` — `tools/cad/Connectivity.cs`; self-owned, не чекаючи Онищенко/Порубльова). Двофазний топологічний аудит = SDF-семпл власного гіроїда + flood-fill (6-зв'язність, Hoshen-Kopelman-стиль) → один обчислювальний прокси **чотирьох** фіз-гейтів: open-pore↔Архімед ([`01_02 §1.2`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)), pore-percolation↔µCT + EAAE flow-through ([`01_02 §1.3`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS), FM-B gas-lock), solid-disconnected↔AM floating-island + електро-цілісність аноду ([`01_01 §1`](01_01_Coaxial_Gyroid_Topology_and_PEEK)), closed-pore↔trapped-powder. Pure-managed → display-less xUnit. 7-SKU sweep + 12 тестів green: open=100%, closed=0%, percolate=XYZ скрізь. **Adaptive-resolution знахідка:** solid-зв'язність вимагає кроку ≤ стінка/2 (≈період/16) — груба сітка рве тонку стінку у фальшиві «острівці» (24–61% @ 0.3мм → ~0% adaptive); pore-фаза стабільна за будь-якого кроку. Лишається важка C++/CFD topological-integrity нога. Канон [`01_02 §6`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS).
...
- [x] 🤖 **Деталь 4 radome v2c PicoGK ✅ 2026-06-20** (`Radome.cs`) — hollow PEEK купол Ø25 + shield-дзвін (≥3/R≥5) + bayonet-socket (L-slot) + PCB cavity + rim O-ring groove; verify OK (hollow 71% · bell 12.5 · cavity 13 · mate-fit) + 3 xUnit (31 total). catode-O₂ = **периметр** (canon-fix `01_04 §5.5` ✅). ⚠ **MATE-Ø flag:** lugs→Ø29 vs frozen Ø25 → enclosing-skirt/lugs-inboard = 👤 HW.17 reconcile
...
| ID | Пункт | Канон |
|----|-------|-------|
| DOC-T.27 | 🤖 🟡 **Deep-audit §05/§06/§07/§08** — той самий deep-archival метод, що застосовано до §04: per-item verify-canon ЧИТАННЯМ дому → cement (Стан-вердикт, без done-`[x]`) → re-examine priority/WHO/STAGE → archive resolved + promote misclassified backlog-блокери → fix drift по нитці. **§05 ✅ 2026-06-16:** 7 items code↔canon-verified (всі real, 0 phantom); drift swept по нитці — 00_01 §6.x→05_05 code-sweep · batchMint cap 200→100 · blackout-SLA row · dangling E.7 · SLASH-1 cause-routing honesty-gap. **§06 ✅ 2026-06-16:** 18 items claim-verified проти реального коду (всі real, 0 phantom); drift swept по нитці — DOC-T.29 line-ref kill (`ClassName:NNN`+укр-проза, 06_02/03/04/07_01 + guard розширено) · 06_01 max_conn «2×5»→4×3/pool-5 (database.yml SSOT) · 06_05 threads ENV-form · 06_08 stale `04_02 §13b`-характеристика→реальні доми (`RPC_FALLBACK_ENV_KEYS`/`Web3::ChainlinkRouterVersion`) · S1.5 `<INGRESS_ANCHOR_IP>` · S4.3 volatile-count. Далі §07/§08 | `00_06 §4` |
