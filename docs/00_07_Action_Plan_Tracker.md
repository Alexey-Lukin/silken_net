...
- **P2** · 🤖+👤 · 🟡 · → [`05_02`](05_02_Proof_of_Growth_Pipeline)
- **Стан:** Code-half усунуто (2026-06-28); живий 👤-residual = deploy-flip. Два scale-ризики (лінза 100 млрд; НЕ горить на TRL-3): **(1) Strict-queue starvation** — `config/sidekiq.yml` `:strict:true`, money-черги нижче за intake (`uplink`(1) > `critical`-slash(3) > `web3_critical`-mint(6)): firehose телеметрії у `uplink` дренажиться першим → slash/insurance/mint/ConfirmationWorker голодують. **Канонізовано як deploy-рішення** [`06_08 §2.5`](06_08_Resilience_and_Failover_Policy): виділений money-path процес (`critical,web3_critical,web3,alerts`) фізично окремо від intake-процесу — детерміновано unstarvable, обидва SLO §2.4 задоволені; weighted-черги відкинуто (probabilistic + жертвує critical-precedence). Тригер flip'у = uplink-backlog↑ або mint-SLO breach; single-process baseline коректний поки firehose малий. YAML-коментар виправлено (strict ізолює В МЕЖАХ процесу, не між intake↔money). **(2) Partition-scan hot-paths** — закрито: `BlockchainConfirmationWorker` тепер несе `created_at_iso` (lower-bound вікно, дзеркало ARCH.50; 7 enqueue-сайтів прокинуто) + **partial index** `index_blockchain_transactions_in_flight` (`(status,created_at) WHERE status IN (0,1)`) робить pending-discovery (MintBatchCollector / MintCarbonCoinWorker) index range-scan замість full-history seq-scan. **[ARCH.12 note]** майбутній weekly Merkle = ще один partition-scan hot-path (запит МУСИТЬ нести `created_at` bound) + ієрархічний (cluster-subtree→root) обов'язковий. Канон [`06_08`](06_08_Resilience_and_Failover_Policy) / [`05_02`](05_02_Proof_of_Growth_Pipeline).
- **Свідомі skip'и (verified, не lazy):** **#3** `BlockchainMintingService#initialize where(id:)` — PK leading-колонка `id` вже index-served per-partition (partition-count = малий обмежений constant, НЕ O(all-history)); threading min_created_at безпечно вимагає batch-min created_at, якого 2/3 callerів не мають без зайвого запиту, + ризик виключити reset-to-pending старі ids. **#4 slash / #5 anchor sum** — all-time aggregates, семантично unprunable (slash потребує all-time minted; anchor — all-time SFC supply); long-term = denormalized counter / on-chain `totalSupply`, не partition-prune. **pending-discovery created_at-вікно** — НЕ ставити: reset-to-pending (`MintCarbonCoinWorker` raw `update_all` на RPC-error) тримає старий created_at, а `MAX_PENDING_AGE_MINUTES` робить старі pending *urgent* → нижня межа осиротила б stranded funds. Partial index = безпечний еквівалент.
- [x] 🤖 partition-prune: ConfirmationWorker `created_at_iso` (7 enqueue) + partial index pending-discovery ✅ 2026-06-28
- [x] 🤖 queue-starvation: канонізовано dedicated money-path процес + тригер ([`06_08 §2.5`](06_08_Resilience_and_Failover_Policy)) + sidekiq.yml коментар-фікс ✅ 2026-06-28
