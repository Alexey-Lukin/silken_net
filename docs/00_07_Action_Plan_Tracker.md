...
> Друга вісь зрізу (доповнює мілстоун-рядки ↑): вузькі «ключі»-дії, на які трекер сходиться across секції (звірено item-by-item 2026-07-03; §06-пас 2026-07-04 — INF.4-ключ ВІДКРИТО рішенням 07-03 і знято з таблиці, INF.20 вирішено (в)+wired того ж дня). Тонкі ID-вказівники; деталь у пунктах.
...
- [ ] 🤖+👤 **[P0, deep-audit 2026-07-04, verify byte-level] positive-A tamper-сигнал колізує з software-fault:** `BIO_STATUS_VM_ERROR (0x60)` — прошивка пише його при mruby-exception / unprovisioned-device / VM-OOM (`firmware/soldier/main.c`) — декодується у `tamper_detected` (біти 6..5 = `0b11`, `interpret_status(3)`) — той самий wire-код, що розкриття корпусу → `AlertDispatchService` піднімає `vandalism_breach` → `CauseEvidence#positive_a?` true для **ВСЬОГО** кластера (`.exists?` cluster-scope; `source_tree` свідомо не звужує). Наслідок: транзієнтний софт-збій на БУДЬ-ЯКОМУ дереві робить посуху (Cat-B, мала б freeze/insurance) «доведеним tamper» → необоротний slash жертви. Суперечить §1/§6 «не карати жертву». Фікс (firmware↔backend parity, НЕ наосліп): окремий wire-сигнал genuine-tamper vs VM_ERROR (2 status-біти вичерпані → потрібен окремий біт/поле, як `PANIC_FLAG_BIT`) АБО per-tree scope + фізична кореляція. Дім `05_05 §3.2` + `03_01`/`03_03` decode. Не задеплоєно → correctness-before-deploy.
- [ ] 🤖 **[deep-audit 2026-07-04, verify byte-level] chainsaw-спліт не ловить реальну пилку:** дискримінатор гейтить на `bio_status_anomaly?` (status=2), а справжня пилка (`ml_event_id==3` → panic-TX, `Trigger_Emergency_LoRa_TX`) лишає status=homeostasis + `PANIC_FLAG_BIT` (bit 7, ОКРЕМО від status-бітів) → декодується `panic?=true, bio_status=homeostasis` → падає у `seismic_anomaly` (acoustic=255≥200), НЕ `chainsaw_detected`. PANIC-TX-маркер (`if telemetry_log.panic?` у повідомленні) **структурно недосяжний** (anomaly? і panic? взаємовиключні на реальних даних). Мета founder-рішення 07-03 (пилка без жару ≠ rejected_fraud) формально тримається (panic→seismic, не fire), але НЕ через задуманий шлях — `seismic_anomaly` не в `requires_satellite_consensus?` → не йде non-fire Field-Audit-маршрутом. Гейт має читати `panic? || bio_status_anomaly?`. Дім `05_05 §3.2` + `03_03 §7.2` (застаріла діаграма `EwsAlertCreatorService` — сервіс не існує, реальний = `AlertDispatchService`).
- [ ] 🤖 **[deep-audit 2026-07-04] penalty_factor-uplift self-referential (ПЕРЕД активацією inert-uplift ↑):** `comms_no_ack?` / `critical_unmaintained?` не виключають ТУ САМУ `vandalism_breach`, що дала `positive_a?` (виключають лише `:field_audit`) → на кожному tamper-Cat-A slash обидва сигнали true на одній події (unresolved + >30хв) → `combine_penalty_factor` = 2.0 = `PENALTY_FACTOR_MAX` майже завжди → множник СИСТЕМАТИЧНО на стелі, не «cause-driven градуйований». + `comms_no_ack?` рахує БУДЬ-ЯКИЙ critical alert (не лише node-offline, всупереч §6 «root-cause вузол/шлюз offline»). INERT зараз (`slash_cause_uplift_enabled` default false) — фіксити ПЕРЕД founder-активацією; виключати alert, уже спожитий як positive-A доказ цього ж інциденту. Дім `05_05 §6`.
...
- [ ] 👤 створити Gnosis Safe (3/5|2/3) на Polygon + деплой з `ADMIN_ADDRESS=<Safe>` `REQUIRE_SAFE_ADMIN=true` + transfer admin-ролей → Timelock (крім `pause`, E.2) + активація on-chain governance (розблокує `ParameterSyncWorker` — див. GOV.1 read-path)
...
- [x] 🤖 **[M2] mint-fail звільняє locked ✅ 2026-07-04** (`1c9a0ef8`): `fail!` на mint-tx (дискримінатор `locked_points`) повертає `locked_balance` — revert-гілка ConfirmationWorker робила голий `fail!` → бали форестера заморожені назавжди (release жив лише в `MintingRollbackService` ← тільки `retries_exhausted`). Дім [`04_02 §4`](04_02_Business_Logic_and_Services).
- [x] 🤖 **[M6] batchMint/individual double-mint guard ✅ 2026-07-04** (`1c9a0ef8`): мережева помилка `transact` ПІСЛЯ broadcast → escalate `manual_review`, НЕ сліпий individual re-mint (pre-broadcast whitelist revert/nonce/known/underpriced/insufficient → безпечний individual; решта → ambiguous). `send_clean_batch` fallback раніше individual-мінтив увесь під-батч навіть коли batchMint міг landed → double-mint. Дзеркало burn ARCH.48.
- [~] 🤖 Chainlink «double-request» seam (A9) — дедуп: ЄДИНИЙ дім = **ARCH.53** (доля прив'язана до PATH-1-статусу, який трекає ARCH.53). Тут прибрано (був дублікат).
...
- [ ] 🤖 (residual, low) crash-recovery hardening: B1 retry-exhaustion (>5 retry → Dead Set, recovery-крони нема; рідко + PATH 1 latent); true-concurrency double-slash — `BurnCarbonTokensWorker` без `unique_for`, in_flight `unsettled_within` guard не атомарний з intent-creation (TOCTOU; Kredis lock серіалізує on-chain, не ре-чекає) → partial-unique-index `(sourceable) WHERE status IN (pending,sent,manual_review)` АБО worker `unique_for`. ⚠️ `unique_for` = **Sidekiq Enterprise-фіча** (зараз шим-no-op — [`04_02 §Sidekiq`](04_02_Business_Logic_and_Services) DOC-R.10); founder-рішення 2026-07-04: Enterprise = планова покупка перед mainnet, НЕ блокер — де money-idempotency спирається на `unique_for`, познач залежність, не будуй костиль. Pre-existing ARCH.45 gap; sequential in_flight (типовий випадок безпечний). **[дедуп: раніше дублювався у ARCH.45 — тепер ЄДИНИЙ дім тут як Chainlink-сиблінг]**
...
#### SLASH.2 — slash() рахує pre-tax БД-суму > on-chain балансу (тихий no-op + evasion)
- **P1** · 🤖+👤 · 🟢 · → [`05_05 §3`](05_05_Slashing_and_Risk_Policy), `05_03`
- **Стан:** deep-audit 2026-07-04 (O2). `BlockchainBurningService`: `total_minted_amount = sum(:amount)` по confirmed tx = **pre-tax** суми, але форестер on-chain тримає `Σ(amount − tax)` (DynamicTax пішов у treasury) або менше (SCC вільно переказуваний). При `slash_ratio→1.0` (damage 1.0, Cat-A) `burn_amount = total_minted > balanceOf(investor)` → on-chain `require(balanceOf >= amount)` (`SilkenCarbonCoin.sol`) revert. Backend оптимістично мітить `@naas_contract.status = :breached` + `SCC_SLASHED_TOTAL` ДО підтвердження; ConfirmationWorker `fail!` НЕ знімає `:breached` і НЕ re-slash-ить коректною сумою → **економічне покарання тихо не застосовується**, метрика бреше. Evasion: інвестор переказує 1 wei → будь-який повний slash revert-ить. Фікс: читати `balanceOf` перед slash і клампити `burn_amount = min(burn_amount, balance)`; АБО on-chain `slashUpTo(investor, maxAmount)` (pre-deploy — ідеальний момент додати); АБО ConfirmationWorker revert-гілка знімає `:breached` + алерт. Не задеплоєно (slash positive-A-gated + INERT) → pre-mainnet.
- [ ] 🤖+👤 clamp burn_amount до balanceOf (backend) АБО slashUpTo on-chain (рішення — контракт vs backend)

#### GOV.1 — Governance-параметр pipeline напів-Потьомкінський (read-path + missing keys)
- **P1** · 🤖+👤 · 🟢 · → [`05_06`](05_06_Governance_and_DAO), `05_03`
- **Стан:** deep-audit 2026-07-04 (S4, verify byte-level). `Governance::ParameterSyncWorker` синхронізує 13 параметрів у `SystemParameter`, АЛЕ споживачі читають **хардкод-константи**: `SilkenNet::Attractor` (`BASE_SIGMA/RHO/BETA`, `DT`, `ITERATIONS`), `ContractHealthCheckService` (`Rational(1,5)` slash-threshold), `AiInsight::SLASH_STRESS_THRESHOLD` (0.83), `TokenomicsEvaluatorWorker::EMISSION_THRESHOLD` (10_000) — жоден не читає назад. Тобто DAO-голосування за ці параметри сьогодні **не має ефекту**. **Нюанс:** 8 Lorenz-параметрів = свідомо DCI-locked (bit-identical Float з прошитим firmware — міняти через DAO зламало б device↔server parity), АЛЕ тоді синхронізувати їх = майбутня пастка; `emission/slash/stress` = просто недороблений read-path. Плюс: `slash_gamma`/`slash_penalty_factor_max` (05_05 стверджує «DAO-governed via ProtocolParameters») + `scc_per_tonne_co2`/`scc_fallback_price_usd_cents` (визначені в `ProtocolParameters.sol`) — **НЕ в `PARAMETER_MAP` і/або не в контракті** → змінити on-chain неможливо; fallback-ціна ще й з mismatch (контракт _cents, `price_oracle_service` читає `scc_fallback_price_usd` долари — 100×). + `ProtocolParameters.setParameter` без on-chain bounds → мис-скейл (tax=2e18 «200%») → backend `forester_amount<0` → mint-halt (фікс = clamp у sync-воркері, контракт immutable). Канон-чесність: `05_06 §7` «✅ Реалізовано» виправлено (↓). Дім `05_06 §7`.
- [ ] 🤖 вирішити долю Lorenz-sync (видалити з `PARAMETER_MAP` як DCI-locked пастку АБО документувати read-only-mirror) + дороблення read-path для `emission/slash/stress` (не-DCI)
- [ ] 🤖 узгодити `PARAMETER_MAP` ↔ `ProtocolParameters` ключі (`slash_gamma`/`pf_max`/`scc_per_tonne`/`fallback_price`; unit-mismatch _cents↔долари) + clamp economic-параметрів у sync-воркері (bounds)

#### CONTRACT.1 — Pre-mainnet contract-ops hardening (парасоля)
- **P2** · 🤖+👤 · 🟢 · → `05_03`, `05_04`, `05_06`, `06_07`
- **Стан:** deep-audit 2026-07-04 (O2/O4 enterprise-standard пас; контракти НЕ задеплоєно → pre-deploy вікно). Тест-парк + audit-стек **вище стандарту** (fair-launch no-premice, Halmos+Medusa gating, role-matrix Deploy.t.sol пін). Залишкові hardening-пункти ДО Hacken-вікна: (1) **guardian CANCELLER** — Safe не має прямого `CANCELLER_ROLE` (лише Governor) → 48h veto-вікно фактично невіведене (Governor-cancel = новий DAO-цикл >8д); +1 рядок `Deploy.s.sol` `_wireTimelock`; (2) **on-chain anomaly monitoring** — 0 незалежного спостерігача за подіями не-backend-походження (compromised MINTER→rogue `CarbonMinted` / SLASHER→масовий `TokenSlashed`; slash навмисно обходить pause → 48h-revoke єдиний стоп) → легкий reconcile-воркер on-chain events ↔ `BlockchainTransaction`-наміри (Tenderly/Defender опційно); (3) NatSpec `max 200` → `MAX_BATCH_SIZE=100` (SCC/SFC — stale, вводить в оману Hacken); (4) MAX_SUPPLY 1B — трасувати деривацію (10k GP=1 SCC × дерев×років) АБО позначити «свідомо-щедра стеля»; (5) migration/pause/deploy-smoke runbook-и (immutable one-shot — незворотні); (6) slash-події без cluster/reason-id (`GovernanceSlashed(investor, amount)` — subgraph не атрибутує); (7) `proposalThreshold` 100 SFC = 0.0001% (spam-griefing, розглянути частку supply); (8) CI gas-snapshot + coverage-floor гейти. Дім `05_03`/`06_07 §1`.
- [ ] 🤖 (дешеві, зараз-able) NatSpec 200→100 · guardian-CANCERLLER рядок · gas-snapshot/coverage-floor CI
- [ ] 🤖+👤 (Hacken-вікно) on-chain event-reconcile monitoring · migration/pause/deploy-smoke runbook-и · MAX_SUPPLY-деривація · slash-event contextHash · proposalThreshold-політика

#### MRV.1 — Money-path audit-trail + MRV data-lineage (compliance)
- **P2** · 🤖+👤 · 🟢 · → [`05_02`](05_02_Proof_of_Growth_Pipeline), `04_01`
- **Стан:** deep-audit 2026-07-04 (O3 enterprise-standard). Pre-mainnet compliance-хвіст для carbon-registry (ISO 14064 / Verra): (1) **money-переходи не пишуться в наявний SHA-256 `AuditLog`-ланцюг** (пишеться лише з `factory_flashing`/`codex`; mint/slash/payout «хто/чому» = freetext `notes`, колонки actor нема) — tamper-evident per-transition trail; (2) **PATH 2 live-mint не лінкує SCC→telemetry-batch** (`chainlink_request_id`/`zk_proof_ref` лише на PATH 1; оптимістичний mint на `wallet.balance` growth-points) → аудитор не простежить credit→measurements; (3) `dependent: :delete_all` на `wallet.blockchain_transactions` + Tree→telemetry — хардделіт стирає MRV-докази під виданими кредитами → guard/soft-delete; (4) web3-специфічні incident runbook-и (chain reorg — 0 обробки; double-mint→дії; EOA money-key compromise step-by-step; manual_review-завал resolution + `MintingAdminController`/UI, якого НЕМАЄ → зараз єдиний шлях з manual_review = прямий DB-запис). Дім `05_02` (модель довіри) + `06_08` (runbook-и).
- [ ] 🤖+👤 (pre-registry-submission) money-transition → AuditLog hash-chain · PATH 2 lineage (Merkle-witness ARCH.12 дотично) · delete_all→soft-delete guard · web3 incident runbook-и (reorg/double-mint/key-compromise/manual_review-resolution + admin-UI)

#### KYC.1 — Hadron KYC approval-шлях недосяжний (mint-blocker на свіжому деплої)
- **P1** · 👤 · 🟢 · → [`05_02`](05_02_Proof_of_Growth_Pipeline)
- **Стан:** deep-audit 2026-07-04 (S1). `Polygon::HadronComplianceService#verify_investor!` — ЄДИНЕ місце, що пише `hadron_kyc_status: "approved"` — має **0 викликів** (grep увесь `app/`); DB-дефолт `pending`; `Tree#build_default_wallet` не ставить. Mint-guard вимагає approved (тепер per-tx skip [S2] — не валить батч, АЛЕ skipped tx лишаються `:pending` навічно). Тобто на свіжому деплої/сіді ЖОДЕН органічний гаманець не мінтиться. Продуктове рішення: коли/як гаманець стає approved — авто при binding `crypto_public_address`? ручний KYC-flow? `simulate_kyc_check` у dev? (PR-контекст `dfe7c1ee` «for institutional tokenization» → guard мав би стосуватись лише інституційних гаманців). Дім `05_02` (KYC-гейт).
- [ ] 👤 продуктове рішення: KYC-approval-шлях (auto-on-binding / ручний flow / institutional-only guard) → потім 🤖 wiring
...
- **Стан:** Аудит 2026-07-04 (O3-MUST): web/job/coap у static SDL + `docker_image` tf-var default + `coap-daemon` systemd-юніт на анкорі (`compute.tf`) — усі на `ghcr.io/…:latest`, який `mirror-ghcr` перезаписує кожним push у main → будь-який container-restart / provider-міграція / VM-reboot підтягне ІНШИЙ образ, ніж тестований, і немає rollback-цілі (мутабельний тег не відкотиш). Контраст: alloy запінено `v1.16.3` (INF.14), base-образ digest-pinned. Mirror вже пушить іммутабельні `sha-<commit>`-теги + semver на release — лишилося споживати їх. Суміжне: Cloud SQL proxy `ADD` у Dockerfile — version-pinned URL без `--checksum` (та сама пін-непослідовність, дрібно).
- [x] 🤖 ✅ 2026-07-04: анкор-образ параметризовано tf-var'ом `coap_daemon_image` (pull + ExecStart) + обидва tfvars.example несуть sha-pin приклад (akash `docker_image` example → `sha-CHANGE_ME`)
...
- [ ] 🔗 опц. `--checksum` на Cloud-SQL-proxy `ADD` у Dockerfile (єдиний непін-digest артефакт; дрібне)

#### OPS.11 — FinOps guards: GCP billing budget + AKT escrow watch
- **P1** · 👤 · ⚪ · → [`06_02`](06_02_Akash_Network_Integration)
- **Стан:** Не розпочато (O3-MUST ×2): (1) жодного `google_billing_budget`/alert — `disk_autoresize=true` + egress можуть тихо рости в рахунок на особисту карту; (2) Akash lease живе з escrow, який ВИГОРАЄ — на нулі lease закривається (web+job+coap помирають разом), і ніщо runway не моніторить; `scrape-target-down` алерт при цьому летить у неналаштований contact point (S2.2). Мінімум дня-1: over-fund депозит + календарне нагадування.
- [ ] 👤 GCP billing budget + email-пороги (50/90/100%)
- [ ] 👤 AKT: initial over-fund (≥2× місячна оцінка) + календар поповнення
- [ ] 🤖 scheduled workflow `akash query market lease` → runway-check + fail-notify (після першого lease; патерн coap_smoke)

#### ARCH.55 — stuck-`:sent` mint re-arm sweeper (money-residual)
- **P1** · 🤖 · 🟢 · → [`06_08 §2.3`](06_08_Resilience_and_Failover_Policy), [`04_02`](04_02_Business_Logic_and_Services)
- **Стан:** ✅ **Machine-half SHIPPED** (deep-audit §05, 2026-07-04). `StuckSentTransactionSweeperWorker` (cron :05/:35) re-arm-ить `BlockchainConfirmationWorker` для tx, що застрягли у `:sent` довше 15 хв — клас, який жоден попередній money-audit не ловив: OOM/евікшн ПІД ЧАС поллінгу (pending-discovery дивиться лише pending/processing; `MintingRollbackService` — тільки з `retries_exhausted`). Ключується на `sent_at` (момент broadcast), НЕ `created_at` (reset-to-pending тримає старий → ARCH.52 trap); дедуп по tx_hash з earliest created_at (partition-prune). **Передумова [S2]:** `sent_at` тепер проставляється через `mark_as_sent!` — mint (`blockchain_minting_service.rb`) + Etherisc (`insurance_payout_worker.rb`) робили голий `update!(status: :sent)` → `sent_at` вічно NULL. Покриває mint+burn+insurance (спільний ConfirmationWorker). Ідемпотентність дубля з живим поллером тримається на AASM `confirm`; з Sidekiq Enterprise `unique_for` дедуплікує (⚠️ зараз шим-no-op — Enterprise = 👤 покупка перед mainnet, не блокер). Дім [`04_02 §4`](04_02_Business_Logic_and_Services).
- [x] 🤖 sweeper + спека ✅ 2026-07-04 (`1c9a0ef8`)
- [ ] 🤖 (scale-хвіст) partial index на `status = :sent` для sweeper-scan (зараз status-scan у межах recovery-обсягу; ARCH.52-клас — додати коли `:sent`-обсяг зросте)
...
- **Стан:** Аудит 2026-07-04: 06_08 §2.2/§2.3 ✅-ив ~10 механізмів, яких у коді НЕМАЄ — канон переписано чесно (нереалізоване позначено 🟡 target інлайн), а самі механізми зібрано СЮДИ як опційний backlog. ⚠️ НЕ чеклист «зробити все» — кожен проходить YAGNI-фільтр при live-потребі; більшість latent (PATH 1) або спостерігачі. **06_08-target'и:** IotexBackfillWorker-cron · FilecoinReconcileWorker (re-pin з AuditLog) · streamr/celo buffer-lists · anchor gas-gate (`MAX_ANCHOR_GWEI`+пул) · Solana RPC-fallback-каскад · `peaq_long_outage` alert · LINK-monitor (лише при замиканні PATH 1) · TheGraph `eth_getLogs`-fallback · Klima-активація хвости (toggle/manual_review/nonce-lock) · breaker reschedule-on-open. **O4 day-2 (SHOULD-early-live):** Cloud-SQL-proxy supervisor у entrypoint АБО `/up` DB-залежний (зараз мертвий-proxy контейнер живе вічно — Akash не робить readiness-рестартів) · `DB_POOL` vs `max_io_threads 16` (io-bound сплеск > pool 5 → ConnectionTimeout; підняти pool на io-ролях) · Upstash region-pin (europe-west1) + Akash placement-attributes той самий континент + Rack::Attack → MemoryStore поки web count:1 (крос-регіон RTT на кожен запит) · zero-downtime: migration-крок окремо від entrypoint при count≥2 · `/ready`-interval tune 10-15s при INF.10-фліпі. **O3 (SHOULD-post-deploy):** GH Environments для production (секрет-скоупінг money-ключів + wait-timer замість plan-gate) · Loki-pipeline в Alloy (Akash lease-логи ефемерні — гинуть при міграції; зараз виживають лише Sentry-exceptions) · зовнішній synthetic uptime (Grafana Synthetic — самоскрейп гине разом із lease) · WIF для CI→GCP (static SA-key лишається тільки Akash-виняток) · tfsec/checkov + scheduled drift-plan. **Чесний YAGNI-NOW (не робити):** canary/blue-green · cross-region backup (баланс on-chain) · ro-rootfs/cap-drop · burn-rate SLO · PagerDuty · sops · game-days · Dragonfly-on-Akash · SO_RCVBUF (до planetary).
...
| ARCH.4 | Governance DAO — on-chain protocol constants (`SilkenGovernor`+`SilkenTimelock`+`ProtocolParameters`+`ParameterSyncWorker`) — resolved code-annotation (orphan-ID, цитований у контрактах/`05_01`/`05_03`/`05_06` без 00_07-дому; активація → SEC.1, read-path → GOV.1) | `05_06` |
