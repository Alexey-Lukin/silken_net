...
|----|----------------|-------------------|-----|--------|
| DOC-T.9 | Documentation `02_03` §9.3 raніше використовувала 15 mA/50 ms для LoRa TX. Виправлено на 120 mA/100 ms (~39 мДж) per SX1262 datasheet. Firmware energy accounting **не верифіковано незалежно** | `02_03`, `firmware/soldier/main.c` | Лабораторне вимірювання поточного TX (HW.x) + cross-ref у `02_03` після верифікації | ⏸️ Заблоковано лаб-стендом || DOC-T.26 | `docs/protocols/**` (19 .md, AI-visible, load-bearing `PIPELINE_STATUS`) реферить канон **relative-path** лінками (`../../NN_NN`), але решта ref-integrity-guards (`bare_doc_ref`/`bare_section_ref`/`crossref_label_form`/`link_label_target_mismatch` + value-guards) чекають абсолютний `(NN_NN_Name)` href + сканують лише top-level `files` → системно минають protocols→канон-рефи. (DOC-T.16 `section_ref_after_doclink` цей піддерево вже покрив — relative-aware + окремий scan-pass.) | `docs/protocols/**`, `lib/docs_linter.rb` | Relative-href (`(?:\.\./)*`) у решті ref-integrity-регексів + protocols scan-pass + sweep наявних | ⚪ Approved |
