...
- [ ] 🔗 (live-gated) `Treasury::MonitorService` стежить ОДИН газ-гаманець на чейн (Prometheus gauge за `network`-лейблом). INF.22 side-fix навів polygon→MINTER, але **SLASHER** (2-й Polygon-signer, slashing-газ) + aux (Etherisc/Puro/Klima) баланси НЕ моніторяться — два polygon-записи колізили б за спільним лейблом; ANCHOR покрито (ethereum→ANCHOR). Живе, коли slashing (SLASH-1 A-set) / aux-шляхи активуються — нині дормантні (slashing freeze-only). Фікс = per-signer лейбл + окремий threshold-param. Дім [`06_02`](06_02_Akash_Network_Integration).
