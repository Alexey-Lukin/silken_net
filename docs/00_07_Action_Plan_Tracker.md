...
| **Перший live-деплой** = акаунти/значення Фази −1 + секрети (`S1.1`+`S4.3`) + `OPS.11`-FinOps-значення (tf/workflow ✅ 07-05) → deploy фазами [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) (👤; SSH-нога wired — INF.20 (в)) | верифікації `INF.11/12/13/14/15/16` · `INF.17`-coap.env · `S1.5`-IP · `INF.20`-IAP-вхід · `PUMA-IPV6-1` · `INF.10`-фліп · `S5.2` · `S3.2`-staging · `S6.1`-Upstash · `INF.21`-pin · `SEC.19`-region-placement (EU-PII residency) · `INF.24`-auditor-on-chain-звірка (`akash query audit`) · Grafana-сесія (`S2.1→S2.2` + `S2.4`-SLO + `FW.18b`) · активує escrow-watch (repo Variable `AKASH_OWNER_ADDRESS` — workflow ✅) |
...
- **P1** · 🤖+👤 · 🟡 · → [`06_04 §5.5/§5.6`](06_04_Secrets_Checklist), [`06_02`](06_02_Akash_Network_Integration), `SECURITY_ASSURANCE §6`
- **Стан:** 7-агентний sweep (2S+5O) розкрив, що «більша діра» = не одна, а багатошарова. **Git-history CLEAN** (нема irreversible-leak; `master.key` ніколи не committed — робить публічний `credentials.yml.enc` safe-by-design). Найгостріше — **at-rest-latch ≠ runtime-latch**: Akash-провайдер читає `/proc/<pid>/environ` живого процесу, тож crown-jewels (`RAILS_MASTER_KEY`+`PROVISIONING_MASTER_KEY`, на web+job+coap) provider-visible попри будь-яке at-rest-шифрування. **Присуд (Opus-конвергенція №6+№7):** Framing-3 keyless-**на**-Akash — НЕ переносити job на GCP (crown-jewels на web все одно → theater + resize-cost + SPOF); розчинити кожен unbounded-fraud-ключ через KMS-signing (SEC.17) + KMS-MAC (PROVISIONING). «Sealed-never-undone» НЕ коштує Akash — коштує pre-mainnet firmware-KDF-зміну. `PROVISIONING` = crown-jewel (HKDF-корінь 6 класів ключів, fleet-wide-forge, **вищий за minter-key**; Bitwarden-vault-adapter = `NotImplementedError`, живий лише `EnvAdapter` — під-емфазована operational-custody-діра). Відкрите ↓ (phased).
- [x] 🤖 **Behavior-preserving surface-reduction ✅ SHIPPED (07-09)** — `.dockerignore` (.enc + terraform поза public-image) · CHAINLINK-delete-from-job (web-only, zero-consumers) · Sentry breadcrumbs/inline-URL-scrub · `scripts/deploy_secret_scan` never-drift-guard (no-key-literal + sextet-job-only, CI-wired)
- [ ] 🤖 **NOW behavior-preserving** — `credentials→ENV` (8 сервісів iotex/puro/streamr/the_graph/filecoin/peaq/hadron/dclimate → розчиняє `RAILS_MASTER_KEY` з web/coap) + `SECRET_KEY_BASE`-env + cache `iotex_seed`/`K_ota` (PROVISIONING з telemetry-hot-path) + scope `master_key_strength_check` (як `web3_network_guard signer_process:`)
- [ ] 🔗 **pre-mainnet (decisive)** — SEC.17 KMS-signing (розширити на 5 EVM-ключів, не лише minter/slasher) + **KMS-MAC `PROVISIONING`** (Expand-only HKDF backend+firmware разом; 3-й keyring `silken-mac-ew1` поряд `silken-disk/sign-ew1`) → crown-jewel з кожного Akash-процесу
- [ ] 🤖 **terraform-state latch** (Sonnet #1 central-hole: 3-я plaintext-копія ВСІХ секретів у GCS-state; `db_password` LIVE-today через CI) — CMEK-state-bucket + tight-IAM + GCS-versioning-retention drop
...
- [ ] 🤖 **rotation-on-compromise + canon-honesty** — runbook crown-jewels (`RAILS_MASTER_KEY`+`PROVISIONING` = un-rotatable/no-procedure, дзеркало S6.14 §5.4) + `SECURITY_ASSURANCE §6` secrets-runtime-residual + master-key-custody-prominence (Opus#7 #3)
- [ ] 🤖 **DB-holes** (Sonnet) — `identities.access_token/refresh_token/auth_data` plaintext → `encrypts`; `hardware_keys` AR-Encryption production-key-wiring unconfirmed → verify (не в `credentials.yml.enc`)
