...
- [ ] 🤖 `batch_payout_service` race — Linux-CI-only; блокер = невідтворюваність (87 прогонів: 52 macOS + 35 Linux-стенд, 0 failures; обидві гіпотези перевірені й не підтверджені ↑). NEXT: failing-example з наступного CI red-run (рядок + assertion message). **НЕ** маскувати seed-pin (вбиває order-detection для всього suite). **НЕ** застосовувати дефенсивний sort без repro (маскування, не корінь).
...
- **Стан:** Виявлено Dependabot-sweep (2026-07-09, PR #460). aasm 6.0.0 (5 Jul 2026) breaking для нас **вузький**: PR #880 «avoid namespace naming collisions» зламав згенеровані bang-методи подій у ЄДИНІЙ namespaced-машині — `Firmwareable` concern (`aasm :firmware, namespace: :firmware`, який include-ять Gateway + Actuator). `start_download!`/`schedule_update!`/`complete_update!`/`fail_update!`/`start_verification!`/`start_flashing!`/`reset_firmware!` → undefined → 31 rspec-fail (firmwareable_spec + gateway_spec). Другий breaking 6.0 (`whiny_persistence: true` тепер default) нас **НЕ** зачіпає — усі 10 наших FSM (blockchain_transaction money-path включно) вже явно ставлять `whiny_persistence`. Потрібно: aasm 6.0 upgrade-нотатки → оновити namespaced call-sites на нові імена (або прибрати `namespace:`, якщо колізії нема) + синхронізувати специ + full rspec. Money-path не зачеплений. PR #460 лишено відкритим як робочу гілку-нагадування.
- [ ] 🤖 aasm 6.0: перейменувати namespaced firmware bang-методи (Firmwareable/Gateway/Actuator) + специ + full rspec
...
- [ ] 🤖 (pre-mainnet) `Web3::OracleSigner` seam (12 `sender_key`-call-sites, `LocalEnvSigner` default = behavior-preserving) → `Web3::KmsSigner` — заміна raw-ENV-приватника у mint/burn (crypto-послідовність + offline-тести → [`06_04 §5.5`](06_04_Secrets_Checklist))
...
- [x] 🤖 **rotation-on-compromise + canon-honesty ✅ SHIPPED (2026-07-10)** — runbook crown-jewels = [`06_04 §5.8`](06_04_Secrets_Checklist) (ordered-degradation, чесно НЕ «ротація за 15 хв»; A=PROVISIONING: stop-provisioning → fleet-недовіра (K_seed ламає DCI-інваріант ВСЬОГО флоту offline-derive'ом; fraud що ПРОХОДИТЬ Z-звірку = сигнатура) → K_ota морозить OTA (rescue лише SWD) → re-flash Queen-first; B=RAILS/secret_key_base entangled: credentials-re-encrypt ≠ revoke, справжній revoke = SECRET_KEY_BASE-ротація → інвалідація сесій+`generates_token_for` (api_access 30д хвіст)+Codex pair-HMAC). 🔑 Sweep-знахідки вшиті: 6-й клас iotex_seed у §5.2 (перелічувалось 5 із «6») + незадокументований `Codex::PairSelectorService`-консюмер + **ENV-first пастка** (credentials:edit НЕ ротує ENV-заведений ключ → нота в §5.4-2a + §5.8 header) + EDSK/voice-сім'я = явний non-consumer (SecureRandom, не HKDF). Canon-honesty: 03_06 «Rails Vault + HSM» custody-міф → чесний ENV-стан (2 рядки); custody-prominence (Opus#7 #3) — `RAILS_MASTER_KEY` тепер названий у `SECURITY_ASSURANCE §6` на ім'я (+ §6 оновлено: tfstate-copy CMEK-sealed, runbooks written). ⚠️ Відкритий TODO всередині runbook'а: звірити з IoTeX, чи W3bstream пінить pubkey per-DID (перший крок інциденту A)
...
- [ ] 🤖 **[buildable, post-deploy]** CI/infra-hardening пакет (дім = [`06_07`](06_07_CICD_and_Runbook_Index) + `ci_maturity`-gotchas) — розкладено на 4 незалежні жили (O3):
- [x] 🤖 **GH Environments (prod) ✅ SHIPPED (2026-07-10)** — environment `production` створено API (wait-timer 10 хв + ref-policy `v*`-теги∪`main`; live-verified GET) ДО yml-мержу (авто-створення першим run'ом = unprotected — порядок критичний); `environment:` на `verify-secrets`+`deploy` у `deploy-production.yml` (НЕ `terraform` — TF-вари repo-level, drift ділить; wait = **per-job** → 2×10хв/release, свідомо). 🔑 Timing-інверсія: «post-deploy»-мітка знята — секрети ще НЕ провіжнуті (лише `PROJECT_PAT` repo-level), тож environment ДО provisioning = founder кладе п'ятірку одразу в правильний дім (GitHub значень назад не віддає — пізніше перенесення = ручне зі значеннями). 🔑 Попутні canopy-фікси: (1) **`.kamal/secrets` → `.kamal/secrets-common`** — з destination Kamal читає ЛИШЕ `-common`+`secrets.<dest>` (verified Kamal::Secrets source), плейн-файл для `-d canopy` невидимий → canopy-нога падала б `ConfigurationError` на 1-му деплої; (2) canopy-гейт вимагав `ORACLE_MINTER/SLASHER`, яких web-only destination не мусить споживати → BOOT_CRITICAL/RUNTIME/env-блоки підрізано (інакше environment-скоупінг вимикав би canopy назавжди `configured=false`); (3) **[R1-Opus HIGH] «без `job:`-секції» ≠ web-only** — Kamal destination-merge = `deep_merge!` keys-UNION (verified gem-source 2.12), омітнута роль УСПАДКОВУЄТЬСЯ з base разом із money-`env.secret` → фікс = **array-form `servers:`** у `deploy.canopy.yml` (масив ≠ hash → замінює цілком; runnable-пруф + `role_names(Array)=["web"]`) — web-only тепер структурний, і локально теж; (4) **[R1-Opus MEDIUM] legacy `ORACLE_PRIVATE_KEY` знято з Kamal/GH-поверхонь** (`servers.job.env.secret` + `secrets-common` + deploy-production env:) — unprovisioned-but-mapped = present-empty `""` → guard format-raise ПІСЛЯ зеленого verify (він єдиний із набору не гейтився); GH-environment-набір = **п'ятірка**. Доми: [`06_04 §1`](06_04_Secrets_Checklist) (куди класти) · [`06_07 §1`](06_07_CICD_and_Runbook_Index) · deploy-скіл. Verify 👤 = перший release-run (wait-timer у дії + п'ятірка з environment)
- [ ] 🤖 **(low, наступниця R1-MEDIUM) `ORACLE_PRIVATE_KEY` full retirement** — legacy shared-fallback лишився ЛИШЕ на Akash SDL-поверхні (SDL/`deploy.yaml.tpl`/`terraform/akash` tfvars+variables; там його прикриває `REQUIRED_SECRET_NOT_SET`-дисципліна Console-інжекту). Повний retirement = зняти з SDL-поверхонь + `web3_network_guard` legacy-гілки (`SIGNER_FALLBACKS` base-resolve) + сервісні `ENV.fetch(...) { ENV.fetch("ORACLE_PRIVATE_KEY") }` fallback-ланцюги (Celo/Klima/Puro/Etherisc → dedicated keys, ARCH.50-патерн) + [`06_04 §2.1`](06_04_Secrets_Checklist)/§4. E.2-чистота: жодного shared-ключа. Value-gated (не блокер: fallback спрацьовує лише коли dedicated відсутній)
- [x] 🤖 **WIF CI→GCP ✅ SHIPPED (2026-07-10)** — keyless OIDC замінив довгоживучий `GCP_SA_KEY` у deploy/deploy-production/drift: `terraform/wif.tf` (pool + GitHub-OIDC provider з owner-scoped `attribute_condition` + repo-scoped `principalSet` на SA-binding, `lowerAscii()` case-safe проти mixed-case owner'а) + `google-github-actions/auth` WIF-форма (`id-token: write` per-job) + Kamal registry `oauth2accesstoken` + short-lived access-token. CI-secret `GCP_SA_KEY` вилучено; deploy-gate тепер = repo **Variables** `GCP_WORKLOAD_IDENTITY_PROVIDER`/`GCP_SERVICE_ACCOUNT`; Akash `GCP_SA_KEY_BASE64` лишається ЄДИНИМ static-key винятком ([`06_02 §Security Exception`](06_02_Akash_Network_Integration)). Chicken-egg: 1-й apply = founder-local ADC створює WIF, далі CI keyless. Доми: [`06_04 §1.1`](06_04_Secrets_Checklist) · [`00_05 §2.7`](00_05_GitHub_Projects_and_IaC_Automation). Verify 👤 = live round-trip на 1-му deploy (actionlint + `terraform validate` зелені локально; config-half як уся §06). ⚠️ Keyless стосується *auth*; CI-terraform-apply **права** = окреме pre-existing питання ↓
- [ ] 🔗 **(deploy-day founder-decision) CI terraform-apply права** — keyless *auth* готовий, але CI impersonує least-privilege deploy-SA БЕЗ IAM/WIF/serviceusage-admin → CI `terraform apply` / drift-`plan` рефреш IAM/WIF/service-ресурсів = 403. Pre-existing, не WIF-регресія: CI-terraform-apply прав ніколи не було в terraform — старий `GCP_SA_KEY` або ніс ті самі вузькі ролі, або (якщо ширший bootstrap-SA) WIF — це ще й security-звуження. Вибір моделі: **(а)** `terraform apply` лишається founder-local (Фаза 0), CI-нога = лише `kamal deploy` (registry push + VM) — **рекомендовано** (не роздувати deploy-SA); **(б)** дати deploy-SA `workloadIdentityPoolAdmin`/`projectIamAdmin`/`serviceUsageAdmin` **+`serviceAccountAdmin`** (без останньої рефреш `google_service_account_iam_member` deploy_wif/act-as все одно 403 — SA-level policy) — ширший blast-radius, privesc-концерн (дзеркало SEC.22 deploy-SA god-credential), ще один аргумент за (а). → [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform)
- [x] 🤖 **IaC-scan ✅ SHIPPED (2026-07-09)** — `Sec · IaC Scan` (Trivy `config` — tfsec deprecated→Trivy; SARIF, **soft-fail** hygiene, SHA-pin+`version:`, skip-dirs akash/grafana/firmware-extern) ловить terraform/Dockerfile-misconfig ДО деплою; `Ops · TF Drift` (scheduled `plan -detailed-exitcode` GCP-root, guard=3 secrets, `compute.tf lifecycle ignore_changes` akash-ip = anti-phantom-drift) = **config-half**, активується з першим `apply`. 2×Sonnet+Opus review → 4 blocking/should-fix виловлено (wrapper-exit2-swallow · missing TF_VAR · guard-1-secret · akash-ip permanent-drift). **2-й Opus-review (07-09) → deploy_secret_scan hardened:** `.dockerignore`/`Dockerfile` → alloy path-filter (tripwire був сліпий до власного файлу, а mirror-ghcr САМЕ на ньому перебілдить публічний образ) · SECRET_NAME breadth · Invariant B allow-list (не web/coap-deny) · drift billing-var doc-note. Доми → [`00_05 §2.7`](00_05_GitHub_Projects_and_IaC_Automation) + [`06_07 §1`](06_07_CICD_and_Runbook_Index)
- [x] 🤖 IaC-scan baseline-triage ✅ (2026-07-09, post-1st-CI-run) — 8 Trivy-findings review'нуто індивідуально (НЕ blanket-ignore): **4 REAL fix у terraform** (GCP-0030 `block-project-ssh-keys` · GCP-0020/0014 db-log-flags · **GCP-0033 boot-disk CMEK** — `kms.tf` keyring `silken-disk-ew1`, дім [`06_04 §5.6`](06_04_Secrets_Checklist); pre-deploy timing = нуль-cost ForceNew) + **4 by-design** → `.trivyignore` (GCP-0074 deny-all-misflag · GCP-0017/0031 Akash/Anchor-public-IP mitigated · DS-0026 Kamal-healthcheck). ⚠️ drift `init` передумова = GCS state-bucket [S5.6] провіжн + WIF-provider Variable (keyless auth, INF.22)
