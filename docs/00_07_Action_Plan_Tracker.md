...
- [ ] 🤖 Rails bump-guard + deploy-тракт: `FirmwaresController#deploy → OtaTransmissionWorker` arg-mismatch (сигнатури НЕ збігаються, fan-out по gateway'ях відсутній — деплой з UI мертвий) → полагодити тракт + guard `id > cluster-hiwater` (нова колонка `clusters.ota_version_hiwater`, structure.sql-squash) — знахідка backend-агента 2026-07-12
