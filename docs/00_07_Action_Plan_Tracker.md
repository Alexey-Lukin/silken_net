...
- [ ] 🤖 medusa-бінар sha256-verify: `solidity_audit.yml` качає medusa version-pinned БЕЗ `sha256sum -c` (aderyn/actionlint — verified), але [`06_07 §1`](06_07_CICD_and_Runbook_Index) числить medusa серед hash-pinned → drift. Fix: `.sha256`-fetch+verify (дзеркало aderyn-блоку, якщо реліз його публікує) АБО скоригувати 06_07 на version-pin-only. Hash-pin policy тут.
