...
- [x] 🤖 HW.1.PicoGK: **Деталь 4 radome v2c ✅ 2026-06-20** (`Radome.cs`, main) — hollow PEEK купол Ø25 + shield-дзвін ≥3/R≥5 + bayonet-socket (L-slot, mate Деталь-3 lugs) + PCB cavity (antenna↔Ti≥12) + rim O-ring groove; verify OK (hollow 71% · bell-rise 12.5 · cavity 13 · mate-fit) + 3 xUnit (**31 total**). **🏁 PicoGK анкер-CAD-родина ПОВНА** (coin→anode-v2→ARCH.25→barbs→Деталь3→Деталь4); machine-half анкера закрита. MATE-Ø → **виміряно** capsule-end assembly (рядок нижче)
- [x] 🤖 HW.1.PicoGK: **capsule-end assembly ✅ 2026-06-20** (`Assembly.cs` + `anchor_assembly` CEM) — перша **інтеграційна** верифікація: зводить Деталь 3↔4 за bayonet-datum (lock-groove↔lugs) і МІРЯЄ mismatch (radial −2.0 / bayonet-Z 6.42 / RF 8<12 мм) + skirt Ø30 vs inboard Ø25 Δ-кандидати. **Аудит-стіл** (mismatch = незведений Z-stack → HW.17/HW.8, не виробничий gate); 6 mate-math xUnit. Канон `02_02 §4.4`
- [x] 🤖 HW.1.PicoGK: **Zone-2 PEEK-втулка (Деталь 2) + повний осьовий стек ✅ 2026-06-20** (`Zone2Sleeve.cs` + `AxialStack.cs` + 2 CEM) — родина тепер **6/6 генераторів**; стек = **другий інтеграційний артефакт** (press-fit Zone 1↔2↔3, дзеркало capsule-end): **F1** shank Ø9 placeholder ≠ bore Ø11 = 1 мм зазор → НЕ press-fit · **F2** insertion budget 6 мм · **F3** span 63 мм + bus continuous → **HW.8.9**. AUDIT-стіл (render-sanity gate); +9 pure xUnit (Zone2Sleeve 3 + AxialStack 6 → 55 total). Канон `02_02 §4.5`. Grounding (web+source): BasePipe Z-origin verified (LEAP `Frames.cs`) · Noyron Boolean multi-part = наш `BoolAdd`+lift · H7/s6 press-fit = десятки µm (ISO 286) → F1 промах ~50×
