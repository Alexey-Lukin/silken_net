...
- [ ] 🤖 **founder-рішення 2026-07-03: SubGHz vendoring Шлях A** (свіжа сесія — firmware, compile-verify, НЕ money-path): `git submodule add stm32-mw-subghz-phy` @v1.5.0 (`extern/subghz-phy`, BSD-3-Clause, SHA `7dc059f3` = трійка з HAL v1.6.0+CMSIS v1.4.0) + реєстрація `RadioEvents_t` у обох `main.c` (закриває латентний `Radio.Init(NULL)`) + замінити owned-stub `hal_glue/radio.h` реальним → hal_check compile-lane бачить radio-шлях. Розблоковує money-path bench-зрізи (§12.5 pin-таблиця; sx126x.c для on-chip не існує — лише `radio_driver/`)
...
- [ ] 🤖 **founder-рішення 2026-07-03: chainsaw-тип-спліт** (свіжа сесія — новий `alert_type: :chainsaw_detected` enum + міграція `structure.sql` + `AlertDispatchService` дискримінатор acoustic-vs-thermal; зараз пилка+пожежа злиті в один `fire_detected`, `PANIC_FLAG`/cavitation TinyML-клас не спливає окремим alert-типом) — корисна СПОСТЕРЕЖУВАНІСТЬ незалежно від slash. ⚠️ **НЕ в A-сет** до field-validation TinyML (chainsaw-клас = synthetic placeholder `03_03 §4.2`, а `slash()` необоротний → A-сет консервативний ПРАВИЛЬНО, не борг)
...
- [ ] 🤖 **founder-рішення 2026-07-03: PRUNE** (свіжа сесія, money-path — verify money-integrity перед видаленням): видалити мертвий Toucan (0 enqueue-callerів, Ruthless Prune) — `Wallet#lock_for_toucan_bridge!` + `toucan_bridged_balance` колонка (міграція!) + `ToucanBridgeWorker` + `Toucan::BridgeService` (BRIDGE_ABI) + specs + `TOUCAN_BRIDGE_CONTRACT_ADDRESS` (`config/deploy.yml` + `06_04`) + канон (`04_01`/`04_02`/`05_01`/`07_01`). Зчеплено E.20 defer — expansion повертається з git при E.20-go. НЕ активувати (не на критичному шляху)
- [ ] 🤖 `finalize_spend!` (dead sibling) — видалити РАЗОМ з Toucan-prune (той самий escrow-примітив, що E.20 реюзнув би → при E.20-go воскресає з git + `[E.20]`-коментарем; [`04_06 §B.2`](04_06_Testing_Guide_and_Coverage)). ⚠️ `[target]`-marker як конвенція в репо НЕ існує (grep-verified) — реальна = видалення або `# [ID] DEAD`-коментар
- [ ] 🤖 (лише якщо колись активувати замість prune) `ToucanBridgeWorker.sidekiq_retries_exhausted` → симетричний rollback (`balance↑`+`locked↓`) + тест failure-path + in-flight інваріант `locked ≤ balance`
...
- [ ] 🤖 **founder-рішення 2026-07-03: ДЕМОУТ** (свіжа сесія, money-path — impact+тести): прибрати on-chain `sendRequest` LINK-cost (`Chainlink::OracleDispatchService#send_on_chain_request` — інакше перший prod-деплой за `06_04`-секретами платить LINK за callback, що не прилетить, `WEB3_STRICT_MODE` raise'ить без секретів), лишити tokenomics-mint (PATH 2) + ex-post clawback як чесну L0-custodial модель. Канон-reframe: 6 «Critical Path»/audit-critical Chainlink-claim-місць у [`05_01`](05_01_Multichain_Architecture) → чесний trust-origin. Замкнути PATH 1 відкинуто (DON-інженерія при TRL-3 передчасна). ⚠️ `chainlink_request_id` тоді теж — прибрати або чесно = tx_hash
