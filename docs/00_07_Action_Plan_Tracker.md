...
- **Стан:** Rake-конвеєр (Гілки A+B) канонізовано + host-доведено: `provisioning_sessions` AASM + **authenticated 2-Person Rule** (`approve_with_credentials!`/Argon2id; console-bypass закрито кодом — guard `credentials_verified?` → сирий `approve!` падає `AASM::InvalidTransition`, RSpec-покрито) + execute-шим (real subprocess capture / stop-on-fail) + **master-key DI ✅ (2026-07-02)**: preflight-ключ від `MasterKeySource` більше НЕ відкидається — `Session` несе його параметром у всі **три** HKDF-читачі (`HardwareKeyService` / `OtaHmacKeyService` / `SilkenNet::SeedDerivation.derive_seed` — третій, якого знахідка не називала; runtime-викликачі лишаються на ENV-fallback) → non-ENV adapter (Bitwarden/HSM) підключається без правок сервісів; DI-proof спеки (адаптерний ключ ≠ ENV → деривація йде з адаптера). bench-residual = фізичний SWD-флеш. Канон [`03_06 §5`](03_06_Factory_Flashing_and_Key_Provisioning) (+ §1 pipeline).
