...
| 7. Solana micro-rewards | Solana | RPC eviction, ATA missing, low SOL у gas wallet | ✅ **fallback-каскад** `SOLANA_RPC_URL_FALLBACK_*` (INF.22) — `Solana::MintingService#execute_rpc_call` пробує primary→fallback по черзі, per-service circuit-breaker "Solana" (`Web3::HttpClient`, 3 збої/60с); skip-clean (порожні → single-RPC). Solana ≠ EVM, тож не `ResilientClient`. Додатковий durable-захист: intent-marker + hourly reconcile (`BatchPayoutService`), `:not_found` on-chain → `manual_review` (ARCH.45/ARCH.51) — RPC-фейл не губить кошти, лише затримує. | Нічого (Solana — самодостатня rail). |
