...
| 7. Solana micro-rewards | Solana | RPC eviction, ATA missing, low SOL у gas wallet | ✅ **fallback-каскад** `SOLANA_RPC_URL_FALLBACK_*` (INF.22) — ⚠️ **у КОДІ; у деплой-ланцюгу ці змінні відсутні, тобто в проді каскад порожній** ([`ARCH.114`](00_07_Action_Plan_Tracker), переміряно 2026-09-01; те саме для Celo й Polygon — див. §3 Реалізаційні Якорі) — `Solana::MintingService#execute_rpc_call` пробує primary→fallback по черзі, per-service circuit-breaker "Solana" (`Web3::HttpClient`, 3 збої/60с); skip-clean (порожні → single-RPC). Solana ≠ EVM, тож не `ResilientClient`. Додатковий durable-захист: intent-marker + hourly reconcile (`BatchPayoutService`), `:not_found` on-chain → `manual_review` (ARCH.45/ARCH.51) — RPC-фейл не губить кошти, лише затримує. | Нічого. ⚠️ Micro-reward enqueue лише з unwired oracle-callback (ARCH.53 демоут) → pipeline зараз **latent** (PATH-1 сім'я, як IoTeX B1); «самодостатня» стосується durable-механіки, не досяжності. |
| 7. Celo ReFi | Celo | Forno RPC down, низький cUSD balance | Multi-RPC fallback (E.49 `RPC_FALLBACK_ENV_KEYS` = `CELO_RPC_URL_FALLBACK_1/2`, [`04_02`](04_02_Business_Logic_and_Services)) — ⚠️ **знову ж таки в КОДІ: у деплой-ланцюгу обидві змінні відсутні, тож у проді Celo їде на одному URL** ([`ARCH.114`](00_07_Action_Plan_Tracker) 2026-09-01) — + dedicated signer ARCH.50. Якщо всі RPC down → Sidekiq-retry; durable-захист = intent-marker + `CeloConfirmationWorker` reconcile для `:sent`. **Застрягле `:pending` без tx_hash (transient-timeout → dedup-skip, self-masking) → `CeloRewardReconcileWorker` sweep (:25/:55) → `:manual_review` (ARCH.64 — раніше тиха недоплата cUSD).** (🟡 target буфер-list `celo_pending_payouts` не реалізовано — INF.22.) | Нічого (Celo — самодостатня rail). |
| 8. Polygon + Hadron | Polygon EVM + Hadron compliance | Polygon RPC saturated, Hadron KYC service down | Polygon: ⚠️ `Web3::ResilientClient` circuit breaker + retry — **у КОДІ; у проді не інстанціюється взагалі** (`all_urls.size <= 1` → голий `Eth::Client`), бо фолбек-URL у деплой-ланцюг не доїжджає ([`ARCH.114`](00_07_Action_Plan_Tracker) 2026-09-01) — тобто живий тут лише Sidekiq-retry. Hadron: `hadron_kyc_status` — **персистентна колонка** wallet (НЕ кеш із TTL): уже-approved гаманці мінтяться і при лежачому Hadron; нові KYC чекають — **`HadronKycVerificationWorker` exhaust (retry:5, разовий `after_commit`-enqueue) → `HadronKycReverifyWorker` cron (:50) доверифіковує застряглі `pending` (ARCH.65 — інакше pending-KYC → тихий mint-skip назавжди).** `MintCarbonCoinWorker` retry 5× → `MintingRollbackService` → `manual_review` при невизначеному стані. | Крок 9 (The Graph) — оскільки graph індексує Polygon events. |
...
| Multi-RPC fallback — **Celo** (`RPC_FALLBACK_ENV_KEYS`) | `Celo::CommunityRewardService` (E.49 in [`00_07`](00_07_Action_Plan_Tracker)) | 🟡 **КОД реалізовано, у ПРОДІ порожній** — і різниця ярусів тут вирішальна. Переміряно 2026-09-01 ([`ARCH.114`](00_07_Action_Plan_Tracker)): `CELO_RPC_URL_FALLBACK_1/2`, `SOLANA_RPC_URL_FALLBACK_1/2` і `INFURA_POLYGON_RPC_URL` мають **нуль** входжень у `config/deploy*.yml`, `.kamal/**` і обох deploy-воркфлоу — заповнені вони лише в `.env.example`, тобто в контейнер не доїжджають. ⛔ **І доки URL один, `ResilientClient` не інстанціюється взагалі** (`all_urls.size <= 1` → голий `Eth::Client`): failover тут не «є, але мовчить», а фізично відсутній на ВСІХ ТРЬОХ мережах. Доти цей рядок стояв `✅ Реалізовано` і був єдиним місцем, де оператор під час RPC-інциденту читав би стан |
...
>
> ⛔ **Спільна передумова console-рецептів §4.4/§4.6 НЕ доведена:** `kamal app exec … "bin/rails console"` жодного разу не виконувався на живому контейнері — ⚠️ **і з 2026-09-01 причина ІНША, ніж тут стояло: деплой БУВ** (контейнер стартує), просто бут не завершується, тож живого процесу, у який заходити, не існує. Доки перший запуск не записаний, ці рецепти інертні. Дім передумови — [`06_07 §3`](06_07_CICD_and_Runbook_Index) (шапка Runbook-індексу) + [`00_07`](00_07_Action_Plan_Tracker) `OPS.20`; `coap`-роль для консолі не брати. ⊕ **2026-09-02: дорога існує, але ІНША за приписану** — `kamal app exec` з ноутбука падає на OS-Login-акаунті SA, а `gcloud compute ssh --tunnel-through-iap` + `docker exec` виконано (read-only runner); робоча форма — [`06_01`](06_01_Deployment_Kamal_Terraform) блок ops-команд, стан — [`OPS.20`](00_07_Action_Plan_Tracker).
