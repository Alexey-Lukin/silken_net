...
| 10. KlimaDAO | Polygon (KlimaDAO contracts) | Contract paused, approve revert | `KlimaRetirementWorker` (retry 3) існує, але **DEAD — 0 enqueue-сайтів** (активація = свідоме рішення). 🟡 target при активації: manual_review-хвіст + `ProtocolParameters`-toggle + ARCH.49 nonce-lock. | Нічого (retirement — окрема rail, не блокує emission). |
