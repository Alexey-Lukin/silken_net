...
- 🔴 **Колізія `METRICS_PORT` коштує не падіння, а СЛІПОТИ.** `MetricsExporter.start` має `rescue SystemCallError` (з регресійним спеком), тож N−1 процесів просто живуть без `/metrics`. Наслідок: порт візьме один, `up{process="job"}` лишиться 1, і `sn-alert-scrape-target-down` мовчатиме, поки money-SLO решти процесів не скрейпляться. Лік — ТРИ звʼязані правки: порт/сервіс per-процес · `expose` у `deploy.yaml` **І** в `.tpl` · нові таргети з унікальним `process`-лейблом у `config.alloy`. Остання найлегше забувається.
