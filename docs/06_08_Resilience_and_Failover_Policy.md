...
- offline у робочому стані → AASM `report_fault!` + `EwsAlert(queen_offline)` (критичний, анти-спам по кластеру) → `AlertNotificationWorker` (email/Telegram усім `admin`+`forester` організації **власника кластера**; push оголошено недоступним — `Notifications::DeliveryChannels`); повернення в ефір → `recover!` + auto-resolve алерту; attest-lapse спостереження (`last_attested_at` > 24h при online QATT-Королеві → метрика+warn). Grafana: `silkennet_gateways_faulty` P0-правило.
