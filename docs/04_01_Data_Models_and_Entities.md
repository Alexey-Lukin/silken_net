...
>
> ⛔ **`status`-скан — НЕ цей клас, і межа там ШКІДЛИВА** (ратифіковано ARCH.52): `where(status: :pending)` без `created_at` коректний, бо `:pending`-рядок буває як завгодно старим — KYC-skip, circuit-HOLD, накопичення колектора й `LockTimeout` лишають його `:pending` цикл за циклом, і нижня межа осиротила б застряглі кошти. ⚠️ Сирого reset `:processing→:pending`, на якому присуд спершу стояв, з 2026-09-22 у дереві немає ([`00_07`](00_07_Action_Plan_Tracker) ARCH.62: він знімав чужий claim) — упала ПІДСТАВА, заборона вистояла на цій. Важіль для скану — partial index `(status, created_at) WHERE status IN (0,1)`, він уже стоїть. Правило коротко: **`created_at`-вікно прунить звертання за ВІДОМИМ рядком (id/tx_hash); множину невідомого розміру прунить індекс.**
>
...
| `push_token` | string | Токен пристрою для FCM-доставки. ⛔ **Ратифікована відмова [ARCH.60]: multi-device таблиці device-token НЕ будувати** — singleton-колонки достатньо, бо FCM-клієнта до Phase-2-app фізично немає, і таблиця під нуль пристроїв є структурою під уявний попит. ⚠️ Транспорту НЕ МАЄ: `DeliveryChannels.available?(:push)` віддає жорсткий `false`, адаптера в дереві немає — поле збирається під канал, який ще не збудовано ([`ARCH.108`](00_07_Action_Plan_Tracker)). ⚫ Сусідні поля каналів знято разом із самими каналами: `phone_number` (⚖️ ARCH.78 2026-08-20, SMS) і `telegram_chat_id` (⚖️ [`ARCH.60`](00_07_Action_Plan_Tracker) 2026-09-06) — обидва за одним критерієм «поле без каналу є PII без цілі processing», і те, що `push_token` йому теж відповідає, **ВИРІШЕНО ⚖️ founder 2026-09-06 — але НЕ зняттям колонки**: знято ПОВЕРХНЮ ЗБОРУ (форма · екшен `update_settings` · маршрут `PATCH`), а колонка лишилась, бо канал не відкинуто, а відкладено до мобільного клієнта ([`ARCH.108`](00_07_Action_Plan_Tracker)). 🔴 Чому не самий `permit`: `notification_params` ніс РІВНО ОДИН ключ, тож зняття поля лишило б `permit()` порожнім, а `update({})` повертає `true` — «збережено» без збереження. **Отже читання стану лишилось, збір припинився, а майбутня нога нічого не втратила.** |
...
>
> **[E.66] Toucan-prune:** `lock_for_toucan_bridge!` / `finalize_spend!` / `toucan_bridged_balance` видалено (flow DEAD, 0 enqueue-callerів; failure-path мав money-integrity діру — несиметричний rollback). Escrow-примітив воскресає з git ЛИШЕ якщо колись зʼявиться підстава — ⛔ **не «при E.20-go»: маркетплейс ⚫ won't-do ⚖️ 2026-08-24 з конституційної підстави, тож умова, записана тут, не настане** (виправлено 2026-09-08; escrow тут є ярусом гільдійської економіки, який [`05_05 §3.1`](05_05_Slashing_and_Risk_Policy) відкликав) (locked у mint-flow = «сконвертовано назавжди» by design — finalize не потрібен).
...
| Enum | Значення |
|------|----------|
| `token_type` | `carbon_coin(0) / forest_coin(1) / cusd(2) / usdc(3)` — 🔴 **це ВАЛЮТА рядка, і вона ⊥ `blockchain_network` (транспорт)** [ARCH.120]: агрегат `net_minted_supply` фільтрує саме тут і мережі не читає, тож чужа валюта під `:carbon_coin` мовчки підсумується намінтованим SCC. `usdc(3)` заведено рівно тому — Solana-мікровинагороди писались у USDC під `:carbon_coin` |
...
- `fail(reason)` (any→failed, вкл. **manual_review**)
  ...
  - 🔴 **ДРУГА обовʼязковість того самого споживача, і її бракувало 9 діб — до інциденту canopy 2026-09-07.** Скоуп широкий не лише щодо `:manual_review`: `confirmation_scope` не фільтрує **термінальні** стани взагалі, тож повторний прогін на тому самому хеші (retry · sweeper `ARCH.55` · дубль-enqueue) бачить і вже-`confirmed` рядки. А події ідемпотентні **асиметрично**: `fail` приймає `:failed` — але цю ідемпотентність ОПЛАЧЕНО явним гардом усередині хука (`release_locked_points_on_fail! if aasm.from_state != :failed`), — тоді як `confirm` `:confirmed` не приймає. Тому споживач **мусить розділяти множину сам**: `settled, actionable = txs.partition(&:status_confirmed?)`. ⚠️ Ціна пропуску не «шумний Sentry»: виклик іде всередині `ActiveRecord::Base.transaction`, тож на батчі зі ЗМІШАНИМИ станами виняток на одному рядку відкочував підтвердження СУСІДІВ — такий батч не сходився ніколи (реально: 441 подія Sentry + 47 джоб у DeadSet на одному `tx_hash`). ⛔ **І лік НЕ в тому, щоб зробити подію самолупною**: її `before`-хук штампує `confirmed_at`, а по цій колонці `TreeChronicleService` сортує й датує хроніку дерева — самолуп переставив би дату on-chain події на час RETRY, тобто «чистіший» фікс тихо фальсифікував би свідчення ([`00_01 §1.1`](00_01_Vision_Mission_and_Roadmap)). 🔑 Дзеркальна вимога до revert-гілки: там уже-`confirmed` рядок є **РОЗБІЖНІСТЮ** з ланцюгом (reorg або наша помилка), не повтором — його не чіпають автоматично й про нього кричать поіменно. Периметр перевірено: `confirm!` має 4 сайти, решта три огороджені ярусом вище (`CeloConfirmationWorker` — `unless tx.status_sent?`; `MintingRollbackService` — `return if tx.status.in?(...)`; обидва Solana — `may_confirm? && !status_manual_review?`), і по всьому дереву це ЄДИНИЙ сайт, де транзакція обгортає цикл по колекції з різнорідними станами.
...
> **[ARCH.97/ARCH.96] `net_minted_supply(token_type)` — One-Home «скільки монет реально в обігу».** Σ(`confirmed` mints) − Σ(`confirmed` burns) для одного типу токена; burn розпізнається **колонкою `direction`** ([ARCH.95]; slash-інтент пишеться з ДОДАТНИМ `amount`, тож знак сумі його не видає, а `sourceable_type` після ARCH.95 відповідає на інше питання — «який burn є слешем»). Chainable — комбінується з `.for_cluster` / `.for_organization`. **Незворотні споживачі — поіменно:** поле `total_scc_supply` L1-якоря ([`05_04 §3`](05_04_Ethereum_L1_State_Anchor)) · база розміру спалення ([`05_05 §3`](05_05_Slashing_and_Risk_Policy)) · гард запасу монет ESG-погашення (`KlimaDao::RetirementService#retirable_scc` — [`05_01 §KlimaDAO`](05_01_Multichain_Architecture)). ⚠️ Тут стояло «**два** живі споживачі», і третій — саме той, що вирішує, скільки монет ГАСИТИ — пропав із переліку; клас відомий (переказ, що НАЗИВАЄ споживачів, гниє тихіше за код), тож перелік ведеться іменами, не числом. Читальні сайти (`ChainAuditService`, дашборд-поверхні) сюди свідомо не входять — вони не роблять незворотного, але успадковують ту саму величину. ⚠️ Величина **не кумулятивна** — slash її зменшує; для «скільки взагалі колись намінтили» це НЕ той метод.
...
| `total_service_fee` | decimal | Загальна сума оплати за послугу (> 0) — ⛔ не «funding»: імʼя колонки несе предмет (BIZ.22 F3) |
| `start_date` / `end_date` | timestamp | Строки контракту |
| `cancellation_terms` | jsonb | Ключ-сет умов дострокового виходу: `early_exit_fee_percent` · `burn_accrued_points` · `min_days_before_exit`. Читають `ContractTerminationService` (лише `burn_accrued_points`/`min_days_before_exit`) і `contracts/show` (рендер із запису); `burn_accrued_points = true` — єдиний шлях, яким `slash()` викликається як **погоджена форфейтура** (`contractual: true`, поза positive-A gate → [`05_05 §3.2`](05_05_Slashing_and_Risk_Policy)). ⚠️ `early_exit_fee_percent` — історичні ДАНІ без обчислювального читача: методи fee/refund зняті [BIZ.22, ⚖️ 2026-08-30 — Опція 1 MSA, без повернень і штрафів]. ⛔ **Саме імʼя `burn_accrued_points` перейменуванню НЕ підлягає — won't-do з підставою, мігровано з трекера 2026-09-04:** це JSONB-ключ ДАНИХ, тож перейменування = міграція рядків + канон-доми + спек-сайти, і підстава стоїть на ПОРЯДКУ величини роботи, а не на лічбі (яка дрейфує). ⚠️ Ключ при цьому **бреше про одиницю**: виконавець палить SCC-МОНЕТИ, не бали — це виправлено у ЗНАЧЕННЯХ, які бачить підписант, а не в адресі (курс 10 000:1, [`05_03`](05_03_Tokenomics_SCC_and_SFC)); носій у точці читання прапорця стоїть біля `store_accessor` у моделі |
...
| `insurance_premium_amount` | `total_service_fee * INSURANCE_PREMIUM_RATE` (5%) — обчислювальний метод |
| `forester_share_amount` | `total_service_fee * 0.95` — частка лісника (обчислювальний метод) |
| `self.total_insurance_premiums` | Σ премій (5%) по активованих (active/fulfilled/breached) контрактах; off-chain USDC-факт, НЕ on-chain подія. 🔴 **Викликається у ДВОХ формах, і вибір форми — рішення про приналежність, не про стиль:** на класі це агрегат **усієї платформи**, на relation (`org.naas_contracts.…`) — внесок однієї організації (`where` чейниться на `current_scope`, як у `BlockchainTransaction.net_minted_supply`). Фінзвіт ([`04_03 §5.14а`](04_03_REST_API_v1_Reference)) бере САМЕ relation-форму: класова клала pooled-агрегат по всіх орендарях у звіт одного, а це ще й securities-фактор F8 ([ARCH.90](00_07_Action_Plan_Tracker)). 💰 **Перелік статусів тут НЕСУЧИЙ, і одного разу він уже коштував грошей** (мігровано з трекера 2026-09-04): `BlockchainBurningService` перезаписував `:cancelled` на `:breached` наприкінці contractual-шляху, тож договір, який замовник ЗАКОННО скасував, лишався в цій множині — і 5% премії за ним далі стояли у Real-Yield звіті. Тобто дефект статусу читався не як помилка стану, а як ДОХІД. Шов закрито [SLASH-1, 2026-08-29]; **рефлекс: міняючи цей набір статусів, спитай не «чи він повний», а «чий рядок сюди потрапляє через ЧУЖИЙ дефект»** |
...
| `alert_type` | `severe_drought(0) / vandalism_breach(2) / fire_detected(3) / system_fault(5) / entropy_anomaly(6) / field_audit(7) / queen_offline(8) / queen_uplink_lost(9) / chainsaw_detected(10) / firmware_fault(11) / firmware_reverted(12) / firmware_canary_trip(13) / actuator_stuck(14) / emergency_response_undeliverable(15) / slash_dispatch_failed(16) / comms_fault(17) / hardware_fault(18) / telemetry_divergence(19)` (prefix: true) — 🔴 **`comms_fault(17)` [SLASH-1, 2026-09-04] вирізано з кошика `system_fault`, і підстава тут НОВА проти трьох попередніх виносів:** ті троє (`actuator_stuck` · `emergency_response_undeliverable` · `slash_dispatch_failed`) вирізали, бо винуватцем була ПЛАТФОРМА; цей — бо винуватця встановити НЕМОЖЛИВО (пускач = сатурований лічильник провалених flush-розмов Королеви, який не розрізняє, чий бік упав — наш CoAP чи бекхол оператора). Тобто дискримінатор «ХТО ПОРОДИВ» має третю відповідь, НЕВІДОМО, і при незворотному `slash()` вона падає на той самий бік, що й «ми» ([`05_05 §3.2`](05_05_Slashing_and_Risk_Policy) — асиметрія забороняє карати за невизначеність). ⚠️ Тип виводиться з `message_key` у ВОРКЕРІ (`ALERT_TYPE_BY_MESSAGE_KEY`), бо предикати `penalty_factor` читають `alert_type` і до ключа сліпі за побудовою; дедуп теж по ТИПУ, інакше стоячий `system_fault` іншого предмета глушив би рівно ту подію, задля видимості якої тип і вирізали. ⚖️ **ФОРМА решти розколу ратифікована 2026-09-04: один тип на КЛАС АТРИБУЦІЇ, не на подію** — `comms_fault(17)` забирає канальну родину (деградований uplink · слабкий CSQ — канон [`00_04 §2`](00_04_Nature_as_a_Service_Contracts) уже ратифікував, що «карати лісника за збитий шлюз = false slash»), `hardware_fault(18)` — залізну (`power_loss` · `gateway_overheat` · `gateway_freezing` · `gateway_hardware_fault` · `actuator_fault*`), а подію далі розрізняє `message_key`. Тип-на-ключ дав би стільки ж дискримінації дорожче. ✅ **ТРЕТІЙ розкол — `telemetry_divergence(19)`, DCI-розбіжність device-Z ⊥ server-Z (код 2026-09-04, канонізовано 2026-09-06):** підставу дав САМ канон, не новий аналіз — [`05_05 §6`](05_05_Slashing_and_Risk_Policy) називає дві причини divergence (whole-block replay = атакер · розбіжність версій прошивки після OTA = НАША) і виносить вердикт «самостійна divergence → категорія C (заморозка + peer-review), а НЕ автоматичний burn». 🔴 Отже сигнал, оголошений НЕДОСТАТНІМ для спалення, роздував МНОЖНИК того самого спалення — і через ОБИДВА предикати: у whitelist `comms_no_ack?` він потрапляв через спільний тип, хоча сигналом зв'язку не є ЗОВСІМ, а в `critical_unmaintained?` входив за замовчуванням, хоча виїзд лісника DCI-розбіжності не лікує (лік є РОЗСЛІДУВАННЯМ, як у `firmware_fault`). ⚠️ **Імʼя типу навмисно описує СПОСТЕРЕЖЕННЯ, а не вердикт** — канон відмовляє divergence у статусі доведеного шахрайства, тож `fraud_*` у типі стверджував би те, що §6 заперечує; `message_key` теж описує спостереження: `telemetry_divergence_detected` (⚖️ [SLASH-1], делеговано 2026-09-22 — ключ клієнт-видимий через API, а текст друкував «ФРОД» на операторському екрані). ⛔ Legacy-ключ `fraud_telemetry_detected` лишається в локалях із тим самим чесним текстом і без писача: рендер читає `message_key` з ДАНИХ, тож історичні рядки без нього впали б на `humanize`-фолбек — знову «Fraud telemetry detected» (пін — `alert_dispatch_service_spec`). Метрика `silkennet_telemetry_fraud_detected_total` сюди не належить — її інкрементує шлях ВІДХИЛЕННЯ пакетів у `TelemetryUnpackerService`, спільне лише слово. ⚠️ `hardware_decay` свідомо лишено в `system_fault` (він `medium`, тобто поза периметром за severity) — але при появі critical-писача переносити ОБОВʼЯЗКОВО. 🔴 **І кожен розкол мусить починатися з питання «хто ще читає цей токен НЕ за тим призначенням»:** перший тихо звузив `Gateway#system_fault?` (ops-вісь «чи їхати патрульному» ⊥ вісь атрибуції), тож заведено One-Home `EwsAlert::GATEWAY_FAULT_TYPES` — новий тип додається туди ТИМ САМИМ комітом. — ⛔ **цілі `1` і `4` зарезервовані, не вільні**: enum-значення лягає в колонку, тож переприсвоєння мовчки перейменувало б історичні рядки; вердиктів «комахи»/«сейсміки» немає, бо немає вимірювача — класу «комаха» в TinyML не існує ([`03_03 §4.2`](03_03_TinyML_Acoustic_Inference)), сейсмічного каналу теж [ARCH.102]; `field_audit` = аудит на місці (причина невизначена: no-data blackout / freeze без прямого доказу A / insurance-кандидат), свідомо окремий від `system_fault` (поломка заліза/зв'язку), щоб не накручувати penalty_factor через `comms_no_ack?` (gap-D) і не конфлатити дедуп ([SLASH-1](00_07_Action_Plan_Tracker)); `queen_offline(8)`/`queen_uplink_lost(9)` = дзеркальна пара gateway-health (Rails помітив тишу / Королева кричить через Helium — ARCH.54/34); `chainsaw_detected(10)` = acoustic-anomaly АБО panic-TX без термального сигналу ([SLASH-1] спліт + P0-фікс гейта `panic? \|\| bio_status_anomaly?`: реальна пилка їде panic-кадром зі status=homeostasis; non-fire маршрут dClimate → Field-Audit; ⚠️ НЕ в A-сет slash'а до field-validation TinyML); `firmware_fault(11)` = софт-збій прошивки (wire `vm_error`: mruby-crash/OOM/unprovisioned — vendor-attributable ops-тріаж; НЕ в A-сеті, НЕ в `comms_no_ack?`, виключений з `critical_unmaintained?`); `vandalism_breach(2)` — [SLASH-1 P0] авто-writer'а немає, створюється лише ручною Field-Audit C→A ескалацією ([`06_08 §4`](06_08_Resilience_and_Failover_Policy)); `actuator_stuck(14)` = [ARCH.58] Rails загубив слід ВЛАСНОЇ команди (актуатор числиться active понад вікно наказу) — vendor-attributable, як `firmware_fault`, тож поза A-сетом, поза `comms_no_ack?` і виключений з `critical_unmaintained?`; носій свідомо ВЛАСНИЙ, бо `system_fault` дав би подвійний упліфт, а cluster-level `field_audit` осліпив би per-tree dead-man switch ([`06_08 §1.3`](06_08_Resilience_and_Failover_Policy)); машинного resolve не має — фізичний стан пристрою невідомий; `emergency_response_undeliverable(15)` = [ARCH.75] аварійну фізичну відповідь **не відправлено** — єдиний тип, що свідчить про НЕ-дію: команд не створено взагалі, замість невалідних рядків, які доти лягали мовчки й не вміли навіть померти. Причин ЧОТИРИ на двох осях дедупу (дім механіки — [`04_02 §7`](04_02_Business_Logic_and_Services)): доїхати не може КОНКРЕТНИЙ пристрій (протокол просить понад `Actuator#max_active_duration_s` АБО каденс шлюза довший за вікно релевантності кроку) ⊥ виконати нема ЧИМ цілий крок протоколу (пристрою цього типу в кластері не встановлювали АБО жоден зі встановлених не придатний — стан пристрою чи мовчазний шлюз). ⚠️ Друга вісь свідомо не колапсує в «немає інструментів»: «залізо не купили» і «залізо є, але недосяжне» — різні дії людини, і згортання їх в одне повідомлення приписувало б операторові власну недоробку платформи; класифікація дзеркалить `actuator_stuck` (vendor-attributable → поза A-сетом, поза `comms_no_ack?`, виключений з `critical_unmaintained?`), бо конфігурацію задали МИ, і виїзд лісника числа не лікує; `slash_dispatch_failed(16)` = [SLASH-1 ⚖️ 2026-09-04] НАШ `slash()` не доїхав on-chain (RPC-збій; `burn_failure_ambiguous` = міг піти в мемпул). Класифікація дзеркалить `actuator_stuck`, але підстава ГОСТРІША за решту трійки: попередні типи заводили, **щоб не потрапити** в `system_fault`, а цей — щоб **вийти** з нього, бо подія вже там сиділа. Доти провал власного спалення створював critical-алерт на кластері, що на наступному проході садив `penalty_factor` оператора рівно на стелю (`PF_NO_ACK` + `PF_NO_MAINTENANCE` = `PENALTY_FACTOR_MAX`) — той самий self-ref, який уже лікували для `vandalism_breach` [P1-3], лише винуватцем була ПЛАТФОРМА. Машинного resolve немає: для `ambiguous` припис прямий — звірити експлорер ([`06_08 §4`](06_08_Resilience_and_Failover_Policy)). ⛔ **`entropy_anomaly(6)` — ІСТОРИЧНИЙ з 2026-09-05:** писача (`ClusterEntropyAnalyzerWorker`) знято, значення лишається ЛИШЕ щоб архівні рядки рендерились (`EwsAlert#message` є РЕНДЕРОМ, локаль-ключ `entropy_pre_stress` живий із тієї ж підстави) — **не брати його під новий кластерний детектор**; вимір, що вбив підставу, стоїть над самим значенням у `app/models/ews_alert.rb` і в [`06_03 §2.6`](06_03_Prometheus_Observability) |
...
| `assigned_to_id` | bigint, FK→users, nullable | **[E.20]** Виконавець, що взяв тривогу. Пишуть лише `EwsAlert#claim!`/`#release!` — обидва вимагають `status_active?`, тож приєднання ПІСЛЯ резолюції неможливе за побудовою. Претензія лише на нічию (`AlreadyAssigned` → 409); відпустити може виконавець **АБО** admin+ (`NotAssignee` → 403), інакше хибний клік замикав би тривогу назавжди. ⛔ **Третього дієслова — прямого ПЕРЕХОПЛЕННЯ чужої тривоги — свідомо НЕМА** (⚖️ founder 2026-08-28): сценарій «А поїхав, їде Б» закривають наявні примітиви (`release!` дозволений і admin+, тож адмін відпускає — Б бере), а будь-яка форма односкладового перехоплення сьогодні була б здогадом про робочий процес, якого ще ніхто не проходив; тригер перегляду — перша реальна колізія в полі, не аналогія. |
...
| `dclimate_ref` | string | ⚠️ **НЕ посилання на dClimate — рядок генерується ЦІЛКОМ у нас** (`generate_dclimate_ref`: наш таймстемп + `SecureRandom.hex(8)` + вікно запиту). Жодного CID, підпису чи он-чейн-прив'язки вендора в ньому нема й не було; це аудит-трейл ВИБІРКИ, з якої виведено вердикт (ARCH.111), а не свідчення третьої сторони. 🔴 Опис «посилання на dClimate» стояв тут до 2026-09-05 і був класом **СЛОВО** ([`00_01 §1.1`](00_01_Vision_Mission_and_Roadmap)) — мітка, що ручається, за якою не стоїть ніхто. ⛔ Префікс стане ХИБНИМ у день зміни джерела й міняється разом із дротуванням, одним комітом ([`00_07`](00_07_Action_Plan_Tracker) S3.2) |
...
| Метод | Опис |
|-------|------|
| `resolve!(user:, notes:, key:, params:)` | Закрити тривогу + закрити пов'язаний MaintenanceRecord. [I18N.1] Слід закриття — запис у **`resolution_log`** (jsonb-масив): машинні викликачі дають `key:`+`params:` (у БД їде ідентифікатор події, фраза збирається локаллю ГЛЯДАЧА через `#resolution_texts`, fail-open `humanize` — той самий контракт, що `message_key`), людина — вільний `notes:` (text-запис, мова резолвера, не локалізується). Без обох — дефолтний ключ деривується від АГЕНТА (`operator_closed` ⊥ `system_closed`). Час — поле `at` самого запису. Ключі — `alerts.resolutions.*` ×4; appendери поза resolve! (dclimate) кличуть `#log_resolution`. 🔒 **Доказ Категорії A** (`CATEGORY_A_EVIDENCE_TYPES`, сьогодні `vandalism_breach`; перелік читають і `Slashing::CauseEvidence`, і uplift-предикат `critical_unmaintained?`) `resolve!` закриває ЛИШЕ для платформи — `closable_by?(user)` істинний тільки для `super_admin`, інакше `EvidenceLocked` (і для машинного виклику без `user:`): 403 у `alerts#resolve`, воркер лікування не закриває, рядок UI кнопки не показує. Бекстоп — валідація `on: :update`: будь-який інший вихід доказу з `:active` (`ignore!`, голий запис статусу) без платформи-резолвера → `RecordInvalid`. Закриття доказу не переписує привʼязаних записів обслуговування — вони належать звинуваченій організації (SLASH-1, ⚖️ делеговано 2026-09-22; політика — [`05_05 §3.2`](05_05_Slashing_and_Risk_Policy)) |
...
| `decommissioning` | 4 | Демонтаж. ⚠️ **Одна рука, і це вихід дерева з флоту:** `EcosystemHealingWorker` асинхронно одразу після створення запису робить дерево (`:active`/`:dormant`) `removed` — без підпису й без фото (фото на кожне збереження вимагають лише `repair`/`installation`, `#evidence_backed?`; `biomass_extraction` — лише на створенні), сам перехід запускає `Tree#trigger_slashing_protocol`, а гілка смерті рахує `removed` поруч із `deceased` ([`05_05 §3`](05_05_Slashing_and_Risk_Policy)). Ризик у бік заниження (бенефіціарові шкодити собі невигідно); чия друга рука оформлює смерть, що прийшла тишею, — [`00_07`](00_07_Action_Plan_Tracker) SLASH-1 (виміряно 2026-09-23) |
...
⊕ **І цей документ стереже ще одна вісь того самого скрипта — перевірка 5, заведена [SLASH-1]:** значення `EwsAlert` `enum :alert_type` ⟷ згадка КОЖНОГО тут, бо незадокументований тип є незадокументованим ВХОДОМ у незворотний `slash()`. ⛔ Оголошена стеля — гейт судить ЗГАДАНІСТЬ ключа, ніколи правильність його підстави. (Нумерація 4-ї перевірки пропущена свідомо: вона про [`04_02`](04_02_Business_Logic_and_Services), не про цей документ.)
...
**Решта (ручний семантичний аудит, [`00_06 §3.0`](00_06_SSOT_Documentation_Standard), поки не автоматизовано):** кожна `include AASM`-модель має state-перелік у своєму §; поліморфні `_type/_id` пари §10 «Карта Зв'язків» ⟷ реальні колонки `structure.sql`.
