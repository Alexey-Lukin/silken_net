...
> - якщо **документ випередив схему** (модель/таблиця документована як існуюча, але немає у `structure.sql` чи `app/models/`) — створюємо/оновлюємо запис у `docs/00_07_Action_Plan_Tracker` як невиконану задачу;
> - якщо **нема ні там, ні там** — приймаємо рішення (потрібне → реєструємо в `00_07`; не потрібне → видаляємо плани з 04_01).
>
...
> Періодичний аудит — кожен Cool-down цикл Shape Up (`00_04` §5.5).
...
2. Якщо drift вимагає коду (наприклад, треба додати колонку у модель, бо схема її має, а AR не знає) — заведіть запис у `docs/00_07` з UID (`E.NN` / `S6.NN`) і посиланням сюди.
