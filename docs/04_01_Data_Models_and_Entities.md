...
| `otp_required_for_login` | boolean | **[S6.21] Заявка, а не механізм:** шлях входу цього прапорця не читає (`sessions#create` + `authenticate_user!`), тож підняте значення захисту не додає — воно лише друкує «MFA Active» на трьох поверхнях (`users/profile` · `account_security#show` · `user_blueprint`). Тому напрямок **увімкнути закритий** (`account_security#toggle_mfa` → 501 `mfa_not_implemented`), а вимкнути лишається відкритим — він заявку знімає. Звʼязок «вхід перевіряє ⟷ запис дозволений» тримає `spec/security/mfa_claim_honesty_spec.rb`; відкриється разом із TOTP → [`00_07`](00_07_Action_Plan_Tracker) S6.21 |
...
**Методи:** `find_with_partition_pruning(id, created_at = nil)` _(клас)_, `net_minted_supply(token_type)` _(клас)_, `net_minted_by_cluster(cluster_ids, token_type)` _(клас)_, `explorer_url`, `solana_network?`, `celo_network?`, `broadcast_status_change`.
...
| `emitted_tokens` | numeric, **nullable** | ⚫ **МЕРТВА КОЛОНКА — семантику знято ⚖️-присудом 2026-08-19** [ARCH.103]. Писача не було НІКОЛИ, а величина не мала ВИЗНАЧЕННЯ: кластер несе кілька контрактів одночасно (перетин періодів не заборонений), а mint-рядок не має посилання на контракт — його має лише burn (`sourceable: NaasContract`). Присуд: контрактну семантику скасовано на користь КЛАСТЕРНОЇ, усі HTML-поверхні перепідписані на `BlockchainTransaction.net_minted_by_cluster` / `.for_cluster(…).net_minted_supply`. ⚠️ Колонку ще НЕ знято зі схеми: три JSON-читачі віддають її назовні (серед них голий `as_json` без `only:`), і жоден request-приклад її не пінить — тобто зняття пройшло б ЗЕЛЕНИМ, мовчки забравши поле в споживача. Порядок: спершу дати JSON кластерну величину, тоді різати ([`00_07`](00_07_Action_Plan_Tracker) ARCH.103) |
...
| `audit_logs` — `ip_address` · `user_agent` · `user_id` | 🔴 **PII в APPEND-ONLY** | Ядро напруги з [`ARCH.57`](00_07_Action_Plan_Tracker): ланцюг tamper-evident, тож рядок не видаляють — erasure тут можлива лише ПСЕВДОНІМІЗАЦІЄЮ поля при збереженні хеш-ланцюга. Це і є те, що `AnonymizeUserService` мусить розвʼязати, і чому він НЕ зводиться до `destroy_all` |
...
| `maintenance_records` — `biomass_passport_tx_hash` | **НЕ PII** | Хибний позитив свіпу по імені: TX-хеш паспорта біомаси Puro.earth |
