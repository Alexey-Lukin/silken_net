...
| `direction` | string | **[ARCH.95]** Напрямок руху: `mint` (емісія) / `burn` (вилучення з обігу). `NOT NULL`, default `mint`. 🔴 Дискримінатор ОДИН для всіх агрегатів (`net_minted_supply`, `net_minted_by_cluster`, `#burn?`) — доти напрямок ДЕРИВУВАВСЯ з `sourceable_type = NaasContract`, і [ARCH.101] цю деривацію ратифікував на передумові «єдиний slash-шлях», яку ESG-погашення зняло. ⛔ Знак `amount` напрямку НЕ несе: slash пишеться ДОДАТНИМ. Причин `burn` дві — slash (несе `sourceable`) і ESG-погашення (не несе); інваріант `slash_intent_must_be_a_burn` не дає першій поїхати мінтом |
