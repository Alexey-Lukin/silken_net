...
- **Поточний TRL:** TRL 8 — Схема БД затверджена, міграції написані, поліморфні зв'язки та індекси оптимізовані для planetary-scale highload. Відкрите: SSOT Drift Register моніторинг (§12) → [`09_06`](09_06_Action_Plan_Tracker).
...
| [09_06_Action_Plan_Tracker](09_06_Action_Plan_Tracker) | Open backlog (SSOT Drift Register §12) |
...
| `vpd` | numeric | **[HW.32]** Vapor Pressure Deficit (kPa) — прямий confounder сокоруху (False-Slashing guard, `05_05 §6/§7`). Hot-path: device шле VPD-індекс; nullable. ⚠️ НЕ входить у Lorenz-Z (DCI-guard) |
...
> - якщо **документ випередив схему** (модель/таблиця документована як існуюча, але немає у `structure.sql` чи `app/models/`) — створюємо/оновлюємо запис у `docs/09_06_Action_Plan_Tracker` як невиконану задачу;
> - якщо **нема ні там, ні там** — приймаємо рішення (потрібне → реєструємо в `09_06`; не потрібне → видаляємо плани з 04_01).
>
...
> Періодичний аудит — кожен Cool-down цикл Shape Up (`09_03` §5.5).
...
2. Якщо drift вимагає коду (наприклад, треба додати колонку у модель, бо схема її має, а AR не знає) — заведіть запис у `docs/09_06` з UID (`E.NN` / `S6.NN`) і посиланням сюди.
