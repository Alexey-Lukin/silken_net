...
```
10 000 uAKT/block × 10 блоків/хв × 60 × 24 × 30 = 432 000 блоків/міс
432 000 блоків × 10 000 uAKT = 4 320 000 000 uAKT = 4 320 AKT/міс   ← стеля web-сервісу
усі чотири сервіси (10 000+5 000+2 000+1 000 = 18 000 uAKT/блок) → 7 776 AKT/міс
```

> 🔴 **Тут стояло «432 000 AKT/місяць» — завищення рівно в 100 разів проти власної ж формули** (виміряно 2026-08-29 двома незалежними проходами). Помножено 432 000 блоків на 10 000 і водночас лишено зайвий множник; конверсія ÷10⁶ у другому рядку була правильна. ⚠️ Клас, а не описка: число стояло в доці, чия таблиця §5 стверджує «потенційно дешевше», тобто **єдиний кількісний аргумент на користь платформи був хибним на два порядки й ніким не перевірявся**. Той самий вузол зіпсовано інакше в `deploy/akash/deploy.yaml` (коментар ділить на 10⁶ величину, що вже в AKT). **Числа цього доку перераховують, не цитують.**
...
| **Цензурна стійкість** | ❌ (Web2, GCP control) | ⚠️ **НЕ ДОВЕДЕНО** — див. нижче | Мета архітектури, **не стан** |

> 🔴 **Клас `СЛОВО` — мітка без вимірювача ([`00_05 §7`](00_05_AI_Native_Operating_Model), Фолкнер), і припис для нього однаковий: дати вимір або зняти поверхню.** Виміряно 2026-08-29 трьома незалежними проходами. (1) Твердження живе рівно в прозі: сім місць, **нуль** у [`00_01`](00_01_Vision_Mission_and_Roadmap), **нуль** у [`SECURITY_ASSURANCE_CASE`](SECURITY_ASSURANCE_CASE), **нуль** у `spec/`; жодного гейта, метрики чи пункту трекера за ним немає. (2) У **нашій власній** моделі загроз Akash-провайдер стоїть **актором загрози** (`SECURITY_ASSURANCE_CASE §2.2`), не несе гарда в жодній із шести меж довіри й двічі згаданий у residual risks ПРОТИ нас; у [`06_08 §2.2`](06_08_Resilience_and_Failover_Policy) він — ланка, що ламається, а GCP — той, хто ловить. (3) 🔑 **Найважче: под на Akash не бутиться без GCP** — `bin/docker-entrypoint` без автентифікації Cloud SQL Auth Proxy (`GCP_SA_KEY_BASE64`) виходить `exit 1`, а INF.22-супервізор робить це постійним; стан у Cloud SQL, бекап у GCS, CoAP-PRIMARY на анкері (INF.17), TLS у Cloudflare, образ у GHCR. **Отже рахувати «Akash + GCP» як дві незалежні ноги — це подвійний рахунок одного контролера**, заборонений ⛔-правилом [`00_05 §7`](00_05_AI_Native_Operating_Model) (Аттар/Навої: другий деплой із тим самим контролером вибірки не додає свідка). ⛔ **Не відновлювати «✅» без вимірювача.** Чесний присуд формулюється не «який вендор безпечніший», а **«яку концентрацію ми приймаємо — і чи ми її НАЗВАЛИ»**; носієм цього є інфра-половина реєстру стоячих повноважень ([`00_07`](00_07_Action_Plan_Tracker) `ARCH.114`).
