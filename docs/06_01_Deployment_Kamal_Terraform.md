...
⛔ **Чого ця таблиця НЕ може знати, і це не пропуск інвентаря, а межа дерева:** на кого оформлені акаунти, хто платить, чи є 2FA й recovery-контакт, чи має хтось, крім власника, доступ бодай до одного важеля (`iap_admin_members` у `posture.auto.tfvars` з 2026-08-31 несе ОДНУ адресу власника — тож питання вже не «чи є доступ», а «чи є ДРУГА пара рук»; дефолт змінної лишається `[]`), і де фізично лежать `RAILS_MASTER_KEY`/`PROVISIONING_MASTER_KEY` (процедура зберігання у vault — 👤, невиконана, `DR.1`). **Ці рядки заповнює людина, і саме вони перетворюють інвентар на присуд.**
