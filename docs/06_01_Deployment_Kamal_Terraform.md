...
| **7** | **Schema bootstrap від squashed init_consolidated** | **[INF.7 — Phase 7]** На свіжій базі деплой `bin/rails db:setup` (= `db:create` + `db:schema:load` + `db:seed`). Ми **НЕ** використовуємо `db:migrate` в продакшні до першого деплою — всі pre-launch міграції згорнуті в `db/migrate/20260709140000_init_consolidated.rb` (останній сквош 2026-07-09), а схема живе в `db/structure.sql` (включно з усіма 9 Codex-таблицями + 4 RANGE-партиційними таблицями + початковими партиціями `_default` + поточним вікном). `schema_migrations` містить рівно ОДИН рядок `20260709140000`. Якщо хтось додає incremental міграцію після цього — `StrongMigrations.start_after = 20260709140000` змусить її пройти всі checks. **НЕ** робіть squash повторно після першого деплою (втратите history) без zero-downtime плану. |
...
# Web3 oracle keys (інакше Sidekiq DeadSet; legacy ORACLE_PRIVATE_KEY знято з kamal-ноги — INF.22):
