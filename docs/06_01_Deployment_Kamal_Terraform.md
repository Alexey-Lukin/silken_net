...
- **Поточний TRL:** TRL 5 (🔺 `4→5`, TRL Gate Event — ⚖️ founder 2026-09-03; критерій §4.1 «HIL-валідація §3» виконано: `HilQueenSimulator` провів CoAP-інтейк кінець-у-кінець у живому середовищі, слот обслуговує трафік. Дім числа — [`00_03 §1`](00_03_TRL_Matrix_HIL_and_Beyond), тут дзеркало-джерело) — інфраструктура РОЗГОРНУТА (перший `terraform apply` 56/56, 2026-08-31), а **`kamal deploy` ПРОВОДИВСЯ й провів увесь ланцюг до запущеного контейнера** (2026-09-01: WIF → AR → OS Login → IAP → SSH-під-SA → мережа → proxy → build/push/pull → старт → Rails-бут). 🔑 **Точне формулювання несуче в обидва боки, і доти цей рядок брехав у бік бідності — а цитували його як «дім факту» щонайменше чотири інші сторінки:** «деплою не було» БІЛЬШЕ НЕ ПРАВДА, «застосунок працює» — ЩЕ НЕ ПРАВДА. 🔴 **І цей хвіст СУПЕРЕЧИВ власному початку рядка до 2026-09-05 — підйом 4→5 дописали згори, а підставу внизу лишили.** Стояло: «бут не завершується (`web3_network_guard` на незаведених адресах слоту), тож трафіку не обслуговувано жодного разу». Обидві половини читались як чинні, і хвіст брехав у бік БІДНОСТІ — саме той напрямок, що не має хибного токена й тому виживає найдовше. ✅ **Чинний стан, виміряний 2026-09-05:** `INF.27` закрито (адреси слоту заведені), `Kamal Deploy (Canopy)` = `success`, `/up` = 200, `/ready` = `{database: true, redis: true}`, слот обслуговує трафік. Console-рецепти репо БІЛЬШЕ не інертні за цією причиною — доступ виміряно живим (`gcloud … --tunnel-through-iap` + `docker exec`, скіл `deploy` #10).
