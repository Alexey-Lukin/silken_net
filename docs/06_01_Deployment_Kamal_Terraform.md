...
| [`00_07` — Action Plan Tracker](00_07_Action_Plan_Tracker) | S1.1, S1.5, DEPLOY-1 (INF.4 · INF.6 · S5.6 → §🗄️) |
...
[`06_04 §2`](06_04_Secrets_Checklist), приймальний рядок `Listening on coap://0.0.0.0:5683 slot=<слот>`
→ Фаза 4.
...
> назвали»**. ⚠️ Дім самої ВІДПОВІДІ — таблиця важелів нижче в цій секції разом із ⛔-блоком «чого дерево знати не може»; відкритим лишився лише сам ПІДПИС — нога [`00_07`](00_07_Action_Plan_Tracker) `DEPLOY-1` Фаза −1 (ARCH.114 закрито й заархівовано 2026-09-02, коли другий RPC-птах заведено). Розрізняти несуче: інакше вказівник замикається в кільце — канон шле в трекер, трекер назад у канон.
...
⛔ **Чого ця таблиця НЕ може знати, і це не пропуск інвентаря, а межа дерева:** на кого оформлені акаунти, хто платить, чи є 2FA й recovery-контакт, чи має хтось, крім власника, доступ бодай до одного важеля (`iap_admin_members` у `posture.auto.tfvars` з 2026-08-31 несе ОДНУ адресу власника — тож питання вже не «чи є доступ», а «чи є ДРУГА пара рук»; дефолт змінної лишається `[]`), і де фізично лежать `SECRET_KEY_BASE`/`PROVISIONING_MASTER_KEY` (другий у vault + offline з 09-01; офлайн-копія першого — 👤, невиконана, `DR.1`; `RAILS_MASTER_KEY` з 09-02 незамінним не є). **Ці рядки заповнює людина, і саме вони перетворюють інвентар на присуд.**
...
| **Grafana Alloy (metrics agent)** | ✅ | — | — | Kamal **accessory** (`files:`-монтування `deploy/alloy/config.alloy`), скрейпить пʼять таргетів по стабільних DNS-аліасах ролей: три production (`silken-web`/`-job`/`-coap` — ⚖️ [OPS.37 2026-08-30]) + два canopy (`canopy-web`/`-job` зі `slot=canopy` на таргеті — ⚖️ [OPS.37 2026-09-03], механіка в ноті web-ролі `config/deploy.yml`); пускач = крок «Ensure Alloy accessory is running» в обох deploy-воркфлоу (boot ідемпотентний; зміна `config.alloy` → свідомий `kamal accessory reboot alloy`) |
...
✅ **Присуд ⚖️ founder 2026-09-03 ([`00_07`](00_07_Action_Plan_Tracker) INF.28): canopy отримує ВЛАСНИЙ Redis як Kamal-accessory на app-хості** (`config/deploy.canopy.yml` `accessories.redis`: `redis:7.4-alpine`, AOF на диску хоста, `requirepass` витягується в `cmd` з того самого `REDIS_URL`, який overlay ремапить із `CANOPY_REDIS_URL` — один секрет, один дім; порт опубліковано на VPC-адресі хоста, щоб анкерний демон дістав його як `redis://:<hex>@<app_host_ip>:6379/0`; бутиться кроком `Kamal Deploy to Canopy` ПЕРЕД ролями, ідемпотентно). Підстава — квота, не смак: job-роль на Free спалила б 500 k команд за ~1.2 доби. Production лишається Upstash за побудовою (платний план — [`00_07`](00_07_Action_Plan_Tracker) S1.1), `deploy_workflow_parity_spec` тримає `REDIS_URL ≠ CANOPY_REDIS_URL`. Ціна названа: staging-SPOF без керованої durability (втрата хоста = черга canopy й testnet-nonce-и, [`06_06 §5.4`](06_06_Disaster_Recovery_and_Backup)); підстава відмови `options.publish` для Alloy ([`06_03 §2.9`](06_03_Prometheus_Observability)) тут не діє — accessory не роллиться.
...
- [x] **SSL/TLS режим `Full (strict)`** — Cloudflare→origin вимагає валідного сертифіката на
      ...
      віддавав би 521/525 — **пройдено**, а не чинний. ⚠️ **Custom, не Automatic:** режим CF
      «Automatic SSL/TLS mode» переобирає його САМ за сканом origin (побачено увімкненим
      2026-09-02 вранці, next scan Sep 26 / Sep 30) — тобто чужий автомат над ратифікованим ⚖️.
      ✅ **Закріплено того ж вечора на ОБОХ зонах: SSL/TLS → Configure → Custom → Full (strict)**
      («Automatic mode disabled»), тож планові скани режим більше не переобирають; без цього
      присуд тримався б дефолтом вендора.
...
> ⚠️ **Кроки 1 і 5 чесні ЛИШЕ з VM або з CI** (`gcloud compute ssh … --tunnel-through-iap`, або джоба): з ноутбука за корпоративним TLS-перехоплювачем (Zscaler — §TLS-fallback крок 2) `openssl s_client` показує сертифікат проксі, а «direct hit blocked» не відрізняється від «проксі не пустив». TLS-ярус із ноутбука неміряний ЗА ПОБУДОВОЮ — не читай його зелень як вердикт (INF.4).
...
|---------|------------------|-------------|
| `curl https://… → 525 SSL handshake failed` | Cloudflare→origin не може встановити TLS. ⚠️ **Спершу спитай, ЯКИЙ хост:** apex `silkennet.app` віддає 525 ОЧІКУВАНО, доки base-маніфест не рендерився (kamal-proxy має `host:` лише для `canopy.silkennet.app`; виміряно 2026-09-02) — це не сертифікат і не привід чіпати режим | Лише якщо 525 на ЖИВОМУ хості: перевірити, що origin має валідний сертифікат (пара Origin CA, збіг МОДУЛЯ). ⛔ **`Full (strict)` НЕ знижувати до `Full`** — ратифікований ⚖️, і саме його CF-автомат «Automatic SSL/TLS mode» уміє переобрати сам; діагностику веди з VM (`openssl s_client` на origin), не режимом |
...
**Upstash ×1** (production; canopy — self-hosted Kamal-accessory на app-хості, ⚖️ 2026-09-03; тариф — за КОМАНДАМИ, не за інстансами: [`00_07`](00_07_Action_Plan_Tracker) INF.28) → `rediss://` URL (+ `redis://:<hex>@<app_host_ip>:6379/0` для canopy) · **два домени:
...
```bash
# 1) ВІКНО REPLACE — один раз, ✅ ВИКОНАНО 2026-09-03 (⚖️ founder, INF.17): скрипт переїхав у `metadata.startup-script`
#    (`compute.tf`), replace анкера пройшов (~1.5 хв), state чистий, `app-host-ip` виставлено знову. Відтоді зміна
#    образу/скрипта = in-place + reset — ⚠️ але **`reset` потрібен НЕ завжди, і межа коштує рестарту живого інтейку**: heredoc пише `coap.env` під гардом `if [ ! -f … ]`, тобто на ЗАПОВНЕНОМУ анкері зміна шаблона не доїжджає ніколи й `reset` не купує нічого, крім бонсу PRIMARY CoAP-демона (виміряно 2026-09-03: рядок `ACTIVE_STORAGE_SERVICE=local` довелось дописати в живий файл РУКАМИ). `reset` потрібен там, де скрипт справді має ПЕРЕвиконатись — пін образу, HAProxy-конфіг, systemd-юніт. Ланцюг нижче лишається рецептом на випадок НАСТУПНОГО replace (напр. machine type):
#    static IP і DNS переживають, boot-диск, coap.env і `app-host-ip` — ні.
...
✅ **Sepolia — 2026-09-02 (chain 11155111), деплоєр = ОДНОРАЗОВИЙ ключ, не MetaMask.** `Deploy.s.sol` лишає деплоєра без жодної ролі (усі права йдуть `ADMIN_ADDRESS`, оракулам і Timelock), тож ключ згенеровано `cast wallet new --json` у `~/.silken_keys/sepolia_deployer.json` (mode 600) і жодного разу не показано — це зняло експорт ключа з MetaMask як крок узагалі; профінансовано **Google Cloud Web3 faucet** (див. нижче), не Chainlink. Адреси (EIP-55): Timelock `0x430c8e6E5545be0CF3cc33Cd24F2e3E65e840F83` · SCC `0x31FA6795F0ED8db8215edaB376aaFe55C5d16DD1` · SFC `0x2bE18b42DF3561Cc0e2250f5047cA170fcc8991d` · **`ETHEREUM_ANCHOR_CONTRACT` `0x584906C1Ef7A60b203bEFD743fA55b6bb55A9c36`** (canopy `env.clear`; член `SILENT_ADDRESS_ENVS` гарда з `web: true` — INF.27 §🗄️) · Governor `0xFED9016ce8A590676d7A495fBe9d9D6382e1C3D9` · ProtocolParameters `0xb24040b32805200419cA837ba027947Ae6E134B3`. Квитанції (`…/11155111/run-latest.json`): 12, усі `status: 0x1`, Σ витрачено **12 784 622** газу при `effectiveGasPrice` 1.0656 gwei (EIP-1559, без `--legacy` і без піна — Sepolia не Amoy) = **0.01362 ETH**; на деплоєрі лишилось ≈0.036 ETH на наступні testnet-прогони. Бекенд читає звідси лише `StateRootAnchor`; решта пʼять на Sepolia — надлишок деплой-скрипта без гілок по чейнах.
...
⊕ **Sepolia-нога дешева — виміряно 2026-09-02: `0.01362 ETH`** (прогноз казав 0.0330 при 2.6 gwei, фактично 1.07 gwei; одного дрипу Google-крана 0.05 вистачило),
...
доказової поверхні ISO 14064/Verra). ✅ **Тобто це дослівно клас `SILENT_ADDRESS_ENVS` — і з 2026-09-02 змінна В гарді** (`web: true`, заведено
разом із Sepolia-адресою; INF.27 §🗄️): модель «одна змінна = одна історія гучності» розвʼязано критерієм
«читає БУДЬ-ЯКИЙ процес класу» — гучний Sidekiq-сайт більше не «покриває» тихий rake-сайт у web-контейнері.
...
ATA руками. ✅ **Devnet ЗАКРИТО 2026-09-03.** Кран — `faucet.solana.com` + Connect GitHub (👤; 0.5 SOL на fee-payer `6Kb1JSsJrjUxF8A1QbTiN3F2etxPyNuvXFYypQSwuMGZ`), далі машинно `spl-token`: **mint `dozxyxe6u4V9v6dayEp5fxMUx4qUxt5hDVjbhBdSJmf`** (supply 1 000 000) · **ATA `7eUT1t3vuF8WK7fioogHQEkyQmXtyhkNEmStneasKomE`**; звірено НЕ звітом CLI, а прямим `getTokenSupply`/`getTokenAccountsByOwner` повз реле. 🔴 **`--decimals 6` несуче, не стильове:** `Solana::MintingService#build_spl_transfer_checked_message` вшиває `USDC_DECIMALS = 6` у кожну `TransferChecked`, тож мінт іншої розрядності валив би КОЖНУ виплату **on-chain**, а не при конфігурації — тобто помилка була б видна аж на першій виплаті. ⚠️ **Форма keypair'а в секреті — hex-seed на 64 символи, а не JSON `solana-keygen`:** файл CLI несе 64 БАЙТИ (`seed‖pubkey`), у секрет іде hex ПЕРШИХ 32 (`Ed25519Crypto::SigningService` решту відкидає з raise). Сам keypair — `~/.silken_keys/canopy_devnet_feepayer.json` (0600, поруч із sepolia-деплоєром); доти він лежав у scratchpad ЧУЖОЇ сесії під `/private/tmp`, тобто на носії, який система прибирає без попередження, — а на ньому стоять чотири `CANOPY_SOLANA_*`. ⛔ **`spl-token` не працює за TLS-перехоплювальним проксі, і це властивість БІНАРЯ, не мережі** — обхід і механізм у скілі `deploy` гоча 16. `REQUIRE_SAFE_ADMIN` лишається **unset/false** (Safe-гейти mainnet-only — скрипт
...
🔴 **І ДРУГИЙ `forge script` — на Ethereum mainnet, бо `StateRootAnchor` L1 живе ТАМ, а не на Polygon:** `cd contracts && forge script script/Deploy.s.sol --rpc-url "$ALCHEMY_ETHEREUM_RPC_URL" --broadcast` (той самий скрипт без гілок по чейнах деплоїть усі шість; бекенд читає звідти лише `StateRootAnchor`, решта — надлишок, як у Фазі 2t на Sepolia) → адреса → `ETHEREUM_ANCHOR_CONTRACT` у `config/deploy.yml` env.clear. З 2026-09-02 ця змінна — член `SILENT_ADDRESS_ENVS` гарда з `web: true`, тож **без L1-якоря production-рендер Фази 5 не стартує** (web і job відмовляють на плейсхолдері). ⛔ **Polygon-якір у цю змінну — заборонена дія:** добре сформована адреса на ХИБНОМУ чейні є тихим класом INF.27 Q2 і поїде прямо в MRV-lineage бандл аудиторові; жоден гейт цього не побачить (EIP-55 чейну не знає). Ціна: тижневий якір коштує газ Ethereum mainnet — бюджет і ключ `ETHEREUM_ANCHOR_PRIVATE_KEY` (квінтет, Environment `production`) мають бути заведені ДО цього кроку ([`05_04`](05_04_Ethereum_L1_State_Anchor)).
...
export CANOPY_REDIS_URL=redis://:<hex>@<app_host_ip>:6379/0   # self-hosted accessory (INF.28); .kamal/secrets.canopy ремапить у REDIS_URL
...
```bash
# на app-хості (IAP-ssh): усередині живого контейнера canopy — web АБО job (роль жива з 09-03)
...
`CANOPY_COAP_HOST`/`PRODUCTION_COAP_HOST` (INF.6) · метрики: пʼять process-таргетів ОГОЛОШЕНО (з 09-03), з них ЖИВИХ два — canopy-web/canopy-job (`up=1`); три production-таргети `up=0`, бо production не деплоївся, і саме вони тримають `sn-alert-scrape-target-down` під silence до Фази 5 (S2.4);
...
Puma dual-stack (PUMA-IPV6-1) — `kamal app exec -i "curl -sf -o /dev/null -w '%{http_code}\n' http://[::1]:3000/up"` → ✅ **виміряно на canopy 2026-09-02: `000` — і це властивість МЕРЕЖІ, не дефект** (Docker-мережа `kamal` без IPv6: `docker network inspect kamal` → `EnableIPv6=false`, `/proc/net/if_inet6` у контейнері порожній; Puma слухає `0.0.0.0:3000`, v4-loopback `/up` → `200`; результат і межа — [`06_05 §IPv6`](06_05_Puma_Configuration)). 🔴 **Тут стояло `ss -tlnp | grep 3000`, і жодне з трьох прочитань кроку не виконується (виміряно 2026-08-31):** на машині оператора `ss` немає (Linux-утиліта), на app-хості порт 3000 не опублікований (ролі мають `network-alias`, не `publish` — див. §Kamal), а в контейнері `ss` не встановлений (`Dockerfile` ставить рівно `curl libjemalloc2 libvips postgresql-client`; `iproute2` немає). ⚠️ ОЧІКУВАНЕ значення при цьому чинне й переміряне проти самого гема: `puma-8.0.2` `Configuration.default_tcp_host` = `ipv6_interface_available? ? '::' : '0.0.0.0'` — тобто «`[::]:3000`» було твердженням про ГЕМ, а не про наш namespace: без IPv6 у контейнері та сама формула чесно дає `0.0.0.0`, і зламаною була проба, а хибним — очікування. `curl` тут і є доказом: відповідь на **IPv6-loopback** можлива лише при bind на `::`, а `/up` виключений з `force_ssl`-редиректу й з `host_authorization` (`probe_paths`, `production.rb`), тож 200 не маскується ані 301, ані 403. Без будь-яких пакетів той самий факт дає `kamal app exec -i "grep -i ':0BB8 ' /proc/net/tcp"` (v4; `/proc/net/tcp6` на canopy порожній) · money fail-closed
(INF.11) · Sentry release (S5.2) · **mailer = reset-лінк реально долітає й не 403-иться** (хост у тілі листа = `APP_HOST` → web-хост, домен резолвиться, `RAILS_ALLOWED_HOSTS` пропускає — ex-INF.25/INF.13 §🗄️; ⚠️ canopy пошту скіпає свідомо, `SILKENNET_SKIP_MAIL_TRANSPORT_CHECK=1`, тож ця перевірка = PRODUCTION) · `DB_POOL` job-ролі (canopy `6` при `-c 4`, production `17`; роль на canopy є з 09-02, перший її бут — після Devnet-двійників) · entrypoint fail-loud (✅ canopy 2026-09-02: контейнер вийшов під `-e` на впалому сіді — так і знайдено неатомарність `db:prepare`, [`06_06 §5.6`](06_06_Disaster_Recovery_and_Backup)) · **fallback-перемикання інтейку один раз** (stop `coap-daemon` → start `coap-relay` → `coap_smoke` зелений через дормантну Kamal `coap`-роль; INF.17) — ⚠️ **ця перевірка = PRODUCTION**, і не через обережність: на canopy роль `coap` нейтралізована `hosts: []` ([`config/deploy.canopy.yml`](../config/deploy.canopy.yml)), тож fallback-адресата там нема ЗА ПОБУДОВОЮ, і оператор, що виконає крок на canopy, перемикатиметься в порожнечу · **другий RPC — ручна `eth_chainId`-проба ДО деплою** (реєстр судить лише присутність змінної, ніколи придатність URL) **і рестарт контейнера ПІСЛЯ** заведення (пул кешує клієнта per-thread і читає ENV на побудові; ARCH.114) · гаманці з газом
(Pre-Flight #3).
...
ліцензує будь-який майбутній піддомен мовчки, тож поіменний перелік обрано свідомо → [INF.10] фліп `proxy.healthcheck.path: /ready` ✅ canopy фліпнуто 2026-09-02 (передумова `/ready→200` виміряна крізь CF); production — після власного першого деплою. Верифікація cutover — healthcheck-лог kamal-proxy: на canopy три невдалі тики, поки Puma підіймалась, потім `healthy` → `Deployed` (провенанс пункту — [`00_07`](00_07_Action_Plan_Tracker) INF.10 §🗄️). ⚠️ `interval` — bring-up-значення `3` в обох блоках у мить фліпу; на canopy вже стояче `10` (2026-09-02, нижній край смуги 10-15 с — [INF.22]), production тримає `3` до ВЛАСНОГО фліпу; ⛔ `ssl: true` сюди не повертати — його єдиний дім §TLS-fallback.
...
cold-start → підняти deploy_timeout (canopy несе `deploy_timeout: 360` з 2026-09-03: холодний entrypoint із сідом ≈ 4,5 хв, а 30-с дефолт убивав контейнер із дописаною роботою — [`06_06 §6`](06_06_Disaster_Recovery_and_Backup) drill #2); проба = ReadinessController, [`06_05`](06_05_Puma_Configuration)) →
