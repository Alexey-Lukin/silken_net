...
- [x] **Домен у Cloudflare** — `silkennet.app` (web) і `silkennet.com` (його піддомен
      ...
      ⚠️ Зона активна ≠ трафік ходить: **DNS-записів у `silkennet.app` НУЛЬ**, тож наступний
      рядок про три A-записи є реальною роботою, а не формальністю.
