...
але чи адреса з ТОГО ланцюга, він не перевіряє (шапка `subgraph/subgraph.yaml`) → `graph deploy`
у Studio тим самим 👤-акаунтом, що й репетиція Фази 2t, з URL на `version/latest`, не на пін версії.
