...
- [x] **Cloudflare account** — акаунт живий, обидві зони на плані `Free`, і цього ДОСИТЬ.
      ...
      `firmwares/index` — по одному на шлюз, тобто необмежено) — саме тому мультиплексування тут
      ...
      Реалістична стеля — десятки сокетів; будь-яке з циркулюючих чисел (100 ⊥ 100 000) лишає нас
      під нею на порядки. ⛔ **Дзеркальна пастка: платні фічі тут уміють ЛАМАТИ, а не
...
- ✅ **Три рядки вище ЗАКРИТО 2026-09-01, і закрито ЖИВИМ виміром, не дашбордом** — вони стояли
      ...
      **anycast Cloudflare** (`188.114.96.11`/`.97.11` — помаранчева хмарка стоїть, origin
      ...
      рівно той сценарій дає `522`, і він названий двома абзацами нижче. **Другу половину
      (`→ ingress_ip`) доводить лише `200` замість `522` на першому запиті — крок 2
      §Verification.** ⚠️ Отже `dig` лишається правильним інструментом для ХМАРКИ й хибним для
...
⛔ **«Витрата фіксована байткодом» — ХИБНО обома половинами, і це виправлення, а не уточнення.**
(1) `16 663 812` є сумою ЛІМІТІВ, тобто forge-оцінкою з множником ≈1.29 (`used/limit` = 0.773–0.823
поконтрактно), а не витратою. (2) Вона не фіксована: dry-run того ж дня дав **16 215 803**, з
поконтрактним розходженням до **10.5%** (`SilkenTimelock` 1 872 171 ⊥ 2 068 315). Величина, що
змінюється між двома прогонами, за визначенням не є властивістю байткоду.
🔑 **Що з аргументу ВИСТОЯЛО:** пін лишається несучим саме тому, що сабміт перевіряє
`gasLimit × gasPrice`, а не факт витрати — тобто на балансі треба **0.4999**, скільки б потім не
списалось. Ціна ж рухається: `forge` того дня оцінював **48.24 gwei**, `cast gas-price` давав
30.00, 44.10 і 130.25 у різні години. ⚠️ **Але формулювання «без піна кран не покриває фазу
ВЗАГАЛІ» було перебільшенням:** за реальною витратою @48 gwei вийшло б ≈0.62 POL проти 0.6
доступних — бракує 0.02, тобто ОДНОГО заходу крана, а не принципової недосяжності. ⚠️ І не бери одну цифру ціни:
