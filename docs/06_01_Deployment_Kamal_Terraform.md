...
- **Поточний TRL:** TRL 5 (🔺 `4→5`, TRL Gate Event — ⚖️ founder 2026-09-03; критерій §4.1 «HIL-валідація §3» виконано. Дім числа — [`00_03 §1`](00_03_TRL_Matrix_HIL_and_Beyond), тут дзеркало-джерело).
- **Стан деплою, виміряно 2026-09-05:** інфраструктура розгорнута, `kamal deploy` провів увесь ланцюг до запущеного контейнера, `Kamal Deploy (Canopy)` = `success`, `/up` = 200, `/ready` = `{database: true, redis: true}` — **слот обслуговує трафік**; console-рецепти репо доступні живим тунелем (скіл `deploy` §Gotchas #10). 🔑 **Цей рядок цитують як дім факту щонайменше чотири інші сторінки, тож правити стан деплою треба ТУТ і завжди разом із датою виміру** — інакше сусіди цитують стан, якого вже немає.
...
| **2** | **`.kamal/secrets-common` файл існує + повний** | Kamal читає секрети з `.kamal/secrets-common` (**не** з environment). **Канонічний набір — `config/deploy.yml env.secret`**, і він гейтований ланцюгом рівності [`06_04 §1`](06_04_Secrets_Checklist) (§1.1 ∪ §1.4 = `env.secret` = `.kamal/secrets-common` = `env:` кроку `kamal deploy` обох воркфлоу); семантика кожної змінної й клас «boot-critical ⊥ lazy» — [`06_04 §2.1`](06_04_Secrets_Checklist). ⛔ **Не заводити тут поіменного переліку — виміряно й відкинуто:** третя копія стоїть ПОЗА гейтованим ланцюгом вище, тож старіє мовчки й **лише в бік НЕПОВНОТИ** (`env.secret` росте комітами, перелік пишуть один раз), а ціна лягає на оператора в день деплою — незаведена змінна дає порожній інжект і boot-crash. ⛔ `KREDIS_REDIS_URL` **не додавати** [B1] — Kredis читає `REDIS_URL` як є (`config/redis/shared.yml`), а порожній інжект перебив би це значення; підстава — [`06_04 §2.1`](06_04_Secrets_Checklist). |
...
> | `CSP_ENFORCE` | `env.clear` | `false` (оголошено ЯВНО в ОБОХ маніфестах з 2026-09-07) | Переводить CSP з report-only у enforced. 🔴 **⛔ НЕ «після burn-in 1–2 тижні» — ця форма стояла тут місяцями й називала передумову, якої не існує:** `report-uri`/`report-to` у політиці немає ЖОДНОГО, тож звіти нікуди не їдуть. ⚖️ founder 2026-09-07: фліп іде за СВІДОМИМ ручним димовим прогоном із devtools на кожному рендерному маршруті; колектора немає й не буде (ціна відмови — у місці дії). |
...
**Субграф Фази 2t (✅ 2026-09-05):** `graph deploy` у Studio — субграф `silkennet-amoy` (Studio-акаунт
`1758737`), версія `v0.0.1`, три data-source на Amoy-адресах броадкасту 2026-09-01, `_meta` віддає
`hasIndexingErrors: false`; endpoint — `THE_GRAPH_API_URL` у canopy `env.clear`. ⚖️ Підстава — не
«полагодити порожнечу» (вона чесна: `configured?`-гейт + фолбек `nil`, ARCH.119), а РЕПЕТИЦІЯ: Фаза 2
вимагає `graph deploy` на mainnet-день із того самого акаунта.
...
🔁 **Субграф — ТРЕТІЙ крок тієї ж фази, і без нього ESG-аудиторська поверхня лишається на Amoy:**
у `subgraph/subgraph.yaml` — `network: polygon-amoy → polygon` + три MAINNET-адреси
(SCC · SFC · ProtocolParameters) разом із їхнім блоком деплою (`startBlock`); ⛔ testnet-значення
ЗАМІНИТИ, не «бампати». `validate_addresses.sh` у `CI · Subgraph` не пропустить нульову адресу,
але чи адреса з ТОГО ланцюга, він не перевіряє (коментар dataSource у `subgraph/subgraph.yaml`) →
`graph deploy` у Studio тим самим 👤-акаунтом, що й репетиція Фази 2t (вище). Для Studio dev-endpoint URL —
`version/latest`, не пін версії; після публікації в децентралізовану мережу URL стає КЛЮЧОВАНИМ
(`gateway.thegraph.com/api/<key>/…`) і їде в `env.secret` ([`06_04 §2.2`](06_04_Secrets_Checklist)).
...
+ contact point (S2.4 — дашборд і правила в стеку з 2026-08-29, канал — з 2026-08-30). 🔑 **`import.rb` бере ІНШИЙ креденшел, ніж Alloy, і Фаза −1 називала лише Alloyʼвий:** `GRAFANA_REMOTE_WRITE_{URL,USERNAME,TOKEN}` — це push метрик, а скрипт ходить в **адмін-API** й hard-fail'ить без `GRAFANA_URL` + `GRAFANA_API_TOKEN` (service-account, роль Editor+). ⚠️ І дзеркально: `ALERT_CONTACT_EMAIL` / `ALERT_CONTACT_TELEGRAM_{TOKEN,CHATID}` — **off-by-default**, тож без них скрипт contact point просто ПРОПУСКАЄ, лишаючись зеленим. Верифікаційний крок, який не може провалитись, верифікацією не є: якщо канал уже задротований (08-30), пінь його ЧИТАННЯМ (`--verify`), а не мовчазним успіхом імпорту · `/sidekiq` під `super_admin`-сесією → 200, під org-`admin` і анонімом → 404 (звужено ⚖️ 2026-09-05 — підстава над `mount` у `config/routes.rb`)
...
🔴 **CSP burn-in — крок ПЕРЕПИСАНО 2026-09-07, бо стара форма («burn-in 1-2 тижні → `CSP_ENFORCE=true`») називала передумову, якої НЕ МОЖНА виконати:** у політиці немає ані `report-uri`, ані `report-to`, тож `report_only` рятує сторінку від поламки, але порушення бачить лише консоль випадкового відвідувача — «чекати два тижні звітів» є очікуванням без спостерігача в кінці. ✅ **⚖️ УХВАЛЕНО founder-ом 2026-09-07 — варіант (б): колектора НЕМАЄ і не буде; фліп іде за СВІДОМИМ ручним димовим прогоном із devtools по КОЖНОМУ рендерному маршруту.** Відкинуто (а) — колектор у Sentry (той приймає CSP-звіти на Security-Header ендпоінт): дешевий у коді, але додає ЗОВНІШНЬОГО отримувача даних сторінки, тобто рядок у `ropa_art30` і в реєстрі субпроцесорів. Спостерігача за таку ціну ми не купуємо заради разової перевірки розкатки, яку людина робить за десять хвилин. ⛔ **Фліп — canopy ПЕРШИМ і окремим комітом:** canopy є репетицією, і спільний коміт обох слотів зробив би її вироком проду (підстава записана разом із присудом у `819ec279`; сам ⚖️ founder-а — про варіант (б), не про порядок). ⚠️ **Ціна відмови названа вголос, щоб її ніхто не перевідкрив як недогляд:** після фліпа про порушення ми дізнаємось лише від ЛЮДИНИ, що пройшла маршрути, або від користувача зі зламаною сторінкою — каналу немає й не буде. Це прийнятно для гейта розкатки й НЕ прийнятно для стоячого контролю безпеки: якщо CSP колись стане доказом, а не гігієною, перевідкриття — це (а), і його ціна — рядок у вендор-реєстрі, не код. ⛔ Не флипати за календарем. 🔑 **Проводку зроблено 2026-09-07 і вона ПОВНА: `CSP_ENFORCE: "false"` явно в `env.clear` ОБОХ маніфестів.** ⚠️ Число «пʼять поверхонь доставки» тут НЕ застосовне — воно про ланцюг СЕКРЕТА; `env.clear` Kamal емітить із маніфесту дослівно (перевірено сусідами `POSTGRES_HOST` і `APP_HOST`: нуль хітів у `.kamal/secrets-common` і в обох воркфлоу), тож «дозаводити ланцюг» означало б покласти не-секрет у секретний шлях, проти чого й стоїть `deploy_secret_scan`. Отже в день Фази 5 це ОДИН токен, а не проводка плюс фліп.

🔕 **І ЗНЯТИ GRAFANA-SILENCE на `sn-alert-scrape-target-down` × `slot=production` — крок ЦІЄЇ фази, не «колись потім».** Три production-таргети чесно `up=0`, поки production не задеплоєно, тож P0-канал глушено свідомо (⚖️ founder 2026-09-03: «тривога, що звучить завжди, не звучить ніколи»); рендер знімає підставу глушника В ТУ Ж МИТЬ, і з цієї секунди заглушений P0 більше не є обережністю, а є сліпотою до справжньої продової аварії. ⛔ **Носія в коді цей крок не має за побудовою:** silences живуть ПОЗА IaC, `import.rb --verify` їх не читає (оголошена стеля), тож жоден гейт репо не почервоніє — рядок рунбука і є єдиним, що спрацює в момент дії. Матчер СВІДОМО вузький (`alertname` І `slot`), щоб продова аварія після рендера НЕ була заглушена заодно. 📅 **І в глушника є ДАТА, якої немає в жодному гейті** (створено 2026-09-03; поточний строк і журнал продовжень — нога [`00_07`](00_07_Action_Plan_Tracker) `S2.4`). Строк тут — дата, а умова — подія (рендер), і подія до строку може не настати, тож шум повернеться САМ. Дія в день строку: перепитати «чи вже є production», і якщо ні — продовжити, датувавши продовження в S2.4; поки стоїть Фаза ∅ нижче, строк глушника й дата перегляду зупинки — ОДИН день. ⚠️ **Якщо продовжень набереться три поспіль — це вже не тиша, а сигнал**: значить, рендер не є найближчим горизонтом, і глушник треба замінити присудом про сам канал. Стан ноги → [`00_07`](00_07_Action_Plan_Tracker) `S2.4`.
...
#   duty_cycle_review_by   = "YYYY-MM-DD"  # строк повернення — ОБОВʼЯЗКОВИЙ, див. нижче
terraform apply    # підняття назад — ті самі два значення в ALWAYS/RUNNING, дата → ""
```

⏳ **Зупинка без строку повернення — тиша, а не рішення** (⚖️ founder 2026-09-19, «амана»; [`00_05 §7`](00_05_AI_Native_Operating_Model)). Зупинений стек має ДВА наслідки, яких сама зупинка не оголошує, і обидва виміряно на першій зупинці 2026-09-15: (1) кожен deploy-релевантний push іде IAP-тунелем у TERMINATED VM і падає `4003 failed to connect to backend` — і червоне стає вічним, бо детектор судить від останнього УСПІШНОГО деплою, тож за три доби цей шум прочитали як збій GCP; (2) Alloy живе на тій самій VM, тож гасне ВЕСЬ метричний тракт, і `sn-alert-scrape-target-down` іде в `NoData → Alerting` — чесно, за призначенням. Лік для (1) — детектор `Deploy · Canopy` читає цей самий posture-файл і при `TERMINATED` пропускає деплой ЛИШЕ до `duty_cycle_review_by`; з цього дня (або без дати) деплой знову йде й червоніє, змушуючи перепитати «чи Фаза ∅ ще триває» — забута дата лягає на ГУЧНИЙ бік (механізм — коментар кроку `decide` у `.github/workflows/deploy.yml`, пін — `spec/deploy/deploy_relevance_base_spec.rb`). Для (2) строк глушника production-скрейпу й дата перегляду — один день ([`00_07`](00_07_Action_Plan_Tracker) S2.4).
