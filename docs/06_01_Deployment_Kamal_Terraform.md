...
🔴 **CSP burn-in — крок ПЕРЕПИСАНО 2026-09-07, бо стара форма («burn-in 1-2 тижні → `CSP_ENFORCE=true`») називала передумову, якої НЕ МОЖНА виконати:** у політиці немає ані `report-uri`, ані `report-to`, тож `report_only` рятує сторінку від поламки, але порушення бачить лише консоль випадкового відвідувача — «чекати два тижні звітів» є очікуванням без спостерігача в кінці. ⚖️ Вибір належить founder-у й записаний у місці дії (`config/initializers/content_security_policy.rb`): або (а) завести колектор (Sentry уже має наш DSN і приймає CSP-звіти — але це НОВИЙ зовнішній отримувач, отже рядок у `ropa_art30`), або (б) зняти мову про «звіти» і флипати після СВІДОМОГО ручного димового прогону з devtools по кожному рендереному маршруту. ⛔ Не флипати за календарем.
