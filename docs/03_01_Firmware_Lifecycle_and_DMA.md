...
> **DR15 вже зайнято (FW.2 CCM Frame Counter, freeze-contract):** наступна фіча, що потребує RTC-resident state з переживанням VBAT-loss, не отримає регістра. Це не катастрофа — нижче три життєздатні шляхи (deferred TRL-7, реалізація НЕ потрібна зараз, лише фіксуємо дизайн).
