...
| **LoRaMac-node (LoRaWAN MAC)** | ARCH.34 Helium SOS: OTAA + DevNonce + EU868 — те, що кличе adapter `Helium_Mac_SendSos` (owned-обв'язка ✅ у `queen/main.c`+`helium_sos.h`) | ✅ завендорено (2026-07-05; ⚠️ `subghz-phy/lorawan/` то LBM radio-шар SWL2001, НЕ MAC — окремий submodule) | `extern/stm32-mw-lorawan`@v2.6.2 (SHA `3b2c8b73`, ST-форк LoRaMac-node, BSD-3; той самий Radio_s API; `Conf/*_template.h` → owned-glue) |
