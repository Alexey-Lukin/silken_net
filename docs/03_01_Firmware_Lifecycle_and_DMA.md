...
> **In-silico L4 validation (2026-05-25):** Michaelis-Menten + Arrhenius модель підтверджує `BASELINE_DELTA_T_S=60` фізично обґрунтованим. Очікувані значення: здорове дерево (10 мМ глюкози, 25°C) → delta_t ≈ 44.7 с; стресоване (5 мМ, 5°C) → ≈ 237.5 с; Monte Carlo 90% CI для healthy: 17.8–150.5 с (усі три — η_BQ 0.68 post-[`00_07` HW.47](00_07_Action_Plan_Tracker); машинний дім чисел — `cache/kinetics/delta_t_lookup.json` + `monte_carlo.json`). Деталі → [`01_03 §3.4 L4`](01_03_EBFC_Enzymatic_Bio_Fuel_Cell), [`in_silico/SUMMARY.md`](protocols/ebfc/in_silico/SUMMARY.md).
...
>
> **Wear-бюджет (зчеплений з E.63-шкалою delta_t):** 2 КБ сторінка = 254 елементи; запис «снапшот циклу» = K елементів/пробудження → erase раз на ⌊254/K⌋ циклів; 10k endurance × 2 сторінки. При K=4 це ⌊254/4⌋ = 63 пробудження на erase × 20 000 erase = 1.26 млн пробуджень: при delta_t = 44.7 с (оптимістичний кут L4, η_BQ 0.68 post-[`00_07` HW.47](00_07_Action_Plan_Tracker)) → **~1.8 року**; при realistic 1.77 год ([`02_03 §9.8`](02_03_BQ25570_MPPT_Nano_Power)) → століття. ⚠️ Підсумок свідомо КОНСЕРВАТИВНИЙ — він не бере «compact пропускає erase уже-чистої цілі» з блоку вище, який у steady-state подвоює ресурс (~3.6 року). ⛔ Підсумок писати РАЗОМ із множниками: голе число в цьому рядку неперевірне. Якщо польова delta_t впаде під ~5 хв стабільно — розширити page-set (4-8 сторінок, адреси вниз від 122) або лишити SRAM2-retain (−800 нА) свідомим trade-off (FW.54).
>
