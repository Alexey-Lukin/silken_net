...
>
> **Розміщення (freeze-contract):** сторінки **122-123** (`0x0803D000`-`0x0803DFFF`) — хвіст зайнятий: 124 = ключі/K_seed/роль (`FLASH_KEY_ADDR`), 125 = **K_ota** (`FLASH_OTA_KEY_ADDR 0x0803E800`, FW.23 — окрема сторінка, бо K_ota per-cluster і польова заміна стирає лише її; сюди переїхав 2026-06-11: первісний `0x0803D000` мовчки колідував із цим KV-регіоном — mount стер би ключ), 126(-127) = OTA contract (`MRUBY_CONTRACT_FLASH_ADDR`), 127 = Queen UID. Mount + HAL-глю ✅ написано у `main.c` (спільний гейт `FW17_RATCHET_ENABLED || FW8_PARSER_ENABLED`); верифікація erase/program — bench.
>
