...
| **LoRaMac-node (LoRaWAN MAC)** | ARCH.34 Helium SOS: OTAA + FCntUp + EU868 — те, що кличе adapter `Helium_Mac_SendSos` (owned-обв'язка ✅ у `queen/main.c`+`helium_sos.h`) | 🔴 **НЕ завендорено** — ⚠️ `subghz-phy/lorawan/` то LBM radio-шар (SWL2001), НЕ MAC-стек | **вендорити `extern/stm32-mw-lorawan`@v2.6.2** (ST-форк LoRaMac-node, BSD-3; той самий Radio_s API; команда — [`00_07` ARCH.34](00_07_Action_Plan_Tracker)) |
