...
- [ ] `ETHEREUM_ANCHOR_CONTRACT` — StateRootAnchor (Ethereum L1, weekly anchor). 🔴 Споживачів ДВА, і другий ТИХИЙ: `Ethereum::StateAnchorService` падає на плейсхолдері (`Eth::Address` `CheckSumError`), а `Mrv::LineageReportService` читає `ENV[...]` без дефолту прямо в ключ `anchor_contract` доказового MRV-бандла — добре сформована ХИБНА адреса пройшла б туди мовчки. Тому це третій член класу `SILENT_ADDRESS_ENVS` гарда (`web: true`) — ✅ заведено 2026-09-02 РАЗОМ із Sepolia-адресою (`0x584906C1…9c36`, canopy несе її в `env.clear`); production лишає плейсхолдер бази до Фази 2, і web-контейнер прод на ньому НЕ стартує — за задумом, Фаза 2 передує Фазі 3 ([`00_07`](00_07_Action_Plan_Tracker) INF.27).
