...
**Крок 4: Post-incident.** Слід уже в ланцюгу (`action: "stream_epoch_rotated"`, metadata `from`/`to`) — звірити з `AuditLog.verify_chain_integrity(org.id)`. ⚠️ Якщо витік стосувався **гаманця, кластера чи актуатора** (record-form стріми), епоха їх НЕ накриває — там єдиний важіль поки що глобальний (§5.8), і це відкритий residual → [`00_07`](00_07_Action_Plan_Tracker) SEC.25.
