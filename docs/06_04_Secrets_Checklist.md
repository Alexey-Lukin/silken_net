...
2. **Containment — два ключі, дві половини:**
   ...
   - Справжній revoke = **ротація `secret_key_base`** (інжект нового `SECRET_KEY_BASE` у deploy-ENV — механіка Phase-2 §5.7) = one-shot інвалідація **всіх**: dashboard-сесій · `password_reset` (15 хв) · `email_verification` (24 год) · **`api_access` (30 днів — усі API-клієнти re-issue)** · **`m2m_access` (30 днів, [SEC.16] — НАЙДОРОЖЧИЙ хвіст: кожна польова Королева мусить перевидати токен через Ed25519-підпис, і зробити це може лише сама)** · Codex battle-pairing HMAC (`Codex::PairSelectorService`, 5-хв TTL — найм'якший) · CSRF · ActiveStorage signed IDs.
