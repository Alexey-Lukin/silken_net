...
- [ ] RPC: `ALCHEMY_POLYGON_RPC_URL` · `ALCHEMY_ETHEREUM_RPC_URL` · `SOLANA_RPC_URL` · `CELO_RPC_URL` (порожній RPC `web3_network_guard` толерує — крім ДВОХ із зашитим code-side fallback'ом, і кожен фіксить свій бік осі: `CELO_RPC_URL` на `mainnet` ⊥ `ALCHEMY_POLYGON_RPC_URL` на `testnet`; маркер-невідповідність оголошеному `WEB3_CHAIN_ENV` — raise в ОБИДВА боки, §2.1)
...
- [ ] `SOLANA_RPC_URL` — Solana RPC. ⚠️ **ТРЕТЯ змінна з зашитим code-side fallback'ом** (поряд із `CELO_RPC_URL`→Alfajores і `ALCHEMY_POLYGON_RPC_URL`→`polygon-rpc.com` — рядок про «ДВОХ» вище стосується лише тих, кого судить boot-guard). Цю судить ВЛАСНИЙ read-сайт (`Solana::MintingService#solana_rpc_urls` raise'ить на порожню змінну, коли слот оголошує `mainnet` — з [OPS.37] за віссю `WEB3_CHAIN_ENV`, не за `Rails.env`, бо на testnet-слоті Devnet-fallback і є правильним приземленням). **БЕЗ цього ENV дефолт = Solana Devnet** — мікро-винагороди USDC підуть на тестову мережу (`E.47` у [`00_07`](00_07_Action_Plan_Tracker))! Mainnet: Helius/QuickNode. Опц. `SOLANA_RPC_URL_FALLBACK_1/2` — fallback-каскад (INF.22, `Solana::MintingService#execute_rpc_call`); порожні = single-RPC.
