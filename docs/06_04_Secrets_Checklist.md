...
- [ ] `RAILS_MASTER_KEY` — Rails credentials decryption key. **[B1]** CI читає його як GitHub Secret; `.kamal/secrets-common` тепер ENV-first — формою `$(printf '%s' "\${RAILS_MASTER_KEY:-$(cat config/master.key)}")`, ⛔ НЕ голим `${…:-…}`: Kamal парсить файл Dotenv'ом, який бере `${RAILS_MASTER_KEY` і лишає хвіст літералом, тож гола форма доставляла `<key>:-}` — присутній, непорожній, ХИБНИЙ ключ, невидимий жодному presence-гейту (виміряно на першому буті canopy 2026-09-02; носій — `spec/deploy/kamal_secrets_parse_spec.rb`, tripwire B5 у `deploy_secret_scan`) — `config/master.key` gitignored, відсутній у CI checkout, тож GitHub Secret мусить перемогти, інакше boot-decrypt падає.
...
- [ ] `RAILS_MASTER_KEY` — **[B1]** ENV-first (форма і підстава — §1.1, рядок `RAILS_MASTER_KEY`; голий `${…:-…}` заборонений). CI бере з GitHub Secret (§1.1; `config/master.key` gitignored → відсутній у checkout); локально fallback на файл.
- [ ] `SECRET_KEY_BASE` — рядок 22 (одразу за `RAILS_MASTER_KEY`, і порядок тут змістовний: у контейнері перший без другого не працює — образ не несе `credentials.yml.enc`). Дім опису — §1.1
...
**Phase-2 (👤, deploy-gated): drop `RAILS_MASTER_KEY`.** Після інжекту `SECRET_KEY_BASE` (= поточне `credentials.secret_key_base`, інакше ВСІ сесії ламаються — §5.2 entangled) + AR-encryption keys + service keys, ніщо не читає vault у runtime → `RAILS_MASTER_KEY` droppable з web/coap/job. ⚠️ **`SECRET_KEY_BASE` із цього переліку ВИБУВ 2026-09-01** — його задротовано всім ланцюгом як boot-critical (§1.1), бо він є передумовою ПЕРШОГО буту, а не гартуванням після нього (причина — §1.1, дім опису; тут не переказуємо). Рядок нижче доти називав його «свідомо НЕ доданим»; це було істинне в день написання, і тим самим проходом його звідти й вилучено — тож нижче вже НЕМАЄ чого читати як хибне. **НЕ** додано в deploy-ENV сьогодні (свідомо): service-креденшели ВСІХ восьми сервісів, перелічених рядком вище (present-placeholder → 401) — inject-at-deploy через Console. 🔴 **Одиниця тут ІНША, ніж у тому рядку, і сплутати їх коштує три невведені значення: сервісів вісім, а ЗМІННИХ більше** — `iotex`, `peaq` і `streamr` несуть по дві кожен. Тож інжектити треба за ПЕРЕЛІКОМ, а не за числом; числа тут свідомо не тримаємо — воно росте з кожним новим сервісом, а протухла таля читається як інвентар.
