...
- [ ] `ETHEREUM_ANCHOR_CONTRACT` — StateRootAnchor (Ethereum L1, weekly anchor). 🔴 Споживачів ДВА, і другий ТИХИЙ: `Ethereum::StateAnchorService` падає на плейсхолдері (`Eth::Address` `CheckSumError`), а `Mrv::LineageReportService` читає `ENV[...]` без дефолту прямо в ключ `anchor_contract` доказового MRV-бандла — добре сформована ХИБНА адреса пройшла б туди мовчки. Тому це третій член класу `SILENT_ADDRESS_ENVS` гарда (`web: true`) — рядок заводиться РАЗОМ із Sepolia-адресою, не раніше (canopy успадковує плейсхолдер бази; [`00_07`](00_07_Action_Plan_Tracker) INF.27).
