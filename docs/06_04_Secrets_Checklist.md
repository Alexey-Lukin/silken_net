...
>
> 🔴 **«Deploy-order» тут — ЖОРСТКИЙ порядок, а не побажання, і саме він робив перший підйом неможливим.** Формат-гілка `address_violations` не скоуплена процесом (на відміну від presence-гілки поруч), тож три плейсхолдери `CARBON/FOREST_COIN_CONTRACT_ADDRESS` + `DAO_TREASURY_ADDRESS` валять бут **будь-якого** контейнера, а не лише job-ролі. І це коректно: web справді споживає SCC-адресу (`ChainAuditService` ← `Api::V1::SystemAuditsController`), тож звузити гілку означало б зняти покриття рівно там, де його куплено. **Отже контракти деплояться ПЕРЕД першим підйомом слоту — на тому чейні, який слот оголошує в `WEB3_CHAIN_ENV`**: testnet-контракти перед canopy, mainnet-контракти перед production ([`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) Фаза 2t/2, [`00_03 §3.3`](00_03_TRL_Matrix_HIL_and_Beyond) — testnet передує mainnet за TRL-визначенням, не за смаком).
