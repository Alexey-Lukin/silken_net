...
- ~~`RAILS_MASTER_KEY`~~ — **ЗНЯТО з усіх пʼяти deploy-поверхонь 2026-09-02 (SEC.22 Phase-2)** — global `env.secret` · `.kamal/secrets-common` · крок `kamal deploy` + `BOOT_CRITICAL` обох воркфлоу · heredoc `coap.env` анкера. Підстава — вимір: образ не несе `credentials.yml.enc`, тож у контейнері ключ не розшифровував НІЧОГО від першого буту, а vault репо тримає ЄДИНИЙ ключ — `secret_key_base`, який їде окремим рядком нижче. Ратчет — `spec/deploy/kamal_secrets_parse_spec.rb` («ships no RAILS_MASTER_KEY on either leg») + `anchor_coap_env_spec` (denylist). Урок форми `$(printf '%s' "\${VAR:-…}")` (Dotenv бере `${VAR` і лишає хвіст літералом — виміряно на першому буті canopy 2026-09-02 саме на цьому ключі) живе далі в `.kamal/secrets.canopy` і в тій самій спеці. Локально ключ лишається в `config/master.key` для `credentials:edit`.
- [x] `SECRET_KEY_BASE` — Rails `secret_key_base` (заведено 2026-09-01). **Boot-critical, і саме тому він ТУТ, а не в §2.1:** образ не несе `credentials.yml.enc` (`.dockerignore` тримає його поза публічним GHCR), тож у контейнері `RAILS_MASTER_KEY` нема чого розшифровувати — без цієї змінної Rails падає «Missing `secret_key_base` for 'production'» ще до будь-якого гарда (виміряно, прогін 33499749733). Значення = поточне `credentials.secret_key_base`: інша величина розлогінює ВСІХ (§5.2 entangled). ⚠️ Ротація зчеплена — не міняти окремо від сесій/CSRF/ActiveStorage-URL
...
- [x] `GCP_PROJECT_ID` — ID GCP проєкту = **`silkennet`** (заведено 2026-09-01). ⚠️ Доти тут стояв приклад `silken-net-prod`, якого не існує — а приклад у чек-листі читається як значення
...
**Boot-critical** (порожній → контейнер падає на boot; `verify-secrets` блокує; гейт покриває і infra-передумови `GCP_PROJECT_ID` + WIF-Variables `GCP_WORKLOAD_IDENTITY_PROVIDER`/`GCP_SERVICE_ACCOUNT` — SA-JSON `GCP_SA_KEY` вилучено, CI keyless WIF INF.22; SSH-секрети ЗНЯТО, INF.20 (в) IAP keyless, див. §1.1):
...
- [ ] RPC: `ALCHEMY_POLYGON_RPC_URL` (**boot-critical на production з 2026-09-02**, не lazy — `SILENT_RPC_ENVS`, presence на web+job; доти `verify-secrets` warn-ив те, що гард відмовляв при буті) · `ALCHEMY_ETHEREUM_RPC_URL` · `SOLANA_RPC_URL` · `CELO_RPC_URL` — production; **canopy = testnet-двійники `CANOPY_ALCHEMY_POLYGON_RPC_URL` · `CANOPY_ALCHEMY_ETHEREUM_RPC_URL` · `CANOPY_SOLANA_RPC_URL` · `CANOPY_CELO_RPC_URL`, repo-level, і на canopy вони BOOT-CRITICAL** — усі чотири Alchemy-ендпоінти пробиті chainId/genesis-hash з позитивним і негативним контролем 2026-09-02 (unset → overlay віддає гучний плейсхолдер, який chain-гард читає як не-testnet ендпоінт і відмовляє; `verify-secrets` тому скіпає чисто, не бутить — [INF.27], 2026-09-02) (порожній RPC `web3_network_guard` толерує — крім ДВОХ, кожна на своєму боці осі: `CELO_RPC_URL` на `mainnet` ⊥ `ALCHEMY_POLYGON_RPC_URL` на `testnet`. ⚠️ Судить він досі двох, але **зашитий фолбек лишився в ОДНОЇ** — Celo свій втратив ⚖️ 2026-08-31, і його правило вижило на іншій підставі (переносить `KeyError` із першої продової події на деплой); маркер-невідповідність оголошеному `WEB3_CHAIN_ENV` — raise в ОБИДВА боки, §2.1)
- [ ] **Canopy testnet-підписанти** [OPS.37 ⚖️ 2026-09-02] — `CANOPY_ORACLE_MINTER_PRIVATE_KEY` · `CANOPY_ORACLE_SLASHER_PRIVATE_KEY` (**boot-critical на canopy**: гард вимагає пари в signer-процесі) · `CANOPY_ETHEREUM_ANCHOR_PRIVATE_KEY` · `CANOPY_ORACLE_CELO_PRIVATE_KEY` (**теж boot-critical на canopy, хоч на production ці два lazy** — ревʼю 2026-09-02: оверлей `.kamal/secrets.canopy` перетворює ВІДСУТНІЙ двійник на ПРИСУТНІЙ плейсхолдер `…_NOT_SET`, а format-гілка гарда його відкидає; «двійникове імʼя ніколи не lazy») · Solana-четвірка `CANOPY_SOLANA_WALLET_KEYPAIR` / `CANOPY_SOLANA_FEE_PAYER_PUBKEY` / `CANOPY_SOLANA_FEE_PAYER_TOKEN_ACCOUNT` / `CANOPY_SOLANA_USDC_MINT_ADDRESS` (**boot-critical** — `solana_violations` у signer-процесі; ⚠️ presence-гілка плейсхолдер КОВТАЛА — `"…_NOT_SET".present?` = true — тож із 09-02 гард несе `_NOT_SET`-tripwire у presence-гілках, `guard-craft` #58). Усі вісім = `JOB_CRITICAL` у `deploy.yml`: відсутній двійник знімає з `kamal deploy --roles` ЛИШЕ job-роль, web їде далі (per-role, не per-slot; членство обох воркфлоу порівнює `deploy_workflow_parity_spec` modulo `CANOPY_`). Repo-level, throwaway. ✅ EVM-четвірка заведена 2026-09-02: ключі трьох оракулів Amoy — throwaway EOAs сесії 2026-09-01 (адреси звірено `cast wallet address`, ролі — `hasRole` на Amoy і Sepolia, газ 0.05 POL ×3 і 0.012 ETH переказано з деплоєрів), Celo-Sepolia-підписант згенеровано (без газу; кран — 👤). Solana-четвірка — після Devnet SPL-mint ([`00_07`](00_07_Action_Plan_Tracker) DEPLOY-1 Фаза 2t). Ланцюг той самий, що в RPC-двійників: overlay B4 · `BOOT_CRITICAL`/`RUNTIME` · обидва написання в кроці `kamal deploy` — і той самий порядок «значення → імʼя»: доки Solana-двійники не заведені, `Deploy · Canopy` скіпає чисто.
- [ ] Solana (**boot-critical на production з 2026-09-02**, не lazy: `solana_violations` вимагає presence усіх чотирьох у signer-процесі — доти `verify-secrets` warn-ив те, що гард відмовляв при буті; ревʼю OPS.37): `SOLANA_WALLET_KEYPAIR` (**hex-seed 32 байти = 64 hex-символи**, як читає `Ed25519Crypto::SigningService`; НЕ JSON-масив `solana-keygen` — тим форматом `.env.example` брехав до 2026-09-02, і перший мінт упав би на `Invalid SOLANA_WALLET_KEYPAIR`) · `SOLANA_FEE_PAYER_PUBKEY` (base58-адреса того ж ключа) · `SOLANA_FEE_PAYER_TOKEN_ACCOUNT` · `SOLANA_USDC_MINT_ADDRESS`
- [ ] Пошта й канали: `SMTP_USER_NAME` · `SMTP_PASSWORD` · `TELEGRAM_BOT_TOKEN` — секрети `env.secret`, які `verify-secrets` НЕ перевіряє (їх нема ані в `BOOT_CRITICAL`, ані в `RUNTIME`): порожні = канал чесно вимкнено, не збій. ⚠️ `SMTP_ADDRESS`/`MAIL_FROM` — сусідні й **boot-critical** (гард `mail_transport_check`), але вони `env.clear`, тож живуть у §2.1 ✅ `TELEGRAM_BOT_TOKEN` заведено 2026-08-30 (бот `SilkenNetBot`; не гейтує деплой — його немає ані в `BOOT_CRITICAL`, ані в `RUNTIME`).
- [ ] `TURBO_SIGNED_STREAM_KEY` — **[SEC.25 Ф3]** stream-only важіль відкликання. 🔴 **Тут його не було до 2026-09-01, і саме ця відсутність була половиною дефекту:** оператор, що провіжить за §1, його не заводив, а `verify-secrets` мовчав би й далі (він у `RUNTIME`, тобто лише warn). Друга половина жила у воркфлоу — змінну не мапив крок `kamal deploy` на ЖОДНОМУ зі слотів, тож навіть заведена вона приходила в контейнер порожньою. Обидві закрито тим самим комітом; носій ланцюга — новий приклад «delivery half» у `spec/deploy/env_fetch_declaration_spec.rb`. ⚠️ Відмова тиха за побудовою: `turbo_stream_verifier.rb` читає `ENV[…].presence` і за `nil` деривує ключ із `secret_key_base` — застосунок працює, важеля просто немає (ціну названо в шапці самого ініціалізатора)
...
- ~~`RAILS_MASTER_KEY`~~ — знято 2026-09-02 (SEC.22 Phase-2; підстава й ратчет — §1.1, той самий рядок).
- [ ] `SECRET_KEY_BASE` — з 2026-09-02 ЄДИНИЙ ключ Application-core master-класу в деплої (образ не несе `credentials.yml.enc`, тож саме він, а не master-key, є передумовою буту — і саме він відтепер незамінний для DR, [`06_06 §4`](06_06_Disaster_Recovery_and_Backup)). Дім опису — §1.1
...
> ⛔ **`CLOUD_SQL_INSTANCE_CONNECTION_NAME` і `GCP_SA_KEY_BASE64` — НЕ заводити, ніде й ніколи [OPS.37].** Auth Proxy знято з рантайм-шляху (Kamal деплоїть УСЕРЕДИНІ VPC → приватний IP напряму), а WIF лишився **безвинятковим**: SA-ключа не існує в цьому дереві взагалі. 🔴 **Провенанс варто тримати, бо він про КЛАС, а не про два імені:** ці рядки пережили власний зріз коду на пів дня, тобто **👤-процедура наказувала оператору СТВОРИТИ довгоживучий SA-ключ, чиє зникнення той самий зріз оголосив закриттям [`ARCH.114`](00_07_Action_Plan_Tracker)** — код і чекліст розійшлись у протилежні боки, і почервоніти не мало чому. Переїхало сюди 2026-08-31 із §Quickstart у [`06_01`](06_01_Deployment_Kamal_Terraform), коли ту секцію згорнули у вказівник: доти це був ЄДИНИЙ дім заборони в усьому дереві.
>
...
### 2.1. Runtime-змінні: семантика й наслідки відсутності

> **Що це за секція.** Тут описано, ЩО кожна змінна робить і чим коштує її відсутність — незалежно від того, якою поверхнею вона їде. Провіжн-чек-лист (що саме заводити в GitHub Secrets) живе в **§1.1 ∪ §1.4**; ця секція — семантичний дім, не список закупівлі. Більшість записів нижче — `env: clear:` і справді не секрети; частина — секрети з `env.secret`, і вони позначені в тілі рядка.
>
> 🔴 **Заголовок цієї секції до 2026-09-01 читався «ENV-only змінні (НЕ у `.kamal/secrets-common`, потрібні воркерам)», і був хибним для власного вмісту:** `SMTP_USER_NAME` · `SMTP_PASSWORD` · `TURBO_SIGNED_STREAM_KEY` · `TELEGRAM_BOT_TOKEN` лежать у `.kamal/secrets-common` (рядки 88·89·60·93), а `ETHEREUM_ANCHOR_PRIVATE_KEY`, RPC-четвірка й Solana-набір — теж секрети. Ціна була не косметична: заголовок казав операторові «цього немає в Kamal-ланцюзі», тож провіжнячи за §1 він пропускав пʼять імен, яких §1 не називав, — і саме так `TURBO_SIGNED_STREAM_KEY` дожив до дня деплою незаведеним. **Мітка без вимірювача** ([`00_05 §7`](00_05_AI_Native_Operating_Model), клас `СЛОВО`): секція давно переросла свою назву, а назву ніхто не переміряв.

- [ ] `RELEASE_VERSION` — оверрайд Sentry-release для НЕ-Kamal процесів; Kamal-контейнери беруть `KAMAL_VERSION`, який Kamal інжектить сам ([`06_03 §1.2`](06_03_Prometheus_Observability)). ⛔ Не повертати в `env.clear` формою `"${RELEASE_VERSION}"` — віддалений shell розгортає її в порожній рядок (виміряно 2026-09-02).
...
- [ ] `ALCHEMY_POLYGON_RPC_URL` — keyed Polygon RPC (Primary; сьогодні Alchemy — акаунтний вендор тут один за ⚖️ ARCH.114, фолбеки keyless). 🔴 **Presence з 2026-09-02 судить boot-гард на job І web — `SILENT_RPC_ENVS`, порушення `[rpc]` [INF.27 Q1]:** `ChainAuditService` ковтає ВІДСУТНІСТЬ RPC у той самий `rescue StandardError`, що й відсутність адреси (KeyError і `Eth::Client.create("")` — один rescue, одне хибне «all clean»), тож CI-клас RUNTIME (warn-only) деплоїв би canopy зеленим без Polygon-RPC узагалі. ⊕ **Canopy читає ці чотири імені з testnet-двійників** `CANOPY_ALCHEMY_POLYGON_RPC_URL` · `CANOPY_ALCHEMY_ETHEREUM_RPC_URL` · `CANOPY_SOLANA_RPC_URL` · `CANOPY_CELO_RPC_URL` (ремап формою B4 у `.kamal/secrets.canopy`, гейт `deploy_secret_scan`; у `deploy.yml` двійники — `BOOT_CRITICAL`, і рядок кроку `kamal deploy` експортує їх під ОБОМА іменами, як `REDIS_URL`/`CANOPY_REDIS_URL`). 🔴 **Імена другого слота — `POLYGON_RPC_URL_FALLBACK_1/2` (мережева конвенція, як у Celo/Solana); вендорне `INFURA_POLYGON_RPC_URL` знято 2026-09-02, а доти рядок казав `ALCHEMY_POLYGON_RPC_URL_FALLBACK_*`, якого не читав ЖОДЕН рядок коду** (виправлено 2026-09-01, [`ARCH.114`](00_07_Action_Plan_Tracker)). Ціна помилки була не косметична: заведений за цим чеклістом секрет не робив би нічого й виглядав би зеленим — фальшива обіцянка другого провайдера рівно на тій поверхні, по якій секрети й заводять. Дім складу каскаду — реєстр `NETWORK_FALLBACK_ENV_KEYS` у `app/services/web3/rpc_connection_pool.rb` (він же єдиний спосіб додати провайдера: пул кешує клієнта per-thread за ІМЕНЕМ ENV-ключа, тож каскад, оголошений kwargʼом на сайті виклику, мовчки не діє). ⚠️ Honesty-гейти ДВА: `rpc_connection_pool_spec` вимагає кожен ключ реєстру в `.env.example`, а спека гарда — у `Security::Web3NetworkGuard::RPC_FALLBACK_URL_ENVS` (chain-вісь), тож перейменування слота є синхронною правкою трьох файлів. ✅ **Каскад ЖИВИЙ з 2026-09-02** (⚖️ founder: keyless-публічні PublicNode/dRPC як другий птах, акаунтний вендор лише якщо вкусять rate-limit-и): обидва фолбеки стоять літералами в `env.clear` базового маніфесту — не секрети, бо ключа провайдера немає, і саме тому їхній ЧЕЙН судить гард, а не `deploy_secret_scan`, чий якір `_RPC_URL\z` суфікса `_FALLBACK_N` не бачить, — canopy перевизначає їх Amoy-двійниками (рядок фолбеків нижче); з двома URL `ResilientClient` інстанціюється, тобто circuit-breaker і failover на Polygon існують фізично. ⛔ Доти, при одному URL, він не інстанціювався взагалі (`all_urls.size <= 1` → голий `Eth::Client`).
- [ ] `ALCHEMY_ETHEREUM_RPC_URL` — Alchemy RPC для Ethereum L1
- [ ] `POLYGON_RPC_URL_FALLBACK_1/2` · `CELO_RPC_URL_FALLBACK_1/2` · `SOLANA_RPC_URL_FALLBACK_1` — **другий птах кожної мережі: keyless публічні ендпоінти ІНШИХ операторів [ARCH.114 ⚖️ founder 2026-09-02]** (PublicNode · dRPC · офіційний Solana; Ethereum другого птаха не має свідомо — тижневий якір покриває Sidekiq-retry). 📏 **Вимір з app-VM 2026-09-02** (ноутбукові проби під корпоративним TLS-проксі брешуть): PublicNode живий на Polygon · Amoy · Ethereum · Sepolia · Celo; dRPC — на всіх, крім Sepolia; 1RPC — 4 з 5; celo-sepolia — forno/dRPC/Ankr живі; мертві — BlockPI (521) · Blast (403) · Ankr і LlamaRPC на Polygon · `rpc-amoy.polygon.technology` (NXDOMAIN); Solana — лише офіційні mainnet-beta/devnet, PublicNode/dRPC віддають 403. Це підстава вибору пари PublicNode+dRPC, і вона старіє — переміряти при першому rate-limit у логах. Живуть в `env.clear` базового маніфесту літералами — не секрети; canopy перевизначає КОЖНЕ імʼя testnet-двійником (Amoy · Celo-Sepolia · Devnet), бо destination deep_merge є обʼєднанням ключів і неперевизначений mainnet-фолбек став би ендпоінтом, на який стейджинг сяде після першої аварії primary. Гард судить ЧЕЙН фолбеків тією ж віссю, що й primary (`Security::Web3NetworkGuard::RPC_FALLBACK_URL_ENVS`), присутності не вимагає — каскад опційний за побудовою (пул: один URL = голий `Eth::Client`, два+ = `ResilientClient` із circuit-breaker). ⛔ Keyed-URL під цими іменами заборонений: `deploy_secret_scan` бачить лише суфікс `_RPC_URL`, тож ключ тут був би комітнутим секретом без прапорця — keyed-фолбек іде через `env.secret` під імʼям із суфіксом `_RPC_URL`. Виміряні живими з app-VM 2026-09-02 (`eth_chainId`, контроль обабіч); проби з ноутбука під Zscaler брешуть. Названа стеля: публічні ендпоінти без SLA — rate-limit у логах є тригером акаунтного вендора.
- [ ] `SOLANA_RPC_URL` — Solana RPC. ⚠️ **ДРУГА (і остання) змінна з зашитим code-side fallback'ом** — поряд із `ALCHEMY_POLYGON_RPC_URL`→`polygon-rpc.com`. 🔴 **Множини тут ДВІ й вони більше не збігаються, і саме їхнє злиття колись дало «ТРЕТЯ»:** boot-guard судить ДВІ змінні (`CELO_RPC_URL` на `mainnet` ⊥ `ALCHEMY_POLYGON_RPC_URL` на `testnet`), а зашитий фолбек лишився в ОДНІЄЇ з них — Celo свій втратив ⚖️ 2026-08-31 (знято, не перецілено), і його правило вижило на новій підставі. Цю судить ВЛАСНИЙ read-сайт (`Solana::MintingService#solana_rpc_urls` raise'ить на порожню змінну, коли слот оголошує `mainnet` — з [OPS.37] за віссю `WEB3_CHAIN_ENV`, не за `Rails.env`, бо на testnet-слоті Devnet-fallback і є правильним приземленням). **БЕЗ цього ENV дефолт = Solana Devnet** — мікро-винагороди USDC підуть на тестову мережу (`E.47` у [`00_07`](00_07_Action_Plan_Tracker))! Mainnet: Helius/QuickNode. Каскад `SOLANA_RPC_URL_FALLBACK_1/2` (INF.22, `Solana::MintingService#execute_rpc_call`): з 2026-09-02 `FALLBACK_1` заведено keyless (офіційний `api.mainnet-beta` ⊥ на canopy `api.devnet`; PublicNode і dRPC для Solana віддають 403), `FALLBACK_2` відсутній свідомо — рядок фолбеків вище; порожні = single-RPC.
- [ ] `CARBON_COIN_CONTRACT_ADDRESS` — адреса SCC контракту після deploy (`BlockchainMintingService`, `ENV.fetch` без default → `KeyError` на першому SCC mint). Тихий споживач (`ChainAuditService` хибне «all clean») → також у boot-guard `address_violations`. 🔴 **Presence цієї змінної вимагається НЕ лише в signer-процесі — з 2026-09-01 і у WEB-контейнері**, бо тихий споживач тут контролер-досяжний (`Api::V1::SystemAuditsController#index` → `ChainAuditService`), тобто на web-only слоті (canopy) ВІДСУТНЯ змінна доти бутила чисто й давала `delta: 0, critical: false` назавжди, тоді як плейсхолдер бут відмовляв — протилежні вердикти на тому самому споживачі. Скоуп тепер **per-variable** (`web:` у `SILENT_ADDRESS_ENVS`): сусідні `FOREST_COIN_CONTRACT_ADDRESS`/`DAO_TREASURY_ADDRESS` лишаються signer-only свідомо — у них веб-читача немає. ⛔ Не «спростити» це назад у беззастережну вимогу: `coap_listener` вантажить усі ініціалізатори, а його `coap.env` контракт-адрес не несе — ⛔ **і це СПОСТЕРЕЖЕННЯ про сід-heredoc `terraform/compute.tf`, не канонізований інваріант: «за побудовою (§5.7)» тут стояло ХИБНО** (§5.7 — це secrets-at-rest latch), а гейта за спостереженням немає взагалі — процес-класів ТРИ, не два
- [ ] `FOREST_COIN_CONTRACT_ADDRESS` — адреса SFC контракту після deploy (той самий `ENV.fetch`-патерн → `KeyError` на першому SFC mint). ⛔ **«Тихий audit-споживач» тут стояло ХИБНО — SFC-половини `ChainAuditService` НЕ ІСНУЄ** (він читає лише SCC-адресу); єдиний read-сайт — гілка `forest_coin` у `BlockchainMintingService`, і вона недосяжна за раннім `return`. Тобто змінна сидить у SILENT-наборі boot-гарда за успадкуванням, а її read-сайт ГУЧНИЙ — переклад у `loud` — відкрите питання, дім якого — шапка `app/services/security/web3_network_guard.rb` (критерій `web:` = «читає БУДЬ-ЯКИЙ процес класу»), не трекер; хід двофайловий: `SILENT_ADDRESS_ENVS` ⊕ пін членства у `web3_env_loudness_spec` → boot-guard `address_violations`). ✅ placeholder заведено у Kamal env.clear (INF.12 machine-half; fill = post-`forge deploy`).
...
- [ ] `ETHEREUM_ANCHOR_CONTRACT` — StateRootAnchor (Ethereum L1, weekly anchor). 🔴 Споживачів ДВА, і другий ТИХИЙ: `Ethereum::StateAnchorService` падає на плейсхолдері (`Eth::Address` `CheckSumError`), а `Mrv::LineageReportService` читає `ENV[...]` без дефолту прямо в ключ `anchor_contract` доказового MRV-бандла — добре сформована ХИБНА адреса пройшла б туди мовчки. Тому це третій член класу `SILENT_ADDRESS_ENVS` гарда (`web: true`) — ✅ заведено 2026-09-02 РАЗОМ із Sepolia-адресою (`0x584906C1…9c36`, canopy несе її в `env.clear`); production лишає плейсхолдер бази до Фази 2, і web-контейнер прод на ньому НЕ стартує — за задумом: mainnet-контракти (Фаза 2, включно з `StateRootAnchor` на Ethereum mainnet — окремий `forge script`, ⛔ не Polygon-якір) передують production-рендеру (Фаза 5); правило порядку живе в [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform), провенанс — INF.27 §🗄️.
- [ ] `PROTOCOL_PARAMETERS_CONTRACT_ADDRESS` — ProtocolParameters.sol (governance sync; `ENV[]` nil-safe → skip-sync)
- [ ] `CELO_RPC_URL` — Celo RPC, **ОБОВʼЯЗКОВИЙ: код-фолбека БІЛЬШЕ НЕМАЄ** (⚖️ founder 2026-08-31 — на money-шляху «працює без конфіга» є небезпекою, не зручністю; знято, не перецілено → [`05_01 §9. Celo (ReFi Community Rewards)`](05_01_Multichain_Architecture)). ⚠️ БЕЗ значення → `KeyError` на КОЖНОМУ Celo-виклику (виплата · підтвердження · відкат мінту), а в treasury-свіпі — `status: :error` + `TREASURY_CHECK_ERRORS_TOTAL`. З 2026-07-12 **умовно гейтовано boot-guard'ом**: `ORACLE_CELO_PRIVATE_KEY` присутній ∧ RPC blank → violation (озброєний Celo-шлях без mainnet-RPC не бутиться; неозброєний — чистий). Mainnet: Forno/Alchemy.
...
>
> 🔴 **«Deploy-order» тут — ЖОРСТКИЙ порядок, а не побажання, і саме він робив перший підйом неможливим.** З 2026-09-02 [INF.27 Q3] format-гілка `address_violations` скоуплена ТИМ САМИМ по-змінним предикатом, що й presence: web валить бут на плейсхолдері будь-якої з ДВОХ адрес, які він читає (SCC — `ChainAuditService` ← `Api::V1::SystemAuditsController`; `ETHEREUM_ANCHOR_CONTRACT` — `Mrv::LineageReportService` через rake у web-контейнері, з 2026-09-02), — job на будь-якій із чотирьох, а дормантна Kamal-роль `coap` на жодній (вона успадковує глобальні плейсхолдери, яких не читає, і доти падала на трьох `[address]`, з якими оператор не мав що робити). Порядок від цього не мʼякшає: web без справжньої SCC-адреси не піднімається, і саме web є першим рендером Фази 3. **Отже контракти деплояться ПЕРЕД першим підйомом слоту — на тому чейні, який слот оголошує в `WEB3_CHAIN_ENV`**: testnet-контракти перед canopy, mainnet-контракти перед production ([`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) Фаза 2t/2, [`00_03 §3.3`](00_03_TRL_Matrix_HIL_and_Beyond) — testnet передує mainnet за TRL-визначенням, не за смаком).
...
- [ ] `WEB3_STRICT_MODE` — `true` у production. У production **АБО** `WEB3_STRICT_MODE=true` (belt-and-suspenders — Hadron harden 2026-07-10) Hadron-stub raise при відсутності ENV + oracle-callback HMAC fail-fast (SEC.5; Chainlink-dispatch більше не STRICT-gated — local marker, ARCH.53). Заведено в `config/deploy.yml` env.clear (INF.11); canopy успадковує (`RAILS_ENV=production`). ⚠️ Це вісь «**загартований рантайм**» — вона НЕ каже нічого про те, до якого чейну слот має право торкатись; це сусідній рядок ↓.
- [ ] `WEB3_CHAIN_ENV` — `mainnet` (заведено в `config/deploy.yml` env.clear) ∈ `mainnet`/`testnet`; **відсутнє → `mainnet`**, тобто забутий прапор лягає на суворий бік. Друга вісь `Web3NetworkGuard` поруч із `signer_process:` [OPS.37 — розщеплення токена `production`, бо той ніс ОБИДВА твердження: «загартований рантайм» І «це справжні гроші», а стейджинг потребує першого без другого]. ⛔ **Не bypass:** значення є ТВЕРДЖЕННЯМ про проводку, і напрямки дзеркальні — на `mainnet` падає testnet-ендпоінт, на `testnet` падає mainnet-ендпоінт (стейджинг, здатний підписати справжню транзакцію). Нерозпізнане значення = власне порушення. Скоупить лише A1 (`chain_violations`); presence/формат адрес, ключів і Solana-четвірки вісь НЕ послаблює. Дім механіки — [`04_02 §8`](04_02_Business_Logic_and_Services); стан testnet-слоту: Amoy і Sepolia живі з 2026-09-02 (INF.27 закрито — §🗄️ в [`00_07`](00_07_Action_Plan_Tracker)); відкрите — Devnet ([`00_07`](00_07_Action_Plan_Tracker) `DEPLOY-1` Фаза 2t) і роль job canopy (`OPS.37`).
- [ ] `DEPLOYMENT_SLOT` — `production` (base) ⊥ `canopy` (override у `config/deploy.canopy.yml`); **відсутнє → `Rails.env`**. **Не секрет.** ⚠️ Змінна НЕ нова — вона народилась для accessory Alloy (мітка `slot`, [`06_03 §2.9`](06_03_Prometheus_Observability)); [INF.27, 2026-08-30] додав її в ГЛОБАЛЬНИЙ `env.clear`, тобто вона тепер досяжна Rails-ролям. 🔑 **Це ТРЕТЯ вісь поруч із двома вище, і питання в неї власне: «який це ДЕПЛОЙ», тоді як `WEB3_STRICT_MODE` питає «чи загартований рантайм», а `WEB3_CHAIN_ENV` — «до якого чейну можна торкатись».** Обидва слоти біжать `RAILS_ENV=production` (canopy свідомо), тож `Rails.env` на це питання не відповідає, і кожна поверхня, що МІТИТЬ ним спільний зовнішній ресурс, штампувала `production` двічі. Читач — One-Home `config/deployment_slot.rb`; споживачі — Sentry-`environment` · namespace кешу (`config/cache.yml`) · обидва бакети ActiveStorage (`config/storage.yml`) · поле `slot` у JSON-логу (`production.rb`) · два boot-гарди. ⛔ Читати ЛИШЕ через One-Home, ніколи голим `ENV.fetch("DEPLOYMENT_SLOT")` без дефолту: шаблон `coap.env` анкера (`terraform/compute.tf`) несе її лише з 2026-09-02 — як рядок 1 **перемикача слоту інтейку** ([OPS.37] ⚖️: один анкерний демон, слот = `DEPLOYMENT_SLOT` · `POSTGRES_DATABASE` · `REDIS_URL` + рестарт; рецепт — [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) Фаза 1), а файл створюється ОДИН раз — заповнений раніше анкер живе без рядка, і фолбек тримає його на `production` мовчки. Носій — `spec/deploy/deployment_slot_axis_spec.rb` + `anchor_coap_env_spec` (три рядки перемикача).
- [ ] `RAILS_ALLOWED_HOSTS` — comma-separated allowlist хостів для захисту від DNS-rebinding атак (підтримує leading `.` для subdomain wildcard, напр. `.silkennet.com` покриє `api.silkennet.com`). **Канон-ТРІЙКА (INF.4 Опція A): `silkennet.app,canopy.silkennet.app,api.silkennet.com`** — web-хост треба перелічити **явно**: wildcard `.silkennet.com` його НЕ покриває (інший TLD; `ActionDispatch::HostAuthorization` матчить суфікс літерально → 403 block-all на КОЖЕН запит, крім probe-шляхів ↓). 🔴 **Тут стояла ПАРА до 2026-08-31, і бракувало саме живого хоста:** canopy успадковує це значення з бази (destination deep_merge = keys-UNION), а рядок без провідної крапки матчиться ТОЧНОЮ рівністю (`sanitize_string` → `/\A<host>(?::\d+)?\z/i`), тож `canopy.silkennet.app` діставав би 403 на кожен запит — і мовчки, бо probe-шляхи з `host_authorization` виключені, отже healthcheck зелений і деплой рапортує успіх. ⊕ Форма `.silkennet.app` (провідна крапка) покрила б і apex, і піддомен одним рядком, але ліцензує будь-який майбутній піддомен без рішення — поіменний перелік обрано свідомо. ⛔ **Опцію B (apex-redirect) ВІДХИЛЕНО ⚖️ 2026-08-30 разом із застосуванням Опції A — тож `silkennet.com` сюди НЕ додається, і це не пропуск.** Підстава відмови: apex-redirect додає рухомих частин (ще один хост у allowlist, ще один редірект-стрибок у auth-лінку, ще одна поверхня, де TLS може розійтись) заради нуля виграшу — Опція A привʼязує mailer-хост прямо до web-хоста й розвʼязує ту саму проблему без жодної з них. ⚠️ **Рядок стояв тут умовним («якщо піде варіантом B») ще добу після того, як варіант B помер, і мігровано його сюди 2026-09-01 саме тому:** відмова жила ЛИШЕ в тілі [`INF.25`](00_07_Action_Plan_Tracker), тож трим того пункту лишив би цю умову описувати шлях, якого вже немає — тобто наступний читач сумлінно «доробив» би відхилене. Якщо не встановлено — Rails логує попередження `[SECURITY]` при кожному старті контейнера. Встановлюється через Kamal `env.clear`. **⚠️ Обов'язково для production.** Probe-шляхи `/up`/`/ready`/`/metrics` виключені і з `host_authorization`, і з `force_ssl`-redirect — single-sourced `probe_paths`/`probe_request` у `production.rb` **[S6.18]** (дрейф однієї копії ламав би deploy health-check за зеленим boot).
- [ ] `APP_HOST` — хост для Action Mailer `default_url_options` (`config/environments/production.rb`; `ENV.fetch("APP_HOST", "silkennet.app")`). Дефолт `silkennet.app` = web-хост (⚖️ INF.25 Опція A, застосовано 2026-08-30 після купівлі обох доменів; доти стояв `silkennet.com` — apex, не підʼєднаний до web, тобто auth-лінки летіли б у 403). Заведено в `config/deploy.yml` env.clear (INF.13). ⚠️ Це хост у ТІЛІ листа — транспорт і відправник окремі, нижче. ⛔ Доменний свіп сюди НЕ заходить: `api.silkennet.com` (CoAP) коректний завжди — firmware хардкодить його як `COAP_SERVER_HOST`, дрейф = пере-прошити флот (INF.4).
...
- [ ] `ACTIVE_STORAGE_SERVICE` — імʼя сервісу з `config/storage.yml` для `config.active_storage.service` (default `production_mirror` = S3 primary + GCS mirror). **Canopy: `local`** (Disk у контейнері, ефемерно — staging без storage-кредів; ⚖️ founder 2026-09-02): доти кожен бут canopy друкував `Error retrieving instance profile credentials: HTTP 404` — AWS SDK без креденшелів іде в EC2-IMDS на GCP-машині.
...
- [ ] `TLS_ORIGIN_CERT_PEM` / `TLS_ORIGIN_KEY_PEM` — **Cloudflare Origin CA пара для kamal-proxy [INF.4, 2026-08-30].** Секрети, і поверхня в них НЕ ТА, що в решти цього розділу: вони їдуть не через `env.secret`, а через `proxy.ssl.{certificate_pem,private_key_pem}` — Kamal 2.12 приймає там **імена kamal-секретів**, не шляхи. ⚠️ **Передумова, а не оздоба:** CF стоїть у `Full (strict)` на обох зонах, що вимагає валідного сертифіката на origin, а HAProxy анкера — `mode tcp` passthrough і нічого не термінує; без цієї пари web-ярус віддає 521/525 на першому ж запиті ([`06_01 §Сертифікат НА ORIGIN`](06_01_Deployment_Kamal_Terraform)). ⛔ Не заміняти на Let's Encrypt: під `Full (strict)` CF ходить на origin лише по HTTPS, тож HTTP-01 не доставляється, а `ssl: true` лишається шляхом TLS-fallback БЕЗ CF. Один wildcard-сертифікат (`silkennet.app` + `*.silkennet.app`) покриває обидва слоти. 🔑 **Ключ генерується ЛОКАЛЬНО, CF підписує лише CSR** (форма «Use my private key and CSR» — рецепт і приймальна перевірка збігом МОДУЛЯ в [`06_01 §Сертифікат НА ORIGIN`](06_01_Deployment_Kamal_Terraform)); шлях «хай CF згенерує пару» родить приватний ключ на серверах вендора й свідомо не береться. ✅ **ОБИДВА заведено 2026-08-31** (`_CERT_PEM` 06:03Z · `_KEY_PEM` 06:07Z), і збіг МОДУЛЯ перевірено — тобто CF підписав НАШ CSR. Сертифікат ПУБЛІЧНИЙ за природою (світиться в кожному рукостисканні), секретом є лише формою доставки; ⚠️ у ключа планка зберігання НИЖЧА за master-ключі: втрата НЕ незворотна (Origin CA перевидається безкоштовно за дві хвилини), витік — дає видати себе за наш origin перед CF. ⛔ Не змішувати з `SECRET_KEY_BASE`/`PROVISIONING_MASTER_KEY` ([`00_07`](00_07_Action_Plan_Tracker) `DR.1`), які саме незамінні (`RAILS_MASTER_KEY` з 2026-09-02 — ні, §5.7). Ланцюг `secrets-common` + workflow-`env:` **енфорсить** `spec/deploy/env_fetch_declaration_spec.rb`, і з 2026-08-31 він уже НЕ «з моменту, коли розкоментують» — блок живий (`fc4083c5`), тож гейт судить його сьогодні. 🔴 **І ланцюга було замало — це виміряно того ж дня:** він доводить, що ІМʼЯ резолвиться, а питання було про ЗНАЧЕННЯ. `secrets-common` оголошує обидва імені, тож `Kamal::Secrets#[]` не кидає «Secret not found» — віддає **порожній рядок**; `Kamal::Cli::App::SslCertificates#run` гардить формою `if cert_content = …`, а `""` у Ruby істинний ⇒ kamal завантажує ПОРОЖНІ `cert.pem`/`key.pem` (0644), проксі не віддає TLS, CF відповідає 521/525 — **за цілком зеленим деплоєм**, і доти пара не була ані в `BOOT_CRITICAL`, ані в `RUNTIME`, тобто не давала навіть `::warning::`. ✅ Обидва імені додано в `BOOT_CRITICAL` обох deploy-воркфлоу; носій — той самий спек (мутація: зняття пари з будь-якого `BOOT_CRITICAL` червонить поіменно). ⚠️ `521` без `525` має ДРУГУ причину, виміряну 2026-09-02 при заведеній парі: origin без HAProxy на анкері (startup-script; [`06_01`](06_01_Deployment_Kamal_Terraform) Pre-Flight крок 10) — читай 521 спершу як «origin не існує». 📅 Сертифікат: issuer CF Origin CA, SAN `silkennet.app` + `*.silkennet.app`, **чинний 15 років від випуску 2026-08-31** — точну дату читай із самого сертифіката (`openssl x509 -noout -enddate`), не з канону (приймальна перевірка збігом модуля — [`06_01 §Сертифікат НА ORIGIN`](06_01_Deployment_Kamal_Terraform)).
- [ ] `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `GCS_CREDENTIALS_JSON` — **Active Storage, і це ЄДИНІ облікові дані `config/storage.yml`, які досі не мали рядка НІДЕ [SEC.22, знайдено 2026-08-31].** Форма там `ENV["X"].presence || credentials.dig(...)`, тобто ENV-primary рівно як у восьми сервісів §5.7 — але в перелік «вісім» вони не входять, у `.env.example` їх не було, і в цьому розділі теж. 🔴 **Чому це не косметика:** Phase-2 [SEC.22] знімає `RAILS_MASTER_KEY` після інжекту «восьми + AR-ключів»; виконаний ДОСЛІВНО, він лишає Active Storage взагалі без креденшелів — а це тихо вбиває фотодокази `MaintenanceRecord`, ЄДИНУ людську доказову поверхню платформи. Сьогодні латентно лише тому, що сховище ще не провіжнене; правильний момент завести — **ДО** першого блоба, бо після нього Phase-2 стає незворотним для вже завантажених. ⚠️ `GCS_CREDENTIALS_JSON` несе JSON keyfile ЦІЛКОМ (не шлях) — секрет того ж класу, що money-ключі за формою доставки, і йому місце в `env.secret`, а не в `env.clear`. Сусідня четвірка НЕ секретна й лише конфігураційна: `AWS_REGION` (дефолт `eu-central-1`) · `AWS_BUCKET` / `GCS_BUCKET` (дефолт `silken-net-<DEPLOYMENT_SLOT>` — **не** `Rails.env`: обидва слоти біжать `production`, і нерозведене імʼя дало б canopy право purge-ити продовий обʼєкт, [INF.27]) · `GCS_PROJECT`. ⚠️ 2026-09-02 латентність знято виміром: кожен бут canopy друкує `Error retrieving instance profile credentials: HTTP 404` — без цих креденшелів AWS SDK іде в EC2-IMDS на GCP-машині (`production_mirror` = S3 primary), ✅ ⚖️ 2026-09-02: canopy → `ACTIVE_STORAGE_SERVICE=local` (рядок вище), AWS-пара лишається PRODUCTION-питанням (нога — [`S1.1`](00_07_Action_Plan_Tracker)).
...
> ⚠️ **З SEC.22 (credentials→ENV 2026-07; Phase-2 2026-09-02) цей перелік — ЛОКАЛЬНИЙ ФОЛБЕК, не продовий дім:** кожен читач має форму `ENV["X"].presence || credentials.x` (гейт `credentials_env_fallback_spec`), у деплой `RAILS_MASTER_KEY` не їде, а vault репо тримає ЄДИНИЙ ключ — `secret_key_base`; продові значення цих сервісів заводяться ENV-рядками §2.1 (кошик `S1.1`). Локально ключі ставляться через `bin/rails credentials:edit`; відсутність → `nil`-помилки або explicit raise у відповідному сервісі **при виклику** (не на boot). ⛔ Не заводь продове значення сюди: `credentials:edit` НЕ ротує те, що вже стоїть у deploy-ENV (§5.7). Перевірено grep'ом `credentials.*` по `app/`+`config/`.
...
⊕ **Клас ширший за поіменний tripwire (ARCH.114 §🗄️, 2026-09-02): won't-do закриває СПОЖИВАЧА, не КРЕДЕНШЕЛ.** `PROJECT_PAT` простояв у repo-секретах 24 дні після зняття єдиного споживача (OPS.1 ⚫) з нулем читачів у `.github/`, і жоден retired-tripwire його не знав — список знає лише імена, які хтось додав. Ретируючи споживача, тим самим рухом знімай його секрет у репо І токен в акаунті (`gh secret delete` + Settings → Tokens), інакше стояче повноваження переживає своє won't-do мовчки.
...
**Рішення (2026-07-06):** приватники `ORACLE_MINTER_PRIVATE_KEY`/`ORACLE_SLASHER_PRIVATE_KEY` (та решта oracle-ключів) живуть **plaintext у deploy-ENV** — у момент mint'у за реальну вартість ENV-ключ мінтера/слешера = найбільша одинична точка катастрофи. 🔴 **Поверхня ШИРША за ENV, і це переміряно 2026-08-30 проти джерела kamal, а не виведено:** `Kamal::Cli::App::Boot#run` вивантажує секрети ролі у файл `.kamal/apps/…/env/roles/job.env` (`mode: "0600"`) **на диск app-хоста**, тож квінтет є at-rest на PD, а не лише в `/proc/environ` живого процесу. Формула «діра = plaintext у deploy-ENV» описувала половину; мітигація at-rest — CMEK `app-boot` (§5.6, shipped), справжній seal лишається цим пунктом. Custody-поріг вирішено: **GCP Cloud KMS remote-signer** (asymmetric secp256k1) — ключ ніколи не покидає HSM, backend шле лише digest. Обрано над Fireblocks (enterprise-cost; забирає broadcast/nonce → перетин з `BlockchainConfirmationWorker`/ARCH.47-lock) і Safe-module-mint (on-chain роль + relayer = знову ключ; це admin-вектор SEC.1, не hot-mint): KMS = HSM-grade + дешево + автоматизований hot-path + GCP уже в стеку. ⚠️ **Ціна тут доти стояла «~$0.06/ключ/міс» — це тариф SOFTWARE-захисту, а `EC_SIGN_SECP256K1_SHA256` існує ЛИШЕ на рівні HSM** (виміряно по прайс-листу й довіднику алгоритмів 2026-09-02): HSM-версія EC-ключа — **$2.50/міс** (перші 2 000 версій у проєкті, далі $1.00) і ≈$1–2 за 10 000 підписів. Для двох ключів на оракульних обсягах це ≈$5/міс плюс копійки за операції — рішення не змінюється, змінилась одиниця, і саме такі одиниці протухають мовчки. ⊕ Три виміри, куплені тим самим проходом і потрібні при impl: (1) `eth` 0.5.17 не має жодного seam-у для зовнішнього підписанта на рівні `Client#transact` (той жорстко робить `tx.sign(key)` на `Eth::Key`), тож `KmsSigner#transact` будує `Eth::Tx::Eip1559` сам і вливає підпис публічним `Eth::Tx#sign_with(r‖s‖v)` — усі 12 сайтів виклику йдуть `legacy: false`, тобто EIP-1559-only достатньо; (2) гем `google-cloud-kms` відсутній у `Gemfile.lock` і тягне `grpc` (нативний C-ext, якого в дереві немає) — це інший клас залежності, ніж наявні REST-геми Google, і він іде через скіл `dependency-update` (збірка образу `amd64`); (3) `Treasury::MonitorService` свідомо обходить `OracleSigner` і читає plaintext-ключі для балансів — після KMS цей read-only сайт мусить деривувати адресу з публічного ключа, інакше він єдиний лишиться на plaintext.
...
1. **`Web3::OracleSigner` seam** — eth-gem `client.transact(sender_key:)` хардкодить локальний `Eth::Key` (KMS не має чим його повернути), тож потрібен інтерфейс `address` + `transact(...)` + `static_call(...)` навколо 7 signing-сервісів (minting/burning/celo/puro/klima/etherisc/anchor; 12 call-sites, minting сам 5 — 4 `transact` + 1 `call` (число переміряно проти дерева 2026-08-26; доти тут стояло 6)). `LocalEnvSigner` (default) делегує as-is — behavior-preserving, дзеркало SEC.3 master-key DI.
   ⛔ **Не брати сюди число ГРОШОВОГО КВІНТЕТУ (5) — воно хибне ОБАБІЧ, і саме тому спокусливе:** дві множини схожі розміром, але не перетинаються за призначенням. Квінтет (`§1.1`) є переліком того, що лежить у deploy-ENV **job-ролі**, і його пʼятий член `SOLANA_WALLET_KEYPAIR` — **Ed25519**, який цей KMS-шлях не бере ЗА ПОБУДОВОЮ (secp256k1-only, `SECURITY_ASSURANCE_CASE §6`). Тобто квінтет одночасно **завищує** на Solana й **занижує** на `puro`/`klima`/`etherisc`. Скоуп KMS задає `Web3::OracleSigner`, а не набір секретів.
...
**Рішення (2026-07-09):** boot-disk Ingress Anchor'а тримає `coap.env` (`SECRET_KEY_BASE` + AR-encryption-ключі — `RAILS_MASTER_KEY` знято з heredoc 2026-09-02, §5.7; **НЕ** `PROVISIONING_MASTER_KEY` — INF.17 2026-07-10 прибрав його з **АНКЕРНИХ** coap-поверхонь, fleet-forge root off анкера; ⚠️ читати це як «з усіх coap» зашироко, і межа не косметична: дормантна Kamal-роль `coap` не має `env:`-оверрайду, тож успадковує ГЛОБАЛЬНИЙ `env.secret` разом із цим ключем — per-role secret-exclude у Kamal не існує. Названо й прийнято дослівно над роллю в `config/deploy.yml`; ризик відкривається лише активацією fallback-ролі) at-rest. Поверх дефолтного GMEK — **CMEK** (`terraform/kms.tf`: keyring `silken-disk-ew1`, key `anchor-boot`, symmetric) для key-lifecycle-контролю (disable/rotate/audit/crypto-shred). Wrap DEK робить **Compute Engine Service Agent** (`service-<num>@compute-system`, key-level encrypter/decrypter — **НЕ** deploy-SA; `google_project_service_identity` compute-excluded → constructed string + `depends_on` compute-API). SHIPPED pre-deploy (timing: `kms_key_self_link`=ForceNew → на живий VM = replacement; до 1-го apply = нуль-cost).
...
| `silken-mac-ew1` | `provisioning-mac` | **MAC** (Expand-only HKDF: backend + firmware разом — crown-jewel `PROVISIONING` ніколи не в процесі) · job/coap SA | 🔗 SEC.22 pre-mainnet, тір 2 у [`03_06 §5`](03_06_Factory_Flashing_and_Key_Provisioning) (четвертий keyring поряд трьох наявних; імʼя ключа — робоче, до заведення) |
...
**coap PROVISIONING-drop — DEFINITIVE 2026-07-10.** coap-guard (`master_key_strength_check.rb` `$PROGRAM_NAME`-skip) дозволяє coap бутитись без `PROVISIONING_MASTER_KEY` (coap лише enqueue-ить, ключів не деривує — code-proven: демон → PDU-парс → `perform_async`, нуль key-derivation). Раніше deploy-поверхні й анкор несли його «для parity»; знято з **анкор-coap-поверхонь** (heredoc `compute.tf` + guard) — HKDF fleet-forge root off coap `/proc/environ` [SEC.22]. ⚠️ **«З усіх» тут стояло зашироко (звірено 2026-08-30):** дормантна Kamal `coap`-роль успадковує глобальний `env.secret` включно з `PROVISIONING` — named+accepted прямо в `config/deploy.yml` (Kamal не має per-role secret-exclude; PRIMARY = анкер), тож ризик відкривається лише активацією fallback'у. Web/job тримають shared tf-var `provisioning_master_key` (реально деривують). Regression-guard: `spec/deploy/anchor_coap_env_spec.rb` (forbidden-set — шість токенів: PROVISIONING + money-квінтет).

**Phase-2 — ✅ SHIPPED 2026-09-02: `RAILS_MASTER_KEY` знято з УСІХ пʼяти deploy-поверхонь** (global `env.secret` · `.kamal/secrets-common` · крок `kamal deploy` + `BOOT_CRITICAL` обох воркфлоу · heredoc `coap.env` анкера — пʼята, якої перелік у трекері не називав). Підстава — ВИМІР, не план: образ не несе `credentials.yml.enc` (`.dockerignore`, інваріант C `deploy_secret_scan`), тож у контейнері ключ не розшифровував НІЧОГО ще з першого буту, а vault репо тримає ЄДИНИЙ ключ — `secret_key_base`, який їде окремим `SECRET_KEY_BASE` (§1.1). «Інжект восьми сервісів через Console» як передумова дропу був кроком над ПОРОЖНЬОЮ множиною — усі вісім читають `ENV[..].presence || credentials.x`, і credentials-половина в контейнері була `nil` завжди. 🔴 **Той самий вимір показав, що heredoc анкера був мертвий на першому буті ІНАКШЕ:** він ніс `RAILS_MASTER_KEY` і НЕ ніс `SECRET_KEY_BASE`, а `active_storage.verifier` кличе `message_verifier` при буті БУДЬ-ЯКОГО Rails-процесу — демон падав би тим самим «Missing `secret_key_base`», що й перший canopy-контейнер (run 33499749733); тепер шаблон несе `SECRET_KEY_BASE`, пін — `anchor_coap_env_spec`. Наслідок для DR: втрата `RAILS_MASTER_KEY` більше не незворотна (порожній vault регенерується), незамінним лишається `SECRET_KEY_BASE` — [`06_06 §4`](06_06_Disaster_Recovery_and_Backup). ⚠️ Що Phase-2 НЕ закриває, і це не його вісь: service-креденшели восьми сервісів у production досі ніде не заведені (present-placeholder → 401; inject-at-deploy через Console) — і одиниця тут ІНША, ніж «вісім»: `iotex`, `peaq` і `streamr` несуть по ДВІ змінні кожен, тож інжектити треба за ПЕРЕЛІКОМ `.env.example`/§2.1, а не за числом; так само пара сховища `storage.yml` — [`S1.1`](00_07_Action_Plan_Tracker), ДО першого production-блоба (§2.1).
