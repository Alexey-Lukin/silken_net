...
- **Beyond TRL 9 — НЕ «TRL 10-12».** Шкала закінчується на **9** і міряє виключно готовність самої ТЕХНОЛОГІЇ; масштаб, cross-biome адаптація та forest-level emergence є системною й виробничою зрілістю. Вигадувати «TRL 10-12» ми не станемо — це читалось би некомпетентно перед ESA/NASA, які тримаються 1–9. Натомість профільні шкали: **SRL** (System Readiness — інтеграційна зрілість: cross-biome generalization, forest-level emergence, edge self-evolution, AI-adversarial security) і **MRL** (Manufacturing Readiness, 1-10 — серійний друк SKU, заводське штампування). Деталі → [`00_08 §1`](00_08_Beyond_TRL9_Planetary_Roadmap).
...
| `HilLorenzGenerator` | mruby Lorenz curves з різних tree species, environmental conditions (temp, vibration), faulty/normal patterns | `lib/hil/lorenz_generator.rb` | TinyML training data + Rails Attractor validation |
