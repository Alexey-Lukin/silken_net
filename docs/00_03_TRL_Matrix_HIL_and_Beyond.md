...
|-----------|--------|------|-------------------------------|
| `bin/forest_simulator` | Емулює **Queen→Backend CoAP-батчі** (AES-256-**CBC**) з телеметрією Soldier'ів. ⛔ **НЕ** несе Lorenz: усі поля — `rand()`, і жодного перемикача режимів у скрипті немає (перевірено 2026-08-22). Lorenz-дзеркало живе в `HilLorenzGenerator` рядком нижче — не плутай їх, інакше рівномірний шум зарахується як Lorenz-валідація. 🔴 **НАСЛІДОК ЦІЄЇ СТЕЛІ НА ЖИВОМУ СЛОТІ — виміряно 2026-09-05, і він не був названий: прогін ЗАПАЛЮЄ критичний алерт `Telemetry fraud detected`.** `bio_status` береться `rand(0..3)`, а DCI (`check_z_divergence!`) саме й питає, чи збігається заявлений пристроєм стан із серверним Z із насіння — тож розбіжність гарантована, і алерт **істинно-позитивний для механізму**: «шахраєм» тут є наш власний симулятор. ⚠️ **Тобто симулятор не може валідувати DCI ЗА ПОБУДОВОЮ** — уся суть DCI в тому, що зійшлись ДВА НЕЗАЛЕЖНІ обчислення, а сторона, згенерована `rand`, не зійдеться ніколи. Він чесно емулює дріт і крипту (тому AES/CoAP і працюють), але не обчислення, а перевіряється саме воно. ⛔ **Операційна ціна названа:** регулярні демо-прогони роблять критичний fraud-канал вічно-червоним, тобто безмовним — той самий закон, що ратифікований для production-silence і DeadSet ([`00_07`](00_07_Action_Plan_Tracker) S2.4). Шлях апгрейду теж названий: вивести `bio_status` тим самим `Attractor`, що й сервер — тоді DCI стає перевірним, а не завжди-червоним. ⚠️ Це Queen-рівень: per-Soldier LoRa-хоп — на залізі **AES-128** (ECB→CCM, [`03_05`](03_05_Hardware_Symmetric_Crypto_and_Security)), а до бекенду доходить CoAP-батч (AES-256-CBC). **Два режими (див. ⚠️ нижче):** `load_test_mode` (батч кожні 3–8 сек — стрес черг) та `realistic_mode` (CIFO-точний: рідкі об'ємні батчі ~раз на годину/45 записів + packet loss + jitter мобільної мережі) | `bin/forest_simulator` (load_test існує; realistic_mode — TODO) | Локальна розробка Rails + sidekiq + Web3 pipeline |
