...
| 06 DevOps | 5 | 9 | 🔻 **5→4 при зрізі [`OPS.37`](00_07_Action_Plan_Tracker) 2026-08-29, і це чесний наслідок, а не втрата:** пʼятірку модуля ніс знятий док про платформу, яку ніколи не будували; вцілілий мінімум — [`06_01`](06_01_Deployment_Kamal_Terraform)=4, і після зрізу він НА критичному шляху, а не «fallback», тож винятку рядок більше не потребує. Гейт (переміряно 2026-08-31): **інфра ІСНУЄ** — перший `terraform apply` пройшов 56/56 (дві VM, Cloud SQL, VPC, WIF, KMS, бюджет). Не пройдено інше й саме воно тримає число: **застосунок ДЕПЛОЇВСЯ 2026-09-01, і число тримає вже НЕ це** — серія прогонів `kamal deploy -d canopy` провела ланцюг до запущеного контейнера, а boot-critical набір canopy закрито; 🔴 **і ця підстава ВПАЛА 2026-09-03 — рядок відтоді ЗАНИЖУЄ, а не завищує:** доти його тримав «завершений бут, якого не було» (гард відмовляв на незаведених адресах слоту), а тепер бут завершується і слот **обслуговує трафік** — `canopy-web`/`canopy-job` віддають `/metrics` (`up=1`), `/ready`=200 крізь Cloudflare, і шлях end-to-end пройдено кінець-у-кінець: симулятор → UDP → анкерний демон → enqueue → job → рядок у `TelemetryLog` бази ТОГО слоту. ✅ **І присуд ухвалено того ж дня — `4→5`, TRL Gate Event, ⚖️ founder 2026-09-03.** Це не наслідок зеленого прогону: §4.1 називає критерій іменем — «з лабораторії до pilot **потребує HIL-валідації (§3)**», — і саме вона й відбулась: `HilQueenSimulator` (`lib/hil/queen_simulator.rb`) провів CoAP-інтейк кінець-у-кінець у живому середовищі. Агрегат-правило «рядок = мінімум member-TRL» тримається: [`06_01`](06_01_Deployment_Kamal_Terraform) піднято тим самим присудом. 🔴 **Що тримає число на 5, а не вище — і з 09-03 це вже НЕ деплой сам по собі:** рядок є МІНІМУМОМ member-TRL, а на пʼятірці стоять ТРИ доки — [`06_01`](06_01_Deployment_Kamal_Terraform), [`06_04`](06_04_Secrets_Checklist) (production-квінтет не заведено) і [`06_06`](06_06_Disaster_Recovery_and_Backup) (§5.2 у ПОВНІЙ формі не прогнано — «витягти ≠ відкотити»); решта чотири вже на 6. Тобто стеля модуля переїхала з деплою на СЕКРЕТИ й DR, і саме там її наступний рух. ⛔ `6→7` гейтовано §4.1 однозначно — «реальний LoRa-лінк і реальний CoAP-інтейк, **не симулятор**» — тож сьогоднішній HIL-прогін той гейт НЕ відчиняє за побудовою. Похідне для [`06_01`](06_01_Deployment_Kamal_Terraform): **production не деплоївся жодного разу** ([`00_07`](00_07_Action_Plan_Tracker) DEPLOY-1 Фази 2/5), тобто живий лише staging-слот |
...
> **HIL vs SIL (термінологічна чистота):** *Hardware-in-the-Loop* імітує **залізо/фізику** (MCU, LoRa-мережа, EBFC-крива) — `forest_simulator`, `HilQueenSimulator`, **`HilSoldierNode`** (Lorenz-ланцюг вузла, з 2026-09-06), `HilLorenzGenerator`. *Software-in-the-Loop* стабить **програмний API** (стаб-сервіси без реального заліза) — це **Web3 SIL** нижче, який раніше помилково звався «HilWebPipelineSimulator». Розрізнення важливе: SIL валідує лише логіку (TRL 5-6), HIL відтворює фізичне навантаження.

| Симулятор | Імітує | Файл | Замінює реальний компонент для |
|-----------|--------|------|-------------------------------|
| `bin/forest_simulator` | Емулює **Queen→Backend CoAP-батчі** (AES-256-**CBC**) з телеметрією Soldier'ів. ✅ **НЕСЕ Lorenz із 2026-09-06 — стеля ЗНЯТА, і цей рядок є її спростуванням.** Доти тут стояло «⛔ НЕ несе Lorenz: усі поля — `rand()`», і наслідок був названий, але не виправлений: `bio_status = rand(0..3)` проти серверного Z із насіння давав ГАРАНТОВАНУ розбіжність, тож перший multi-node прогін на canopy 2026-09-05 запалив критичний `Telemetry fraud detected`. Алерт був **істинно-позитивним**: «шахраєм» був наш власний симулятор, а регулярні демо-прогони робили критичний fraud-канал вічно-червоним, тобто безмовним (той самий закон, що тримає глушник production-скрейпу — [`06_01 §DEPLOY-DAY`](06_01_Deployment_Kamal_Terraform) Фаза 5). **Тепер вузол РАХУЄ:** модель Солдата винесено в `Hil::SoldierNode` (сусід `Hil::QueenSimulator`) — власний Lorenz-ланцюг, cold-start із per-device `K_seed` через `SilkenNet::SeedDerivation`, далі продовження з ВЛАСНОГО хвоста, як прошивка тягне його з RTC DR16-DR18; статус-байт пакується дзеркалом `SilkenNet::Attractor.pack_status_byte`. Скрипт лишився тонким викликачем, бо спек у `bin/` не буває. ⚠️ **НОВА СТЕЛЯ, названа замість знятої — читай як специфікацію того, чого прогін НЕ доводить: це НЕ доказ незалежності ОБЧИСЛЕНЬ.** Обидві сторони крутять `SilkenNet::Attractor`, тож доводиться ТРАКТ (пакування → шифр → транспорт → розпакування → продовження стану → DCI-звірка). Байт-парність із кремнієм тримають ІНШІ гейти — 200-кейсовий sweep проти СПРАВЖНЬОГО контракту (`spec/services/silken_net/attractor_spec.rb` через підпроцес `tools/firmware/contract_runner.rb`) і FW.55 QEMU byte-parity. ⛔ Той міст у симулятор не переноситься: `firmware/bio_contracts/bio_contract.rb` перевизначає `SilkenNet::Attractor` у тому ж процесі, а `/firmware` і `/tools` свідомо не їдуть у Docker-образ ([OPS.10] `.dockerignore`) — тобто на canopy, де симулятор і біжить, того файлу фізично немає. ⚠️ Друга стеля: ланцюг припускає ПОРЯДОК пакетів на дерево; пауза між батчами (3–8 с) на порядки більша за час розпакування, і ту саму передумову має справжній флот. ⚠️ Третя стеля: пороги у вузлі **ПРИСТРОЄВІ** (`Tree::GLOBAL_LORENZ_Z_MIN/MAX`), а не per-species — прошивку per-species значеннями не провіжинять, і `check_z_divergence!` судить саме пристроєвою смугою [FW.8]; ⛔ не читати прогін як перевірку governance-ланцюга порогів. 🔑 **One-Home оголошених стель — шапка `lib/hil/soldier_node.rb`**: цей рядок їх ПЕРЕЛІЧУЄ, а не визначає, тож при розходженні правда там, і звіряти треба проти неї, а не проти памʼяті про неї. ⚠️ Це Queen-рівень: per-Soldier LoRa-хоп — на залізі **AES-128** (ECB→CCM, [`03_05`](03_05_Hardware_Symmetric_Crypto_and_Security)), а до бекенду доходить CoAP-батч (AES-256-CBC). **Два режими (див. ⚠️ нижче):** `load_test_mode` (батч кожні 3–8 сек — стрес черг) та `realistic_mode` (CIFO-точний: рідкі об'ємні батчі ~раз на годину/45 записів + packet loss + jitter мобільної мережі) | `bin/forest_simulator` + `lib/hil/soldier_node.rb` (load_test існує; realistic_mode — TODO) | Локальна розробка Rails + sidekiq + Web3 pipeline |
...
| Software TRL 7 (Operational prototype) | ~pilot 100 дерев | Все HIL + chaos engineering (**аплікаційний** ✅ — `spec/integration/proof_of_growth_chaos_engineering_spec.rb`: падіння IoTeX 24h, reorg Polygon, timeout Chainlink, каскадні мультичейн-збої; **інфраструктурний** — окремої платформи (Chaos Mesh) свідомо НЕ заводимо: дім цілі — [`06_08 §2.7`](06_08_Resilience_and_Failover_Policy) п.4, носій — quarterly DR-drill [`DR.1`](00_07_Action_Plan_Tracker); трекер-пункт `E.27` заархівовано 2026-09-05, читати як провенанс) + **реальний testnet pipeline ОБОВ'ЯЗКОВИЙ** (Polygon Amoy + Solana Devnet + Ethereum Sepolia) з реальними RPC-вузлами | 🟡 Частково (chaos engineering ✅; testnet pipeline: Amoy і Sepolia живі з 2026-09-02 — слот canopy `testnet`, контракти на обох, `StateRootAnchor` Sepolia у гарді (INF.27 закрито — §🗄️ в [`00_07`](00_07_Action_Plan_Tracker)); Devnet і роль `job` на canopy ЗАКРИТО 2026-09-03 — мінт-тракт на testnet пройдено кінець-у-кінець (дім Фази 2t — [`00_07`](00_07_Action_Plan_Tracker) `DEPLOY-1`; форма canopy — `OPS.37` §🗄️)) |
| Software TRL 8 (Production-validated) | ~1000+ дерев у полі | **HIL відтворює 1000+ віртуальних дерев** + **повний testnet stress** (Amoy/Devnet/Sepolia: gas spikes, RPC 429, nonce collisions, DON latency, orphaned blocks) + Slither/Aderyn high-severity = 0 + Halmos symbolic proofs + Medusa property-fuzz clean + multi-sig deployment dry-run. Mainnet — лише TRL 9. | 🟡 Контракти TRL 9-ready (Foundry/Slither/Aderyn/Halmos/Medusa); backend↔chain integration потребує testnet-стресу мінт-тракту — роль `job` на canopy бутнула 2026-09-03, і мінт-тракт на testnet із того дня пройдений (форма canopy — [`00_07`](00_07_Action_Plan_Tracker) `OPS.37` §🗄️; слот і контракти живі — INF.27 §🗄️) |
...
> ⚖️ **«Трек C — Code-as-Mechanics» ВИДАЛЕНО, і підстава не технічна.** Важка механіка полімерів (в'язкопружність PEEK, Prony series, термонапруження Ti+PEEK, 20-річний creep) канонічно закріплена за лабораторією ЧНУ на ANSYS LS-DYNA ([`01_01 §4.3`](01_01_Coaxial_Gyroid_Topology_and_PEEK) · [`00_02`](00_02_Academic_Integration_and_IP)). Паралельний AI-трек на CalculiX дав би **другий, неузгоджений** результат і знецінив би роботу професорів. Наш конвеєр покриває хімію, геометрію, кінетику та **легкий closed-form mechanics-bound** (аналітичний Lamé) — швидку оцінку-границю, що ЯВНО відкладає авторитетну mesh-FEA і Prony-fit до партнера. 🔴 **Уточнення 2026-09-12, і воно матеріальне: «БЕЗ mesh» тут стояло й перестало бути правдою.** In-house з’явився воксельний скінченно-елементний розв’язувач (`tools/cad`, верб `fea`) — сотні тисяч трилінійних гексаедрів на власному CG. **Присуд про Трек C він НЕ порушує, і межу треба тримати названою:** наш розв’язувач лінійно-пружний, ізотропний, малих деформацій і міряє ОДНУ величину — апарентну жорсткість ґратки ([`01_01 §5.2`](01_01_Coaxial_Gyroid_Topology_and_PEEK)); усе, заради чого Трек C зрізали, лежить поза його стелею **за побудовою** (в’язкопружність, Prony, контакт, концентрація напружень у зубці, 20-річний creep) і лишається за партнером. ⛔ Не читати цей прецедент як дозвіл на паралельний трек із полімерної механіки.
...
| FEA mechanical (полімер, в’язкопружність, контакт) | ANSYS Workbench | **PyAnsys / PyMAPDL / PyDPF** (headless — **у партнера**) |
| FEA пружності ҐРАТКИ (лінійна, гомогенізація) | — | **власний воксельний CG** у `tools/cad` (верб `fea`) — без комерційної ліцензії; стеля оголошена в [`01_01 §5.2`](01_01_Coaxial_Gyroid_Topology_and_PEEK) |
