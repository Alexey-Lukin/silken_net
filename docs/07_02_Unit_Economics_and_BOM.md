...
При кластері **100 дерев** та blended failure ~5% на рік (середнє по 5 рокам), v3 BOM:
- 5 заміна Soldier × $46 (v3 CAPEX) = $230 / рік
- 5 інсталяцій × $2.20 (праця) = $11 / рік
- **Total Replacement OPEX: ~$20.10 / місяць** на кластер 100 дерев

**Оновлений OPEX (доповнення до §6):**

| Стаття | Сума / міс (v3 BOM) |
|---|---|
| Зв'язок Queen (eSIM) | ~$1.50 |
| Хмара / RPC | ~$5.00 |
| Амортизація Queen (заміна шлюзу) | ~$3.50 |
| **Soldier replacements (BIZ.7, ~5% annual, v3 ціна)** | **~$20.10** |
| **Сумарний OPEX на кластер (з replacements)** | **~$30–$35 / місяць** |
...
**Baseline (v3 BOM, з урахуванням BIZ.7 OPEX):**
- Кластер 100 дерев: ~433 SCC/місяць (як §7.2)
- Дохід: 433 × $0.30 = $130 / місяць
- OPEX (з replacements + battery): ~$31 / місяць
- **Net = $130 − $31 = $99 / місяць**
- **Payback Period: $5,100 / $99 ≈ ~52 місяці** (v3 чесна оцінка; v2-еталон $4,000/$104=~38 міс залишається в історичних архівах)

**При $1.00/SCC (ReFi premium):**
- Net = $433 − $31 = $402 / місяць
- **Payback Period: ~13 місяців** (premium absorbs the OPEX increase)

> **Висновок BIZ.7 (v3 актуальний):** Перехід на тризонний анкер з HIP + PEEK CNC + PTFE-GDL додав ~$11 до собівартості вузла та подовжив Payback з ~38 до ~52 місяців у baseline. Це не змінює стратегічну життєздатність — при $1.00/SCC payback залишається у "зоні Hi-tech VC" (<18 міс). Критично — **інвестувати в EBFC Gen 2.0** (HW.5 — FAD-GDH + Laccase + ZIF, 20-25 років) для зниження failure rate до <1%/рік у Years 4+ → довгостроковий gross margin покриває v3 CAPEX premium.



## 📈 9. Порівняльна таблиця: Стара vs Нова Архітектура

| Показник | v1 (LTC3108 + 4 голки + U.FL) | v2 (Матрьошка + SMD + PEEK) | **v3 (Тризонний коаксіальний анкер)** ⭐ Поточна |
|---|---|---|---|
| **Вартість вузла** | ~$55–70 | ~$32–35 | **~$42.50–$50 (середня $46)** |
| **Кількість компонентів BOM** | 18+ | 11 | **13** (додано HIP, PEEK CNC, PTFE-GDL) |
| **Зональна архітектура** | Моноліт 120 мм | Моноліт 120 мм | **Zone 1 (Ti-анод) + Zone 2 (PEEK термо-розрив) + Zone 3 (катод-фланець)** |
| **Тепловий міст анод-катод** | ❌ Прямий контакт | ❌ Прямий контакт | **✅ Усунено через PEEK Zone 2** |
| **Кисневий доступ до катода** | ❌ Катод у заболоні | ❌ Катод у заболоні | **✅ Zone 3 на межі кори/повітря** |
| **Час збірки антени** | ~30–60 с (ручна) | ~0.1 с (robot) | **~0.1 с (robot)** |
| **IP68 герметизація** | Складна | Монолітна | **Монолітна + Hard Gold pogo-площадка** |
| **Вандалостійкість антени** | ❌ | ✅ (під PEEK-радомом) | **✅** |
| **Payback Period (кластер @ $0.30/SCC)** | >15 міс | ~34 міс | **~44 міс (baseline), ~52 міс (з BIZ.7)** |
| **Payback Period (кластер @ $1.00/SCC)** | — | ~10 міс | **~12 міс (baseline), ~13 міс (з BIZ.7)** |
| **Маса вузла (electronics)** | ~45 г | ~28 г | **~32 г** (PEEK Zone 2 додає масу) |
