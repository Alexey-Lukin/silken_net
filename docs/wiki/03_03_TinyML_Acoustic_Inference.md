...
| **`Run_Inference()` виклик** | 🔴 BLOCKER — **закоментовано** (`// ml_event_id = Run_Inference(...)`) |
...
3. `acoustic_events` скидається в `0` після пакування (`lora_payload[7] = ...; acoustic_events = 0;`), але завдяки `HAL_RTCEx_BKUPWrite(DR0, acoustic_events)` перед STOP2 — накопичення відновлюється після пробудження. Якщо RTC write відбувся після переповнення — дані вже зіпсовані.
...
Класичний паттерн уникнення race condition: якщо ISR спрацює між `while (!audio_ready)` та `WFI`, без `__disable_irq()` CPU засне і ніколи не прокинеться.
...
void Trigger_Emergency_LoRa_TX(void)
{
    ...
    // Байт 11: PANIC_TTL = 5 (стандартний TTL = 3)
    panic_payload[11] = PANIC_TTL; // 5 стрибків замість 3
