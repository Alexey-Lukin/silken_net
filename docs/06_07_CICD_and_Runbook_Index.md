...
- **Поточний TRL:** TRL 6 — workflows активні; production+canopy deploy налаштовані; **`main` захищено branch-protection** (required status checks = усі **7** детермінованих PR-гейтів: `CI passed` + `Docs passed` + `Solidity passed` + `CAD passed` + `ML passed` + `In-silico passed` + `IaC passed`, `enforce_admins=false` → owner лишає прямий push, PR-и гейтяться); `coap-smoke` gate заведений, але dormant до host-Variable (INF.6).
...
| `iac_scan.yml` → **Sec · IaC Scan** | PR/push `terraform/**`·`Dockerfile` + dispatch | Trivy `config` misconfig-скан IaC (terraform + Dockerfile) → SARIF у Security tab; **soft-fail** hygiene-репорт (ловить misconfig ДО першого деплою). Політика (tool-вибір · SHA-pin) — [`00_05 §2.7`](00_05_GitHub_Projects_and_IaC_Automation). [INF.22] |
...
> **Gate-perimeter invariant (OPS.14, landed):** branch-protection вимагає рівно дві перевірки (`CI passed` + `Docs passed`), а КОЖЕН інший `pull_request`-workflow job-gating але merge-**advisory** — тож новий детермінований PR-гейт міг народитися ПОЗА required-множиною непоміченим (саме так money-path `solidity_audit` лишався advisory, поки канон казав «all gating»). `scripts/workflow_gate_perimeter.rb` (HARD `docs.yml`, pure-Ruby) стереже сам периметр: курований `PERIMETER`-SSOT класифікує кожен PR-workflow у три відра — **`:required`** (merge-blocking через `if: always()`-агрегат, чиє `name:` ∈ branch-protection: `ci.yml`→`CI passed`, `docs.yml`→`Docs passed`) · **`:advisory_by_design`** (свідомо не блокує, з ратифікованою причиною — `ssot_guard.yml`, path-gated red-X інформує) · **`:flip_pending`** (МАЄ стати required, трекається: `solidity_audit.yml` → [`00_07`](00_07_Action_Plan_Tracker) OPS.15; `cad_smoke`/`ml_smoke`/`in_silico_smoke`/`iac_scan` → OPS.16). HARD-осі: некласифікований PR-гейт → RED «класифікуй» · мертвий запис (workflow зник / більше не PR-тригер) · `:required` без свого `if: always()`-агрегату. Опційний `--live` (локально, не CI — токен CI не читає protection) звіряє `:required`-мітки з реальними `required_status_checks`. Advisory-лічильник `:flip_pending` спадає до 0, як OPS.15/OPS.16 приземляють агрегати + оновлюють branch-protection (тоді запис `PERIMETER` `:flip_pending`→`:required`).
