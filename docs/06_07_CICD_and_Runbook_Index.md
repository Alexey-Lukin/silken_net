...
| `solidity_audit.yml` → **CI · Solidity** | PR/push `contracts/**` | Smart-contract audit: static (Slither + Aderyn) · symbolic (Halmos) · fuzz (Foundry + Medusa). Aderyn SARIF → Security tab; gate на Slither/Aderyn high, Halmos/Medusa non-gating (advisory до tune) |
