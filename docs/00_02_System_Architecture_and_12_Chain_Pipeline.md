...
- **Енергетика:** Ензимний біопаливний елемент (EBFC):
  - **Анод (Zone 1, у ксилемному соку):** Combi-CLEA `GOx + Catalase` + Os-полімер + MWCNT (Gen 1.0) — каталаза нейтралізує H₂O₂; або кислотостійкі мутанти GDH (Gen 2.0, без H₂O₂)
  - **Катод (Zone 3, на межі кори/повітря):** Laccase-AuNP DET + MWCNT, ORR атмосферного O₂ крізь PTFE-GDL (0.2–1.0 µm пори)
...
Дерево дихає. Ксилемний сік тече вгору, генеруючи понад 500 мВ завдяки метаболізму глюкози через тризонний анкер (`01_01`): Combi-CLEA `GOx+Catalase` на аноді Zone 1 у заболоні + Laccase-AuNP DET на катоді Zone 3 на межі кори/повітря. BQ25570 MPPT заряджає суперконденсатор 0.47 Ф. Коли напруга досягає порогу — STM32WLE5JC Soldier прокидається зі сну STOP2 (2.1 µА).
