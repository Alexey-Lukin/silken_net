...
| **Realistic продуктивність (Variant C)** | **~5.4 SCC/дерево/рік** (calibration-pending [E.63]; стеля Δt=600s ≈ 326) |
