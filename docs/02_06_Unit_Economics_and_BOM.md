...
> ⚠️ **Прогін на CCM-точці обидві сусідні моделі відхиляють** (`scc_rate.rb variant_c_s=7884 --assert` → exit 1, `uncertainty_budget.rb delta_t_s=7884 --assert` → exit 1) — але це означає «точка поза self-consistency-вікном моделі», **а не «число хибне»**: 4.00 реальне. Вимір і три важелі розвилки — [`00_07`](00_07_Action_Plan_Tracker) ARCH.8 (там же ⛔ «не відвантажувати CCM, доки розвилка відкрита»).
