...
> ⚠️ **«baseline» тут — ВХІД СЦЕНАРІЮ, не курс системи, і поруч живе число з іншого світу** (звірено 2026-08-26): fallback оракула [`00_04 §3`](00_04_Nature_as_a_Service_Contracts) стоїть на **$25.50** — у 5× вище за найоптимістичніший рядок цієї ж таблиці. Розбіжність не операційна (ціну сьогодні не читає ЖОДЕН прод-шлях — `PriceOracleService` має нуль споживачів, `attested_value_usd` `null` за [ARCH.103]), але вона робить будь-який ROI-розрахунок залежним від того, який якір читач ухопить першим. ⛔ Не «примирювати» їх усередненням — це два різні ЖАНРИ гіпотези; присуд, який із них є курсом системи (і чи взагалі система має курс), — [`00_07`](00_07_Action_Plan_Tracker) DOC-T.89.
