...
|---|---|---|
| **Анкер Zone 1 + Zone 3 (Anode + Cathode)** | Ti-6Al-4V Grade 5 — ⚠️ **BASELINE-якір ціни, не обраний сплав**: V-free напрям для Zone 1 ратифіковано (founder 2026-06-21), а конкретний сплав down-select'иться Stage-2 6-сплавним coin bake-off ([`01_02 §2.5`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)) — той самий caveat, що вже стоїть на рядку порошку §3 і на vendor-критеріях §8.1.1. SLM+HIP друк (Zone 1) + SLM/EBM (Zone 3). Маса ~9 г сумарно (пористість гіроїда 65%). Включає Hard Gold pogo-площадку, **монолітну шину анода** ([`01_01 §1.4`](01_01_Coaxial_Gyroid_Topology_and_PEEK) — частина анодного друку, без окремого Cu-провідника) та PTFE-GDL катодну мембрану. | $20.00–$25.00 |
...
- **Вартість сировини:** Порошок Ti-6Al-4V **Grade 5** коштує **$126–$147 за кг**. ⚠️ Це якір на BASELINE-сплаві: V-free напрям для Zone 1 обрано (founder 2026-06-21), а конкретний сплав down-select'иться Stage-2 6-сплавним coin bake-off ([`01_02 §2.5`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)) — Nb/Zr-порошки дорожчі, тож число перерахують після bake-off.
- **Економія маси:** Пористість 65% (ном., [`01_01 §5.2`](01_01_Coaxial_Gyroid_Topology_and_PEEK)) знижує вагу анкера з 26.5 г до **~9 г** (= §1.2). Вартість **голого порошку** на виріб — **~$1.15–1.35** (9 г × $126–147/кг з рядка вище, тож той самий bake-off-caveat успадковується); друк/HIP — окремий рядок нижче. ⚠️ Buy-to-fly (невикористаний порошок, підпори, вихід рециклінгу) у це число НЕ входить і ніде в каноні не оцінений.
...
| Хмара / RPC (GCP + вузли ланцюгів) — телеметрія, ZK-докази | ~$5.00 |
| Амортизація (резерв на заміну пошкоджених шлюзів) | ~$3.50 |
| **Сумарний OPEX на кластер** | **~$10–$15 / місяць** |

> Soldiers не мають **інфраструктурного** OPEX — вони автономні енергетично й не платять за зв'язок (EBFC + Supercap, zero connectivity cost).

⚠️ **Але це НЕ «zero-maintenance», і таблиця вище цієї статті ще не містить.** [`01_04 §5.5 C`](01_04_CODIT_and_Xylemointegration) робить **візит форестера раз на 5–7 років** (механічне зчищення приростаючої тканини навколо PEEK Radome) умовою 20-річної довговічності катода, а не аварійною дією; праця арбористів фігурує в цьому документі лише як **одноразовий CAPEX** монтажу ([`§2`](#-2-витратні-матеріали-та-інсталяція)). Отже payback-крива §7.3 цієї праці **не враховує** — читати її як повну вартість володіння передчасно. Заведення статті — відкритий пункт [`00_07`](00_07_Action_Plan_Tracker) HW.28; каденція залежить від ⚖️ вибору покриття B (Fluoropel = UV-труп за 1–2 роки проти 20-річного клейму), тож число не вигадується наперед.
...
> ⚠️ **«baseline» тут — ВХІД СЦЕНАРІЮ, не курс системи, і тепер це єдиний ціновий якір у дереві.** Доти поруч жило число з іншого світу — fallback цінового оракула на **$25.50**, у 5× вище за найоптимістичніший рядок цієї ж таблиці. ⚖️ **Присуд (founder, 2026-08-26): розбіжність знято ВИДАЛЕННЯМ, а не арбітражем** — оракул мав нуль прод-споживачів, тож замість вибору «який із двох якорів правдивий» прибрано сам механізм разом із його governance-ключем. Курсу протокол не тримає ([`00_04 §3`](00_04_Nature_as_a_Service_Contracts)): система міряє КІЛЬКІСТЬ, вартість множить споживач. ⛔ Отже числа цієї таблиці не можна цитувати як ціну SCC — вони лишаються **сценарними входами** payback-моделі, і поява другого якоря деінде є дефектом, а не даними.
...
**Стратегічний принцип:** мати у backlog щонайменше **2 кваліфіковані EU-альтернативи** з підтвердженою здатністю друкувати Ti-6Al-4V **Grade 5** (baseline) **І ванадій-вільні імплант-сплави** (Nb/Fe/Zr), TPMS-гіроїди ≥60% пористості, ISO 13485 (медичні імпланти). ⛔ **Не звужувати вимогу до Ti-6Al-4V ELI:** канон називає його поіменно як пастку (ELI зберігає ~4% V — [`01_02 §2.5`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)), а фінальний сплав ще не обрано, тож вендор мусить уміти надрукувати те, що виграє bake-off, а не те, чим був baseline.
...
**Дії при спрацюванні тригера (👤 операційні) — це RUNBOOK, не черга.** Кроки виконуються ЩОЙНО активовано backup, і форму навмисно змінено з чекбоксів на нумерацію 2026-08-22: чекбокс у каноні читається як відкрита робота, а рішення активувати EU-плече **відкладене** (`BIZ.8`, §🗄️ Архів — founder 2026-07-23, «не треба поки що іноземні»; самі кандидати ідентифіковані в `BIZ.6`). Дім стану — трекер, дім процедури — тут.

1. Отримати quotes від 2-3 EU підрядників (мінімальна партія 100 шт).
2. Підписати NDA та **Frame Agreement** з Tier-1 кандидатом (без зобовʼязання обсягу). **Умови, узгоджені для production-шару: +20 % premium до spot-quote · 30-денна активація · SLA.** ⚠️ Premium купує не ціну, а ГОТОВНІСТЬ: spot-quote дешевший рівно доти, доки постачальник не потрібен терміново, а тригери вище настають саме терміново. ⛔ Це production-procurement, інший workflow, ніж R&D-RFQ (spot-quote, academ-co-pub) — їх не зливати.
3. Передати **PicoGK CAD-STL** (`tools/cad` `build` — анкер/катод/радом STL готові; AM-бюро друкують зі STL, **STEP не потрібен**) + factory spec (HW.1, HW.2) кожному кандидату.
4. Замовити пробну партію 10 шт у Tier-1 EU підрядника для quality benchmarking vs UA.
