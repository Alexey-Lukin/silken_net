...
|---|---|---|
| **Анкер Zone 1 + Zone 3 (Anode + Cathode)** | Ti-6Al-4V Grade 5 — ⚠️ **BASELINE-якір ціни, не обраний сплав**: V-free напрям для Zone 1 ратифіковано (founder 2026-06-21), а конкретний сплав down-select'иться Stage-2 6-сплавним coin bake-off ([`01_02 §2.5`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)) — той самий caveat, що вже стоїть на рядку порошку §3 і на vendor-критеріях §8.1.1. SLM+HIP друк (Zone 1) + Zone 3: CNC із прутка або SLM з обовʼязковим HIP ([`01_02 §1.7`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS), ⚖️ 2026-09-18). Маса ~9 г сумарно (пористість гіроїда 65%). Включає Hard Gold pogo-площадку, **монолітну шину анода** ([`01_01 §1.4`](01_01_Coaxial_Gyroid_Topology_and_PEEK) — приварений тягнутий дріт того самого сплаву, без окремого Cu-провідника — ⚠️ **не «частина друку»: присуд 2026-09-10 обрав ЗВАР, і для BOM це інша стаття** (куповані дроти + операція зварювання), а не нуль рядків) та PTFE-GDL катодну мембрану. 🔴 **Два елементи ратифікованого 2026-09-10 lining-присуду ([`01_01 §1.4`](01_01_Coaxial_Gyroid_Topology_and_PEEK) · [`00_07` HW.34](00_07_Action_Plan_Tracker)) у цю ціну НЕ входять і ніде в каноні не оцінені:** покупна PEEK-трубка зі стінкою **0.15 мм** (структурний лайнер каналу — не витратник, а деталь). ⚠️ **Дата присуду вище — 09-10, а його ПІДСТАВУ пересуджено 09-11, і для вартісного боку це не дрібниця:** лайнер лишився структурним, але несе тепер **ЗНОС**, а не фатигу стрижня — тобто це **поверхня тертя в наскрізному каналі на двадцять років**, чий ресурс обмежено лише БЮДЖЕТОМ зносу (`55_bus_mechanical` §`wear_budget`), а питому швидкість зносу пари не виміряно ніде — вирок дає трибо-тест ([`00_07`](00_07_Action_Plan_Tracker) HW.34). Наслідок саме для цієї сторінки: рядок не можна закривати самою ціною трубки — при протиранні до металу відмовляє ВУЗОЛ (коротке анод↔катод), тож поруч із ціною стоїть незакрита валідація, а не запас і **механообробка каналу Ø1.35 після друку** — отвір є первинним датумом деталі зі співвісністю/биттям 0.05, чого as-printed LPBF не тримає, а відношення глибини до діаметра високе. Ціну дає RFQ ([`00_07` BIZ.17](00_07_Action_Plan_Tracker)), не оцінка звідси. | $20.00–$25.00 |
...
> 🔴 **Node-total вище є НИЖНЬОЮ МЕЖЕЮ, і рахувати його повним не можна — назви неоцінених елементів стоять поіменно, бо саме цю суму читає зовнішній читач.** Поіменно: **buy-to-fly** DMLS (невикористаний порошок, підпори, вихід рециклінгу — §3 визнає це прямо) · **PEEK-лайнер каналу** (купована трубка: ID = дріт − 11.41 мкм · OD = ID + 0.30, ⚖️ 2026-09-18, [`01_01 §1.4`](01_01_Coaxial_Gyroid_Topology_and_PEEK)) · **механообробка каналу Ø1.35 після друку** (лайнер і механообробка — з lining-присуду 2026-09-10; номінал відкрито до Ø1.35 присудом про зазор 2026-09-11, [`00_07`](00_07_Action_Plan_Tracker) HW.34 — операція та сама, змінився лише розмір) · **PEEK-кільце ізоляції в площині площадки** (Ø ≥ 4.0 у зенковці навколо виходу каналу — нова PEEK-деталь, [`02_02 §1.2`](02_02_Blind_Mate_Pogo_Pin_Interface)) · **елемент закриття каналу шини на виході** ([`02_02 §3.1`](02_02_Blind_Mate_Pogo_Pin_Interface)) · **поз. 14–19 електроніки** ([`02_01 §3`](02_01_Hardware_Architecture_and_BOM), TOTAL їх свідомо не включає): OVP-кламп · індуктивності BQ25570 · тактування HSE32 · LSE · SMPS · RF-тракт — а рядок RF Deck досі стоїть на ціні з епохи модуля, тоді як чіп на платі з 2026-09-25 міняє саму позицію 1 — кільце й закриття ратифіковано вимогами ⚖️ 2026-09-18 ([`00_07` HW.34](00_07_Action_Plan_Tracker)), геометрія — після P/N піна HW.9. ⚠️ Це не «діапазон покриє»: діапазон $54.65–$69.65 успадкований від сплав-невизначеності bake-off, а не резерв під неоцінене. Ціни buy-to-fly, лайнера й механообробки каналу приходять із RFQ ([`00_07` BIZ.17](00_07_Action_Plan_Tracker)), кільце й закриття запиту ще не мають (геометрії немає), і доти будь-яке «повна собівартість вузла» про це число є перебільшенням.
...
| **Модуль пружності** | ~110 ГПа (сплав) | **≈10.9 ГПа осьово / ≈7.7 ГПа радіально** — виміряно voxel-FE, дім [`01_01 §5.2`](01_01_Coaxial_Gyroid_Topology_and_PEEK) |
| **Собівартість серії** | $50–$80 | **$15–$30** |

⚠️ **Рядок модуля НЕ читати як «імітує деревину» — саме так він і читався до 2026-09-12.** Ґратка зменшує РОЗРИВ із деревиною, а не закриває його: анкер навантажений поперечним модулем стовбура `E_R`/`E_T` ≈ 0.5–1.5 ГПа, тож розрив лишається ~5–15-кратним ([`01_01 §5.1`](01_01_Coaxial_Gyroid_Topology_and_PEEK)). Вигода тут вагова й вартісна, а вимоги «ізоеластичності» канон більше не тримає — ціль 13–15 ГПа знято ⚖️ 2026-09-17, лишилась виміряна характеристика ([`01_01 §5`](01_01_Coaxial_Gyroid_Topology_and_PEEK)).
...
|---|---|---|---|---|
| 1 | MCU & LoRa | STM32WLE5JC + 5 dBi (N, RAKARG18) | Ідентично | — |
...
| **Сумарний ІНФРАСТРУКТУРНИЙ OPEX на кластер** | **~$10–$15 / місяць** |
| Обслуговування катода — візит лісника раз на 5–7 років ([`01_04 §5.5 C`](01_04_CODIT_and_Xylemointegration)) | **межа ≤ ~$2.6–3.7 / міс** (виведено ↓) |
...
⚠️ **Але це НЕ «zero-maintenance», і до 2026-09-18 таблиця вище цієї статті не містила.** [`01_04 §5.5 C`](01_04_CODIT_and_Xylemointegration) робить **візит форестера раз на 5–7 років** (механічне зчищення приростаючої тканини навколо PEEK Radome) умовою 20-річної довговічності катода, а не аварійною дією; праця арбористів фігурує в цьому документі лише як **одноразовий CAPEX** монтажу ([`§2`](#-2-витратні-матеріали-та-інсталяція)). **Стаття заведена рядком вище 2026-09-18 — і саме як МЕЖА, не кошторис.** ⚖️ Захист B знято (founder 2026-09-17, [`01_04 §5.5`](01_04_CODIT_and_Xylemointegration)), тож каденція більше не залежить від refresh-циклу покриття: вона стоїть на геометрії A + візиті C, і канон дає їй число — раз на 5–7 років.

**Звідки межа.** Праця арбористів на монтажі — **$220 на 100 дерев** ([`§5.2`](#52-витратні-матеріали-та-інсталяція)); зчищення приростаючої тканини навколо радома легше за буріння з монтажем тих самих 100 вузлів, тож праця одного проходу приймається **не вищою** за неї. Розподілена на 60–84 місяці (5–7 років), вона дає **≤ $2.6–3.7 / міс на кластер**. ⚠️ **Що в цьому числі НЕ виміряно:** ставка лісника, дорога, кількість дерев на один прохід; до того ж виконавцем канон робить **штатного лісника організації** (Модель A, [`00_07`](00_07_Action_Plan_Tracker) HW.28), тобто справжня стаття — це час штату, а не контрактний тариф арбориста, і тариф тут служить лише стелею. Перший справжній вимір дає перший візит.

**Вплив на payback.** Крива §7.3 бере інфраструктурний OPEX $12 і цієї статті не містить; на baseline-сценарії ($0.30/SCC, дохід $130/міс) верхня межа зсуває payback з ≈58 до ≈59 місяців (6 805 / (130 − 15)). Тобто читати §7.3 як повну вартість володіння досі передчасно, але похибка від цієї статті — **одиниці місяців, не кратність**; replacements і батарея, які важать більше, живуть у [`§8a`](#-8a-replacement-opex-та-деградація-обладнання-biz7).
...
> **⚠️ SCC-rate модель (self-consistent, calibration-pending):** 10,000 growth_points = 1 SCC. Realistic — Variant C (`delta_t` ≈ 1.95 год — енергомодель з обраним радіо-фронтендом [`02_03 §9.6`](02_03_BQ25570_MPPT_Nano_Power); під CCM-кадр 30 Б каденс іде до ≈ 2.22 год): ~12.3 пакети/добу × ~12 stored GP = ~148 GP/добу → **~5.4 SCC/дерево/рік**. ⚠️ **Точка крихка, і це властивість моделі, не округлення:** вона стоїть за 46 с від сходинки wire-GP 12 → 10 (`scc_rate.rb variant_c_s=7074` дає 4.46, −17 %), а задекларовані зміщення ланцюга в обидва боки (TinyML-вікно · TCXO · струм TX · ККД buck, [`02_03 §9`](02_03_BQ25570_MPPT_Nano_Power)) кожне значно більші за цю відстань — тож «~5.4» є точкою моделі, а не межею чи обіцянкою (адверсарне ревʼю 2026-09-26). ⚖️ **Робочу точку перенесено founder 2026-09-26 (за рекомендацією).** Доти грошові моделі стояли на 1.77 год = 6372 с, порахованих без пост-TX RX-вікна (прошивка відкриває його щоциклу) і на струмі HP-підсилювача чужого чипа; ланцюг, який виконує прошивка, дав 7027 с — на +10.3 %, поза ±8 % невизначеності Δt. **Підстава:** число з ланцюга, який виконує прошивка, на непевному `P_gen` краще за оптимістичне на тому самому `P_gen`. **Ціна:** грошова лінія −32 % (~7.9 → ~5.4 SCC/дерево/рік) і `U(k=2)` 18 → 28 % у кожному клієнтському домі. **Найслабша ланка:** обидві точки стоять на `P_gen` 15 µW без джерела ([`02_03 §9.2`](02_03_BQ25570_MPPT_Nano_Power)), і його вимір (coin Stage 2) може зсунути точку сильніше за цей рух; RX-вікно ще й важіль — його справжні ворота ([`00_07`](00_07_Action_Plan_Tracker) FW.50) повернули б частину. Носій проти повторного тихого відставання — `tx_cadence_budget.rb --assert` (грошова точка ↔ енергомодель, ±8 %; ⚠️ ці ±8 % — сирий джитер Δt до EMA, не невизначеність точки, і через сходинку GP у грошах зелена смуга ширша, ніж виглядає — [`00_07`](00_07_Action_Plan_Tracker) ARCH.8). Magnitude = f(EBFC recharge `delta_t`), placeholder до bench [E.63]; фізична стеля Δt=600s ≈ 326. Модель+гейт: `tools/firmware/scc_rate.rb`. Арбітр [`05_03`](05_03_Tokenomics_SCC_and_SFC) `MAX_SUPPLY=1B ≈ 20M дерево-років` ⇒ 50 (у діапазоні [5.4, 326]). ⚠️ 1 TX/год (Δt=3600s) = energy-NEGATIVE без мітигацій ([`02_03 §9.5`](02_03_BQ25570_MPPT_Nano_Power)) — НЕ baseline. Стара «~1 SCC/тиждень / 44–52 SCC/рік» брала несумісні packets×GP (24 і 50 з різних `delta_t`) — superseded.
...
| Параметр | Значення |
|---|---|
| Packets / day (Variant C, Δt ≈ 1.95 год) | **~12.3** |
| Wire GP / packet (Variant C `delta_t`, m ≈ 0.03) | **~6** |
| Stored GP / packet (×2 backend upscale) | **~12** |
| Stored GP / день / дерево | **~148** |
| Днів до 1 SCC (10,000 GP) | **~68 днів** |
| **Realistic продуктивність (Variant C)** | **~5.4 SCC/дерево/рік** (calibration-pending [E.63]; точка моделі за 46 с від сходинки wire-GP 12 → 10, де 4.46 — §7.1; стеля Δt=600s ≈ 326) |
...
| 1 дерево / рік (~5.4 SCC) | **~2.7 кг CO₂** |
| Кластер 100 дерев / рік (~540 SCC) | **~0.27 tCO₂** |

> 🔴 **ШІСТЬ числових рядків вище — ECB-ера (кадр 16 Б), тобто те, що відвантажено сьогодні; мітка стоїть тут, бо врізка над таблицею її має, а сама таблиця не мала — і читач бере ХІД із таблиці.** Ерні: packets/day · wire GP · stored GP · GP/добу · днів до 1 SCC · realistic SCC/рік. Під CCM-кадром 30 Б каденс іде на ≈ 2.22 год, метаболічна `m` насичується на нулі ще до тієї точки, і в гілці гомеостазу wire GP сідає на підлогу `GP_HOMEO_MIN = 5`: **5.39 → 3.94 SCC/дерево/рік (−26.9 %)**, а похідні рядки CO₂ — 2.7 → **2.0 кг** і 0.27 → **0.2 tCO₂**.
> ⛔ **Від ери НЕ залежать три величини в цій же таблиці, і їх не чіпати:** «1 SCC = 0.5 кг CO₂» — **дзеркало** [`05_03`](05_03_Tokenomics_SCC_and_SFC) й on-chain `ProtocolParameters`, правити ТАМ (курс не є функцією каденсу) · фізична стеля ≈326 SCC при Δt = 600 с — `scc_rate.rb` друкує її однаковою в обох ерах · множник ×2 backend-upscale.
> ⚠️ **Прогін на CCM-точці обидві сусідні моделі відхиляють** (`scc_rate.rb variant_c_s=8006 --assert` → exit 1, `uncertainty_budget.rb delta_t_s=8006 --assert` → exit 1) — але це означає «точка поза self-consistency-вікном моделі», **а не «число хибне»**: 3.94 реальне. Вимір і важелі розвилки (підйом `DELTA_T_SLOW_S` відхилено, SF — доданий) — [`00_07`](00_07_Action_Plan_Tracker) ARCH.8 (там же ⛔ «не відвантажувати CCM», доки не зміряно `P_gen`, link budget на SF8 і самопрозряд EDLC).
...
**Baseline: 1 SCC/тиждень/дерево** ⚠️ (**optimistic** — realistic Variant C ~5.4 SCC/рік (§7.1) дає на порядок менший дохід, а з OPEX кластер на цій точці не окупається взагалі (§7.3); economics жорстко gated на bench EBFC recharge `delta_t` — числа нижче тримати як стелю-сценарій до калібрування)**, ціна $0.30/SCC, baseline OPEX $12 (§6, без replacements):**
...
>
> ⚠️ **Стеля-сценарій:** 433 SCC/міс бере **optimistic** ~52 SCC/дерево/рік; realistic Variant C = **~5.4 SCC/рік** (§7.1) → дохід ×~0.10, тобто ≈ $13.5/міс при $0.30 — майже рівно baseline OPEX $12, і кластер на цій точці не окупається взагалі. ⛔ Не рахувати payback realistic-точки без OPEX: так виходить «×~10», що читається як довга окупність, а не як її відсутність. Уся крива нижче = optimistic-стеля до bench-калібрування EBFC recharge `delta_t` [E.63]; НЕ committed-число.
...
> **Стратегія виходу на прибутковість:** при $0.30/SCC кластер окупається ~4.8 року (baseline) / ~5 років (optimistic виручка + realistic OPEX з replacements, §8a.4; на realistic Variant C не окупається — §7.3). Цільовий ReFi ринок ($1–5/SCC) скорочує Payback до 3–16 місяців. Scale 50K+ (CAPEX $5,405) скорочує ще ~20%. Ключовий KPI: **growth_points/day** — прямий індикатор здоров'я лісу та швидкості монетизації.

---

## 🏗️ 8. Виробничі хаби в Україні (Supply Chain)

Проєкт спирається на локальні центри адитивних технологій для виготовлення найскладнішої частини — титанових Zone 1 та Zone 3 тризонного анкера (SLM+HIP; Zone 3 — також CNC із прутка, [`01_02 §1.7`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)), плюс PEEK CNC-фрезерування Zone 2.
...
2. **ALT Ukraine (ТОВ «Адитивні лазерні технології України») — місто виробництва НЕ звірено:** власне виробництво принтерів Alfa-280 та лабораторія матеріалознавства. Резервний хаб. ⚠️ Юрадреса за сайтом вендора — Дніпро, за дзеркалом ЄДР (звірено 2026-09-25) — Одеса; адреси виробничого майданчика не називає жодне з двох джерел ([`00_07`](00_07_Action_Plan_Tracker) BIZ.17).
3. **Черкаси (місцева PCBA + збірка — виконавця НЕ підтверджено):** SMD-пайка (роботом), conformal coating Parylene C (CVD; Sylgard 184 повне заливання відхилено — глушить TinyML акустику, [`02_02 §3.4`](02_02_Blind_Mate_Pogo_Pin_Interface)), фінальна збірка (**«Marriage»**) плат із титановими анкерами та PEEK-радомами. ⚠️ **Названий тут доти SVS-ARTA цих послуг не заявляє** (desk 2026-09-24: публічний перелік послуг — будівництво й валідація чистих приміщень, вентиляція, 3D-сканування й 3D-друк; ні PCBA, ні CVD-камери), а UA-носія Parylene не знайшли два проходи — адресати покриття зараз EU ([`ua_vendor_map`](protocols/procurement/ua_vendor_map.md) §5). Роль хабу — вимога, не підрядник ([`00_07`](00_07_Action_Plan_Tracker) HW.11).
...
3. Передати **PicoGK CAD-STL** (`tools/cad` `build`; AM-бюро друкують зі STL, **STEP не потрібен**) + factory spec (HW.1, HW.2) кожному кандидату. 🔴 **STL ПЕРЕБУДОВУВАТИ в день відправки, ніколи не брати готові з `out/`:** та тека лежить ПОЗА кожним нашим гейтом і не падає — вона тихо бреше (скіл `picogk`), а зварна гілка 2026-09-18 ([`00_07`](00_07_Action_Plan_Tracker) HW.1) зняла друковане осердя з анода, тож будь-який доперевимірний файл описує тіло, якого завод не отримає. ⛔ **Радом у цей крок НЕ входить:** його заводського креслення немає за **ратифікованим відкладенням** — геометрія несе два незастосовані присуди, і аркуш, виданий сьогодні, надрукував би відкинуту півсферичну корону ([`00_07`](00_07_Action_Plan_Tracker) HW.1 · HW.33).
...
DMLS-завод (Варшава / Льовен) [EU Backup, BIZ.6]
  │  Анкери (партія)
  ▼
PCBA + Збірка (Черкаси — виконавця не підтверджено)
...
| Electronics random failure (component-level) | ~5% | MTBF STM32WLE5 > 1M годин |
...
- Кластер 100 дерев: ~433 SCC/місяць (як §7.2 — стеля-сценарій optimistic; на realistic Variant C кластер не окупається, §7.3)
