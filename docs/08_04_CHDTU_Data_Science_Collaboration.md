...
- **Відкрите:** формальна співпраця ЧДТУ (Data Science, actuarial) — не розпочато → [`00_07`](00_07_Action_Plan_Tracker) (UNI.*).
...
### 1.4. Доц. Юрій Володимирович Хоменко — Прецизійна Механіка та DMLS Post-Processing

**Кафедра:** Металорізальних верстатів та комп'ютеризованих технологій, ЧДТУ
**Науковий ступінь, учене звання:** Кандидат технічних наук, доцент.
**Звання:** Заслужений винахідник України.
**Бібліометрія:** Понад 80 патентів України з прецизійної обробки, вібраційних
процесів у механіці та розробки спеціальних різальних інструментів.
**Спеціалізація:** Розробка нестандартних різальних інструментів,
вібродіагностика процесів обробки, оптимізація геометрії інструментальних
вузлів для специфічних матеріалів та форм.

**Чому Хоменко — критичний партнер (комплементарність до §1.1-§1.3):**

Існуючі партнери ЧДТУ покривають дані (Карапетян §1.1), радіо (Гончаров §1.2)
та акустику (Базіло/Бондаренко §1.3). Хоменко закриває останню непокриту
вертикаль — **прецизійну механіку та обробку матеріалів**. Це найслабша
ланка зараз для масштабування з R&D-партії на серію.

**Точка перетину зі Silken Net:**

1. **DMLS post-processing для Ti-6Al-4V гіроїда ([`01_01`](01_01_Coaxial_Gyroid_Topology_and_PEEK)):**
   Після DMLS-друку гіроїд має шорстку поверхню (Ra ≥ 10 μm) та внутрішні
   напруження. Хоменко може спроєктувати **спеціальний інструмент для
   фінішної механічної обробки** внутрішніх каналів гіроїда (де класичні
   фрези не проходять через геометрію) для досягнення Ra ≤ 2.5 μm на
   біоелектродних зонах.
2. **Оптимізація різьби різання в живій деревині ([`02_02 §Installation`](02_02_Blind_Mate_Pogo_Pin_Interface)):**
   Анкер не "вкручується" просто — він повинен різати ксилему мінімально
   травматично. Хоменко може порадити **кут заточки + крок різьби** для
   мінімізації механічного шоку та запобігання callus over-formation
   (CODIT, [`01_04`](01_04_CODIT_and_Xylemointegration)).
3. **Field Kit інструментарій (deinstall) — cross-ref [`08_05 §1.5 Несен`](08_05_CHIPB_Fire_Safety_Integration):**
   Несен у ЧІПБ працює над динамометричним інструментом для нетравматичної
   деінсталяції анкера. Хоменко = міст у CHDTU machine shop для прототипування
   цього інструменту в металі.
4. **Серійне виробництво (TRL 7→8, deferred):** Коли проєкт перейде з 50
   DMLS-анкерів на 50K, потрібен перехід на **лиття за моделями, що
   випалюються (investment casting)** або багатоосьове фрезерування —
   це експертиза кафедри металорізальних верстатів.

**Конкретний запит (мінімалістичний):**
- Розрахунок та прототипування різальної геометрії гіроїдного анкера
  (кут заточки, крок різьби, мікроморфологія шорсткості).
- Спільне керівництво студентськими роботами з DMLS post-processing.
- Патентний аудит дизайну анкера (cross-ref [`08_03`](08_03_Joint_Publications_and_IP_Strategy))
  — людина з 80 патентами здатна "простукати" нашу конструкцію на
  патентну чистоту і знайти унікальні вузли для окремого захисту.

**Чим Silken Net корисний Хоменку:**
- Реалізація "в титані" — DMLS Ti-6Al-4V як платформа для нових патентів.
- Вихід на міжнародний рівень публікацій / патентів у DeepTech-середовищі.
...
**Status:** 🔵 Identified — outreach у [`00_07`](00_07_Action_Plan_Tracker)
як UNI-track ЧДТУ.
...
4. **Git-based Collaboration:** ЧДТУ R-скрипти → `lib/analytics/r/`, ФОТІУС Python/C → `lib/analytics/python/`, ФЕТР RF-вимірювальні дані (S11, path loss, EMC) → `lib/measurements/rf/`, ПМКТ акустичні калібрувальні дані → `lib/datasets/acoustic_training/`. Всі інтегруються через CI/CD ([`00_05`](00_05_GitHub_Projects_and_IaC_Automation)).
