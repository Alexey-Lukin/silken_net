...
- **Поточний TRL:** TRL 5 — backup-конфіг IaC присутній і ввімкнений (Cloud SQL PITR + deletion_protection; **HA — `REGIONAL` у дефолті, чинний деплой `ZONAL` оверрайдом**, §нижче); §5.1 + половина §5.2 прогнані в drill 2026-09-02 (RTO 11:00 на staging — §6), §5.7 — повним циклом 2026-09-03 (RPO 0, RTO ≈ 16 хв). 🔑 **Єдина ЖИВА підстава TRL 5 тут — постура `ZONAL` при дефолті `REGIONAL`, і вона гейтована НЕ деплоєм production, а «першим живим аплінком»** (`terraform/posture.auto.tfvars`, §2 нижче). ⚖️ Зчеплення СВІДОМЕ й правильне: REGIONAL HA захищає дані, яких ще не збирають. ⚠️ Наслідок вголос: TRL цієї сторінки вимірює не власну зрілість, а СИСТЕМНИЙ TRL — «застрягання» тут є чесністю, не боргом.
- **Відкрите:** квартальний ритм drill (наступний — §5.2 у повній формі) → [`00_07`](00_07_Action_Plan_Tracker) DR.1. ✅ Master-key backup **повний** із 2026-09-03: `PROVISIONING_MASTER_KEY` + AR-трійка + `SECRET_KEY_BASE` — офлайн-копії у vault; `RAILS_MASTER_KEY` незамінним не є (vault тримає лише `secret_key_base`, у деплой ключ не їде).
