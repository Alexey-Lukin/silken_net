...
- **Поточний TRL:** TRL 5 — backup-конфіг IaC присутній і ввімкнений (Cloud SQL PITR + deletion_protection; **HA — `REGIONAL` у дефолті, але чинний деплой `ZONAL` оверрайдом**, §нижче), §5.1 + половина §5.2 прогнані в першому drill 2026-09-02 (RTO 11:00 на staging — §6), §5.7 — повним циклом у другому 2026-09-03 (RPO 0, RTO ≈ 16 хв). ✅ **Master-key backup БІЛЬШЕ НЕ відкритий — офлайн-копію `SECRET_KEY_BASE` збережено founder 2026-09-03** ([`00_07`](00_07_Action_Plan_Tracker) DR.1); рядок називав його операційною задачею ще два дні по тому, тобто пережив власну причину. 🔑 **Отже єдина ЖИВА підстава TRL 5 тут — постура `ZONAL` при дефолті `REGIONAL`, і вона гейтована НЕ деплоєм production, а «першим живим аплінком»** (`terraform/posture.auto.tfvars`, §2 нижче). ⚖️ Це зчеплення СВІДОМЕ й правильне: REGIONAL HA захищає дані, яких ще не збирають, тобто до першого вузла це витрата на охорону порожньої бази. ⚠️ Наслідок, який варто назвати вголос: TRL цієї сторінки вимірює не її власну зрілість, а СИСТЕМНИЙ TRL — вона підніметься разом із анкером у дереві, не раніше, і «застрягання» тут є чесністю, а не боргом.
