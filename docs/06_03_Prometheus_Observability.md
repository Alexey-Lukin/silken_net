...
| Prometheus scrape config | `deploy/akash/config.alloy` | ✅ 3 таргети `web:80`/`job:9394`/`coap:9395` (лейбл `process`; реєстр in-process — §2.9), 15s, Basic Auth |
...
| 5 | **`up` scrape-health alert** | `prometheus.scrape` авто-емітить `up{job="silken_net_scraper"}`; алерт на `==0` = таргет впав → метрики «осліпли». ⚠️ таргетів тепер ТРИ — вираз без `instance`/`process` скаже лише «хтось із трьох мертвий» | ops |
