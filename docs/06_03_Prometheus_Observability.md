...
| `config.rails.structured_logging.enabled` | `false` | ⚖️ [OPS.22, founder 2026-09-05] 7.0 увімкнув за замовчуванням ЛОГИ (метрики — ще з 6.3.0, активні з першим викликом `Sentry.metrics`; доказ із коду — коментар `config/initializers/sentry.rb`) і ПРИБРАВ обидва вимикачі `enable_logs`/`enable_metrics` (запис у них тепер — `NoMethodError` на буті), тож постава ОГОЛОШЕНА, а не успадкована. Цей рядок гасить категорію, яку 7.0 шле без запиту — лог на кожен запит |
...
> **[ARCH.61]** Actioning DeadSet-алертів: **`/sidekiq`** (Sidekiq::Web, змонтований за **`super_admin`-only** route-constraint + SEC.16 salt-stamp — ⚠️ звужено ⚖️ 2026-09-05 із `admin_or_above?`, бо вкладки Retries/Dead несуть `tree_id`/`wallet_id`/DID і суми ВСІХ орендарів, тобто клієнтський адміністратор діставав вікно повз тенант-ізоляцію (⚖️ founder; підстава — коментар над `mount` у `config/routes.rb`); app-constraint = єдиний шлюз — HAProxy path-ACL немає; unmatched → 404). Механіка constraint — [`04_03 §1`](04_03_REST_API_v1_Reference).
...
> 🏷️ **Цей блок і наступний (`📊` · `🎚️`) власного НУМЕРОВАНОГО заголовка не мають, а корпус адресує їх міткою §2.8 — так їх називає й реєстр домів [`00_06 §2`](00_06_SSOT_Documentation_Standard).** ⚠️ Тому читач, що прийшов по реєстр метрик і побачив під §2.8 заголовок про Circuit-Breaker, дивиться НЕ на хибний реф: мітка ширша за нумеровану секцію, і продовження — нижче. ⛔ Не «лагодити» цитувальників поодинці: їх десятки, вони узгоджені з реєстром домів, і масова заміна зламала б саме ту адресу, яку [`00_06`](00_06_SSOT_Documentation_Standard) оголошує канонічною.
...
| `silkennet_blockchain_tx_reverted_total` | алертна | `direction`, `token_type` | EVM money txs (mint/burn) whose receipt reverted on-chain (SLASH-1) — один на квитанцію; для слешу revert лишає договір `:breached` без спалення, машинного повтору свідомо немає, будить людину `sn-alert-money-tx-reverted` (рецепт [`06_08 §4.6`](06_08_Resilience_and_Failover_Policy)) |
...
| `silkennet_treasury_monitor_timestamp_seconds` | алертна | — | **[S2.4, 2026-09-06]** Unix-час ЗАВЕРШЕНОГО проходу `Treasury::MonitorService` — свідок свіжості **восьми** алертованих ґейджів (7× money-path + `mint_volume_window_scc`). 🔴 Без нього ті вісім замерзали на останньому значенні, а їхні власні правила читали замерзле число як здорове. ⛔ **Ставиться ЛИШЕ коли ОБИДВА ковтальні підпроходи, що пишуть алертовані ґейджі, доповіли завершення** (`update_money_path_metrics` · `detect_mint_volume_anomaly!` — кожен має власний `rescue`, що СВІДОМО не валить monitor-цикл): штамп у кінці `perform` без цього гарду свідчив би «воркер стартував», а не «прохід дійшов до кінця», і брехав би переконливіше за відсутність. ⚠️ **Оголошена стеля, і межа проведена ОДНИМ питанням — «чи нога пише ґейдж, на який дивиться правило»:** поза гардом свідомо лишились `update_payout_float_metrics` (у її `payout_float_balance` алерту НЕМА — діагностичний ярус) і `update_metrics` (девʼятий ґейдж `oracle_balance_ratio` озвучує `silkennet_treasury_check_errors_total`, INF.26). Втягнути будь-яку означало б платити другим свідченням за вже засвідчене. ⛔ **І правило НЕ сміє нести `> 0`:** безлейблову серію `prometheus-client` реєструє зі значенням 0.0 від старту процесу (виміряно 2026-09-06), тож фільтр зробив би алерт сліпим рівно до «воркер не біг ЖОДНОГО разу після рестарту». Фільтр виправданий лише там, де каденс писача ДОВШИЙ за вікно `for` (добовий `partition_sample_timestamp_seconds` — так; `*/5` і `*/15` — ні, і з `tree_sweep` його знято тим самим виміром). 📐 **І нульова серія тут РЕАЛЬНА, не артефакт:** реєстр у кожного процесу свій, а штамп пише job — тож `{process="web"}` тримає 0 від бута назавжди (виміряно на живому canopy поруч зі здоровим job-штампом). ⛔ Не «лагодь» цей нуль фільтром: його роботу вже робить `max by (slot)`, яке бере справжній штамп слоту (виміряно: застій 67 с, правило тихе), а коли job помирає — лишається саме той нуль, і правило нарешті ГОВОРИТЬ. `max` дає і тишу в нормі, і голос у дефекті; `> 0` забирав друге, не додаючи першого. Правило — `sn-alert-treasury-monitor-stale` (поріг 2 год із ПРЕДМЕТА: `limbo_locked_total` рахує кошти, застряглі понад годину; ⚠️ впаде до 1 год, щойно увімкнуть ARCH.62-детектор — `MINT_CIRCUIT_TTL` рівно година, і мертвий монітор ТИХО роззброює circuit-breaker) |
| `silkennet_trees_silent` | алертна | — | Current number of active trees silent beyond the silence threshold (set on each staleness sweep). ⚠️ **Свідок свіжості — рядок нижче:** без нього застигле число читалось би як «тиші немає» |
| `silkennet_tree_sweep_timestamp_seconds` | алертна | — | **[S2.4 · SILENCE-1, 2026-09-05]** Unix-час ЗАВЕРШЕНОГО проходу tree-staleness — свідок свіжості гейджа вище. 🔴 Без нього в dead-man switch не було ВЛАСНОГО dead-man'а: помер `TreeStalenessSweepWorker` → `trees_silent` замерзає → `sn-alert-trees-silent` мовчить у зелене, тобто механізм, що ловить тишу, сам стає тихим. ⛔ Штамп ставиться ПІСЛЯ обох операцій проходу, не на старті: інакше падіння на середині лишало б свіжий штамп над застиглим числом і брехало б переконливіше за відсутність. Форма — та сама, що `silkennet_partition_sample_timestamp_seconds` (ратифікована ARCH.70), і взята свідомо замість `noDataState: NoData` (⚖️ У-ВЕЙ 09-05) |
**Histograms:**
...
- **Cardinality budget** (`config.alloy`, `prometheus.relabel`) — `labeldrop` per-identity міток (`did`/`tree_id`/`peaq_did`/`wallet_address`/`tx_hash`) перед `remote_write`. Реєстр (§2.8) свідомо bounded — єдина growth-вісь `cluster_id` **лишається** легітимною; guard не дає випадковій майбутній per-DID мітці тихо підірвати active-series біллінг (Grafana Cloud біллить за series/DPM). Нульовий ефект сьогодні (таких міток нема) — стоячий запобіжник на write-boundary. 🔴 **Власник осі — `silkennet_cluster_tree_count_drift`** (SLASH-1, знаменник тригера слешингу), і назвати його тут обов'язково: ⛔ per-forest entropy, якою цю вісь виправдовували, більше не існує — увесь тракт знято 2026-09-05 (§2.6). Вісь жива, її НАЗВАНИЙ привід мертвий, і жоден гейт на це не червоніє, бо обидва твердження валідні поодинці.
...
- 🔑 **Кому потрібен свідок свіжості — вирішує ФОРМА `.set`, а не крон писача** [S2.4, класифікація 2026-09-06]. Питання «чи ряд МУСИТЬ існувати завжди» звучить як питання про каденс воркера, але каденс на нього не відповідає: `.set` буває за гардом. `silkennet_cluster_tree_count_drift` пише `ClusterHealthCheckWorker` щодоби — і все ж ряду в стеку НЕМА жодного за 7 діб, бо запис стоїть за `return if cached == live`: ґейдж ПОДІЄВИЙ, його відсутність є ЗДОРОВʼЯМ, і сторож свіжості над ним був би правилом над порожньою множиною. ⛔ **І виміряти це `count_over_time`-ом НЕ можна** — той рахує СКРЕЙПИ, а не записи: Prometheus-ґейдж, раз записаний, віддається на кожному скрейпі, доки живий процес, тож мертвий писач дає БЕЗПЕРЕРВНИЙ ряд замороженим числом. Ряд свідчить рівно в один бік (нуль семплів ⇒ писач не спрацював жодного разу); решту читай у коді. 🔴 **І смертей у писача ДВІ, з різним ліком:** (А) метод кинув, `rescue` ковтнув — лік є ГОЛОС у самому `rescue` (взірець `Treasury::MonitorService#check_balance` → `TREASURY_CHECK_ERRORS_TOTAL` + `sn-alert-treasury-check-errors`), ⚠️ але він працює лише там, де мітки лічильника описують предмет: DB-прохід підписанта не має, і втискати його під `signer` = брехня форми; (Б) воркер не біг узагалі — жоден `rescue` не спрацює, і `noDataState: NoData` тут НЕ рятує, бо голодування черги лишає ряд ЗАМЕРЗЛИМ, а не порожнім (зникає він лише на рестарті процесу). 🔑 **Штамп `*_timestamp_seconds` після ПОВНОГО проходу накриває обидві смерті одним приладом** — він не просувається ні коли метод кинув, ні коли воркер не біг; саме тому «після», а не «на старті» (взірці `silkennet_partition_sample_timestamp_seconds` ARCH.70 · `silkennet_tree_sweep_timestamp_seconds` SILENCE-1). 📐 **Периметр рахуй ПИСАЧАМИ, не ґейджами:** алертовані ґейджі без сторожа колапсують у чотирьох писачів, і `Treasury::MonitorService` несе девʼять із них проти одного в кожного з решти трьох — один штамп там коштує стільки ж, скільки будь-який інший, а накриває вдевʼятеро більше. ⛔ **Загального числа тут навмисно немає:** воно є похідним від переліку нижче й від `deploy/grafana/alerts/silkennet-alerts.yaml`, тож перевіряється командою (`grep -cE "silkennet_<ґейдж>([^a-z_]|$)"` по кожному імені), а не памʼяттю — і саме воно тут одного разу розійшлося з власним переліком у ТОМУ Ж абзаці. Розгортання лишається подієвим ([`00_07`](00_07_Action_Plan_Tracker) S2.4), не масовим. 📋 **Стан периметра станом на 2026-09-06:** штамп має `Treasury::MonitorService` (девʼять ґейджів; писався першим саме за важелем). Лишаються ТРИ писачі, по одному алертованому ґейджу кожен — `GatewayStalenessSweepWorker` (`gateways_faulty`) · `HadronKycReverifyWorker` (`hadron_kyc_pending_depth`) · `MintStallProbeWorker` (`mint_eligible_unminted_depth`). ⚠️ Серед них НЕ рівні за ризиком: писач `mint_eligible_unminted_depth` сидить у черзі `low`(9), ОСТАННІЙ у strict-priority, тож його голодування правдоподібніше за смерть двох інших — і саме його правило (`sn-alert-mint-stall-depth`, `noDataState: NoData`) накриває смерть ПРОЦЕСУ, не голодування ВОРКЕРА. Якщо колись робитимемо виняток зі строку — там.
...
| 4 | ✅ **Cardinality budget** (2026-06-04) — `prometheus.relabel` `labeldrop` per-identity (`config.alloy`) | `cluster_id` лишається (легітимна growth-вісь; власник — `silkennet_cluster_tree_count_drift`, SLASH-1 — ⛔ НЕ per-forest entropy: той тракт знято 2026-09-05, §2.6); per-DID labels (`did`/`tree_id`/`peaq_did`/`wallet_address`/`tx_hash`) дропаються до remote_write, щоб майбутня випадкова мітка не підірвала active-series біллінг (Grafana Cloud біллить за series / DPM) | ✅ DONE |
