...
> **[ARCH.61]** Actioning DeadSet-алертів: **`/sidekiq`** (Sidekiq::Web, змонтований за **`super_admin`-only** route-constraint + SEC.16 salt-stamp — ⚠️ звужено ⚖️ 2026-09-05 із `admin_or_above?`, бо вкладки Retries/Dead несуть `tree_id`/`wallet_id`/DID і суми ВСІХ орендарів, тобто клієнтський адміністратор діставав вікно повз тенант-ізоляцію ([`00_07`](00_07_Action_Plan_Tracker) S2.4); app-constraint = єдиний шлюз — HAProxy path-ACL немає; unmatched → 404). Механіка constraint — [`04_03 §1`](04_03_REST_API_v1_Reference).
