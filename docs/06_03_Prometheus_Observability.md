...
- **CI-gate `alloy_config_validate`** (`.github/workflows/ci.yml` → `CI · Code`) — `grafana/alloy fmt` парсить `config.alloy`; **path-gated** під `alloy`-домен (`changes`-job: біжить коли чіпається `deploy/**`); River parse-error = **red CI замість crash-loop sidecar** на Akash-деплої.
