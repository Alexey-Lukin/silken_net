...
> **Ruthless Pruning (зворотний бік zero-content-loss).** «Zero-loss» стосується **реструктуризації** (перенесення *живого* факту в новий дім), а НЕ архівного накопичення. Спростовану гіпотезу чи застарілий після піву концепт (напр. п'єзо-тригер пробудження, LTC3108) **видаляють з активного канону**, а не «мігрують про всяк випадок» — Git тримає історію, цього досить. Мертві ідеї роздувають контекст для LLM-агентів → ризик галюцинацій «воскреслими» концептами. Якщо концепт має історичну/навчальну цінність — виносять у явно марковану **legacy-appendix** сторінку (взірець — [`02_06`](02_06_Legacy_Breadboard_Appendix), яку лінтери вже exempt-ять), а не лишають у активному дереві; за потреби виключити її з AI-контексту (`.cursorignore` / `.aiignore` — наразі відсутній, опційно). Повернення ретированої назви замикає `DocsLinter::DEPRECATED_TERMS` (§3).
