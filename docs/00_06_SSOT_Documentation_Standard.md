...
| **source line-ref drift** (DOC-T.15) | volatile `*.c:N`/`*.h:N` line-refs у `docs/**` + `.github/**` — файл росте, рядок указує на ХИБНИЙ зміст (FW.4-трійка цілила в `DEFAULT_TTL` замість `Run_Inference`). Цитуй стабільний symbol/`#define`/функцію, не номер рядка. Сканує docs+.github (НЕ source-дерева — там line-ref у коментарях легітимні; root `CLAUDE.md` поза скоупом); НЕ skip-fence (рефи живуть у `// path:N` всередині fenced-блоків). Прибрав 30 рефів + sync copilot blocker-table → CLAUDE.md §12 | `bin/rails docs:check_refs` (HARD 2026-06-10; `lib/docs_linter.rb`) |
