...
> ✅ **Епоха шипнута (2026-07-29, SEC.25 Ф3) — відкликання існує.** `organizations.stream_epoch` (integer, default 1, NOT NULL) входить в імʼя: `telemetry_stream_org_7_e1`. Важіль — `Organization#rotate_stream_epoch!`: запамʼятати попередню епоху → `increment!` → штовхнути **tombstone** (`broadcast_refresh_to`) у ПОКИНУТУ адресу → лишити слід ARCH.57 (`stream_epoch_rotated`). Порядок несучий: постав tombstone першим — і релоуд відрендерить ще стару епоху, тобто глядач осяде рівно на тій адресі, яку ми кидаємо. Tombstone теж несучий: без нього bump = тиха втрата живості для чесних, тобто той самий режим відмови, за який відкинуто TTL у (в). ✅ **Морф ротацію НЕ ламає, і це перевірено в джерелі, а не припущено** (2026-08-17): tombstone прилітає на дашборд-сторінки, де метатег вмикає морф, і напрошується страх «морф лишить старий вузол підписки, тож відкликання не доїде». Не лишить — `TurboCableStreamSourceElement` оголошує `observedAttributes = ["channel", "signed-stream-name"]`, а `attributeChangedCallback` при живій підписці робить `disconnectedCallback()` + `connectedCallback()` (`turbo.js:5423-5442`); спецкейсу, що виводив би цей елемент з-під морфу, у морф-колбеках немає, тож зміна імені сама перепідписує клієнта. ⛔ Піна на це свідомо НЕ пишемо — інваріант належить контракту гема, і тест червонів би на апгрейді Turbo, а не на нашій регресії; **натомість цю пару рядків перечитують при мажорному апгрейді.**
>
