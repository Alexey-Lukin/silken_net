...
> 🔴 **Частина ключів корпусу стоїть ПОЗА набором форм своєї мови — і це свідомо.** `lv` має
> рівно `one`/`other`, тож нулю дістається `other` («0 minūtes»), тоді як латиська граматика
> вимагає родового множини («0 minūšu»). Лік — ключ `zero:`, який
> `I18n::Backend::Pluralization#pluralize` бере **шорткатом ПЕРЕД правилом**
> (`return entry[:zero] if count == 0 && entry.has_key?(:zero)`); сам гем позначив цю гілку
> депрекованою (ruby-i18n#629), тож ключ живий, але не назавжди. Множину таких ключів беруть
> прогоном (`rg -n 'zero:' config/locales/`), ніколи з памʼяті — вона росте разом із каталогом.
>
> ⚠️ **`i18n.plural.keys` цього НЕ бачить за побудовою:** він віддає перелік ПРАВИЛА, а шорткат
> стоїть до правила. Тому «lv не має категорії `zero`» і «lv рендерить `zero`-лист» правдиві
> ОДНОЧАСНО — про різні обʼєкти; розводить їх лише рантайм на межах (0·1·2·5·11·21), ніколи
> перелік категорій. Підтримуваний наступник — явний CLDR-ключ `'0'`; міграцію відкладено,
> бо `i18n-tasks 1.1.2` його не знає й вимагав би в КОЖНІЙ локалі каталогу, тобто ціна росла б
> із каталогом, а не з попитом (§8.1а). Стан → [`00_07`](00_07_Action_Plan_Tracker) I18N.1;
> носій обох подій — `spec/i18n/pluralization_gate_spec.rb`.
