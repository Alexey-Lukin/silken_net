...
| голий `Organization` | ⚠️ **поки ніхто** | `ResetActuatorStateWorker` · `ActuatorCommandWorker.broadcast_command_state_static` — обидва в `command_status_{id}`; ЄДИНИЙ тракт, який вирішено дотягнути ([`00_07`](00_07_Action_Plan_Tracker) UI.4) |
...
>
> ⚠️ Лишається **один** тракт у порожнечу — `command_status_{id}` — свідомо, бо його вирішено дотягнути. Стан на 2026-07-27: **ціль тепер РЕАЛЬНО існує в DOM** (`Actuators::Show` рендерить `CommandStatusBadge` замість власного рукописного `<span>`; заразом зник другий, розійдений рендерер того самого стану — сторінка малювала `border-*`, компонент `bg-*`). Бракує підписки: жодна сторінка актуаторів не має `turbo_stream_from`, а продюсери шлють у голий `organization`. Дротування йде в парі з міграцією payload'а (§8.1а), бо бейдж несе `t()`.
>
> 🔴 **Перед вмиканням підписки — fail-шляхи.** Живі місця, що валять команду (`Downlink::PendingQueueService` TTL/oversized, `ActuatorCommand` override-cancel і pre-dispatch failures), броадкасту не роблять узагалі; єдині, що роблять, сидять у мертвому push-тракті. Дротування без них дасть бейдж, який застигає на «виконується» для провалених команд — **живість, що бреше, гірша за чесну статику**.
>
...
> ✅ **Цілей без сторінки більше немає** (2026-07-27). Остання — `command_status_{id}` (`Actuators::CommandStatusBadge`, продюсери `ResetActuatorStateWorker` + `ActuatorCommandWorker.broadcast_command_state_static`) — тепер рендериться на `Actuators::Show`. Решту пʼять знято раніше того ж дня разом із їхніми продюсерами. **Але тракт ще не живий:** підписки немає ніде, тож броадкаст і далі йде в порожнечу — просто вже з іншої причини (не «нікуди», а «нікому»). Це два різні дефекти, і другий закривається в парі з §8.1а → [`00_07`](00_07_Action_Plan_Tracker) UI.4.
>
