...
> | `reveal` (fade-up, **одноразовий**) | **0** | повернув би `opacity-0` НАЗАВЖДИ | ⚠️ міна на запобіжнику — застереження в шапці контролера |
>
...
>
> 🔴 **Найцінніше тут — `reveal`, і саме тому, що він НЕ спрацьовує: ланцюг доведений, пускача нуль.** Одноразовість (`unobserve` після першого показу) перетворює косметичний відкат на **постійну** втрату: вузол переживає морф → Stimulus не переграється → `class` вертає `opacity-0` → показати вже нікому. **Правило, ширше за цей контролер: клієнтський стан, який ставиться ОДИН раз і не вміє поставитись удруге, несумісний із морфом за побудовою** — питай не «чи відкотиться», а «чи є хто його поверне». Застереження живе в шапці самого контролера, бо його прочитає той, хто дротуватиме.
