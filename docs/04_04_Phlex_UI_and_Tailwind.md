...
🔒 **Стеля названа:** узгодження РОДУ всередині самого повідомлення лишається за `rails-i18n` — для `uk` він дає одну форму `errors.messages.blank` («не може бути пустим»), яка не узгоджується з жіночими підметами. Цей фікс зробив розходження ВИДИМИМ (доти підмет був англійський і ні з чим не узгоджувався), але не створив його; дім рішення → [`00_07`](00_07_Action_Plan_Tracker) I18N.1.
