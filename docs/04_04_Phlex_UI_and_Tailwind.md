...
>
> 🔒 **Той самий інваріант діє на ДІЇ всередині сторінки, не лише на меню [UI.6].** Фільтр сайдбара закрив вхід, але гарди з `only:` сидять ГЛИБШЕ за дію, якою пункт відкривається: сторінка видима легально, а кнопка на ній веде в 403. Свіп дав **клас із семи місць у чотирьох компонентах**, не три випадки. Закрито чотири роль-предикатні: `TreeFamilies::Index` ×2 (найгостріше — блобило **адміну**, бо мутації там super_admin-only), `Alerts::Row` (найбуденніше — бойову кнопку бачив кожен investor), `OracleVisions::Index` (цілий пульт симуляції показувався форестеру). Три точки `Maintenance::Show` лишено свідомо: гард там **запис-залежний** (автор-або-admin), під `min_role`-форму не лягає, а посадка вже мʼяка — HTML-redirect із flash, а не JSON-блоб.
>
