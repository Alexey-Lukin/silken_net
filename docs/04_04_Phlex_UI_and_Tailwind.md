...
| `"blockchain_ledger_org_{id}_e{epoch}"` | `BlockchainTransactions::Index` | `BlockchainTransaction#broadcast_ledger_signal` — **сигнал** (§8.1б): сторінка має фільтри, пагінацію **й дефолтне вікно по `created_at`**, тож `prepend` суперечив би всім трьом |
