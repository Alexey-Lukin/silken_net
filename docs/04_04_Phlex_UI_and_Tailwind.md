...
| `"ews_alerts_org_{id}"` | `Alerts::Index` | `EwsAlert#broadcast_alert_update` (рядок `Alerts::Row` у `dom_id`) · `#broadcast_new_alert` (**сигнал** — сторінка має фільтри й пагінацію) |
...
>
> ✅ **Ціна сигналу нижча, ніж здається, бо він закриває чотири речі одним відʼємним діфом:** форма лишається у власника · семантика «прибрати + підтягнути наступну» стає безкоштовною (сторінка переграє власний запит) · рендер їде в локалі **глядача**, тож поверхня зникає з міграції I18N.2 сама · спец-кейс `citations: []` випаровується разом із push'ем.
>
> ⚠️ **Умова придатності — morph.** `broadcast_refresh_*_to` без `<meta name="turbo-refresh-method" content="morph">` дає повний reload зі скидом скролу; при 5-секундному тротлі на потоці тривог це гірше за дефект, який лікуємо. Метатеги стоять у `DashboardLayout`. Зворотний бік: morph зберігає DOM-вузли, тож усе, чий стан **будує клієнт, а сервер не віддає**, мусить нести `data-turbo-permanent` — інакше morph вирізає його й не викликає `connect()`, який міг би відновити. Живий приклад — полотно Leaflet (`Dashboard::Map`).
