...
- **Кожен екран, до якого тумблер дотягується, мусить працювати в обох темах.** Сира палітрова утиліта на поверхні чи тексті — дефект теми, а не естетичний вибір (§3.5).
- **Екран має право бути свідомо однотемним, лише якщо він це ОГОЛОШУЄ** — хардкодом теми й відсутністю тумблера, як `Views::Layouts::AuthLayout` (`html(class: "h-full dark")`). Тихо ламатись під тумблером — не «однотемність», а поломка.
...
**Тема перемикається РІВНО однією шафою — класом `.dark` на `<html>`** (ставить FOUC-скрипт у `dashboard_layout`, далі `theme_controller`, §7.1). Світла = відсутність класу. Механіка того, чому шафа саме одна, — §3.
