...
>
> ⚠️ Лишається **один** тракт у порожнечу — `command_status_{id}` — свідомо, бо його вирішено дотягнути: компонент уже рендерить правильний id, а `Actuators::Show` малює статус рукописним `<span>` повз нього. Дротування йде в парі з міграцією payload'а (§8.1а), бо бейдж несе `t()`.
>
...
> ⚠️ **Ціль БЕЗ сторінки, що її рендерить, лишилась одна** — `command_status_{id}` (`Actuators::CommandStatusBadge`, продюсери `ResetActuatorStateWorker` + `ActuatorCommandWorker.broadcast_command_state_static`). Компонент рендерить рівно цей id; бракує двох речей — `Actuators::Show` малює статус рукописним `<span>` повз компонент, і жодна сторінка актуаторів не підписана. Дотягується в парі з §8.1а → [`00_07`](00_07_Action_Plan_Tracker) UI.4. Решту пʼять цілей знято 2026-07-27 разом із їхніми продюсерами.
>
