...
| `AccountSecurity` | `Show`, `MfaSetup`, `RecoveryCodes` | `Show` — `user:`; `MfaSetup` — `user:` + `error:` (422 verify-коду на місці; QR — інлайн-SVG rqrcode з нашого ж `otp_provisioning_uri`, CSP-чистий); `RecoveryCodes` — `codes:` (одноразовий reveal recovery-набору після активації/ротації — session-маркер знімається контролером ДО рендеру, тож компонент бачить набір рівно раз) [S6.21] |
