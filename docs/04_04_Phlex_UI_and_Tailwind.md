...
| `Notifications` | `Settings` | `user:`, `available_channels:` (дефолт `[]` — платформений факт `DeliveryChannels`, а не властивість користувача). 🔴 **Доти рядок називав `settings:` — kwargʼа з таким іменем не існує** [DOC-T.79]; ⚠️ `user:` тут читає ВЛАСНІ поля адресата (`telegram_chat_id`/`push_token`), тобто це не актор-гейт UI.5/UI.6, і плутати ці дві ролі не можна |
