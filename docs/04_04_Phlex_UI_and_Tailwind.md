...
> **Module:** 04_04 · **TRL:** 8 → target 9 · **Epoch:** Cycle 1 · **Status:** Operational  
> Reverse-shaped from live codebase. Single Source of Truth for the Gaia 2.0 design system.  
> Stack: **Rails 8.1 · Phlex · Tailwind CSS 4 · TailwindMerge · Stimulus · Turbo 8**  
> ⚠️ TRL 9 requires resolution of BLOCKER-01 (Turbo Drive state loss) — see [Section 11](#11-blockers-needs-action).
...
11. [Blockers (Needs Action)](#11-blockers-needs-action)
...
> **Note:** The `map` controller currently has **no `disconnect()` method**. This means the Leaflet map instance is not explicitly destroyed on Turbo Drive navigation, which is a contributing factor to BLOCKER-01 (see [Section 11](#11-blockers-needs-action)).
...
## 11. Blockers (Needs Action)

### 🔴 BLOCKER-01: Hotwire/Turbo Drive State Loss — Stimulus Controllers

**Severity:** High  
**Status:** Known — Audit Required in Separate Cycle  
**Affected Modules:** Oracle Visions (Lorenz Attractor charts), Wallets (balance frames)

**Symptom:**  
During complex page transitions between the Lorenz Attractor visualizations (`/oracle_visions`) and Wallet balances (`/wallets/:id`) via Turbo Drive, Stimulus controllers occasionally lose their internal state. Specifically:

- The `map` controller's `markers` hash (DID → Leaflet Marker) may be reset mid-session, requiring a full re-render of all map pins.
- Lorenz simulation slider state (values set in `OracleVisions::SimulationPanel`) may revert to defaults when navigating away and back.
- The `matrix-rain` canvas controller may orphan its `setInterval` handle if Turbo Drive replaces the `<body>` without triggering `disconnect()` cleanly.

**Root Cause (Hypothesis):**  
Turbo Drive performs full `<body>` replacement on navigation. Elements without `data-turbo-permanent` are torn down and re-initialized. If a controller's `connect()` has heavy setup (Leaflet map initialization, canvas context), the teardown/reinit cycle may be incomplete or race with ongoing Turbo Stream broadcasts.

**Required Action (Next Cycle):**
1. Audit all pages for elements that should be `data-turbo-permanent` (sidebar, theme switcher, live telemetry header).
2. Implement `disconnect()` cleanup in `map_controller.js` (destroy Leaflet map instance: `this.map.remove()`).
3. Investigate whether `matrix-rain` interval is properly cleared on Turbo visits; add `this.interval && clearInterval(this.interval)` guard.
4. Consider `data-turbo-cache="false"` on pages with complex controller state if `data-turbo-permanent` is insufficient.

**No code changes in this cycle** — this blocker is documented here as a mandatory fix target for the next Shape Up cycle.

---

*Document auto-generated via Reverse Shaping (Cycle 1) from:*  
