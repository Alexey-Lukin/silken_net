...
| 🔴 `opacity-*` на ЗМІСТОВНОМУ тексті | **Заборонено — це прихований множник контрасту, якого не бачить ЖОДЕН із двох наших приладів** [UI.3, виміряно 2026-08-14]. Статичний гейт судить існування токена; браузерний збирач композитить стек ФОНІВ. Альфу самого переднього плану не питає ні той, ні той. Інстанс: головне число `OracleVisions::ForecastCard` під `opacity-40` дає **1.71–2.18:1** проти 4.71–6.98:1 тих самих кольорів при повній непрозорості. ⚠️ `group-hover:opacity-100` ліком не є: на тачі hover'а немає, і 1.4.3 винятку «читабельно при наведенні» не має. **Форма: згасання йде на ДЕКОРАТИВНОГО сусіда, значення лишається непрозорим** |
