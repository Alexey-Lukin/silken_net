...
- **Поточний TRL:** TRL 8 — рушій (Phlex + `@theme`-токени + `tokens()`) production-grade; **токен-міграцію ВИЧЕРПАНО 2026-08-20** (порція 10 — `UI.1`/`UI.2`/`UI.3` усі в §🗄️), а `gaia:lint_tokens` **enforced у CI з 2026-08-07, і його периметр із 08-20 = ВЕСЬ `components/` + `shared/` + `layouts/`** (дім переліку — `default_scopes` у `lib/tasks/gaia_lint.rake`, не тут: рядок-дзеркало старів би при кожній новій піддиректорії); 🔴 **а доменні компоненти §3.5 з 2026-08-07 більше НЕ дозволяє** — цей рядок пережив власну підставу й до 2026-08-15 казав «явно дозволяє, тож міграція косметична», тобто найчитаніша лінія доку суперечила §3.5 за двісті рядків нижче (клас «канон легалізував відхилення описом»). Сира поверхня чи сирий текст на доменній сторінці = **дефект теми**, не естетика. ⚠️ **Цей рядок пережив власну підставу ВДРУГЕ й у протилежний бік — виправлено 2026-08-27 (DOC-T.91):** доти він казав «периметр гейта звужений, правило ні» й «raw-Tailwind у прикладних компонентах лишається (міграція UI.1-3)», тоді як периметр давно розширено до всього дерева компонентів, а всі три UI-пункти архівовані — тобто носія, який мав би це зловити, більше не існувало. 🔑 **Клас варто тримати саме тут, бо він трапився двічі на ОДНОМУ рядку: найчитаніша лінія доку старіє в обидва боки — спершу дозволяючи те, що правило вже заборонило, потім занижуючи те, що гейт уже накриває.** Потребує production verification.
