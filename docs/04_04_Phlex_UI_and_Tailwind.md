...
| `AccountSecurity` | `Show`, `MfaSetup`, `RecoveryCodes` | `Show` — `user:` + `identities:` + `password_error:` + `erasure_error:` (два слоти помилок, а не один: відмова стирання мусить зʼявитись У СВОЇЙ формі — спільний слот показав би її над полями зміни пароля, тобто в секції, якої людина не чіпала [SEC.18]); `MfaSetup` — `user:` + `error:` (422 verify-коду на місці; QR — інлайн-SVG rqrcode з нашого ж `otp_provisioning_uri`, CSP-чистий); `RecoveryCodes` — `codes:` (одноразовий reveal recovery-набору після активації/ротації — session-маркер знімається контролером ДО рендеру, тож компонент бачить набір рівно раз) [S6.21] |
