...
|---|---|---|---|
| `DashboardLayout` | `app/views/layouts/dashboard_layout.rb` | `Phlex::Rails::Layout` | Основний layout з sidebar, top bar, breadcrumbs, **flash-регіонами** (↓). Топ-бар несе **індикатор робочого контексту** (`acting_organization:`, super_admin-only) — і це не оздоба: `organizations#switch` flash **не ставить**, тож індикатор лишається єдиним каналом підтвердження саме для перемикання [UI.6]. ⚠️ Тут доти стояло ще й «цей layout `flash` не рендерить **узагалі**» — правда до [SEC.25], знято 2026-08-01 разом із появою поверхні. Станів свідомо два («обрано X» / «не обрано»): дім формули розходження (`Current#switched_context?`) **амбієнтний**, а `Current` як джерело даних у в'ю заборонений його ж шапкою; третій стан має сенс лише для super_admin із домашньою організацією й додається одним рядком, коли такий зʼявиться. ⚠️ Назва організації інтерпольована в `aria-label` — інакше він замінив би доступне імʼя цілком і SR-користувач чув би «змінити організацію», ніколи — яку саме ([UI.3] у сайдбарі) |
...
🔴 **Обидва live-regions присутні в DOM ЗАВЖДИ, навіть порожні.** Це не стиль, а умова роботи скрінрідера: регіон, вставлений у DOM разом зі своїм вмістом, AT не оголошує — вона слухає зміни всередині вже відомого їй регіону. Категорій **наразі** дві (`notice` → `role="status"`/polite · `alert` → `role="alert"`/assertive), і вони не взаємозамінні: APG вимагає вживати assertive ощадливо, тож «успішно збережено» не сміє перебивати те, що людина читає.

⚖️ **Ратифіковано 2026-08-01 (ще не збудовано):** категорій буде **чотири** — `success` · `error` · `pending` · `security`, — але **регіонів лишається два**, бо регіон тримає a11y-контракт, а категорія — семантику й тон. Мапінг: `error` + `security` → assertive, `success` + `pending` → polite. Це заразом і форма закриття a11y-залишку: assertive-регіон AT озвучує й тоді, коли вузол приходить разом зі вмістом, тож усе, що МУСИТЬ бути почуте (відмова · зміна безпекового стану), потрапляє в надійний канал **без жодного JS**; ціна названа чесно — буденне «збережено» для незрячого може лишитись тихим. Тонів вигадувати не треба, усі чотири існують парами «фон+текст» в обох темах (`status-danger`/`warning`/`success`/`info`). ⚠️ Міграція **атомарна**: після перейменування компонент мовчки дропає невідому категорію, тож пропущений сайт = тихо зникле повідомлення. Стан і сторож → [`00_07`](00_07_Action_Plan_Tracker) SEC.25 Ф3.

⚠️ **`flash.*` — НЕ простір flash-повідомлень.** Із 45 ключів (у **двох** домах: `config/locales/flash/` + `config/locales/defaults/`) близько 13 живуть виключно в JSON-відповідях — актуатори, телеметрія, ланцюг. Не бери імʼя неймспейса за периметр роботи.
...
⚠️ **Морф і flash:** вузол, якого немає у свіжій відповіді, Idiomorph видаляє — але Turbo **перехоплює** це, підставляючи власний колбек (`beforeNodeRemoved = node => this.beforeNodeMorphed(node)` → cancelable `turbo:before-morph-element`, гард `instanceof Element`). Тобто утримати повідомлення через морф **можливо**, і для наших регіонів навіть простіше: вони рендеряться завжди, тож при морфі мають пару й подія приходить штатним шляхом. ⛔ **Але утримувати його ми свідомо НЕ будемо (won't-do 2026-08-01):** серед 43 flash-сайтів редиректів «на ту саму адресу» рівно три, і flash на morph-підписаній сторінці садять два рядки одного контролера. Гард ще й виробляв би БРЕХЛИВЕ підтвердження при роботі пачкою (утримана плашка першої дії лишалась би за другу) і осідав у кеші снапшотів, тобто «Назад» оголошував би дію десятихвилинної давнини. Повний розбір пʼятьох підстав → [`00_07`](00_07_Action_Plan_Tracker) SEC.25. Канал підтвердження на таких сторінках — **СТАН** (як `Alerts::Row` уже робить), не повідомлення.
...
> - **Індикатор і кнопка — одна зміна.** Кнопка без індикатора створює стан «дивлюсь чужі дані, і на екрані про це нічого». ⚠️ Тут доти стояло «а `DashboardLayout` `flash` не рендерить, тож іншого каналу підтвердження не існує» — знято 2026-08-01 разом із flash-поверхнею (§8). Індикатор лишається несучим із **іншої** причини: він показує стан **постійно**, тоді як flash живе один запит, — але `switch` і досі не ставить жодного повідомлення, тобто для скрінрідера перемикання тенанта не оголошується взагалі → [`00_07`](00_07_Action_Plan_Tracker) SEC.25.
>
...
>
> ⚙️ **Перемикання робочого контексту йде повним перезавантаженням (`data: { turbo: "false" }` на `form:`), і причина одна-єдина:** десктопний сайдбар позначений `data-turbo-permanent`, тобто переживає Turbo-візит **разом зі своїм org-скоупленим лічильником тривог** — без reload він показав би число попереднього тенанта. ⚠️ Поширена хибна підстава, яку тут уже писали: «інакше не оживуть вебсокети». Неправда — Turbo сам відкриває зʼєднання наново (`subscriptions.add` → `ensureActiveConnection`), тож на неї спиратись не можна. Носій атрибута — `form:`, як у `LocaleSwitcher` (`button_to` кладе решту опцій на `<button>`; сьогодні працюють обидва, бо Turbo перевіряє ще й submitter, але «той самий патерн» має бути тим самим носієм).
>
