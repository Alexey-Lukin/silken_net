...
>
> **Форма для запис-залежного гарда — предикат на МОДЕЛІ** (`MaintenanceRecord#mutable_by?`), а не `min_role:` і не політика: правило мусять читати троє (гард батька, гард вкладеного контролера, компонент), а Phlex тут Pundit не торкається ніде. Канонічний поділ ідіомів це дозволяє прямо — предикат відповідає за «роль × авторство», асоціативний скоуп за приналежність ([`04_03 §3`](04_03_REST_API_v1_Reference)), тож предикат про організацію свідомо мовчить, і це обовʼязок викликача. ⚠️ Три шари пінів не заміняють одне одного: формулу пінить model-спека, послух компонента предикату — компонентна, а те, що актор реально доїжджає, — **request**-спека (при fail-closed дефолті негативний приклад сліпий до забутої проводки).
>
