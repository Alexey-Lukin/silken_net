...
|---|---|---|
| `Alerts` | `Index`, `Row`, `Show` | `alert:` (`Badge` знято 2026-07-27 — UI без жодного рендерера); `Show` [ARCH.31] = table-обгортка Row (голий `<tr>` поза table браузер викидав) + SOP-панель: три операційні кроки (acknowledge → звірка незалежного свідка → Field-Audit) і чесний плейсхолдер доменної тактики партнера (UNI.12) |
| `Clusters` | `Grid`, `Item`, `Show` | `cluster:` · **`Show` бере ще `gateways:`, `recent_alerts:`, `active_contract:` і пару покриття `health_measured:`/`total:` — усі БЕЗ дефолту** [ARCH.84]: `nil`-дефолт на парі зробив би забуту проводку невідрізнимою від «виміряно повністю», бо `measurement_coverage` мовчить в обох випадках (той самий вибір, що `Gateways::Index#latest_logs`, PERF.1). ⚠️ `trees:` тут доти стояло помилково — `Show` дерев не приймає ЖОДНИХ; колонка props гейтом `component_doc_sync` **не покрита за його оголошеною стелею** («class names only, never props»), тож звіряється очима при кожній правці конструктора |
