...
| Компонент | Файл | Props | Опис |
|---|---|---|---|
| `Telemetry::LiveStream` | `telemetry/live_stream.rb` | `organization:` | Live telemetry HUD (декоративний canvas знято — §7.3), sticky `<thead>`, `turbo_stream_from TurboStreams::Name.org(:telemetry, org)` (org — це скоуп стріму, не декорація; епоха в імені = відкликання, §8.1) ⚠️ `tbody(id: FEED_TARGET)` рендериться **без капа рядків** — JS-контролера, що ріже стрічку, у дереві немає, і це свідомо: DOM-trim лікував би симптом продюсера, форма якого все одно міняється ([`00_07`](00_07_Action_Plan_Tracker) UI.16). |
...
| `telemetry_feed` · `feed_placeholder` | `Telemetry::LiveStream` | `UnpackTelemetryWorker#broadcast_to_matrix` — ✅ **rescue-ізольовано [UI.4, 2026-08-15]**. Найдорожчий інстанс правила «прикраса не сміє вбити тракт», і ціна тут вища за money-path-дзеркало: виклик стоїть у `perform` **перед** `TelemetryUnpackerService`, а зовнішній `rescue` виняток не ковтає, а ПЕРЕКИДАЄ його заради Sidekiq-retry — тобто стабільно падаючий броадкаст робив батч таким, що **ніколи не розпакується** (ретраї вичерпуються → dead set, черга №1, найдорожчі дані платформи). ⚠️ Асиметрію створив прохід, який роздавав ізоляцію сусідам: він узяв `prepend`+плейсхолдер саме звідси, а `rescue` дав лише двом продюсерам `BlockchainTransaction` — **зразок скопіювали, захист ні** ⚠️ **І структурна особливість, якої немає в жодного іншого тракту цієї таблиці: показане цією стрічкою не існує НІДЕ, крім самого кадру броадкасту** [UI.16]. `telemetry_logs` не має жодної колонки з сирим payload (розпакувальник пише розібрані поля), тож hex конверта живе рівно стільки, скільки повідомлення в кабелі — після перезавантаження сторінки стрічка порожня, і backfill'у для неї не існує в принципі. Це відрізняє її від усіх сусідів тут, де «reload лікує» (рядок уже в БД). Наслідок для будь-якої майбутньої правки: **тюнінг ДОСТАВКИ тут не є повним фіксом за побудовою** — доки одиниця = сирий конверт, сторінка лишається нечитабельною і від потоку, і від порожнечі. 🔴 **І з 2026-09-05 предмет [UI.16] ПЕРЕВИЗНАЧЕНО (⚖️ founder за скріншотом живого canopy): дефект СЕМАНТИЧНИЙ і чинний СЬОГОДНІ, при одному дереві.** Заголовок колонки дослівно каже лісгоспові «СИРИЙ ВМІСТ CoAP (HEX-ПОТІК)», і це не стає читабельнішим при жодній каденції — тоді як пункт відкладав роботу на `Tree.count ≥ 10 000`, тобто причепив пускач не до тієї проблеми. ⊕ Зміна одиниці на розібраний рядок лікує ТРИ речі одразу: зміст · backfill · і найдорожчий hazard тракту — трансляція стоїть у `perform` **перед** `TelemetryUnpackerService`, тож її довелось рятувати `rescue`-ізоляцією [UI.4]; після розпакування збій прикраси фізично не може завадити розпакуванню. ⚠️ Одиницею брати **зведення батчу** (шлюз · N записів · час · стан), не запис: ~45 рядків на флаш повернули б throughput-аргумент. ⛔ **І окремо про заглушку — клас, що вжалив ЦЕЙ компонент уже ВДРУГЕ:** `display:flex` на `td` знімає `display:table-cell`, а `colspan` діє ВИКЛЮЧНО на table-cell, тож клітинка мовчки падає в ширину однієї колонки (спіймано на живому canopy 09-05; сусідній інстанс — [UI.3] віньєтка, мертва 5,5 місяця). Обидва рази CSS був написаний і не діяв, а поверхня виглядала «майже правильно», тож ніхто не питав. Носій — приклад у `spec/views/components/telemetry/live_stream_spec.rb`; окремого гейта свідомо немає: периметр класу в дереві після фіксу НУЛЬ ([`00_07`](00_07_Action_Plan_Tracker) UI.16). |
