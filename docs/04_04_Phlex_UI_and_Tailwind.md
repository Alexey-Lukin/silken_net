...
|---|---|---|
| `font-mono` | JetBrains Mono → Fira Code → SF Mono → Cascadia Code → системний mono | `--font-mono` |
| `font-sans` | Inter → system-ui → -apple-system → … | `--font-sans` |
...
>
> ⚠️ **І чесно про наслідок фіксу:** жоден із двох шрифтів у застосунок не
> завантажується — ні `@font-face`, ні вебшрифт-лінка, ні вендорених файлів. Фікс
> робить справжнім *побажання*: користувач із JetBrains Mono локально побачить її,
> решта — наступний придатний зі стеку. Чи підключати вебшрифти (вага, CSP, офлайн,
> приватність) — окреме рішення → [`00_07`](00_07_Action_Plan_Tracker) UI.3.
