...
|---|---|---|---|
| `DashboardLayout` | `app/views/layouts/dashboard_layout.rb` | `Phlex::Rails::Layout` | Основний layout з sidebar, top bar, breadcrumbs. Топ-бар несе **індикатор робочого контексту** (`acting_organization:`, super_admin-only) — і це не оздоба: цей layout `flash` не рендерить **узагалі**, а `organizations#switch` його й не ставить, тож індикатор — єдиний можливий канал підтвердження, що перемикання відбулось [UI.6]. Станів свідомо два («обрано X» / «не обрано»): дім формули розходження (`Current#switched_context?`) **амбієнтний**, а `Current` як джерело даних у в'ю заборонений його ж шапкою; третій стан має сенс лише для super_admin із домашньою організацією й додається одним рядком, коли такий зʼявиться. ⚠️ Назва організації інтерпольована в `aria-label` — інакше він замінив би доступне імʼя цілком і SR-користувач чув би «змінити організацію», ніколи — яку саме ([UI.3] у сайдбарі) |
