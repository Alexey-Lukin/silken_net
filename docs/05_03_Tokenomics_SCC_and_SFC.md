...
| Timelock `CANCELLER_ROLE` (guardian) | **Safe** | 🔴 **скасувати БУДЬ-ЯКУ операцію в черзі однією tx — включно з тією, яку DAO вже ухвалив.** Тобто це постійне вето над рішеннями DAO | ⚖️ **НЕ ВИРІШЕНО** — renounce-рядок SEC.1 називає Timelock-admin і PROPOSER, а CANCELLER не називає, тож за буквальним виконанням цей важіль переживає активацію DAO. Легітимна опція (security-council veto) — але мусить бути ПРИСУДОМ, а не пропуском → [`00_07`](00_07_Action_Plan_Tracker) ARCH.112 |
