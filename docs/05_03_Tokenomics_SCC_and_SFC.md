...
🔴 **Спостережуваність цього реєстру має власну діру, і вона не в контрактах.** Кожна зміна ролі емітить успадковані `RoleGranted`/`RoleRevoked`, `pause` емітить `Paused`, Timelock емітить `CallScheduled`/`CallExecuted`/`Cancelled` — і **жодна з цих подій не індексується субграфом**: його `eventHandlers` знають лише грошові події (`CarbonMinted`/`TokenSlashed`/`ForestMinted`/`GovernanceSlashed`). Тобто зовнішній спостерігач, що читає нашу платформену «правду», бачить рух ГРОШЕЙ і не бачить ані закону, ані складання чи НЕ-складання повноважень із цієї таблиці. Це та сама вісь дібраної вибірки — реєстр і підстава в [`05_02 §Хто контролює ВИБІРКУ`](05_02_Proof_of_Growth_Pipeline).
