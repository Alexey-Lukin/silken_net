...
| `MINTER_ROLE` (SCC+SFC) | **backend-оракул (EOA)** | мінт до `MAX_SUPPLY`. Ключ береться через seam `Web3::OracleSigner`, джерело сьогодні — plaintext-ENV → [`00_07`](00_07_Action_Plan_Tracker) SEC.17 | ⛔ ні — операційна роль; blast-radius обмежують circuit-breaker + ex-post clawback |
