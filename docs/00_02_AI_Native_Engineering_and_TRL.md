...
- **Стан:** Філософія імплементована й використовується як ядро проєкту. Шкала готовності тут **незастосовна за визначенням** — предмет сторінки процес, а не технологія ([`00_03 §1`](00_03_TRL_Matrix_HIL_and_Beyond), DOC-T.70).
...
- [5. Критерій закриття задачі та TRL Gate Events](#-5-критерій-закриття-задачі-та-trl-gate-events)
<!-- TOC:AUTO:END -->
...
5. **Рев'ю (власник + детерміновані гейти):** для нижчих TRL (1-4) носієм рев'ю є **CI** — вісім required-чеків на `main` ([`06_07 §2`](06_07_CICD_and_Runbook_Index)); власник читає діф, але окремої церемонії схвалення там немає, бо автор і рев'ювер — одна особа, і ритуал давав би лише видимість другої пари очей. Свідоме рішення власника обов'язкове на **TRL Gate Events** (§5.1) — саме там рівень піднімається, а не на закритті задачі.
...
Розробка відбувається синхронно у трьох вимірах, тож софтові домени не простоюють в очікуванні заліза — саме це й декаплять HIL-симулятори ([`00_03 §3`](00_03_TRL_Matrix_HIL_and_Beyond)):
...
## 📊 5. Критерій закриття задачі та TRL Gate Events

Кожна задача несе рівень TRL; відкритий залишок живе в [`00_07`](00_07_Action_Plan_Tracker), поточні рівні — в [`00_03 §1`](00_03_TRL_Matrix_HIL_and_Beyond). Задача не вважається закритою, доки її реалізація не буде підтверджена:
...
### 5.1 TRL Gate Events — де підняття рівня НЕ автоматичне

Три переходи вимагають свідомого рішення власника, а не зеленого CI:

- **4 → 5** — з лабораторії до pilot: потребує HIL-валідації ([`00_03 §3`](00_03_TRL_Matrix_HIL_and_Beyond)).
