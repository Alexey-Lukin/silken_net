...
Ми не міряємо роботу "спринтами" чи "сторі-поїнтами". Прогрес будь-якого з 7 Модулів SilkenNet вимірюється виключно за рівнем технологічної готовності:
...
- **TRL 4-5 (Prototyping):** Створення MVP. Макетні плати (Breadboards) — валідація **в залізі**, написання першого коду. ⚠️ **Симуляції/in-silico сюди НЕ входять** — за строгим NASA/ISO 16290 вони лишаються TRL 3 (§4a; та сама ригористика, що й «без TRL 10-12»). *(Головний агент: coding-agent).*
- **TRL 6-7 (Field Testing):** Анкер вкручено в справжнє дерево. Дані йдуть через тестову мережу (Canopy Environment).
- **TRL 8-9 (Operational):** Кваліфікована система в реальних умовах. Mainnet (Polygon/Ethereum). ⚠️ Серійне виробництво / заводське штампування — це **MRL, не TRL** (наступний буліт): технологія на TRL 9 не стає «технічно готовішою» від друку мільйона анкерів.
- **Beyond TRL 9 (Planetary Intelligence, vision) — НЕ «TRL 10-12»:** Шкала NASA/ISO 16290 закінчується на **TRL 9** і вимірює виключно **готовність самої технології**. Технологія на TRL 9 не стає «технічно готовішою» від друку 1 млн анкерів замість 100 — масштаб, cross-biome адаптація та forest-level emergence є **системною та виробничою** зрілістю, а не технологічною. Тому ми НЕ вигадуємо «TRL 10-12» (це виглядало б некомпетентно перед ESA / NASA, які суворо дотримуються 1-9), а використовуємо профільні шкали:
...
2. **🚦 Validation Gate (ОБОВ'ЯЗКОВИЙ, перед SSOT):** Гіпотеза LLM стає «законом» ЛИШЕ після незалежної валідації одним із:
   ...
   - **домен-експерт** (ЧНУ/ЧДТУ — Мінаєв, Гусак тощо);
...
3. **Єдине Джерело Істини (SSOT / `docs/`):** Архітектор фіксує **валідоване** рішення у канон-доці `docs/NN_NN_*.md` як жорстку Специфікацію (Markdown); Wiki отримує його one-way генерацією, джерелом не є ([`00_06`](00_06_SSOT_Documentation_Standard)). *Це закон — але тільки після Gate.*
...
- **SSOT One-Home (одна річ — один дім):** кожен факт має **ОДИН** канонічний дім; решта доків **реферять**, не дублюють (інакше — тихий SSOT-дрейф). Коли тема переростає свій док, її **виносять у власну канон-сторінку** (migrate-first, zero content-loss); а спростовану/застарілу після піву гіпотезу **видаляють** (Ruthless Pruning — Git тримає історію), а не «мігрують про всяк випадок» у wiki-звалище ([`00_06 §4`](00_06_SSOT_Documentation_Standard)). Реєстр домів + restructure/prune-метод — [`00_06 §2/§4`](00_06_SSOT_Documentation_Standard).

  **Що з цього автоматизовано — три яруси, а не «лінтери vs руки»** (повний реєстр гейтів — [`00_06 §3`](00_06_SSOT_Documentation_Standard)):
  1. **owned-value** — re-statement конкретного значення поза його домом (Lorenz β, RTC-реєстри, mint/carbon-курс: owner-only vocabulary) — `docs:check_refs`;
  2. **структурний / registry-sync** — доки й код мусять бути 1:1, і гейт це рахує, а не читає (`model_doc_sync`, `cem_canon_sync`, `governance_*_sync`, `ruby_version_sync`, `protocols_ref_check`, `code_tracker_id_check`), плюс **мета-гейти на самі гейти** (`guard_registry_sync`, `workflow_gate_perimeter`) — бо реєстр гейтів гниє в один бік так само, як канон;
  3. **семантичний клас із названою стелею** — там, де слово несе намір, а не значення (`offering_lexicon_check`: інвест-лексика на клієнтських поверхнях). Такий гейт **мусить** декларувати, чого він НЕ бачить, інакше зелений колір починає означати «не перевірено».

  **Ручним лишається лише читання наміру** — той самий факт іншими словами. І там є клас, якого не ловить жоден лінтер, бо це твердження про **нас самих**: канон описує гейт слабшим (soft-fail / advisory / planned), ніж він уже став. Дрейф інвертований — читання канону дає **завищений** список боргу, тож інвентар суворості міряється **прогоном**, не грепом (інстанси: `iac_scan` описаний soft-fail у двох доках при `exit-code: '1'`; периметр «рівно дві required» при семи). Сиблінг-сліпота: dangling-ref гейт валідує `NN_NN`-лінки, а не прозову згадку модуля — саме так посилання на розчинений Модуль 08 прожило в цьому файлі до 2026-07-25.
...
**Трек B — Code-as-CAD** ([`01_02 §6`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)): тут **дві різні ролі**, які не можна плутати. **PicoGK (C#)** — справжній *CAD-as-code*: SDF/воксельне ядро, де AI-агент пише **детермінований генератор**, що обчислює геометрію (новий тип гіроїда) з коду (CEM-методологія LEAP 71 Noyron — *алгоритм, НЕ генеративний ML*; [`01_02 §6`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)). **✅ SHIPPED 2026-06-20** — анкер v1 + per-species 5-SKU (`tools/cad`). **nTop Automate** (CLI/API) — це НЕ авторинг геометрії кодом, а **headless parameter-sweep** уже створеного в GUI `.ntop`-воркфлоу: AI варіює вхідні змінні (товщина стінки, період комірки → 5 SKU за хвилини), але **базовий шаблон спершу «наклацує» GUI-інженер**. Підсумок: from-scratch generation = PicoGK; автоматизована параметризація готового шаблону = nTop Automate.

> **⚠️ «Трек C — Code-as-Mechanics» ВИДАЛЕНО:** важка механіка полімерів (в'язкопружність PEEK, Prony series, термонапруження Ti+PEEK, 20-річний creep) канонічно закріплена за **лабораторією Гусака (ЧНУ) на ANSYS LS-DYNA** ([`01_01 §4.3`](01_01_Coaxial_Gyroid_Topology_and_PEEK), [`07_03`](07_03_Academic_Integration_and_IP)). Паралельний AI-трек на CalculiX дав би **другий, неузгоджений** результат і знецінив би роботу професорів. In-silico конвеєр Архітектора покриває **Хімію** (PySCF/OpenMM), **Геометрію** (CAD-as-code), **Кінетику** (scipy) та **легкий closed-form mechanics-bound** (аналітичний Lamé термонапруження / stress-relaxation, scipy, БЕЗ mesh) — швидку оцінку-границю, що ЯВНО відкладає авторитетну важку mesh-FEA (barb-tip stress-concentration) + Prony-series в'язкопружний fit до Гусака. Важка mesh-FEA/creep лишається аутсорсною R&D-функцією ЧНУ.
...
- (a) фізичними даними або результатами лабораторних тестів — партнерський/лабораторний шар живе у [`07_03`](07_03_Academic_Integration_and_IP) (Модуль 08 розчинено, DOC-T.12), **АБО**
