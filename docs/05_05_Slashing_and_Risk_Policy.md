...
> **Сигнали Кат-A — реальність коду (фаза 1 свідомо КОНСЕРВАТИВНА = лише tamper).** Єдиний однозначний сигнал A сьогодні — **`EwsAlert` `vandalism_breach`** (critical, unresolved; піднімає `AlertDispatchService` з `TelemetryLog.bio_status_tamper_detected`). ⚠️ Раніше тут стояв `HardwareKey.tamper_detected_at` — **такої колонки немає** (doc-drift, виправлено). **[SLASH-1] chainsaw-спліт виконано:** окремий `chainsaw_detected` (acoustic-vs-thermal дискримінатор `AlertDispatchService`: anomaly без термального порога = вирубка, не вогонь; термальний сигнал пріоритетний) — сигнал РОЗРІЗНИМИЙ і їде non-fire маршрутом dClimate у Field-Audit (до спліту FIRMS бачив «ясне небо» → тавро `rejected_fraud` на жертві вирубки); **в A-сет свідомо НЕ входить до field-validation TinyML** (chainsaw-клас = synthetic placeholder, [`03_03 §4.2`](03_03_TinyML_Acoustic_Inference); `slash()` необоротний). `critical_unmaintained?` рахує й force-majeure-типи (aged `fire_detected` без maintenance → пропустив би burn на природній пожежі) — **не береться у ворота фази 1**. Розширення A-сету (scoped-unmaintained, chainsaw після field-validation) + активація inert `penalty_factor`-uplift — 👤 DAO-ратифікація (рейка `ProtocolParameters`→`SystemParameter`, [`05_06`](05_06_Governance_and_DAO)).
...
- **Hardware tamper detection** (`TelemetryLog.bio_status_tamper_detected` → critical `EwsAlert` `vandalism_breach`) — якщо є: автоматично категорія A, незалежно від dClimate. Це і є positive-A-сигнал гейта (§3.2). ⚠️ `HardwareKey.tamper_detected_at` (стояло тут раніше) — такої колонки не існує (doc-drift, виправлено).
...
**Стан (verified):**
- Драйвер slashing — `stress_index` (`ContractHealthCheckService`: tree ≥0.83 / cluster >20% дерев ≥0.83). Той самий поріг (`AiInsight::SLASH_STRESS_THRESHOLD`) сайзить і damage у `BlockchainBurningService` (ARCH.46 — тригер ≡ розмір).
