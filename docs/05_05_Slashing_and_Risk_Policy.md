...
| [`07_03` — Academic Institutions Registry](07_03_Academic_Integration_and_IP) | Партнерський ростер ФОТІУС/ЧНУ + академічний вихід для ground-truth протоколу (сам протокол — §8) |
...
- Параметричні тригери (Trigger-2 джерело): `dClimate.fire_detected ≥ FRP 10MW` (FIRMS, ✅ реалізовано). ⚠️ **Drought/pest супутникового оракула НЕМАЄ** — fire-FIRMS не адьюдикує не-пожежу (`Dclimate::VerificationService` повертає `:inconclusive` для них) → посуха/шкідник ідуть у **Field Audit / DAO** (Кат-C, §5), доки не з'явиться реальне джерело (dClimate `drought_index` / soil-moisture / ДСНС-API — 👤 [`00_07` S3.2/INS.1/UNI.12](00_07_Action_Plan_Tracker); фізичний дрон-fallback E.20/E.34). `NOAA earthquake ≥ M6` — теж майбутнє джерело.
...
- ✅ **GAP closed (heuristic):** heuristic-fallback (`calculate_stress_index_heuristic`, активний доки ML-модель не натренована) тепер вмонтовує обидва прямі сигнали: **sap** (`sap_stress_contribution`, signed below-baseline) + **acoustic/cavitation** (`acoustic_stress_contribution` — count лише **стрес-класів** TinyML: cavitation/chainsaw, **НЕ** вітер (клас-1) чи тиша (клас-0); нот.13 — акустика категоріальна (клас 0-4), тож для ML вона one-hot, для евристики — клас-фільтр, а не лінійний count усіх подій) — обидва **inert доки калібрування не задасть** ENV (`STRESS_SAP_*` / `STRESS_ACOUSTIC_*`); кожен обмежений так, що **корелює, але не слешить сам** ([E.64] status-0 → **0.0**; Z-anomaly(2) bounded **0.6 ≪ 0.83** «Z alone never slashes» — degenerate `avg_z>2`/weather-`temp` confound-члени **прибрано**; **[SLASH-1 P0]** `vm_error`(3) → **0.0** — софт-збій прошивки ≠ біо-стрес, старий `≥3 → 1.0` робив кластерний OTA-баг max-стресом на кожному дереві (тригер + damage разом), ops-сигнал = `firmware_fault`-алерт), а sap+acoustic беруться через **max(), не суму** (correlated drought — SLASH-SAFETY §6). Лишилось (on-device `Run_Inference` ✅ приземлено — FW.4): ground-truth калібрування порогів (протокол **§8**; lab-партнер [`07_03 §1.2`](07_03_Academic_Integration_and_IP) (ПМКТ калібрувальний датасет)). Деталі реалізації — [`04_02`](04_02_Business_Logic_and_Services).
...
**Дизайн** (польова валідація — ЧНУ біо-хаб + Data Science Карапетян + лабораторія Гусака; партнерський ростер і академічний вихід → [`07_03`](07_03_Academic_Integration_and_IP)):
