...
> **Реалізація cause-routing (поточний стан).** Формального централізованого `cause_classification` A/B/C-gate у коді **ще немає** (SLASH-1): `BlockchainBurningService#calculate_slash_ratio` ([`04_02`](04_02_Business_Logic_and_Services)) рахує лише **розмір** slash (convex §3), причину не перевіряє, і `cause_classification`/`freeze_balance!` поки не існують. Натомість причина зараз **відводиться upstream, ще до виклику burn**: **C** (cluster-wide blackout) → `ContractHealthCheckService#flag_data_blackout!` (EwsAlert + Field Audit, burn не запускається; §6); **B** (пожежа/стихія) → `Dclimate::VerificationService` (`fire_confirmed` → `InsurancePayoutWorker`; §4); **A** (negligence, default) → лишається шлях `slash()`. Цільовий дизайн — **positive-A-evidence guard (§3.2)**: burn лише за доведеної Категорії A, інакше freeze (відновлює C-дефолт §2).
...
### 3.2 Positive-A-evidence guard [SLASH-1] — рекомендований cause-gate (потрібен ЗАРАЗ)

**Проблема (verified у коді).** 4 ЖИВІ тригери burn — daily `ContractHealthCheckService` (stress>20%) · `Dclimate#handle_clear_sky_no_fire` (no-fire→`rejected_fraud`) · `Tree#trigger_slashing_protocol` (`after_update_commit` deceased/removed → bulk) · `ContractTerminationService` (`burn_accrued_points`-flag) — усі через чокпоінт `BlockchainBurningService`. **Лише `flag_data_blackout!` перевіряє причину** (blackout-кейс daily-шляху); решта 3 палять БЕЗ верифікації Категорії A → конкретні false-slash діри: природна **пожежа** як >20% стресу · **хвороба/посуха** як «фрод» (clear_sky) · природна **смерть** дерева → bulk-slash. Це **інвертує власний дефолт §2** («C = default safety = freeze»): код палить, поки не відведено, а має freeze, поки причину не визначено як A.

**Рекомендація — positive-A-evidence guard на чокпоінті `BlockchainBurningService`.** Перед `slash()` вимагати **≥1 прямий доказ A**: `HardwareKey.tamper_detected_at` · chainsaw-acoustic · `critical_unmaintained?` (no firebreak/maintenance після P0-алерту). Немає → **freeze + Field Audit (C), НЕ burn** (як `flag_data_blackout!`). Ці сигнали **вже в коді** (`comms_no_ack?`/`critical_unmaintained?`/tamper) — годують лише інертний `penalty_factor`-uplift (§3); рекомендація: перевикористати їх як **ворота** burn. Один guard на чокпоінті накриває всі 4 тригери.
