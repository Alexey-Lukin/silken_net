...
| [`05_03` — Tokenomics SCC and SFC](05_03_Tokenomics_SCC_and_SFC) | `slash()` / `slashUpTo()` [SLASH.2]; Dynamic Tax (insurance-pool funding); GAMMA / PENALTY_FACTOR_MAX — `SystemParameter`, on-chain `ProtocolParameters`-ключі `slash_gamma`/`slash_penalty_factor_max` **заведено** (контракт-half), sync у `PARAMETER_MAP` + read-path — ще → [`00_07` GOV.1](00_07_Action_Plan_Tracker) |
