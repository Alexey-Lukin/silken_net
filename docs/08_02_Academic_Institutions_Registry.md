...
|---|---|---|---|
| доц. **Ярмілко А.В.** | IIoT / embedded / lightweight crypto | AES-128-CCM LoRa-тракт ([`03_05 §2.1`](03_05_Hardware_Symmetric_Crypto_and_Security), FW.2 🟢 — bench-gated); TTL-flood dependability (Ст.3 — ⚠️ mesh відкладено, ARCH.43); SPI/DMA енергооптимізація ([`03_01`](03_01_Firmware_Lifecycle_and_DMA) — дім DMA; [`02_01`](02_01_Hardware_Architecture_and_BOM) лише реферить). ⚠️ ECDH знято 2026-07-17 — **не обраний варіант** (ADR [`03_05 §3.8`](03_05_Hardware_Symmetric_Crypto_and_Security): «Hash-Ratchet зараз, ECDH природно з SE050-L2»); НЕ йти з ним до партнера | Ст. 3/23/24 |
...
| доц. **Косенюк Г.В.** | військова радіотехніка, теорія кодування | RF impedance/Link Budget/ground-plane ([`02_01 §5`](02_01_Hardware_Architecture_and_BOM)); енерго-свідомий вибір coding-rate ([`00_07` FW.61/E.15](00_07_Action_Plan_Tracker)); hash-захист потоків ([`03_05`](03_05_Hardware_Symmetric_Crypto_and_Security)). ⚠️ «FEC» знято 2026-07-17 — E.15 ⚫ (CR 4/5 Hamming уже в кремнії SX126x; [`03_05`](03_05_Hardware_Symmetric_Crypto_and_Security) про FEC каже лише «no FEC») | Ст. 3/23 |
...
| доц. **Бушин І.М.** | CNN / computer vision / BSP | CNN-NDVI synthesis → Digital Twin ([`03_03 §10`](03_03_TinyML_Acoustic_Inference)); BSP-кластеризація аномалій — ⚠️ **publication vehicle TBD** ([`00_07` ARCH.19](00_07_Action_Plan_Tracker): Ст.10 вилучена, `BSP` у Ст.24a = 0 входжень). ⚠️ «фізика DMLS» знято 2026-07-17 — не його фах (CV/BSP), і це комірка Гусака ([`01_02`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)) | Ст. 24a |
...
| ст.викл. **Любченко К.М.** | нейромережі, GA, Master of Logic | GA-оптимізація backend ML (`InsightGeneratorService`, [`04_02`](04_02_Business_Logic_and_Services), Open Research); NSGA-II TinyML + циркадні пороги ([`03_03`](03_03_TinyML_Acoustic_Inference), Open Research). ⚠️ «КНФ/ДНФ верифікація контрактів» знято 2026-07-17 — фантом (тема = 1 входження на весь репо: цей самий рядок; у [`05_02`](05_02_Proof_of_Growth_Pipeline) її нема) | Ст. 24a |
...
| **Теліженко О.В.** (Олександра) | етнодизайн / UX | Народний художник України (етнодизайн, монументальне мистецтво). Орнаментальний патерн-бренд: «Silken Net» = текстильна метафора, mesh-топологія що обплітає бір ≈ рушник-оберіг → [`04_04`](04_04_Phlex_UI_and_Tailwind) (Phlex UI motifs), [`02_05`](02_05_Queen_Hardware_and_Starlink) (Queen radome graphics); B2B data-viz / UX-дашборд для ESG-інвесторів → ⚠️ атрибуція **у складі того самого ⚖️** ([`00_07` UNI.19](00_07_Action_Plan_Tracker)), не окремий факт: «єдина UX-людина §5» = аргумент від дефіциту, а бакалаврська [`08_01 §3`](08_01_Joint_Publications_and_IP_Strategy) = **те саме дзеркало, не незалежний доказ** (обидва вказівники знято 07-17); Genesis NFT-трек → [`08_03`](08_03_External_Stakeholders_Registry). ⚠️ Ст.33-ланка «selection super-hydrophobic coating» — **хибна атрибуція** (фторполімерна хімія ≠ її фах; присуд по coating = founder, [`00_07` HW.28](00_07_Action_Plan_Tracker) ⚖️) → склад Ст.33 потребує перегляду | Ст. 33 ⚖️ |
