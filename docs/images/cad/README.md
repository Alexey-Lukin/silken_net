...
| `cathode_flange.png` | Cathode flange 3D render — re-rendered 2026-09-14, after the Zone-3 ratchet fix of 2026-09-13. ⚠️ It predates 2026-09-18, so the shank under the disc still carries the Zone-3 DIN-471 groove removed that day (`00_07` HW.26); the top face it shows did not change. Unpinned; re-render needs the PicoGK viewer | `render cem/cathode_flange.json` |
...
| `anchor_zone1_pine_section.png` | Longitudinal **cutaway** (anode close-up). ⚠️ **Rendered on the RETIRED printed-core branch:** the gold core it shows is NOT part of the printed anode since 2026-09-18 — the part is printed without a core and the bus wire is welded to its top face (`01_01 §3` step 1b). PNGs carry no pin; the re-render is display-gated (`00_07` HW.1) | `section cem/anchor_zone1.pine.json` |
| `anchor_axial_stack_section.png` | **Full bus PATH cutaway**: the wire (gold) runs up the PEEK gap, through the cathode channel, to the flange-top pogo pad → the capsule (`01_01 §1.4`). ⚠️ **Rendered on the RETIRED printed-core branch**, so the gold also runs down through the anode; since 2026-09-18 the wire starts at the anode's top face (`01_01 §3` step 1b). Re-render is display-gated (`00_07` HW.1) | `section cem/anchor_axial_stack.json` |
| `anchor_assembly.png` | Capsule-end assembly (cathode flange ↔ PEEK radome, `02_02 §4.4`) — ⚠️ rendered 2026-09-14: its flange shank still carries the Zone-3 DIN-471 groove removed 2026-09-18 (`00_07` HW.26); unpinned | `render cem/anchor_assembly.json` |
