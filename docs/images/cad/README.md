...
| `anchor_axial_stack_section.png` | **Full bus PATH cutaway**: the wire (gold) starts at the anode's top face, runs up the PEEK gap, through the cathode channel, to the flange-top pogo pad → the capsule (`01_01 §1.4` · `§3` step 1b). ⛔ Gold continuing DOWN inside the anode means the render predates the welded branch. ⚠️ The radome cap on top is still the FULL HEMISPHERE — the flat crown R5 is ratified and NOT yet applied (`00_07` HW.33 · HW.30), so a re-render does NOT remove it. Re-render is display-gated | `section cem/anchor_axial_stack.json` |
| `anchor_assembly.png` | Capsule-end assembly (cathode flange ↔ PEEK radome, `02_02 §4.4`) — grooveless Zone-3 shank (`00_07` HW.26). ⚠️ The radome cap here is still the FULL HEMISPHERE: the flat crown R5 is ratified and NOT yet applied, gated on the piezo ⚖️ (`00_07` HW.33 · HW.30); unpinned | `render cem/anchor_assembly.json` |
