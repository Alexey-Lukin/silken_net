...
| [`cathode_flange.drawing.svg`](cathode_flange.drawing.svg) | Деталь 3 cathode flange drawing (Ø25 + 3× bayonet; the PEEK iso-ring `02_02 §1.2` asks for and the concept Ø4.5 GND pad are both labelled NOT IN GEOMETRY — the anode contact is the bus-wire end in the bore. The ring is REQUIRED since 2026-09-18 — countersink + flush PEEK ring Ø ≥ 4.0 — and so is the bus channel's own closure at its exit, the capsule's second sealing element; both geometries wait on the pogo pin P/N, `00_07` HW.34 / HW.9) | `draw cem/cathode_flange.json` |
| `cathode_flange.png` | Cathode flange 3D render — Ø25 disc, 3× bayonet lugs, barbed shank with NO groove on this end (`00_07` HW.26). Unpinned; re-render needs the PicoGK viewer | `render cem/cathode_flange.json` |
| [`mechanical_lock_zone1.drawing.svg`](mechanical_lock_zone1.drawing.svg) | §4.3 ratchet shank, Zone-1 anchor end — barb envelope + DIN-471 groove (kept until HW.26 G1/G3; no ring fitted as a backup) | `draw cem/mechanical_lock.zone1.json` |
| [`mechanical_lock_zone3.drawing.svg`](mechanical_lock_zone3.drawing.svg) | §4.3 ratchet shank, Zone-3 flange end — same generator, same local ratchet profile (the lean looks opposite only in the assembled view); NO groove on this end since 2026-09-18 (HW.26) | `draw cem/mechanical_lock.zone3.json` |
...
| `anchor_zone1_pine_section.png` | Longitudinal **cutaway** (anode close-up) — the lattice runs to the axis: the anode is printed WITHOUT a core, and the bus wire is welded to its top face instead (`01_01 §3` step 1b · `§1.4`). ⛔ A gold rod down the axis here means the render predates that branch. PNGs carry no pin; the re-render is display-gated | `section cem/anchor_zone1.pine.json` |
| `anchor_axial_stack_section.png` | **Full bus PATH cutaway**: the wire (gold) starts at the anode's top face, runs up the PEEK gap, through the cathode channel, to the flange-top pogo pad → the capsule (`01_01 §1.4` · `§3` step 1b). ⛔ Gold continuing DOWN inside the anode means the render predates the welded branch. 🔴 **The radome cap on this image is the retired FULL HEMISPHERE, and since 2026-09-22 that is ordinary staleness, not an un-applied verdict:** the flat crown R5 IS applied in the generator (`00_07` HW.33; its gate ⚖️ HW.30 settled the same day), so a re-render DOES fix it — and until someone runs one, this picture shows a cap the part no longer has. Re-render is display-gated | `section cem/anchor_axial_stack.json` |
| `anchor_assembly.png` | Capsule-end assembly (cathode flange ↔ PEEK radome, `02_02 §4.4`) — grooveless Zone-3 shank (`00_07` HW.26). 🔴 **The radome cap here is the retired FULL HEMISPHERE — stale since 2026-09-22, when the ratified flat crown R5 was applied** (its gate, the piezo ⚖️ `00_07` HW.30, settled that day; `00_07` HW.33). A re-render fixes it; it is display-gated and this row is unpinned, so nothing reds | `render cem/anchor_assembly.json` |
