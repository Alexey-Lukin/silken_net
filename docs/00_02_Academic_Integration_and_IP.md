...
- **Лабораторні дані відсутні** — більшість статей §2 gated на лабораторію (мокра хімія / ICP-MS / VNA / акустичний стенд). **Виняток — Стаття 1**: суто in-silico, дані готові, лабораторії й партнера не потребує; до сабміту лишились фігури й ACS-пас ([`00_07`](00_07_Action_Plan_Tracker) HW.5.IS).
...
> **Режим контрактації:** лаб-MoU ЧНУ/ЧДТУ. 🔴 **Перелік того, що MoU «розблоковує», виправлено 2026-09-17 (desk-звірка офіційних сторінок):** ані **ICP-MS**, ані **SEM**, ані **EIS** на публічних сторінках ЧНУ не заявлені — є XRD, XRF, ЕПР, ІЧ і вольтамперометрія; **ICP-MS у Черкасах немає взагалі**, найближчі акредитовані по воді — ДПСС Київської обл. і БУВР Причорномор'я, а з відкритим прейскурантом — НУБіП (V і Ti в одній лабораторії, Al — в іншій). SEM у Черкасах є в ЧДТУ (лабораторія ім. Дубровської) і застарілий РЕМ-200 у ЧНУ. ⚠️ Звірку роблено desk-пошуком, не листом: якщо прилад є, але не на сайті, це закриє одне питання Спрягайлу. Раніше цей рядок обіцяв те, чого перевірка не бачить; СЄУ/ЧІПБ/ЧМА = персональні контакти (люди з експертизою без лаб-доступу — MoU = порожній overhead).

### 1.1 ЧНУ — Черкаський національний університет

Дві площини під одним дахом: **Hard Science** (хімія/фізика/біологія, TRL 1–4) і **кібернетика/ПЗ/радіо** (ФОТІУС, TRL 3–6). Парасольовий MoU через Спрягайла — один підпис відкриває лабораторії ЧНУ; що в них є, а чого немає (ICP-MS у Черкасах немає взагалі), — «Режим контрактації» в шапці §1.
...
| доц. **Ярмілко А.В.** (ФОТІУС) | IIoT / embedded | Engaged partner (зустрічі 2026, вхід у ректорат); SPI/DMA-консультація ([`00_07` E.9](00_07_Action_Plan_Tracker)); mesh Markov/percolation-теорія (Open Research → [`06_08`](06_08_Resilience_and_Failover_Policy)). ⚠️ Крипто-профіль (lightweight crypto · airtime↔CCM · far-horizon PQC) знято 2026-09-18 — founder: з крипто не повʼязаний | — (з авторів Ст. 23 знято ⚖️ founder 2026-09-19: ділянки в антенній статті немає) |
...
- **B2G land-access:** Юрій Сегеда (Засл. природоохоронець, директор ДП «Смілянське лісове господарство» — ⚠️ **юрособи з такою назвою більше немає: держліси реорганізовано в ДСГП «Ліси України» з ФІЛІЯМИ** (desk-звірка 2026-09-17), тож посада й адресат листа потребують уточнення — єдиний лісовий масив із Черкасами; мешкає в с. Геронимівка, серце Черкаського бору, найближча точка до Genesis-полігону) — доступ до заказників Смілянщини + еко-аудит; ПЗФ-сумісність координується зі Спрягайлом. Дім-стан → [`00_07`](00_07_Action_Plan_Tracker) (STK.2).
...
- **B2G land-shield:** Олександр Дзюбенко (Засл. лісівник України, директор Центрального лісового офісу — ⚠️ нині **ДСГП** «Ліси України», не ДП; ⚠️ **посаду не звірено:** офіційний графік прийому ДП (ред. 2026-08-07) називає директором цієї філії іншу особу — дім розбіжності [`00_07`](00_07_Action_Plan_Tracker) STK.1 — керує держлісами Черкащини вкл. Черкаський бір; + д.е.н., проф. каф. лісового господарства ЧДТУ за сумісництвом) — легальний доступ до полігону в держлісі (перекласифікація анкера «втручання»→«науково-вимірювальний прилад» + Pilot Site MoU) + адмінміст ДП «Ліси України» ↔ ЧДТУ. Дім-стан → [`00_07`](00_07_Action_Plan_Tracker) (STK.1).
...
**Ключові результати:** математична модель деградації анкера на 20 років; протокол акселерованого тесту (12 тижнів @ 40°C ≈ 2.2–5.7 польових років — ізотерма, [`01_02 §2`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS)); верифікація self-healing покриття на мікрокапсулах з 8-HQ інгібітором. (Отримує ICP-MS Ti/Al/V від Статті 1.)
...
| Архітектор (Silken Net) | Дизайн гіроїда (TPMS, пористість — 65 % номінал зі смугою приймання 60–70 % (⚖️ 2026-09-17; доти тут стояло «~65–67 %, CEM first-pass, не зафіксована константа»), [`01_01 §5.2`](01_01_Coaxial_Gyroid_Topology_and_PEEK); Ti-6Al-4V), концепт Compute-by-Geometry, TinyML pipeline |
...
| **Гусак А.М.** (ЧНУ) | калібрація Kirkendall-моделі (наш script 51) проти виміряного V/Al release (20-рік екстраполяція); саме ICP-MS-вимірювання — зовнішня лабораторія, бо в Черкасах приладу немає («Режим контрактації», §1) |
...
**Тип зв'язку:** Послідовний — зовнішня лабораторія вимірює концентрації (ICP-MS; у Черкасах приладу немає) → ЧНУ (Гусак) калібрує модель → ЧМА (Суховий) оцінює біологічний вплив + Safety Margin.
...
🔴 **ТРЕТЯ вакансія, окрема від двох вище — privacy/GDPR-юрист (особа TBD, не контактовано).** Предмет інший і в переліку вище не представлений жодним пунктом: retention-строки per-юрисдикція · форма стирання там, де запис незнищенний (append-only аудит-ланцюг ⊥ оригінали фотодоказів, Art.17(3)) · представник Art.27 · review чернеток ToS/Privacy/Cookie й Art.30/Art.35-артефактів перед показом наглядовому органу. **Це НЕ підмножина крипто-вакансії** (MiCA/securities) і не господарське право Аблязова — і саме тому воно вимагає власного рядка: три документи чернеткового шару вже адресують читача сюди по цю компетенцію, а розділ доти її не називав. Роль канонізована тут 2026-08-23 — той самий прецедент, що з титулованим юристом у [§1.5](#15-сєу--східноєвропейський-університет). Дім-стан → [`00_07`](00_07_Action_Plan_Tracker) SEC.18 (обидва ⚖️) + BIZ.3 (публікація B2C-документів).

🔴 **ЧЕТВЕРТА вакансія, і вона не підмножина жодної з трьох — податковий фахівець (особа TBD, не контактовано).** Предмет: чи операція підпадає під «кваліфікований дохід» 90%-тесту Дія.City · ПДВ-режим carbon-послуги й місце постачання · момент оподаткування token-доходу фізособи. ⛔ **Крос-бордер інвойсинг і банк-контур сюди НЕ входять** — вони про валютний нагляд і господарське право, тобто домен Аблязова ([`00_07`](00_07_Action_Plan_Tracker) UNI.14 · [`tax_posture_ua`](protocols/legal/tax_posture_ua.md) Q2.3), і перелік предметів уже раз був ширший за те, що витримує читання цілі. **Це НЕ облік криптоактивів** (Гедз М.Й., [§1.5](#15-сєу--східноєвропейський-університет)): облік каже, ЯК записати операцію, оподаткування — ЧИ вона підпадає під режим; і не господарське/комерційне право Аблязова. Роль канонізована тут 2026-09-22 — вимір, що її показав, стоїть у [`00_07`](00_07_Action_Plan_Tracker) UNI.14 (прогін реєстру: податкового права не має ЖОДЕН із трьох СЄУ-фахівців) і доти канон-носія не мав, тож два готові пакети питань далі адресувались компетенції, якої в каналі немає. Дім-стан → [`00_07`](00_07_Action_Plan_Tracker) UNI.14 + BIZ.20 (90%-тест Дія.City).

⊕ **Одна поверхня цієї компетенції фахівця НЕ потребує й тому від пакета відділювана — власна ІПК від ДПС** (письмовий запит платника про ПДВ-режим carbon-послуги). ⚠️ Токен «ІПК» несе ДВА значення, і розведені вони МІЖ ЯРУСАМИ, а не всередині одного: чернетковий шар знає лише «підняти ВЖЕ ВИДАНУ консультацію як первинне джерело» ([`tax_posture_ua`](protocols/legal/tax_posture_ua.md) §Q2.1), а «подати ВЛАСНИЙ запит» живе в [`00_07`](00_07_Action_Plan_Tracker) UNI.14 — спільного слова для грепу в них немає, тому «потребує ІПК» у трекері читалось як залежність від фахівця.

⚠️ **Дві поверхні privacy/GDPR-компетенції несуть на юрканал ОДНИМ пакетом, а ухвалюють ПООКРЕМО** ([`00_07`](00_07_Action_Plan_Tracker) SEC.18). В append-only аудит-ланцюзі предмет — **поле всередині незмінного рядка** (звідси crypto-shredding як фіналіст поруч із винятком Art.17(3)); у фотодоказах `maintenance_records` — **цілий бінарний доказ**, чий «строк» є частиною retention-питання, а не окремою формою стирання. Спільне в них рівно те, що робить їх ОДНИМ питанням до юриста: чи Art.17(3)(b)/(e) покриває наш доказовий інтерес. **Розділяти пакет — втрачати спільну підставу; зливати присуди — плутати різні предмети.**
