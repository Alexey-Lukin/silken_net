...
| проф. **Гусак А.М.** | фізика (Wiley-монографія, UCLA-колаб.) | PEEK creep (Prony/Maxwell-Wiechert, виміряні дані) + ICP-MS-калібрація Kirkendall-моделі (наш script 51) → [`01_02 §2`](01_02_Ti_6Al_4V_Metallurgy_and_DMLS), [`01_01`](01_01_Coaxial_Gyroid_Topology_and_PEEK); FEA-E homogenization гіроїда (⚖️ [`00_07` HW.33](00_07_Action_Plan_Tracker) — self-own-кандидат vs школа Гусака). ⚠️ **Питання звузилось 2026-09-10: вибір ГІЛКИ закрито присудом (network), відкрите лише ТОЧНЕ значення E** — не просити партнера розсудити sheet-vs-network | Ст. 2/29 |
...
| **Ярмілко А.В.** (ЧНУ ФОТІУС) | Engaged-партнер (embedded; вхід у ректорат). ⚠️ Внесок у статтю не визначений: єдина роль, що тут стояла (airtime↔CCM-крипто кадру · far-horizon PQC), знята 2026-09-18 — founder: з крипто не повʼязаний |
...
⚠️ **Дві поверхні цієї компетенції несуть на юрканал ОДНИМ пакетом, а ухвалюють ПООКРЕМО** ([`00_07`](00_07_Action_Plan_Tracker) SEC.18). В append-only аудит-ланцюзі предмет — **поле всередині незмінного рядка** (звідси crypto-shredding як фіналіст поруч із винятком Art.17(3)); у фотодоказах `maintenance_records` — **цілий бінарний доказ**, чий «строк» є частиною retention-питання, а не окремою формою стирання. Спільне в них рівно те, що робить їх ОДНИМ питанням до юриста: чи Art.17(3)(b)/(e) покриває наш доказовий інтерес. **Розділяти пакет — втрачати спільну підставу; зливати присуди — плутати різні предмети.**
