...
**And the honest half:** today the system stands at **TRL 3**, gated by the anchor and the biofuel cell — in-silico validated, not yet in-vitro; zero nodes in a forest. The trillion is what we are building *toward* and measuring *against*, never a claim about what runs today. This section is the public rendering; the canonical home of the constraint is [`docs/00_01 §1.1`](docs/00_01_Vision_Mission_and_Roadmap.md), which is where the rest of the canon cites the 10¹² denominator from. Current per-module readiness lives in [`docs/00_03 §1`](docs/00_03_TRL_Matrix_HIL_and_Beyond.md); the scaling phases and the beyond-TRL-9 agenda both in [`docs/00_01 §4`](docs/00_01_Vision_Mission_and_Roadmap.md).
