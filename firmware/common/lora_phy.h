 ...
 * (+14 дБм @ SF9; його запас — там, не тут). Цей заголовок є
 ...
 * усередині себе).
 *
 * ⚠️ Регуляторна половина закрита цим файлом ЛИШЕ по потужності Королеви
 * (assert у блоці TX): канал на межі смуги 868.0, умова «антена до 2 дБі»,
 * ЕВП Солдата без обраної антени й робочий цикл OTA лишаються відкритими
 * (00_07 FW.61 · ARCH.24). Тут — те, що програмує прошивка; чи це дозволено
 * в ефірі, кажуть ті пункти.
 */
...
/* datarate: SF9 — Сценарій C 02_03 §9.8. */
...
/* ── TX ─────────────────────────────────────────────────────────────────
 * Потужність — друге поле профілю, що РІЗНИТЬСЯ між вузлами (⚖️ founder
 * 2026-09-24, 00_07 FW.61), тож шов бере її аргументом, як RX — rxContinuous:
 *   Солдат   +14 дБм — Сценарій C (02_03 §9.6/§9.8). ⛔ НЕ +22: той сценарій
 *     списано (негативний енергобаланс, 02_03 §9.5).
 *   Королева +10 дБм — стеля ЕВП НКЕК для SRD 868.0–868.6 МГц при її 5-dBi
 *     антені (assert нижче). Вона здебільшого ПРИЙМАЄ, а стеля ЕВП на прийом
 *     не діє, тож підсилення антени лишається на всьому uplink; ціна — −4 дБ
 *     downlink (OTA · beacon · CMD). [transitional] остаточне число — після
 *     виміру ЕВП на стенді (00_07 FW.61, 👤 radio-bench). */
#define LORA_PHY_TX_POWER_DBM_SOLDIER  14
#define LORA_PHY_TX_POWER_DBM_QUEEN    10

/* ЕВП = провідна + підсилення антени − 2.15 (дБі → дБд), у сотих дБ, щоб
 * лишитись у цілих: 10 + 5 − 2.15 = 12.85 дБм, запас ≈ 1.1 дБ під незвірений
 * допуск PA. Підсилення — дзеркало 02_05 §7 (антена 5 dBi), стеля — постанова
...
/* Запас після ефіру кадру, мс — скільки ще чекати ПОНАД `Radio.TimeOnAir`,
 * перш ніж дати радіо наступну команду (`lora_phy_apply.h` `Lora_Phy_Send`).
 * Кадр стартує не в мить `Radio.Send`, а після пробудження TCXO: ≤ 2 мс у
 * NT2016SF вузла (02_01 §3.1 поз. 16), ~5 мс у модулі LoRa-E5 Королеви
 * (оцінка Zephyr — 00_07 FW.46), плюс мілісекундна гранулярність HAL_Delay.
 * Ціна запасу — стільки ж запізнюється вухо Солдата після власного TX; рефлекс
 * Королеви, що стартує раніше, втрачає ≤ 10 мс своєї 32.8-мс преамбули.
 * [transitional] точний кінець дає TxDone-IRQ, який жодна прошивка ще не
 * реєструє; його латентність — вимір стенда (00_07 FW.61, 👤 radio-bench). */
#define LORA_PHY_TX_GUARD_MS           10u
