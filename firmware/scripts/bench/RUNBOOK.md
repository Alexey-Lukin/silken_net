...
## 3. Живлення (FW.54 / FW.50 / E.63 — дані для шкали `delta_t`!)
...
| Крок | Як | Очікуване |
|---|---|---|
| 5.1 **Verbatim-звірка ноти** | роздобути SIM7070_…_CoAP(S)_Application Note **V1.03** (Techship/SIMCom; публічні дзеркала 403) і звірити проти `03_02 §4`. AT Manual V1.03 + TCPUDP-нота V1.02 звірені посторінково лише на до-HW.41 команди (FW.60-розвідка 2026-07-12, `03_02 §4`) — ⚠️ **`AT+CGDCONT`/`AT+CNACT` (HW.41, 2026-09-09) проти V1.03 ЩЕ НЕ звірені**: граматика зі стандарту (3GPP TS 27.007 §10.1.1) і з загального SIMCom TCP/IP набору відповідно — той самий клас ризику, що FW.56 колись мав. 🔬 **Звірено 2026-09-25 зі СТАРШОЮ версією** — SIM7080 Series AT Command Manual V1.02 (2020-02-26, публічне дзеркало Waveshare; CoAP-API там інший — `CCOAPINIT`/`CCOAPURL`, не наш `CCOAPNEW`, тож версії розходяться і висновок лише орієнтир): `AT+CNACT=<pdpidx>,<action>` з pdpidx 0…3 — наш `=1,1` синтаксично валідний; **але APN APP-контексту задає окрема `AT+CNCFG=<pdpidx>,<ip_type>,<APN>`**, якої прошивка не шле (без неї APN = NULL → «subscription value»), і звʼязку «pdpidx CNACT = cid CGDCONT», на якому стоїть коментар `queen/main.c`, мануал не стверджує. Питання для V1.03 і транскрипту 5.2: чи потрібна `CNCFG`, і в якому просторі імен живе pdpidx. Residual = CoAP(S)-нота + обидві нові команди | граматика збігається АБО оновити `at_engine`-транскрипти |
| 5.2 Живі транскрипти | minicom-лог: ATE0/AT/CNMP/**CMNB**/**CGDCONT**/CPSMS/CEDRXS ×2 (AcT 4 і 5)/**CNACT** + CDNSGIP + CCOAPNEW/SEND/DEL (порядок з `03_02 §4`; CGDCONT/CNACT — HW.41; `CMNB=3` і друга CEDRXS — ⚖️ HW.41 2026-09-26: обидва RAT, eDRX для кожного) | лог ≡ скриптовані транскрипти `test_at_engine.c` (URC-порядки, таймінги) |
...
| 5.7 **FW.60 poll-downlink live** | Rails ставить pending CMD (`Downlink::PendingQueueService`) → повний flush → після `send_success` Королева сама `GET poll/<uid>` сирим CA\*-трактом (`sim7070_udp.h`; механіка [`03_02 §4а`](../../../docs/03_02_Queen_Gateway_Firmware.md)) | verbatim URC-порядок `CAOPEN → '>' → CASEND → +CADATAIND → CARECV(≤1459 Б) → CACLOSE` ≡ host-транскриптам (**довгий CARECV-буфер = модемна невідома** — головне, що міряємо); CMD доставлено, UUID-дедуп тримає; кожен poll несе `[0x9C][ts:4]` → RTC-sync задарма; **наступний poll несе ехо `cmd=<token>`, і лише воно переводить наказ `:sent` → `acknowledged`** (`observe_delivered_command!`, FW.63) — до ехо наказ перевидається кожним poll'ом; загублену 2.05 (зімітувати дропом відповіді) рятує саме це ехо, а не CoAP-ретрай: poll-тракт ретрансміту не має |
...
| [bench:coap] | §5 (+ §6 VBAT-droop) | FW.3 · FW.56 · FW.58 · FW.60 · FW.63 · HW.15 |
...
| [bench:radio-phy] | §6 (bullet «PHY-профіль на кремнії» ↓) | FW.61 |
| [bench:rf-mockup] | §6 (bullet «RF-макет» ↓) | HW.33 · HW.17 |
| [bench:bq-coldstart] | §6 (bullet «`VIN(CS)` власної партії» ↓) | HW.46 |
...
- **Reset-cause на кремнії** (`[bench:reset-cause]`, FW.59): по черзі викликати кожну причину й звірити старші три біти health-flags наступного QATT-конверта — power-cycle · NRST-кнопка · заблокувати main-loop до укусу IWDG (26.6 с) · навмисний HardFault. 🔴 **ПЕРШИМ пунктом — лінкер-скрипт:** переконатись, що `.noinit` винесено ЗА зону обнулення startup-коду (`objdump -h` на `.elf` + перевірка меж `_sbss`/`_ebss`). Без цього маркер гине на кожному ребуті й HardFault мовчки читається як `software` — деградація чесна, але невидима: ARM-джоба збирає glue OBJECT-бібліотекою без лінкування, host-сюїта лінкера не має взагалі, тож ЖОДЕН зелений CI цього не покриває ([`03_02 §7.1`](../../../docs/03_02_Queen_Gateway_Firmware.md)).
- **PHY-профіль на кремнії** (`[bench:radio-phy]`, FW.61): аналізатором звірити SF/BW/CR/преамбулу проти профілю [`03_05 §2.1`](../../../docs/03_05_Hardware_Symmetric_Crypto_and_Security.md), виміряти потужність на виході PA і окремо підтвердити `rxContinuous` у Королеви. 🔴 **Потужність тут не «ще одне число»:** `RadioInit` лишає `SUBGRF_SetTxParams(RFO_LP, 0, …)`, тож саме вимір на виході доводить, що `SUBGRF_SetRfTxPower(14)` перевів тракт у HP-плече, а не лишив його на низькопотужному — різниця в ~14 дБ, тобто увесь link budget 150–200 м. `rxContinuous` перевіряється спроможністю зловити ДРУГИЙ кадр без ре-арму: без `SetRxConfig` драйвер програмує RX-single, і перший пакет виглядає успіхом. ⊕ Заразом виміряти латентність TxDone → присуд про `LORA_PHY_TX_TIMEOUT_MS` (сьогодні 0 = без програмного таймауту, успадковано з panic-шляху).
...
- **BME280** I2C bring-up — ✅ логіка готова (`Bme280_Read_Calib`/`Bme280_Forced_Read`, `bme280.h`, host-suite `make -C firmware/test bme280` зелена; ops-шов `Bme280_Ops` — форма `g_ota_flash_ops`); 👤 на bench: HAL-опс (`HAL_I2C_Mem_Read/Write`, 0x76/0x77) у `firmware/soldier/main.c` разом із CCM-флипом → live read + gate-timing (≥ t_startup 2 мс + t_ON TPS22860; t_measure на морозі) + ⚠️ VDDIO НЕ за гейтом (датащит §3.2) + точка калібрування `Bme280_Vpd_Index`→kPa (формула+квант канонізовані `02_01 §3.4`).
...
- **`Write_OTA_Contract_To_Flash`** (Soldier) — ✅ логіка готова (`flash_ota.c`, host-suite `make -C firmware/test flash_ota` зелена, power-cut-safe magic-last); 👤 на bench: HAL_FLASH erase/program-фаза (`g_ota_flash_ops`, `firmware/soldier/main.c`) на реальній STM32 + e2e OTA-day.
- **`Ota_Sha_Persist`/`Ota_Sha_Verify`** (Queen, FW.52) — ✅ логіка готова (`ota_sha_guard.h`, host-suite `make -C firmware/test ota_sha_guard` зелена, 12/12, той самий magic-last патерн на СВОЄМУ HAL-глю); 👤 на bench: HAL_FLASH erase/program-фаза (`queen_ota_sha_ops`, `firmware/queen/main.c`) на реальній STM32 — окрема пара від Soldier-івської вище, інший чіп. **Новий сценарій, якого до FW.52 не існувало:** re-request ПІСЛЯ закриття OTA-вікна (`ota_is_active=0`), де персистований хеш ще збігається з буфером — раніше цей шлях мовчав завжди, тепер має відповісти; додати поруч із наявним late-trailer-воскресінням (§5.7 контекст).
- **BQ25570 дільники** (HW, [`02_03 §4`](../../../docs/02_03_BQ25570_MPPT_Nano_Power.md)): ✅ рівняння звірені з SLUSBH2G до плати — `ruby tools/firmware/bq25570_dividers.rb --assert` (HW.7, 2026-09-09). На bench лишається **замір**, і він має три осі, не одну: (1) DMM-замір `VBAT_OV` проти EDLC-стелі + HW.12 зовн. clamp (компаратор+MOSFET — гарантованої TVS-деталі датащити не дають) overcharge-тест; (2) **яке плече до GND, а яке до `VRDIV`** — інверсія дає правильний опір і хибний поріг; (3) чи `ROK3` узагалі запаяний — без нього гістерезис `VBAT_OK` дорівнює нулю, і плата виглядає здоровою. Виміряні номінали підставляти в той самий скрипт: `ruby tools/firmware/bq25570_dividers.rb rov1=… rov2=… rok3=…`.
- **`VIN(CS)` власної партії BQ25570** (`[bench:bq-coldstart]`, HW.46; числа — [`02_03 §1.1`](../../../docs/02_03_BQ25570_MPPT_Nano_Power.md)/`§1.5`): датащит дає лише typ/max (600 / 700 мВ, SLUSBH2G Rev F), а Stage-2 гейт `V_OC` судить паспортною межею — вимір лише уточнює її для НАШОЇ партії. ⚠️ **Що саме міряємо:** під час холодного старту `VIN_DC` ЗАТИСКАЄТЬСЯ на `VIN(CS)` (SLUSBH2G §7.4.2), тож «DMM у момент старту» без умов не дає числа — потрібне джерело, що справді віддає ≥ `PIN(CS)` = 15 мкВт. **Схема:** регульоване джерело `V_src` → послідовний `R_s` 4.7 кΩ (імітує `R_int` у межах стелі §1.5) → `VIN_DC`; накопичувач на `VSTOR` розряджено до 0 В; DMM на `VIN_DC`, другий канал — на `VSTOR`. **(1) Поріг старту:** рампа `V_src` угору від 0.40 В кроком ≤ 10 мВ із витримкою ≥ 60 с на кроці; старт = `VSTOR` росте впродовж витримки. **(2) Напруга затиску:** за `V_src` ≈ 1.0 В (струм через `R_s` ≥ 25 мкА з запасом) прочитати `VIN_DC`, поки `VSTOR` < 1.8 В — це і є `VIN(CS)` цього чипа. Кожен чип партії окремо (n ≥ 3), кімнатна температура, за можливості ще й холодна точка. ⛔ Прилад рампи нижче 0.8 В у переліку стенда відсутній (PPK2 як джерело нижче 0.8 В не йде) — потрібне лабораторне джерело з кроком у мілівольти або 10-обертовий потенціометр від елемента 1.5 В ([`02_04 §1`](../../../docs/02_04_Bench_Build_Guide.md)). **Результат:** розподіл `VIN(CS)` партії → [`02_03 §1.1`](../../../docs/02_03_BQ25570_MPPT_Nano_Power.md) поруч із паспортною вилкою, не замість неї.
...
- **RF-макет** (`[bench:rf-mockup]`, HW.33 · HW.17; споживач SF-осі — ARCH.8): три осі одним сеансом — Z-підлога антена↔Ti · чутливість на SF 9 і 8 · ККД бустера на землі ≤ Ø15.57. Процедура, прилади, конфігурації й приймання по кожній осі — аркуш [`rf_mockup_bench_sheet.md`](../../../docs/protocols/hardware/rf_mockup_bench_sheet.md), тут не дублюються. 🔴 **Eval-плата з власною антеною не міряє ККД нашої землі — і так само не судить Z-підлогу для обраного класу антени (бустер),** тож обидві осі йдуть і на тестовій платі. ⛔ AT-прошивку LoRa-E5 mini не палити: власним образом радіо до board-freeze не піднімається ([`02_04 §3.7`](../../../docs/02_04_Bench_Build_Guide.md)).
- **П'єзо interrupt-storm поріг** (`03_03 §1.2`, HW.30 cb-4/5) — ✅ архітектуру ратифіковано 2026-09-10 (HW.30): hardware R-дільник + RC-поріг перед EXTI (DNP-контур, `02_01 §3` поз. 6) — перша лінія, SW rate-limiter — друга. На bench лишаються **номінали** дільника/RC і поріг SW-лінії. ⚠️ [bench:acoustic]-риг дає лише ЕЛЕКТРИЧНУ половину — криву спрацювання EXTI від амплітуди на клемах пʼєзо (аркуш, вісь 3); амплітуд вітру/дощу ⊥ пилки на анкері в дереві немає, і без них номінал не вибирається (аркуш §5).
- **Acoustic-стенд** ⚠️ (стимул = **розгортка 2–8 кГц**, ⚖️ 2026-09-08 — дім присуду `00_07` HW.30; «16 kHz» було частотою ДИСКРЕТИЗАЦІЇ тракту `03_03`, не резонансом. Процедура, прилади, rig і що привозить кожна з трьох осей — аркуш [`acoustic_bench_sheet.md`](../../../docs/protocols/hardware/acoustic_bench_sheet.md), тут не дублюються) (спільний день HW.11 + HW.30, той самий Ti-coin і той самий п'єзо — ⚠️ записати ЯКИЙ: bench-ЗП-3 ≠ production-Mallory): HW.11 coating-attenuation з/без (`02_02 §3.4`) + HW.30 SMD-piezo+Sil-Pad voltage-spike vs стара ∅27мм через-отв. (`02_01 §6`) — один стенд-день, спільний rig. 🔴 **Що саме везти з дня, змінилось ⚖️ 2026-09-22:** dB-числа з таблиці `02_02 §3.4` знято (вона тепер лише РАНЖУЄ матеріали), тож цей стенд став ЄДИНИМ джерелом будь-якого числа демпфування в дереві. Привозити треба **криву по смузі 2–8 кГц і запропонований pass-поріг**, а не вердикт «пройшло» — інакше порожнє місце в каноні лишиться порожнім, і наступний прохід знову впреться в таблицю без acceptance.
