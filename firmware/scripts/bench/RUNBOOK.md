...
| 2.1b e2e CCM uplink-day (ПІСЛЯ 2.1-PASS) | фліп обох firmware-гейтів на бенч-парі + backend `TELEMETRY_CCM_ENABLED=true` (staging) → Soldier шле 28B → Queen 29B-батч → Rails `process_ccm_chunk` | `TELEMETRY_CCM_DECRYPT_OK_TOTAL` росте; MIC-fail=0; FC-replay guard ловить повтор кадру; panic-CCM (кнопка EXTI) долітає з `PANIC_FLAG` у розшифрованому статусі; 16B-Солдат поруч → `ccm_legacy_telemetry_drops` інкремент (atomic-cutover чесний) | FW.2 e2e + фліп-гейти (а)/(б) підтверджені живими |
...
| 2.6 FW.17 ротація ключа e2e (ПІСЛЯ 2.1-фліпу — ECB-downlink не сміє командувати ротацією) | фліп трьох гейтів (`FW17_RATCHET_ENABLED` + Queen `FW20_Q2_CMD_RELAY_ENABLED` + ENV `FW17_RATCHET_DOWNLINK_ENABLED`) → `HardwareKeyService#rotate!` для Tree → Queen-реле 0x9E (reflex услід за uplink'ом, `soldier_cmd_queue`) → Soldier re-key CRYP → uplink декриптується НОВИМ ключем (`clear_grace_period!` у логах) → power-cycle: Flash-KV `0x13` версія переживає, boot re-derive `Key_Ratchet_Apply` | grace закрито без ручного втручання; після ребуту вузол на K_v (не K0); rollback/replay-кадр мовчки відкинуто | FW.17 активація (`03_05 §3.8`) |
