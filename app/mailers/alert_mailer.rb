...
class AlertMailer < ApplicationMailer
  ...
  # про критичність у черзі `alerts`(2), фан-аут оперативних каналів їхав туди ж, а
  # ЄДИНИЙ формальний лист падав за `downlink`(4), тобто за чанками OTA-кампанії.
  # ⚫ Тим фан-аутом був Telegram, і його знято ⚖️ 2026-09-06 [ARCH.60]. Підстава
  # присуду це НЕ послаблює, а посилює: доки push не задротований [ARCH.108], лист
  # лишається ЄДИНИМ, що взагалі доходить до людини — «один із двох» став «єдиним»,
  # і повернення його в `default`(5) коштувало б тепер дорожче, ніж коштувало б тоді.
