...
class Wallet < ApplicationRecord
  ...
  #
  # ⛔ [ARCH.116] ПРОДАКШН-ВИКЛИКАЧІВ НУЛЬ — і це не борг, а стан, який канон тепер
  # називає вголос. `05_05 §2/§5` доти обіцяли, що на форс-мажорі кошти заморожуються
  # сюди; обіцянку ЗНЯТО (⚖️ 2026-08-29), бо механізму немає, а страховий тракт
  # (`Dclimate::VerificationService` → `InsurancePayoutWorker`) її не читає — перевірено
  # грепом: `locked_balance` там нуль входжень. Єдиний писач колонки — `lock_and_mint!`,
  # тобто надгробок СКОНВЕРТОВАНИХ балів, а не ескроу.
  # 🔴 І перш ніж дротувати цей метод під «заморозку»: одиниця тут БАЛИ, а SCC клієнта
  # лежить on-chain МОНЕТАМИ ([ARCH.95]) — тобто ескроу є ІНШОЮ архітектурою
  # (on-chain hold / окремий облік), а не викликом цього методу. Метод лишається живим
  # для double-spend-семантики pending-транзакцій, якою він і є.
  ...
  # це в мінт. **Передаєш бали сюди — спитай, чи вони вже зважені.**
  # Дім величини й самої колонки → `04_01 §6` (картка `Wallet`) + `TreeFamily`.
  #
