...
class Actuator < ApplicationRecord
  ...
  enum :device_type, {
    ...
    # `app/services/`), і дротувати її сюди не треба: верифікація дроном має дешевший дім
    # у bounty-нозі [`E.20`], яка ще й обходить регуляторну стіну автономного польоту
    # над пожежею. Тобто це forward-контракт на ЗАЛІЗО, не заготовка гілки ERS.
