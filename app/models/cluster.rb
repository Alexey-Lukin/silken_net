...
class Cluster < ApplicationRecord
  ...
  # ⛔ І це НЕ форма, яку тримав сусід `ClusterEntropyAnalyzerWorker`
  # (`return if score.nil?`): той ПРОПУСКАВ запис, тобто лишав стояти вчорашнє
  # значення. ⚠️ Воркера знято 2026-09-05 разом з усім трактом `entropy_anomaly`
  # (дім присуду — enum `EwsAlert#alert_type`), тож він лишається ЯК ІСТОРІЯ ФОРМИ,
  # а не як адреса живого коду; сама заборона від цього не слабшає — цю колонку
  # переписує щонічний `Cluster.find_each`, тож пропуск давав би понеділковий 0.42
  # на вівторковій темряві: підміна виміру, лише постаріла, і тим небезпечніша,
  # що правдоподібна.
