...
class ParametricInsurance < ApplicationRecord
  ...
  #
  # ⛔ 1 ЗАРЕЗЕРВОВАНЕ (колишній `forest_coin`) — не бери його під новий тип.
  # [DOC-T.89, ⚖️ 2026-08-26] SFC виплачувати НЕ можна: правил емісії SFC не існує
  # НІДЕ, а мінт SFC роздає ГОЛОСИ (`_delegate(to, to)` на обох мінт-шляхах —
  # `05_06 §4`), тобто страхова подія тихо переливала б governance-вагу. Це ЗНЯТТЯ
  # можливості, а не фіксація параметра, тож securities-блокер [BIZ.22] його не
  # накриває — воно звужує, а не обіцяє.
  #
  # 🔑 Знято саме тут, у ДЖЕРЕЛІ вибору, а не гардом на шляху виплати: гард
  # відхиляє те, що вже підписано, тобто лишає полісу існувати у стані, який
  # система відмовляється виконувати. Значення нема в енумі — інвестор не може
  # його обрати, а поліс не може дійти до виплати в неможливому типі.
  enum :token_type, { carbon_coin: 0 }, prefix: true
