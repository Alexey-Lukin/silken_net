...
class AiInsight < ApplicationRecord
  ...
  # гігієна. Тренер (`lib/tasks/ai_train.rake` — ⛔ знято з дерева 2026-09-05
  # разом із бекендним ML-шаром; урок про ДАНІ нижче переживає його) будував вектор фіч із
  # `average_temperature` + `reasoning[max_acoustic]` — ⚠️ ДВІ фічі з 2026-09-05, не
  # чотири (`avg_z` знято як DCI-печатку [E.64/E.52], `avg_vcap` як вироджений
  # VDDA-проксі [FW.50/ARCH.99]; механізм ARCH.84 не змінився, змінилась АРНІСТЬ),
  # а КЛАСТЕРНИЙ рядок жодної з них не має й досі — він агрегат. Виміряно
  # рантаймом: такий рядок заходив у набір як **`[0.0, 0.0]`** (0 °C, нуль акустики)
