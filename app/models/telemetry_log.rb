...
class TelemetryLog < ApplicationRecord
  ...
  #   · `severe_drought` народжувався ДВОМА гілками (пристрійний `z < 2.0` АБО
  #     серверний `Attractor.homeostatic?` поза per-family смугою), а `healthy?`
  #     другу не перевіряв ЖОДНОГО разу — ⚠️ друга гілка знята 2026-09-05 (E.64),
  #     тож підстава лишається чинною ЯК ІСТОРІЯ рішення ARCH.84, а не як опис коду — для родини з `critical_z_min > 2.0`
  #     стрік ріс, ПОКИ причина тривоги тривала;
  #   · `entropy_anomaly` — інший рівень агрегації: Shannon по кластеру за добу
  #     проти трьох пакетів одного дерева. ⚠️ Сам той тип знято 2026-09-05 (писача
  #     більше немає), тож і ця нога — історія рішення ARCH.84, а не опис коду.
