...
class TreeFamily < ApplicationRecord
  ...
  # per-species override-ів. Системний вердикт про дерево дає `Tree#effective_lorenz_thresholds`
  # (кластер → родина → глобальний дефолт), і саме його споживає рантайм —
  # `TelemetryUnpackerService#check_z_divergence!`; викликачів у `app/`/`lib/` цей
  # метод не має ЖОДНОГО. ⛔ Не тягни його в продовий шлях «бо назва пасує»:
  # так governance-шар знімається мовчки. Легітимний ужиток — твердження про
  # саму породу (спеки насіння й родинних меж), де кластера в питанні немає.
