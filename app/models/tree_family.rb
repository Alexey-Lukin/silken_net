...
class TreeFamily < ApplicationRecord
  ...
  #
  # 🔴 [FW.8, виправлено 2026-09-05] Тут стояло «і саме його споживає рантайм —
  # `TelemetryUnpackerService#check_z_divergence!`». Більше НІ: DCI судить за
  # `Tree#device_lorenz_thresholds` (смуга, ЧИННА НА ПРИСТРОЇ), бо порівнювати
  # треба два обчислення, а не дві конфігурації. Родинна смуга лишається
  # чинною у БІО-питанні — `AlertDispatchService` → `Attractor.homeostatic?`
  # (той бере пару `critical_z_min/max` НАПРЯМУ, тобто теж повз кластерні
  # override-и — та сама асиметрія, що й тут).
