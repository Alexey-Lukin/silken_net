...
class Tree < ApplicationRecord
  ...
  # --- КОНСТАНТИ ШКАЛИ ЖИВЛЕННЯ ---
  # ⚠️ [ARCH.99] Імена й числа тут описують ІОНІСТОР, а величина, яку вони міряють —
  # `latest_voltage_mv` — це мВ **VDDA**, шина живлення MCU (VREFINT-калібрування,
  # `03_01` FW.50). Каналу Vcap на вузлі не існує, тож шкала прикладена не до тієї
  # величини: номінальні 3300 мВ дають ~18 % «заряду», а верхні 81 % шкали недосяжні
  # за побудовою. Стеля свідома й позначена — присуд про самі числа відкритий
  # (`00_07` ARCH.99 ⚖️); тут виправлено лише ІМЕНА того, що показується людині.
  VCAP_MIN_MV = 2800   # Мінімальна робоча напруга (нижче — STM32 втрачає mesh-relay)
  VCAP_MAX_MV = 5500   # Стеля шкали: паспорт іоністора, НЕ діапазон вимірюваної шини
  LOW_POWER_MV = 3300  # Поріг критичного рівня енергії ⚠️ = сам номінал шини VDDA
  ...
  # ⚠️ [ARCH.99] Лінійна проєкція на шкалу іоністора — див. застереження над константами.
  def charge_percentage
    return 0 if supply_voltage_mv.zero?

    ((supply_voltage_mv - VCAP_MIN_MV).to_f / (VCAP_MAX_MV - VCAP_MIN_MV) * 100).clamp(0, 100).to_i
  end

  # Перевірка критичного рівня енергії для виживання вузла
  def low_power?
    supply_voltage_mv > 0 && supply_voltage_mv < LOW_POWER_MV
  end
