...
class InsightGeneratorService < ApplicationService
  ...
  # `Mrv::LineageReportService` гілки «джерельні рядки застаріли» не має й досі:
  # після [ARCH.70] дроп дасть `subroot_diverged`, тобто НАСЛІДОК без причини.
  # ⚠️ Аргумент від 08-21 звучав сильніше («видав би ЧУЖИЙ діагноз — дерево
  # змінило кластер») і цю силу втратив; чинне слабше: тракт не відрізняє нашу
  # ретеншн-дію від підміни payload'а, і саме тому другий механізм зникнення
  # рядків лишається забороненим;
  ...
  # ⚠️ Порядок фіч БУВ контрактом із тренером — ⛔ тренера НЕМАЄ в дереві з
  # 2026-09-05 (`lib/tasks/ai_train.rake` знято разом із бекендним ML-шаром), тож
  # контрагента в цього контракту сьогодні не існує. Рядок стояв теперішнім часом
  # ще й після присуду, який ЦЕЙ САМИЙ коментар цитує двома рядками нижче.
