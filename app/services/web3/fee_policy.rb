...
module Web3
  ...
  #     78 падінь) і бив би по RPC на кожну монету. 🔴 Шляхів апгрейду тут стояло
  #     ДВА, і один із них неможливий за побудовою (виміряно 2026-09-10): `reset!`
  #     чистить `Thread.current`, а детектор застряглого tx біжить у СВОЄМУ потоці —
  #     він скине власного клієнта й не торкнеться тих, що мінтять із протухлим cap.
  #     Лишається TTL на кеш пулу (нога → `00_07` ARCH.62);
