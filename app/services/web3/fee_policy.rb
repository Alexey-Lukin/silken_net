...
module Web3
  ...
  #
  # 🔒 СТЕЛІ, названі явно:
  #   · ЦЕЙ клас відповідає на «яке fee», ніколи на «коли його перечитати». Вимір
  #     тут робиться в мить виклику, а виклик робить ДВА різні моменти: народження
  #     клієнта (`RpcConnectionPool#build_client`) і підпис транзакції
  #     (`Web3::KeySigner#transact` → `RpcConnectionPool#refresh_fee!`). ⛔ Не
  #     заводь тут ані кешу, ані TTL: пускач відомий точно, тож він і є носієм —
  #     а число порога не існує в принципі, бо форми Polygon і Celo інверсні;
  #   · РЕШТА кешованого клієнта далі старіє — освіження стосується САМЕ fee.
  #     Клієнт, узятий із пулу й НЕ проведений через шов підписанта (читальні
  #     шляхи — `Erc20Reader`, `ChainAuditService`, confirmation-воркери), несе
  #     число з народження; їм воно байдуже, бо вони не відправляють транзакцій.
  #     Зʼявиться transact-сайт повз `KeySigner` — він поїде з протухлим cap, і
  #     побачити це можна лише читанням, гейта на цю вісь немає;
  #   · ⊕ і ПЕРЕДУМОВА, без якої вимір виглядав би неможливим: цінові RPC у гемі —
  #     НЕ «лише імена в реєстрі». `Api::COMMANDS.each { define_method }` породжує
  #     з того реєстру СПРАВЖНІ методи, тож `eth_gas_price`,
  #     `eth_max_priority_fee_per_gas` і `eth_fee_history` на клієнті існують
  #     (перевірено `respond_to?` на живому). Саме тому питання було не «чи можна
  #     спитати ціну», а «чи можна ВГАДАТИ формат відповіді» — і відповідь дав
  #     прогін проти живих нод, не читання гема;
