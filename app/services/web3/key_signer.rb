...
module Web3
  ...
  class KeySigner
    ...
    # ⊕ [ARCH.62] Fee тут не ЗАДАЄТЬСЯ — політика лишається однією, і живе вона в
    # `Web3::FeePolicy`; шов лише просить пул ПЕРЕНАКЛАСТИ її перед підписом, бо
    # виміряне число старіє разом із per-thread клієнтом, а Sidekiq-потік живе до
    # деплою. Підстава дому й підстава «чому не TTL» — у `RpcConnectionPool#refresh_fee!`.
    #
    # 🔴 ПОРЯДОК НЕСУЧИЙ, і це найдешевше місце помилитись: освіження стоїть ПЕРЕД
    # `assert_gas_reserve!`, який множить `gas_limit` саме на `max_fee_per_gas`.
    # Освіжити ПІСЛЯ означало б судити резерв за протухлою ціною, а відправити за
    # свіжою — тобто гейт стеріг би не ту транзакцію, яка поїхала.
    #
    # ⚠️ Тут доти стояло, що вимір на шві «повернув би політику в `LocalEnvSigner`
    # і бив би по RPC на кожну монету». Обидві ноги впали: мережу шов не деривує
    # (ENV-ключ віддає пул, `chain_id` не питається), а per-transact RPC цей самий
    # метод уже робить двічі — `eth_estimate_gas` нижче й `get_balance` у резерві.
    # ⊕ ENV-пін мережі лишається дорожчим за вимір, тож запінена мережа (L1-якір)
    # не дістає ЖОДНОГО зайвого виклику — `apply!` віддає статику й виходить.
    # @param client [Eth::Client] per-thread клієнт мережі (параметр, не стан)
    def transact(client, contract, function, *args, **kwargs)
      Web3::RpcConnectionPool.refresh_fee!(client)
