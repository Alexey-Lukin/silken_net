...
module Web3
  ...
  # 🔴 [ARCH.62, 2026-09-22] Маршрут — ратифікований, а ПОВТОР запису безпечний лише тому,
  #   що `Web3::KeySigner#pin_nonce` закріплює слот ДО каскаду: `IOError` нижче накриває
  #   `Eth::Client::RpcError`, тож помилка-відповідь шлюзу, що вже переслав tx, повторює
  #   `transact` тут, а гем без `nonce:` узяв би на фолбеку свіжий `pending` → друга tx.
  #   ⛔ Не кликати `transact` через цей клас повз шов підписанта.
  #
  # Використання:
