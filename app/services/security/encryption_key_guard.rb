...
# `hardware_keys` device-key columns and the `identities` OAuth-token columns.
...
# provisioning + telemetry-decrypt + OAuth would be dead-on-first-use. Catch it at
# boot instead.
module Security
      ...
      REQUIRED_ENVS.filter_map do |var|
        ...
        if value.blank?
          "[ar-encryption] #{var} is not set — HardwareKey provisioning, telemetry " \
            "decrypt and OAuth Identity read/write all raise without it."
