...
module Mrv
  ...
  #
  # Чесні статуси пруфа (ЧОТИРИ, [ARCH.70] ⚖️ 2026-08-29): anchored (повний шлях) ·
  # pending_anchor (лист новіший за останній confirmed-якір — proof після
  # наступного тижневого seal) · subroot_diverged (субкорінь кластера в якорі ≠
  # перерахованому) · unprovable_regrouped (субкорінь збігся, а цього листа в
  # складі кластера немає). ⛔ Останні два — НАСЛІДКИ, не причини: чому склад
  # розійшовся (переїзд дерева · дроп ретеншном · підміна payload'а) цей тракт
  # НЕ ізолює — стеля розгорнута біля самої гілки в `anchor_proof`.
  ...
  class LineageReportService
    ...
    def insurance_payout_entry(tx)
      {
        ...
        # ⚠️ Межа гарантії названа явно, бо вона ЧАСОВА: early-return зʼявився в
        # `596f784e`, а доти писач брав `organization.wallets.first` — той міг бути
        # nil, і колонка це дозволяє (`belongs_to :wallet, optional: true`, бо інші
        # роди рядків гаманця не мають). Рядків із того вікна не існує ніде —
        # продакшн-баз немає, dev пересівається — тож сьогодні це історія. У день
        # першого продакшн-деплою воно стає ПЕРЕДУМОВОЮ: бекфіл, що заведе виплату
        # повз воркер, мусить нести гаманець.
    ...
    def anchor_proof(log)
      ...
      # [ARCH.70 ⚖️ 2026-08-29] Два РІЗНІ факти замість одного імені — але з
      # оголошеною стелею, бо повне розчеплення тут неможливе за побудовою.
      #
      # Доти обидві гілки віддавали `unprovable_regrouped` («дерево змінило
      # кластер»), тобто зовнішній аудитор діставав ПРИЧИНУ там, де ми знаємо
      # лише НАСЛІДОК. Тепер розрізняємо те, що справді розрізнюване:
      #   · `subroot_diverged` — субкорінь кластера в якорі ≠ перерахованому;
      #   · `unprovable_regrouped` — субкорінь збігся, а цього листа в складі немає.
      #
      # 🔴 СТЕЛЯ, названа явно: ПРИЧИНУ розбіжності цей тракт НЕ ізолює.
      # `anchor_cluster_leaf_cids` рахує по ПОТОЧНІЙ приналежності дерев, тож
      # субкорінь розходиться однаково від переїзду дерева В кластер, переїзду
      # ГЕТЬ, дропу рядків ретеншном і від підміни payload'а. Тому статус зветься
      # за наслідком, а не `mismatch`: у сусіда (`TelemetryArchiveBatch`) те слово
      # означає integrity-провал, і позичити його означало б стверджувати tamper
      # там, де причина буває цілком легітимною.
      # ⛔ Не «посилювати» це порівнянням із rebuilt-лічильником сусіда — у цього
      # тракту такого лічильника немає; спершу потрібне джерело, яке переживає
      # переїзд дерева (напр. знімок складу кластера на момент якоря).
      return { status: "subroot_diverged" } if recomputed_subroot != stored_subroot
      return { status: "unprovable_regrouped" } if leaf_index.nil?
