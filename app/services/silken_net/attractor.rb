...
module SilkenNet
  ...
  class Attractor
    ...
    # твердження про здоров'я (Z є DCI-only), тож єдина жива роль цього предиката —
    # ДЗЕРКАЛО firmware-класифікації у спеці DCI-парності: `attractor_spec`
    # порівнює status-біти справжньої прошивки з класифікацією бекенду САМЕ через
    ...
    # їх спростував на ВСІХ трьох осях — 32.3 / 37.8 / 83.8 на 1500 cold-start
    # зернах при ρ=50 **І σ=30**, тобто на ОБОХ клампах разом. ⚠️ Умову називай
    # повністю: та сама ρ=50 з дефолтною акустикою (σ=10) дає |z|max 62.5, тож
    # «при ρ=50» саме по собі число не відтворює. Значення тут — КІНЦЕВЕ z після
    # 250 ітерацій (те, що пакує `Pack_FW2_Device_Z`), не максимум УЗДОВЖ
    # траєкторії: той на тих самих клампах доходить до ~125 (`generate_trajectory`).
    # Число, поставлене поруч із висновком, читається як його
