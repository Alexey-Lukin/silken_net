...
module Solana
  ...
  class MintingService
    ...
    def verify_oracle_balance!(fee_payer_pubkey)
      ...
      response = execute_rpc_call(payload)
      raw = response&.dig("result", "value")

      # 🔴 [2026-09-05] ТРЕТЄ обличчя того самого класу, і воно найтихіше: `nil.to_i`
      # дає НУЛЬ, тож ЗБІЙ ЗЧИТУВАННЯ ставав нерозрізненним і від «не поповнювали»,
      # і від «витратили». Тобто мовчазний RPC доповідав про стан ГАМАНЦЯ.
      # ⛔ Не згортати ці три в одне число: «не прочитали» не є фактом про баланс.
      raise "🚨 [Solana] Баланс Оракула НЕ ПРОЧИТАНО: RPC не повернув `result.value` — це збій ЗЧИТУВАННЯ, не стан гаманця. ⛔ Не тлумачити як нуль." if raw.nil?
      ...
      if balance.zero?
        raise "🚨 [Solana] Оракул НЕ ПРОВІЖИНЕНО: баланс рівно 0 lamports — стан НАЛАШТУВАННЯ, не вичерпання. ⛔ Не шукати витік."
      elsif balance < min_balance_lamports
        raise "🚨 [Solana] Баланс Оракула НИЖЧИЙ ЗА МІНІМУМ: #{balance} lamports " \
              "(поріг: #{min_balance_lamports} lamports / #{min_balance_sol} SOL) — витрачено більше, ніж поповнено."
      end
    end
