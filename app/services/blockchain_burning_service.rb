...
class BlockchainBurningService < ApplicationService
  ...
  def perform
    # =========================================================================
    # [SLASH-1 — АНУЛЯЦІЯ ЗАПИСУ, не механізм вартості]
    #
    # Виклик slash() (внутрішньо — _burn у Solidity) перманентно видаляє токени з
    # totalSupply ERC-20 контракту: одиниці, зараховані за ріст, свідчення якого
    # спростовано, перестають існувати. Це корекція облікового запису, а не
    # винагорода комусь: ⛔ не описувати спалення як зростання вартості чи частки
    # решти власників — платформа ціни SCC не читає й не публікує, а мова
    # «очікуваного прибутку від дій інших» — рівно те, що знято сервісною моделлю
    # (00_07 BIZ.22, fact-pattern F9 у docs/protocols/legal/securities_review.md).
    # =========================================================================
    ...
    # 🔴 [SLASH-1 ⚖️ 2026-09-04, делеговано founder'ом] Контрактна форфейтура міряє
    # ВІКНО ЦЬОГО ДОГОВОРУ, слешинг-за-провину — ні. Підстава й межа:
    #   · База без вікна означала, що достроковий вихід із місячного ПРОДОВЖЕННЯ палить
    #     емісію за роки під ПОПЕРЕДНІМ договором — повністю виконаним і оплаченим.
    #     Це не форфейтура (послугу надано), а конфіскація, і напрямок помилки завжди
    #     проти утримувача.
    #   · Носія «яка монета під яким договором» НЕ заводимо: величина не має ВИЗНАЧЕННЯ,
    #     доки кластер несе кілька договорів ([ARCH.103] зняв `emitted_tokens` саме тому).
    #     Вікно відповідає на інше, визначене питання — «намінтоване, поки договір діяв».
    #   · One-Home не порушено: формула не копіюється, звужується лише СКОУП викликача —
    #     рівно як `.for_cluster`. Докстрінг `net_minted_by_cluster` каже «часового вікна
    #     немає свідомо, бо межа змінила б ВІДПОВІДЬ» — тут зміна відповіді і Є предметом,
    #     тож вікно живе у ВИКЛИКАЧІ, а сам агрегат лишається без нього.
    #   · Верхня межа — `cancelled_at`, не `Time.current`: дія незворотна, тож база мусить
    #     бути ІДЕНТИЧНОЮ на ретраї. Фолбек покриває консольний виклик, де скасування
    #     ще не проставлене.
    # ⛔ Межа названа: на слешинг-за-провину вікно НЕ поширюється — там питання про ШКОДУ
    # лісу, а не про обсяг угоди, і воно тут не міряне. Не «уніфікувати» без власного виміру.
    # ⚠️ Артефакт, свідомо лишений: спалення попереднього договору, виконане ВСЕРЕДИНІ цього
    # вікна, віднімається, хоч мінти, які воно гасило, лежать поза вікном → база може вийти
    # відʼємною. Це на користь утримувача, і `<= 0` нижче її вже ловить.
    base_scope = BlockchainTransaction.for_cluster(@cluster.id)
    base_scope = base_scope.where(created_at: forfeiture_window) if @contractual
    total_minted_amount = base_scope.net_minted_supply(:carbon_coin)
  ...
  # [SLASH-1 ⚖️ 2026-09-04] Термін ЦЬОГО договору — вікно контрактної форфейтури.
  # `start_date` тут не «клієнтське число»: контракти не створюються через API взагалі
  # (`resources :contracts, only: [:index, :show]`), а на цьому ж шляху ця колонка вже
  # несе вагу — `ContractTerminationService#validate_termination!` рахує з неї
  # `min_days_before_exit`. Тобто заперечення класу `MaintenanceRecord#performed_at`
  # («чисельник, який вводить той, чию шкоду міряють») сюди не переноситься.
  def forfeiture_window
    @naas_contract.start_date..(@naas_contract.cancelled_at || Time.current)
  end
