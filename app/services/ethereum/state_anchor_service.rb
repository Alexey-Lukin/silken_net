...
module Ethereum
  ...
  class StateAnchorService
    ...
    def anchor_to_l1!
      ...
      # будь-який споживач, а з 2026-09-11 ще й ПЕРЕД КОЖНИМ ПІДПИСОМ
      # (`Web3::KeySigner#transact` → `refresh_fee!`). ⊕ Для L1 це не змінює
      # нічого й не змінить: `ETHEREUM_*_FEE_GWEI` мають дефолти в самому
      # `ENV.fetch`, тож якір завжди на ЗАПІНЕНІЙ гілці — обидва накладання
      # приносять ті самі 100/2 Gwei, виміру він не дістає, зайвого RPC не
      # платить, і це доводить пін `.twice` у власній спеці сервісу.
      # Цей сервіс перестав бути єдиним місцем у дереві, де
