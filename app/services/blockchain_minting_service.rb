...
class BlockchainMintingService < ApplicationService
  ...
  def process_token_group(client, signer, token_type, txs)
    ...
    # блоку разом із записом у `00_07` SEC.1 (той самий деплой, що вмикає governance).
    # ⛔ Не перецілюй запис назад у `DOC-T.89`: він у §🗄️, тобто резолвиться, але роботи
    # не приймає — протокол зняття ГРОШОВОГО гарда мусить вести в живий дім.
  ...
  # ABI. ⏳ Ціна рішення асиметрична в ЧАСІ, тож робиться ЗАРАЗ, а не «коли знадобиться»:
  # поки ЕМІСІЇ не було, це один рядок; після неї — міграція формату on-chain події з
  # розривом історії.
  # 🔴 Підстава ЗВУЖЕНА 2026-09-06: тут стояло «сьогодні нічого не задеплоєно», і це вже
  # неправда — шість контрактів живуть на Polygon Amoy з 2026-09-02. Аргументувати треба
  # ВІДСУТНІСТЮ ЕМІСІЇ (`totalSupply() == 0`), ніколи відсутністю деплою: перше ще
  # істинне й перевірне on-chain, друге спростоване. Висновок від правки не змінився —
  # змінилось те, чим він тримається [E.64].
