...
class AuditLogBlueprint < Blueprinter::Base
  ...
  view :show do
    ...
    # клас, що вузькі поля `:profile` нижче (`mfa_enabled`/`has_password`):
    # HTML-панель `audit_logs/show` не показує ані ip/user_agent, ані їх, тобто
    # JSON віддає понад те, що видно на сторінці. Але журнал аудиту існує
    # ЗАРАДИ форензики, і ці два поля є його речовиною — адміністратор, що
    # розбирає фрод, потребує їх за призначенням запису.
    ...
    # теж не машинне рішення. ⛔ Не звужувати це МАШИННИМ проходом за
    # аналогією з актором — лише разом із присудом про форензичний периметр
    # (`00_07` SEC.36, досі ⚖️-відкритий).
