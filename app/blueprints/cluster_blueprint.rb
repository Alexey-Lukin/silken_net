...
class ClusterBlueprint < Blueprinter::Base
  ...
  view :show do
    # ⚖️ [SEC.36, 2026-09-07] `geojson_polygon` лишається в `:show` СВІДОМО, і напис
    # стоїть тут, бо доти цей кліренс не мав носія ніде — ні тесту, ні коментаря, ні
    # доку, тож наступний охайний прохід по звуженню payload'ів зрізав би його як
    # «зайву PII» і був би формально правий. Підстава: полігон класифіковано як PII
    # (`04_01 §11`), але скоуп читача АСОЦІАТИВНИЙ — `:show` віддається лише в межах
    # організації-власника, тож третьої сторони в кадрі немає, а сам контур ділянки є
    # предметом послуги, не побічним витоком. ⛔ Звужувати лише разом із присудом про
    # сам скоуп, не окремо.
