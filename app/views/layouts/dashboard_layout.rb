...
class DashboardLayout < ApplicationComponent
  ...
  def render_top_bar
    header(class: top_bar_classes, role: "banner") do
      div(class: "flex flex-col min-w-0") do
        render_breadcrumbs
        h1(class: "text-base md:text-xl font-light tracking-[0.2em] uppercase text-white mt-1 truncate leading-tight") { @title }
      end
  ...
  def top_bar_classes
    "h-16 md:h-20 border-b border-emerald-900/30 flex items-center justify-between px-4 md:px-8 bg-black/50 backdrop-blur-md z-20"
  end

  def render_breadcrumbs
    nav(aria_label: "Breadcrumb", class: "flex text-mini uppercase tracking-widest text-emerald-900 font-bold") do
      ol(class: "flex items-center gap-2") do
        li do
          a(
            href: helpers.api_v1_dashboard_index_path,
            class: "hover:text-emerald-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 transition-colors duration-200"
          ) { "Citadel" }
        end
  ...
  def render_system_telemetry
    div(class: "hidden md:flex items-center gap-4 px-4 py-1.5 border border-emerald-900/50 bg-emerald-950/10") do
      div(class: "flex flex-col text-right") do
        span(class: "text-micro text-emerald-800 uppercase tracking-widest") { "Core Sync" }
        span(class: "text-tiny text-emerald-400") { "1.12 THz" }
      end
  ...
  def render_user_avatar
    div(class: "flex items-center gap-3") do
      div(class: "text-right hidden lg:block") do
        p(class: "text-tiny text-white leading-none") { @current_user&.full_name }
        p(class: "text-micro text-emerald-900 uppercase tracking-widest mt-1") { @current_user&.role }
      end
