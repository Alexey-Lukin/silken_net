...
module Views
  module Shared
    module UI
      ...
      # повідомлення наступним refresh'ем. ⛔ Утримати його технічно МОЖНА (Turbo
      # підставляє власний `beforeNodeRemoved` → cancelable `turbo:before-morph-element`),
      # і тут доти стояло протилежне; але це свідоме won't-do: гард лишав би на екрані
      # підтвердження ПОПЕРЕДНЬОЇ дії при роботі пачкою, тобто брехав би. Там канал
      # підтвердження — СТАН (як `Alerts::Row` уже робить), не повідомлення; повний
      # розбір підстав — канон `04_04 §1`.
      class FlashMessages < ApplicationComponent
        ...
        # невідоме». Реєстрація (`add_flash_types`) живе у ДВОХ коренях —
        # `Api::V1::BaseController` і `ApplicationController` (спільного предка нема).
