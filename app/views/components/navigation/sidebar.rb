  ...
  class Sidebar < ApplicationComponent
    ...
    def render_logo
      div(class: "px-6 py-8 border-b border-gray-200 dark:border-emerald-900/30 transition-colors duration-300") do
        h1(class: "text-gaia-primary font-extralight tracking-[0.4em] uppercase text-lg") { "Silken Net" }
        p(class: "text-[8px] text-gray-400 dark:text-emerald-900 mt-1 uppercase tracking-widest") { "Central Command Citadel" }
      end
    end
    ...
    def render_status_pulse
      div(class: "px-6 py-4 bg-gray-50 dark:bg-emerald-950/10 flex items-center justify-between border-b border-gray-200 dark:border-emerald-900/20 transition-colors duration-300") do
        div(class: "flex items-center space-x-2") do
          ...
          span(class: "text-[9px] text-gray-500 dark:text-emerald-700 uppercase") { "Sync: 1.12 THz" }
        end
        span(class: "text-[9px] text-gray-400 dark:text-emerald-900") { "v8.0.ocean" }
      end
    end

    def section_group(title, &block)
      div(class: "space-y-4") do
        h3(class: "text-[9px] uppercase tracking-[0.3em] text-gray-400 dark:text-emerald-900 px-2") { title }
    ...
    def nav_item(label, path, icon, badge: nil, pulse: false)
      ...
      a(
        ...
        aria_label: label,
        class: tokens(
          "group flex items-center justify-between px-3 py-2 text-[11px] uppercase tracking-widest transition-all duration-300 border-l-2 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-gaia-primary",
          active ? "text-gaia-primary bg-emerald-50 dark:bg-emerald-950/20 border-gaia-primary" : "text-gray-500 border-transparent hover:text-gaia-primary hover:bg-gray-50 dark:hover:bg-emerald-950/5 hover:border-gray-300 dark:hover:border-emerald-900/50"
        )
      ) do
        ...
        if badge&.positive?
          span(class: "bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-500 text-[8px] px-1.5 py-0.5 rounded-sm") { badge }
        elsif pulse
    ...
    def render_user_footer
      div(class: "p-4 border-t border-gray-200 dark:border-emerald-900/30 mt-auto bg-white dark:bg-black transition-colors duration-300") do
        div(class: "flex items-center space-x-3 px-2") do
          div(class: "h-8 w-8 rounded-none border border-gaia-primary flex items-center justify-center text-gaia-primary text-[10px]") { "A" }
          div(class: "flex-1 overflow-hidden") do
            p(class: "text-[10px] text-gray-900 dark:text-emerald-100 truncate") { "Architect" }
            p(class: "text-[8px] text-gray-400 dark:text-emerald-900 uppercase tracking-tighter") { "Full Access Link" }
          end
        end
      end
    end
