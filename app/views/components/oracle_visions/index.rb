  ...
  class Index < ApplicationComponent
    ...
    def header_section
      div(class: "p-6 border border-emerald-900 bg-black/40 backdrop-blur-md flex justify-between items-end") do
        div do
          h3(class: "text-tiny uppercase tracking-[0.5em] text-emerald-700") { t("oracle_visions.index.title") }
          p(class: "text-2xl font-light text-emerald-400 mt-2") { t("oracle_visions.index.confidence", value: "94.2%") }
        end

        # [FINANCIAL ENGINE VISUALIZATION]: Очікуваний врожай
        div(class: "text-right") do
          h4(class: "text-tiny uppercase tracking-widest text-emerald-800 mb-1") { t("oracle_visions.index.expected_yield") }
          div(class: "flex items-baseline justify-end gap-2") do
            ...
            span(class: "text-xs text-emerald-600 font-light italic") { t("oracle_visions.index.yield_unit") }
          end
        end
      end
    end
    ...
    def render_active_simulations_feed
      div(id: "simulation_results", class: "space-y-4") do
        div(class: "flex items-center gap-2 mb-4") do
          ...
          h4(class: "text-tiny uppercase text-gray-600 tracking-widest") { t("oracle_visions.index.active_simulations") }
        end
