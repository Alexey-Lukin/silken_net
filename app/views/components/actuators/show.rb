      ...
      div(class: "space-y-10 animate-in fade-in duration-700") do
        div(class: "grid grid-cols-1 lg:grid-cols-3 gap-8") do
          ...
          div(class: "lg:col-span-2 space-y-4") do
            h3(class: "text-tiny uppercase tracking-widest text-emerald-700") { t("actuators.show.log_title") }
      ...
      div(class: "border border-emerald-900 bg-black overflow-x-auto w-full") do
        table(class: "w-full text-left font-mono text-tiny min-w-[640px]", role: "table") do
          thead(class: "bg-emerald-950/20 text-emerald-800 uppercase text-micro tracking-widest") do
            tr do
              th(scope: "col", class: "p-4") { t("actuators.show.columns.id") }
              th(scope: "col", class: "p-4") { t("actuators.show.columns.operator") }
              th(scope: "col", class: "p-4") { t("actuators.show.columns.payload") }
              th(scope: "col", class: "p-4") { t("actuators.show.columns.status") }
              th(scope: "col", class: "p-4 text-right") { t("actuators.show.columns.executed_at") }
            end
          end
          tbody(class: "divide-y divide-emerald-900/30") do
            @commands.each do |cmd|
              tr(class: "hover:bg-emerald-950/10") do
                ...
                td(class: "p-4 text-emerald-100") { cmd.user&.first_name || t("actuators.show.system_operator") }
                ...
                td(class: "p-4 text-right text-gray-600") { cmd.executed_at&.strftime("%d.%m.%y // %H:%M:%S") || t("actuators.show.not_executed") }
              end
            end
          end
        end
