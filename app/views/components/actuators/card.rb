...
module Actuators
  class Card < ApplicationComponent
    ...
    def render_controls
      ...
      # виконання коментар, не код), а Rails на той час просувала стан при
      # ПОБУДОВІ відповіді (FW.63, закрито 2026-09-09 — build-час тепер робить
      # лише dispatch!), тож команда пройшла б `dispatch!`→`acknowledge!`→`confirmed`
      # і лягла б у ланцюг `Auditable` як виконана. Гучна відмова стала б
      # підробленим доказом на physical-safety поверхні — і цей висновок не
      # залежить від дати: echo підтверджує доставку конверта, не дію.
      div do
