# frozen_string_literal: true

module Actuators
  class Index < ApplicationComponent
    def initialize(cluster:, actuators:, pagy:, active_count: 0)
      @cluster = cluster
      @actuators = actuators
      @pagy = pagy
      @active_count = active_count
    end

    def view_template
      div(class: "space-y-8 animate-in slide-in-from-right duration-700") do
        header_section

        div(class: "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6") do
          if @actuators.any?
            @actuators.each do |actuator|
              render Actuators::Card.new(actuator: actuator)
            end
          else
            render_empty_state
          end
        end

        render Views::Shared::UI::Pagination.new(
          pagy: @pagy,
          url_helper: ->(page:) { api_v1_cluster_actuators_path(@cluster, page: page) }
        )
      end
    end

    private

    def header_section
      div(class: "p-8 border border-emerald-900 bg-black flex flex-col md:flex-row justify-between items-start md:items-center relative overflow-hidden shadow-2xl") do
        # Декоративний фон
        div(class: "absolute top-0 right-0 p-4 text-[60px] font-bold text-emerald-900/5 select-none") { t("actuators.index.decoration") }

        div do
          h3(class: "text-tiny uppercase tracking-[0.5em] text-emerald-700 mb-2") { t("actuators.index.title") }
          h2(class: "text-3xl font-extralight text-emerald-400 tracking-tighter") { t("actuators.index.sector_matrix", name: @cluster.name) }
        end

        div(class: "mt-4 md:mt-0 flex gap-6 text-tiny font-mono") do
          stat_label(t("actuators.index.active_nodes"), @active_count)
          stat_label(t("actuators.index.total_units"), @pagy.count)
        end
      end
    end

    def stat_label(label, value)
      div(class: "text-right") do
        p(class: "text-emerald-900 uppercase") { label }
        p(class: "text-lg text-emerald-100") { value }
      end
    end

    def render_empty_state
      render Views::Shared::UI::EmptyState.new(
        title: t("actuators.index.empty_title"),
        icon: "⚙",
        description: t("actuators.index.empty_description")
      )
    end
  end
end
