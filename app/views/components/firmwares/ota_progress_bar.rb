    ...
    def view_template
      div(id: "ota_progress_#{@uid}", class: "p-4 border border-emerald-900 bg-black font-mono") do
        div(class: "flex justify-between items-center mb-2") do
          span(class: "text-mini text-emerald-700 uppercase tracking-widest") { t("firmwares.ota_progress_bar.link_label", uid: @uid) }
        ...
        div(class: "flex justify-between mt-2 text-micro text-gray-600") do
          span { t("firmwares.ota_progress_bar.chunk", current: @current, total: @total) }
          span { t("firmwares.ota_progress_bar.complete", percent: @percent) }
        end
      end
    end
