    ...
    def view_template
      div(class: "max-w-3xl mx-auto") do
        ...
        form_with(
          ...
          # -----------------------------------------------------------------------
          # AFTERLIFE ECONOMY (Puro.earth CORC — D-MRV)
          # -----------------------------------------------------------------------
          # [E.20] Поле стоїть у формі БЕЗУМОВНО, а не зʼявляється за вибором типу:
          # умовний показ вимагав би Stimulus-контролера, тоді як єдиний його виграш —
          # сховати один рядок від пʼяти інших типів дії. Ціна протилежного промаху
          # більша: без поля випадайка пропонує «Вилучення біомаси», а модель вимагає
          # величину, яку ввести нічим (`validates … if: :action_type_biomass_extraction?`).
          #
          # 🔴 Підказка називає ЦІНУ подання, і це несуче, а не ввічливість: за цим
          # записом стоять ДВІ незворотні дії — оголошення дерева мертвим
          # (`declare_deceased!` → слешинг-тракт) і заявка на CORC у зовнішній реєстр.
          # ⚖️ [E.20, 2026-08-25] Обидві тепер за ПІДПИСОМ (`MaintenanceRecord#attest!`),
          # не за сабмітом — тож текст мусить називати саме засвідчення точкою
          # незворотності, інакше він лякає раніше часу й водночас недоговорює,
          # що саме підпис убиває дерево.
          # ⚠️ Рамка НЕЙТРАЛЬНА (як в OpEx/GPS), попередження несе ТЕКСТ підказки.
          # Панель видима завжди, тож постійно-червона рамка була б тривогою на
          # кожному записі про підмітання листя — і до першого справжнього вилучення
          # біомаси її вже ніхто б не бачив. Сенс несе статичний дискримінатор.
          div(class: "border border-gaia-border p-4 space-y-4") do
            p(class: "text-mini uppercase tracking-widest text-gaia-text-muted mb-2") { t(".afterlife.heading") }
            p(class: "text-micro text-status-danger-accent mb-3") { t(".afterlife.hint") }
            field_container(f, :biomass_yield_kg, t(".afterlife.yield_kg")) do |aria|
              f.number_field :biomass_yield_kg, step: 0.01, min: 0, class: input_classes,
                             placeholder: "e.g. 320.50", **aria
            end
          end
