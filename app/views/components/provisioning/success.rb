  ...
  class Success < ApplicationComponent
    ...
    def view_template
      div(class: "max-w-2xl mx-auto p-10 border-2 border-emerald-500 bg-black animate-in zoom-in duration-700") do
        div(class: "text-center space-y-6") do
          ...
          h2(class: "text-2xl font-light text-white uppercase tracking-widest") { t("provisioning.success.ritual_complete") }
          p(class: "text-sm text-gray-500 font-mono") { t("provisioning.success.woven") }
          ...
          div(class: "mt-10 p-6 bg-zinc-950 border border-emerald-900 text-left space-y-6") do
            ...
            render_data_point(t("provisioning.success.assigned_did"), @device.try(:did) || @device.try(:uid))
            render_data_point(t("provisioning.success.hardware_uid"), @device.uid || t("provisioning.success.not_available"))

            render_hkdf_mode_notice
          end

          div(class: "pt-10") do
            a(href: api_v1_cluster_path(@device.cluster), class: "text-emerald-500 underline underline-offset-8 text-xs uppercase tracking-widest") { t("provisioning.success.view_in_matrix") }
          end
        end
      end
    end
    ...
    def render_hkdf_mode_notice
      div(class: "pt-4 border-t border-emerald-900/30") do
        p(class: "text-mini text-emerald-600 uppercase tracking-widest mb-2") { t("provisioning.success.hkdf.title") }
        div(class: "p-4 bg-black border border-emerald-900 text-emerald-400 font-mono text-sm") do
          plain t("provisioning.success.hkdf.description_before")
          strong { t("provisioning.success.hkdf.algorithm") }
          plain t("provisioning.success.hkdf.description_after")
        end
        p(class: "mt-2 text-micro text-gray-500 uppercase italic") do
          plain t("provisioning.success.hkdf.note")
        end
      end
    end
  end
