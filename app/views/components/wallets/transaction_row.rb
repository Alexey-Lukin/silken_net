...
module Wallets
  class TransactionRow < ApplicationComponent
    ...
    def tx_type_styles
      case @tx.token_type
      # [UI.1] ПРАВИЛО КЛАСУ: монетні токени живуть у ролях ФОН/РАМКА, текст
      # завжди нейтральний `gaia-text`. Жоден із двох не має парного `-text`, і
      # ОБИДВА провалюють AA у текст-ролі на власній `/20`-підкладці — виміряно:
      # carbon 4.66, forest 3.80 (світла) / 4.70 (темна, тобто на межі бару).
      # ⚠️ Доти carbon був виправлений «за зразком» forest-гілки, а та сама
      # провалювала — статус «робочий приклад» є твердженням про походження, не
      # про придатність. Розрізняє монети ФОН, не колір літер.
