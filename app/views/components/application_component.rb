...
class ApplicationComponent < Phlex::HTML
  ...
  # [ARCH.89] Другий дім форматування — ГРОШОВІ величини показу (SCC-емісія ·
  # USD-плата за послугу · вартість робіт у $). Свідомо НЕ formatted_points:
  # той іменований балами, і його точність обґрунтована КРОКОМ ДЖЕРЕЛА балів
  # (нарахування кратні 0.01 за побудовою), тоді як грошові джерела кроку не
  # мають (`net_minted_by_cluster` — різниця сум над `numeric(24,6)`, слеші
  # дробові). Тут 2 — вибір ДИЗАЙНУ показу: центи для USD, а для SCC саме на
  # цій точності ARCH.103 вирівняв розкол «той самий кластер друкувався б із
  # шістьма знаками на одній сторінці й двома на сусідній». Одне обчислення,
  # два різні обґрунтування — тому два доми, а не один із двома іменами.
  #
  # ⛔ JSON НЕ вирівнюємо на цей дім: контролерні JSON-гілки віддають `.round(4)`
  # (рішення ARCH.103), а `WalletBlueprint` — сиру `numeric(24,6)`; машинний
  # споживач рахує сам. Периметр дому — HTML-рендер у `app/views/**`.
  AMOUNT_PRECISION = 2

  def formatted_amount(value)
    value.to_f.round(AMOUNT_PRECISION)
  end
