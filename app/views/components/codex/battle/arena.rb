    ...
    class Arena < ApplicationComponent
      ...
      def render_header
        div(class: "space-y-1") do
          h3(class: "text-tiny uppercase tracking-[0.3em] text-gaia-text-muted") { I18n.t("codex.battle_arena.heading") }
          p(class: "text-mini text-gaia-text-muted") do
            realm_label = @realm ? @realm.name_en : I18n.t("codex.battle_arena.realm_unknown")
            "#{I18n.t('codex.battle_arena.realm_prefix')} #{realm_label}"
          end
        end
      end
      ...
      def render_arena
        div(class: "grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-3 items-stretch") do
          ...
          div(class: "flex items-center justify-center") do
            span(class: "text-tiny uppercase tracking-[0.3em] font-mono text-gaia-text-muted") { I18n.t("codex.battle_arena.vs") }
          end
        ...
        div(
          ...
          div(class: "space-y-1") do
            ...
            p(class: "text-mini text-gaia-text-muted font-mono") do
              I18n.t("codex.battle_arena.elo_summary", elo: node.attunement_elo, matches: node.match_count)
            end
          end
      ...
      def render_pick_form(node)
        form(
          ...
          ) { I18n.t("codex.battle_arena.pick") }
        end
      end
      ...
      def render_skip
        form(
          ...
          ) { I18n.t("codex.battle_arena.skip") }
        end
      end
    end
  end
