  ...
  class Show < ApplicationComponent
    ...
    def render_settings_form
      div(class: "p-6 border border-emerald-900 bg-black") do
        ...
        form(action: helpers.api_v1_settings_path, method: "post", enctype: "multipart/form-data", class: "space-y-6") do
    ...
    def render_logo_field
      div(class: "space-y-2") do
        label(class: "text-[9px] text-gray-600 uppercase tracking-widest block") { "Organization Logo" }
        if @organization.logo.attached?
          div(class: "flex items-center space-x-4 mb-2") do
            span(class: "text-[10px] text-emerald-500 font-mono") { "Current: #{@organization.logo.filename}" }
          end
        end
        input(
          type: "file",
          ...
          class: "w-full bg-zinc-950 border border-emerald-900/50 text-[11px] font-mono text-emerald-400 px-4 py-3 file:mr-4 file:border-0 file:bg-emerald-900/30 file:text-emerald-400 file:text-[10px] file:px-4 file:py-2"
        )
      end
    end
