  ...
  class MfaSetup < ApplicationComponent
    ...
    def render_header
      div(class: "space-y-2") do
        h2(class: "text-xl font-light text-emerald-100 uppercase tracking-widest") { t(".title") }
        p(class: "text-tiny text-gray-500") { t(".scan_hint") }
      end
    end
    ...
    def render_secret_block
      div(class: "text-center space-y-2") do
        p(class: "text-mini text-gray-600 uppercase tracking-widest") { t(".secret_label") }
        code(class: "text-tiny text-emerald-500 tracking-[0.3em] break-all") do
    ...
    def render_confirm_form
      form_with(url: mfa_setup_path, method: :patch, class: "space-y-4 pt-4 border-t border-emerald-900/50") do |f|
        if @error.present?
        ...
        div(class: "space-y-2") do
          label(for: f.field_id(:otp_code), class: "text-tiny text-gray-500 uppercase tracking-widest") { t(".confirm_label") }
          f.text_field :otp_code, class: input_classes, placeholder: "000000",
                                  autofocus: true, autocomplete: "one-time-code", inputmode: "numeric"
        end

        button(type: "submit", class: "px-6 py-2 border border-emerald-500 text-tiny text-emerald-500 " \
    ...
    def input_classes
      "w-full bg-black border border-emerald-900 text-emerald-100 p-4 font-mono text-sm " \
