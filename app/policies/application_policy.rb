...
class ApplicationPolicy
  ...
  # Виміряний blast фліпу — ОДНА політика (`EwsAlertPolicy`, єдина в репо без
  # власного `show?`), і та нині не викликається; `index?` і `Scope#resolve`
  # перевизначені скрізь, тобто там blast нульовий.
  #
  ...
  class Scope
    ...
    # Blast нульовий: власний `Scope` мають усі політики репо (`Codex::CitationPolicy`
    # єдина без нього, але вона успадковує `Codex::ApplicationPolicy::Scope`, не цю).
    # Тобто рядок сторожить виключно МАЙБУТНЮ політику — і саме тому його форма
    # `none`, а не `all`: забутий скоуп має віддавати порожньо, а не все.
    def resolve
      scope.none
    end
