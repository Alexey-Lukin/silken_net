...
module Api
  module V1
    class SessionsController < BaseController
      ...
      #
      # 🔴 [ARCH.69, ⚖️ 2026-08-21] Розходження з контрактом входу закриті ДО дроту,
      # і порядок тут несучий: сьогодні жодне з них не досяжне саме тому, що
      # маршруту немає, тобто ДРОТУВАННЯ і є зняттям запобіжника. Полагоджено
      # чотири: другий фактор · salt-стемп passwordless-акаунта · локаль вітання ·
      # імʼя провайдера. ⚠️ Два «розходження», названі аудитом, виміром НЕ
      # підтвердились: `Errors::NoOrganization` — не помилка, а задизайнений
      # Quarantine-екран (за seeds це ПЕРШИЙ екран super_admin, `04_04 §6.4`), а
      # JSON-гілка тут була б мертвою, бо це browser-callback провайдера.
      # ⏳ Лишаються ДО дроту (обидва потребують маршруту, якого ще нема):
      # `skip_forgery_protection` і покриття callback-шляху edge-лімітером —
      # його правило для входу анкероване на трьох наявних auth-шляхах, тож
      # `/auth/*` під нього не підпадає. ⚠️ Перифраз тут НЕ стиль: межу SEC.32
      # стереже пін, що забороняє цьому файлу називати edge-шар поіменно, бо
      # згадка читалась би як обʼєднання двох незалежних політик; перелік
      # анкерів живе у своєму домі (`config/initializers/rack_attack.rb`).
      def omniauth_create
        ...
        # передбачив цей шлях дослівно: «перший passwordless/OAuth-шлях, який не
        # проставить пароля, зробить її дірою МОВЧКИ». Предикат сьогодні
        # fail-CLOSED (`expected.present?`), тож діри немає — натомість є гірший
        # для людини наслідок: `session[:ps]` = `nil`, сесія записується й
        ...
        # «сесія не існує до другого фактора». Провайдер засвідчує лише ПЕРШИЙ
        # фактор, тож без цього гейта акаунт із увімкненим TOTP діставав би повну
        # сесію з самого провайдерського твердження — тобто OAuth був би обхідним
        # шляхом навколо MFA. Гейт стоїть ПІСЛЯ прив'язки ідентичності (провайдер
        # уже підтвердив володіння, і цей факт зберігаємо) і ПЕРЕД сесією.
        return redirect_to_mfa_challenge(user) if user.mfa_enabled?

        establish_session(user)

        # 🔴 [I18N.3] Дзеркало `create`: вітання пишеться в локалі, якою вже
        # рендериться наступна сторінка. Екшен іде під `skip_before_action`, тож
        # акаунт-щабель порожній на момент `set_locale` — без обгортки людина з
        # `users.locale = "lv"` діставала б латиський дашборд з англійським
        # вітанням. ⚠️ `Identity.provider_name`, а не `titleize`: останній дає
        # «Google Oauth2», і саме для цього мапа й існує.
        #
        # ⚠️ `respond_to` тут СВІДОМО немає, на відміну від `create`: цей екшен —
        # browser-callback, на який людину приводить редирект провайдера, тож
        # JSON-гілка була б недосяжною за побудовою. Гілка без пускача гірша за
        # її відсутність: вона читається як підтримуваний контракт.
        I18n.with_locale(resolve_locale(account: user)) do
          redirect_to dashboard_index_path,
                      success: I18n.t("flash.sessions.authenticated_via",
                                      provider: Identity.provider_name(auth.provider))
        end
      end
