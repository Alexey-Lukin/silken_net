...
module Api
  module V1
    class AuditLogsController < BaseController
      ...
      # [ARCH.83, присуд founder 2026-08-13] Банг тут кидав РЯДКОМ РАНІШЕ, ніж
      # `<< nil` дав би super_admin'у системний ланцюг — тобто журнал governance-змін
      # був недосяжний своєму єдиному призначеному читачеві, доки той не всиновить
      # випадкового тенанта (і тоді ланцюг приклеювався до чужого журналу).
      # Присуд: третього, «першокласного» платформеного стану НЕ заводимо — застосовуємо
      # політику, вже ратифіковану для безтенантних ресурсів (`04_03 §3.1`): читач
      # рендериться, тенанта не вимагає. `AuditLog.none` — для актора без організації
      # і без ролі super_admin: у нього читабельних рядків справді нуль, і це вимір,
      # а не дефолт (⊥ [ARCH.84]).
      def readable_audit_logs
        org_ids = [ acting_organization&.id ].compact
        org_ids << nil if current_user.role_super_admin?
        return AuditLog.none if org_ids.empty?
