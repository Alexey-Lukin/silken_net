...
module Api
  module V1
    class ContractsController < BaseController
      ...
      # ⛔ Цінового шляху в дереві НЕМАЄ ВЗАГАЛІ — ні DEX-читача, ні governance-
      # параметра fallback-ціни. Тому найдешевший хибний «фікс» (лишити зовнішній
      # виклик заради множення на невідоме — витрата без результату, чия тиша ще й
      # читалась би як «оцінка не працює») тут не просто заборонений, а недосяжний:
      # щоб його зробити, оракул треба спершу ПОБУДУВАТИ.
      # 🔓 [⚖️ 2026-08-20, ARCH.103 §🗄️] `nil` НАЗАВЖДИ: споживач бачить емісію в SCC,
      # курс множить сам. Тригер повернення — перший B2B-клієнт із запитом
      # фіат-оцінки, і ціна повернення названа чесно: це читання DEX-quoter'а
      # (+ кеш із TTL + чесна мітка при мовчанні), не архітектура.
