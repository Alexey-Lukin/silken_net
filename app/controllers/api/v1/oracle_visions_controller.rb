    ...
    class OracleVisionsController < BaseController
      ...
      def calculate_expected_yield(org)
        Rails.cache.fetch(Organization.expected_yield_cache_key(org.id), expires_in: 1.hour) do
          ...
          # 🔴 [ARCH.103] ЧИСЕЛЬНИК ЦЬОГО ПРОГНОЗУ СТРУКТУРНО ПОРОЖНІЙ, і це не
          # тимчасовий стан. `telemetry_logs.sap_flow` не має ЖОДНОГО писача —
          # поля немає в жодному wire-форматі, тож у проді колонка завжди `NULL`
          # ([`04_01 §2`]). Отже `sap_index.nil?` істинний для кожного дерева,
          # `measured` лишається **0**, а `value` — константою `0.0`.
          #
          # ⚠️ Це ЄДИНИЙ живий читач тієї колонки, і канон доти стверджував
          # протилежне («читачі знято 2026-08-16»): попередній свіп перелічував
          # читачів за іменем колонки, а сюди воно заходить через мапу
          # `latest_per_tree`, тобто за спільним ВХОДОМ, а не за іменем.
          #
          # ✅ Чому лишено, а не зрізано: число не бреше — покриття (`measured`/
          # `total`) чесно каже «0 з N», тож поверхня вже свідчить про себе
          # правдиво. Нуль викликачів у передпродовому репо вимірює
          # недобудованість, а не смерть; зріз знищив би НАМІР. **Умова
          # оживлення названа: поле `sap_flow` у wire-форматі телеметрії** —
          # щойно писач зʼявиться, цей блок починає рахувати без правок.
