...
class ActuatorSafetySweepWorker
  ...
  # TTL нашого ж STOP — компроміс, а не виведене число: «найгіршого каденсу
  # poll'а» не існує (див. хедер — мовчазна legacy-Королева не флашить ніколи),
  # тож пережити ВСІ сценарії неможливо в принципі. 6 год покриває звичайний
  # таймерний флаш із запасом; безсмертним STOP бути не сміє — інакше він навіки
  # тримав би `live_pending?` істинним і глушив би цю саму ногу на цьому актуаторі.
  STOP_TTL = 6.hours
  STOP_DURATION_S = 1
