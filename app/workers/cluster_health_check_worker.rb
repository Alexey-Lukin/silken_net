...
class ClusterHealthCheckWorker
  ...
  #
  # ⚠️ **Твердження про предмет цієї звірки ЗМІНИЛОСЬ [SLASH-1, ⚖️ 2026-08-26], і
  # рядок мусить це сказати, бо мовчазна версія читалась би як чинна.** Доти
  # лічильник годував ТРИГЕР слешингу (`total_active_trees` → поріг і межа
  # виродження); тепер обидві половини читають `DailyHealthRouter#witnessing_trees`
  # — тих, хто СПРАВДІ свідчив цієї доби, — бо мовчазне дерево не сміє рахуватись
  # свідченням про власне виживання. РОЗМІР спалення знято з лічильника ще
  # [⚖️ 2026-07-30].
  #
  # 🔑 Отже звірка більше НЕ стереже money-path — жоден грошовий вирок цього
  # лічильника вже не питає. Вона лишається чинною на ІНШІЙ підставі: `blackout?`
  # і `skipped?` читають його далі (обидва — питання «чи є взагалі активні
  # дерева», де денормалізація доречна), а дрейф там веде до пропущеної або
  # хибної Field-Audit-повістки. ⛔ Не знімати як «застарілу»: предмет звузився,
  # не зник.
  #
