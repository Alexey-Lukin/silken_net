...
class EcosystemHealingWorker
  ...
  def perform(record_id)
    ...
    ActiveRecord::Base.transaction do
      ...
      if alert&.category_a_evidence?
        # ⛔ [SLASH-1, 2026-09-22] Доказ Кат-A привʼязка ремонту НЕ гасить — незалежно від
        # автора: `positive_a?` читає лише незакритий доказ, а закриває його свідома дія
        # платформи (`06_08 §4.6`), не побічний ефект запису. Навіть `super_admin`, що
        # документує виїзд на tamper, не мусить мимохідь відкликати власний доказ.
        # Звʼязок «запис ↔ тривога» лишається (FK).
        Rails.logger.info "🔒 [Healing] Alert ##{alert.id} — доказ Кат-A: запис ##{record.id} привʼязано, тривогу НЕ закрито."
      elsif alert.present? && !alert.status_resolved?
