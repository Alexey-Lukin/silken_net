...
class DeviceEventWorker
  ...
  # ⛔ [SILENCE-1 / ARCH.109, названо 2026-09-04] ТУТ НЕ КЛИЧУТЬ `tree.mark_seen!`, і це
  # ПРИСУД, а не пропуск — відсутність виглядає дірою рівно тому, що дерево вже
  # резолвлене за DID, тож дописати рядок здається безкоштовним.
  # Підстава — та сама вісь, що зняла людську руку з цього каналу: писати живість має
  # право лише той, хто почув САМЕ ЦЕЙ вузол. Кадр `0x57` цього не доводить: шар 1
  # (Soldier→Queen) — 16B ECB на КЛАСТЕРНОМУ ключі, шар 2 — Ed25519 самої Королеви
  # (`firmware/common/device_event.h`, L1 драбини довіри). Тобто подія свідчить, що
  # ШЛЮЗ щось почув, а не що заговорило це дерево — на відміну від телеметрії, яка
  # автентифікована per-device (session KEYL). Guard нижче цю межу лише звужує до
  # кластера Королеви; per-tree він її не піднімає.
  # 🔴 Ціна помилки одностороння: `mark_seen!` ховає дерево від `Tree.silent`, робить
  # `fresh_signal?` істинним і гасить dead-man switch — тобто рядок тут відтворив би
  # [ARCH.109] машинною рукою замість людської. Дім — `06_08 §1.3`.
