...
module DocsLinter
  ...
  def source_line_ref_drift(path, text)
    ...
    text.each_line.flat_map do |line|
      # 🔴 Стан скидають ЛИШЕ заголовки рівня ≤2. Доти скидав БУДЬ-ЯКИЙ `#`-рядок, і
      # це була прихована вимога «секція-ілюстрація мусить бути ПЛАСКОЮ»: перша ж
      # `###`-підсекція всередині §3 вимикала виняток на весь її хвіст, тобто гейт
      # починав червоніти на власних прикладах діалектів. Спіймано розрізом §3 за
      # предметом (2026-09-08) — і того ж дня пункт `00_07` стверджував, що
      # «`###`-підсекції безпечні», правдиво про `guard_registry_sync` і хибно тут.
      # Виняток є властивістю СЕКЦІЇ, тож і межа його — межа секції, не будь-який
      # заголовок.
      if line.match?(/\A\#{1,2}(?!\#)/)
