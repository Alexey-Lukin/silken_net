...
module CoapGate
  ...
  #
  # 🔴 [FW.63] ЧОГО цей кеш НЕ закриває: втрату самої 2.05. Poll-тракт Королеви
  # ретрансміту НЕ має — `Queen_Poll_Downlink` робить `coap_mid++` на кожну
  # спробу і виходить при порожній відповіді, а `Sim7070_Udp_Fetch` — одна
  # розмова без внутрішніх ретраїв; same-MID retry живе лише в uplink-**PUT**
  # (`COAP_MAX_RETRIES` навколо незмінного `coap_mid`). Загублену 2.05 з CMD
  # рятує не кеш, а ехо-курсор: наказ лишається `:sent` у `.pending` і
  # перевидається наступним poll'ом, доки Королева не відлунить `cmd=<token>`
  # (`Downlink::PendingQueueService#observe_delivered_command!`).
  #
  # 🔴 [FW.63] Тому same-MID сюди приходить ЛИШЕ двома шляхами, і вони мають
  # ПРОТИЛЕЖНУ ціну — тож і мітки метрики в них різні (`poll_duplicate` ⊥
  # `poll_mid_collision`):
