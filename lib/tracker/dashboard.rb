...
DOC_SECTION_REF   = %r{(\d\d_\d\d)(?:_[A-Za-z][A-Za-z0-9_]*)?(?:\.md)?`?\s*((?:§\s*#{DOC_SECTION_TOKEN}[\s,;`+/–—-]*)+)}
    ...
    def self.stale_who(markdown = File.read(DEFAULT_PATH))
      item_residuals(markdown).filter_map do |it|
        ...
        # 🔴 ITEM-WIDE exemption [DOC-T.90, ⚖️ 2026-08-27] — периметр і ПІДСТАВА тут, у
        # ТОЧЦІ ДІЇ. ⚠️ Чесно про провенанс: реєстр `00_06 §3` це вже оголошував
        # («item-wide за побудовою — недетермінований виконавець підкріплює будь-який
        # гліф»), а сюди дійшла лише коротка нота «undetermined executor». Тобто
        # ратифікована підстава існувала й була невидима саме там, де читач вирішує,
        # чи «полагодити» виняток — той самий клас, що §Guard-craft #91 (прочитай
        # оголошену стелю ПЕРШ ніж діагностувати хибні позитиви).
        #
        # ПІДСТАВА (справжня): нога з `🔗`-тегом делегована в інший item, а нога з `🌿` —
        # far-horizon; в обох випадках виконавця ще не призначено, тож meta-гліф є
        # FORWARD-CLAIM («хто закриє, КОЛИ розблокується»), і стандарт це прямо
        # благословляє (`00_05 §3`). Довести, що такий гліф нікого не підпирає,
        # неможливо — майбутнім виконавцем може бути саме він.
        #
        # ПЕРИМЕТР: виняток знімає перевірку з УСЬОГО пункту, не лише з делегованої ноги.
        # Реєстр стверджував «тримають FP на нулі»; 2026-08-27 це ПОМІРЯНО, а не
        # переказано: кандидатів, де leads ≠ ∅ і meta все одно claim'ить гліф поза ними, —
        # жменя; справжніх дефектів серед них НУЛЬ. Майже всі виявились forward-claim'ом
        # на `🔗`-нозі, а один (HW.5.IS) підпертий по-справжньому — просто його лід
        # ДЕКОРАТИВНИЙ (`✨ 🤖`, `🧹 🤖`), тобто стеля `WHO_LEAD` вище, тепер із живим
        # інстансом. Звуження до per-residual червонило б їх усі при точності близько нуля.
        #
        # 🔬 ПЕРЕВИМІР (щоб підстава лишалась досяжною, а не лише вердикт): прожени цей
        # самий метод БЕЗ рядка-винятку нижче й прочитай хіти, розділивши їх на два
        # кошики — `leads` порожній (усі ноги делеговані → forward-claim законний) і
        # `leads` непорожній (єдині справжні кандидати). Другий кошик читається очима
        # менш як за годину; саме він, а не загальне число, є виміром точності звуження.
        #
        # ⛔ Не «полагодь» це, звузивши `any?` до `reject` — форму вже виміряно й
        # відмовлено. Ціна відмови названа: пункт, у якого Є `🔗`-нога і при цьому
        # ПРОТУХЛИЙ гліф на meta, лишається невидимим — і саме так автор цього рядка
        # проґавив власний `🤖` на DOC-T.89.
        #
        # 🔴 ДРУГИЙ прецедент, 2026-09-12 на `HW.1`, і він несе МЕХАНІЗМ, якого перший не
        # мав: ногу з `🔗` і протухлий `🤖` лишила ОДНА Й ТА САМА сесія. Закривши обидві
        # машинні ноги пункту, вона тим же проходом завела `🔗`-ногу («draw radome —
        # відкладено, пускач названо») — і та миттєво ввімкнула item-wide виняток над
        # meta, що саме перестала бути правдивою. Тобто виняток спрацьовує НАЙЧАСТІШЕ в
        # той момент, коли він найменш безпечний: подія, що робить гліф протухлим
        # (остання 🤖-нога закрилась), і подія, що вмикає виняток (зʼявилась делегована
        # нога), — це типово ОДИН прохід по пункту.
        # ⛔ Це НЕ підстава звужувати `any?` — вимір 2026-08-27 лишається чинним. Це
        # підстава для ЧИТАЧА: закривши останню ногу якогось гліфа, перечитай meta
        # РУКАМИ, бо якщо в пункті є `🔗` чи `🌿`, жоден гейт тобі про це не скаже.
